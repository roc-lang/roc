...
**PRECISE FIX LOCATION (final diagnosis, for a dedicated future effort).** The bare `box_of_zst` drops SKIP safely (leak, never mis-free) — so today it is leak-only, NOT corruption. The plan ALREADY unifies fold's direct hidden descriptor with the sibling list (`collectCallHiddenDescriptorArgs` records `source_arg_index=0, rep=Dec`), so the DIRECT hidden descriptor is faithful. Two gaps remain: (1) the box VALUES flowing through fold→callback→nested workers carry the generic worker's bare `box_of_zst` value-descriptor, not fold's faithful hidden Dec — so the drops read the bare one; (2) `erasedCaptureDescriptorRepsForFunctionUse` (lower.zig ~16488) maps the callback closure's `hidden_desc` capture to `.dynamic` because it uses the closure arg's GENERALIZED checked type `arg_types[i] = checked_bodies.expr(closure_arg).ty` (`lowerWorkerCallInto` lower.zig ~5500), where `a` is a flex var. THE FIX: lower the closure argument's worker value (and resolve those captures) against fold's INSTANTIATED param type from `call.source_fn_ty_payload` (param[2] = `(Dec,Dec→Dec)`; see `directCallInstantiationSourceFnType` plan.zig ~3347, `workerCheckedTypeForSource` plan.zig ~3470) instead of the closure's generalized type — flips `capture_desc` dynamic→Dec; then step-2 value-descriptor threading (boundary `assignListRepresentationBoundaryLoopStep` lower.zig ~14385, `sourceValueDescriptorLocalForHiddenArg` `.list`/`.box` deferral extracting `nested_index=0` after `list_get_unsafe`, `templateNestedDescRefsForRep`) propagates Dec to the drops; then `performBoxyBoxDrop` (boxy_runtime ~1277) re-enables the box free automatically (`boxyBoxAllocationPayloadDesc` returns Dec, align 16). RISK: threading the instantiated type changes the closure's layout/boxing at the fold boundary → routes through `assignErasedCallableBoundary` (the erased-callable ABI surface — host_boxed must not regress); a half-threaded version can mis-free (UAF), so it must be landed whole with the full bar green. Fast repro: `zig-out/bin/roc --opt=interpreter test/fx/numeric_fold.roc` → "Sum:15.0" + "21 allocation(s) not freed". Instrumentation points: `[HIDDENARG]` sourceValueDescriptorLocalForHiddenArg, `[CAPMAP]` erasedCaptureDescriptorRepsForFunctionUse, `[BOXDROP-SKIP]` boxy_runtime.zig:1277.
