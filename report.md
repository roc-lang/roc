...
Recommendation leans Option A (single source of truth, no ABI width change, matches boxy's merge-point-consistency principle) — but confirm against the host ABI invariant section and the erased-callable construction the roc-random chain is currently editing before committing. Also-queued dev codegen gaps once this lands: `str_from_utf8` erased-Err-payload (LirCodeGen ~3353), `box_of_zst`/`tag_union` assign_ref invariant (issue_9588), `assign_boxy_adapt` unimplemented.
