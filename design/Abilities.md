...
Putting these together, I can add #[deriving(Serialiable, Deserializable)] to my struct User definition, and then run something like  let user: User = serde_json::from_str(json)? to turn my JSON into a User while handling failed decoding along the way via a Result.
...
I don't know exactly what the API for editor plugins should be yet, but they do have some characteristics that are important:
   ...
   * You can make big, interactive integrations like a regex explorer
