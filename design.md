...
`?` instead contributes its error row to that `Try` result. Success parameters
are related first. Before relating any error rows, the checker collects the
...
integration test cited above. The issue #11470 checker tests additionally reject
...
- `processReturnConstraints` / `collectTryReturnRowUses` /
  ...
  unification establishes a `Try`. A batch structural inventory records nested
