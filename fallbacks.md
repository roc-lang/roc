...
1. Required lookup resolution still uses name matching and runtime error fallback in MIR lowering.
- Problematic state: `e_lookup_required` scans app exports by text (`std.mem.eql`) and unresolved cases emit `.runtime_err_type`.
- Should look like: required lookup is resolved by typed identity before/at lowering; unresolved state is compiler failure (debug panic/unreachable), not runtime fallback.
- References: `src/mir/Lower.zig:1060`, `src/mir/Lower.zig:1073`, `src/mir/Lower.zig:1082`.

2. Dot-access fallback to runtime error in MIR lowering.
- Problematic state: unresolved dot-method dispatch emits `.runtime_err_type`.
- Should look like: no runtime fallback from this stage; unresolved dispatch is an invariant break.
- References: `src/mir/Lower.zig:2169`.

3. Ad-hoc structural equality fallback inside dot-access lowering.
- Problematic state: when nominal dispatch fails for `is_eq`, lowering opportunistically emits `run_low_level(.num_is_eq)`.
- Should look like: structural-eq path is explicit and checker-directed, not opportunistic fallback logic.
- References: `src/mir/Lower.zig:2153`, `src/mir/Lower.zig:2163`.

4. MIR binop lowering still contains fallback-oriented dispatch behavior for unresolved vars.
- Problematic state: unresolved nominal dispatch falls back to low-level numeric ops in lowering path.
- Should look like: dispatch and structural operations decided explicitly upstream, lowering enforces invariants.
- References: `src/mir/Lower.zig:1958`, `src/mir/Lower.zig:1970`.

5. Heuristic multi-env method lookup in type-var dispatch resolution.
- Problematic state: method lookup tries source-env provenance and then origin-env provenance.
- Should look like: checker provides one authoritative resolved target; MIR lowering does no heuristic fallback probing.
- References: `src/mir/Lower.zig:2481`, `src/mir/Lower.zig:2487`.

6. Typed fraction lowering still defaults unknown typed-frac names to `Dec`.
- Problematic state: `e_typed_frac` treats non-`F32`/`F64` names as "Dec or fallback".
- Should look like: frac type mapping is explicit and checker-proven; unknown type names are invariant failure.
- References: `src/mir/Lower.zig:760`, `src/mir/Lower.zig:767`.

7. Monotype conversion still substitutes concrete defaults for unresolved flex/rigid vars.
- Problematic state: `fromTypeVar` maps flex/rigid to `unit` or `dec` based on numeral constraints.
- Should look like: unresolved flex/rigid are resolved upstream or rejected via invariant failure; no silent concrete-type substitution.
- References: `src/mir/Monotype.zig:347`, `src/mir/Monotype.zig:356`.

8. Monotype row-extension traversal truncates on unresolved tails.
- Problematic state: record/tag-union extension walks terminate on non-final structures and on flex/rigid/alias/err tails.
- Should look like: row traversal either fully resolves tail structure or fails as invariant violation; no partial-success truncation.
- References: `src/mir/Monotype.zig:453`, `src/mir/Monotype.zig:455`, `src/mir/Monotype.zig:546`, `src/mir/Monotype.zig:548`.

9. Nominal compatibility seeding currently defaults to permissive success for non-builtin nominals.
- Problematic state: `monotypeCompatibleWithFlatType` returns `true` for all non-builtin nominal types.
- Should look like: compatibility is derived from explicit nominal identity/backing-type rules, not unconditional success.
- References: `src/mir/Lower.zig:2792`, `src/mir/Lower.zig:2793`.

10. Dev backend call-site runtime-error branch fabricates a dummy value.
- Problematic state: `generateLookupCall` `.runtime_error` branch emits crash call and returns `.immediate_i64 = 0`.
- Should look like: runtime-error call path is terminal and never fabricates continuation values.
- References: `src/backend/dev/LirCodeGen.zig:12804`, `src/backend/dev/LirCodeGen.zig:12807`.

11. Dev backend `expect` failure path is not guaranteed non-returning.
- Problematic state: failed `expect` emits `roc_crashed` call and then continues to patch jump and evaluate body.
- Should look like: failure path is terminal (trap/unreachable or ABI-guaranteed noreturn), with no continuation path.
- References: `src/backend/dev/LirCodeGen.zig:10020`, `src/backend/dev/LirCodeGen.zig:10023`, `src/backend/dev/LirCodeGen.zig:10029`.

12. Wasm proc compilation degrades to trap-body and continues.
...
13. Wasm error swallowing in callable pre-binding and closure metadata.
...
14. Wasm optional layout-store semantic fallback.
...
15. Native backend type selection still uses cross-arch fallback aliases.
...
16. Relocation processing ignores some relocation kinds.
...
17. Wasm RC semantics still no-op.
...
18. `Monotype.NominalHint` stores module-aware nominal origin metadata.
...
19. `MIR.Symbol` still leaks encoding details via `isReassignable()`.
...
20. Recursive external-def lowering uses unit-typed placeholder expressions and patches later.
...
21. Tail recursion transform injects placeholder literal arguments for wildcard params.
...
22. Dev codegen callable captures use placeholder in data slot plus side metadata.
...
23. Dev list location keeps legacy compatibility field semantics.
...
24. Calling convention layer still has aarch64 placeholder register model.
...
25. Frame builder hardcodes frame-pointer policy.
...
26. LIR symbol-def registration allows release-mode overwrite.
...
27. MIR symbol-def registration unconditionally overwrites existing entries.
...
28. Wasm lambda-as-value and direct-call closure paths still push dummy placeholders.
...
29. Wasm closure struct-capture layout still computed from ad-hoc repr sizes.
...
30. Dev backend `ll.crash` message forwarding not implemented.
...
31. Dev backend `discriminant_switch` codegen not implemented.
...
32. CallBuilder lacks stack float argument support.
...
33. Register allocator still panics when registers run out (no spilling).
...
34. Wasm list ops not implemented (`list_sort_with`, `list_drop_at`).
...
35. Wasm composite `num_abs` path unimplemented.
...
36. Wasm `hosted_call` expression path is not implemented in expression generator.
