...
1. Calling convention layer still has aarch64 placeholder register model.
...
2. Frame builder hardcodes frame-pointer policy.
...
3. LIR symbol-def registration allows release-mode overwrite.
...
4. MIR symbol-def registration unconditionally overwrites existing entries.
...
5. Wasm lambda-as-value and direct-call closure paths still push dummy placeholders.
...
6. Wasm closure struct-capture layout still computed from ad-hoc repr sizes.
...
7. Dev backend `ll.crash` message forwarding not implemented.
...
8. Dev backend `discriminant_switch` codegen not implemented.
...
9. CallBuilder lacks stack float argument support.
...
10. Register allocator still panics when registers run out (no spilling).
...
11. Wasm list ops not implemented (`list_sort_with`, `list_drop_at`).
...
12. Wasm composite `num_abs` path unimplemented.
...
13. Wasm `hosted_call` expression path is not implemented in expression generator.
