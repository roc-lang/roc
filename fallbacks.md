...
1. Wasm closure struct-capture layout still computed from ad-hoc repr sizes.
...
2. Dev backend `ll.crash` message forwarding not implemented.
...
3. Dev backend `discriminant_switch` codegen not implemented.
...
4. CallBuilder lacks stack float argument support.
...
5. Register allocator still panics when registers run out (no spilling).
...
6. Wasm list ops not implemented (`list_sort_with`, `list_drop_at`).
...
7. Wasm composite `num_abs` path unimplemented.
...
8. Wasm `hosted_call` expression path is not implemented in expression generator.
