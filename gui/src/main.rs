...
use gfx_hal::{
    ...
    window::{PresentMode, Swapchain, SwapchainConfig, Suboptimal},
...
pub const WINDOW_TITLE: &str = "Roc";
...
impl HalState {
    ...
    pub fn new(window: &Window) -> Result<Self, &'static str> {
            ...
            let Gpu { device, mut queues } = unsafe {
                adapter
                    ...
                    .open(&[(&queue_family, &[1.0; 1])],
                        // TODO investigate if we should ask for other features:
                        // https://docs.rs/gfx-hal/0.2.0/gfx_hal/struct.Features.html
                        Features::empty()
                    )
        ...
        let (swapchain, extent, back_images, format, frames_in_flight) = {
            ...
            //
            let (swapchain, back_images) = unsafe {
                device
                    .create_swapchain(&mut surface, swapchain_config, None)
                    .map_err(|_| "Failed to create the swapchain!")?
            };
            (swapchain, extent, back_images, format, image_count as usize)
        };
...
impl UserInput {
    pub fn handle_event(event: &Event) -> Self {
...
fn do_the_render(hal_state: &mut HalState, local_state: &LocalState) -> Result<Option<Suboptimal>, &'static str> {
...
fn main() {
    let mut winit_state = open_window().unwrap();
    ...
    winit_state.events_loop.run_forever(|event| {
        let inputs = UserInput::handle_event(&event);

        if inputs.end_requested {
            winit::ControlFlow::Break
        } else if let Err(e) = do_the_render(&mut hal_state, &local_state) {
