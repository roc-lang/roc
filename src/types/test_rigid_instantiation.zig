...
test "rigid variables need instantiation - polymorphic identity function" {
    ...
    const inst1_var = try instantiate.instantiateVarForTest(&store, func_var, std.testing.allocator);
    ...
    const inst2_var = try instantiate.instantiateVarForTest(&store, func_var, std.testing.allocator);
