...
test "rigid variables need instantiation - multiple type parameters" {
    ...
    const arg_tuple_var = try store.freshFromContent(types.Content{
        .structure = .{ .tuple = .{ .elems = arg_elems_range } },
    });

    const ret_elems_range = try store.appendVars(&.{ rigid_b, rigid_a });
    const ret_tuple_var = try store.freshFromContent(types.Content{
...
test "needsInstantiation" {
    ...
    const str_var = try store.freshFromContent(types.Content{ .structure = .str });
    try std.testing.expect(!store.needsInstantiation(str_var));

    const int_var = try store.freshFromContent(types.Content{ .structure = .{ .num = .{ .int_precision = .u32 } } });
