...
const TestEnv = struct {
    ...
    fn mkStr(self: *Self) std.mem.Allocator.Error!Var {
        // Create backing var (empty tag union to match Str.roc's [Internal])
        const backing_var = try self.module_env.types.freshFromContent(Content{ .structure = .empty_tag_union });
        // Create nominal Str type
        const str_nominal = try self.mkNominalType("Str", backing_var, &[_]Var{});
        // Return a fresh var with the nominal Str type
        return try self.module_env.types.freshFromContent(str_nominal);
    }

    fn mkStrContent(self: *Self) std.mem.Allocator.Error!Content {
        // Create backing var (empty tag union to match Str.roc's [Internal])
        const backing_var = try self.module_env.types.freshFromContent(Content{ .structure = .empty_tag_union });
        // Create and return nominal Str Content
        return try self.mkNominalType("Str", backing_var, &[_]Var{});
    }
...
test "unify - a is builtin and b is flex_var" {
    ...
    const str = try env.mkStrContent();
...
test "unify - flex with constraints unifies with flex with compatible constraints" {
    ...
    const add_fn_a = try env.module_env.types.freshFromContent(try env.mkFuncPure(&[_]Var{int}, int));
    ...
    const add_fn_b = try env.module_env.types.freshFromContent(try env.mkFuncPure(&[_]Var{int}, int));
...
test "unify - flex with multiple constraints" {
    ...
    const to_str_fn = try env.module_env.types.freshFromContent(try env.mkFuncPure(&[_]Var{int}, str));
...
test "unify - flex with constraints fails on incompatible arg types" {
    ...
    const foo_fn_a = try env.module_env.types.freshFromContent(try env.mkFuncPure(&[_]Var{str}, str));
    ...
    const foo_fn_b = try env.module_env.types.freshFromContent(try env.mkFuncPure(&[_]Var{int}, str));
...
test "unify - flex with constraints fails on incompatible return types" {
    ...
    const foo_fn_b = try env.module_env.types.freshFromContent(try env.mkFuncPure(&[_]Var{str}, int));
    const foo_constraint_b = types_mod.StaticDispatchConstraint{
        .fn_name = try env.module_env.getIdentStore().insert(env.module_env.gpa, Ident.for_text("foo")),
        .fn_var = foo_fn_b,
    };
...
test "unify - flex with constraints fails on different arity" {
    ...
    const foo_fn_b = try env.module_env.types.freshFromContent(try env.mkFuncPure(&[_]Var{ str, int }, str));
    const foo_constraint_b = types_mod.StaticDispatchConstraint{
        .fn_name = try env.module_env.getIdentStore().insert(env.module_env.gpa, Ident.for_text("foo")),
        .fn_var = foo_fn_b,
    };
...
test "unify - flex with constraints vs structure captures deferred check" {
    ...
    const structure_var = try env.module_env.types.freshFromContent(Content{ .structure = .empty_tag_union });
