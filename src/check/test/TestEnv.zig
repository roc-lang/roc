...
pub fn assertOneTypeError(self: *TestEnv, expected: []const u8) !void {
    ...
    var report_builder = report_mod.ReportBuilder.init(
        self.gpa,
        self.module_env,
        self.module_env,
...
fn assertNoTypeProblems(self: *TestEnv) !void {
    var report_builder = report_mod.ReportBuilder.init(self.gpa, self.module_env, self.module_env, &self.checker.snapshots, &self.checker.problems, "test", &.{}, &self.checker.import_mapping);
    defer report_builder.deinit();
