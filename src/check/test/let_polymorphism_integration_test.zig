...
test "direct polymorphic identity usage" {
    const source =
        ...
        \\}
    ;
    try typeCheck(source, "{ a: _field, b: Str } where [_c.from_num_literal : _arg -> _ret]");
}
...
test "let-polymorphism with function composition" {
    const source =
        ...
        \\}
    ;
    try typeCheck(source, "_a where [_b.from_num_literal : _arg -> _ret]");
}
...
test "polymorphic empty list" {
    const source =
        ...
        \\}
    ;
    try typeCheck(source, "{ empty: List(_a), nums: List(_b), strs: List(Str) } where [_c.from_num_literal : _arg -> _ret]");
}
...
test "polymorphic record constructor" {
    const source =
        ...
        \\}
    ;
    try typeCheck(source, "{ pair1: { first: _field, second: Str }, pair2: { first: Str, second: _field2 }, pair3: { first: [True]_others, second: [False]_others2 } } where [_a.from_num_literal : _arg -> _ret, _b.from_num_literal : _arg2 -> _ret2]");
}
...
test "polymorphic identity with various numeric types" {
    const source =
        ...
        \\}
    ;
    try typeCheck(source, "{ bool_val: [True]_others, float_val: _field, int_val: _field2 } where [_a.from_num_literal : _arg -> _ret, _b.from_num_literal : _arg2 -> _ret2]");
}
...
test "nested polymorphic data structures" {
    const source =
        ...
        \\}
    ;
    try typeCheck(source, "{ box1: { value: _field }, box2: { value: Str }, nested: { value: { value: _field2 } } } where [_a.from_num_literal : _arg -> _ret, _b.from_num_literal : _arg2 -> _ret2]");
}
...
test "polymorphic option type simulation" {
    const source =
        ...
        \\}
    ;
    try typeCheck(source, "{ opt1: { tag: Str, value: _field }, opt2: { tag: Str, value: Str }, opt3: { tag: Str } } where [_a.from_num_literal : _arg -> _ret]");
}
...
test "polymorphic pipe function" {
    const source =
        ...
        \\}
    ;
    try typeCheck(source, "{ num_result: _field, str_result: _field2 } where [_a.from_num_literal : _arg -> _ret, _b.from_num_literal : _arg2 -> _ret2]");
}
