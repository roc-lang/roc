...
test "integer literal 255 fits in U8" {
    ...
    var problems = try @import("../check_types/problem.zig").Store.initCapacity(gpa, 16);
    defer problems.deinit(gpa);

    var snapshots = try @import("../check_types/snapshot.zig").Store.initCapacity(gpa, 16);
    defer snapshots.deinit();

    var scratch = try unify.Scratch.init(gpa);
    defer scratch.deinit();

    var occurs_scratch = try @import("../check_types/occurs.zig").Scratch.init(gpa);
