...
test "let-polymorphism with empty list" {
    ...
    const empty_list_var = env.store.freshFromContent(empty_list_content);
    ...
    const int_list_var = try instantiate.instantiateVar(env.store, env.regions, test_allocator, empty_list_var);

    // Second usage: instantiate for strings
    const str_list_var = try instantiate.instantiateVar(env.store, env.regions, test_allocator, empty_list_var);
...
test "let-polymorphism with numeric literal" {
    ...
    const num_var = env.store.freshFromContent(num_content);

    // Verify it needs instantiation
    try testing.expect(env.store.needsInstantiation(num_var));

    // First usage: instantiate as I32
    const i32_instance = try instantiate.instantiateVar(env.store, env.regions, test_allocator, num_var);

    // Second usage: instantiate as F64
    const f64_instance = try instantiate.instantiateVar(env.store, env.regions, test_allocator, num_var);
...
test "let-polymorphism with polymorphic function" {
    ...
    const func_content = env.store.mkFuncPure(&.{type_param}, type_param);
    const func_var = env.store.freshFromContent(func_content);

    // Verify it needs instantiation
    try testing.expect(env.store.needsInstantiation(func_var));

    // First usage: instantiate for use with strings
    const str_func = try instantiate.instantiateVar(env.store, env.regions, test_allocator, func_var);

    // Second usage: instantiate for use with numbers
    const num_func = try instantiate.instantiateVar(env.store, env.regions, test_allocator, func_var);
...
test "let-polymorphism with nested structures" {
    ...
    const list_var = env.store.freshFromContent(list_content);

    const u64_content = types.Content{ .structure = .{ .num = .{ .int_precision = .u64 } } };
    const u64_var = env.store.freshFromContent(u64_content);

    // Create record fields
    var fields = std.ArrayList(types.RecordField).init(test_allocator);
    defer fields.deinit();

    const data_field_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("data"), base.Region.zero());
    const count_field_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("count"), base.Region.zero());

    try fields.append(.{ .name = data_field_name, .var_ = list_var });
    try fields.append(.{ .name = count_field_name, .var_ = u64_var });

    const fields_range = env.store.appendRecordFields(fields.items);
    const record_content = types.Content{ .structure = .{ .record = .{ .fields = fields_range, .ext = env.store.fresh() } } };
    const record_var = env.store.freshFromContent(record_content);

    // Verify it needs instantiation (because of the polymorphic list)
    try testing.expect(env.store.needsInstantiation(record_var));

    // First usage: instantiate for integers
    const int_record = try instantiate.instantiateVar(env.store, env.regions, test_allocator, record_var);

    // Second usage: instantiate for booleans
    const bool_record = try instantiate.instantiateVar(env.store, env.regions, test_allocator, record_var);
...
test "let-polymorphism prevents over-generalization" {
    ...
    const i32_var = env.store.freshFromContent(i32_content);

    const list_i32_content = types.Content{ .structure = .{ .list = i32_var } };
    const list_i32_var = env.store.freshFromContent(list_i32_content);
...
test "let-polymorphism with multiple type parameters" {
    ...
    const input_elems = env.store.appendTupleElems(&.{ type_a, type_b });
    const input_tuple_content = types.Content{ .structure = .{ .tuple = .{ .elems = input_elems } } };
    const input_tuple_var = env.store.freshFromContent(input_tuple_content);

    // Create output tuple (b, a)
    const output_elems = env.store.appendTupleElems(&.{ type_b, type_a });
    const output_tuple_content = types.Content{ .structure = .{ .tuple = .{ .elems = output_elems } } };
    const output_tuple_var = env.store.freshFromContent(output_tuple_content);

    // Create the function type
    const func_content = env.store.mkFuncPure(&.{input_tuple_var}, output_tuple_var);
    const func_var = env.store.freshFromContent(func_content);
    ...
    const inst1 = try instantiate.instantiateVar(env.store, env.regions, test_allocator, func_var);
    const inst2 = try instantiate.instantiateVar(env.store, env.regions, test_allocator, func_var);
    const inst3 = try instantiate.instantiateVar(env.store, env.regions, test_allocator, func_var);
...
test "let-polymorphism with constrained type variables" {
    ...
    const add_func_content = env.store.mkFuncPure(&.{ num_var, num_var }, num_var);
    const add_func_var = env.store.freshFromContent(add_func_content);

    // Verify it needs instantiation
    try testing.expect(env.store.needsInstantiation(add_func_var));

    // Instantiate for different numeric types
    const int_add = try instantiate.instantiateVar(env.store, env.regions, test_allocator, add_func_var);
    const float_add = try instantiate.instantiateVar(env.store, env.regions, test_allocator, add_func_var);
...
test "let-polymorphism with simple tag union" {
    ...
    const some_tag_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("Some"), base.Region.zero());
    const some_args = env.store.appendTagArgs(&.{type_param});

    // Create the None tag with no arguments
    const none_tag_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("None"), base.Region.zero());
    // For a tag with no arguments, we use an empty slice
    const none_args = env.store.appendTagArgs(&.{});
    ...
    const tags_range = env.store.appendTags(tags.items);
    const tag_union_content = types.Content{ .structure = .{ .tag_union = .{ .tags = tags_range, .ext = env.store.fresh() } } };
    const option_var = env.store.freshFromContent(tag_union_content);

    // Verify it needs instantiation
    try testing.expect(env.store.needsInstantiation(option_var));

    // Instantiate for different element types
    const string_option = try instantiate.instantiateVar(env.store, env.regions, test_allocator, option_var);
    const number_option = try instantiate.instantiateVar(env.store, env.regions, test_allocator, option_var);
...
test "let-polymorphism interaction with pattern matching" {
    ...
    const maybe_var = env.store.fresh();

    // Create tags
    const just_tag_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("Just"), base.Region.zero());
    const nothing_tag_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("Nothing"), base.Region.zero());

    var tags = std.ArrayList(types.Tag).init(test_allocator);
    defer tags.deinit();

    try tags.append(.{ .name = just_tag_name, .args = env.store.appendTagArgs(&.{type_param}) });
    try tags.append(.{ .name = nothing_tag_name, .args = types.Var.SafeList.Range.empty });

    const tags_range = env.store.appendTags(tags.items);
    const maybe_content = types.Content{ .structure = .{ .tag_union = .{ .tags = tags_range, .ext = env.store.fresh() } } };
    try env.store.setVarContent(maybe_var, maybe_content);

    // Create a function that pattern matches on Maybe: forall a. Maybe a -> Bool
    const bool_content = types.Content{ .structure = .{ .tag_union = .{ .tags = env.store.appendTags(&.{
        .{ .name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("True"), base.Region.zero()), .args = types.Var.SafeList.Range.empty },
        .{ .name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("False"), base.Region.zero()), .args = types.Var.SafeList.Range.empty },
    }), .ext = env.store.fresh() } } };
    const bool_var = env.store.freshFromContent(bool_content);

    const is_just_func_content = env.store.mkFuncPure(&.{maybe_var}, bool_var);
    const is_just_func_var = env.store.freshFromContent(is_just_func_content);

    // Verify it needs instantiation
    try testing.expect(env.store.needsInstantiation(is_just_func_var));

    // Instantiate for different types
    const str_is_just = try instantiate.instantiateVar(env.store, env.regions, test_allocator, is_just_func_var);
    const int_is_just = try instantiate.instantiateVar(env.store, env.regions, test_allocator, is_just_func_var);
...
test "let-polymorphism preserves sharing within single instantiation" {
    ...
    // Create tuple (a, a)
    const pair_elems = env.store.appendTupleElems(&.{ type_param, type_param });
    const pair_content = types.Content{ .structure = .{ .tuple = .{ .elems = pair_elems } } };
    const pair_var = env.store.freshFromContent(pair_content);

    // Create record
    var fields = std.ArrayList(types.RecordField).init(test_allocator);
    defer fields.deinit();

    const first_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("first"), base.Region.zero());
    const second_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("second"), base.Region.zero());
    const pair_name = env.module_env.idents.insert(test_allocator, base.Ident.for_text("pair"), base.Region.zero());
    ...
    // Instantiate once
    const inst = try instantiate.instantiateVar(env.store, env.regions, test_allocator, record_var);
    ...
    const inst2 = try instantiate.instantiateVar(env.store, env.regions, test_allocator, record_var);
