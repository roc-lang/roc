...
/// Check if a type variable contains any flexible type variables that need instantiation.
/// Returns true if the type contains flex vars, false if it's fully concrete.
pub fn needsInstantiation(store: *const TypesStore, var_: Var) bool {
    const resolved = store.resolveVar(var_);
    return needsInstantiationContent(store, resolved.desc.content);
}
...
fn needsInstantiationFlatType(store: *const TypesStore, flat_type: FlatType) bool {
    return switch (flat_type) {
        .str => false,
        .box => |box_var| needsInstantiation(store, box_var),
        .list => |list_var| needsInstantiation(store, list_var),
        .list_unbound => false,
        .tuple => |tuple| blk: {
            const elems_slice = store.getTupleElemsSlice(tuple.elems);
            for (elems_slice) |elem_var| {
                if (needsInstantiation(store, elem_var)) return true;
            }
            break :blk false;
        },
        .num => |num| switch (num) {
            .num_poly => |poly| needsInstantiation(store, poly.var_),
            .int_poly => |poly| needsInstantiation(store, poly.var_),
            .frac_poly => |poly| needsInstantiation(store, poly.var_),
...
fn needsInstantiationRecord(store: *const TypesStore, record: Record) bool {
    ...
    for (fields_slice.items(.var_)) |type_var| {
        if (needsInstantiation(store, type_var)) return true;
    }
    return needsInstantiation(store, record.ext);
}

fn needsInstantiationRecordFields(store: *const TypesStore, fields: RecordField.SafeMultiList.Range) bool {
...
fn needsInstantiationTagUnion(store: *const TypesStore, tag_union: TagUnion) bool {
    ...
    return needsInstantiation(store, tag_union.ext);
}
