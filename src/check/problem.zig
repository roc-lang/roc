...
/// Problem data for when a match expression may not be exhaustive.
/// This occurs when the scrutinee type is an open tag union (can have arbitrary tags)
/// and there's no catch-all pattern to handle unknown tags.
