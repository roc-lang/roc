...
pub fn checkPattern(self: *Self, pattern_idx: ModuleEnv.Pattern.Idx) std.mem.Allocator.Error!void {
    ...
    switch (pattern) {
        .nominal => |nominal| {
            ...
            // 3. Next, unify that instantiated var against the ModuleEnv backing var
            ...
            // Then, unify the nominal type's backing var against the ModuleEnv backing var
    ...
    switch (expr) {
        ...
        .e_call => |call| {
            ...
            if (func_expr != .e_runtime_error) {
                ...
                //
                // TODO: Do we need to insert a ModuleEnv placeholder node here as well?
                // What happens if later this type variable has a problem, and we
                // try to look up its region in ModuleEnv?
        ...
        .e_dot_access => |dot_access| {
            ...
            switch (resolved_receiver.desc.content) {
                ...
                .flex_var => {
                    ...
                    // TODO: Do we need to insert a ModuleEnv placeholder node here as well?
                    // What happens if later this type variable has a problem, and we
                    // try to look up it's region in ModuleEnv?
...
fn checkBinopExpr(self: *Self, expr_idx: ModuleEnv.Expr.Idx, expr_region: Region, binop: ModuleEnv.Expr.Binop) Allocator.Error!bool {
    ...
    switch (binop.op) {
        ...
        .lt, .gt, .le, .ge, .eq, .ne => {
            ...
            const expr_var = @as(Var, @enumFromInt(@intFromEnum(expr_idx)));
            const fresh_bool = try self.instantiateVar(ModuleEnv.varFrom(can.BUILTIN_BOOL), .{ .explicit = expr_region });
            _ = try self.unify(expr_var, fresh_bool);
        },
        .@"and" => {
            const lhs_fresh_bool = try self.instantiateVar(ModuleEnv.varFrom(can.BUILTIN_BOOL), .{ .explicit = expr_region });
            ...
            if (lhs_result.isOk()) {
                const rhs_fresh_bool = try self.instantiateVar(ModuleEnv.varFrom(can.BUILTIN_BOOL), .{ .explicit = expr_region });
...
fn checkIfElseExpr(
    ...
    const bool_var = try self.instantiateVar(ModuleEnv.varFrom(can.BUILTIN_BOOL), .{ .explicit = expr_region });
    ...
    for (branches[1..], 1..) |branch_idx, cur_index| {
        ...
        const branch_bool_var = try self.instantiateVar(ModuleEnv.varFrom(can.BUILTIN_BOOL), .{ .explicit = expr_region });
...
test "lambda with record field access infers correct type" {
    ...
    var cir = try ModuleEnv.init(gpa, "Test");
    defer cir.deinit();
