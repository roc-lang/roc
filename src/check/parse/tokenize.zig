...
pub const TokenizedBuffer = struct {
    ...
    pub fn initCapacity(env: *compile.ModuleEnv, capacity: usize) std.mem.Allocator.Error!TokenizedBuffer {
...
pub const Tokenizer = struct {
    ...
    pub fn init(env: *compile.ModuleEnv, text: []const u8, messages: []Diagnostic) std.mem.Allocator.Error!Tokenizer {
...
fn testTokenization(gpa: std.mem.Allocator, input: []const u8, expected: []const Token.Tag) !void {
    ...
    var env = try compile.ModuleEnv.init(gpa, try gpa.dupe(u8, ""));
    defer env.deinit();
...
pub fn checkTokenizerInvariants(gpa: std.mem.Allocator, input: []const u8, debug: bool) std.mem.Allocator.Error!void {
    var env = try compile.ModuleEnv.init(gpa, gpa.dupe(u8, "") catch unreachable);
    defer env.deinit();
