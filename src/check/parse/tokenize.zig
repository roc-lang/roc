...
pub const Diagnostic = struct {
    ...
    pub fn toStr(self: Diagnostic, gpa: Allocator, source: []const u8, writer: anytype) !void {
        ...
        const info = getDiagnosticPositionInfo(source, newlines, self.begin, self.end);
...
fn getDiagnosticPositionInfo(source: []const u8, newlines: std.ArrayList(usize), begin: u32, end: u32) struct { start_line: u32, start_col: u32, end_line: u32, end_col: u32, line_text: []const u8 } {
...
test "getDiagnosticPositionInfo" {
    const gpa = std.testing.allocator;
    var newlines = std.ArrayList(usize).init(gpa);
    defer newlines.deinit();
    ...
    const info1 = getDiagnosticPositionInfo(source, newlines, 2, 4); // "ne" in line0
    ...
    const info2 = getDiagnosticPositionInfo(source, newlines, 8, 10); // "ne" in line1
