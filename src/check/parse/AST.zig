...
pub fn appendRegionInfoToSexprTree(self: *AST, env: *compile.ModuleEnv, tree: *SExprTree, region: TokenizedRegion) std.mem.Allocator.Error!void {
    const start = self.tokens.resolve(region.start);
...
pub fn parseDiagnosticToReport(self: *AST, env: *compile.ModuleEnv, diagnostic: Diagnostic, allocator: std.mem.Allocator, filename: []const u8) !reporting.Report {
...
pub fn toSExprStr(ast: *@This(), env: *compile.ModuleEnv, writer: std.io.AnyWriter) !void {
...
pub fn toSExprHtml(ast: *@This(), env: *compile.ModuleEnv, writer: std.io.AnyWriter) !void {
    const file = ast.store.getFile();
...
pub const Statement = union(enum) {
    ...
    pub fn pushToSExprTree(self: @This(), env: *compile.ModuleEnv, ast: *AST, tree: *SExprTree) std.mem.Allocator.Error!void {
        switch (self) {
            .decl => |decl| {
...
pub const BinOp = struct {
    ...
    pub fn pushToSExprTree(self: *const @This(), env: *compile.ModuleEnv, ast: *AST, tree: *SExprTree) std.mem.Allocator.Error!void {
        const begin = tree.beginNode();
