...
pub const NodeStore = struct {
    ...
    pub fn addScratchPatternRecordField(store: *NodeStore, idx: PatternRecordFieldIdx) void {
        store.scratch_pattern_record_fields.append(idx) catch exitOnOom();
    }
    ...
    pub fn patternRecordFieldSpanFrom(store: *NodeStore, start: u32) PatternRecordFieldSpan {
        ...
        const ed_start = @as(u32, @intCast(store.extra_data.items.len));
        while (i < end) {
            store.extra_data.append(store.scratch_pattern_record_fields.items[i].id) catch exitOnOom();
            i += 1;
        }
        return .{ .span = .{ .start = ed_start, .len = @as(u32, @intCast(end)) - start } };
    }
    ...
    pub fn addScratchRecordField(store: *NodeStore, idx: RecordFieldIdx) void {
        store.scratch_record_fields.append(idx) catch exitOnOom();
    }
    ...
    pub fn recordFieldSpanFrom(store: *NodeStore, start: u32) RecordFieldSpan {
        ...
        const ed_start = @as(u32, @intCast(store.extra_data.items.len));
        while (i < end) {
            store.extra_data.append(store.scratch_record_fields.items[i].id) catch exitOnOom();
            i += 1;
        }
        return .{ .span = .{ .start = ed_start, .len = @as(u32, @intCast(end)) - start } };
    }
    ...
    pub fn addScratchWhenBranch(store: *NodeStore, idx: WhenBranchIdx) void {
        store.scratch_when_branches.append(idx) catch exitOnOom();
    }
    ...
    pub fn whenBranchSpanFrom(store: *NodeStore, start: u32) WhenBranchSpan {
        ...
        const ed_start = @as(u32, @intCast(store.extra_data.items.len));
        while (i < end) {
            store.extra_data.append(store.scratch_when_branches.items[i].id) catch exitOnOom();
            i += 1;
        }
        return .{ .span = .{ .start = ed_start, .len = @as(u32, @intCast(end)) - start } };
    }
    ...
    pub fn addScratchTypeAnno(store: *NodeStore, idx: TypeAnnoIdx) void {
        store.scratch_type_annos.append(idx) catch exitOnOom();
    }
    ...
    pub fn typeAnnoSpanFrom(store: *NodeStore, start: u32) TypeAnnoSpan {
        ...
        const ed_start = @as(u32, @intCast(store.extra_data.items.len));
        while (i < end) {
            store.extra_data.append(store.scratch_type_annos.items[i].id) catch exitOnOom();
            i += 1;
        }
        return .{ .span = .{ .start = ed_start, .len = @as(u32, @intCast(end)) - start } };
    }
    ...
    pub fn addScratchAnnoRecordField(store: *NodeStore, idx: AnnoRecordFieldIdx) void {
        store.scratch_anno_record_fields.append(idx) catch exitOnOom();
    }
    ...
    pub fn annoRecordFieldSpanFrom(store: *NodeStore, start: u32) AnnoRecordFieldSpan {
        ...
        const ed_start = @as(u32, @intCast(store.extra_data.items.len));
        while (i < end) {
            store.extra_data.append(store.scratch_anno_record_fields.items[i].id) catch exitOnOom();
            i += 1;
        }
        return .{ .span = .{ .start = ed_start, .len = @as(u32, @intCast(end)) - start } };
    }
    ...
    pub fn addScratchToken(store: *NodeStore, idx: TokenIdx) void {
        store.scratch_tokens.append(idx) catch exitOnOom();
    }
    ...
    pub fn tokenSpanFrom(store: *NodeStore, start: u32) TokenSpan {
        ...
        const ed_start = @as(u32, @intCast(store.extra_data.items.len));
        while (i < end) {
            store.extra_data.append(store.scratch_tokens.items[i]) catch exitOnOom();
            i += 1;
        }
        return .{ .span = .{ .start = ed_start, .len = @as(u32, @intCast(end)) - start } };
    }
