    ...
    return struct {
        ...
        fn unifyStructure(
            ...
            switch (b_content) {
                .flex_var => |_| {
                    ...
                    if (@intFromEnum(vars.a.var_) == 97 or @intFromEnum(vars.b.var_) == 86 or @intFromEnum(vars.b.var_) == 99) {
                        std.debug.print("    unifyStructure: merging vars.a=Var({}) with vars.b=Var({}) to structure\n", .{
                            @intFromEnum(vars.a.var_),
                            @intFromEnum(vars.b.var_),
                        });
                    }
                    ...
                    switch (b_flat_type) {
                        ...
                        .record_unbound => |b_fields| {
                            ...
                            if (partitioned.only_in_a.len() > 0 or partitioned.only_in_b.len() > 0) {
                                std.debug.print("Unifying record_unbound: {} shared, {} only_in_a, {} only_in_b\n", .{
                                    partitioned.in_both.len(),
                                    partitioned.only_in_a.len(),
                                    partitioned.only_in_b.len(),
                                });
                            }

                            // For record_unbound, we allow extension - one record can have more fields than the other
                            // The result should have all fields from both records

                            // Unify shared fields
                            const dummy_ext = self.fresh(vars, .{ .structure = .empty_record }) catch return Error.AllocatorError;

                            // Get slices for fields only in a and only in b
                            const a_only_fields = self.scratch.only_in_a_fields.sliceRange(partitioned.only_in_a);
                            const b_only_fields = self.scratch.only_in_b_fields.sliceRange(partitioned.only_in_b);

                            try self.unifySharedFields(
                                vars,
                                self.scratch.in_both_fields.sliceRange(partitioned.in_both),
                                if (partitioned.only_in_a.len() > 0) a_only_fields else null,
                                if (partitioned.only_in_b.len() > 0) b_only_fields else null,
                                dummy_ext,
                            );

                            // The merged type is record_unbound with all fields from both records
                            // (unifySharedFields already created the merged record)
                        },
                        .record_poly => |b_poly| {
        ...
        fn unifySharedFields(
            ...
            for (shared_fields) |shared| {
                // Debug field unification
                if (@intFromEnum(shared.a.var_) == 98 or @intFromEnum(shared.b.var_) == 98 or
                    @intFromEnum(shared.a.var_) == 85 or @intFromEnum(shared.b.var_) == 85) {
                    const a_resolved = self.types_store.resolveVar(shared.a.var_);
                    const b_resolved = self.types_store.resolveVar(shared.b.var_);
                    std.debug.print("    Unifying field: Var({}) ({s}) with Var({}) ({s})\n", .{
                        @intFromEnum(shared.a.var_),
                        @tagName(a_resolved.desc.content),
                        @intFromEnum(shared.b.var_),
                        @tagName(b_resolved.desc.content),
                    });
                }

                try self.unifyGuarded(shared.a.var_, shared.b.var_);

                // Debug after unification
                if (@intFromEnum(shared.a.var_) == 98 or @intFromEnum(shared.b.var_) == 98) {
                    const a_resolved_after = self.types_store.resolveVar(shared.a.var_);
                    const b_resolved_after = self.types_store.resolveVar(shared.b.var_);
                    std.debug.print("      After field unify: Var({}) -> {s}, Var({}) -> {s}\n", .{
                        @intFromEnum(shared.a.var_),
                        @tagName(a_resolved_after.desc.content),
                        @intFromEnum(shared.b.var_),
                        @tagName(b_resolved_after.desc.content),
                    });
                }
