...
const Lowerer = struct {
    ...
    fn requestedConcreteSpecializationReturnType(
        self: *const Lowerer,
        expected_exec_ty: ?type_mod.TypeId,
        comptime context: []const u8,
    ) ?type_mod.TypeId {
        const ret_ty = expected_exec_ty orelse return null;
        if (self.executableTypeIsAbstract(ret_ty)) return null;
        const concrete = self.requireConcreteExecutableType(ret_ty, context);
        if (self.executableTypeContainsNestedCallable(concrete)) return null;
        return concrete;
    }
    ...
    fn executableTypeContainsNestedCallable(self: *const Lowerer, ty: type_mod.TypeId) bool {
        var visited = std.AutoHashMap(type_mod.TypeId, void).init(self.allocator);
        defer visited.deinit();
        return self.executableTypeContainsNestedCallableVisited(ty, true, &visited) catch true;
    }

    fn executableTypeContainsNestedCallableVisited(
        self: *const Lowerer,
        ty: type_mod.TypeId,
        at_root: bool,
        ...
        return switch (self.types.types.items[@intFromEnum(root)]) {
            .placeholder, .unbd, .link, .primitive => false,
            .nominal => |nominal| try self.executableTypeContainsNestedCallableVisited(nominal.backing, at_root, visited),
            .erased_fn => !at_root,
            .list => |elem| try self.executableTypeContainsNestedCallableVisited(elem, false, visited),
            .box => |elem| try self.executableTypeContainsNestedCallableVisited(elem, false, visited),
            .tuple => |elems| blk: {
                for (elems) |elem| {
                    if (try self.executableTypeContainsNestedCallableVisited(elem, false, visited)) {
                        break :blk true;
                    }
                }
                break :blk false;
            ...
            .record => |record| blk: {
                for (record.fields) |field| {
                    if (try self.executableTypeContainsNestedCallableVisited(field.ty, false, visited)) {
                        break :blk true;
                    }
                }
                break :blk false;
            ...
            .tag_union => |tag_union| blk: {
                if (!at_root and tag_union.call != null) {
                    break :blk true;
                }
                for (tag_union.tags) |tag| {
                    for (tag.args) |arg| {
                        if (try self.executableTypeContainsNestedCallableVisited(arg, false, visited)) {
                            break :blk true;
                        }
                    }
                }
                if (tag_union.call) |call| {
                    ...
                    if (try self.executableTypeContainsNestedCallableVisited(call.ret, false, visited)) {
                        break :blk true;
                    }
                }
                break :blk false;
    ...
    fn lowerRequestedExecutableArgTypeFromExpr(
        ...
        if (repr_mode == .natural) {
            if (try self.concreteExecutableTypeFromExprShape(inst, mono_cache, venv, expr_id, source_ty)) |concrete| {
                if (!self.executableTypeIsAbstract(lowered)) {
                    return try self.strengthenErasedCallableExpectedTypeFromCurrent(
                        concrete,
                        self.requireConcreteExecutableType(
                            lowered,
                            "lowerRequestedExecutableArgTypeFromExpr(source concrete)",
                        ),
                    );
                }
                return concrete;
        ...
        const defaulted = switch (expr.data) {
            .int_lit, .dec_lit => try self.makePrimitiveType(.dec),
            else => lowered,
        };
        if (!self.executableTypeIsAbstract(defaulted)) return defaulted;
        if (self.isExecutableCallableType(defaulted)) return defaulted;
        return switch (repr_mode) {
            .natural => defaulted,
            .erased_boundary => self.requireConcreteExecutableType(
                try self.lowerErasedBoundaryExecutableTypeIn(
                    inst.types,
                    mono_cache,
                    source_ty,
                ),
                "lowerRequestedExecutableArgTypeFromExpr(erased boundary)",
            ),
        };
    }

    fn concreteExecutableTypeFromExprShape(
        self: *Lowerer,
        inst: *InstScope,
        mono_cache: *lower_type.MonoCache,
        venv: []const EnvEntry,
        ...
        const lowered = try self.lowerExecutableTypeFromSolvedIn(inst.types, mono_cache, source_ty);
        if (!self.executableTypeIsAbstract(lowered) and
            !self.isExecutableCallableType(lowered) and
            !self.executableTypeContainsNestedCallable(lowered))
        {
            return self.requireConcreteExecutableType(lowered, "concreteExecutableTypeFromExprShape(lowered)");
        }

        const expr = self.input.store.getExpr(expr_id);
        return switch (expr.data) {
            .tag => |tag| blk: {
                const tag_info = self.solvedTagInfoByName(inst.types, source_ty, tag.name) orelse break :blk null;
                const tag_args = self.input.store.sliceExprSpan(tag.args);
                if (tag_info.args.len != tag_args.len) {
                    debugPanic("lambdamono.lower.concreteExecutableTypeFromExprShape tag arg count mismatch");
                }
                const arg_tys = try self.allocator.alloc(type_mod.TypeId, tag_args.len);
                defer self.allocator.free(arg_tys);
                for (tag_args, tag_info.args, 0..) |arg_expr_id, arg_source_ty, i| {
                    arg_tys[i] = (try self.concreteExecutableTypeFromExprShape(
                        inst,
                        mono_cache,
                        venv,
                        arg_expr_id,
                ...
                break :blk try self.synthesizedSingletonTagTypeFromArgTypes(tag.name, arg_tys);
            },
            .list => |items_span| blk: {
                const elem_source_ty = self.solvedListElemType(inst.types, source_ty) orelse break :blk null;
                const items = self.input.store.sliceExprSpan(items_span);
                if (items.len == 0) {
                    if (self.listElemType(lowered) == null) {
                        break :blk null;
                    }
                    break :blk try self.concretizeEmptyValueExecutableType(lowered);
                }
                var elem_exec_ty: ?type_mod.TypeId = null;
                for (items) |item_expr_id| {
                    const next = (try self.concreteExecutableTypeFromExprShape(
                        inst,
                        mono_cache,
                        venv,
                        item_expr_id,
                    ...
                    if (elem_exec_ty) |existing| {
                        if (!self.types.equalIds(existing, next)) break :blk null;
                    } else {
                        elem_exec_ty = next;
                    }
                }
                break :blk try self.synthesizedListTypeFromElem(elem_exec_ty orelse break :blk null);
            },
            .call => |call| blk: {
                ...
                if (self.boxBoundaryBuiltinOpForExpr(venv, base_func_source)) |op| {
                    if (arg_expr_ids.len != 1) break :blk null;
                    const arg_source_ty = if (op == .box_box)
                        self.solvedBoxElemType(inst.types, source_ty) orelse break :blk null
                    else
                        try self.instantiatedSourceTypeForExpr(inst, venv, arg_expr_ids[0]);
                    const arg_exec_ty = (try self.concreteExecutableTypeFromExprShape(
                        inst,
                        mono_cache,
                        venv,
                        arg_expr_ids[0],
                    ...
                    const boxed_result_ty = try self.boxBoundaryResultTypeFromArg(op, arg_exec_ty);
                    break :blk boxed_result_ty;
                }
                break :blk try self.concreteExecutableTypeFromDirectExactCallExpr(
                    inst,
                    mono_cache,
                    venv,
                    expr_id,
    ...
    fn concreteExecutableTypeFromDirectExactCallExpr(
        self: *Lowerer,
        inst: *InstScope,
        mono_cache: *lower_type.MonoCache,
        venv: []const EnvEntry,
        ...
        const base_func_source = self.input.store.getExpr(call.func);
        const exact_symbol = self.directCallableSymbolFromExpr(venv, base_func_source) orelse return null;
        if (specializations.lookupFnExact(self.fenv, exact_symbol) == null and !self.isHostedTopLevelSymbol(exact_symbol)) {
            return null;
        }
        ...
        const explicit_call_shape = inst.types.fnShape(explicit_call_relation_ty);
        const explicit_call_arg_tys = try self.dupeTypeVarIds(
            inst.types.sliceTypeVarSpan(explicit_call_shape.args),
        );
        defer if (explicit_call_arg_tys.len != 0) self.allocator.free(explicit_call_arg_tys);
        if (explicit_call_arg_tys.len != arg_expr_ids.len) {
            debugPanic("lambdamono.lower.concreteExecutableTypeFromDirectExactCallExpr explicit arg arity mismatch");
        }
        ...
        for (arg_expr_ids, explicit_call_arg_tys, 0..) |arg_expr_id, explicit_arg_ty, i| {
            direct_exec_arg_tys[i] = try self.lowerRequestedExecutableArgTypeFromExpr(
                inst,
                mono_cache,
                venv,
                arg_expr_id,
        ...
        return specialized.exec_ret_ty;
    }

    fn defaultExecutableTypeForScalarNumericLiteral(
        self: *Lowerer,
        target_ty: type_mod.TypeId,
        expr: solved.Ast.Expr,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        if (!self.executableTypeIsAbstract(target_ty)) return target_ty;
        return switch (expr.data) {
            .int_lit, .dec_lit => try self.makePrimitiveType(.dec),
            else => target_ty,
        };
    ...
    fn validatePendingExecutableSignature(self: *const Lowerer, pending: *const specializations.Pending) void {
        ...
        if (signature.ret_ty) |ret_ty| {
            if (!self.executableTypeIsAbstract(ret_ty)) {
                _ = self.requireConcreteExecutableType(
                    ret_ty,
    ...
    fn ensurePendingExecutableArgTypes(
        ...
        if (!needs_refresh) return exec_args;

        const summary_types = pending.summary_types orelse
            debugPanic("lambdamono.lower.ensurePendingExecutableArgTypes missing specialization summary store");
        const summary_fn_ty = pending.summary_fn_ty orelse
            debugPanic("lambdamono.lower.ensurePendingExecutableArgTypes missing specialization summary type");
        const summary_args = summary_types.sliceTypeVarSpan(summary_types.fnShape(summary_fn_ty).args);
        if (summary_args.len != exec_args.len) {
            debugPanic("lambdamono.lower.ensurePendingExecutableArgTypes arg arity mismatch");
        }
        ...
        for (summary_args, 0..) |summary_arg_ty, i| {
            const lowered = switch (pending.repr_mode) {
                .natural => try self.lowerExecutableTypeFromSolvedIn(
                    summary_types,
                    &mono_cache,
                    summary_arg_ty,
            ...
            const derived = if (!self.executableTypeIsAbstract(lowered))
                self.requireConcreteExecutableType(lowered, "ensurePendingExecutableArgTypes")
            else if (self.isExecutableCallableType(lowered)) blk: {
                const callable_shape = summary_types.fnShape(summary_arg_ty);
                const callable_source_args = summary_types.sliceTypeVarSpan(callable_shape.args);
                const concrete_callable_exec_ty = try self.concretizeCallableExecutableTypeForCall(
                    &inst,
                    &mono_cache,
                    &.{},
                    summary_arg_ty,
                    ...
                    lowered,
                    null,
                    callable_source_args,
                    null,
                    self.singletonExactCallableSymbolForSourceType(summary_types, summary_arg_ty),
                ) orelse debugPanic(
                    "lambdamono.lower.ensurePendingExecutableArgTypes expected callable executable type",
                );
                break :blk self.requireConcreteExecutableType(
                    concrete_callable_exec_ty,
                    "ensurePendingExecutableArgTypes(callable)",
                );
            } else switch (pending.repr_mode) {
                .natural => {
                    const pending_entry = self.input.symbols.get(pending.name);
                    const pending_name = if (pending_entry.name == base.Ident.Idx.NONE)
                        "<none>"
                    else
                        self.input.idents.getText(pending_entry.name);
                    const source_summary = self.debugSolvedTypeSummary(summary_types, summary_arg_ty);
                    ...
                    debugPanicFmt(
                        "lambdamono.lower.ensurePendingExecutableArgTypes natural arg remained abstract symbol={d} origin={s} name={s} source={s} exec={s}",
                        .{
                            pending.name.raw(),
                            @tagName(pending_entry.origin),
                            pending_name,
                            source_summary.text,
                            exec_summary.text,
                        },
                ...
                .erased_boundary => self.requireConcreteExecutableType(
                    ...
                    "ensurePendingExecutableArgTypes(erased boundary)",
                ),
            };
            if (!self.executableTypeIsAbstract(exec_args[i]) and !self.types.equalIds(exec_args[i], derived)) {
                debugPanic("lambdamono.lower.ensurePendingExecutableArgTypes executable arg signature mismatch against specialization summary");
            }
            exec_args[i] = derived;
        }
        return exec_args;
    ...
    fn ensurePendingExecutableReturnType(
        ...
        if (pending.exec_ret_ty) |ret_ty| {
            return self.requireConcreteExecutableType(
                ret_ty,
                "ensurePendingExecutableReturnType(cached)",
            );
        }
        const summary_types = pending.summary_types orelse
            debugPanic("lambdamono.lower.ensurePendingExecutableReturnType missing specialization summary store");
        const summary_fn_ty = pending.summary_fn_ty orelse
            debugPanic("lambdamono.lower.ensurePendingExecutableReturnType missing specialization summary type");
        var mono_cache = lower_type.MonoCache.init(self.allocator);
        defer mono_cache.deinit();
        const lowered_ret_ty = switch (pending.repr_mode) {
            .natural => try self.lowerExecutableTypeFromSolvedIn(
                summary_types,
                &mono_cache,
        ...
        const ret_ty = if (!self.executableTypeIsAbstract(lowered_ret_ty))
            self.requireConcreteExecutableType(lowered_ret_ty, "ensurePendingExecutableReturnType")
        else if (self.isExecutableCallableType(lowered_ret_ty))
            self.requireConcreteExecutableType(lowered_ret_ty, "ensurePendingExecutableReturnType(callable)")
        else switch (pending.repr_mode) {
            .natural => {
                const source_summary = self.debugSolvedTypeSummary(summary_types, summary_types.fnShape(summary_fn_ty).ret);
                defer source_summary.deinit(self.allocator);
                const exec_summary = self.debugExecutableTypeSummary(lowered_ret_ty);
                defer exec_summary.deinit(self.allocator);
                debugPanicFmt(
                    "lambdamono.lower.ensurePendingExecutableReturnType natural return remained abstract source={s} exec={s}",
                    .{ source_summary.text, exec_summary.text },
                );
            },
            .erased_boundary => self.requireConcreteExecutableType(
                try self.lowerErasedBoundaryExecutableTypeIn(
                ...
                "ensurePendingExecutableReturnType(erased boundary)",
            ),
        };
        pending.exec_ret_ty = ret_ty;
        return ret_ty;
    }
    ...
    fn preparePendingSummary(
        ...
        if (pending.repr_mode == .natural) {
            try self.seedPendingNaturalSummaryFromExecutableSignature(pending);
        }
    ...
    fn seedPendingNaturalSummaryFromExecutableSignature(
        self: *Lowerer,
        pending: *specializations.Pending,
    ) std.mem.Allocator.Error!void {
        const summary_types = pending.summary_types orelse
            debugPanic("lambdamono.lower.seedPendingNaturalSummaryFromExecutableSignature missing specialization summary store");
        const summary_fn_ty = pending.summary_fn_ty orelse
            debugPanic("lambdamono.lower.seedPendingNaturalSummaryFromExecutableSignature missing specialization summary type");
        const exec_args = pending.exec_args_tys orelse
            debugPanic("lambdamono.lower.seedPendingNaturalSummaryFromExecutableSignature missing executable arg signature");
        ...
        if (summary_args.len != exec_args.len) {
            debugPanic("lambdamono.lower.seedPendingNaturalSummaryFromExecutableSignature arg arity mismatch");
        }
        for (summary_args, exec_args) |summary_arg_ty, exec_arg_ty| {
            if (!self.executableTypeIsAbstract(exec_arg_ty)) {
                try self.constrainSolvedTypeToExecutable(summary_types, summary_arg_ty, exec_arg_ty);
            }
        }
        if (pending.exec_ret_ty) |exec_ret_ty| {
            if (!self.executableTypeIsAbstract(exec_ret_ty)) {
                try self.constrainSolvedTypeToExecutable(summary_types, summary_shape.ret, exec_ret_ty);
            }
        }
    }

    fn constrainSolvedTypeToExecutable(
        self: *Lowerer,
        solved_types: *solved.Type.Store,
        source_ty: TypeVarId,
        exec_ty: type_mod.TypeId,
    ) std.mem.Allocator.Error!void {
        const source_id = solved_types.unlinkPreservingNominal(source_ty);
        switch (solved_types.getNode(source_id)) {
            .unbd, .for_a, .flex_for_a => {
                var mapping = std.AutoHashMap(type_mod.TypeId, TypeVarId).init(self.allocator);
                defer mapping.deinit();
                const exact_ty = try self.cloneConcreteExecutableTypeIntoSolvedStore(
                    solved_types,
                    exec_ty,
                    &mapping,
                );
                try self.overwriteSolvedTypeWithExact(solved_types, source_id, exact_ty);
            },
            .nominal => |source_nominal| switch (self.types.getTypePreservingNominal(exec_ty)) {
                .nominal => |exec_nominal| {
                    if (source_nominal.module_idx != exec_nominal.module_idx or
                        source_nominal.ident != exec_nominal.ident)
                    {
                        try self.constrainSolvedTypeToExecutable(solved_types, source_nominal.backing, exec_ty);
                        return;
                    }
                    try self.constrainSolvedTypeToExecutable(solved_types, source_nominal.backing, exec_nominal.backing);
                },
                else => try self.constrainSolvedTypeToExecutable(solved_types, source_nominal.backing, exec_ty),
            },
            .content => |content| switch (content) {
                .primitive => |prim| switch (self.types.getTypePreservingNominal(exec_ty)) {
                    .nominal => |exec_nominal| try self.constrainSolvedTypeToExecutable(solved_types, source_ty, exec_nominal.backing),
                    .primitive => |exec_prim| {
                        if (prim != exec_prim) {
                            debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable primitive mismatch");
                        }
                    },
                    else => debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected primitive executable type"),
                },
                .list => |source_elem| switch (self.types.getTypePreservingNominal(exec_ty)) {
                    .nominal => |exec_nominal| try self.constrainSolvedTypeToExecutable(solved_types, source_ty, exec_nominal.backing),
                    .list => |exec_elem| try self.constrainSolvedTypeToExecutable(solved_types, source_elem, exec_elem),
                    else => debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected list executable type"),
                },
                .box => |source_elem| switch (self.types.getTypePreservingNominal(exec_ty)) {
                    .nominal => |exec_nominal| try self.constrainSolvedTypeToExecutable(solved_types, source_ty, exec_nominal.backing),
                    .box => |exec_elem| try self.constrainSolvedTypeToExecutable(solved_types, source_elem, exec_elem),
                    else => debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected box executable type"),
                },
                .tuple => |source_span| switch (self.types.getTypePreservingNominal(exec_ty)) {
                    .nominal => |exec_nominal| try self.constrainSolvedTypeToExecutable(solved_types, source_ty, exec_nominal.backing),
                    .tuple => |exec_elems| {
                        const source_elems = solved_types.sliceTypeVarSpan(source_span);
                        if (source_elems.len != exec_elems.len) {
                            debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable tuple arity mismatch");
                        }
                        for (source_elems, exec_elems) |source_elem, exec_elem| {
                            try self.constrainSolvedTypeToExecutable(solved_types, source_elem, exec_elem);
                        }
                    },
                    else => debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected tuple executable type"),
                },
                .record => |source_record| switch (self.types.getTypePreservingNominal(exec_ty)) {
                    .nominal => |exec_nominal| try self.constrainSolvedTypeToExecutable(solved_types, source_ty, exec_nominal.backing),
                    .record => |exec_record| {
                        const source_fields = solved_types.sliceFields(source_record.fields);
                        if (source_fields.len != exec_record.fields.len) {
                            debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable record field arity mismatch");
                        }
                        for (source_fields, exec_record.fields) |source_field, exec_field| {
                            if (source_field.name != exec_field.name) {
                                debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable record field mismatch");
                            }
                            try self.constrainSolvedTypeToExecutable(solved_types, source_field.ty, exec_field.ty);
                        }
                    },
                    else => debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected record executable type"),
                },
                .tag_union => |source_union| switch (self.types.getTypePreservingNominal(exec_ty)) {
                    .nominal => |exec_nominal| try self.constrainSolvedTypeToExecutable(solved_types, source_ty, exec_nominal.backing),
                    .tag_union => |exec_union| {
                        if (exec_union.call != null) {
                            debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected ctor tag union executable type");
                        }
                        const source_tags = solved_types.sliceTags(source_union.tags);
                        if (source_tags.len != exec_union.tags.len) {
                            if (self.sourceTypeIsAbstract(solved_types, source_ty)) {
                                var mapping = std.AutoHashMap(type_mod.TypeId, TypeVarId).init(self.allocator);
                                defer mapping.deinit();
                                const exact_ty = try self.cloneConcreteExecutableTypeIntoSolvedStore(
                                    solved_types,
                            ...
                            const source_summary = self.debugSolvedTypeSummary(solved_types, source_ty);
                            defer source_summary.deinit(self.allocator);
                            const exec_summary = self.debugExecutableTypeSummary(exec_ty);
                            defer exec_summary.deinit(self.allocator);
                            const specializing_symbol = self.current_specializing_symbol orelse Symbol.none;
                            const specializing_name = if (!specializing_symbol.isNone())
                                self.input.idents.getText(self.input.symbols.get(specializing_symbol).name)
                            else
                                "<none>";
                            debugPanicFmt(
                                "lambdamono.lower.constrainSolvedTypeToExecutable tag union arity mismatch specializing_symbol={d} specializing_name={s} source={s} exec={s}",
                                .{
                                    specializing_symbol.raw(),
                                    specializing_name,
                                    source_summary.text,
                                    exec_summary.text,
                                },
                        ...
                        for (source_tags, exec_union.tags) |source_tag, exec_tag| {
                            switch (exec_tag.name) {
                                .ctor => |exec_name| {
                                    if (source_tag.name != exec_name) {
                                        debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable tag mismatch");
                                    }
                                },
                                .lambda => debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected ctor tag union executable type"),
                            }
                            const source_args = solved_types.sliceTypeVarSpan(source_tag.args);
                            if (source_args.len != exec_tag.args.len) {
                                debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable tag payload arity mismatch");
                            }
                            for (source_args, exec_tag.args) |source_arg, exec_arg| {
                                try self.constrainSolvedTypeToExecutable(solved_types, source_arg, exec_arg);
                            }
                        }
                    },
                    else => debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected tag-union executable type"),
                },
                .func => |source_fn| {
                    const exec_sig = switch (self.types.getTypePreservingNominal(exec_ty)) {
                        .nominal => |exec_nominal| return try self.constrainSolvedTypeToExecutable(solved_types, source_ty, exec_nominal.backing),
                        else => self.executableCallableSig(exec_ty) orelse
                            debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable expected callable executable type"),
                    };
                    const source_args = solved_types.sliceTypeVarSpan(source_fn.args);
                    if (source_args.len != exec_sig.args.len) {
                        debugPanic("lambdamono.lower.constrainSolvedTypeToExecutable callable arg arity mismatch");
                    }
                    for (source_args, exec_sig.args) |source_arg, exec_arg| {
                        try self.constrainSolvedTypeToExecutable(solved_types, source_arg, exec_arg);
                    }
                    try self.constrainSolvedTypeToExecutable(solved_types, source_fn.ret, exec_sig.ret);
                },
                .lambda_set => {},
            },
            .link => unreachable,
    ...
    fn cloneConcreteExecutableTypeIntoSolvedStore(
        self: *Lowerer,
        solved_types: *solved.Type.Store,
        exec_ty: type_mod.TypeId,
        mapping: *std.AutoHashMap(type_mod.TypeId, TypeVarId),
    ) std.mem.Allocator.Error!TypeVarId {
        if (mapping.get(exec_ty)) |cached| return cached;

        return switch (self.types.getTypePreservingNominal(exec_ty)) {
            .placeholder, .unbd, .link => debugPanic(
                "lambdamono.lower.cloneConcreteExecutableTypeIntoSolvedStore expected concrete executable type",
            ),
            .primitive => |prim| blk: {
                const ty = try solved_types.freshContent(.{ .primitive = prim });
                try mapping.put(exec_ty, ty);
                break :blk ty;
            },
            .list => |exec_elem| blk: {
                const placeholder = try solved_types.freshUnbd();
                try mapping.put(exec_ty, placeholder);
                const elem_ty = try self.cloneConcreteExecutableTypeIntoSolvedStore(solved_types, exec_elem, mapping);
                solved_types.setNode(placeholder, .{ .content = .{ .list = elem_ty } });
            ...
            .box => |exec_elem| blk: {
                ...
                try mapping.put(exec_ty, placeholder);
                const elem_ty = try self.cloneConcreteExecutableTypeIntoSolvedStore(solved_types, exec_elem, mapping);
                solved_types.setNode(placeholder, .{ .content = .{ .box = elem_ty } });
                break :blk placeholder;
            },
            .tuple => |exec_elems| blk: {
                const placeholder = try solved_types.freshUnbd();
                try mapping.put(exec_ty, placeholder);
                const elems = try self.allocator.alloc(TypeVarId, exec_elems.len);
                defer if (elems.len != 0) self.allocator.free(elems);
                for (exec_elems, 0..) |exec_elem, i| {
                    elems[i] = try self.cloneConcreteExecutableTypeIntoSolvedStore(solved_types, exec_elem, mapping);
                }
                solved_types.setNode(placeholder, .{ .content = .{
                    .tuple = try solved_types.addTypeVarSpan(elems),
                } });
                break :blk placeholder;
            },
            .record => |exec_record| blk: {
                const placeholder = try solved_types.freshUnbd();
                try mapping.put(exec_ty, placeholder);
                const fields = try self.allocator.alloc(solved.Type.Field, exec_record.fields.len);
                defer if (fields.len != 0) self.allocator.free(fields);
                for (exec_record.fields, 0..) |exec_field, i| {
                    fields[i] = .{
                        .name = exec_field.name,
                        .ty = try self.cloneConcreteExecutableTypeIntoSolvedStore(solved_types, exec_field.ty, mapping),
                    };
                }
                solved_types.setNode(placeholder, .{ .content = .{
                    .record = .{ .fields = try solved_types.addFields(fields) },
                } });
                break :blk placeholder;
            },
            .tag_union => |exec_union| blk: {
                if (exec_union.call != null) {
                    debugPanic("lambdamono.lower.cloneConcreteExecutableTypeIntoSolvedStore cannot materialize callable executable type as solved type");
                }
                const placeholder = try solved_types.freshUnbd();
                try mapping.put(exec_ty, placeholder);
                const tags = try self.allocator.alloc(solved.Type.Tag, exec_union.tags.len);
                defer if (tags.len != 0) self.allocator.free(tags);
                for (exec_union.tags, 0..) |exec_tag, i| {
                    const tag_name = switch (exec_tag.name) {
                        .ctor => |name| name,
                        .lambda => debugPanic("lambdamono.lower.cloneConcreteExecutableTypeIntoSolvedStore expected ctor tag union"),
                    };
                    const args = try self.allocator.alloc(TypeVarId, exec_tag.args.len);
                    defer if (args.len != 0) self.allocator.free(args);
                    for (exec_tag.args, 0..) |exec_arg, arg_i| {
                        args[arg_i] = try self.cloneConcreteExecutableTypeIntoSolvedStore(solved_types, exec_arg, mapping);
                    }
                    tags[i] = .{
                        .name = tag_name,
                        .args = try solved_types.addTypeVarSpan(args),
                    };
                }
                solved_types.setNode(placeholder, .{ .content = .{
                    .tag_union = .{ .tags = try solved_types.addTags(tags) },
                } });
                break :blk placeholder;
            },
            .nominal => |exec_nominal| blk: {
                const placeholder = try solved_types.freshUnbd();
                try mapping.put(exec_ty, placeholder);
                solved_types.setNode(placeholder, .{ .nominal = .{
                    .module_idx = exec_nominal.module_idx,
                    .ident = exec_nominal.ident,
                    .is_opaque = exec_nominal.is_opaque,
                    .args = try solved_types.addTypeVarSpan(&.{}),
                    .backing = try self.cloneConcreteExecutableTypeIntoSolvedStore(solved_types, exec_nominal.backing, mapping),
                } });
                break :blk placeholder;
            },
            .erased_fn => debugPanic(
                "lambdamono.lower.cloneConcreteExecutableTypeIntoSolvedStore cannot materialize erased callable executable type as solved type",
            ),
        };
    }
    ...
    fn refreshPendingExecutableReturnType(
        self: *Lowerer,
        pending_symbol: Symbol,
        solved_types: *solved.Type.Store,
        mono_cache: *lower_type.MonoCache,
        ...
        const pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
            debugPanic("lambdamono.lower.refreshPendingExecutableReturnType missing queued specialization");
        if (pending.exec_ret_ty) |cached_ret_ty| {
            if (!self.executableTypeIsAbstract(cached_ret_ty)) {
                return self.requireConcreteExecutableType(
                    cached_ret_ty,
                    "refreshPendingExecutableReturnType(cached)",
                );
            }
        }
        const summary_fn_ty = pending.summary_fn_ty orelse
            debugPanic("lambdamono.lower.refreshPendingExecutableReturnType missing specialization summary type");
        const lowered_ret_ty = switch (pending.repr_mode) {
            .natural => try self.lowerExecutableTypeFromSolvedIn(
                solved_types,
                mono_cache,
                solved_types.fnShape(summary_fn_ty).ret,
            ),
            .erased_boundary => try self.lowerErasedBoundaryExecutableTypeIn(
                solved_types,
                mono_cache,
        ...
        const refreshed = if (!self.executableTypeIsAbstract(lowered_ret_ty))
            self.requireConcreteExecutableType(lowered_ret_ty, "refreshPendingExecutableReturnType")
        else if (self.isExecutableCallableType(lowered_ret_ty))
            self.requireConcreteExecutableType(lowered_ret_ty, "refreshPendingExecutableReturnType(callable)")
        else switch (pending.repr_mode) {
            .natural => {
                const source_summary = self.debugSolvedTypeSummary(solved_types, solved_types.fnShape(summary_fn_ty).ret);
                defer source_summary.deinit(self.allocator);
                const exec_summary = self.debugExecutableTypeSummary(lowered_ret_ty);
                defer exec_summary.deinit(self.allocator);
                debugPanicFmt(
                    "lambdamono.lower.refreshPendingExecutableReturnType natural return remained abstract source={s} exec={s}",
                    .{ source_summary.text, exec_summary.text },
                );
            },
            .erased_boundary => self.requireConcreteExecutableType(
                ...
                "refreshPendingExecutableReturnType(erased boundary)",
            ),
        };
        pending.exec_ret_ty = refreshed;
        return refreshed;
    }

    fn strengthenPendingExecutableReturnTypeFromBody(
        self: *Lowerer,
        pending_symbol: Symbol,
        body_ret_ty: type_mod.TypeId,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        if (self.executableTypeIsAbstract(body_ret_ty)) return body_ret_ty;

        const pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
            debugPanic("lambdamono.lower.strengthenPendingExecutableReturnTypeFromBody missing queued specialization");
        const cached_ret_ty = pending.exec_ret_ty orelse {
            pending.exec_ret_ty = body_ret_ty;
            return body_ret_ty;
        };
        if (self.executableTypeIsAbstract(cached_ret_ty)) {
            pending.exec_ret_ty = body_ret_ty;
        ...
        if (self.types.equalIds(cached_ret_ty, body_ret_ty)) return cached_ret_ty;
        const strengthened = try self.strengthenErasedCallableExpectedTypeFromCurrent(
            body_ret_ty,
            cached_ret_ty,
        );
        if (!self.types.equalIds(strengthened, cached_ret_ty)) {
            pending.exec_ret_ty = strengthened;
            return strengthened;
        }
        return cached_ret_ty;
    }

    fn currentPendingExecutableReturnDefaultType(
        self: *Lowerer,
        pending_symbol: Symbol,
        solved_types: *solved.Type.Store,
        mono_cache: *lower_type.MonoCache,
        ...
        const pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
            debugPanic("lambdamono.lower.currentPendingExecutableReturnDefaultType missing queued specialization");
        if (pending.exec_ret_ty) |cached_ret_ty| {
            if (!self.executableTypeIsAbstract(cached_ret_ty)) {
                return self.requireConcreteExecutableType(
                    cached_ret_ty,
                    "currentPendingExecutableReturnDefaultType(cached)",
                );
            }
        }
        const summary_fn_ty = pending.summary_fn_ty orelse
            debugPanic("lambdamono.lower.currentPendingExecutableReturnDefaultType missing specialization summary type");
        const lowered_ret_ty = switch (pending.repr_mode) {
            .natural => try self.lowerExecutableTypeFromSolvedIn(
                solved_types,
                mono_cache,
        ...
        if (!self.executableTypeIsAbstract(lowered_ret_ty)) {
            if (pending.exec_ret_ty == null and
                pending.repr_mode == .natural and
                self.executableTypeContainsNestedCallable(lowered_ret_ty))
            {
                return try self.types.addType(.unbd);
            }
            return self.requireConcreteExecutableType(
                lowered_ret_ty,
                "currentPendingExecutableReturnDefaultType",
            );
        }
        if (self.isExecutableCallableType(lowered_ret_ty)) return lowered_ret_ty;
        return switch (pending.repr_mode) {
            .natural => lowered_ret_ty,
            .erased_boundary => self.requireConcreteExecutableType(
                try self.lowerErasedBoundaryExecutableTypeIn(
                    solved_types,
                    mono_cache,
                ...
                "currentPendingExecutableReturnDefaultType(erased boundary)",
            ),
        };
    }
    ...
    fn explicitExprSourceType(
        ...
        return switch (expr.data) {
            ...
            .int_lit, .dec_lit => {
                ...
                return try self.defaultNumericLiteralSourceType(inst.types, current_ty);
            },
            .list, .record, .tag => {
                const current_ty = try self.cloneInstType(inst, expr.ty);
    ...
    fn defaultNumericLiteralSourceType(
        self: *Lowerer,
        solved_types: *solved.Type.Store,
        current_source_ty: TypeVarId,
    ) std.mem.Allocator.Error!TypeVarId {
        _ = self;
        const root = solved_types.unlinkPreservingNominal(current_source_ty);
        return switch (solved_types.getNode(root)) {
            .unbd, .for_a, .flex_for_a => try solved_types.freshContent(.{ .primitive = .dec }),
            .content => |content| switch (content) {
                .primitive => |prim| switch (prim) {
                    .u8, .i8, .u16, .i16, .u32, .i32, .u64, .i64, .u128, .i128, .f32, .f64, .dec => current_source_ty,
    ...
    fn concretizeCallableExecutableTypeForCall(
        self: *Lowerer,
        inst: *InstScope,
        mono_cache: *lower_type.MonoCache,
        venv: []const EnvEntry,
        ...
        const repr_mode = switch (self.types.getTypePreservingNominal(current_exec_ty)) {
            .tag_union => |tag_union| if (self.tagUnionIsInternalLambdaSet(tag_union.tags))
                specializations.Pending.ReprMode.natural
            else
                return null,
            .erased_fn => specializations.Pending.ReprMode.erased_boundary,
            else => return null,
        };

        const exec_arg_tys = try self.allocator.alloc(type_mod.TypeId, arg_source_tys.len);
        defer if (exec_arg_tys.len != 0) self.allocator.free(exec_arg_tys);
        const requested_exec_ret_ty = self.requestedConcreteSpecializationReturnType(
            expected_result_exec_ty,
            "concretizeCallableExecutableTypeForCall(ret)",
        );
        for (arg_source_tys, 0..) |arg_source_ty, i| {
            const lowered = if (arg_expr_ids) |expr_ids|
                try self.lowerRequestedExecutableArgTypeFromExpr(
                    inst,
                    mono_cache,
                    venv,
                    expr_ids[i],
            ...
            exec_arg_tys[i] = if (!self.executableTypeIsAbstract(lowered))
                self.requireConcreteExecutableType(lowered, "concretizeCallableExecutableTypeForCall(arg)")
            else
                lowered;
        }
        ...
        switch (repr_mode) {
            .natural => {
                const lambda_members = try self.collectExecutableLambdaMembers(
                    inst.types,
                    func_source_ty,
                    current_exec_ty,
                );
                defer if (lambda_members.len != 0) self.allocator.free(lambda_members);

                var merged_exec_ty: ?type_mod.TypeId = null;
                for (lambda_members) |lambda_member| {
                    const requested_symbol = if (lambda_members.len == 1)
                        direct_func_symbol orelse lambda_member.symbol
                    else
                        lambda_member.symbol;
                    const specialized = try self.ensureQueuedCallableSpecializedWithExecSignature(
                    ...
                    const member_call_sig: type_mod.CallableSig = .{
                        .args = try self.types.dupeTypeIds(specialized.exec_args_tys),
                        .ret = specialized.exec_ret_ty,
                    };
                    defer if (member_call_sig.args.len != 0) self.allocator.free(member_call_sig.args);
                    const singleton_exec_ty = try self.makeSingletonExecutableLambdaType(
                        lambda_member.symbol,
                        specialized.exec_capture_ty,
                        member_call_sig,
                    );
                    merged_exec_ty = if (merged_exec_ty) |existing|
                        (try self.mergedExecutableLambdaSetType(existing, singleton_exec_ty)) orelse
                            debugPanic("lambdamono.lower.concretizeCallableExecutableTypeForCall natural callable signature mismatch across lambda members")
                    else
                        singleton_exec_ty;
                }
                return merged_exec_ty orelse
                    debugPanic("lambdamono.lower.concretizeCallableExecutableTypeForCall natural callable expected at least one lambda member");
            },
            .erased_boundary => {
                const capture_ty = self.erasedFnCaptureType(current_exec_ty);
                switch (inst.types.lambdaRepr(func_source_ty)) {
                    .lset => |lambda_members| {
                        var call_sig_args: ?[]const type_mod.TypeId = null;
                        var call_sig_ret: ?type_mod.TypeId = null;
                        for (lambda_members) |lambda_member| {
                            const requested_symbol = if (lambda_members.len == 1)
                                direct_func_symbol orelse lambda_member.symbol
                            else
                            ...
                            if (specialized.exec_capture_ty != capture_ty) {
                                debugPanic("lambdamono.lower.concretizeCallableExecutableTypeForCall erased callable capture mismatch across lambda members");
                            }
                            if (call_sig_args) |existing_args| {
                                if (existing_args.len != specialized.exec_args_tys.len) {
                                    debugPanic("lambdamono.lower.concretizeCallableExecutableTypeForCall erased callable arg arity mismatch across lambda members");
                                }
                                for (existing_args, specialized.exec_args_tys) |existing_arg, next_arg| {
                                    if (!self.types.equalIds(existing_arg, next_arg)) {
                                        debugPanic("lambdamono.lower.concretizeCallableExecutableTypeForCall erased callable arg type mismatch across lambda members");
                                    }
                                }
                                if (!self.types.equalIds(call_sig_ret.?, specialized.exec_ret_ty)) {
                                    debugPanic("lambdamono.lower.concretizeCallableExecutableTypeForCall erased callable return type mismatch across lambda members");
                                }
                            } else {
                                call_sig_args = specialized.exec_args_tys;
                                call_sig_ret = specialized.exec_ret_ty;
                            }
                        }
                        const authoritative_call_sig: type_mod.CallableSig = .{
                            .args = try self.types.dupeTypeIds(call_sig_args orelse
                                debugPanic("lambdamono.lower.concretizeCallableExecutableTypeForCall erased callable expected at least one lambda member")),
                            .ret = call_sig_ret orelse
                                debugPanic("lambdamono.lower.concretizeCallableExecutableTypeForCall erased callable missing return type"),
                        };
                        defer if (authoritative_call_sig.args.len != 0) self.allocator.free(authoritative_call_sig.args);
                        return try self.makeErasedFnType(capture_ty, authoritative_call_sig);
                    },
                    .erased => {
                        const call_shape = inst.types.fnShape(call_relation_ty);
                        const authoritative_ret_ty = self.requireConcreteExecutableType(
                            try self.lowerErasedBoundaryExecutableTypeIn(
                                inst.types,
                                mono_cache,
                                call_shape.ret,
                            ),
                            "concretizeCallableExecutableTypeForCall(erased ret)",
                        );
                        const authoritative_call_sig: type_mod.CallableSig = .{
                            .args = try self.types.dupeTypeIds(exec_arg_tys),
                            .ret = authoritative_ret_ty,
                        };
                        defer if (authoritative_call_sig.args.len != 0) self.allocator.free(authoritative_call_sig.args);
                        return try self.makeErasedFnType(capture_ty, authoritative_call_sig);
                    },
    ...
    fn concretizeEmptyValueExecutableType(
        self: *Lowerer,
        ty: type_mod.TypeId,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        return switch (self.types.getTypePreservingNominal(ty)) {
            .placeholder, .unbd, .link => try self.makeUnitType(),
            .primitive, .erased_fn => ty,
            .list => |elem| try self.synthesizedListTypeFromElem(
                try self.concretizeEmptyValueExecutableType(elem),
            ),
            .box => |elem| try self.internExecutableType(try self.types.internResolved(.{
                .box = try self.concretizeEmptyValueExecutableType(elem),
            })),
            .tuple => |elems| blk: {
                const lowered = try self.allocator.alloc(type_mod.TypeId, elems.len);
                defer if (lowered.len != 0) self.allocator.free(lowered);
                for (elems, 0..) |elem, i| {
                    lowered[i] = try self.concretizeEmptyValueExecutableType(elem);
                }
            ...
            .record => |record| blk: {
                const fields = try self.allocator.alloc(type_mod.Field, record.fields.len);
                defer if (fields.len != 0) self.allocator.free(fields);
                for (record.fields, 0..) |field, i| {
                    fields[i] = .{
                        .name = field.name,
                        .ty = try self.concretizeEmptyValueExecutableType(field.ty),
                    };
                }
                break :blk try self.internExecutableType(try self.types.internResolved(.{
                    .record = .{ .fields = try self.types.dupeFields(fields) },
                }));
            },
            .tag_union => |tag_union| blk: {
                const tags = try self.allocator.alloc(type_mod.Tag, tag_union.tags.len);
                defer if (tags.len != 0) self.allocator.free(tags);
                for (tag_union.tags, 0..) |tag, i| {
                    const args = try self.allocator.alloc(type_mod.TypeId, tag.args.len);
                    defer if (args.len != 0) self.allocator.free(args);
                    for (tag.args, 0..) |arg, arg_i| {
                        args[arg_i] = try self.concretizeEmptyValueExecutableType(arg);
                    }
                    tags[i] = .{
                        .name = tag.name,
                ...
                break :blk try self.internExecutableType(try self.types.internResolved(.{
                    .tag_union = .{
                        .tags = try self.types.dupeTags(tags),
                        .call = if (tag_union.call) |call| .{
                            .args = try self.types.dupeTypeIds(call.args),
                            .ret = call.ret,
                        } else null,
                    },
                }));
            ...
            .nominal => |nominal| blk: {
                const args = try self.allocator.alloc(type_mod.TypeId, nominal.args.len);
                defer if (args.len != 0) self.allocator.free(args);
                for (nominal.args, 0..) |arg, i| {
                    args[i] = try self.concretizeEmptyValueExecutableType(arg);
                }
                break :blk try self.internExecutableType(try self.types.internResolved(.{
                    .nominal = .{
                        .module_idx = nominal.module_idx,
                        ...
                        .is_opaque = nominal.is_opaque,
                        .args = try self.types.dupeTypeIds(args),
                        .backing = try self.concretizeEmptyValueExecutableType(nominal.backing),
                    },
                }));
            },
        };
    ...
    fn eraseBoundaryExecutableType(
        ...
        return switch (self.types.getTypePreservingNominal(ty)) {
            ...
            .tag_union => |tag_union| if (self.tagUnionIsInternalLambdaSet(tag_union.tags))
                try self.makeErasedFnType(
                    self.commonErasedCaptureTypeFromExecutable(tag_union.tags),
                    self.requireExecutableCallableSig(ty, "eraseBoundaryExecutableType"),
                )
            else
                ty,
            .erased_fn => ty,
    ...
    fn synthesizedRecordTypeFromFields(
        self: *Lowerer,
        fields: []const ast.FieldExpr,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        const synthesized = try self.allocator.alloc(type_mod.Field, fields.len);
        defer self.allocator.free(synthesized);
        ...
        std.mem.sort(type_mod.Field, synthesized, &self.input.idents, struct {
        ...
        return try self.types.internResolved(.{
            .record = .{
                .fields = try self.types.dupeFields(synthesized),
            },
        });
    }

    fn synthesizedTupleTypeFromElems(
        self: *Lowerer,
        elems: []const ast.ExprId,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        const elem_tys = try self.allocator.alloc(type_mod.TypeId, elems.len);
        defer self.allocator.free(elem_tys);

        for (elems, 0..) |elem, i| {
            elem_tys[i] = self.output.getExpr(elem).ty;
        }

        return try self.types.internResolved(.{
            .tuple = try self.types.dupeTypeIds(elem_tys),
        });
    }

    fn synthesizedListTypeFromElem(
        self: *Lowerer,
        elem_ty: type_mod.TypeId,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        return try self.types.internResolved(.{ .list = elem_ty });
    }

    fn synthesizedSingletonTagType(
        self: *Lowerer,
        ...
        const tags = try self.types.dupeTags(&.{.{
            .name = .{ .ctor = tag_name },
            .args = try self.types.dupeTypeIds(arg_tys),
        }});
        return try self.types.internResolved(.{
            .tag_union = .{
                .tags = tags,
    ...
    fn synthesizedSingletonTagTypeFromArgTypes(
        self: *Lowerer,
        tag_name: base.Ident.Idx,
        arg_tys: []const type_mod.TypeId,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        const tags = try self.types.dupeTags(&.{.{
            .name = .{ .ctor = tag_name },
            .args = try self.types.dupeTypeIds(arg_tys),
    ...
    fn executableLambdaTagLessThan(_: void, a: type_mod.Tag, b: type_mod.Tag) bool {
        const a_symbol = switch (a.name) {
            .lambda => |symbol| symbol,
            .ctor => debugPanic("lambdamono.lower.executableLambdaTagLessThan expected lambda tag"),
        };
        const b_symbol = switch (b.name) {
            .lambda => |symbol| symbol,
            .ctor => debugPanic("lambdamono.lower.executableLambdaTagLessThan expected lambda tag"),
        };
        return a_symbol.raw() < b_symbol.raw();
    }

    fn mergedExecutableLambdaSetType(
        self: *Lowerer,
        ...
        const left_sig = left_union.call orelse return null;
        const right_sig = right_union.call orelse return null;
        if (!self.executableCallableSigEqual(left_sig, right_sig)) return null;

        var merged = std.ArrayList(type_mod.Tag).empty;
        defer merged.deinit(self.allocator);

        try merged.appendSlice(self.allocator, left_union.tags);
        for (right_union.tags) |tag| {
            var found = false;
            for (merged.items) |existing| {
                const existing_symbol = switch (existing.name) {
                    .lambda => |symbol| symbol,
                    .ctor => debugPanic("lambdamono.lower.mergedExecutableLambdaSetType expected lambda tag"),
                };
                const tag_symbol = switch (tag.name) {
                    .lambda => |symbol| symbol,
                    .ctor => debugPanic("lambdamono.lower.mergedExecutableLambdaSetType expected lambda tag"),
                };
                if (existing_symbol == tag_symbol) {
                    if (!self.executableTagArgsEqual(existing.args, tag.args)) {
                        debugPanic("lambdamono.lower.mergedExecutableLambdaSetType executable lambda capture type mismatch");
                    }
                    found = true;
                    break;
                }
            ...
            if (!found) {
                try merged.append(self.allocator, tag);
            }
        }

        std.mem.sort(type_mod.Tag, merged.items, {}, executableLambdaTagLessThan);
        return try self.types.internResolved(.{
            .tag_union = .{
                .tags = try self.types.dupeTags(merged.items),
                .call = try self.types.dupeCallableSig(left_sig),
            },
        });
    }
    ...
    fn commonExecutableType(
        ...
        if (self.types.equalIds(left_ty, right_ty)) return left_ty;
        const strengthened_right = try self.strengthenErasedCallableExpectedTypeFromCurrent(
            left_ty,
            right_ty,
        );
        if (!self.types.equalIds(strengthened_right, right_ty)) {
            return strengthened_right;
        }
        const strengthened_left = try self.strengthenErasedCallableExpectedTypeFromCurrent(
            right_ty,
            left_ty,
        );
        if (!self.types.equalIds(strengthened_left, left_ty)) {
            return strengthened_left;
        }
        if (try self.mergedExecutableLambdaSetType(left_ty, right_ty)) |merged| {
            return merged;
        }
        return null;
    }
    ...
    fn lowerBindingBody(
        ...
        const bind_name = blk_name: {
            const bind_entry = self.input.symbols.get(bind.symbol);
            break :blk_name if (bind_entry.name == base.Ident.Idx.NONE)
                "<none>"
            else
        ...
        if (std.mem.eql(u8, bind_name, "f")) {
            const bind_exec_summary = self.debugExecutableTypeSummary(bind_exec_ty);
            defer bind_exec_summary.deinit(self.allocator);
            const lowered_exec_summary = self.debugExecutableTypeSummary(lowered_exec_ty);
            defer lowered_exec_summary.deinit(self.allocator);
            std.debug.print(
                "lowerBindingBody bind=f default={s} lowered={s}\n",
                .{ bind_exec_summary.text, lowered_exec_summary.text },
            );
        }
    ...
    fn specializeFn(self: *Lowerer, pending_symbol: Symbol) std.mem.Allocator.Error!SpecializedDef {
        ...
        switch (pending.source_def) {
            .hosted_fn => |hosted_fn| {
                if (pending.repr_mode != .erased_boundary) {
            ...
            .fn_ => |fn_def| {
                ...
                const body_contains_return = self.exprContainsReturn(fn_body);
                const preserved_summary_ret_ty = if (body_contains_return)
                    try self.freshCloneTypeIntoInstFromStore(&inst, inst.types, fn_shape.ret)
                else
                    null;
                ...
                const result: ast.FnDef = switch (pending.repr_mode) {
                    .natural => if (frozen_captures.len == 0) blk_fn: {
                        ...
                        defer self.allocator.free(body_env);
                        const body = blk_body: {
                            const pre_lower_ret_ty = try self.currentPendingExecutableReturnDefaultType(
                                pending_symbol,
                                inst.types,
                                &mono_cache,
                            );
                            ...
                            const authoritative_source_ret_ty = if (body_contains_return and
                                !self.sourceTypeIsAbstract(inst.types, preserved_summary_ret_ty.?))
                            ...
                            var final_ret_cache = lower_type.MonoCache.init(self.allocator);
                            defer final_ret_cache.deinit();
                            const lowered_summary_ret_ty = try self.lowerExecutableTypeFromSolvedIn(
                                inst.types,
                                &final_ret_cache,
                                fn_shape.ret,
                            );
                            if (self.executableTypeIsAbstract(lowered_summary_ret_ty)) {
                                const source_entry = self.input.symbols.get(pending.name);
                                const exec_summary = self.debugExecutableTypeSummary(lowered_summary_ret_ty);
                                defer exec_summary.deinit(self.allocator);
                                const source_summary = self.debugSolvedTypeSummary(inst.types, final_source_ret_ty);
                                defer source_summary.deinit(self.allocator);
                                debugPanicFmt(
                                    "specializeFn natural return still abstract pending={d} source={d} source_name={s} source_origin={s} body_contains_return={} source_body={s} lowered_body={s} source_ret={s} exec_ret={s}",
                                    .{
                                        pending_symbol.raw(),
                                        pending.name.raw(),
                                        self.input.idents.getText(source_entry.name),
                                        @tagName(source_entry.origin),
                                        body_contains_return,
                                        @tagName(body_expr.data),
                                        @tagName(self.output.getExpr(body).data),
                                        source_summary.text,
                                        exec_summary.text,
                                    },
                                );
                            }
                            _ = try self.refreshPendingExecutableReturnType(
                                pending_symbol,
                                inst.types,
                                &final_ret_cache,
                            );
                            const authoritative_ret_ty = try self.strengthenPendingExecutableReturnTypeFromBody(
                                pending_symbol,
                    ...
                    .erased_boundary => blk: {
                        ...
                        var final_ret_cache = lower_type.MonoCache.init(self.allocator);
                        defer final_ret_cache.deinit();
                        const final_ret_ty = try self.refreshPendingExecutableReturnType(
                            pending_symbol,
                            inst.types,
                            &final_ret_cache,
                        );
    ...
    fn strengthenErasedExactCallableExpectedType(
        self: *Lowerer,
        solved_types: *solved.Type.Store,
        mono_cache: *lower_type.MonoCache,
        venv: []const EnvEntry,
        ...
        if (capture_symbols.len == 0) return expected_exec_ty;
        if (self.executableTypeIsAbstract(expected_exec_ty)) return expected_exec_ty;

        const concrete_expected_exec_ty = self.requireConcreteExecutableType(
            expected_exec_ty,
            "strengthenErasedExactCallableExpectedType",
        );

        switch (self.types.getTypePreservingNominal(concrete_expected_exec_ty)) {
            .erased_fn => {},
            else => return expected_exec_ty,
        }

        if (self.erasedFnCaptureType(concrete_expected_exec_ty) != null) return expected_exec_ty;
        ...
        const authoritative_capture_ty = current_capture.ty orelse
            debugPanic("lambdamono.lower.strengthenErasedExactCallableExpectedType missing current erased capture payload type");

        return try self.makeErasedFnType(
            authoritative_capture_ty,
            self.requireExecutableCallableSig(
    ...
    fn exactCallableCaptureSymbolsFromSourceType(
        self: *Lowerer,
        solved_types: *solved.Type.Store,
        source_ty: TypeVarId,
        exact_symbol: Symbol,
    ) std.mem.Allocator.Error![]const Symbol {
        const member = solved_types.requireLambdaMember(source_ty, exact_symbol);
        const out = try self.allocator.alloc(Symbol, member.captures.len);
        errdefer self.allocator.free(out);
        for (member.captures, 0..) |capture, i| {
            out[i] = capture.symbol;
        }
        return out;
    }
    ...
    fn specializeExprWithDefaultTyAndSourceTy(
        ...
        const literal_default_ty = try self.defaultExecutableTypeForScalarNumericLiteral(default_ty, expr);
        const specialized: SpecializedExprLowering = switch (expr.data) {
            ...
            .int_lit => |value| .{ .ty = literal_default_ty, .data = try self.specializeIntLiteral(literal_default_ty, value) },
            ...
            .dec_lit => |value| .{ .ty = literal_default_ty, .data = try self.specializeDecLiteral(literal_default_ty, value) },
            ...
            .tag => |tag| blk: {
                ...
                for (source_args, 0..) |arg_expr_id, i| {
                    ...
                    const expected_arg_ty = if (stable_tag_ty != null and tag_exec_info != null)
                ...
                if (stable_tag_ty != null and tag_exec_info != null) {
                    const info = tag_exec_info.?;
                    for (lowered_args, info.args, 0..) |lowered_arg, strengthened_arg_ty, i| {
                ...
                const result_ty = if (stable_tag_ty) |tag_ty|
                    tag_ty
                else if (requested_tag_ty) |tag_ty|
                    try self.strengthenConcreteTagTypeFromLoweredArgs(tag_ty, tag.name, lowered_args)
                else
                    try self.synthesizedSingletonTagType(tag.name, lowered_args);

                break :blk .{
                    .ty = result_ty,
                    .data = .{ .tag = .{
                        .name = .{ .ctor = tag.name },
                        .discriminant = if (tag_exec_info) |info| info.discriminant else 0,
                        .args = try self.output.addExprSpan(lowered_args),
                    } },
                    .source_ty = try self.refineSingletonTagSourceTypeIfAbstract(
                        inst.types,
                        ty,
                        tag.name,
                        solved_tag_arg_tys,
            ...
            .record => |fields| blk: {
                ...
                const result_ty = if (concrete_record_ty) |record_ty|
                    ...
                    try self.synthesizedRecordTypeFromFields(self.output.sliceFieldExprSpan(lowered_fields));
            ...
            .when => |when_expr| blk: {
                ...
                var result_ty = if (concrete_result_ty) |concrete|
                    concrete
                else
                    self.output.getExpr(lowered_branches.items[0].body).ty;
                if (concrete_result_ty == null) {
                    for (lowered_branches.items[1..]) |branch| {
                        result_ty = try self.commonExecutableType(
                            result_ty,
                            self.output.getExpr(branch.body).ty,
                        ) orelse result_ty;
                    }
                }
            ...
            .if_ => |if_expr| blk: {
                ...
                const result_ty = if (concrete_result_ty) |concrete|
                    ...
                    (try self.commonExecutableType(then_ty, else_ty) orelse then_ty);
                if (!self.executableTypeIsAbstract(result_ty)) {
            ...
            .tuple => |elems| blk: {
                ...
                const result_ty = if (concrete_tuple_ty) |tuple_ty|
                    ...
                    try self.synthesizedTupleTypeFromElems(self.output.sliceExprSpan(lowered_elems));
            ...
            .list => |items| blk: {
                ...
                if (source_items.len == 0) {
                    const empty_list_ty = if (concrete_list_ty) |list_ty|
                        list_ty
                    else if (self.listElemType(default_ty) != null)
                        try self.concretizeEmptyValueExecutableType(default_ty)
                    else if (self.listElemType(explicit_result_ty) != null)
                        try self.concretizeEmptyValueExecutableType(explicit_result_ty)
                    else
                        try self.synthesizedListTypeFromElem(try self.makeUnitType());
                    break :blk .{
                        .ty = empty_list_ty,
                ...
                const result_elem_ty = if (concrete_list_ty) |list_ty|
                    self.listElemType(list_ty) orelse
                        debugPanic("lambdamono.lower.list missing strengthened elem type")
                else if (concrete_expected_elem_ty) |elem_ty|
                    elem_ty
                else
                    self.output.getExpr(lowered[0]).ty;
                const result_ty = if (concrete_list_ty) |list_ty|
                    list_ty
                else
                    try self.synthesizedListTypeFromElem(result_elem_ty);
    ...
    fn specializeBoxBoundaryArgExpr(
        ...
        const concrete_expected_exec_ty = self.requireConcreteExecutableType(
            expected_exec_ty,
            "specializeBoxBoundaryArgExpr",
        );
        const strengthened_expected_exec_ty = try self.strengthenErasedCallableExpectedTypeFromCurrent(
            self.output.getExpr(lowered).ty,
            concrete_expected_exec_ty,
        );
        if (self.types.equalIds(self.output.getExpr(lowered).ty, strengthened_expected_exec_ty)) {
            return lowered;
        }
        return try self.emitExplicitBridgeExpr(lowered, strengthened_expected_exec_ty);
    }
    ...
    fn emitExplicitBridgeExpr(
        ...
        const strengthened_expected_ty = try self.strengthenErasedCallableExpectedTypeFromCurrent(
            actual_ty,
            expected_ty,
        );
        if (self.types.containsAbstractLeaf(strengthened_expected_ty)) {
            debugPanicFmt(
                ...
                .{
                    @intFromEnum(expr_id),
                    @intFromEnum(actual_ty),
                    @intFromEnum(strengthened_expected_ty),
                    @tagName(self.types.getTypePreservingNominal(actual_ty)),
                    @tagName(self.types.getTypePreservingNominal(strengthened_expected_ty)),
                },
            );
        }
        if (actual_ty == strengthened_expected_ty) return expr_id;
        if (self.types.getTypePreservingNominal(strengthened_expected_ty) == .primitive and
            self.types.getTypePreservingNominal(strengthened_expected_ty).primitive == .erased)
        {
            ...
            if (self.output.getExpr(boxed_expr).ty == strengthened_expected_ty) return boxed_expr;
            return try self.output.addExpr(.{
                .ty = strengthened_expected_ty,
    ...
    fn retargetOrEmitExplicitBridgeExpr(
        ...
        const strengthened_expected_ty = try self.strengthenErasedCallableExpectedTypeFromCurrent(
            self.output.getExpr(expr_id).ty,
            expected_ty,
        );
        if (self.types.equalIds(self.output.getExpr(expr_id).ty, strengthened_expected_ty)) {
            return expr_id;
        }
        if (try self.retargetExprToExpectedExecutableType(expr_id, strengthened_expected_ty)) {
            return expr_id;
        }
        return try self.emitExplicitBridgeExpr(expr_id, strengthened_expected_ty);
    }
    ...
    fn lowerVarExprToExpectedExecutableType(
        ...
        const lowered_expr = if (self.executableTypeIsAbstract(strengthened_expected_exec_ty) or
            self.types.equalIds(current_exec_ty, strengthened_expected_exec_ty))
            var_expr
        else
    ...
    fn strengthenErasedCallableExpectedTypeFromCurrent(
        self: *Lowerer,
        current_exec_ty: type_mod.TypeId,
        expected_exec_ty: type_mod.TypeId,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        if (self.executableTypeIsAbstract(expected_exec_ty) or self.executableTypeIsAbstract(current_exec_ty)) {
            return expected_exec_ty;
        }

        switch (self.types.getTypePreservingNominal(expected_exec_ty)) {
            .nominal => |expected_nominal| switch (self.types.getTypePreservingNominal(current_exec_ty)) {
                .nominal => |current_nominal| {
                    if (expected_nominal.module_idx != current_nominal.module_idx or
                        expected_nominal.ident != current_nominal.ident or
                        expected_nominal.is_opaque != current_nominal.is_opaque)
                    {
                        return expected_exec_ty;
                    }
                    if (expected_nominal.args.len != current_nominal.args.len) {
                        return expected_exec_ty;
                    }
                    for (expected_nominal.args, current_nominal.args) |expected_arg, current_arg| {
                        if (!self.types.equalIds(expected_arg, current_arg)) {
                            return expected_exec_ty;
                        }
                    }
                    const strengthened_backing = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                        current_nominal.backing,
                        expected_nominal.backing,
                    );
                    if (self.types.equalIds(strengthened_backing, expected_nominal.backing)) {
                        return expected_exec_ty;
                    }
                    return try self.internExecutableType(try self.types.internResolved(.{ .nominal = .{
                        .module_idx = expected_nominal.module_idx,
                        .ident = expected_nominal.ident,
                        .is_opaque = expected_nominal.is_opaque,
                        .args = expected_nominal.args,
                        .backing = strengthened_backing,
                    } }));
                },
                else => return expected_exec_ty,
            },
            .box => |expected_elem| switch (self.types.getTypePreservingNominal(current_exec_ty)) {
                .box => |current_elem| {
                    const strengthened_elem = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                        current_elem,
                        expected_elem,
                    );
                    if (self.types.equalIds(strengthened_elem, expected_elem)) {
                        return expected_exec_ty;
                    }
                    return try self.internExecutableType(try self.types.internResolved(.{
                        .box = strengthened_elem,
                    }));
                },
                else => return expected_exec_ty,
            },
            .list => |expected_elem| switch (self.types.getTypePreservingNominal(current_exec_ty)) {
                .list => |current_elem| {
                    const strengthened_elem = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                        current_elem,
                        expected_elem,
                    ...
                    return try self.internExecutableType(try self.types.internResolved(.{
                        .list = strengthened_elem,
                    }));
                },
                else => return expected_exec_ty,
            },
            .tuple => |expected_elems| switch (self.types.getTypePreservingNominal(current_exec_ty)) {
                .tuple => |current_elems| {
                    if (expected_elems.len != current_elems.len) {
                        return expected_exec_ty;
                    }
                    const strengthened_elems = try self.allocator.alloc(type_mod.TypeId, expected_elems.len);
                    defer if (strengthened_elems.len != 0) self.allocator.free(strengthened_elems);
                    var changed = false;
                    for (expected_elems, current_elems, 0..) |expected_elem, current_elem, i| {
                        strengthened_elems[i] = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                            current_elem,
                            expected_elem,
                        );
                        if (!self.types.equalIds(strengthened_elems[i], expected_elem)) {
                            changed = true;
                        }
                    }
                    if (!changed) {
                    ...
                    return try self.internExecutableType(try self.types.internResolved(.{
                        .tuple = try self.types.dupeTypeIds(strengthened_elems),
                    }));
                },
                else => return expected_exec_ty,
            },
            .record => |expected_record| switch (self.types.getTypePreservingNominal(current_exec_ty)) {
                .record => |current_record| {
                    if (expected_record.fields.len != current_record.fields.len) {
                        return expected_exec_ty;
                    }
                    const strengthened_fields = try self.allocator.alloc(type_mod.Field, expected_record.fields.len);
                    defer if (strengthened_fields.len != 0) self.allocator.free(strengthened_fields);
                    var changed = false;
                    for (expected_record.fields, current_record.fields, 0..) |expected_field, current_field, i| {
                        if (expected_field.name != current_field.name) {
                            return expected_exec_ty;
                        }
                        strengthened_fields[i] = .{
                            .name = expected_field.name,
                            .ty = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                                current_field.ty,
                                expected_field.ty,
                            ),
                        };
                        if (!self.types.equalIds(strengthened_fields[i].ty, expected_field.ty)) {
                            changed = true;
                        }
                    }
                    if (!changed) {
                    ...
                    return try self.internExecutableType(try self.types.internResolved(.{
                        .record = .{ .fields = try self.types.dupeFields(strengthened_fields) },
                    }));
                },
                else => return expected_exec_ty,
            },
            .tag_union => |expected_tag_union| switch (self.types.getTypePreservingNominal(current_exec_ty)) {
                .tag_union => |current_tag_union| {
                    if (expected_tag_union.tags.len != current_tag_union.tags.len) {
                        return expected_exec_ty;
                    }

                    const strengthened_tags = try self.allocator.alloc(type_mod.Tag, expected_tag_union.tags.len);
                    var initialized_tags: usize = 0;
                    defer {
                        for (strengthened_tags[0..initialized_tags]) |tag| {
                            if (tag.args.len != 0) self.allocator.free(tag.args);
                        }
                        if (strengthened_tags.len != 0) self.allocator.free(strengthened_tags);
                    }

                    var changed = false;
                    for (expected_tag_union.tags, current_tag_union.tags, 0..) |expected_tag, current_tag, i| {
                        if (!std.meta.eql(expected_tag.name, current_tag.name) or
                            expected_tag.args.len != current_tag.args.len)
                        {
                            return expected_exec_ty;
                        }

                        const strengthened_args = try self.allocator.alloc(type_mod.TypeId, expected_tag.args.len);
                        for (expected_tag.args, current_tag.args, 0..) |expected_arg, current_arg, arg_i| {
                            strengthened_args[arg_i] = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                                current_arg,
                                expected_arg,
                            );
                            if (!self.types.equalIds(strengthened_args[arg_i], expected_arg)) {
                                changed = true;
                            }
                        }
                        strengthened_tags[i] = .{
                            .name = expected_tag.name,
                            .args = strengthened_args,
                        };
                        initialized_tags = i + 1;
                    }

                    var strengthened_call: ?type_mod.CallableSig = null;
                    if (expected_tag_union.call) |expected_call| {
                        const current_call = current_tag_union.call orelse return expected_exec_ty;
                        if (expected_call.args.len != current_call.args.len) {
                            return expected_exec_ty;
                        }

                        const strengthened_call_args = try self.allocator.alloc(type_mod.TypeId, expected_call.args.len);
                        for (expected_call.args, current_call.args, 0..) |expected_arg, current_arg, i| {
                            strengthened_call_args[i] = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                                current_arg,
                                expected_arg,
                            );
                            if (!self.types.equalIds(strengthened_call_args[i], expected_arg)) {
                                changed = true;
                            }
                        }
                        const strengthened_call_ret = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                            current_call.ret,
                            expected_call.ret,
                        );
                        if (!self.types.equalIds(strengthened_call_ret, expected_call.ret)) {
                            changed = true;
                        }
                        strengthened_call = .{
                            .args = strengthened_call_args,
                            .ret = strengthened_call_ret,
                        };
                    } else if (current_tag_union.call != null) {
                        return expected_exec_ty;
                    }
                    defer if (strengthened_call) |call| {
                    ...
                    return try self.internExecutableType(try self.types.internResolved(.{
                        .tag_union = .{
                            .tags = try self.types.dupeTags(strengthened_tags),
                            .call = if (strengthened_call) |call| .{
                                .args = try self.types.dupeTypeIds(call.args),
                                .ret = call.ret,
                            } else null,
                        },
        ...
        switch (self.types.getTypePreservingNominal(current_exec_ty)) {
            .erased_fn => {},
            else => return expected_exec_ty,
        }

        const current_sig = self.requireExecutableCallableSig(
            current_exec_ty,
            "strengthenErasedCallableExpectedTypeFromCurrent(current)",
        );
        const expected_sig = self.requireExecutableCallableSig(
            expected_exec_ty,
            "strengthenErasedCallableExpectedTypeFromCurrent(expected)",
        );
        if (!self.executableCallableSigEqual(current_sig, expected_sig)) {
            return expected_exec_ty;
        }

        const current_capture_ty = self.erasedFnCaptureType(current_exec_ty);
        const expected_capture_ty = self.erasedFnCaptureType(expected_exec_ty);
        if (expected_capture_ty == null and current_capture_ty != null) {
            return current_exec_ty;
        }
        if (expected_capture_ty != null and current_capture_ty != null and
            !self.types.equalIds(expected_capture_ty.?, current_capture_ty.?))
        {
            debugPanic("lambdamono.lower.strengthenErasedCallableExpectedTypeFromCurrent capture mismatch");
        }
        return expected_exec_ty;
    }

    fn strengthenConcreteTagTypeFromLoweredArgs(
        self: *Lowerer,
        expected_tag_ty: type_mod.TypeId,
        tag_name: base.Ident.Idx,
        lowered_args: []const ast.ExprId,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        return switch (self.types.getTypePreservingNominal(expected_tag_ty)) {
            .nominal => |nominal| blk: {
                const strengthened_backing = try self.strengthenConcreteTagTypeFromLoweredArgs(
                    nominal.backing,
                    tag_name,
                    lowered_args,
                );
                if (self.types.equalIds(strengthened_backing, nominal.backing)) {
                    break :blk expected_tag_ty;
                }
                break :blk try self.internExecutableType(try self.types.internResolved(.{
                    .nominal = .{
                        .module_idx = nominal.module_idx,
            ...
            .tag_union => |tag_union| blk: {
                const target_index = for (tag_union.tags, 0..) |tag, i| {
                    if (tag.name == .ctor and tag.name.ctor == tag_name) break i;
                } else break :blk expected_tag_ty;

                const target_tag = tag_union.tags[target_index];
                if (target_tag.args.len != lowered_args.len) {
                    break :blk expected_tag_ty;
                }

                const strengthened_args = try self.allocator.alloc(type_mod.TypeId, lowered_args.len);
                defer if (strengthened_args.len != 0) self.allocator.free(strengthened_args);
                var changed = false;
                for (target_tag.args, lowered_args, 0..) |expected_arg_ty, lowered_arg, i| {
                    strengthened_args[i] = try self.strengthenErasedCallableExpectedTypeFromCurrent(
                        self.output.getExpr(lowered_arg).ty,
                        expected_arg_ty,
                    );
                    if (!self.types.equalIds(strengthened_args[i], expected_arg_ty)) {
                        changed = true;
                    }
                }
                if (!changed) break :blk expected_tag_ty;

                const strengthened_tags = try self.allocator.alloc(type_mod.Tag, tag_union.tags.len);
                var initialized_tags: usize = 0;
                defer {
                    for (strengthened_tags[0..initialized_tags]) |tag| {
                        if (tag.args.len != 0) self.allocator.free(tag.args);
                ...
                for (tag_union.tags, 0..) |tag, i| {
                    strengthened_tags[i] = .{
                        .name = tag.name,
                        .args = if (i == target_index)
                            try self.types.dupeTypeIds(strengthened_args)
                        else
                            try self.types.dupeTypeIds(tag.args),
                    };
                    initialized_tags = i + 1;
                }
    ...
    fn specializeVarExpr(
        ...
        if (specializations.lookupFnExact(self.fenv, symbol)) |entry| {
            ...
            const exact_expected_exec_ty = try self.strengthenErasedExactCallableExpectedType(
                solved_types,
                mono_cache,
                venv,
        ...
        if (self.lookupEnvEntry(venv, symbol)) |entry| {
            ...
            if (self.singletonExactCallableSymbolForSourceType(solved_types, refined_source_ty)) |exact_symbol| {
                const capture_symbols = try self.exactCallableCaptureSymbolsFromSourceType(
                    solved_types,
                    refined_source_ty,
                    exact_symbol,
                );
                defer if (capture_symbols.len != 0) self.allocator.free(capture_symbols);
                const exact_entry = specializations.lookupFnExact(self.fenv, exact_symbol) orelse
                    debugPanic("lambdamono.lower.specializeVarExpr env source exact callable missing source definition");
                refined_source_ty = try self.unifyExactCallableSourceType(inst, exact_entry.fn_ty, refined_source_ty);
    ...
    fn lowerExactCallableVarExpr(
        ...
        if (self.executableTypeIsAbstract(expected_exec_ty)) {
            switch (solved_types.lambdaRepr(refined_source_ty)) {
                .lset => {},
                .erased => debugPanic("lambdamono.lower.lowerExactCallableVarExpr abstract executable expectation on erased callable"),
            }
            ...
            for (source_arg_tys, 0..) |arg_ty, i| {
                lowered_arg_tys[i] = try self.lowerExecutableTypeFromSolvedIn(
                    solved_types,
                    mono_cache,
                    arg_ty,
                );
            ...
            const abstract_call_sig: type_mod.CallableSig = .{
                ...
                .ret = try self.lowerExecutableTypeFromSolvedIn(
                    solved_types,
                    mono_cache,
                    fn_shape.ret,
                ),
    ...
    fn captureBindingsFromCaptures(
        ...
        for (captures, 0..) |capture, i| {
            ...
            const exact_fn_symbol = pending_exact orelse
                self.singletonExactCallableSymbolForExecutableType(field_info.ty) orelse
                self.singletonExactCallableSymbolForSourceType(solved_types, capture.ty) orelse
    ...
    fn refinedSourceTypeForExpr(
        ...
        return switch (expr.data) {
            ...
            .int_lit,
            .dec_lit,
            => try self.defaultNumericLiteralSourceType(inst.types, current_source_ty),
            .frac_f32_lit,
            .frac_f64_lit,
            .str_lit,
            .bool_lit,
            ...
            .tag => |tag| blk: {
                ...
                break :blk try self.refineSingletonTagSourceTypeIfAbstract(
                    inst.types,
                    current_source_ty,
                    tag.name,
                    tag_arg_tys,
                ...
                if (box_boundary) |op| {
                    ...
                    switch (op) {
                        ...
                        .box_box => {
                            ...
                            const boxed_payload_ty = self.solvedBoxElemType(inst.types, call_result_ty) orelse
                                debugPanic("lambdamono.lower.refinedSourceTypeForExpr box_box missing solved payload type");
                            try self.unifyIn(inst.types, boxed_payload_ty, refined_arg_ty);
                            break :blk call_result_ty;
                        },
                        else => unreachable,
                    }
                }
            ...
            .low_level => |ll| blk: {
                ...
                const expr_result_ty = try self.explicitExprSourceType(inst, venv, expr_id);
                const constraint_fn_ty = try self.instantiateConstraintFnTypeForExprs(
                    inst,
                    venv,
                    arg_expr_ids,
    ...
    fn refinedSingletonTagSourceType(
        self: *Lowerer,
        solved_types: *solved.Type.Store,
        current_source_ty: TypeVarId,
        tag_name: base.Ident.Idx,
        ...
        const id = solved_types.unlinkPreservingNominal(current_source_ty);
        return switch (solved_types.getNode(id)) {
            .nominal => |nominal| blk: {
                const refined_backing = try self.refinedSingletonTagSourceType(
                    solved_types,
                    nominal.backing,
                    tag_name,
                    arg_tys,
                );
                if (refined_backing != nominal.backing) {
                    var rewritten = nominal;
                    rewritten.backing = refined_backing;
                    solved_types.setNode(id, .{ .nominal = rewritten });
                }
                break :blk id;
            },
            .unbd, .flex_for_a => blk: {
                const singleton = try solved_types.freshContent(.{ .tag_union = .{
                    .tags = try solved_types.addTags(&.{.{
                        .name = tag_name,
                        .args = try solved_types.addTypeVarSpan(arg_tys),
                    }}),
                } });
                solved_types.setNode(id, .{ .link = singleton });
                break :blk id;
            },
            .for_a => debugPanic("lambdamono.lower.refinedSingletonTagSourceType generalized type without instantiation"),
            .content => |content| switch (content) {
                .tag_union => blk: {
                    solved_types.setNode(id, .{ .content = .{ .tag_union = .{
                        .tags = try solved_types.addTags(&.{.{
                            .name = tag_name,
                            .args = try solved_types.addTypeVarSpan(arg_tys),
                        }}),
                    ...
                    "lambdamono.lower.refinedSingletonTagSourceType incompatible source type: {s}",
                    .{@tagName(content)},
                ),
            },
            .link => unreachable,
    ...
    fn refineSingletonTagSourceTypeIfAbstract(
        self: *Lowerer,
        solved_types: *solved.Type.Store,
        current_source_ty: TypeVarId,
        tag_name: base.Ident.Idx,
        ...
        if (!self.sourceTypeIsAbstract(solved_types, current_source_ty)) {
            return current_source_ty;
        }
        return try self.refinedSingletonTagSourceType(
            solved_types,
            current_source_ty,
            tag_name,
            arg_tys,
    ...
    fn applyExactTopLevelFunctionCall(
        ...
        const final_result_exec_ty = if (!self.executableTypeIsAbstract(expected_result_exec_ty))
            try self.strengthenErasedCallableExpectedTypeFromCurrent(
                proc_ret_exec_ty,
                self.requireConcreteExecutableType(
                    expected_result_exec_ty,
                    "applyExactTopLevelFunctionCall",
                ),
            )
        else
            proc_ret_exec_ty;
    ...
    fn specializeStructuralEqExpr(
        ...
        const operand_exec_ty = if (!self.executableTypeIsAbstract(operand_default_ty))
            ...
            receiver_exec_ty
        else if (try self.commonExecutableType(receiver_exec_ty, arg_exec_ty)) |merged|
            merged
        else
    ...
    fn specializeCallExpr(
        ...
        if (direct_func_symbol) |exact_symbol| {
            const exact_capture_symbols: []const Symbol = if (specializations.lookupFnExact(self.fenv, exact_symbol)) |exact_entry|
                ...
                &.{};
            if (specializations.lookupFnExact(self.fenv, exact_symbol) != null or self.isHostedTopLevelSymbol(exact_symbol)) {
                if (self.envHasAllCaptureSymbols(venv, exact_capture_symbols)) {
                    const explicit_call_relation_ty = try self.instantiateExplicitConstraintFnTypeForExprs(
                        inst,
                        venv,
                    ...
                    const direct_exec_ret_ty = self.requestedConcreteSpecializationReturnType(
                        expected_exec_ty,
                    ...
                    for (exact_arg_tys, call_arg_tys, arg_expr_ids, 0..) |exact_arg_ty, call_arg_ty, arg_expr_id, i| {
                        ...
                        const authoritative_arg = self.authoritativeCallableValue(lowered_args[i]);
                        lowered_args[i] = authoritative_arg.expr;
        ...
        const func_default_source_ty = call_relation_ty;
        var func_default_ty = try self.lowerExecutableTypeFromSolvedIn(
            inst.types,
            mono_cache,
            func_default_source_ty,
        );
        if (self.executableTypeIsAbstract(func_default_ty)) {
            if (try self.concretizeCallableExecutableTypeForCall(
                inst,
                mono_cache,
                venv,
                func_source_ty,
                ...
                func_default_ty = concrete_callable_exec_ty;
            }
        }
        ...
        var authoritative_callable = self.authoritativeCallableValue(lowered_func);
        lowered_func = authoritative_callable.expr;
        var lowered_func_exec_ty = authoritative_callable.ty;
        if (self.executableTypeIsAbstract(lowered_func_exec_ty)) {
            const concrete_callable_exec_ty = try self.concretizeCallableExecutableTypeForCall(
                ...
                debugPanicFmt(
                    "lambdamono.lower.specializeCallExpr callable value lowered abstract direct_symbol={?d} func_expr={d} func_var={?d} default_exec={s} lowered_exec={s}",
                    .{
            ...
            lowered_func = try self.retargetOrEmitExplicitBridgeExpr(
                lowered_func,
                concrete_callable_exec_ty,
            );
            authoritative_callable = self.authoritativeCallableValue(lowered_func);
            lowered_func = authoritative_callable.expr;
            lowered_func_exec_ty = authoritative_callable.ty;
        }
        if (self.executableTypeIsAbstract(lowered_func_exec_ty)) {
            const lowered_summary = self.debugExecutableTypeSummary(lowered_func_exec_ty);
            defer lowered_summary.deinit(self.allocator);
            debugPanicFmt(
                "lambdamono.lower.specializeCallExpr callable value remained abstract after concretization func_expr={d} lowered_exec={s}",
                .{
                    @intFromEnum(call.func),
                    lowered_summary.text,
                },
