//! Representation planning from lambdasolved to executable lambdamono plan.
...
pub const Input = solved.Lower.Result;
pub const ExecPlan = plan_mod.ExecPlan;

/// Plan a solved program into executable lambdamono form.
pub fn build(allocator: std.mem.Allocator, input: *Input) std.mem.Allocator.Error!ExecPlan {
    return buildWithEntrypoints(allocator, input, &.{});
}

/// Plan a solved program while generating wrappers for the requested entrypoints.
pub fn buildWithEntrypoints(
    allocator: std.mem.Allocator,
    input: *Input,
    entrypoints: []const Symbol,
) std.mem.Allocator.Error!ExecPlan {
    var planner = Planner.init(allocator, try input.take(allocator), entrypoints);
    defer planner.deinit();
    if (entrypoints.len > 0) {
        planner.entrypoint_wrappers = try allocator.alloc(Symbol, entrypoints.len);
        for (planner.entrypoint_wrappers) |*slot| {
            slot.* = Symbol.none;
        }
    }
    try planner.lowerProgram();
    var explicit_layouts = try layouts_mod.Layouts.initEmpty(allocator, &planner.output);
    errdefer {
        explicit_layouts.deinit(allocator);
    }
    try planner.finalizeLayouts(&explicit_layouts);
    var result = try planner.finish();
...
const Planner = struct {
    ...
    value_facts: std.ArrayList(ValueFact),
    callable_facts: std.ArrayList(CallableFact),
    expr_facts: std.AutoHashMap(ast.ExprId, ValueFactId),
    ...
    fn init(allocator: std.mem.Allocator, input: solved.Lower.Result, entrypoints: []const Symbol) Planner {
        var lowerer = Planner{
            .allocator = allocator,
            .input = input,
            .entrypoints = entrypoints,
            .output = plan_mod.Store.init(allocator),
            .root_defs = .empty,
            .types = undefined,
            ...
            .value_facts = .empty,
            .callable_facts = .empty,
            .expr_facts = std.AutoHashMap(ast.ExprId, ValueFactId).init(allocator),
            .entrypoint_wrappers = &.{},
    ...
    fn deinit(self: *Planner) void {
        for (self.callable_facts.items) |fact| {
            if (fact.captures.len != 0) self.allocator.free(fact.captures);
    ...
    fn finish(self: *Planner) std.mem.Allocator.Error!ExecPlan {
        ...
        self.output = plan_mod.Store.init(self.allocator);
    ...
    fn finalizeTypedSymbol(self: *Planner, bind: *ast.TypedSymbol) std.mem.Allocator.Error!void {
    ...
    fn envExecutableType(self: *const Planner, entry: EnvEntry) type_mod.TypeId {
        return self.valueFact(entry.fact).exec_ty;
    }

    fn envCallable(self: *const Planner, entry: EnvEntry) ?CallableFactId {
        return self.valueFact(entry.fact).callable;
    }
    ...
    fn addEnvEntry(
        ...
        return .{
            .symbol = symbol,
            .fact = try self.addValueFact(symbol, source_ty, exec_ty, callable),
        };
    }
    ...
    fn addCallableFact(
        ...
        captures: []const ValueFactId,
        call_sig: type_mod.CallableSig,
    ) std.mem.Allocator.Error!CallableFactId {
        const owned_captures = try self.allocator.dupe(ValueFactId, captures);
        errdefer if (owned_captures.len != 0) self.allocator.free(owned_captures);
        const owned_args = try self.types.dupeTypeIds(call_sig.args);
        errdefer if (owned_args.len != 0) self.allocator.free(owned_args);
        const index: u32 = @intCast(self.callable_facts.items.len);
        try self.callable_facts.append(self.allocator, .{
            .target = target,
            .repr_mode = repr_mode,
            .source_ty = source_ty,
            .exec_ty = exec_ty,
    ...
    fn callableCapturesForExprFact(
        self: *const Planner,
        expr_id: ast.ExprId,
        target: Symbol,
    ) []const ValueFactId {
    ...
    fn finalizeStmt(self: *Planner, stmt: *ast.Stmt) std.mem.Allocator.Error!void {
        switch (stmt.*) {
    ...
    fn finalizeDef(self: *Planner, def: *ast.Def) std.mem.Allocator.Error!void {
        if (def.result_ty) |ty| {
            def.result_ty = try self.internExecutableType(ty);
        }
    }

    fn finalizeLayouts(self: *Planner, layouts: *layouts_mod.Layouts) std.mem.Allocator.Error!void {
        ...
        for (self.output.defs.items, 0..) |*def, i| {
            ...
            const ret_ty = switch (def.value) {
                .fn_ => def.result_ty orelse
                    debugPanic("lambdamono.exec_plan.finalizeLayouts fn def missing explicit result type"),
                .hosted_fn => def.result_ty orelse
                    debugPanic("lambdamono.exec_plan.finalizeLayouts hosted def missing explicit result type"),
                .val => def.result_ty orelse
                    debugPanic("lambdamono.exec_plan.finalizeLayouts val def missing explicit result type"),
                .run => def.result_ty orelse
                    debugPanic("lambdamono.exec_plan.finalizeLayouts run def missing explicit result type"),
            };
    ...
    fn lowerProgram(self: *Planner) std.mem.Allocator.Error!void {
    ...
    fn lookupPendingBySpecializedSymbol(self: *Planner, symbol: Symbol) ?*specializations.Pending {
    ...
    fn executableTypeIsAbstract(self: *const Planner, ty: type_mod.TypeId) bool {
        return self.types.containsAbstractLeaf(ty);
    }

    fn executableTypeHasLayoutAbstractLeaf(self: *const Planner, ty: type_mod.TypeId) bool {
    ...
    fn lowerRequestedExecutableReturnTypeFromLowLevelArgs(
        ...
        if (source_arg_tys.len != args.len) {
            debugPanic("lambdamono.exec_plan.lowerRequestedExecutableReturnTypeFromLowLevelArgs arg arity mismatch");
        }
    ...
    fn concreteExecutableTypeForNumericLiteral(
        ...
        switch (inst.types.getNode(source_root)) {
            .for_a, .flex_for_a, .unbd => return try self.makePrimitiveType(.dec),
            else => {},
        }
    ...
    fn sourceTypeIsAbstract(self: *Planner, solved_types: *const solved.Type.Store, ty: TypeVarId) bool {
        var visited = std.AutoHashMap(TypeVarId, void).init(self.allocator);
        defer visited.deinit();
    ...
    fn requireConcreteExecutableTypeForBinding(
        ...
        if (self.sourceTypeIsAbstract(inst.types, bind_ty)) {
            ...
            debugPanicFmt(
                "lambdamono.exec_plan.{s} expected concrete binding source type before executable lowering: {s}",
                .{ context, source_key },
            );
        }
    ...
    fn requireConcreteExecutableType(
        ...
        debugPanicFmt(
            "lambdamono.exec_plan.{s} expected concrete executable type, found {s}",
            .{ context, ty_text.text },
        );
    }

    fn freshAbstractExecutableType(self: *Planner) std.mem.Allocator.Error!type_mod.TypeId {
    ...
    fn currentEnvEntryExecutableType(
        ...
        _ = solved_types;
        _ = context;
        return self.envExecutableType(entry);
    }
    ...
    fn promotePendingSummary(self: *Planner, pending: *specializations.Pending) std.mem.Allocator.Error!void {
        if (pending.summary_types != null) return;
        const requested_types = pending.requested_types orelse
            debugPanic("lambdamono.exec_plan.promotePendingSummary missing requested specialization store");
        const requested_ty = pending.requested_ty orelse
            debugPanic("lambdamono.exec_plan.promotePendingSummary missing requested specialization type");
    ...
    fn requirePendingExecutableSignature(
        ...
        return .{
            ...
            .args_tys = pending.exec_args_tys orelse
                debugPanic("lambdamono.exec_plan.requirePendingExecutableSignature missing executable arg signature"),
            .ret_ty = pending.exec_ret_ty,
        };
    }

    fn validatePendingExecutableSignature(self: *const Planner, pending: *const specializations.Pending) void {
        const signature = self.requirePendingExecutableSignature(pending);
        if (signature.capture_ty) |capture_ty| {
            if (self.executableTypeHasLayoutAbstractLeaf(capture_ty)) {
    ...
    fn ensurePendingExecutableArgTypes(
        ...
        for (exec_args) |arg_ty| {
            if (self.executableTypeHasLayoutAbstractLeaf(arg_ty)) {
                debugPanic("lambdamono.exec_plan.ensurePendingExecutableArgTypes abstract executable arg signature");
            }
        }
        return exec_args;
    }
    ...
    fn ensurePendingExecutableReturnType(
        ...
        try self.refreshPendingExecutableReturnTypeFromSummary(pending);
        if (self.executableTypeHasLayoutAbstractLeaf(pending.exec_ret_ty)) {
            return self.requireConcreteExecutableType(
                pending.exec_ret_ty,
                "ensurePendingExecutableReturnType",
            );
        }
        return pending.exec_ret_ty;
    }

    fn currentPendingExecutableReturnType(
        self: *Planner,
        pending: *specializations.Pending,
    ) std.mem.Allocator.Error!type_mod.TypeId {
        try self.refreshPendingExecutableReturnTypeFromSummary(pending);
    ...
    fn absorbExplicitBodyReturnExecutableType(
        ...
        const pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
            debugPanic("lambdamono.exec_plan.absorbExplicitBodyReturnExecutableType missing pending specialization");
        const current_ret_ty = pending.exec_ret_ty;
        if (self.types.equalIds(current_ret_ty, body_ty)) return;
        if (self.executableTypeIsAbstract(current_ret_ty) and !self.executableTypeIsAbstract(body_ty)) {
        ...
        switch (pending.source_def) {
            ...
            .fn_ => {
                const summary_types = pending.summary_types orelse
                    debugPanic("lambdamono.exec_plan.preparePendingSummary missing specialization summary store");
                const summary_fn_ty = pending.summary_fn_ty orelse
                    debugPanic("lambdamono.exec_plan.preparePendingSummary missing specialization summary type");
                ...
                pending.summary_fn_source_ty = try self.cloneTypeFromStoreRec(
                    summary_types,
                    &self.input.types,
                    &exact_mapping,
                    pending.fn_ty,
                );
                if (pending.summary_captures) |captures| {
                ...
                const source_summary_fn_ty = pending.summary_fn_source_ty orelse
                    debugPanic("lambdamono.exec_plan.preparePendingSummary missing cloned source specialization type");
    ...
    fn refreshPendingExecutableReturnTypeFromSummary(
        ...
        const summary_fn_ty = pending.summary_fn_ty orelse
            debugPanic("lambdamono.exec_plan.refreshPendingExecutableReturnTypeFromSummary missing specialization summary type");
        var mono_cache = lower_type.MonoCache.init(self.allocator);
        defer mono_cache.deinit();
        const summary_shape = summary_types.fnShape(summary_fn_ty);
        pending.exec_ret_ty = try self.executableTypeFromSourceWithNumericDefault(
            summary_types,
            &mono_cache,
            summary_shape.ret,
            pending.repr_mode,
    ...
    fn refreshPendingExecutableCaptureTypeFromSummary(
        ...
        const captures = pending.summary_captures orelse
            debugPanic("lambdamono.exec_plan.refreshPendingExecutableCaptureTypeFromSummary missing exact specialization captures");
        if (captures.len == 0) {
        ...
        var mono_cache = lower_type.MonoCache.init(self.allocator);
        defer mono_cache.deinit();
        const lowered = try self.lowerCaptureRecordTypeFromSolved(
            summary_types,
            &mono_cache,
            captures,
        );
        pending.exec_capture_ty = if (self.executableTypeHasLayoutAbstractLeaf(lowered))
    ...
    fn ensureQueuedCallableSpecializedWithExecSignature(
        ...
        const pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
            debugPanic("lambdamono.exec_plan.ensureQueuedCallableSpecializedWithExecSignature missing queued specialization");
        try self.preparePendingSummary(pending);
        return try self.ensureQueuedCallableSpecialized(pending_symbol);
    }
    ...
    fn ensureExactCallableSpecializedForRequestedType(
        ...
        return try self.ensureQueuedCallableSpecializedForRequestedType(
            ...
            mono_cache,
            requested_name,
            self.callableReprMode(requested_name),
            requested_ty,
            capture_facts,
            capture_exec_ty,
            &.{},
    ...
    fn ensureQueuedCallableSpecialized(
        ...
        const pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
            debugPanic("lambdamono.exec_plan.ensureQueuedCallableSpecialized missing queued specialization");
        try self.preparePendingSummary(pending);
            ...
            .done, .specializing => {
                ...
                return .{
                    ...
                    .summary_fn_ty = pending.summary_fn_ty orelse
                        debugPanic("lambdamono.exec_plan.ensureQueuedCallableSpecialized specialization missing summary type"),
            ...
            .pending => {
                ...
                return .{
                    ...
                    .summary_fn_ty = updated.summary_fn_ty orelse
                        debugPanic("lambdamono.exec_plan.ensureQueuedCallableSpecialized completed specialization missing summary type"),
    ...
    fn finishPendingSpecialization(
        ...
        const updated = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
            debugPanic("lambdamono.exec_plan.finishPendingSpecialization lost queued specialization after growth");
    ...
    fn seedEntrypointSpecializations(self: *Planner) std.mem.Allocator.Error!void {
        ...
        for (self.entrypoints) |entry_symbol| {
            ...
            switch (self.input.types.lambdaRepr(entry.fn_ty)) {
                .erased => debugPanic("lambdamono.exec_plan entrypoint specialization expected lambda set"),
                .lset => {
    ...
    fn exprContainsReturn(self: *const Planner, expr_id: solved.Ast.ExprId) bool {
        const expr = self.input.store.getExpr(expr_id);
        return switch (expr.data) {
            .var_,
            .int_lit,
    ...
    fn stmtContainsReturn(self: *const Planner, stmt: solved.Ast.Stmt) bool {
        return switch (stmt) {
        ...
        return switch (expr.data) {
            ...
            .access => |access| {
                ...
                return self.solvedRecordFieldByName(inst.types, record_ty, access.field) orelse
                    debugPanic("lambdamono.exec_plan.explicitExprSourceType missing solved record field");
            },
            .tuple_access => |access| {
                ...
                return self.solvedTupleElemType(inst.types, tuple_ty, access.elem_index) orelse
                    debugPanic("lambdamono.exec_plan.explicitExprSourceType missing solved tuple element");
            },
    ...
    fn buildEntrypointWrappers(self: *Planner) std.mem.Allocator.Error!void {
        if (self.entrypoints.len == 0) return;
        ...
        for (self.entrypoints, 0..) |entry_symbol, entry_idx| {
            ...
            const entry_fn_ty = if (specializations.lookupFnExact(self.fenv, entry_symbol)) |entry|
                ...
                switch (source) {
                    .fn_ => debugPanic("lambdamono.exec_plan.lowerProgram expected entrypoint function symbol to be specialized"),
                    .hosted_fn => debugPanic("lambdamono.exec_plan.lowerProgram expected entrypoint hosted function symbol to be specialized"),
            ...
            if (specializations.lookupFnExact(self.fenv, entry_symbol)) |_| {
                ...
                if (!entry_is_callable or arg_count == 0) {
                    ...
                    current_expr = try self.applyCallableValueCall(
                        ...
                        arg_exprs,
                        final_ret_exec_ty,
                        self.callableTargetForBinding(entry_symbol),
                    );
                }
            }
            ...
            const def_id = try self.output.addDef(.{
                .bind = wrapper_symbol,
                .result_ty = self.output.getExpr(current_expr).ty,
                .value = .{ .fn_ = .{
                    .args = args_span,
                    .body = current_expr,
                } },
    ...
    fn moveEntrypointRootsToEnd(self: *Planner) std.mem.Allocator.Error!void {
    ...
    fn isEntrypointRootSymbol(self: *const Planner, symbol: Symbol) bool {
    ...
    fn applyCallableValueCall(
        ...
        callable_target: ?Symbol,
    ) std.mem.Allocator.Error!ast.ExprId {
        const solved_types = inst.types;
        self.assertCallableExprMatchesSolvedType(solved_types, func_expr, func_source_ty);
        const authoritative_func = self.authoritativeCallableValue(func_expr);
        const func_exec_ty = authoritative_func.ty;
        const func_call_expr = authoritative_func.expr;
        ...
        if (param_source_tys.len != arg_exprs.len or arg_source_tys.len != arg_exprs.len) {
            debugPanic("lambdamono.exec_plan.applyCallableValueCall call arg arity mismatch");
        }
        ...
        return switch (self.types.getType(func_exec_ty)) {
            .erased_fn => blk: {
                ...
                switch (solved_types.lambdaRepr(func_source_ty)) {
                    .lset => |lambda_members| {
                        if (callable_target) |target_symbol| {
                            const capture_facts = self.callableCapturesForExprFact(
                                func_call_expr,
                                target_symbol,
                            );
                            const specialized = try self.ensureQueuedCallableSpecializedForRequestedType(
                            ...
                            if (specialized.exec_capture_ty != func_capture_ty) {
                                ...
                                const direct_entry = self.input.symbols.get(target_symbol);
                                debugPanicFmt(
                                    ...
                                    .{
                                        ...
                                        if (callable_target) |symbol| symbol.raw() else null,
            ...
            .tag_union => |tag_union| blk: {
                ...
                const requested_exec_ret_ty = if (concrete_result_exec_ty) |result_ty|
                    ...
                    try self.executableReturnTypeFromCallRelation(
                        solved_types,
                        mono_cache,
                        param_source_tys,
                        actual_arg_exec_tys,
                ...
                const specialized_branches = try self.allocator.alloc(SpecializedCallableSummary, lambda_members.len);
                ...
                for (lambda_members, 0..) |lambda_member, i| {
                    ...
                    if (common_exec_arg_tys) |expected_arg_tys| {
                        if (specialized_branches[i].exec_args_tys.len != expected_arg_tys.len) {
                            debugPanic("lambdamono.exec_plan.applyCallableValueCall branch arg arity mismatch across specializations");
                        }
                        for (specialized_branches[i].exec_args_tys, expected_arg_tys) |actual_arg_ty, expected_arg_ty| {
                            if (!self.types.equalIds(actual_arg_ty, expected_arg_ty)) {
                                debugPanic("lambdamono.exec_plan.applyCallableValueCall branch arg executable type mismatch across specializations");
                            }
                        }
                    } else {
                        common_exec_arg_tys = specialized_branches[i].exec_args_tys;
                    }
                    if (common_exec_ret_ty) |expected_ret_ty| {
                        if (!self.types.equalIds(specialized_branches[i].exec_ret_ty, expected_ret_ty)) {
                            debugPanic("lambdamono.exec_plan.applyCallableValueCall branch return executable type mismatch across specializations");
                        }
                    } else {
                        common_exec_ret_ty = specialized_branches[i].exec_ret_ty;
                    }
                }
                ...
                const branch_call_args = try self.bridgeCallArgsToExpectedExecutableTypes(
                    ...
                    arg_source_tys,
                    arg_exprs,
                    common_exec_arg_tys orelse exec_call_sig.args,
                );
                ...
                for (lambda_members, 0..) |lambda_member, i| {
                    ...
                    if (lambda_member.capture_ty == null and current_member.captures.len != 0) {
                        debugPanic("lambdamono.exec_plan.applyCallableValueCall missing executable capture payload for lambda member");
                    }
                    const specialized = specialized_branches[i];
    ...
    fn indexTopLevelValues(self: *Planner) std.mem.Allocator.Error!void {
        for (self.input.store.defsSlice()) |def| {
            switch (def.value) {
    ...
    fn boxBoundaryResultTypeFromArg(
        ...
        return switch (op) {
            ...
            .box_unbox => switch (self.types.getTypePreservingNominal(arg_ty)) {
                .box => |elem| elem,
                else => debugPanic("lambdamono.exec_plan.boxBoundaryResultTypeFromArg box_unbox expected boxed executable arg"),
            },
            else => unreachable,
        };
    }
    ...
    fn instantiateConstraintFnTypeForExprs(
        ...
        if (constraint_arg_tys.len != arg_expr_ids.len) {
            debugPanic("lambdamono.exec_plan.instantiateConstraintFnTypeForExprs arg arity mismatch");
        }
    ...
    fn lowerBoxBoundaryCallableTypeIn(
        ...
        return switch (solved_types.getNode(id)) {
            ...
            .content => |content| switch (content) {
                ...
                .lambda_set => debugPanic("lambdamono.exec_plan.lowerBoxBoundaryCallableType bare lambda_set survived to callable boundary lowering"),
            ...
            .for_a,
            .flex_for_a,
            => debugPanic("lambdamono.exec_plan.lowerBoxBoundaryCallableType unbound type survived instantiation"),
        };
    }
    ...
    fn lowerBoxedBoundaryCallableTypeIn(
        ...
        return switch (solved_types.getNode(id)) {
            ...
            .for_a,
            .flex_for_a,
            => debugPanic("lambdamono.exec_plan.lowerBoxedBoundaryCallableType unbound type survived instantiation"),
        };
    }
    ...
    fn eraseBoundaryExecutableType(
        ...
        return switch (self.types.getTypePreservingNominal(ty)) {
            ...
            .tag_union => |tag_union| if (self.tagUnionIsInternalLambdaSet(tag_union.tags))
                debugPanic("lambdamono.exec_plan.eraseBoundaryExecutableType missing explicit erased callable type")
            else
                ty,
            .erased_fn => ty,
            else => ty,
    ...
    fn unlinkExecutableTypeVar(self: *Planner, solved_types: *solved.Type.Store, ty: TypeVarId) TypeVarId {
        const id = solved_types.unlinkPreservingNominal(ty);
        return switch (solved_types.getNode(id)) {
            .nominal => id,
            .content => |content| switch (content) {
    ...
    fn lambdaSetIsErased(_: *Planner, solved_types: *solved.Type.Store, lset: TypeVarId) bool {
    ...
    fn internExecutableType(self: *Planner, ty: type_mod.TypeId) std.mem.Allocator.Error!type_mod.TypeId {
    ...
    fn boxedPayloadType(self: *Planner, solved_types: *solved.Type.Store, ty: TypeVarId) ?TypeVarId {
        const id = solved_types.unlinkPreservingNominal(ty);
        return switch (solved_types.getNode(id)) {
    ...
    fn boxPayloadRequiresErasure(self: *Planner, solved_types: *solved.Type.Store, ty: TypeVarId) bool {
    ...
    fn orderedRecordFields(
        ...
        for (fields, 0..) |field, i| {
            const field_info = self.recordFieldByName(record_ty, field.name) orelse
                debugPanic("lambdamono.exec_plan.record missing field in record type");
            ordered[i] = .{
    ...
    fn executableCallableSig(self: *const Planner, ty: type_mod.TypeId) ?type_mod.CallableSig {
        return switch (self.types.getTypePreservingNominal(ty)) {
    ...
    fn requireExecutableCallableSig(
        ...
        return self.executableCallableSig(ty) orelse
            debugPanicFmt(
                "lambdamono.exec_plan.{s} expected callable executable signature on type {d}",
                .{ context, @intFromEnum(ty) },
            );
    }

    fn isNaturalExecutableCallableType(self: *const Planner, ty: type_mod.TypeId) bool {
        return switch (self.types.getTypePreservingNominal(ty)) {
    ...
    fn erasedFnCaptureType(self: *Planner, ty: type_mod.TypeId) ?type_mod.TypeId {
        var visited = std.AutoHashMap(type_mod.TypeId, void).init(self.allocator);
        defer visited.deinit();
    ...
    fn ensureTopLevelValueLowered(self: *Planner, symbol: Symbol) std.mem.Allocator.Error!type_mod.TypeId {
        ...
        const source = self.top_level_values.get(source_symbol) orelse
            debugPanic("lambdamono.exec_plan.ensureTopLevelValueLowered missing top-level value");

        const expr_id: solved.Ast.ExprId = switch (source) {
            .fn_ => debugPanic("lambdamono.exec_plan.ensureTopLevelValueLowered expected value source, found function"),
            .hosted_fn => debugPanic("lambdamono.exec_plan.ensureTopLevelValueLowered expected value source, found hosted function"),
    ...
    fn rootSourceSymbol(self: *const Planner, symbol: Symbol) Symbol {
        var current = symbol;
        while (true) {
    ...
    fn lowerBindingBody(
        ...
        const bind_ty = blk: {
                ...
                self.sourceTypeIsAbstract(inst.types, cloned))
            {
                ...
                std.debug.panic(
                    "lambdamono.exec_plan.{s} concrete source bind cloned abstract bind={s} source={s} cloned={s}",
    ...
    fn lowerBindingBodyAtSourceTy(
        ...
        if (!self.sourceTypeIsAbstract(inst.types, bind_ty)) {
            ...
            const lowered_body = try self.planExprWithRequiredExecTyAndSourceTy(
                inst,
                mono_cache,
                venv,
                body_expr_id,
            ...
            var lowered_body_expr = lowered_body.expr;
            var lowered_exec_ty = self.requireConcreteExecutableType(
                self.output.getExpr(lowered_body_expr).ty,
                context,
            );
            const authoritative = self.authoritativeCallableValue(lowered_body_expr);
            if (self.isExecutableCallableType(authoritative.ty)) {
                lowered_body_expr = authoritative.expr;
                lowered_exec_ty = self.requireConcreteExecutableType(authoritative.ty, context);
            }
            return .{
                .source_ty = bind_ty,
                .exec_ty = lowered_exec_ty,
                .body = lowered_body_expr,
            };
        }
        ...
        var lowered_body_expr = lowered_body.expr;
        var lowered_exec_ty = self.output.getExpr(lowered_body_expr).ty;
        const authoritative = self.authoritativeCallableValue(lowered_body_expr);
        if (self.isExecutableCallableType(authoritative.ty)) {
            lowered_body_expr = authoritative.expr;
            lowered_exec_ty = authoritative.ty;
        }
    ...
    fn preRefineBindingInEnv(
        ...
        return try self.addEnvEntry(
            bind.symbol,
            bind_ty,
            try self.lowerExecutableTypeFromSolvedIn(inst.types, mono_cache, bind_ty),
            null,
        );
    }
        ...
        return switch (stmt) {
            ...
            .reassign => |reassign| {
                ...
                if (self.lookupEnvEntry(venv, reassign.target)) |target_entry| {
                    ...
                    try self.unifyIn(inst.types, self.envSourceType(target_entry), body_source_ty);
                }
            ...
            .for_ => |for_stmt| {
                ...
                const elem_ty = self.solvedListElemType(inst.types, iterable_ty) orelse
                    debugPanic("lambdamono.exec_plan.preRefineStmtSourceTypes expected list iterable source type");
    ...
    fn preRefineExprSourceTypes(
        ...
        switch (expr.data) {
            ...
            .for_ => |for_expr| {
                ...
                const elem_ty = self.solvedListElemType(inst.types, iterable_ty) orelse
                    debugPanic("lambdamono.exec_plan.preRefineExprSourceTypes expected list iterable source type");
                try self.unifyIn(inst.types, pat_ty, elem_ty);
                const additions = try self.collectPatBindingsAtSourceTy(inst, mono_cache, source_pat, pat_ty);
                defer self.allocator.free(additions);
                const body_env = try self.concatEnv(venv, additions);
    ...
    fn refineCallArgSourceTypesIn(
        ...
        if (arg_expr_ids.len != expected_arg_tys.len) {
            debugPanic("lambdamono.exec_plan.refineCallArgSourceTypesIn arg arity mismatch");
        }
        var changed = true;
        while (changed) {
            changed = false;
            ...
            .content => |content| switch (content) {
                .primitive => |prim| .{
                    ...
                    .type_ident = switch (prim) {
                        ...
                        .erased => debugPanic(
                            "lambdamono.exec_plan.maybeAttachedMethodOwnerForType reached erased primitive dispatcher",
                        ),
                    },
                },
                .list => .{
        ...
        return self.maybeAttachedMethodOwnerForType(solved_types, dispatcher_ty) orelse {
            ...
            switch (solved_types.getNode(dispatcher_id)) {
                .content => |content| {
                    ...
                    debugPanicFmt(
                        "lambdamono.exec_plan.attachedMethodOwnerForType expected monomorphic nominal/list/box dispatcher for method {s}: node={s} key={s}",
                    ...
                    defer if (!std.mem.eql(u8, key, "<oom>")) solved_types.allocator.free(key);
                    debugPanicFmt(
                        "lambdamono.exec_plan.attachedMethodOwnerForType expected monomorphic dispatcher type for method {s}: node={s} key={s}",
    ...
    fn resolveAttachedMethodTargetFromExpr(
        ...
        const raw_symbol = self.input.attached_method_index.get(.{
            ...
            "lambdamono.exec_plan.resolveAttachedMethodTargetFromExpr missing attached method for monomorphic dispatcher",
        );
        return self.callableTargetForBinding(raw_symbol) orelse debugPanicFmt(
            "lambdamono.exec_plan.resolveAttachedMethodTargetFromExpr attached method {s} resolved to binding {d} but not an exact callable",
    ...
    fn resolveAttachedMethodTarget(
        ...
        const raw_symbol = self.input.attached_method_index.get(.{
            ...
            "lambdamono.exec_plan.resolveAttachedMethodTarget missing attached method for monomorphic dispatcher",
        );
        return self.callableTargetForBinding(raw_symbol) orelse debugPanicFmt(
            "lambdamono.exec_plan.resolveAttachedMethodTarget attached method {s} resolved to binding {d} but not an exact callable",
            .{ self.input.idents.getText(method_name), raw_symbol.raw() },
        );
    }

    fn specializeFn(self: *Planner, pending_symbol: Symbol) std.mem.Allocator.Error!SpecializedDef {
        const pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
            debugPanic("lambdamono.exec_plan.specializeFn missing queued specialization");
        const source_def = pending.source_def;
        const repr_mode = pending.repr_mode;
        const capture_facts = pending.capture_facts;
        ...
        switch (source_def) {
            .hosted_fn => |hosted_fn| {
                ...
                if (hosted_args.len != proc_arg_exec_tys.len) {
                    debugPanic("lambdamono.exec_plan.specializeFn hosted arg arity mismatch");
                }
            ...
            .fn_ => |fn_def| {
                const summary_types = pending.summary_types orelse
                    debugPanic("lambdamono.exec_plan.specializeFn missing specialization summary store");
                const summary_fn_ty = pending.summary_fn_ty orelse
                    debugPanic("lambdamono.exec_plan.specializeFn missing specialization summary type");
                const summary_captures = pending.summary_captures orelse
                    debugPanic("lambdamono.exec_plan.specializeFn missing exact specialization captures");
                var inst = InstScope.borrow(self.allocator, summary_types);
                defer inst.deinit();
                var mono_cache = lower_type.MonoCache.init(self.allocator);
                defer mono_cache.deinit();
                const fn_body = fn_def.body;
                const frozen_captures = summary_captures;
                    ...
                    .natural => if (frozen_captures.len == 0) blk_fn: {
                        ...
                        const body = blk_body: {
                            const pre_body_pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
                                debugPanic("lambdamono.exec_plan.specializeFn lost pending specialization before reading natural return signature");
                            const pre_lower_ret_ty = try self.currentPendingExecutableReturnType(pre_body_pending);
                            ...
                            const post_body_pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
                                debugPanic("lambdamono.exec_plan.specializeFn lost pending specialization after lowering natural body");
                        ...
                        const capture_exec_ty = initial_proc_capture_ty orelse
                            debugPanic("lambdamono.exec_plan.specializeFn missing queued executable capture signature");
                        ...
                        try self.preRefineExprSourceTypes(&inst, &mono_cache, provisional_body_env, fn_body);
                        const pre_body_pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
                            debugPanic("lambdamono.exec_plan.specializeFn lost pending specialization before reading captured natural return signature");
                        const pre_lower_ret_ty = try self.currentPendingExecutableReturnType(pre_body_pending);
                        ...
                        const post_body_pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
                            debugPanic("lambdamono.exec_plan.specializeFn lost pending specialization after lowering captured natural body");
                        const body_ty = self.output.getExpr(body).ty;
                        self.absorbExplicitBodyReturnExecutableType(pending_symbol, body_ty);
                        const authoritative_ret_ty = try self.ensurePendingExecutableReturnType(post_body_pending);
                    ...
                    .erased_boundary => blk: {
                        ...
                        if (frozen_captures.len != 0) {
                            const lowered_capture_ty = initial_proc_capture_ty orelse
                                debugPanic("lambdamono.exec_plan.specializeFn missing queued erased executable capture signature");
                        ...
                        const post_body_pending = self.lookupPendingBySpecializedSymbol(pending_symbol) orelse
                            debugPanic("lambdamono.exec_plan.specializeFn lost pending specialization after lowering erased body");
    ...
    fn buildFnBodyEnv(
        ...
        if (arg_facts.len != 0 and arg_facts.len != args.len) {
            debugPanic("lambdamono.exec_plan.buildFnBodyEnv queued arg fact arity mismatch");
        }
        ...
        for (args, arg_tys, arg_exec_tys, 0..) |arg, arg_ty, arg_exec_ty, i| {
            if (builtin.mode == .Debug) {
                if (solved_types.maybeLambdaRepr(arg_ty)) |repr| {
                    if (repr == .erased and
                        self.types.getTypePreservingNominal(arg_exec_ty) != .erased_fn)
                    {
                        debugPanicFmt(
                            "lambdamono.exec_plan.buildFnBodyEnv erased callable arg mismatch symbol={d} arg_index={d} arg_exec_ty={d} arg_exec_tag={s}",
                            .{
                                arg.symbol.raw(),
                                i,
    ...
    fn valueFactsForSymbols(
        self: *Planner,
        symbols: []const Symbol,
        venv: []const EnvEntry,
    ) std.mem.Allocator.Error![]const ValueFactId {
        if (symbols.len == 0) return &.{};
        const out = try self.allocator.alloc(ValueFactId, symbols.len);
        errdefer self.allocator.free(out);
        for (symbols, 0..) |symbol, i| {
            const entry = self.lookupEnvEntry(venv, symbol) orelse
                debugPanic("lambdamono.exec_plan.valueFactsForSymbols missing binding");
            out[i] = entry.fact;
        }
        return out;
    }
    ...
    fn valueFactsForEnv(
        ...
        if (venv.len == 0) return &.{};
        const out = try self.allocator.alloc(ValueFactId, venv.len);
        errdefer self.allocator.free(out);
        for (venv, 0..) |entry, i| {
            out[i] = entry.fact;
        }
    ...
    fn callableCapturesForTarget(
        ...
        if (specializations.lookupFnExact(self.fenv, target)) |entry| {
            return try self.valueFactsForSymbols(entry.capture_symbols, venv);
        }
        if (self.isHostedTopLevelSymbol(target)) {
            return &.{};
        }
        debugPanic("lambdamono.exec_plan.callableCapturesForTarget missing callable metadata");
    }

    fn currentCapturePayload(
        self: *Planner,
        mono_cache: *lower_type.MonoCache,
        facts: []const ValueFactId,
    ) std.mem.Allocator.Error!CurrentCapturePayload {
        if (facts.len == 0) {
            return .{ .env = &.{}, .facts = &.{}, .ty = null };
        }

        const capture_env = try self.allocator.alloc(EnvEntry, facts.len);
        errdefer self.allocator.free(capture_env);
        const capture_bindings = try self.allocator.alloc(lower_type.CaptureBinding, facts.len);
        defer self.allocator.free(capture_bindings);

        for (facts, 0..) |fact_id, i| {
            const fact = self.valueFact(fact_id);
            capture_env[i] = .{
                .symbol = fact.symbol,
                .fact = fact_id,
            };
    ...
    fn queueCallableSpecializationWithResolvedExecSignature(
        ...
        if (arg_facts.len != 0 and arg_facts.len != exec_arg_tys.len) {
            debugPanic("lambdamono.exec_plan.queueCallableSpecializationWithResolvedExecSignature queued arg fact arity mismatch");
        }
        const exec_call_sig: type_mod.CallableSig = .{ .args = exec_arg_tys, .ret = exec_ret_ty };
        const callable_exec_ty = switch (repr_mode) {
        ...
        const source = self.top_level_values.get(self.rootSourceSymbol(symbol)) orelse
            debugPanic("lambdamono.exec_plan.queueCallableSpecializationWithExplicitSignature missing top-level callable source");
        return switch (source) {
            ...
            .fn_ => debugPanic("lambdamono.exec_plan.queueCallableSpecializationWithExplicitSignature missing exact function env entry"),
            .val, .run => debugPanic("lambdamono.exec_plan.queueCallableSpecializationWithExplicitSignature expected exact callable symbol"),
        };
    }
    ...
    fn bindCaptureLets(
        ...
        while (idx > 0) {
            ...
            const field_info = self.recordFieldByName(capture_record_ty, capture_name) orelse
                debugPanic("lambdamono.exec_plan.bindCaptureLets missing capture field");
    ...
    fn planExpr(
        ...
        return try self.planExprAtSourceTy(inst, mono_cache, venv, expr_id, ty);
    }
    ...
    fn planExprAtSourceTy(
        ...
        const expr = self.input.store.getExpr(expr_id);
        const required_exec_ty = try self.explicitValueExecutableTypeForExpr(
            inst.types,
            mono_cache,
            venv,
            expr_id,
        ...
        return try self.planExprWithRequiredExecTy(inst, mono_cache, venv, expr_id, expr, ty, required_exec_ty);
    }

    fn planExprWithRequiredExecTy(
        self: *Planner,
        inst: *InstScope,
        mono_cache: *lower_type.MonoCache,
        venv: []const EnvEntry,
        ...
        return (try self.planExprWithRequiredExecTyAndSourceTy(
            inst,
            mono_cache,
            venv,
            expr_id,
    ...
    fn planExprWithRequiredExecTyAndSourceTy(
        self: *Planner,
        inst: *InstScope,
        mono_cache: *lower_type.MonoCache,
        venv: []const EnvEntry,
        ...
        if (expr.data == .inspect) {
            return .{
                .expr = try self.specializeInspectExpr(inst, mono_cache, venv, expr.data.inspect, required_exec_ty),
                .source_ty = ty,
            };
        }
        const specialized: SpecializedExprLowering = switch (expr.data) {
            ...
            .tag => |tag| blk: {
                ...
                const requested_tag_ty = if (!self.sourceTypeIsAbstract(inst.types, tag_source_ty)) blk_tag_ty: {
                    ...
                    self.requireConcreteExecutableType(required_exec_ty, "planExpr(tag)")
                else
                    null;
                const stable_tag_ty = if (requested_tag_ty) |tag_ty|
                    self.stableConcreteExpectedExecutableType(tag_ty, "planExpr(tag stable)")
                else
                    null;
                const tag_exec_info = if (requested_tag_ty) |tag_ty|
                    self.executableTagInfoByName(tag_ty, tag.name) orelse {
                        ...
                        debugPanicFmt(
                            "lambdamono.exec_plan.tag missing executable tag info tag={s} exec_ty={s}",
                ...
                if (tag_exec_info) |info| {
                    if (info.args.len != source_args.len) {
                        debugPanic("lambdamono.exec_plan.tag expected tag arg count mismatch");
                    }
                }
                ...
                const result_ty = if (source_args.len != 0)
                    ...
                    self.requireConcreteExecutableType(tag_ty, "planExpr(tag)")
                else
                    debugPanic("lambdamono.exec_plan.tag missing explicit executable tag result type");
                const final_tag_info = self.executableTagInfoByName(result_ty, tag.name) orelse {
                    ...
                    defer tag_ty_summary.deinit(self.allocator);
                    debugPanicFmt(
                        "lambdamono.exec_plan.tag missing final executable tag info tag={s} exec_ty={s}",
                        .{ self.input.idents.getText(tag.name), tag_ty_summary.text },
                    );
                };
                if (final_tag_info.args.len != source_args.len) {
                    debugPanic("lambdamono.exec_plan.tag final executable tag arg count mismatch");
                }
            ...
            .record => |fields| blk: {
                ...
                for (source_fields, 0..) |field, i| {
                    const source_field_ty = self.solvedRecordFieldByName(inst.types, ty, field.name) orelse
                        debugPanic("lambdamono.exec_plan.record missing solved record field");
                    ...
                    const expected_field_ty = if (concrete_record_ty) |record_ty|
                        (self.recordFieldByName(record_ty, field.name) orelse
                            debugPanic("lambdamono.exec_plan.record missing executable record field")).ty
                    else
                        try self.lowerExecutableTypeFromSolvedIn(
                            inst.types,
                            mono_cache,
                    ...
                    const value = try self.planExprWithRequiredExecTy(
                        inst,
                        mono_cache,
                        venv,
                        field.value,
            ...
            .access => |access| blk: {
                ...
                const field_info = self.recordFieldByName(record_ty, access.field) orelse
                    debugPanic("lambdamono.exec_plan.access missing record field");
            ...
            .let_ => |let_expr| blk: {
                ...
                const rest = try self.planExprWithRequiredExecTyAndSourceTy(
                    inst,
                    mono_cache,
                    rest_env,
                    let_expr.rest,
            ...
            .low_level => |ll| blk: {
                ...
                const lowered_args = switch (ll.op) {
                    .box_box, .box_unbox => blk_args: {
                        ...
                        for (source_args, 0..) |arg_id, i| {
                            ...
                            const requested_exec_ty = (try self.explicitProjectedExecutableTypeForExpr(
                                inst,
                                mono_cache,
                                venv,
                                arg_id,
                                constraint_arg_tys[i],
                            )) orelse try self.lowerExecutableTypeFromSolvedIn(inst.types, mono_cache, constraint_arg_tys[i]);
                            out[i] = try self.planExprWithRequiredExecTy(
                                inst,
                                mono_cache,
                                venv,
                                arg_id,
                ...
                const result_ty = switch (ll.op) {
                    .box_box => blk_ty: {
                        if (args.len != 1) {
                            debugPanic("lambdamono.exec_plan.planExpr box_box expected one arg");
                        }
                        break :blk_ty try self.boxBoundaryResultTypeFromArg(.box_box, self.output.getExpr(args[0]).ty);
                    },
                    .box_unbox => blk_ty: {
                        if (args.len != 1) {
                            debugPanic("lambdamono.exec_plan.planExpr box_unbox expected one arg");
                        }
                ...
                switch (ll.op) {
                    .box_box, .box_unbox => {
                        if (args.len != 1) debugPanic("lambdamono.exec_plan.planExpr box op expected one arg");
                        if (ll.op == .box_unbox) {
                            const arg_ty = self.output.getExpr(args[0]).ty;
                            if (self.types.getType(arg_ty) != .box) {
                                debugPanic("lambdamono.exec_plan.planExpr box_unbox expected boxed executable arg");
                            }
                        }
                    },
                    else => {},
                ...
                const source_result_ty = switch (ll.op) {
                    .box_box => blk_ty: {
                        if (source_args.len != 1) {
                            debugPanic("lambdamono.exec_plan.planExpr box_box expected one source arg");
                        }
                        ...
                        const boxed_payload_ty = self.solvedBoxElemType(inst.types, ty) orelse
                            debugPanic("lambdamono.exec_plan.planExpr box_box missing solved payload type");
                        try self.unifyIn(inst.types, boxed_payload_ty, refined_arg_ty);
                        break :blk_ty ty;
                    },
                    .box_unbox => blk_ty: {
                        if (source_args.len != 1) {
                            debugPanic("lambdamono.exec_plan.planExpr box_unbox expected one source arg");
                        }
                        ...
                        break :blk_ty self.solvedBoxElemType(inst.types, boxed_arg_ty) orelse
                            debugPanic("lambdamono.exec_plan.planExpr box_unbox missing solved payload type");
                    },
            ...
            .when => |when_expr| blk: {
                ...
                if (lowered_branches.items.len == 0) {
                    debugPanic("lambdamono.exec_plan.planExpr(when) removed all branches");
                }
                ...
                const result_ty = if (concrete_result_ty) |concrete|
                    ...
                    for (lowered_branches.items[1..]) |branch| {
                        ...
                        if (!self.types.equalIds(first_branch_ty, branch_ty)) {
                            debugPanic("lambdamono.exec_plan.planExpr(when) branch executable types disagree without an explicit result type");
                        }
                    }
            ...
            .if_ => |if_expr| blk: {
                ...
                const else_lowered = try self.planExprWithRequiredExecTyAndSourceTy(
                    inst,
                    mono_cache,
                    venv,
                    if_expr.else_body,
            ...
            .tuple => |elems| blk: {
                ...
                const concrete_tuple_ty = if (!self.executableTypeIsAbstract(required_exec_ty))
                    self.requireConcreteExecutableType(required_exec_ty, "planExpr(tuple)")
                else
                    null;
                ...
                if (source_elem_tys) |elem_tys| {
                    if (elem_tys.len != source_elems.len) {
                        debugPanic("lambdamono.exec_plan.tuple expected solved elem count mismatch");
                    }
                }
                if (expected_elem_tys) |elem_tys| {
                    if (elem_tys.len != source_elems.len) {
                        debugPanic("lambdamono.exec_plan.tuple expected elem count mismatch");
                    }
                }
                ...
                for (source_elems, 0..) |elem_expr_id, i| {
                    ...
                    const lowered_elem = try self.planExprWithRequiredExecTy(
                        inst,
                        mono_cache,
                        venv,
                        elem_expr_id,
                ...
                if (concrete_tuple_ty) |tuple_ty| {
                    const bridge_elem_tys = self.tupleElemTypes(tuple_ty) orelse
                        debugPanic("lambdamono.exec_plan.tuple missing explicit elem types");
                ...
                const result_ty = if (concrete_tuple_ty) |tuple_ty|
                    ...
                    debugPanic("lambdamono.exec_plan.tuple missing explicit executable result type");
                break :blk .{ .ty = result_ty, .data = .{ .tuple = lowered_elems } };
            },
            .tag_payload => |tag_payload| blk: {
                const tag_union = try self.planExpr(inst, mono_cache, venv, tag_payload.tag_union);
                ...
                const exec_tag_info = self.executableTagInfoByDiscriminant(
                    ...
                    self.singletonExecutableCtorTagInfo(tag_union_ty) orelse
                    {
                        ...
                        debugPanicFmt(
                            "lambdamono.exec_plan.tag_payload missing executable tag payload types tag={s} tag_id={d} union={s}",
                            .{
                ...
                if (tag_payload.payload_index >= exec_tag_info.args.len) {
                    debugPanic("lambdamono.exec_plan.tag_payload missing payload");
                }
            ...
            .list => |items| blk: {
                ...
                const expected_elem_ty = if (concrete_list_ty) |list_ty|
                    self.listElemType(list_ty) orelse
                        debugPanic("lambdamono.exec_plan.list missing explicit elem type")
                ...
                const concrete_expected_elem_ty = if (expected_elem_ty) |elem_ty|
                    if (!self.executableTypeIsAbstract(elem_ty))
                        self.requireConcreteExecutableType(elem_ty, "planExpr(list elem)")
                    else
                        null
                else
                    null;
                ...
                for (source_items, 0..) |item_expr_id, i| {
                    ...
                    const lowered_item = try self.planExprWithRequiredExecTy(
                        inst,
                        mono_cache,
                        venv,
                        item_expr_id,
                ...
                const result_ty = if (concrete_list_ty) |list_ty|
                    ...
                    for (lowered[1..]) |lowered_item| {
                        if (!self.types.equalIds(elem_ty, self.output.getExpr(lowered_item).ty)) {
                            debugPanic("lambdamono.exec_plan.list item executable types disagree without an explicit result type");
                        }
                    }
            ...
            .runtime_error => |msg| .{ .ty = required_exec_ty, .data = .{ .runtime_error = msg } },
    ...
    fn decFromWholeInt(_: *Planner, value: i128) std.mem.Allocator.Error!i128 {
    ...
    fn decToF64(_: *Planner, value: i128) f64 {
    ...
    fn specializeInspectExpr(
        ...
        const value_expr = try self.planExpr(inst, mono_cache, venv, value_expr_id);
    ...
    fn buildInlineInspectValueExpr(
        ...
        return switch (self.types.getTypePreservingNominal(value_ty)) {
            .placeholder => debugPanic("lambdamono.exec_plan.buildInlineInspectValueExpr unresolved executable type"),
            .unbd => debugPanic("lambdamono.exec_plan.buildInlineInspectValueExpr abstract executable type leaked to inspect"),
            .link => unreachable,
            .primitive => |prim| switch (prim) {
    ...
    fn specializeInspectValueExpr(
        ...
        return switch (self.types.getType(value_ty)) {
            .placeholder => debugPanic("lambdamono.exec_plan.specializeInspectValueExpr unresolved executable type"),
            .unbd => debugPanic("lambdamono.exec_plan.specializeInspectValueExpr abstract executable type leaked to inspect"),
            .link => unreachable,
            .primitive => |prim| switch (prim) {
                .str => self.makeLowLevelExpr(result_ty, .str_inspect, &.{value_expr}),
                .bool => self.makeBoolInspectExpr(value_expr, result_ty),
    ...
    fn makePrimitiveType(self: *Planner, prim: type_mod.Prim) std.mem.Allocator.Error!type_mod.TypeId {
        return try self.types.internResolved(.{ .primitive = prim });
    }

    fn makeUnitType(self: *Planner) std.mem.Allocator.Error!type_mod.TypeId {
        return try self.types.internResolved(.{ .record = .{ .fields = &.{} } });
    }

    fn isEmptyRecordType(self: *Planner, ty: type_mod.TypeId) bool {
        return switch (self.types.getTypePreservingNominal(ty)) {
    ...
    fn specializeBoxBoundaryArgExpr(
        ...
        const source_ty = try self.instantiatedSourceTypeForExpr(inst, venv, expr_id);
        const expr = self.input.store.getExpr(expr_id);
        const required_exec_ty = if (op == .box_box and inst.types.maybeLambdaRepr(source_ty) != null)
        ...
        const lowered_expr = (try self.planExprWithRequiredExecTyAndSourceTy(
            inst,
            mono_cache,
            venv,
            expr_id,
        ...
        switch (op) {
            ...
            .box_unbox => {
                ...
                if (self.types.getTypePreservingNominal(arg_ty) != .box) {
                    debugPanic("lambdamono.exec_plan.specializeBoxBoundaryArgExpr box_unbox expected boxed executable arg");
                }
                return lowered;
            },
            else => unreachable,
        ...
        switch (source_repr) {
            ...
            .erased => {
                return switch (self.types.getTypePreservingNominal(lowered.ty)) {
                    ...
                    else => debugPanic("lambdamono.exec_plan.lowerBoxBoundaryExpr solved erased callable lowered as non-erased executable"),
                };
            },
        }
    }
        ...
        return switch (callable.data) {
            .tag => |tag| blk: {
                if (tag.discriminant >= lambda_members.len) {
                    debugPanic("lambdamono.exec_plan.lowerConcreteCallableAsErased tag discriminant out of bounds");
                }
                ...
                break :blk try self.makeErasedPackedFnExpr(
                    ...
                    capture_expr,
                    forced_capture_ty,
                    self.callableCapturesForExprFact(callable_id, lambda_members[tag.discriminant].symbol),
                );
            },
            .packed_fn => callable_id,
            else => blk: {
                const erased_capture_ty = forced_capture_ty orelse
                    debugPanic("lambdamono.exec_plan.lowerConcreteCallableAsErased missing explicit erased capture type");
                const branches = try self.allocator.alloc(ast.Branch, lambda_members.len);
                defer self.allocator.free(branches);
                for (lambda_members, 0..) |lambda_member, i| {
    ...
    fn emitExplicitBridgeExpr(
        ...
        if (self.types.containsAbstractLeaf(expected_ty)) {
            debugPanicFmt(
                "lambdamono.exec_plan.emitExplicitBridgeExpr abstract expected executable type expr={d} actual_ty={d} expected_ty={d} actual_tag={s} expected_tag={s}",
                .{
                    @intFromEnum(expr_id),
        ...
        if (self.executableTypesHaveErasedCallableShapeMismatch(actual_ty, expected_ty)) {
            ...
            defer expected_summary.deinit(self.allocator);
            debugPanicFmt(
                "lambdamono.exec_plan.emitExplicitBridgeExpr erased callable shape mismatch expr={d} actual={s} expected={s}",
    ...
    fn isExecutableCallableType(self: *const Planner, ty: type_mod.TypeId) bool {
        return switch (self.types.getTypePreservingNominal(ty)) {
            .erased_fn => true,
            .tag_union => |tag_union| self.tagUnionIsInternalLambdaSet(tag_union.tags),
            else => false,
        ...
        while (true) {
            ...
            switch (expr.data) {
                .bridge => |inner| {
                    ...
                    if (self.isExecutableCallableType(expr.ty) and !self.executableTypeHasLayoutAbstractLeaf(expr.ty)) {
    ...
    fn makeErasedPackedFnExpr(
        ...
        inst: *InstScope,
        requested_ty: TypeVarId,
        erased_call_sig: type_mod.CallableSig,
        symbol: Symbol,
        ...
        if (forced_capture_ty != null and capture_facts.len == 0) {
            debugPanic("lambdamono.exec_plan.makeErasedPackedFnExpr forced capture type on captureless lambda");
        }
        ...
        if (capture_expr_opt == null) {
            if (capture_ty) |expected_ty| {
                if (self.isEmptyRecordType(expected_ty)) {
                    capture_ty = null;
                } else if (capture_facts.len != 0) {
                    debugPanic("lambdamono.exec_plan.makeErasedPackedFnExpr missing capture payload for non-empty capture type");
                }
            }
        ...
        if (capture_ty == null and capture_facts.len != 0) {
    ...
    fn lowerExactExecutableCallableAsErased(
        ...
        const tag_union = switch (self.types.getTypePreservingNominal(callable_ty)) {
            .tag_union => |tag_union| tag_union,
            else => debugPanic("lambdamono.exec_plan.lowerExactExecutableCallableAsErased expected executable lambda-set type"),
        };
        if (!self.tagUnionIsInternalLambdaSet(tag_union.tags)) {
            debugPanic("lambdamono.exec_plan.lowerExactExecutableCallableAsErased expected internal lambda-set tags");
        }
        ...
        return switch (callable.data) {
            .tag => |tag| blk: {
                ...
                const capture_expr = switch (capture_args.len) {
                    0 => null,
                    1 => capture_args[0],
                    else => debugPanic("lambdamono.exec_plan.lowerExactExecutableCallableAsErased expected at most one capture payload"),
                };
                break :blk try self.makeErasedPackedFnExpr(
                    inst,
                    source_ty,
                    ...
                    capture_expr,
                    forced_capture_ty,
                    self.callableCapturesForExprFact(callable_id, lambda_symbol),
                );
            },
            else => blk: {
                const erased_capture_ty = forced_capture_ty orelse
                    debugPanic("lambdamono.exec_plan.lowerExactExecutableCallableAsErased missing explicit erased capture type");
                ...
                for (tag_union.tags, 0..) |tag_info, i| {
                    ...
                    if (tag_info.args.len == 0) {
                        const branch_capture_facts = self.callableCapturesForExprFact(callable_id, lambda_symbol);
                        branches[i] = .{
                            .pat = try self.output.addPat(.{
                                .ty = callable_ty,
                                .data = .{ .tag = .{
                        ...
                        if (tag_info.args.len != 1) {
                            debugPanic("lambdamono.exec_plan.lowerExactExecutableCallableAsErased expected at most one capture payload");
                        }
                        const capture_symbol = try self.input.symbols.add(base.Ident.Idx.NONE, .synthetic);
                        const capture_pat = try self.output.addPat(.{
                            .ty = tag_info.args[0],
    ...
    fn lookupTagNameBytes(self: *Planner, tag_name: type_mod.TagName) []const u8 {
    ...
    fn lookupIdentNameBytes(self: *Planner, ident: base.Ident.Idx) []const u8 {
    ...
    fn specializeVarExpr(
        ...
        if (self.lookupEnvEntry(venv, symbol)) |entry| {
            if (self.envCallable(entry)) |callable_id| {
                const callable = self.callableFact(callable_id);
                const target_symbol = callable.target;
                const target_entry = specializations.lookupFnExact(self.fenv, target_symbol) orelse
                    debugPanic("lambdamono.exec_plan.specializeVarExpr env exact callable missing source definition");
                if (self.envHasAllCaptureSymbols(venv, target_entry.capture_symbols)) {
                    return try self.lowerExactCallableVarExpr(
                        inst,
                        mono_cache,
                        venv,
                ...
                const packaged_capture_ty: ?type_mod.TypeId = switch (self.types.getTypePreservingNominal(current_exec_ty)) {
                    .erased_fn => self.erasedFnCaptureType(current_exec_ty),
                    .tag_union => self.executableCaptureTypeForLambdaMember(current_exec_ty, target_symbol),
                    else => null,
                };
                if (self.isExecutableCallableType(current_exec_ty) and
                    !self.executableTypeHasLayoutAbstractLeaf(current_exec_ty) and
                    (target_entry.capture_symbols.len == 0 or packaged_capture_ty != null))
                {
                ...
                for (target_entry.capture_symbols) |capture_symbol| {
                    if (self.lookupEnvEntry(venv, capture_symbol) != null) continue;
                    try missing.writer(self.allocator).print(
                        "{d}:{s} ",
                        .{
                            capture_symbol.raw(),
                ...
                debugPanicFmt(
                    "lambdamono.exec_plan.specializeVarExpr env exact callable missing explicit captures symbol={d} exact={d} exec={s} missing={s}",
                    .{
                        symbol.raw(),
                        target_symbol.raw(),
        ...
        if (self.callableTargetForBinding(symbol)) |target_symbol| {
        ...
        debugPanicFmt(
            "lambdamono.exec_plan.specializeVarExpr unbound variable {d} ({s}) origin {s}",
    ...
    fn lowerExactCallableVarExpr(
            ...
            if (source_repr == .erased) {
                ...
                var authoritative_capture_ty = if (capture_symbols.len == 0)
                    ...
                    current_capture.ty orelse
                        debugPanic("lambdamono.exec_plan.lowerExactCallableVarExpr missing current erased capture payload type for abstract exact callable");
                ...
                if (capture_symbols.len != 0 and authoritative_capture_ty == null) {
                    debugPanic("lambdamono.exec_plan.lowerExactCallableVarExpr abstract erased callable dropped non-empty capture fact payload");
                }
                const specialized = try self.ensureQueuedCallableSpecializedWithExecSignature(
                    solved_types,
                    target_symbol,
                ...
                if (specialized.exec_capture_ty != authoritative_capture_ty) {
                    debugPanic("lambdamono.exec_plan.lowerExactCallableVarExpr abstract erased specialization capture type mismatch");
                }
        ...
        switch (self.types.getTypePreservingNominal(concrete_required_exec_ty)) {
            .tag_union => |tag_union| {
                if (!self.tagUnionIsInternalLambdaSet(tag_union.tags)) {
                    debugPanic("lambdamono.exec_plan.lowerExactCallableVarExpr expected executable callable type");
                }
            ...
            .erased_fn => {
                var authoritative_capture_ty = self.erasedFnCaptureType(concrete_required_exec_ty);
                if (capture_symbols.len != 0) {
                    authoritative_capture_ty = current_capture.ty orelse
                        debugPanic("lambdamono.exec_plan.lowerExactCallableVarExpr missing current erased capture payload type for captured exact callable");
                }
                if (authoritative_capture_ty) |capture_ty| {
                    if (self.isEmptyRecordType(capture_ty)) {
                        authoritative_capture_ty = null;
                ...
                if (capture_symbols.len != 0 and authoritative_capture_ty == null) {
                    debugPanic("lambdamono.exec_plan.lowerExactCallableVarExpr erased callable dropped non-empty capture fact payload");
                }
                const specialized = try self.ensureQueuedCallableSpecializedWithExecSignature(
                    solved_types,
                    target_symbol,
                ...
                if (specialized.exec_capture_ty != authoritative_capture_ty) {
                    debugPanic("lambdamono.exec_plan.lowerExactCallableVarExpr erased specialization capture type mismatch");
                }
                const authoritative_exec_ty = try self.makeErasedFnType(
                    authoritative_capture_ty,
                    authoritative_call_sig,
    ...
    fn specializeCaptureRecord(
        ...
        const capture_fields = switch (self.types.getTypePreservingNominal(capture_ty)) {
            .nominal => |nominal| switch (self.types.getTypePreservingNominal(nominal.backing)) {
                ...
                else => debugPanic("lambdamono.exec_plan.specializeCaptureRecord expected capture record"),
            },
            .record => |record| record.fields,
            else => debugPanic("lambdamono.exec_plan.specializeCaptureRecord expected capture record"),
        };
        if (builtin.mode == .Debug and capture_fields.len != captures.len) {
            debugPanic("lambdamono.exec_plan.specializeCaptureRecord capture field count mismatch");
        }
        ...
        for (capture_fields, 0..) |field, i| {
            ...
            const fact = self.valueFact(capture.fact);
            var capture_expr = try self.makeVarExpr(fact.exec_ty, capture.symbol);
    ...
    fn addSpecializedExpr(
        ...
        if (specialized.fact) |fact| {
            try self.expr_facts.put(expr_id, fact);
        }
        return expr_id;
    }
    ...
    fn executableCaptureTypeForLambdaMember(
        ...
        const tag_union = switch (self.types.getType(callable_exec_ty)) {
            .tag_union => |tag_union| tag_union,
            else => debugPanic("lambdamono.exec_plan.executableCaptureTypeForLambdaMember expected executable lambda-set type"),
        };
        for (tag_union.tags) |tag| {
            switch (tag.name) {
                .lambda => |tag_symbol| if (tag_symbol == symbol) {
                    return switch (tag.args.len) {
                        0 => null,
                        1 => tag.args[0],
                        else => debugPanic("lambdamono.exec_plan.executableCaptureTypeForLambdaMember expected at most one capture payload"),
                    };
                },
                .ctor => {},
            }
    ...
    fn collectExecutableLambdaMembers(
        ...
        const lambdas = switch (solved_types.lambdaRepr(requested_ty)) {
            .erased => debugPanic("lambdamono.exec_plan.collectExecutableLambdaMembers expected concrete lambda-set"),
            .lset => |lset| lset,
        };
    ...
    fn captureBindingsFromCaptures(
        ...
        if (capture_facts.len != captures.len) {
            debugPanic("lambdamono.exec_plan.captureBindingsFromCaptures capture fact arity mismatch");
        }
    ...
    fn callableTargetForBinding(self: *const Planner, symbol: Symbol) ?Symbol {
        ...
        return switch (source) {
            ...
            .val => |expr_id| self.callableTargetFromExpr(self.input.store.getExpr(expr_id)),
            .run => null,
        };
    }

    fn isHostedTopLevelSymbol(self: *const Planner, symbol: Symbol) bool {
    ...
    fn callableReprMode(self: *const Planner, symbol: Symbol) specializations.Pending.ReprMode {
        _ = self;
        _ = symbol;
        return .natural;
    }
    ...
    fn callableTargetFromExpr(self: *const Planner, expr: solved.Ast.Expr) ?Symbol {
        return switch (expr.data) {
    ...
    fn callableTargetForExprFact(
        self: *const Planner,
        venv: []const EnvEntry,
        expr: solved.Ast.Expr,
    ) ?Symbol {
        if (self.callableTargetFromExpr(expr)) |symbol| return symbol;
        if (expr.data != .var_) return null;
        if (self.lookupEnvEntry(venv, expr.data.var_)) |entry| {
            const callable_id = self.envCallable(entry) orelse return null;
            return self.callableFact(callable_id).target;
        }
        return null;
    }
    ...
    fn instantiatedSourceTypeForExpr(
        ...
        return switch (expr.data) {
            .var_ => |symbol| if (self.lookupEnvEntry(venv, symbol)) |entry| blk: {
                const entry_ty = self.envSourceType(entry);
                if (self.envCallable(entry) != null) {
                    const entry_id = inst.types.unlinkPreservingNominalConst(entry_ty);
                    switch (inst.types.getNode(entry_id)) {
                        .content => |content| switch (content) {
                            .func => {},
                            else => break :blk entry_ty,
                        },
                        else => break :blk entry_ty,
                    }
                    break :blk entry_ty;
            ...
            .access => |access| {
                ...
                return self.solvedRecordFieldByName(inst.types, record_ty, access.field) orelse
                    debugPanic("lambdamono.exec_plan.instantiatedSourceTypeForExpr missing solved record field");
            },
            .tuple_access => |access| {
                const tuple_ty = try self.instantiatedSourceTypeForExpr(inst, venv, access.tuple);
                return self.solvedTupleElemType(inst.types, tuple_ty, access.elem_index) orelse
                    debugPanic("lambdamono.exec_plan.instantiatedSourceTypeForExpr missing solved tuple element");
            },
            .tag_payload => |tag_payload| {
    ...
    fn tagArgSourceTypesForExpr(
        ...
        if (self.solvedTagInfoByName(inst.types, union_source_ty, tag_name)) |tag_info| {
            if (tag_info.args.len != arg_expr_ids.len) {
                debugPanic("lambdamono.exec_plan.tagArgSourceTypesForExpr tag arg count mismatch");
            }
            return self.dupeTypeVarIds(tag_info.args);
        }
        debugPanic("lambdamono.exec_plan.tagArgSourceTypesForExpr missing explicit tag info on source result type");
    }
    ...
    fn tagArgSourceTypesForPat(
        ...
        if (self.solvedTagInfoByName(inst.types, union_source_ty, tag_name)) |tag_info| {
            if (tag_info.args.len != arg_pat_ids.len) {
                debugPanic("lambdamono.exec_plan.tagArgSourceTypesForPat tag arg count mismatch");
            }
            return self.dupeTypeVarIds(tag_info.args);
        }
        debugPanic("lambdamono.exec_plan.tagArgSourceTypesForPat missing explicit tag info on source result type");
    }
        ...
        return switch (expr.data) {
            ...
            .list => |items_span| blk: {
                ...
                const elem_source_ty = self.solvedListElemType(inst.types, current_source_ty) orelse
                    debugPanic("lambdamono.exec_plan.refinedSourceTypeForExpr expected solved list source type");
            ...
            .record => |fields_span| blk: {
                ...
                for (fields) |field| {
                    const field_source_ty = self.solvedRecordFieldByName(
                        ...
                        current_source_ty,
                        field.name,
                    ) orelse debugPanic("lambdamono.exec_plan.refinedSourceTypeForExpr missing solved record field");
            ...
            .call => |call| blk: {
                ...
                if (box_boundary) |op| {
                    if (arg_expr_ids.len != 1) {
                        debugPanic("lambdamono.exec_plan.refinedSourceTypeForExpr box boundary call must have arity 1");
                    }
                    switch (op) {
                        .box_unbox => {
                            ...
                            break :blk self.solvedBoxElemType(inst.types, boxed_arg_ty) orelse
                                debugPanic("lambdamono.exec_plan.refinedSourceTypeForExpr box_unbox missing solved payload type");
                        },
                        .box_box => {
                            ...
                            const boxed_payload_ty = self.solvedBoxElemType(inst.types, current_source_ty) orelse
                                debugPanic("lambdamono.exec_plan.refinedSourceTypeForExpr box_box missing solved payload type");
                            try self.unifyIn(inst.types, boxed_payload_ty, refined_arg_ty);
                            break :blk current_source_ty;
                        },
                        else => unreachable,
            ...
            .dispatch_call => |method_call| blk: {
                ...
                if (frozen_arg_tys.len != method_args.len + 1) {
                    debugPanic("lambdamono.exec_plan.refinedSourceTypeForExpr dispatch arg arity mismatch");
                }
                const refined_receiver_ty = try self.refinedSourceTypeForExpr(
                    inst,
                    venv,
            ...
            .type_dispatch_call => |method_call| blk: {
                ...
                if (frozen_arg_tys.len != method_args.len) {
                    debugPanic("lambdamono.exec_plan.refinedSourceTypeForExpr type dispatch arg arity mismatch");
                }
                try self.refineCallArgSourceTypesIn(
                    inst,
                    venv,
            ...
            .let_ => |let_expr| blk: {
                ...
                const rest_fact = try self.addValueFact(
                    let_expr.bind.symbol,
                    bind_ty,
                    try self.lowerExecutableTypeFromSolvedIn(inst.types, &local_mono_cache, bind_ty),
                    null,
                );
                const rest_env = try self.extendEnv(venv, .{
                    .symbol = let_expr.bind.symbol,
    ...
    fn bridgeCallArgsToExpectedExecutableTypes(
        ...
        if (arg_exprs.len != source_arg_tys.len or arg_exprs.len != expected_arg_tys.len) {
            debugPanic("lambdamono.exec_plan.bridgeCallArgsToExpectedExecutableTypes arg arity mismatch");
        }
    ...
    fn applyExactTopLevelFunctionCall(
        ...
        if (proc_arg_exec_tys.len != arg_exprs.len or arg_source_tys.len != arg_exprs.len) {
            debugPanic("lambdamono.exec_plan.applyExactTopLevelFunctionCall call arg arity mismatch");
        }
        ...
        const call_args = if (proc_capture_exec_ty) |capture_exec_ty| blk: {
            const capture = capture_expr orelse
                debugPanic("lambdamono.exec_plan.applyExactTopLevelFunctionCall missing capture arg for captured specialization");
            ...
            if (capture_expr != null) {
                debugPanic("lambdamono.exec_plan.applyExactTopLevelFunctionCall unexpected capture arg for captureless specialization");
            }
                ...
                .erased_fn => {},
                    ...
                    switch (expr.data) {
                        .var_ => |symbol| debugPanicFmt(
                            "lambdamono.exec_plan.assertCallableExprMatchesSolvedType expected erased callable expr, found {s} for expr {d} data {s} symbol {d}",
                            .{
                            ...
                            "lambdamono.exec_plan.assertCallableExprMatchesSolvedType expected erased callable expr, found {s} for expr {d} data {s}",
                            .{
                                @tagName(self.types.getTypePreservingNominal(expr_ty)),
                                @intFromEnum(func_expr),
                                @tagName(expr.data),
                    ...
                    for (tag_union.tags, lambdas) |tag, lambda| {
                        ...
                        switch (tag.name) {
                            .lambda => |lambda_symbol| if (lambda_symbol != lambda.symbol) {
                                debugPanic("lambdamono.exec_plan.assertCallableExprMatchesSolvedType lambda symbol mismatch");
                            },
                            .ctor => debugPanic("lambdamono.exec_plan.assertCallableExprMatchesSolvedType expected lambda tag"),
                        }
                        if (expected_capture_count == 0) {
                            if (tag.args.len != 0) {
                                debugPanic("lambdamono.exec_plan.assertCallableExprMatchesSolvedType unexpected capture payload");
                            }
                        } else if (tag.args.len != 1) {
                            debugPanic("lambdamono.exec_plan.assertCallableExprMatchesSolvedType missing capture payload");
                        }
                    }
                },
                else => debugPanic("lambdamono.exec_plan.assertCallableExprMatchesSolvedType expected lambda-set expr"),
            },
        }
    }
    ...
    fn specializeStructuralEqExpr(
        ...
        const operand_required_exec_ty = try self.lowerExecutableTypeFromSolvedIn(
            inst.types,
            mono_cache,
            operand_source_ty,
        );
        ...
        var lowered_receiver = try self.planExprWithRequiredExecTy(
            inst,
            mono_cache,
            venv,
            eq.lhs,
            lhs_expr,
            receiver_source_ty,
            operand_required_exec_ty,
        );
        if (try self.retypeDivergingExpr(lowered_receiver, required_exec_ty)) |diverging| {
            return .{
        ...
        var lowered_arg = try self.planExprWithRequiredExecTy(
            inst,
            mono_cache,
            venv,
            eq.rhs,
        ...
        if (try self.retypeDivergingExpr(lowered_arg, required_exec_ty)) |diverging| {
            return .{
                .ty = self.output.getExpr(diverging).ty,
                .data = self.output.getExpr(diverging).data,
            };
        ...
        const operand_exec_ty = if (!self.executableTypeIsAbstract(operand_required_exec_ty))
            self.requireConcreteExecutableType(
                operand_required_exec_ty,
            ...
            debugPanic("lambdamono.exec_plan.specializeStructuralEqExpr abstract operand executable types disagree");
        ...
        if (!self.types.equalIds(self.output.getExpr(eq_expr).ty, required_exec_ty)) {
            eq_expr = try self.emitExplicitBridgeExpr(eq_expr, required_exec_ty);
        }
    ...
    fn methodEqIdent(self: *const Planner) base.Ident.Idx {
        return self.input.idents.lookup(base.Ident.for_text("is_eq")) orelse
            debugPanic("lambdamono.exec_plan.methodEqIdent missing is_eq ident");
    }
    ...
    fn specializeMethodEqExpr(
        ...
        if (dispatch_arg_tys.len != 2) {
            debugPanic("lambdamono.exec_plan.specializeMethodEqExpr method_eq arg arity mismatch");
        }
        ...
        const rhs_expr = self.input.store.getExpr(eq.rhs);
        lowered_args[1] = try self.planExprWithRequiredExecTy(
            inst,
            mono_cache,
            venv,
            eq.rhs,
    ...
    fn specializeDispatchCallExpr(
        ...
        if (initial_arg_tys.len != method_args.len + 1) {
            debugPanic("lambdamono.exec_plan.specializeDispatchCallExpr initial method arg arity mismatch");
        }
        const refined_receiver_ty = try self.refinedSourceTypeForExpr(
            inst,
            venv,
        ...
        const target_entry = specializations.lookupFnExact(self.fenv, target_symbol) orelse
            debugPanic("lambdamono.exec_plan.specializeDispatchCallExpr resolved attached method target missing exact callable definition");
        ...
        if (exact_requested_arg_tys.len != method_args.len + 1) {
            debugPanic("lambdamono.exec_plan.specializeDispatchCallExpr exact requested method arg arity mismatch");
        }
        ...
        if (fn_arg_tys.len != method_args.len + 1) {
            debugPanic("lambdamono.exec_plan.specializeDispatchCallExpr method arg arity mismatch");
        }
    ...
    fn specializeTypeDispatchCallExpr(
        ...
        if (initial_arg_tys.len != method_args.len) {
            debugPanic("lambdamono.exec_plan.specializeTypeDispatchCallExpr initial method arg arity mismatch");
        }
        try self.refineCallArgSourceTypesIn(
            inst,
            venv,
        ...
        if (fn_arg_tys.len != method_args.len) {
            debugPanic("lambdamono.exec_plan.specializeTypeDispatchCallExpr method arg arity mismatch");
        }
        ...
        for (method_args, 0..) |arg_expr_id, i| {
            const arg_expr = self.input.store.getExpr(arg_expr_id);
            lowered_args[i] = try self.planExprWithRequiredExecTy(
                inst,
                mono_cache,
                venv,
                arg_expr_id,
    ...
    fn specializeCallExpr(
        ...
        if (self.boxBoundaryBuiltinOpForExpr(venv, base_func_source)) |op| {
            if (arg_expr_ids.len != 1) {
                debugPanic("lambdamono.exec_plan.specializeCallExpr box boundary call must have arity 1");
            }
            ...
            const source_result_ty = switch (op) {
                .box_unbox => blk: {
                    const boxed_arg_ty = try self.instantiatedSourceTypeForExpr(inst, venv, arg_expr_ids[0]);
                    break :blk self.solvedBoxElemType(inst.types, boxed_arg_ty) orelse
                        debugPanic("lambdamono.exec_plan.specializeCallExpr box_unbox missing solved payload type");
                },
                .box_box => blk: {
                    const arg_source_ty = try self.instantiatedSourceTypeForExpr(inst, venv, arg_expr_ids[0]);
                    const refined_arg_ty = try self.refinedSourceTypeForExpr(inst, venv, arg_expr_ids[0], arg_source_ty);
                    const boxed_payload_ty = self.solvedBoxElemType(inst.types, current_result_source_ty) orelse
                        debugPanic("lambdamono.exec_plan.specializeCallExpr box_box missing solved payload type");
                    try self.unifyIn(inst.types, boxed_payload_ty, refined_arg_ty);
        ...
        const initial_callable_target = self.callableTargetForExprFact(venv, base_func_source);
        ...
        const callable_target = initial_callable_target;
        if (callable_target) |target_symbol| {
            ...
            const capture_symbols: []const Symbol = if (target_source_entry) |target_entry|
                target_entry.capture_symbols
            else if (self.isHostedTopLevelSymbol(target_symbol))
                &.{}
            else
                &.{};
            if (target_source_entry != null or self.isHostedTopLevelSymbol(target_symbol)) {
                if (self.envHasAllCaptureSymbols(venv, capture_symbols)) {
                    ...
                    if (specialized.exec_args_tys.len != exact_arg_tys.len) {
                        debugPanic("lambdamono.exec_plan.specializeCallExpr direct exact arg arity mismatch against specialization summary");
                    }
        ...
        const func_required_exec_ty = try self.explicitValueExecutableTypeForExpr(
            inst.types,
            mono_cache,
            venv,
            call.func,
        ...
        var lowered_func = try self.planExprWithRequiredExecTy(
            inst,
            mono_cache,
            venv,
            call.func,
        ...
        if (self.executableTypeHasLayoutAbstractLeaf(lowered_func_exec_ty)) {
            const required_summary = self.debugExecutableTypeSummary(func_required_exec_ty);
            defer required_summary.deinit(self.allocator);
            const lowered_summary = self.debugExecutableTypeSummary(lowered_func_exec_ty);
            defer lowered_summary.deinit(self.allocator);
            debugPanicFmt(
                "lambdamono.exec_plan.specializeCallExpr callable value missing explicit executable signature direct_symbol={?d} func_expr={d} func_var={?d} default_exec={s} lowered_exec={s}",
                .{
                    if (callable_target) |symbol| symbol.raw() else null,
        ...
        if (callable_target) |target_symbol| {
            const target_repr_mode: specializations.Pending.ReprMode = switch (self.types.getTypePreservingNominal(lowered_func_exec_ty)) {
            ...
            const capture_ty: ?type_mod.TypeId = switch (self.types.getTypePreservingNominal(lowered_func_exec_ty)) {
                ...
                .tag_union => self.executableCaptureTypeForLambdaMember(lowered_func_exec_ty, target_symbol),
                else => null,
            };
            const capture_facts = self.callableCapturesForExprFact(lowered_func, target_symbol);
            const specialized = try self.ensureQueuedCallableSpecializedForRequestedType(
                inst.types,
                mono_cache,
                target_symbol,
        ...
        if (builtin.mode == .Debug and inst.types.lambdaRepr(func_source_ty) == .erased) {
            ...
            if (lowered_func_expr.data == .var_ and self.types.getTypePreservingNominal(lowered_func_exec_ty) != .erased_fn) {
                if (self.lookupEnvEntry(venv, lowered_func_expr.data.var_)) |entry| {
                    ...
                    defer self.allocator.free(binding_summary);
                    debugPanicFmt(
                        "lambdamono.exec_plan.specializeCallExpr erased callable var mismatch symbol={d} entry_exec_ty={d} entry_exec_tag={s} lowered_exec_ty={d} lowered_exec_tag={s} bindings={s}",
                        .{
                            lowered_func_expr.data.var_.raw(),
                            @intFromEnum(self.envExecutableType(entry)),
                            @tagName(self.types.getTypePreservingNominal(self.envExecutableType(entry))),
    ...
    fn specializeBlockExpr(
        ...
        final_required_exec_ty: type_mod.TypeId,
        ...
        const lowered_final = try self.planExprWithRequiredExecTyAndSourceTy(
            inst,
            mono_cache,
            env,
            block.final_expr,
    ...
    fn specializeForExpr(
        ...
        const iterable = try self.planExpr(inst, mono_cache, venv, for_expr.iterable);
        const pat_exec_ty = self.listElemType(self.output.getExpr(iterable).ty) orelse
            debugPanic("lambdamono.exec_plan.specializeForExpr expected list iterable executable type");
        const source_pat = self.input.store.getPat(for_expr.patt);
        const pat_ty = try self.cloneInstType(inst, source_pat.ty);
    ...
    fn collectPatBindingsAtSourceTy(
        ...
        return switch (pat.data) {
            .var_ => |symbol| if (symbol.isNone())
                ...
                const entry = try self.addEnvEntry(symbol, ty, lowered_ty, null);
                break :blk try self.allocator.dupe(EnvEntry, &.{entry});
            },
        ...
        return switch (pat.data) {
            ...
            .tag => |tag| blk: {
                ...
                if (source_args.len != exec_tag_info.args.len) {
                    debugPanic("lambdamono.exec_plan.patPossibleAtExecutableTy executable tag arg count mismatch");
                }
    ...
    fn patExhaustsExecutableTy(
        ...
        return switch (pat.data) {
            ...
            .tag => |tag| blk: {
                ...
                const source_args = self.input.store.slicePatSpan(tag.args);
                if (source_args.len != exec_tag_info.args.len) {
                    debugPanic("lambdamono.exec_plan.patExhaustsExecutableTy executable tag arg count mismatch");
                }
        ...
        return switch (pat.data) {
            .var_ => |symbol| .{
                ...
                .additions = if (symbol.isNone())
                    ...
                    const entry = try self.addEnvEntry(symbol, ty, lowered_ty, null);
                    break :blk_additions try self.allocator.dupe(EnvEntry, &.{entry});
                },
            },
            .bool_lit => |value| .{
                .pat = try self.output.addPat(.{
            ...
            .tag => |tag| blk: {
                ...
                defer if (arg_tys.len != 0) self.allocator.free(arg_tys);
                const exec_tag_info = self.executableTagInfoByName(lowered_ty, tag.name) orelse
                    debugPanic("lambdamono.exec_plan.specializePat missing executable tag info");
                const exec_arg_tys = exec_tag_info.args;
                if (exec_arg_tys.len != source_args.len) {
                    debugPanic("lambdamono.exec_plan.specializePat expected executable tag arg count mismatch");
                }
                for (source_args, 0..) |arg_pat, i| {
                    const arg_pat_data = self.input.store.getPat(arg_pat);
    ...
    fn specializeStmt(
        ...
        return switch (stmt) {
            ...
            .reassign => |reassign| .{
                .stmt = try self.output.addStmt(.{
                    .reassign = .{
                        ...
                        .body = try self.planExpr(inst, mono_cache, venv, reassign.body),
                    },
                }),
            ...
            .debug => |expr| .{
                .stmt = try self.output.addStmt(.{ .debug = try self.planExpr(inst, mono_cache, venv, expr) }),
                .env = try self.cloneEnv(venv),
            },
            .expect => |expr| .{
                .stmt = try self.output.addStmt(.{ .expect = try self.planExpr(inst, mono_cache, venv, expr) }),
                .env = try self.cloneEnv(venv),
            },
            .crash => |msg| .{
            ...
            .for_ => |for_stmt| blk: {
                const iterable = try self.planExpr(inst, mono_cache, venv, for_stmt.iterable);
                const pat_exec_ty = self.listElemType(self.output.getExpr(iterable).ty) orelse
                    debugPanic("lambdamono.exec_plan.specializeStmt(for_) expected list iterable executable type");
                const source_pat = self.input.store.getPat(for_stmt.patt);
                const pat_ty = try self.cloneInstType(inst, source_pat.ty);
                const finalized_pat = try self.specializePatAtSourceAndExecutableTy(
                    inst,
    ...
    fn unifyIn(self: *Planner, types: *solved.Type.Store, left: TypeVarId, right: TypeVarId) std.mem.Allocator.Error!void {
        var visited = std.AutoHashMap(u64, void).init(self.allocator);
        defer visited.deinit();
    ...
    fn unifyRec(
        ...
        switch (types.getNode(l)) {
            ...
            .for_a => debugPanic("lambdamono.exec_plan.unify generalized type without instantiation"),
            else => {},
        }
        ...
        const merged = switch (left_node) {
            .nominal => |left_nominal| switch (right_node) {
                .nominal => |right_nominal| {
                    ...
                    {
                        debugPanic("lambdamono.exec_plan.unify incompatible nominal types");
                    }

                    const left_args = types.sliceTypeVarSpan(left_nominal.args);
                    const right_args = types.sliceTypeVarSpan(right_nominal.args);
                    if (left_args.len != right_args.len) {
                        debugPanic("lambdamono.exec_plan.unify nominal arity mismatch");
                    }
                ...
                else => debugPanicFmt("lambdamono.exec_plan.unify incompatible nodes: left={s} right={s}", .{ @tagName(left_node), @tagName(right_node) }),
            },
            ...
            else => debugPanic("lambdamono.exec_plan.unify incompatible types"),
        };
    ...
    fn unifyContent(
        ...
        switch (left) {
            .primitive => |prim| switch (right) {
                .primitive => |other| {
                    if (prim != other) {
                        debugPanic("lambdamono.exec_plan.unify incompatible primitives");
                    }
                    return .{ .primitive = prim };
                },
                .lambda_set => {
                    if (prim == .erased) return .{ .primitive = .erased };
                    debugPanicFmt("lambdamono.exec_plan.unify incompatible types: left={s} right={s}", .{ @tagName(left), @tagName(right) });
                },
                else => debugPanicFmt("lambdamono.exec_plan.unify incompatible types: left={s} right={s}", .{ @tagName(left), @tagName(right) }),
            },
            .func => |func| switch (right) {
                .func => |other| {
                    ...
                    if (func_args.len != other_args.len) {
                        debugPanicFmt("lambdamono.exec_plan.unify incompatible function arity: left={d} right={d}", .{ func_args.len, other_args.len });
                    }
            ...
            .tuple => |tuple| switch (right) {
                .tuple => |other| {
                    ...
                    if (left_elems.len != right_elems.len) debugPanic("lambdamono.exec_plan.unify tuple arity mismatch");
        ...
        while (i < left_tags.len or j < right_tags.len) {
            ...
            switch (order) {
                ...
                .eq => {
                    ...
                    if (left_args.len != right_args.len) {
                        debugPanic("lambdamono.exec_plan.unify tag arity mismatch");
                    }
    ...
    fn lookupEnvEntry(_: *const Planner, venv: []const EnvEntry, symbol: Symbol) ?EnvEntry {
        var i = venv.len;
        while (i > 0) {
            i -= 1;
            const entry = venv[i];
    ...
    fn cloneEnv(self: *Planner, env: []const EnvEntry) std.mem.Allocator.Error![]EnvEntry {
        return try self.allocator.dupe(EnvEntry, env);
    }

    fn extendEnv(self: *Planner, env: []const EnvEntry, extra: EnvEntry) std.mem.Allocator.Error![]EnvEntry {
    ...
    fn concatEnv(self: *Planner, left: []const EnvEntry, right: []const EnvEntry) std.mem.Allocator.Error![]EnvEntry {
    ...
    fn debugExecutableTypeSummary(self: *const Planner, ty: type_mod.TypeId) DebugTypeText {
        var out = std.ArrayList(u8).empty;
    ...
    fn debugSolvedTypeSummary(self: *const Planner, solved_types: *const solved.Type.Store, ty: TypeVarId) DebugTypeText {
        var out = std.ArrayList(u8).empty;
...
test "lambdamono exec plan tests" {
