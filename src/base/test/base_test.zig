...
test "Ident.Store basic CompactWriter roundtrip" {
    ...
    const idx1_value = @intFromEnum(@as(SmallStringInterner.Idx, @enumFromInt(@as(u32, idx1.getIdx()))));
...
test "Ident.Store comprehensive CompactWriter roundtrip" {
    ...
    for (test_idents) |test_ident| {
        ...
        try std.testing.expectEqual(test_ident.expected_idx, idx.getIdx());
    }
...
// TODO: This test fails with an alignment issue because CompactWriter doesn't support
// serializing multiple independent structures. The test needs to be redesigned.
