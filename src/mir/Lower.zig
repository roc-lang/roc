...
/// Polymorphic specialization cache: maps (symbol_key, caller_monotype) to specialized symbol.
poly_specializations: std.AutoHashMap(u128, MIR.Symbol),

/// Deferred block lambda definitions that need concrete types from a call-site
/// before they can be lowered.
deferred_block_lambdas: std.AutoHashMap(u32, DeferredBlockLambda),

/// Per-call-site cycle-breaker map for nominal types being resolved.
...
fn labelTextForCompare(self: *const Self, label: anytype) ?[]const u8 {
    return switch (@TypeOf(label)) {
        ...
        Monotype.StructuralNameId => self.store.monotype_store.getNameText(label),
...
fn identsStructurallyEqual(self: *const Self, lhs: anytype, rhs: anytype) bool {
    ...
    if (@TypeOf(lhs) == Monotype.StructuralNameId and @TypeOf(rhs) == Monotype.StructuralNameId and @intFromEnum(lhs) == @intFromEnum(rhs)) return true;
...
fn importMonotypeFromStoreRec(
    ...
    const resolved = source_store.resolve(monotype);
    switch (resolved.kind) {
        .builtin => {
            if (resolved.isUnit()) return self.store.monotype_store.unit_idx;
            if (resolved.builtinPrim()) |prim| return self.store.monotype_store.primIdx(prim);
            return monotype;
        },
        .rec => {
            if (builtin.mode == .Debug) {
                std.debug.panic("importMonotypeFromStore: unexpected recursive type", .{});
            }
            unreachable;
        },
    ...
    const placeholder = try self.store.monotype_store.reserveRecursive();
    try imported.put(monotype, placeholder);

    const result: Monotype.Idx = switch (resolved.kind) {
        .list => try self.store.monotype_store.internList(
            try self.importMonotypeFromStoreRec(source_store, source_store.listElem(resolved), from_module_idx, to_module_idx, imported),
        ),
        .box => try self.store.monotype_store.internBox(
            try self.importMonotypeFromStoreRec(source_store, source_store.boxInner(resolved), from_module_idx, to_module_idx, imported),
        ),
        .tuple => blk: {
            const idx_top = self.mono_scratches.idxs.top();
            defer self.mono_scratches.idxs.clearFrom(idx_top);
            for (source_store.tupleElems(resolved)) |elem_mono| {
                try self.mono_scratches.idxs.append(try self.importMonotypeFromStoreRec(source_store, elem_mono, from_module_idx, to_module_idx, imported));
            }
            break :blk try self.store.monotype_store.internTuple(self.mono_scratches.idxs.sliceFromStart(idx_top));
        },
        .func => blk: {
            const idx_top = self.mono_scratches.idxs.top();
            defer self.mono_scratches.idxs.clearFrom(idx_top);
            for (source_store.funcArgs(resolved)) |arg_mono| {
                try self.mono_scratches.idxs.append(try self.importMonotypeFromStoreRec(source_store, arg_mono, from_module_idx, to_module_idx, imported));
            }
            const ret = try self.importMonotypeFromStoreRec(source_store, source_store.funcRet(resolved), from_module_idx, to_module_idx, imported);
            break :blk try self.store.monotype_store.internFunc(self.mono_scratches.idxs.sliceFromStart(idx_top), ret, source_store.funcEffectful(resolved));
        },
        .record => blk: {
            const fields_top = self.mono_scratches.fields.top();
            defer self.mono_scratches.fields.clearFrom(fields_top);
            for (source_store.recordFields(resolved)) |field| {
                try self.mono_scratches.fields.append(.{
                    .name = field.name,
                    .ty = try self.importMonotypeFromStoreRec(source_store, field.ty, from_module_idx, to_module_idx, imported),
                });
            }
            break :blk try self.store.monotype_store.internRecord(self.mono_scratches.fields.sliceFromStart(fields_top));
        },
        .tag_union => blk: {
            const tags_top = self.mono_scratches.tags.top();
            defer self.mono_scratches.tags.clearFrom(tags_top);
            for (source_store.tagUnionTags(resolved)) |tag| {
                const payload_top = self.mono_scratches.idxs.top();
                defer self.mono_scratches.idxs.clearFrom(payload_top);
                for (source_store.getIdxListItems(tag.payloads)) |payload_mono| {
                    try self.mono_scratches.idxs.append(try self.importMonotypeFromStoreRec(source_store, payload_mono, from_module_idx, to_module_idx, imported));
                }
                try self.mono_scratches.tags.append(.{
                    .name = tag.name,
                    .payloads = try self.store.monotype_store.internIdxList(self.mono_scratches.idxs.sliceFromStart(payload_top)),
                });
            }
            break :blk try self.store.monotype_store.internTagUnion(self.mono_scratches.tags.sliceFromStart(tags_top));
        },
    ...
    self.store.monotype_store.finalizeRecursive(placeholder, result);
    return result;
}

fn monotypeIdxIsValid(_: *const Self, monotype: Monotype.Idx) bool {
    return !monotype.isNone();
}

fn monotypeIsUnit(_: *const Self, monotype: Monotype.Idx) bool {
    return !monotype.isNone() and monotype.isUnit();
}
...
fn monotypeIsWellFormedRec(
    ...
    return switch (resolved.kind) {
        .builtin => true,
        .rec => false,
        .list => self.monotypeIsWellFormedRec(self.store.monotype_store.listElem(resolved), seen),
        .box => self.monotypeIsWellFormedRec(self.store.monotype_store.boxInner(resolved), seen),
        .tuple => blk: {
            for (self.store.monotype_store.tupleElems(resolved)) |elem| {
        ...
        .func => blk: {
            ...
            break :blk self.monotypeIsWellFormedRec(self.store.monotype_store.funcRet(resolved), seen);
        },
        .record => blk: {
            for (self.store.monotype_store.recordFields(resolved)) |field| {
                if (!self.monotypeIsWellFormedRec(field.ty, seen)) break :blk false;
            }
            break :blk true;
        },
        .tag_union => blk: {
...
fn lowerExprWithMonotypeOverride(
    ...
    if (resolved.desc.content == .err) {
        return switch (expr) {
            ...
            .e_call => |call_expr| self.lowerCall(module_env, expr_idx, call_expr, self.store.monotype_store.unit_idx, region),
    ...
    return switch (expr) {
        ...
        .e_lookup_local => |lookup| {
            ...
            if (!self.lowered_symbols.contains(symbol_key) and !self.in_progress_defs.contains(symbol_key)) {
                ...
                const monotype_matches_cached = cached_monotype == monotype;
            ...
            if (self.symbol_monotypes.get(symbol.raw())) |bound_monotype| {
                const same_bound_mono = bound_monotype == monotype;
                if (same_bound_mono) {
        ...
        .e_lookup_external => |ext| {
            ...
            const symbol_key: u64 = @bitCast(symbol);
            if (!self.lowered_symbols.contains(symbol_key) and !self.in_progress_defs.contains(symbol_key)) {
                const ext_def_idx: CIR.Def.Idx = @enumFromInt(ext.target_node_idx);
                const ext_def = target_env.store.getDef(ext_def_idx);
                _ = try self.lowerExternalDefWithType(symbol, ext_def.expr, if (self.monotypeIsWellFormed(monotype)) .{ .idx = monotype, .module_idx = self.current_module_idx } else null);
                return try self.store.addExpr(self.allocator, .{ .lookup = symbol }, monotype, region);
            }
            ...
            if (self.lowered_symbols.get(symbol_key)) |cached_expr| {
                const cached_monotype = self.store.typeOf(cached_expr);
                const monotype_matches_cached = cached_monotype == normalized_lookup_monotype;
                if (!normalized_lookup_monotype.isNone() and !monotype_matches_cached) {
                    if (self.lookupPolySpecialization(symbol_key, normalized_lookup_monotype)) |spec_symbol| {
                        return try self.store.addExpr(self.allocator, .{ .lookup = spec_symbol }, monotype, region);
                    }
                }
                return try self.store.addExpr(self.allocator, .{ .lookup = symbol }, monotype, region);
...
fn lookupPolySpecialization(self: *Self, symbol_key: u64, caller_monotype: Monotype.Idx) ?MIR.Symbol {
    return self.poly_specializations.get(polySpecKey(symbol_key, caller_monotype));
}
...
fn bindPatternMonotypes(
    ...
    switch (pattern) {
        ...
        .applied_tag => |tag| {
            const resolved_mono = self.store.monotype_store.resolve(monotype);
            const mono_tags = if (resolved_mono.kind == .tag_union)
                self.store.monotype_store.tagUnionTags(resolved_mono)
            else
                typeBindingInvariant(
            ...
            const mono_payloads = self.store.monotype_store.getIdxListItems(mono_tags[tag_idx].payloads);
        ...
        .record_destructure => |record_pat| {
            const resolved_mono = self.store.monotype_store.resolve(monotype);
            const mono_fields = if (resolved_mono.kind == .record)
                self.store.monotype_store.recordFields(resolved_mono)
            else if (resolved_mono.isUnit())
                &[_]Monotype.FieldKey{}
            else
                typeBindingInvariant(
            ...
            for (module_env.store.sliceRecordDestructs(record_pat.destructs)) |destruct_idx| {
                ...
                try self.bindPatternMonotypes(module_env, pat_idx, mono_fields[field_idx].ty);
            }
        },
        .list => |list_pat| {
            const resolved_mono = self.store.monotype_store.resolve(monotype);
            const elem_mono = if (resolved_mono.kind == .list)
                self.store.monotype_store.listElem(resolved_mono)
            else
                typeBindingInvariant(
        ...
        .tuple => |tuple_pat| {
            const resolved_mono = self.store.monotype_store.resolve(monotype);
            const mono_elems = if (resolved_mono.kind == .tuple)
                self.store.monotype_store.tupleElems(resolved_mono)
            else
                typeBindingInvariant(
...
fn bindProcTemplateBoundaryMonotypes(
    ...
    const resolved_fn = self.store.monotype_store.resolve(fn_monotype);
    if (resolved_fn.kind != .func) unreachable;
    const arg_monos = self.store.monotype_store.funcArgs(resolved_fn);
    ...
    try self.bindTypeVarMonotypes(ModuleEnv.varFrom(boundary.body_expr), self.store.monotype_store.funcRet(resolved_fn));
}
...
fn lowerLambdaSpecialized(
    ...
    const ret_monotype = blk: {
        const resolved_func = self.store.monotype_store.resolve(monotype);
        break :blk if (resolved_func.kind == .func) self.store.monotype_store.funcRet(resolved_func) else unreachable;
    };
...
fn reserveProcInstSkeleton(self: *Self, proc_inst_id: Monomorphize.ProcInstId) Allocator.Error!MIR.ProcId {
    ...
    const resolved_proc_func = self.store.monotype_store.resolve(proc_monotype);
    if (resolved_proc_func.kind != .func) unreachable;

    const proc_id = try self.store.addProc(self.allocator, .{
        .fn_monotype = proc_monotype,
        .params = MIR.PatternSpan.empty(),
        .body = MIR.ExprId.none,
        .ret_monotype = self.store.monotype_store.funcRet(resolved_proc_func),
        .debug_name = MIR.Symbol.none,
...
fn buildClosureValueFromCaptureRequests(
    ...
    const captures_tuple_monotype = try self.store.monotype_store.internTuple(capture_monotypes);
...
fn lowerClosureProcBodyForSeed(
    ...
    const orig_func_resolved = self.store.monotype_store.resolve(monotype);
    ...
    const captures_tuple_monotype = if (capture_monotypes_snapshot.items.len == 0)
        ...
        break :blk try self.store.monotype_store.internTuple(capture_monotypes_snapshot.items);
    };
    ...
    self.store.getProcPtr(proc_id).* = .{
        ...
        .params = all_params,
        .body = lifted_body,
        .ret_monotype = self.store.monotype_store.funcRet(orig_func_resolved),
        .debug_name = MIR.Symbol.none,
        .source_region = region,
...
fn monotypeCanRefine(
    ...
    const ms = &self.store.monotype_store;
    const existing_resolved = ms.resolve(existing);
    const candidate_resolved = ms.resolve(candidate);

    if (existing_resolved.kind != candidate_resolved.kind) {
        return existing_resolved.isUnit();
    }

    return switch (existing_resolved.kind) {
        .builtin => existing_resolved.isUnit(),
        .rec => false,
        .list => self.monotypeCanRefine(ms.listElem(existing_resolved), ms.listElem(candidate_resolved)),
        .box => self.monotypeCanRefine(ms.boxInner(existing_resolved), ms.boxInner(candidate_resolved)),
        .tuple => blk: {
            const existing_elems = ms.tupleElems(existing_resolved);
            const candidate_elems = ms.tupleElems(candidate_resolved);
        ...
        .func => blk: {
            const existing_args = ms.funcArgs(existing_resolved);
            const candidate_args = ms.funcArgs(candidate_resolved);
            if (existing_args.len != candidate_args.len) break :blk false;
            if (ms.funcEffectful(existing_resolved) != ms.funcEffectful(candidate_resolved)) break :blk false;
            ...
            break :blk try self.monotypeCanRefine(ms.funcRet(existing_resolved), ms.funcRet(candidate_resolved));
        },
        .record => blk: {
            const existing_fields = ms.recordFields(existing_resolved);
            const candidate_fields = ms.recordFields(candidate_resolved);
            ...
            for (existing_fields, candidate_fields) |existing_field, candidate_field| {
                ...
                if (!(try self.monotypeCanRefine(existing_field.ty, candidate_field.ty))) break :blk false;
            }
            break :blk true;
        },
        .tag_union => blk: {
            const existing_tags = ms.tagUnionTags(existing_resolved);
            const candidate_tags = ms.tagUnionTags(candidate_resolved);
            ...
            for (existing_tags, candidate_tags) |existing_tag, candidate_tag| {
                ...
                const existing_payloads = ms.getIdxListItems(existing_tag.payloads);
                const candidate_payloads = ms.getIdxListItems(candidate_tag.payloads);
...
fn lookupMonomorphizedProcInst(
    ...
    for (self.monomorphization.proc_insts.items, 0..) |proc_inst, idx| {
        ...
        if (imported_proc_mono.eql(fn_monotype)) {
    ...
    if (std.debug.runtime_safety) {
        ...
        std.debug.panic(
            ...
            .{
                ...
                fn_monotype_module_idx,
                @as(u32, @bitCast(fn_monotype)),
                @as(u32, @bitCast(self.store.monotype_store.resolve(fn_monotype))),
            },
        );
    }
    unreachable;
...
fn procInstReturnMonotype(self: *Self, proc_inst_id: Monomorphize.ProcInstId) Allocator.Error!Monotype.Idx {
    ...
    const resolved_import = self.store.monotype_store.resolve(imported_fn_mono);
    return if (resolved_import.kind == .func) self.store.monotype_store.funcRet(resolved_import) else unreachable;
}
    ...
    if (call_low_level_op) |ll_op| {
        if (ll_op == .list_append_unsafe) {
            ...
            if (arg_exprs.len == 2) {
                ...
                const elem_monotype = if (self.store.monotype_store.resolve(list_mono_idx).kind == .list)
                    self.store.monotype_store.listElem(self.store.monotype_store.resolve(list_mono_idx))
                else
                    Monotype.Idx.none;
...
fn lowerCallWithLoweredFunc(
    ...
    const args_top = self.scratch_expr_ids.top();
    defer self.scratch_expr_ids.clearFrom(args_top);
    const expected_arg_monotypes = if (self.store.monotype_store.resolve(func_mono).kind == .func)
        self.store.monotype_store.funcArgs(self.store.monotype_store.resolve(func_mono))
    else
        &[_]Monotype.Idx{};
...
fn lowerBlock(self: *Self, module_env: *const ModuleEnv, block: anytype, monotype: Monotype.Idx, region: Region) Allocator.Error!MIR.ExprId {
    ...
    for (deferred_decl_slots.items) |slot| {
        ...
        const stmt_pattern = if (pattern_monotype == lowered_monotype)
            slot.pattern
        else
    ...
    if (da.args) |args_span| {
        ...
        if (uses_runtime_receiver and std.mem.eql(u8, module_env.getIdent(da.field_name), "contains")) {
            ...
            if (explicit_args.len == 1) {
                ...
                if (!receiver_monotype.isNone() and self.store.monotype_store.resolve(receiver_monotype).kind == .list) {
                    const list_elem = self.store.monotype_store.listElem(self.store.monotype_store.resolve(receiver_monotype));
                    const receiver = try self.lowerExpr(da.receiver);
                    const lowered_arg = if (!list_elem.isNone() and self.monotypeIsWellFormed(list_elem))
                        try self.lowerExprWithMonotypeOverride(explicit_args[0], list_elem)
                    else
        ...
        for (explicit_args, 0..) |arg_idx, i| {
            ...
            if (builtin.mode == .Debug and param_i >= expected_param_monos.len) {
                std.debug.panic(
                    "MIR Lower invariant: dispatch proc arg arity mismatch for dot access '{s}' ({d} params, arg index {d})",
                    .{ module_env.getIdent(da.field_name), expected_param_monos.len, param_i },
                );
            }
            const arg_override = if (param_i < expected_param_monos.len and self.monotypeIsWellFormed(expected_param_monos[param_i]))
                expected_param_monos[param_i]
            else
                Monotype.Idx.none;
...
fn findModuleForOrigin(self: *Self, source_env: *const ModuleEnv, origin_module: Ident.Idx) u32 {
    return self.findModuleForOriginMaybe(source_env, origin_module) orelse {
        if (builtin.mode == .Debug) {
            std.debug.panic(
                "findModuleForOrigin: no module found for origin '{s}'",
                .{source_env.getIdent(origin_module)},
            );
        }
        unreachable;
    };
...
fn monotypeDispatchCompatible(expected: Monotype.Idx, actual: Monotype.Idx) bool {
...
fn resolveDispatchTargetForDotCall(
    ...
    for (self.types_store.sliceAllStaticDispatchConstraints()) |constraint| {
        ...
        const compatible = if (fn_args.len == 0)
            ...
            monotypeDispatchCompatible(fn_args[0], receiver_monotype);
...
fn resolveUnresolvedTypeVarDispatchTarget(
    ...
    for (self.all_module_envs, 0..) |candidate_env, candidate_module_idx_usize| {
        ...
        for (defs) |def_idx| {
            ...
            if (candidate_mono != desired_func_monotype) continue;
...
fn specializeMethod(self: *Self, symbol: MIR.Symbol, caller_func_monotype: ?Monotype.Idx) Allocator.Error!MIR.Symbol {
    ...
    if (caller_func_monotype) |caller_monotype| {
        if (!caller_monotype.isNone()) {
            ...
            if (self.lookupPolySpecialization(symbol_key, normalized_caller_monotype)) |spec_symbol| {
                return spec_symbol;
            }
            ...
            if (self.in_progress_defs.contains(symbol_key)) {
                ...
                const same_specialization = if (active_monotype.isNone())
                    ...
                    active_monotype == normalized_caller_monotype;
                ...
                if (!same_specialization) {
                    ...
                    _ = try self.lowerExternalDefWithType(spec_symbol, def_expr, .{ .idx = caller_monotype, .module_idx = self.current_module_idx });
                    return spec_symbol;
                }
            }
            ...
            if (self.lowered_symbols.get(symbol_key)) |cached_expr| {
                const cached_monotype = self.store.typeOf(cached_expr);
                if (cached_monotype != normalized_caller_monotype) {
                    const def_expr = self.findDefExprByMetadata(symbol_meta) orelse {
                        if (std.debug.runtime_safety) {
                            std.debug.panic(
    ...
    _ = try self.lowerExternalDefWithType(symbol, def_expr, if (caller_func_monotype) |cm| .{ .idx = cm, .module_idx = self.current_module_idx } else null);
    return symbol;
}
    ...
    const result = if (cirExprIsProcBacked(current_env, cir_expr_idx)) blk: {
        ...
        const proc_inst_id = proc_inst_blk: {
            if (requested_monotype) |req| {
                if (self.monotypeIsWellFormed(req.idx) and self.store.monotype_store.resolve(req.idx).kind == .func) {
                ...
                if (builtin.mode == .Debug) {
                    ...
                    std.debug.panic(
                        ...
                        .{
                            ...
                            if (requested_monotype) |req| @as(u32, @bitCast(req.idx)) else std.math.maxInt(u32),
