...
pub const Pass = struct {
    ...
    fn finalizeResolvedDirectCallProcInst(
        ...
        const resolved_fn_mono = result.monotype_store.resolve(proc_inst.fn_monotype);
        std.debug.assert(resolved_fn_mono.kind == .func);
        const arg_exprs = module_env.store.sliceExpr(call_expr.args);
        ...
        try self.recordCurrentExprMonotype(
            ...
            module_idx,
            call_expr_idx,
            result.monotype_store.funcRet(resolved_fn_mono),
            proc_inst.fn_monotype_module_idx,
        );
    }
    ...
    fn assignCallableArgProcInstsFromParams(
        ...
        param_monos: Monotype.IdxListId,
        fn_monotype_module_idx: u32,
    ) Allocator.Error!void {
        const module_env = self.all_module_envs[module_idx];
        const param_mono_items = result.monotype_store.getIdxListItems(param_monos);
        if (arg_exprs.len != param_mono_items.len) unreachable;

        for (arg_exprs, 0..) |arg_expr_idx, i| {
            const param_mono = param_mono_items[i];
    ...
    fn prepareCallableArgsForProcInst(
        ...
        const resolved_fn_mono = result.monotype_store.resolve(proc_inst.fn_monotype);
        std.debug.assert(resolved_fn_mono.kind == .func);
        const proc_inst_fn_payload = result.monotype_store.getFuncPayload(resolved_fn_mono);

        try self.assignCallableArgProcInstsFromParams(
            result,
            module_idx,
            arg_exprs,
            proc_inst_fn_payload.args,
            proc_inst.fn_monotype_module_idx,
        );
    ...
    fn assignCallableArgProcInstsFromCallMonotype(
        ...
        const resolved_callee = result.monotype_store.resolve(callee_monotype.idx);
        if (resolved_callee.kind != .func) return;
        const fn_payload = result.monotype_store.getFuncPayload(resolved_callee);

        const arg_exprs = self.all_module_envs[module_idx].store.sliceExpr(call_expr.args);
        const fn_args = result.monotype_store.getIdxListItems(fn_payload.args);
        if (arg_exprs.len != fn_args.len) return;
    ...
    fn resolveDirectCallFnMonotype(
        ...
        return result.monotype_store.internFunc(arg_monotypes.items, ret_monotype, effectful);
    }
    ...
    fn bindCurrentPatternFromResolvedMonotype(
        ...
        switch (pattern) {
            ...
            .applied_tag => |tag_pat| {
                const resolved_tag_type = result.monotype_store.resolve(resolved_mono.idx);
                if (resolved_tag_type.kind != .tag_union) return;
                const mono_tags = result.monotype_store.tagUnionTags(resolved_tag_type);
                const tag_idx = self.tagIndexByNameInSlice(
                    &result.monotype_store,
                    module_idx,
                    tag_pat.name,
                    resolved_mono.module_idx,
                    mono_tags,
                ...
                const mono_payloads = result.monotype_store.getIdxListItems(mono_tag.payloads);
            ...
            .record_destructure => |record_pat| {
                const resolved_rec_type = result.monotype_store.resolve(resolved_mono.idx);
                const mono_fields: []const Monotype.FieldKey = switch (resolved_rec_type.kind) {
                    .record => result.monotype_store.recordFields(resolved_rec_type),
                    .builtin => if (resolved_rec_type.isUnit()) &.{} else return,
                    else => return,
                };
                for (module_env.store.sliceRecordDestructs(record_pat.destructs)) |destruct_idx| {
                    const destruct = module_env.store.getRecordDestruct(destruct_idx);
                    switch (destruct.kind) {
                        .Required, .SubPattern => |sub_pattern_idx| {
                            ...
                            try self.bindCurrentPatternFromResolvedMonotype(
                                ...
                                module_idx,
                                sub_pattern_idx,
                                resolvedMonotype(mono_fields[field_idx].ty, resolved_mono.module_idx),
                            );
                        },
                        .Rest => {},
                    }
            ...
            .list => |list_pat| {
                const resolved_list_type = result.monotype_store.resolve(resolved_mono.idx);
                if (resolved_list_type.kind != .list) return;
                const elem_mono = result.monotype_store.listElem(resolved_list_type);
            ...
            .tuple => |tuple_pat| {
                const resolved_tuple_type = result.monotype_store.resolve(resolved_mono.idx);
                if (resolved_tuple_type.kind != .tuple) return;
                const mono_elems = result.monotype_store.tupleElems(resolved_tuple_type);
    ...
    fn bindCurrentCallFromProcInst(
        ...
        const proc_inst = result.getProcInst(proc_inst_id);
        const resolved_fn = result.monotype_store.resolve(proc_inst.fn_monotype);
        if (resolved_fn.kind != .func) unreachable;
        const fn_payload = result.monotype_store.getFuncPayload(resolved_fn);

        const arg_exprs = self.all_module_envs[module_idx].store.sliceExpr(call_expr.args);
        const param_monos = result.monotype_store.funcArgs(resolved_fn);
        ...
        try self.bindCurrentExprTypeRoot(result, module_idx, call_expr_idx, fn_payload.ret, proc_inst.fn_monotype_module_idx);
        try self.recordCurrentExprMonotype(result, module_idx, call_expr_idx, fn_payload.ret, proc_inst.fn_monotype_module_idx);
    }
    ...
    fn bindCurrentCallFromFnMonotype(
        ...
        const resolved_fn2 = result.monotype_store.resolve(fn_monotype);
        if (resolved_fn2.kind != .func) return;
        const fn_payload2 = result.monotype_store.getFuncPayload(resolved_fn2);
        ...
        const arg_exprs = self.all_module_envs[module_idx].store.sliceExpr(call_expr.args);
        const param_monos = result.monotype_store.funcArgs(resolved_fn2);
        ...
        try self.bindCurrentExprTypeRoot(result, module_idx, call_expr_idx, fn_payload2.ret, fn_monotype_module_idx);
        try self.recordCurrentExprMonotype(result, module_idx, call_expr_idx, fn_payload2.ret, fn_monotype_module_idx);
    }
    ...
    fn bindCurrentDispatchFromProcInst(
        ...
        const proc_inst = result.getProcInst(proc_inst_id);
        const resolved_dispatch_fn = result.monotype_store.resolve(proc_inst.fn_monotype);
        if (resolved_dispatch_fn.kind != .func) unreachable;
        const dispatch_fn_payload = result.monotype_store.getFuncPayload(resolved_dispatch_fn);

        var actual_args = std.ArrayList(CIR.Expr.Idx).empty;
        defer actual_args.deinit(self.allocator);
        try self.appendDispatchActualArgs(module_idx, expr, &actual_args);

        const param_monos = result.monotype_store.funcArgs(resolved_dispatch_fn);
        ...
        try self.bindCurrentExprTypeRoot(result, module_idx, expr_idx, dispatch_fn_payload.ret, proc_inst.fn_monotype_module_idx);
        try self.recordCurrentExprMonotype(result, module_idx, expr_idx, dispatch_fn_payload.ret, proc_inst.fn_monotype_module_idx);
    }
            ...
            if (std.debug.runtime_safety) {
                ...
                std.debug.panic(
                    ...
                    .{
                        ...
                        @tagName(expr),
                        expr_region,
                        @as(u32, @bitCast(existing.idx)),
                        existing.module_idx,
                        @as(u32, @bitCast(existing.idx)),
                        @as(u32, @bitCast(resolved.idx)),
                        resolved.module_idx,
                        @as(u32, @bitCast(resolved.idx)),
    ...
    fn recordDemandedRecordFieldMonotypes(
        ...
        const resolved_rec = result.monotype_store.resolve(record_mono.idx);
        if (resolved_rec.kind != .record) return;
        const mono_fields = result.monotype_store.recordFields(resolved_rec);

        const module_env = self.all_module_envs[module_idx];
        for (module_env.store.sliceRecordFields(record_expr.fields)) |field_idx| {
            const field = module_env.store.getRecordField(field_idx);
            const mono_field_idx = self.recordFieldIndexByNameInSlice(
                &result.monotype_store,
                module_idx,
                field.name,
            ...
            const mono_field = mono_fields[mono_field_idx];
            try self.recordCurrentExprMonotype(
                result,
                module_idx,
                field.value,
    ...
    fn dotDispatchHandledWithoutProcInst(
        ...
        const resolved_receiver = result.monotype_store.resolve(receiver_monotype);
        return switch (resolved_receiver.kind) {
            .record, .tuple, .list => true,
            .builtin => resolved_receiver.isUnit(),
    ...
    fn binopDispatchHandledWithoutProcInst(
        ...
        const resolved_lhs = result.monotype_store.resolve(lhs_monotype);
        return switch (resolved_lhs.kind) {
            .record, .tuple, .list => true,
            .builtin => true, // covers unit and prim
            .tag_union => blk: {
    ...
    fn lookupDispatchConstraintForAssociatedMethod(
        ...
        for (module_env.types.sliceAllStaticDispatchConstraints()) |constraint| {
            ...
            const resolved_fn_mono = result.monotype_store.resolve(fn_monotype.idx);
            if (resolved_fn_mono.kind != .func) continue;

            const fn_args = result.monotype_store.funcArgs(resolved_fn_mono);
    ...
    fn resolveDispatchTargetForDotCall(
        ...
        for (module_env.types.sliceAllStaticDispatchConstraints()) |constraint| {
            ...
            const resolved_fn_mono2 = result.monotype_store.resolve(fn_mono.idx);
            if (resolved_fn_mono2.kind != .func) continue;
            const fn_args = result.monotype_store.funcArgs(resolved_fn_mono2);
    ...
    fn resolvedDispatchTargetMatchesInvocationSignature(
        ...
        const resolved_candidate = result.monotype_store.resolve(candidate_mono);
        if (resolved_candidate.kind != .func) return false;
        const candidate_payload = result.monotype_store.getFuncPayload(resolved_candidate);
        const candidate_args = result.monotype_store.funcArgs(resolved_candidate);

        var actual_args = std.ArrayList(CIR.Expr.Idx).empty;
        defer actual_args.deinit(self.allocator);
        try self.appendDispatchActualArgs(source_module_idx, source_expr, &actual_args);

        if (candidate_args.len != actual_args.items.len) return false;
        ...
        for (actual_args.items, 0..) |arg_expr_idx, i| {
            ...
            const expected_mono = candidate_args[i];
            if (!try self.monotypesStructurallyEqualAcrossModules(
                result,
                expected_mono,
                target_def.module_idx,
    ...
    fn lookupAssociatedMethodTemplateForMonotype(
        ...
        const resolved_mono = result.monotype_store.resolve(monotype);

        const type_ident: Ident.Idx = switch (resolved_mono.kind) {
            .builtin => blk: {
                if (resolved_mono.isUnit()) return null;
                const prim = resolved_mono.builtinPrim() orelse return null;
    ...
    fn ensureBuiltinBoxUnboxProcInst(
        ...
        const fn_monotype = try result.monotype_store.internFunc(&.{box_monotype}, inner_monotype, false);
    ...
    fn resolveTypeVarMonotypeWithBindings(
        ...
        // module_idx tracked via scratches.module_env
    ...
    fn remapMonotypeBetweenModulesRec(
        ...
        const resolved = result.monotype_store.resolve(monotype);
        switch (resolved.kind) {
            .builtin => return monotype, // unit and prim are identity-mapped
            .rec => {
                if (builtin.mode == .Debug) {
                    std.debug.panic("remapMonotypeBetweenModules: unexpected rec indirection", .{});
                }
                unreachable;
            },
        ...
        // Use a rec slot as a placeholder for cycle-breaking
        const placeholder = try result.monotype_store.reserveRecursive();
        try remapped.put(monotype, placeholder);

        const mapped_mono: Monotype.Idx = switch (resolved.kind) {
            .list => try result.monotype_store.internList(
                try self.remapMonotypeBetweenModulesRec(
                    result,
                    result.monotype_store.listElem(resolved),
                    from_module_idx,
                    to_module_idx,
                    remapped,
                    scratches,
            ...
            .box => try result.monotype_store.internBox(
                try self.remapMonotypeBetweenModulesRec(
                    result,
                    result.monotype_store.boxInner(resolved),
                    from_module_idx,
                    to_module_idx,
                    remapped,
                    scratches,
            ...
            .tuple => blk: {
                ...
                for (result.monotype_store.tupleElems(resolved)) |elem_mono| {
                ...
                break :blk try result.monotype_store.internTuple(
                    scratches.idxs.sliceFromStart(idx_top),
                );
            },
            .func => blk: {
                const func_payload = result.monotype_store.getFuncPayload(resolved);
                const idx_top = scratches.idxs.top();
                defer scratches.idxs.clearFrom(idx_top);

                for (result.monotype_store.funcArgs(resolved)) |arg_mono| {
                    try scratches.idxs.append(try self.remapMonotypeBetweenModulesRec(
                        result,
                        arg_mono,
                        from_module_idx,
                ...
                break :blk try result.monotype_store.internFunc(
                    scratches.idxs.sliceFromStart(idx_top),
                    mapped_ret,
                    func_payload.effectful,
                );
            },
            .record => blk: {
                ...
                for (result.monotype_store.recordFields(resolved)) |field| {
                    try scratches.fields.append(.{
                        .name = field.name,
                        .ty = try self.remapMonotypeBetweenModulesRec(
                            result,
                            field.ty,
                            from_module_idx,
                            to_module_idx,
                ...
                break :blk try result.monotype_store.internRecord(
            ...
            .tag_union => blk: {
                ...
                for (result.monotype_store.tagUnionTags(resolved)) |tag| {
                    const payload_top = scratches.idxs.top();
                    defer scratches.idxs.clearFrom(payload_top);

                    for (result.monotype_store.getIdxListItems(tag.payloads)) |payload_mono| {
                        try scratches.idxs.append(try self.remapMonotypeBetweenModulesRec(
                            result,
                            payload_mono,
                            from_module_idx,
                    ...
                    const mapped_payloads = try result.monotype_store.internIdxList(
                ...
                break :blk try result.monotype_store.internTagUnion(
        ...
        result.monotype_store.finalizeRecursive(placeholder, mapped_mono);
        return mapped_mono;
    }
    ...
    fn bindFlatTypeMonotypesInStore(
        ...
        switch (flat_type) {
            .fn_pure, .fn_effectful, .fn_unbound => |func| {
                if (resolved_mono.kind != .func) unreachable;
                const mono_args = result.monotype_store.funcArgs(resolved_mono);
                const mono_ret = result.monotype_store.funcRet(resolved_mono);
                ...
                try self.bindTypeVarMonotypesInStore(result, module_idx, store_types, common_idents, bindings, func.ret, mono_ret);
            },
            .nominal_type => |nominal| {
                ...
                if (origin.eql(common_idents.builtin_module) and ident.eql(common_idents.list)) {
                    if (resolved_mono.kind != .list) unreachable;
                    const type_args = store_types.sliceNominalArgs(nominal);
                    if (type_args.len != 1) unreachable;
                    try self.bindTypeVarMonotypesInStore(result, module_idx, store_types, common_idents, bindings, type_args[0], result.monotype_store.listElem(resolved_mono));
                    return;
                }

                if (origin.eql(common_idents.builtin_module) and ident.eql(common_idents.box)) {
                    if (resolved_mono.kind != .box) unreachable;
                    const type_args = store_types.sliceNominalArgs(nominal);
                    if (type_args.len != 1) unreachable;
                    try self.bindTypeVarMonotypesInStore(result, module_idx, store_types, common_idents, bindings, type_args[0], result.monotype_store.boxInner(resolved_mono));
                    return;
                }

                if (origin.eql(common_idents.builtin_module) and builtinPrimForNominal(ident, common_idents) != null) {
                    if (resolved_mono.kind != .builtin) unreachable;
                    return;
                }
            ...
            .record => |record| {
                if (resolved_mono.kind == .builtin and resolved_mono.isUnit()) {
                ...
                const mono_fields = result.monotype_store.recordFields(resolved_mono);
                ...
                rows: while (true) {
                    ...
                    for (field_names, field_vars) |field_name, field_var| {
                        const field_idx = self.recordFieldIndexByName(&result.monotype_store, module_idx, field_name, module_idx, mono_fields);
            ...
            .record_unbound => |fields_range| {
                ...
                if (resolved_mono.kind != .record) unreachable;
                const mono_fields_ub = result.monotype_store.recordFields(resolved_mono);
                const fields = store_types.getRecordFieldsSlice(fields_range);
                for (fields.items(.name), fields.items(.var_)) |field_name, field_var| {
                    const field_idx = self.recordFieldIndexByName(&result.monotype_store, module_idx, field_name, module_idx, mono_fields_ub);
                    try self.bindTypeVarMonotypesInStore(
                        result,
                        module_idx,
                        store_types,
                        ...
                        bindings,
                        field_var,
                        mono_fields_ub[field_idx].ty,
                    );
                }
            },
            .tuple => |tuple| {
                if (resolved_mono.kind != .tuple) unreachable;
                const mono_elems = result.monotype_store.tupleElems(resolved_mono);
            ...
            .tag_union => |tag_union| {
                if (resolved_mono.kind != .tag_union) unreachable;
                const mono_tags = result.monotype_store.tagUnionTags(resolved_mono);
            ...
            .empty_record => {
                if (resolved_mono.kind != .builtin and resolved_mono.kind != .record) unreachable;
            },
            .empty_tag_union => {
                if (resolved_mono.kind != .tag_union) unreachable;
            },
                ...
                .nominal_type => |nominal| {
                    ...
                    if (nominal.origin_module.eql(common.builtin_module)) {
                        ...
                        if (ident.eql(common.box)) {
                            ...
                            const resolved_box = result.monotype_store.resolve(outer_mono);
                            if (resolved_box.kind != .box) unreachable;
                            try self.ensureBuiltinBoxUnboxProcInst(result, module_idx, outer_mono, result.monotype_store.boxInner(resolved_box));
                            if (type_args.len == 1) {
                                try self.resolveStrInspektHelperProcInstsForTypeVarWithSeen(result, module_idx, type_args[0], visiting);
                            }
                            return;
                            ...
                            if (!resolved_func.effectful) {
                                ...
                                if (param_vars.len == 1) {
                                    ...
                                    if (!method_func_mono.isNone()) {
                                        ...
                                        const resolved_method_fn = result.monotype_store.resolve(method_func_mono);
                                        if (resolved_method_fn.kind != .func) unreachable;
                                        const method_ret = result.monotype_store.funcRet(resolved_method_fn);
                                        const resolved_ret = result.monotype_store.resolve(method_ret);
                                        const is_str = resolved_ret.kind == .builtin and resolved_ret.builtinPrim() == .str;
                                        if (!is_str) {
    ...
    fn resolveStrInspektHelperProcInstsForMonotype(
        ...
        const resolved_inspekt = result.monotype_store.resolve(monotype);
        switch (resolved_inspekt.kind) {
            .builtin => {}, // unit and prim
            .list => try self.resolveStrInspektHelperProcInstsForMonotype(result, module_idx, result.monotype_store.listElem(resolved_inspekt)),
            .box => {
                const inner = result.monotype_store.boxInner(resolved_inspekt);
                try self.ensureBuiltinBoxUnboxProcInst(result, module_idx, monotype, inner);
                try self.resolveStrInspektHelperProcInstsForMonotype(result, module_idx, inner);
            },
            .tuple => {
                for (result.monotype_store.tupleElems(resolved_inspekt)) |elem_mono| {
                    try self.resolveStrInspektHelperProcInstsForMonotype(result, module_idx, elem_mono);
                }
            },
            .func => {},
            .record => {
                for (result.monotype_store.recordFields(resolved_inspekt)) |field| {
                    try self.resolveStrInspektHelperProcInstsForMonotype(result, module_idx, field.ty);
                }
            },
            .tag_union => {
                for (result.monotype_store.tagUnionTags(resolved_inspekt)) |tag| {
                    for (result.monotype_store.getIdxListItems(tag.payloads)) |payload_mono| {
        ...
        return switch (module_env.store.getExpr(expr_idx)) {
            .e_record => |record| blk: {
                for (module_env.store.sliceRecordFields(record.fields)) |field_idx| {
                    const field = module_env.store.getRecordField(field_idx);
                    if (self.identsStructurallyEqualAcrossModules(&result.monotype_store, module_idx, field.name, field_name_module_idx, field_name)) {
    ...
    fn resolveTagPayloadExpr(
        ...
        return switch (module_env.store.getExpr(expr_idx)) {
            .e_tag => |tag_expr| blk: {
                if (!self.identsStructurallyEqualAcrossModules(&result.monotype_store, module_idx, tag_expr.name, tag_name_module_idx, tag_name)) {
                    break :blk null;
                }
    ...
    fn seedProcBodyBindingsFromSignature(
        ...
        const resolved_seed_fn = result.monotype_store.resolve(proc_inst.fn_monotype);
        if (resolved_seed_fn.kind != .func) return;
        const seed_fn_payload = result.monotype_store.getFuncPayload(resolved_seed_fn);
        ...
        const seed_fn_args = result.monotype_store.funcArgs(resolved_seed_fn);
        if (boundary.arg_patterns.len != seed_fn_args.len) {
            if (std.debug.runtime_safety) {
                std.debug.panic(
        ...
        for (boundary.arg_patterns, 0..) |pattern_idx, i| {
            const param_mono = seed_fn_args[i];
            try self.bindTypeVarMonotypes(
                result,
                module_idx,
                &module_env.types,
    ...
    fn seedTemplateBindingsFromFnMonotype(
        ...
        const module_env = self.all_module_envs[template.module_idx];
        const boundary = self.procBoundaryInfo(template.module_idx, template.cir_expr) orelse return;
        const resolved_tmpl_fn = result.monotype_store.resolve(fn_monotype.idx);
        if (resolved_tmpl_fn.kind != .func) return;
        const tmpl_fn_payload = result.monotype_store.getFuncPayload(resolved_tmpl_fn);
        const tmpl_fn_args = result.monotype_store.funcArgs(resolved_tmpl_fn);

        if (boundary.arg_patterns.len != tmpl_fn_args.len) {
            if (std.debug.runtime_safety) {
                std.debug.panic(
        ...
        for (boundary.arg_patterns, 0..) |pattern_idx, i| {
            const param_mono = tmpl_fn_args[i];
            try self.bindTypeVarMonotypes(
                result,
                template.module_idx,
                &module_env.types,
    ...
    fn labelTextAcrossModules(self: *Pass, store: *const Monotype.Store, module_idx: u32, label: anytype) []const u8 {
        return switch (@TypeOf(label)) {
            base.Ident.Idx => self.all_module_envs[module_idx].getIdent(label),
            Monotype.StructuralNameId => store.getNameText(label),
    ...
    fn identsStructurallyEqualAcrossModules(
        ...
        if (@TypeOf(lhs) == Monotype.StructuralNameId and @TypeOf(rhs) == Monotype.StructuralNameId and lhs == rhs) {
            return true;
        }

        const lhs_text = self.labelTextAcrossModules(store, lhs_module_idx, lhs);
        const rhs_text = self.labelTextAcrossModules(store, rhs_module_idx, rhs);
    ...
    fn recordFieldIndexByName(
        ...
        _ = mono_module_idx;
        const field_name_text = self.all_module_envs[template_module_idx].getIdent(field_name);
        for (mono_fields, 0..) |mono_field, field_idx| {
            if (std.mem.eql(u8, field_name_text, store.getNameText(mono_field.name))) {
        ...
        if (std.debug.runtime_safety) {
            std.debug.panic(
                "Monomorphize: record field '{s}' missing from monotype (template_module={d})",
                .{ field_name_text, template_module_idx },
            );
        }
        unreachable;
    }

    fn recordFieldIndexByNameInSlice(
        self: *Pass,
        store: *const Monotype.Store,
        template_module_idx: u32,
        field_name: base.Ident.Idx,
        ...
        return self.recordFieldIndexByName(store, template_module_idx, field_name, mono_module_idx, mono_fields);
    }

    fn tagIndexByNameInSlice(
        self: *Pass,
        store: *const Monotype.Store,
        template_module_idx: u32,
        ...
        _ = mono_module_idx;
        const tag_name_text = self.all_module_envs[template_module_idx].getIdent(tag_name);
        for (mono_tags, 0..) |mono_tag, tag_idx| {
            if (std.mem.eql(u8, tag_name_text, store.getNameText(mono_tag.name))) {
    ...
    fn remainingRecordTailMonotype(
        ...
        return try result.monotype_store.internRecord(remaining_fields.items);
    }
    ...
    fn remainingTagUnionTailMonotype(
        ...
        return try result.monotype_store.internTagUnion(remaining_tags.items);
    }
    ...
    fn bindTagPayloadsByName(
        ...
        for (mono_tags) |mono_tag| {
            if (!std.mem.eql(u8, tag_name_text, result.monotype_store.getNameText(mono_tag.name))) continue;
            const tag_payloads = result.monotype_store.getIdxListItems(mono_tag.payloads);

            if (payload_vars.len != tag_payloads.len) {
                if (std.debug.runtime_safety) {
                    std.debug.panic(
            ...
            for (payload_vars, 0..) |payload_var, i| {
                const mono_payload = tag_payloads[i];
                try self.bindTypeVarMonotypes(
                    result,
                    template_module_idx,
                    template_types,
                ...
                if (std.debug.runtime_safety) {
                    ...
                    std.debug.panic(
                        ...
                        .{
                            ...
                            existing.module_idx,
                            @as(u32, @bitCast(existing.idx)),
                            @as(u32, @bitCast(resolved_mono.idx)),
                            resolved_mono.module_idx,
                            @as(u32, @bitCast(resolved_mono.idx)),
                            @intFromEnum(self.active_proc_inst_context),
                            if (self.active_root_expr_context) |root_expr_idx| @intFromEnum(root_expr_idx) else std.math.maxInt(u32),
    ...
    fn bindFlatTypeMonotypes(
        ...
        const resolved_bind = result.monotype_store.resolve(monotype);
        const common_idents = ModuleEnv.CommonIdents.find(&self.all_module_envs[template_module_idx].common);

        switch (flat_type) {
            .fn_pure, .fn_effectful, .fn_unbound => |func| {
                if (resolved_bind.kind != .func) unreachable;
                const bind_fn_args = result.monotype_store.funcArgs(resolved_bind);
                const bind_fn_ret = result.monotype_store.funcRet(resolved_bind);

                const type_args = template_types.sliceVars(func.args);
                if (type_args.len != bind_fn_args.len) unreachable;

                for (type_args, 0..) |arg_var, i| {
                    const mono_arg = bind_fn_args[i];
                    try self.bindTypeVarMonotypes(
                        result,
                        template_module_idx,
                        template_types,
            ...
            .nominal_type => |nominal| {
                ...
                if (origin.eql(common_idents.builtin_module) and ident.eql(common_idents.list)) {
                    if (resolved_bind.kind != .list) unreachable;
                    ...
                    try self.bindTypeVarMonotypes(
                        ...
                        ordered_entries,
                        type_args[0],
                        result.monotype_store.listElem(resolved_bind),
                        mono_module_idx,
                    );
                    return;
                }
                ...
                if (origin.eql(common_idents.builtin_module) and ident.eql(common_idents.box)) {
                    if (resolved_bind.kind != .box) unreachable;
                    const type_args = template_types.sliceNominalArgs(nominal);
                    if (type_args.len != 1) unreachable;
                    try self.bindTypeVarMonotypes(
                        result,
                        ...
                        ordered_entries,
                        type_args[0],
                        result.monotype_store.boxInner(resolved_bind),
                        mono_module_idx,
                    );
                    return;
                }
                ...
                if (origin.eql(common_idents.builtin_module) and builtinPrimForNominal(ident, common_idents) != null) {
                    if (resolved_bind.kind != .builtin) unreachable;
                    return;
                }
            ...
            .record => |record| {
                if (resolved_bind.kind == .builtin and resolved_bind.isUnit()) {
                    if (flatRecordRepresentsEmpty(template_types, record)) return;
                    if (builtin.mode == .Debug) {
                        std.debug.panic("Monomorphize invariant violated: non-empty template record matched unit monotype (template_module={d}, mono_module={d}, active_proc_inst={d}, monotype={d})", .{ template_module_idx, mono_module_idx, @intFromEnum(self.active_proc_inst_context), @as(u32, @bitCast(monotype)) });
                    }
                    unreachable;
                }
                if (resolved_bind.kind != .record) {
                    if (builtin.mode == .Debug) {
                        std.debug.panic("Monomorphize invariant violated: expected record monotype for template record, got {s} (template_module={d}, mono_module={d}, active_proc_inst={d}, monotype={d})", .{ @tagName(resolved_bind.kind), template_module_idx, mono_module_idx, @intFromEnum(self.active_proc_inst_context), @as(u32, @bitCast(monotype)) });
                    }
                    unreachable;
                }
                const bind_rec_fields = result.monotype_store.recordFields(resolved_bind);
                var seen_field_indices: std.ArrayListUnmanaged(u32) = .empty;
                defer seen_field_indices.deinit(self.allocator);
                ...
                rows: while (true) {
                    ...
                    for (field_names, field_vars) |field_name, field_var| {
                        ...
                        try appendSeenIndex(self.allocator, &seen_field_indices, field_idx);
                        const mono_field = bind_rec_fields[field_idx];
                        try self.bindTypeVarMonotypes(
                            result,
                            template_module_idx,
                            template_types,
            ...
            .record_unbound => |fields_range| {
                ...
                if (resolved_bind.kind != .record) unreachable;
                const bind_ub_fields = result.monotype_store.recordFields(resolved_bind);
                ...
                for (field_names, field_vars) |field_name, field_var| {
                    ...
                    const mono_field = bind_ub_fields[field_idx];
                    try self.bindTypeVarMonotypes(
                        result,
                        template_module_idx,
                        template_types,
            ...
            .tuple => |tuple| {
                if (resolved_bind.kind != .tuple) unreachable;
                const bind_elems = result.monotype_store.tupleElems(resolved_bind);
                const elem_vars = template_types.sliceVars(tuple.elems);
                if (elem_vars.len != bind_elems.len) unreachable;
                for (elem_vars, 0..) |elem_var, i| {
                    const elem_mono = bind_elems[i];
                    try self.bindTypeVarMonotypes(
                        result,
                        template_module_idx,
                        template_types,
            ...
            .tag_union => |tag_union| {
                if (resolved_bind.kind != .tag_union) unreachable;
                const bind_tags = result.monotype_store.tagUnionTags(resolved_bind);
            ...
            .empty_record => {
                if (resolved_bind.kind != .builtin and resolved_bind.kind != .record) unreachable;
            },
            .empty_tag_union => {
                if (resolved_bind.kind != .tag_union) unreachable;
            },
    ...
    fn monotypesStructurallyEqualRec(
        ...
        const rhs_u32: u32 = @bitCast(rhs);
        const key: u64 = (@as(u64, lhs_u32) << 32) | @as(u64, rhs_u32);

        if (seen.contains(key)) return true;
        try seen.put(key, {});

        const lhs_resolved = result.monotype_store.resolve(lhs);
        const rhs_resolved = result.monotype_store.resolve(rhs);
        if (lhs_resolved.kind != rhs_resolved.kind) return false;

        return switch (lhs_resolved.kind) {
            .rec => unreachable,
            .builtin => lhs_resolved.eql(rhs_resolved),
            .list => try self.monotypesStructurallyEqualRec(result, result.monotype_store.listElem(lhs_resolved), result.monotype_store.listElem(rhs_resolved), seen),
            .box => try self.monotypesStructurallyEqualRec(result, result.monotype_store.boxInner(lhs_resolved), result.monotype_store.boxInner(rhs_resolved), seen),
            .tuple => blk: {
                const lhs_elems = result.monotype_store.tupleElems(lhs_resolved);
                const rhs_elems = result.monotype_store.tupleElems(rhs_resolved);
            ...
            .func => blk: {
                const lhs_args = result.monotype_store.funcArgs(lhs_resolved);
                const rhs_args = result.monotype_store.funcArgs(rhs_resolved);
                if (result.monotype_store.funcEffectful(lhs_resolved) != result.monotype_store.funcEffectful(rhs_resolved)) break :blk false;
                ...
                break :blk try self.monotypesStructurallyEqualRec(result, result.monotype_store.funcRet(lhs_resolved), result.monotype_store.funcRet(rhs_resolved), seen);
            },
            .record => blk: {
                const lhs_fields = result.monotype_store.recordFields(lhs_resolved);
                const rhs_fields = result.monotype_store.recordFields(rhs_resolved);
                ...
                for (lhs_fields, rhs_fields) |lhs_field, rhs_field| {
                    ...
                    if (!try self.monotypesStructurallyEqualRec(result, lhs_field.ty, rhs_field.ty, seen)) {
                        break :blk false;
                    }
                }
                break :blk true;
            ...
            .tag_union => blk: {
                const lhs_tags = result.monotype_store.tagUnionTags(lhs_resolved);
                const rhs_tags = result.monotype_store.tagUnionTags(rhs_resolved);
                if (lhs_tags.len != rhs_tags.len) break :blk false;
                for (lhs_tags, rhs_tags) |lhs_tag, rhs_tag| {
                    const lhs_payloads = result.monotype_store.getIdxListItems(lhs_tag.payloads);
                    const rhs_payloads = result.monotype_store.getIdxListItems(rhs_tag.payloads);
    ...
    fn monotypesStructurallyEqualAcrossModulesRec(
        ...
        return switch (lhs_resolved.kind) {
            .rec => unreachable,
            .builtin => lhs_resolved.eql(rhs_resolved),
            .list => try self.monotypesStructurallyEqualAcrossModulesRec(
                result,
                result.monotype_store.listElem(lhs_resolved),
                lhs_module_idx,
                result.monotype_store.listElem(rhs_resolved),
                rhs_module_idx,
                seen,
            ),
            .box => try self.monotypesStructurallyEqualAcrossModulesRec(
                result,
                result.monotype_store.boxInner(lhs_resolved),
                lhs_module_idx,
                result.monotype_store.boxInner(rhs_resolved),
                rhs_module_idx,
                seen,
            ),
            .tuple => blk: {
            ...
            .func => blk: {
                ...
                break :blk try self.monotypesStructurallyEqualAcrossModulesRec(
                    result,
                    result.monotype_store.funcRet(lhs_resolved),
                    lhs_module_idx,
                    result.monotype_store.funcRet(rhs_resolved),
                    rhs_module_idx,
                    seen,
                );
            },
            .record => blk: {
                ...
                for (lhs_fields) |lhs_field| {
                    ...
                    for (rhs_fields, 0..) |rhs_field, rhs_i| {
                        ...
                        // StructuralNameIds are canonical within the shared name pool
                        if (lhs_field.name != rhs_field.name) continue;
                        if (!try self.monotypesStructurallyEqualAcrossModulesRec(
                            result,
                            lhs_field.ty,
                            lhs_module_idx,
            ...
            .tag_union => blk: {
                ...
                for (lhs_tags) |lhs_tag| {
                    ...
                    for (rhs_tags, 0..) |rhs_tag, rhs_i| {
                        if (rhs_used.items[rhs_i]) continue;
                        // StructuralNameIds are canonical within the shared name pool
                        if (lhs_tag.name != rhs_tag.name) continue;
