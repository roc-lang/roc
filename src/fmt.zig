    ...
    fn formatHeader(fmt: *Formatter, hi: HeaderIdx) !FormattedOutput {
        ...
        switch (header) {
            .app => |a| {
                ...
                try fmt.formatCollection(provides_region, .square, IR.NodeStore.ExposedItemIdx, provides, Formatter.formatExposedItem);
                try fmt.pushAll(" { ");
                try fmt.pushTokenText(a.platform_name);
                try fmt.pushAll(": platform ");
                _ = try fmt.formatExpr(a.platform);
    ...
    fn formatTypeAnno(fmt: *Formatter, anno: IR.NodeStore.TypeAnnoIdx) !IR.Region {
        ...
        switch (a) {
            ...
            .@"fn" => |f| {
                ...
                // while (ii <= region.end) {
                //     if (tags[ii] == .Newline) {
                //         try fmt.pushAll("(dbg:found newline)");
                //         break;
                //     }
                //     if (tags[ii] == .Comma and (tags[ii + 1] == .CloseSquare or
                //         tags[ii + 1] == .CloseRound or
                //         tags[ii + 1] == .CloseCurly))
                //     {
                //         try fmt.pushAll("(dbg:found comma close)");
                //         break;
                //     }
                //     var gp = std.heap.GeneralPurposeAllocator(.{}){}; // mutable
                //     const allocator = gp.allocator(); // this is now *DebugAllocator, not *const
                //     defer _ = gp.deinit(); // don't forget to deinit if you're done with it

                //     try fmt.pushAll(try std.fmt.allocPrint(allocator, "(dbg:found {s})", .{@tagName(tags[ii])}));
                //     ii += 1;
                // }

                // if (multiline) {
                //     try fmt.pushAll("(dbg:multiline)");
                // } else {
                //     try fmt.pushAll("(dbg:singleline)");
                // }
                var i: usize = 0;
