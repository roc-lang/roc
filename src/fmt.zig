...
const TokenIdx = tokenizer.Token.List.Idx;
...
pub fn formatFile(fmt: *Formatter) []const u8 {
    ...
    const file = fmt.ast.store.getFile(.{ .id = 0 });
    ...
    return fmt.buffer.toOwnedSlice() catch exit_on_oom();
}
    ...
    switch (statement) {
        .decl => |d| {
            ...
            if (body.whitespace != null) {
                fmt.buffer.appendSlice(" =") catch exit_on_oom();
            } else {
                fmt.buffer.appendSlice(" = ") catch exit_on_oom();
            }
        ...
        .import => |i| {
            fmt.buffer.appendSlice("import ") catch exit_on_oom();
...
fn formatIdent(fmt: *Formatter, ident: TokenIdx, qualifier: ?TokenIdx) void {
    if (qualifier) |q| {
        fmt.pushTokenText(q);
        fmt.buffer.append('.') catch exit_on_oom();
    }
...
fn formatHeader(fmt: *Formatter, hi: HeaderIdx) void {
    ...
    switch (header) {
        .app => |a| {
            fmt.buffer.appendSlice("app [") catch exit_on_oom();
            // Format provides
            var i: usize = 0;
            for (a.provides) |p| {
                fmt.pushTokenText(p);
            ...
            fmt.buffer.appendSlice(": platform ") catch exit_on_oom();
...
fn pushIndent(fmt: *Formatter) void {
    ...
    for (0..fmt.curr_indent) |_| {
        fmt.buffer.appendSlice(indent) catch exit_on_oom();
    }
}
...
fn pushTokenText(fmt: *Formatter, ti: TokenIdx) void {
    ...
    fmt.buffer.appendSlice(text) catch exit_on_oom();
}
