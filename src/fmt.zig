...
pub const FormattingResult = struct {
    ...
    files_to_reformat: []const []const u8,
};
...
pub fn formatPath(gpa: std.mem.Allocator, arena: std.mem.Allocator, base_dir: std.fs.Dir, path: []const u8, check: bool) !FormattingResult {
    ...
    if (base_dir.openDir(path, .{ .iterate = true })) |const_dir| {
        ...
        while (try walker.next()) |entry| {
            if (entry.kind == .file) {
                if (formatFilePath(gpa, entry.dir, entry.basename, check)) |is_already_formatted| {
                    success_count += 1;
                    if (check and !is_already_formatted.?) {
                        try files_to_reformat.append(entry.basename);
                    }
                } else |err| {
        ...
        if (formatFilePath(gpa, base_dir, path, check)) |is_already_formatted| {
            success_count += 1;
            if (check and !is_already_formatted.?) {
    ...
    return .{ .success = success_count, .failure = failed_count, .files_to_reformat = try files_to_reformat.toOwnedSlice() };
}
...
pub fn formatFilePath(gpa: std.mem.Allocator, base_dir: std.fs.Dir, path: []const u8, check: bool) !?bool {
    ...
    if (check) {
        ...
        return std.mem.eql(u8, formatted_bytes, contents);
    }
    ...
    // We did not check if formatting changed the file, so we return null
    return null;
}
