...
fn getAnnotationHtmlTag(annotation: Annotation) []const u8 {
    return switch (annotation) {
        ...
        .code_block => "pre",
        .inline_code => "code",
        .keyword, .type_variable, .error_highlight, .warning_highlight, .suggestion, .symbol, .path, .literal, .comment, .underline, .dimmed, .symbol_qualified, .symbol_unqualified, .module_name, .record_field, .tag_name, .binary_operator, .source_region, .reflowing_text => "span",
    };
}
