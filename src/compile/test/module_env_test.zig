...
test "ModuleEnv serialization and interpreter evaluation" {
    ...
    const EvalBuiltinTypes = eval.BuiltinTypes;

    const Check = check.Check;
    const Allocators = base.Allocators;

    var test_env_instance = EvalTestEnv.init(gpa);
    ...
    {
        const builtin_types_local = EvalBuiltinTypes.init(builtin_indices, builtin_module.env, builtin_module.env, builtin_module.env);
        var interpreter = try EvalInterpreter.init(gpa, &original_env, builtin_types_local, builtin_module.env, &[_]*const can.ModuleEnv{}, &checker.import_mapping, null, null, roc_target.RocTarget.detectNative());
    ...
    // Test 2: Full serialization and deserialization with interpreter evaluation
    {
        ...
        // The original expression index should still be valid since the NodeStore structure is preserved
        {
            ...
            const builtin_types_local = EvalBuiltinTypes.init(builtin_indices, builtin_module.env, builtin_module.env, builtin_module.env);
            var interpreter = try EvalInterpreter.init(gpa, deserialized_env, builtin_types_local, builtin_module.env, &[_]*const can.ModuleEnv{}, &checker.import_mapping, null, null, roc_target.RocTarget.detectNative());
            defer interpreter.deinit();
