...
test "ModuleEnv.Serialized roundtrip" {
    ...
    try testing.expectEqual(@as(u32, 60), original.common.idents.interner.entry_count);
    ...
    // Should have same 60 identifiers as original: hello, world, TestModule + 18 well-known identifiers + 19 type identifiers + 3 field/tag identifiers + 7 more identifiers + 2 Try tag identifiers + 1 method identifier + 2 Bool tag identifiers + 6 from_utf8 identifiers from ModuleEnv.init()
    // (Note: "Try" is now shared with well-known identifiers, reducing total by 1)
    try testing.expectEqual(@as(u32, 60), env.common.idents.interner.entry_count);
