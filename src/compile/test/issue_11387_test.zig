...
//! ever reach a host call. The same programs written with `=>` check cleanly
//! and run against a host.
...
const Workspace = struct {
    ...
    fn init(app_source: []const u8, platform: PlatformFiles) !Workspace {
...
fn expectReports(app_path: []const u8, expectation: ReportExpectation) !void {
    ...
    try std.testing.expectEqual(lines.len, report_count);
}
...
fn runRoot(lowered: *const lir.CheckedPipeline.LoweredProgram) !void {
