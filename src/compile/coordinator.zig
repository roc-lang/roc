...
pub const Coordinator = struct {
    ...
    fn handleParsed(self: *Coordinator, result: *ParsedResult) !void {
        ...
        const pkg = self.packages.get(result.package_name) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: package '{s}' not found for parsed result (module={s}, id={})\n", .{
                result.package_name, result.module_name, result.module_id,
            });
            unreachable;
        };
        const mod = pkg.getModule(result.module_id) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' for parsed result (module={s})\n", .{
    ...
    fn handleCanonicalized(self: *Coordinator, result: *CanonicalizedResult) !void {
        ...
        const pkg = self.packages.get(result.package_name) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: package '{s}' not found for canonicalized result (module={s}, id={})\n", .{
                result.package_name, result.module_name, result.module_id,
            });
            unreachable;
        };
        const mod = pkg.getModule(result.module_id) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' for canonicalized result (module={s})\n", .{
                result.module_id, result.package_name, result.module_name,
            });
            unreachable;
        };
        ...
        for (result.discovered_local_imports.items) |imp| {
            ...
            const current_mod = pkg.getModule(result.module_id) orelse {
                if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' after ensureModule in canonicalized handler (module={s})\n", .{
                    result.module_id, result.package_name, result.module_name,
                });
                unreachable;
            };
        ...
        const mod_after_imports = pkg.getModule(result.module_id) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' after local imports in canonicalized handler (module={s})\n", .{
                result.module_id, result.package_name, result.module_name,
            });
            unreachable;
        };
    ...
    fn handleTypeChecked(self: *Coordinator, result: *TypeCheckedResult) !void {
        ...
        const pkg = self.packages.get(result.package_name) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: package '{s}' not found for type_checked result (module={s}, id={})\n", .{
                result.package_name, result.module_name, result.module_id,
            });
            unreachable;
        };
        const mod = pkg.getModule(result.module_id) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' for type_checked result (module={s})\n", .{
                result.module_id, result.package_name, result.module_name,
            });
            unreachable;
        };
    ...
    fn handleParseFailed(self: *Coordinator, result: *messages.ParseFailure) !void {
        ...
        const pkg = self.packages.get(result.package_name) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: package '{s}' not found for parse_failed result (module={s}, id={})\n", .{
                result.package_name, result.module_name, result.module_id,
            });
            unreachable;
        };
        const mod = pkg.getModule(result.module_id) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' for parse_failed result (module={s})\n", .{
                result.module_id, result.package_name, result.module_name,
            });
            unreachable;
        };
    ...
    fn handleCycleDetected(self: *Coordinator, result: *messages.CycleDetected) !void {
        const pkg = self.packages.get(result.package_name) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: package '{s}' not found for cycle_detected result (id={})\n", .{
                result.package_name, result.module_id,
            });
            unreachable;
        };
        const mod = pkg.getModule(result.module_id) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' for cycle_detected result\n", .{
    ...
    fn handleCacheHit(self: *Coordinator, result: *CacheHitResult) !void {
        ...
        const pkg = self.packages.get(result.package_name) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: package '{s}' not found for cache_hit result (module={s}, id={})\n", .{
                result.package_name, result.module_name, result.module_id,
            });
            unreachable;
        };
        const mod = pkg.getModule(result.module_id) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' for cache_hit result (module={s})\n", .{
                result.module_id, result.package_name, result.module_name,
            });
            unreachable;
        };
        ...
        const mod_after_imports = pkg.getModule(result.module_id) orelse {
            if (builtin.mode == .Debug and builtin.os.tag != .freestanding) std.debug.print("BUG: module id={} not found in package '{s}' after imports in cache_hit handler (module={s})\n", .{
                result.module_id, result.package_name, result.module_name,
            });
            unreachable;
        };
