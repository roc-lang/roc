...
// REGRESSION TEST: str_is_empty snapshot should return True/False, not TypeMismatch
test "Repl - Str.is_empty should work - REGRESSION TEST" {
    var test_env = TestEnv.init(std.testing.allocator);
    defer test_env.deinit();
    ...
    // Line 1: Str.is_empty("")
    const result1 = try repl.step("Str.is_empty(\"\")");
    defer std.testing.allocator.free(result1);

    // Should return True, not TypeMismatch
    if (std.mem.indexOf(u8, result1, "TypeMismatch") != null) {
        std.debug.print("\nexpected: True\nactual: {s}\n", .{result1});
        return error.TestUnexpectedResult;
    }
    try testing.expectEqualStrings("True", result1);

    // Line 2: Str.is_empty("a")
    const result2 = try repl.step("Str.is_empty(\"a\")");
    defer std.testing.allocator.free(result2);

    // Should return False, not TypeMismatch
    if (std.mem.indexOf(u8, result2, "TypeMismatch") != null) {
        std.debug.print("\nexpected: False\nactual: {s}\n", .{result2});
        return error.TestUnexpectedResult;
    }
    try testing.expectEqualStrings("False", result2);
}
