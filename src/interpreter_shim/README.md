...
5. The CIR is lowered through the CIR → MIR → LIR → RC pipeline, then the interpreter evaluates the result
...
4. The data is deserialized into a `ModuleEnv`, lowered through CIR → MIR → LIR → RC, and executed by the interpreter
