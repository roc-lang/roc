    ...
    while (cmd_idx < ncmds and offset + 8 <= bytes.len) : (cmd_idx += 1) {
        ...
        if (cmd == 0x19) {
            ...
            while (sect_idx < nsects and sect_offset + 80 <= bytes.len) : (sect_idx += 1) {
                ...
                // Relocatable Mach-O objects can leave the segment name empty, so
                // key off the section name directly instead of requiring __TEXT.
...
fn findRocEvalOffsetMachO(bytes: []const u8, _: []const u8) Error!usize {
...
fn dlsymLookup(name: []const u8) ?usize {
    if (resolveBuiltinWrapper(name)) |addr| return addr;
...
fn resolveBuiltinWrapper(name: []const u8) ?usize {
    // Refcount helpers referenced by unprefixed symbol names.
    const utils = builtins.utils;
    const utils_entries = [_]struct { name: []const u8, addr: usize }{
    ...
    for (utils_entries) |entry| {
        if (std.mem.eql(u8, name, entry.name)) return entry.addr;
    }

    // Every registered builtin resolves to its wrapper in-process, so a
    // symbol the codegen can emit is always resolvable here.
...
test "resolve every registered builtin wrapper" {
    // The dev backend can emit a relocation to any registry member, so the
    // JIT path must resolve all of them.
    inline for (comptime std.enums.values(builtins.builtin_registry.BuiltinFn)) |b| {
        try std.testing.expectEqual(
            @intFromPtr(&@field(dev_wrappers, b.symbolName())),
            resolveBuiltinWrapper(b.symbolName()) orelse return error.TestUnexpectedResult,
        );
    }
}
