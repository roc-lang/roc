...
pub fn MonoExprCodeGenFor(comptime CodeGen: type, comptime GeneralReg: type, comptime FloatReg: type, comptime Condition: type) type {
    return struct {
            ...
            switch (op) {
                ...
                .eq => {
                    if (comptime builtin.cpu.arch == .x86_64) {
                        const before_offset = self.codegen.currentOffset();
                        try self.codegen.emitCmp(.w64, result_reg, lhs_reg, rhs_reg, condEqual());
                        const after_offset = self.codegen.currentOffset();
                        std.debug.print("[DEBUG cmp eq] lhs_reg={}, rhs_reg={}, result_reg={}, bytes: ", .{ @intFromEnum(lhs_reg), @intFromEnum(rhs_reg), @intFromEnum(result_reg) });
                        const cmp_bytes = self.codegen.emit.buf.items[before_offset..after_offset];
                        for (cmp_bytes) |b| {
        ...
        fn generateIfThenElse(self: *Self, ite: anytype) Error!ValueLocation {
            ...
            if (comptime builtin.cpu.arch == .x86_64) {
                std.debug.print("[DEBUG if-then-else] x86_64 arch, {} branches\n", .{branches.len});
            }
            ...
            for (branches, 0..) |branch, branch_idx| {
                const before_cond_offset = self.codegen.currentOffset();

                // Generate condition
                const cond_loc = try self.generateExpr(branch.cond);
                const cond_reg = try self.ensureInGeneralReg(cond_loc);

                // DEBUG: Print condition code details
                if (comptime builtin.cpu.arch == .x86_64) {
                    const after_cond_offset = self.codegen.currentOffset();
                    std.debug.print("[DEBUG if-then-else] branch {} cond generated: offset {}->{}, reg={}\n", .{ branch_idx, before_cond_offset, after_cond_offset, @intFromEnum(cond_reg) });
                    // Print the generated bytes for condition
                    const cond_bytes = self.codegen.emit.buf.items[before_cond_offset..after_cond_offset];
                    std.debug.print("[DEBUG if-then-else] cond bytes: ", .{});
                    for (cond_bytes) |b| {
                        std.debug.print("{x:0>2} ", .{b});
                    }
                    std.debug.print("\n", .{});
            ...
            if (comptime builtin.cpu.arch == .x86_64) {
                std.debug.print("[DEBUG if-then-else] end_offset: {}, result_reg: {}\n", .{ end_offset, @intFromEnum(result_reg) });
            }
