...
pub fn LirCodeGen(comptime target: RocTarget) type {
    ...
    return struct {
        ...
        fn generateTagUnionComparisonByLayout(
            ...
            if (total_size == 0) {
                // Zero-sized tag union (shouldn't happen but handle gracefully)
                return .{ .immediate_i64 = if (op == .eq) 1 else 0 };
            }

            // Ensure both values on stack
            ...
            // Compare the full tag union word-by-word (discriminant + payload)
            var cmp_off: u32 = 0;
            while (cmp_off < total_size) {
                ...
                if (remaining < 8) {
                    // Mask to only compare meaningful bytes
                ...
                // Compare: if not equal, set result to 0
                if (comptime target.toCpuArch() == .aarch64) {
        ...
        fn generateTupleComparisonByLayout(
            ...
            // Get the tuple base offsets on the stack
            const lhs_base = try self.ensureRecordOnStack(lhs_loc, ls.layoutSizeAlign(stored_layout).size);
            const rhs_base = try self.ensureRecordOnStack(rhs_loc, ls.layoutSizeAlign(stored_layout).size);

            // Compare element by element, AND-ing the results together
            const result_reg = try self.allocTempGeneral();
            try self.codegen.emitLoadImm(result_reg, 1); // Start with "all equal"
            ...
            while (elem_i < elem_count) : (elem_i += 1) {
                ...
                if (elem_layout_idx == .dec or elem_layout_idx == .i128 or elem_layout_idx == .u128 or elem_size == 16) {
                    // 128-bit element: compare as i128 (two 64-bit parts)
                    ...
                    self.codegen.freeGeneral(lhs_reg);
                    self.codegen.freeGeneral(rhs_reg);
                } else if (elem_size == roc_str_size) {
                    // String/list-sized element: compare as 3 x pointer-sized chunks
                    const tmp_a = try self.allocTempGeneral();
                    const tmp_b = try self.allocTempGeneral();
                    const xor_acc = try self.allocTempGeneral();
                    try self.codegen.emitLoadImm(xor_acc, 0);
                    ...
                    var chunk: i32 = 0;
                    while (chunk < roc_str_size) : (chunk += target_ptr_size) {
                        try self.codegen.emitLoadStack(.w64, tmp_a, lhs_elem_off + chunk);
                        try self.codegen.emitLoadStack(.w64, tmp_b, rhs_elem_off + chunk);
                        if (comptime target.toCpuArch() == .aarch64) {
                            try self.codegen.emit.eorRegRegReg(.w64, tmp_a, tmp_a, tmp_b);
                            try self.codegen.emit.orrRegRegReg(.w64, xor_acc, xor_acc, tmp_a);
                        } else {
                ...
                // AND element result into accumulator: result &= elem_eq
                if (comptime target.toCpuArch() == .aarch64) {
...
test "code generator initialization" {
    ...
    var codegen = HostLirCodeGen.init(allocator, &store, null, null);
