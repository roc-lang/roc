...
///
/// This is the canonical builtin registry: each member's `symbolName()` is
/// the exported linker symbol, and its wrapper in `dev_wrappers.zig` defines
/// the call ABI. See `src/builtins/builtin_registry.zig`.
