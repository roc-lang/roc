...
pub fn LirCodeGen(comptime target: RocTarget) type {
    ...
    return struct {
        ...
        /// Layout-disambiguated symbol location cache.
        ...
        /// when the value location alone is not enough.
        symbol_source_exprs: std.AutoHashMap(u64, LirExprId),
        /// Concrete layout overrides for symbols while compiling specialized lambdas.
        /// Keyed by symbol only; used by getExprLayout(lookup) when lambda params
        /// are specialized from call-site argument layouts.
        ...
        /// Tracks closure metadata for stack slots that store closure payloads.
        /// This allows later lookups of stack-backed callable values to dispatch
        /// as closures instead of misinterpreting payload bytes as fn pointers.
        closure_stack_info: std.AutoHashMap(i32, ClosureStackInfo),
        active_closure_base: ?ActiveClosureBaseInfo = null,
        ...
        /// Scratch buffer for argument locations during call generation
        ...
        pub const ValueLocation = union(enum) {
            ...
            lambda_code: struct {
                ...
                ret_layout: layout.Idx,
                /// Original expression that produced this lambda code when available.
                /// Used to recover callable source metadata when needed.
                source_expr: ?LirExprId = null,
            },
        ...
        fn symbolKey(symbol: Symbol) u64 {
            return @bitCast(symbol);
        }

        fn normalizedLayoutKey(_: *Self, layout_idx: layout.Idx) u64 {
            return @intFromEnum(layout_idx);
        }

        fn symbolLayoutKey(self: *Self, symbol: Symbol, layout_idx: layout.Idx) u128 {
            const upper = @as(u128, symbolKey(symbol)) << 64;
            const lower = @as(u128, self.normalizedLayoutKey(layout_idx));
            return upper | lower;
        }

        fn locationMatchesLayout(self: *Self, loc: ValueLocation, layout_idx: layout.Idx) bool {
            if (layout_idx == .str) return loc == .stack_str;
            if (layout_idx == .i128 or layout_idx == .u128 or layout_idx == .dec) return loc == .stack_i128;

            if (self.layout_store) |ls| {
                if (@intFromEnum(layout_idx) < ls.layouts.len()) {
                    const layout_val = ls.getLayout(layout_idx);
                    if (layout_val.tag == .list or layout_val.tag == .list_of_zst) return loc == .list_stack;
                }
            }
        ...
        fn putSymbolLocationWithOverride(
            self: *Self,
            symbol: Symbol,
            declared_layout: layout.Idx,
            effective_layout: layout.Idx,
            loc: ValueLocation,
        ) Allocator.Error!void {
            try self.putSymbolLocation(symbol, declared_layout, loc);
            if (effective_layout != declared_layout) {
                try self.putSymbolLocation(symbol, effective_layout, loc);
                try self.symbol_layout_overrides.put(symbolKey(symbol), effective_layout);
            } else {
                _ = self.symbol_layout_overrides.remove(symbolKey(symbol));
            }
        }
        ...
        fn putSymbolLocation(self: *Self, symbol: Symbol, layout_idx: ?layout.Idx, loc: ValueLocation) Allocator.Error!void {
            ...
            if (layout_idx) |li| {
                const key = self.symbolLayoutKey(symbol, li);
            ...
            // Callable bindings should always be available under the lambda's
            // semantic function layout key as well, even when the pattern's
            // stored layout idx differs.
            if (loc == .closure_value) {
                const lambda_expr = self.store.getExpr(loc.closure_value.lambda);
                if (lambda_expr == .lambda) {
                    const fn_layout = lambda_expr.lambda.fn_layout;
                    if (layout_idx == null or layout_idx.? != fn_layout) {
                        const fn_key = self.symbolLayoutKey(symbol, fn_layout);
                        try self.symbol_locations_by_layout.put(fn_key, loc);
                        if (self.closureInfoFromValueLocation(loc)) |info| {
                            try self.symbol_closure_infos_by_layout.put(fn_key, info);
                        } else {
                            _ = self.symbol_closure_infos_by_layout.remove(fn_key);
                        }
                    }
                }
            }
            ...
            if (layout_idx) |li| {
                ...
                if (self.isFunctionLayout(li)) {
                    if (self.symbol_locations.get(symbolKey(symbol))) |loc| {
                        if (loc == .closure_value or loc == .lambda_code) {
                            return loc;
                        }
                    }
                }
        ...
        fn putSymbolClosureInfo(self: *Self, symbol: Symbol, layout_idx: layout.Idx, info: ClosureStackInfo) Allocator.Error!void {
            try self.symbol_closure_infos_by_layout.put(self.symbolLayoutKey(symbol, layout_idx), info);
        }
        ...
        fn getSymbolClosureInfo(self: *Self, symbol: Symbol, layout_idx: ?layout.Idx) ?ClosureStackInfo {
            if (layout_idx) |li| {
                if (li == layout.Idx.none) {
                    if (self.symbol_locations.get(symbolKey(symbol))) |loc| {
                        return self.closureInfoFromValueLocation(loc);
                    }
                    return null;
                }
                if (self.symbol_closure_infos_by_layout.get(self.symbolLayoutKey(symbol, li))) |info| {
                    return info;
                }
                if (self.isFunctionLayout(li)) {
                    if (self.symbol_locations.get(symbolKey(symbol))) |loc| {
        ...
        pub fn reset(self: *Self) void {
            ...
            self.symbol_layout_overrides.clearRetainingCapacity();
        ...
        fn getExprLayout(self: *Self, expr_id: LirExprId) ?layout.Idx {
            ...
            if (expr == .lookup) {
                ...
                if (self.symbol_layout_overrides.get(symbolKey(lookup.symbol))) |override_layout| {
        ...
        fn generateExpr(self: *Self, expr_id: LirExprId) Allocator.Error!ValueLocation {
            const raw_loc = try self.generateExprRaw(expr_id);
            if (self.getExprLayout(expr_id)) |expr_layout| {
                // Use layout-aware stabilization directly so width-sensitive values
                // (for example f32) are materialized with the correct byte width.
                return try self.stabilizeBindingLocation(expr_layout, raw_loc);
            }
            return try self.stabilize(raw_loc);
        }
        ...
        fn generateLowLevel(self: *Self, ll: anytype) Allocator.Error!ValueLocation {
            ...
            switch (ll.op) {
                ...
                .list_with_capacity => {
                    ...
                    const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_with_capacity_packed);
                ...
                .list_get => {
                    ...
                    switch (elem_layout_val.tag) {
                        ...
                        .scalar => {
                            if (elem_layout_val.data.scalar.tag == .str) {
                                try self.emitStrIncref(result_loc, 1);
                            }
                        },
                        .box, .box_of_zst => try self.emitBoxIncref(result_loc, 1),
                        else => {},
                    }
                ...
                .list_concat => {
                    ...
                    const result_offset = self.codegen.allocStackSlot(roc_str_size);
                    const alignment_bytes = elem_size_align.alignment.toByteUnits();
                    const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_concat_packed);

                    {
                        // roc_builtins_list_concat(out, a, b, alignment, element_width, elements_refcounted, roc_ops)
                        const base_reg = frame_ptr;
                        var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                ...
                .list_prepend => {
                    ...
                    const result_offset = self.codegen.allocStackSlot(roc_str_size);
                    const alignment_bytes = elem_size_align.alignment.toByteUnits();
                    const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_prepend_packed);

                    {
                        // roc_builtins_list_prepend(out, list, alignment, element, element_width, elements_refcounted, roc_ops)
                        const base_reg = frame_ptr;
                        var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                ...
                .list_repeat => {
                    ...
                    const alignment_bytes = elem_size_align.alignment.toByteUnits();
                    const result_offset = self.codegen.allocStackSlot(roc_str_size);
                    const cap_fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_with_capacity_packed);
                    const base_reg = frame_ptr;
                ...
                => {
                    ...
                    return try self.callScalarToI128(src_reg, @intFromPtr(&dev_wrappers.roc_builtins_u64_to_dec_packed), .u64_to_dec);
                },
                ...
                => {
                    ...
                    return try self.callScalarToI128(src_reg, @intFromPtr(&dev_wrappers.roc_builtins_i64_to_dec_packed), .i64_to_dec);
                },
                ...
                => {
                    ...
                    const fn_addr = @intFromPtr(&dev_wrappers.roc_builtins_dec_to_i64_trunc_packed);
                ...
                .dec_to_f64 => {
                    ...
                    const src_loc = try self.generateExpr(args[0]);
                    const parts = try self.getI128Parts(src_loc, .signed); // Dec is signed i128
                    return try self.callI128PartsToF64(parts, @intFromPtr(&dev_wrappers.roc_builtins_dec_to_f64_packed), .dec_to_f64);
                },
                .dec_to_f32_wrap => {
                ...
                => {
                    ...
                    return try self.callI128PartsToF64(parts, @intFromPtr(&dev_wrappers.roc_builtins_i128_to_f64_packed), .i128_to_f64);
                },
                .u128_to_f32,
                .u128_to_f64,
                => {
                    ...
                    return try self.callI128PartsToF64(parts, @intFromPtr(&dev_wrappers.roc_builtins_u128_to_f64_packed), .u128_to_f64);
                },
                ...
                => {
                    ...
                    const src_loc = try self.generateExpr(args[0]);
                    const freg = try self.ensureInFloatReg(src_loc);
                    return try self.callF64ToI128(freg, @intFromPtr(&dev_wrappers.roc_builtins_f64_to_i128_trunc_packed), .f64_to_i128_trunc);
                },
                .f32_to_u128_trunc,
                .f64_to_u128_trunc,
                => {
                    ...
                    const src_loc = try self.generateExpr(args[0]);
                    const freg = try self.ensureInFloatReg(src_loc);
                    return try self.callF64ToI128(freg, @intFromPtr(&dev_wrappers.roc_builtins_f64_to_u128_trunc_packed), .f64_to_u128_trunc);
                },
                ...
                .str_to_utf8 => {
                    ...
                    return try self.callStr1RocOpsToResult(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_to_utf8_packed), .str_to_utf8, .list);
                },
                .str_is_empty => {
                    if (args.len != 1) unreachable;
                    const str_loc = try self.generateExpr(args[0]);
                    const str_off = try self.ensureOnStack(str_loc, roc_str_size);
                    return try self.callStr1ToScalar(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_is_empty_packed), .str_is_empty);
                },
                .str_is_eq => {
                    if (args.len != 2) unreachable;
                ...
                .str_concat => {
                    ...
                    const a_off = try self.ensureOnStack(a_loc, roc_str_size);
                    const b_off = try self.ensureOnStack(b_loc, roc_str_size);
                    return try self.callStr2RocOpsToStr(a_off, b_off, @intFromPtr(&dev_wrappers.roc_builtins_str_concat_packed), .str_concat);
                },
                .str_contains => {
                    if (args.len != 2) unreachable;
                    const a_loc = try self.generateExpr(args[0]);
                    ...
                    const a_off = try self.ensureOnStack(a_loc, roc_str_size);
                    const b_off = try self.ensureOnStack(b_loc, roc_str_size);
                    return try self.callStr2ToScalar(a_off, b_off, @intFromPtr(&dev_wrappers.roc_builtins_str_contains_packed), .str_contains);
                },
                .str_starts_with => {
                    if (args.len != 2) unreachable;
                    const a_loc = try self.generateExpr(args[0]);
                    ...
                    const a_off = try self.ensureOnStack(a_loc, roc_str_size);
                    const b_off = try self.ensureOnStack(b_loc, roc_str_size);
                    return try self.callStr2ToScalar(a_off, b_off, @intFromPtr(&dev_wrappers.roc_builtins_str_starts_with_packed), .str_starts_with);
                },
                .str_ends_with => {
                    if (args.len != 2) unreachable;
                    const a_loc = try self.generateExpr(args[0]);
                    ...
                    const a_off = try self.ensureOnStack(a_loc, roc_str_size);
                    const b_off = try self.ensureOnStack(b_loc, roc_str_size);
                    return try self.callStr2ToScalar(a_off, b_off, @intFromPtr(&dev_wrappers.roc_builtins_str_ends_with_packed), .str_ends_with);
                },
                .str_count_utf8_bytes => {
                    if (args.len != 1) unreachable;
                    const str_loc = try self.generateExpr(args[0]);
                    const str_off = try self.ensureOnStack(str_loc, roc_str_size);
                    return try self.callStr1ToScalar(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_count_utf8_bytes_packed), .str_count_utf8_bytes);
                },
                .str_caseless_ascii_equals => {
                    if (args.len != 2) unreachable;
                    const a_loc = try self.generateExpr(args[0]);
                    ...
                    const a_off = try self.ensureOnStack(a_loc, roc_str_size);
                    const b_off = try self.ensureOnStack(b_loc, roc_str_size);
                    return try self.callStr2ToScalar(a_off, b_off, @intFromPtr(&dev_wrappers.roc_builtins_str_caseless_ascii_equals_packed), .str_caseless_ascii_equals);
                },
                .str_repeat => {
                    ...
                    return try self.callStr1U64RocOpsToStr(str_off, count_off, @intFromPtr(&dev_wrappers.roc_builtins_str_repeat_packed), .str_repeat);
                },
                .str_trim => {
                    if (args.len != 1) unreachable;
                    const str_loc = try self.generateExpr(args[0]);
                    const str_off = try self.ensureOnStack(str_loc, roc_str_size);
                    return try self.callStr1RocOpsToResult(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_trim_packed), .str_trim, .str);
                },
                .str_trim_start => {
                    if (args.len != 1) unreachable;
                    const str_loc = try self.generateExpr(args[0]);
                    const str_off = try self.ensureOnStack(str_loc, roc_str_size);
                    return try self.callStr1RocOpsToResult(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_trim_start_packed), .str_trim_start, .str);
                },
                .str_trim_end => {
                    if (args.len != 1) unreachable;
                    const str_loc = try self.generateExpr(args[0]);
                    const str_off = try self.ensureOnStack(str_loc, roc_str_size);
                    return try self.callStr1RocOpsToResult(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_trim_end_packed), .str_trim_end, .str);
                },
                .str_split => {
                    ...
                    const a_off = try self.ensureOnStack(a_loc, roc_str_size);
                    const b_off = try self.ensureOnStack(b_loc, roc_str_size);
                    return try self.callStr2RocOpsToResult(a_off, b_off, @intFromPtr(&dev_wrappers.roc_builtins_str_split_packed), .str_split, .list);
                },
                .str_join_with => {
                    ...
                    return try self.callStr2RocOpsToResult(list_off, sep_off, @intFromPtr(&dev_wrappers.roc_builtins_str_join_with_packed), .str_join_with, .str);
                },
                .str_reserve => {
                    ...
                    return try self.callStr1U64RocOpsToStr(str_off, spare_off, @intFromPtr(&dev_wrappers.roc_builtins_str_reserve_packed), .str_reserve);
                },
                .str_release_excess_capacity => {
                    if (args.len != 1) unreachable;
                    const str_loc = try self.generateExpr(args[0]);
                    const str_off = try self.ensureOnStack(str_loc, roc_str_size);
                    return try self.callStr1RocOpsToResult(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_release_excess_capacity_packed), .str_release_excess_capacity, .str);
                },
                .str_with_capacity => {
                    ...
                    const roc_ops_reg = self.roc_ops_reg orelse unreachable;
                    const result_offset = self.codegen.allocStackSlot(roc_str_size);
                    const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_str_with_capacity_packed);
                ...
                .str_drop_prefix => {
                    ...
                    const a_off = try self.ensureOnStack(a_loc, roc_str_size);
                    const b_off = try self.ensureOnStack(b_loc, roc_str_size);
                    return try self.callStr2RocOpsToResult(a_off, b_off, @intFromPtr(&dev_wrappers.roc_builtins_str_drop_prefix_packed), .str_drop_prefix, .str);
                },
                .str_drop_suffix => {
                    if (args.len != 2) unreachable;
                    const a_loc = try self.generateExpr(args[0]);
                    ...
                    const a_off = try self.ensureOnStack(a_loc, roc_str_size);
                    const b_off = try self.ensureOnStack(b_loc, roc_str_size);
                    return try self.callStr2RocOpsToResult(a_off, b_off, @intFromPtr(&dev_wrappers.roc_builtins_str_drop_suffix_packed), .str_drop_suffix, .str);
                },
                .str_with_ascii_lowercased => {
                    if (args.len != 1) unreachable;
                    const str_loc = try self.generateExpr(args[0]);
                    const str_off = try self.ensureOnStack(str_loc, roc_str_size);
                    return try self.callStr1RocOpsToResult(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_with_ascii_lowercased_packed), .str_with_ascii_lowercased, .str);
                },
                .str_with_ascii_uppercased => {
                    if (args.len != 1) unreachable;
                    const str_loc = try self.generateExpr(args[0]);
                    const str_off = try self.ensureOnStack(str_loc, roc_str_size);
                    return try self.callStr1RocOpsToResult(str_off, @intFromPtr(&dev_wrappers.roc_builtins_str_with_ascii_uppercased_packed), .str_with_ascii_uppercased, .str);
                },
                .str_with_prefix => {
                    ...
                    return try self.callStr2RocOpsToResult(str_off, pfx_off, @intFromPtr(&dev_wrappers.roc_builtins_str_with_prefix_packed), .str_with_prefix, .str);
                },
                .str_from_utf8_lossy => {
                    ...
                    const list_loc = try self.generateExpr(args[0]);
                    const list_off = try self.ensureOnStack(list_loc, roc_list_size);
                    return try self.callStr1RocOpsToResult(list_off, @intFromPtr(&dev_wrappers.roc_builtins_str_from_utf8_lossy_packed), .str_from_utf8_lossy, .str);
                },
                .str_from_utf8 => {
                    ...
                    try builder.addRegArg(roc_ops_reg);
                    try self.callBuiltin(&builder, @intFromPtr(&dev_wrappers.roc_builtins_str_from_utf8_packed), .str_from_utf8);
                ...
                .list_set => {
                    ...
                    const alignment_bytes = elem_size_align.alignment.toByteUnits();
                    const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_replace_packed);

                    {
                        // roc_builtins_list_replace(..., elements_refcounted, roc_ops)
                        const base_reg = frame_ptr;
                        var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                ...
                .list_sort_with => {
                    ...
                    const alignment_bytes = elem_size_align.alignment.toByteUnits();
                    const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_sort_with_packed);
                ...
                .box_box => {
                    ...
                    const heap_ptr_slot: i32 = self.codegen.allocStackSlot(8);
                    {
                        ...
                        try builder.addRegArg(roc_ops_reg);
                        try self.callBuiltin(&builder, @intFromPtr(&dev_wrappers.roc_builtins_allocate_with_refcount_packed), .allocate_with_refcount);
                    }
        ...
        fn callListSublist(self: *Self, ll: anytype, list_loc: ValueLocation, n_loc: ValueLocation, mode: enum { drop_first, drop_last, take_first, take_last }) Allocator.Error!ValueLocation {
            ...
            const result_offset = self.codegen.allocStackSlot(roc_str_size);
            const alignment_bytes = elem_size_align.alignment.toByteUnits();
            const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_sublist_packed);

            {
                // roc_builtins_list_sublist(..., elements_refcounted, roc_ops)
                const base_reg = frame_ptr;
                var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
        ...
        fn callListSplitOp(self: *Self, ll: anytype, list_loc: ValueLocation, mode: enum { first, last }) Allocator.Error!ValueLocation {
...

            {
                ...
                try builder.addImmArg(0); // elements_refcounted=false
                try builder.addRegArg(roc_ops_reg);
                try self.callBuiltin(&builder, @intFromPtr(&dev_wrappers.roc_builtins_list_sublist_packed), .list_sublist);
            }
        ...
        fn callListReserveOp(self: *Self, list_loc: ValueLocation, spare_loc: ValueLocation, ll: anytype) Allocator.Error!ValueLocation {
            ...
            const result_offset = self.codegen.allocStackSlot(roc_str_size);
            const alignment_bytes = elem_size_align.alignment.toByteUnits();
            const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_reserve_packed);

            {
                // roc_builtins_list_reserve(..., elements_refcounted, roc_ops)
                const base_reg = frame_ptr;
                var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
        ...
        fn callListReleaseExcessCapOp(self: *Self, list_loc: ValueLocation, ll: anytype) Allocator.Error!ValueLocation {
            ...
            const result_offset = self.codegen.allocStackSlot(roc_str_size);
            const alignment_bytes = elem_size_align.alignment.toByteUnits();
            const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_list_release_excess_capacity_packed);

            {
                // roc_builtins_list_release_excess_capacity(..., elements_refcounted, roc_ops)
                const base_reg = frame_ptr;
                var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
        ...
        fn generateLookup(self: *Self, symbol: Symbol, layout_idx: layout.Idx) Allocator.Error!ValueLocation {
            ...
            // while a capture expects a concrete closure storage layout.
            if (self.isFunctionLayout(layout_idx)) {
                if (self.getSymbolLocation(symbol, null)) |loc| {
                    if (self.closureInfoFromValueLocation(loc) != null) {
                        const adapted = self.adaptLocationForLayout(loc, layout_idx);
                        try self.putSymbolLocation(symbol, layout_idx, adapted);
                        return adapted;
                    }
                }
            }
            ...
            if (self.layout_store) |ls| {
                if (symbol.module_idx < ls.all_module_envs.len) {
                    ...
                    if (symbol.ident_idx.idx < env.common.idents.interner.bytes.len()) {
                        sym_name = env.getIdent(symbol.ident_idx);
                    }
                }
            }
        ...
        fn generateIntTryConversion(self: *Self, ll: anytype, args: []const LirExprId) Allocator.Error!ValueLocation {
            ...
            if (info.src_bits > 64) {
                ...
                const fn_addr: usize = if (info.src_signed)
                    @intFromPtr(&dev_wrappers.roc_builtins_i128_try_convert_packed)
                else
                    @intFromPtr(&dev_wrappers.roc_builtins_u128_try_convert_packed);
                ...
                if (info.src_signed) {
                    ...
                    const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_int_try_signed_packed);
                    const base_reg = frame_ptr;
                    var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                    try builder.addLeaArg(base_reg, result_offset);
                    ...
                    const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_int_try_unsigned_packed);
                    const base_reg = frame_ptr;
                    var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                    try builder.addLeaArg(base_reg, result_offset);
                    try builder.addRegArg(src_reg);
        ...
        fn generateNumFromStr(self: *Self, ll: anytype, args: []const LirExprId) Allocator.Error!ValueLocation {
            ...
            if (payload_idx == .dec) {
                const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_dec_from_str_packed);
                var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                try builder.addLeaArg(base_reg, result_offset);
                ...
                const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_float_from_str_packed);
                var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                try builder.addLeaArg(base_reg, result_offset);
                try builder.addMemArg(base_reg, str_off);
                try builder.addMemArg(base_reg, str_off + 8);
                ...
                const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_int_from_str_packed);
                var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                try builder.addLeaArg(base_reg, result_offset);
                try builder.addMemArg(base_reg, str_off);
                try builder.addMemArg(base_reg, str_off + 8);
            ...
            switch (info.tgt_kind) {
                .int => {
                    ...
                    if (info.src_kind == .dec) {
                        ...
                        const parts = try self.getI128Parts(src_loc, .signed); // Dec is signed i128
                        const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_dec_to_int_try_unsafe_packed);
                        const base_reg = frame_ptr;
                        var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                        try builder.addLeaArg(base_reg, result_offset);
                        try builder.addRegArg(parts.low);
                        ...
                        const freg = try self.ensureInFloatReg(src_loc);
                        const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_f64_to_int_try_unsafe_packed);
                        const base_reg = frame_ptr;
                ...
                .f32 => {
                    ...
                    if (info.src_kind == .dec) {
                        // Dec to f32
                        const parts = try self.getI128Parts(src_loc, .signed); // Dec is signed i128
                        const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_dec_to_f32_try_unsafe_packed);
                        const base_reg = frame_ptr;
                        var builder = try Builder.init(&self.codegen.emit, &self.codegen.stack_offset);
                        try builder.addLeaArg(base_reg, result_offset);
                        try builder.addRegArg(parts.low);
                        ...
                        // f64 to f32
                        const freg = try self.ensureInFloatReg(src_loc);
                        const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_f64_to_f32_try_unsafe_packed);
                        const base_reg = frame_ptr;
                ...
                .dec => {
                    ...
                    const fn_addr: usize = if (info.tgt_signed)
                        @intFromPtr(&dev_wrappers.roc_builtins_i128_to_dec_try_unsafe_packed)
                    else
                        @intFromPtr(&dev_wrappers.roc_builtins_u128_to_dec_try_unsafe_packed);
                    const base_reg = frame_ptr;
        ...
        fn callDecMul(self: *Self, lhs_parts: I128Parts, rhs_parts: I128Parts, result_low: GeneralReg, result_high: GeneralReg) Allocator.Error!void {
            const fn_addr = @intFromPtr(&dev_wrappers.roc_builtins_dec_mul_saturated_packed);
        ...
        fn callDecDiv(self: *Self, lhs_parts: I128Parts, rhs_parts: I128Parts, result_low: GeneralReg, result_high: GeneralReg) Allocator.Error!void {
            const fn_addr = @intFromPtr(&dev_wrappers.roc_builtins_dec_div_packed);
            const roc_ops_reg = self.roc_ops_reg orelse unreachable;
            const result_slot = self.codegen.allocStackSlot(16);
            const base_reg = frame_ptr;
        ...
        fn callDecDivTrunc(self: *Self, lhs_parts: I128Parts, rhs_parts: I128Parts, result_low: GeneralReg, result_high: GeneralReg) Allocator.Error!void {
            const fn_addr = @intFromPtr(&dev_wrappers.roc_builtins_dec_div_trunc_packed);
            const roc_ops_reg = self.roc_ops_reg orelse unreachable;
            const result_slot = self.codegen.allocStackSlot(16);
            const base_reg = frame_ptr;
        ...
        fn callI128DivRem(
            ...
            const fn_addr: usize = if (is_unsigned)
                if (is_rem) @intFromPtr(&dev_wrappers.roc_builtins_num_rem_trunc_u128_packed) else @intFromPtr(&dev_wrappers.roc_builtins_num_div_trunc_u128_packed)
            else if (is_rem) @intFromPtr(&dev_wrappers.roc_builtins_num_rem_trunc_i128_packed) else @intFromPtr(&dev_wrappers.roc_builtins_num_div_trunc_i128_packed);
        ...
        fn compareFieldByLayout(
            ...
            if (field_layout.tag == .struct_) {
                ...
                const eq_loc = try self.callStr2ToScalar(lhs_off, rhs_off, @intFromPtr(&dev_wrappers.roc_builtins_str_equal_packed), .str_equal);
        ...
        fn generateIfThenElse(self: *Self, ite: anytype) Allocator.Error!ValueLocation {
            ...
            for (branches) |branch| {
                ...
                if (first_branch) {
                    ...
                    const body_is_closure = self.closureInfoFromValueLocation(body_loc) != null;
                    // Use stack for types > 8 bytes (e.g., i128, Dec) or stack-based values
                    if (body_is_closure or result_size > 8) {
                ...
                if (result_slot) |slot| {
                    ...
                    try self.updateClosureStackInfoForCopy(slot, body_loc);
            ...
            if (result_slot == null and result_reg == null) {
                const else_is_closure = self.closureInfoFromValueLocation(else_loc) != null;
                // Use stack for types > 8 bytes (e.g., i128, Dec) or stack-based values
                if (else_is_closure or result_size > 8) {
                    result_slot = self.codegen.allocStackSlot(result_size);
                } else {
                    switch (else_loc) {
                        .stack, .stack_str, .list_stack => {
            ...
            if (result_slot) |slot| {
                ...
                try self.updateClosureStackInfoForCopy(slot, else_loc);
            } else if (result_reg) |reg| {
                        ...
                        for (inner_args) |inner_arg_id| {
                            ...
                            switch (inner_arg) {
                                .bind => |inner_bind| {
                                    ...
                                    try self.putSymbolLocation(inner_bind.symbol, inner_bind.layout_idx, inner_loc);
                                },
                                .wildcard => {},
                                else => {},
                            }
        ...
        fn emitStringPatternCheck(self: *Self, str_lit_idx: anytype, value_loc: ValueLocation) Allocator.Error!void {
            ...
            const eq_loc = try self.callStr2ToScalar(val_off, lit_off, @intFromPtr(&dev_wrappers.roc_builtins_str_equal_packed), .str_equal);
            const eq_reg = try self.ensureInGeneralReg(eq_loc);
        ...
        fn generateMatch(self: *Self, when_expr: anytype) Allocator.Error!ValueLocation {
            ...
            const tu_disc_size: u8 = if (value_layout_val.tag == .tag_union) blk: {
                ...
                const scalar_size = ls.layoutSizeAlign(value_layout_val).size;
                // Non-tag unions (e.g. Bool) carry the discriminant in the value itself.
                // Use their actual scalar width so pattern checks only compare meaningful bits.
                break :blk @intCast(@max(@as(u32, 1), @min(scalar_size, @as(u32, 8))));
            };
            ...
            for (branches, 0..) |branch, i| {
                ...
                switch (pattern) {
                    ...
                    .as_pattern => |as_pat| {
                        ...
                        try self.putSymbolLocation(as_pat.symbol, as_pat.layout_idx, value_loc);
        ...
        fn storeMatchResult(
            ...
            const closure_info = self.closureInfoFromValueLocation(body_loc);
            ...
            if (!use_stack_result.*) {
                const needed_size: ?u32 = switch (body_loc) {
                    ...
                    .stack_i128 => 16,
                    .immediate_i128 => 16,
                    .closure_value => |cv| self.closureStorageSize(cv.representation),
                    else => null,
                };
                if (needed_size) |size| {
            ...
            if (use_stack_result.*) {
                ...
                if (closure_info) |info| {
                    try self.closure_stack_info.put(result_slot.*, info);
                } else {
                    _ = self.closure_stack_info.remove(result_slot.*);
                }
            } else {
                const body_reg = try self.ensureInGeneralReg(body_loc);
        ...
        fn generateStruct(self: *Self, s: anytype) Allocator.Error!ValueLocation {
            ...
            for (field_exprs, 0..) |field_expr_id, i| {
                ...
                try self.updateClosureStackInfoForCopy(field_base, field_loc);
            }
        ...
        fn valueSizeFromLoc(_: *Self, loc: ValueLocation) u32 {
            return switch (loc) {
                ...
                .immediate_i64, .general_reg, .float_reg, .immediate_f64 => 8,
                else => 8,
            };
        }

        /// Return an exact width for locations that already encode one.
        /// Returns null for width-ambiguous locations (immediates/register-only values).
        fn exactSizeFromLoc(self: *Self, loc: ValueLocation) ?u32 {
            return switch (loc) {
                .stack => |s| s.size.byteCount(),
                .stack_i128 => 16,
                .stack_str => roc_str_size,
        ...
        fn closureInfoFromValueLocation(self: *Self, loc: ValueLocation) ?ClosureStackInfo {
            return switch (loc) {
                .closure_value => |cv| .{
                ...
                .stack => |s| self.closure_stack_info.get(s.offset),
                .stack_i128 => |off| self.closure_stack_info.get(off),
                .stack_str => |off| self.closure_stack_info.get(off),
                else => null,
            };
        }

        fn closureInfoFromExprId(self: *Self, expr_id: LirExprId) ?ClosureStackInfo {
            return self.closureInfoFromExprIdWithExpectation(expr_id, false);
        }

        fn closureInfoFromExprIdWithExpectation(
            self: *Self,
            expr_id: LirExprId,
            ...
            return switch (expr) {
                ...
                .lookup => |lookup| blk: {
                    const lookup_expects_callable = expect_callable or self.isFunctionLayout(lookup.layout_idx);
                    ...
                    if (self.preboundCallableInfo(lookup.symbol, lookup.layout_idx)) |info| {
                        break :blk info;
                    }
                    if (self.getSymbolDefExact(lookup.symbol)) |def_expr_id| {
                        break :blk self.closureInfoFromExprIdWithExpectation(def_expr_id, lookup_expects_callable);
                    }
                    break :blk null;
                },
                .call => |call| blk: {
                    if (!self.isFunctionLayout(call.ret_layout)) break :blk null;
                ...
                .if_then_else => |ite| blk: {
                    ...
                    const branches = self.store.getIfBranches(ite.branches);
                    for (branches) |branch| {
                        const branch_info = self.closureInfoFromExprIdWithExpectation(branch.body, expect_callable) orelse break :blk null;
                        if (!std.meta.eql(branch_info, merged)) break :blk null;
                    }
                    break :blk merged;
                },
                .match_expr => |m| blk: {
                ...
                .discriminant_switch => |sw| blk: {
                    ...
                    if (branches.len == 0) break :blk null;
                    const merged = self.closureInfoFromExprIdWithExpectation(branches[0], expect_callable) orelse break :blk null;
                    for (branches[1..]) |branch_expr_id| {
                        const branch_info = self.closureInfoFromExprIdWithExpectation(branch_expr_id, expect_callable) orelse break :blk null;
                        if (!std.meta.eql(branch_info, merged)) break :blk null;
                    }
                    break :blk merged;
                },
                .nominal => |nom| self.closureInfoFromExprIdWithExpectation(nom.backing_expr, expect_callable),
                .block => |b| self.closureInfoFromExprIdWithExpectation(
                    b.final_expr,
        ...
        fn preboundCallableFromExprId(self: *Self, expr_id: LirExprId) ?PreboundCallable {
            ...
            return switch (expr) {
                    ...
                    if (closure.representation == .one_capture) {
                        ...
                        if (captures.len == 1) {
                            ...
                            if (self.isFunctionLayout(cap.layout_idx)) {
                                pb.capture_alias_symbol = cap.symbol;
                                pb.capture_alias_layout = cap.layout_idx;
                                if (self.getSymbolClosureInfo(cap.symbol, cap.layout_idx)) |cap_info| {
                                    pb.capture_alias_info = cap_info;
                                } else if (self.getSymbolLocation(cap.symbol, cap.layout_idx)) |cap_loc| {
                                    pb.capture_alias_info = self.closureInfoFromValueLocation(cap_loc);
                                }
                            }
                        }
                    }
                ...
                .lookup => |lookup| blk: {
                    ...
                    if (!self.isFunctionLayout(lookup.layout_idx)) break :blk null;
                    if (self.getSymbolDefExact(lookup.symbol)) |def_expr_id| {
        ...
        fn prebindCallableArgsForLambdaCall(self: *Self, params_span: lir.LirPatternSpan, args_span: anytype) Allocator.Error!void {
            ...
            while (i < len) : (i += 1) {
                ...
                if (self.preboundCallableFromExprId(arg_expr_id)) |pb| {
                    try self.prebound_callables_by_layout.put(self.symbolLayoutKey(bind.symbol, bind.layout_idx), pb);
                }
            }
        }
        ...
        fn hasPreboundCallableForParams(self: *Self, params_span: lir.LirPatternSpan) bool {
            ...
            for (params) |pat_id| {
                ...
                if (pat == .bind) {
                    if (self.prebound_callables_by_layout.contains(self.symbolLayoutKey(pat.bind.symbol, pat.bind.layout_idx))) {
                        return true;
                    }
                }
            }
        ...
        fn applyPreboundCallableForBinding(
            ...
            const closure_loc: ValueLocation = .{ .closure_value = .{
                ...
                .representation = pb.info.representation,
                .lambda = pb.info.lambda,
                .captures = pb.info.captures,
            } };

            // Keep callable bindings available under both storage and semantic fn layouts.
            try self.putSymbolLocation(symbol, layout_idx, closure_loc);
            try self.putSymbolClosureInfo(symbol, layout_idx, pb.info);
            const lambda_expr = self.store.getExpr(pb.info.lambda);
            if (lambda_expr == .lambda) {
                const fn_layout = lambda_expr.lambda.fn_layout;
                if (fn_layout != layout_idx) {
                    try self.putSymbolLocation(symbol, fn_layout, closure_loc);
                    try self.putSymbolClosureInfo(symbol, fn_layout, pb.info);
                }
            }

            var mapped_underlying_capture = false;
            if (pb.info.representation == .one_capture) {
                const captures = self.store.getCaptures(pb.info.captures);
                if (captures.len == 1) {
                    const cap = captures[0];
                    if (self.isFunctionLayout(cap.layout_idx)) {
                        if (self.getSymbolClosureInfo(cap.symbol, cap.layout_idx)) |cap_info| {
                            try self.closure_stack_info.put(stack_offset, cap_info);
                            mapped_underlying_capture = true;
                        } else if (self.getSymbolLocation(cap.symbol, cap.layout_idx)) |cap_loc| {
                            if (self.closureInfoFromValueLocation(cap_loc)) |cap_info| {
                                try self.closure_stack_info.put(stack_offset, cap_info);
                                mapped_underlying_capture = true;
                            }
                        }
            ...
            if (pb.capture_alias_symbol) |cap_symbol| {
                ...
                if (!self.isFunctionLayout(cap_layout) and pb.capture_alias_info == null) {
                    return;
                }
        ...
        fn returnedClosureInfoFromCallableExpr(self: *Self, expr_id: LirExprId) ?ClosureStackInfo {
            const returned_expr_id = self.applyCallableExprOnce(expr_id, 0) orelse return null;
            return self.closureInfoFromExprIdWithExpectation(returned_expr_id, false);
        }

        fn applyCallableExprOnce(self: *Self, expr_id: LirExprId, depth: u16) ?LirExprId {
            ...
            return switch (expr) {
                ...
                .lookup => |lookup| blk: {
                    if (self.getSymbolClosureInfo(lookup.symbol, lookup.layout_idx)) |info| {
                        break :blk self.applyCallableExprOnce(info.lambda, depth + 1);
                    }
                    if (self.getSymbolLocation(lookup.symbol, lookup.layout_idx)) |loc| {
                        if (self.closureInfoFromValueLocation(loc)) |info| {
                            break :blk self.applyCallableExprOnce(info.lambda, depth + 1);
                ...
                .call => |inner_call| blk: {
                    const first_result = self.applyCallableExprOnce(inner_call.fn_expr, depth + 1) orelse break :blk null;
                    break :blk self.applyCallableExprOnce(first_result, depth + 2);
                },
                else => null,
            };
        }

        fn closureResultLayoutFromInfo(self: *Self, info: ClosureStackInfo) layout.Idx {
            return switch (info.representation) {
                ...
                .no_captures => blk: {
                    const lambda_expr = self.store.getExpr(info.lambda);
                    if (lambda_expr == .lambda) {
                        break :blk lambda_expr.lambda.fn_layout;
                    }
                    break :blk .named_fn;
                },
            };
        ...
        fn captureStorageSizeInScope(self: *Self, captures_span: lir.LIR.LirCaptureSpan) u32 {
            const ls = self.layout_store orelse return 0;
        ...
        fn closureValueFromStackOffset(self: *Self, stack_offset: i32) ?ValueLocation {
            const info = self.closure_stack_info.get(stack_offset) orelse return null;
            if (self.active_closure_base) |active| {
                if (self.isCurrentClosureBaseInfo(info, stack_offset, active.stack_offset, active.lambda, active.captures)) {
                    return null;
                }
            }
            return .{ .closure_value = .{
        ...
        fn isCurrentClosureBaseInfo(
            _: *Self,
            info: ClosureStackInfo,
            capture_offset: i32,
            current_stack_offset: i32,
            current_lambda: lir.LirExprId,
            current_captures: lir.LIR.LirCaptureSpan,
        ) bool {
            return capture_offset == current_stack_offset and
                info.lambda == current_lambda and
                info.captures.start == current_captures.start and
                info.captures.len == current_captures.len;
        }

        fn updateClosureStackInfoForCopy(self: *Self, dest_offset: i32, source_loc: ValueLocation) Allocator.Error!void {
            if (self.closureInfoFromValueLocation(source_loc)) |info| {
                try self.closure_stack_info.put(dest_offset, info);
            } else {
                _ = self.closure_stack_info.remove(dest_offset);
            }
        }
        ...
        fn generateStructAccess(self: *Self, access: anytype) Allocator.Error!ValueLocation {
            ...
            if (struct_expr == .lookup) {
                ...
                if (self.symbol_source_exprs.get(symbol_key)) |source_expr_id| {
                    ...
                    if (source_expr == .struct_) {
                        const source_fields = self.store.getExprSpan(source_expr.struct_.fields);
                        if (access.field_idx < source_fields.len) {
                            const source_field_loc = try self.generateExpr(source_fields[access.field_idx]);
                            switch (source_field_loc) {
                                .closure_value, .lambda_code => return source_field_loc,
                                else => {},
                            }
                        }
                    }
        ...
        fn copyValueToStackOffset(self: *Self, offset: i32, loc: ValueLocation) Allocator.Error!void {
            switch (loc) {
                ...
                .closure_value => |cv| {
                    const closure_size = self.closureStorageSize(cv.representation);
                    if (closure_size == 0) return;
                    try self.copyBytesToStackOffset(offset, loc, closure_size);
                },
        ...
        fn copyBytesToStackOffset(self: *Self, dest_offset: i32, loc: ValueLocation, size: u32) Allocator.Error!void {
            ...
            switch (loc) {
                ...
                .float_reg => |reg| {
                    switch (size) {
                        4 => {
                            ...
                            if (comptime target.toCpuArch() == .aarch64) {
                                try self.codegen.emit.fmovRegReg(.double, tmp, reg);
                                try self.codegen.emit.fcvtFloatFloat(.single, tmp, .double, tmp);
                            } else {
                                try self.codegen.emit.movsdRegReg(tmp, reg);
                                try self.codegen.emit.cvtsd2ssRegReg(tmp, tmp);
                            }
                            try self.emitStoreStackF32(dest_offset, tmp);
                        },
                        8 => try self.codegen.emitStoreStackF64(dest_offset, reg),
                        else => unreachable,
                    }
                    return;
                },
                .immediate_f64 => |val| {
                    ...
                    switch (size) {
                        4 => {
                            ...
                            try self.codegen.emitLoadImm(reg, @as(i64, @intCast(bits32)));
                            try self.codegen.emitStoreStack(.w32, dest_offset, reg);
                        },
                        8 => {
                            const bits64: u64 = @bitCast(val);
                            try self.codegen.emitLoadImm(reg, @bitCast(bits64));
                            try self.codegen.emitStoreStack(.w64, dest_offset, reg);
                        },
                        else => unreachable,
                    }
                ...
                .stack, .stack_str, .stack_i128, .list_stack, .closure_value => {
                    // Handle stack locations below
                },
                .lambda_code => |lc| {
                    // Store the absolute code address as an 8-byte pointer value.
                    // lambda_code stores buffer offsets; convert to a real address
                    // using PC-relative addressing so captured functions can be
        ...
        fn generateStrLiteral(self: *Self, str_idx: base.StringLiteral.Idx) Allocator.Error!ValueLocation {
            ...
            if (str_bytes.len < roc_str_size) {
                ...
                const roc_ops_reg = self.roc_ops_reg orelse unreachable;
                const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_allocate_with_refcount_packed);
        ...
        fn promoteMutatedSymbolsToMutableSlots(
            ...
            while (it.next()) |entry| {
                ...
                while (loc_it.next()) |loc_entry| {
                    ...
                    const size: u32 = if (self.exactSizeFromLoc(loc_entry.value_ptr.*)) |loc_size| blk: {
                        break :blk loc_size;
                    } else blk: {
                        const layout_val = ls.getLayout(layout_idx);
        ...
        fn generateIntToStr(self: *Self, its: anytype) Allocator.Error!ValueLocation {
            ...
            const roc_ops_reg = self.roc_ops_reg orelse unreachable;
            const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_int_to_str_packed);
            const result_offset = self.codegen.allocStackSlot(roc_str_size);
            const base_reg = frame_ptr;
        ...
        fn generateFloatToStr(self: *Self, fts: anytype) Allocator.Error!ValueLocation {
            ...
            const roc_ops_reg = self.roc_ops_reg orelse unreachable;
            const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_float_to_str_packed);
            const result_offset = self.codegen.allocStackSlot(roc_str_size);
            const base_reg = frame_ptr;
        ...
        fn callDecToStrWrapped(self: *Self, val_loc: ValueLocation) Allocator.Error!ValueLocation {
            ...
            try builder.addRegArg(roc_ops_reg);
            try self.callBuiltin(&builder, @intFromPtr(&dev_wrappers.roc_builtins_dec_to_str_packed), .dec_to_str);
        ...
        fn generateDiscriminantSwitch(self: *Self, ds: anytype) Allocator.Error!ValueLocation {
            ...
            const disc_info: struct { reg: GeneralReg, use_w32: bool } = if (union_layout.tag == .tag_union) blk: {
                const tu_data = ls.getTagUnionData(union_layout.data.tag_union.idx);
                const disc_offset: i32 = @intCast(tu_data.discriminant_offset);
                const disc_size: u8 = tu_data.discriminant_size;
                const use_w32 = disc_size <= 4 or (disc_offset + 8 > @as(i32, @intCast(tu_data.size)));
                const reg = try self.loadAndMaskDiscriminant(value_loc, use_w32, disc_offset, disc_size);
                ...
                const scalar_size = ls.layoutSizeAlign(union_layout).size;
                const disc_size: u8 = @intCast(@max(@as(u32, 1), @min(scalar_size, @as(u32, 8))));
                const use_w32 = disc_size <= 4;
                const reg = try self.loadAndMaskDiscriminant(value_loc, use_w32, 0, disc_size);
                break :blk .{ .reg = reg, .use_w32 = use_w32 };
                ...
                if (inner_layout.tag == .scalar or inner_layout.tag == .zst) {
                    ...
                    const scalar_size = ls.layoutSizeAlign(inner_layout).size;
                    const disc_size: u8 = @intCast(@max(@as(u32, 1), @min(scalar_size, @as(u32, 8))));
                    const use_w32 = disc_size <= 4;
                    const reg = try self.loadAndMaskDiscriminant(value_loc, use_w32, 0, disc_size);
                    break :blk .{ .reg = reg, .use_w32 = use_w32 };
                    ...
                    if (disc_size < 4) {
                        const mask: i32 = (@as(i32, 1) << @as(u5, @intCast(disc_size * 8))) - 1;
                        if (comptime target.toCpuArch() == .aarch64) {
                            const mask_reg = try self.allocTempGeneral();
                            try self.codegen.emitLoadImm(mask_reg, mask);
                            try self.codegen.emit.andRegRegReg(.w32, disc_reg, disc_reg, mask_reg);
        ...
        fn storeResultToSlot(self: *Self, slot: i32, loc: ValueLocation, slot_size: u32) Allocator.Error!void {
            ...
            switch (loc) {
                ...
                .lambda_code => {
                    // Lambda code pointers should not flow through match result storage.
                    unreachable;
                },
                .closure_value => |cv| {
                    // Match branches can legitimately evaluate to closures.
                    self.codegen.freeGeneral(temp_reg);
                    try self.copyBytesToStackOffset(slot, .{ .closure_value = cv }, slot_size);
                    return;
                },
                .stack_str => |off| {
                    var offset: u32 = 0;
                    ...
                    if (pattern == .bind) {
                        ...
                        if (is_self_recursive) {
                            ...
                            if (inner == .lambda) {
                                ...
                                try self.putSymbolLocation(pattern.bind.symbol, pattern.bind.layout_idx, .{ .lambda_code = .{
                ...
                .bind => |bind| {
                    ...
                    if (force_mutable or bind.symbol.ident_idx.attributes.reassignable) {
                        ...
                        if (self.mutable_var_slots.get(symbol_key)) |var_info| {
                            ...
                            const declared_size: u32 = blk: {
                                if (expr_layout_override) |expr_layout| {
                                    if (@intFromEnum(expr_layout) < ls.layouts.len()) {
                                        ...
                                        const expr_layout_size = ls.layoutSizeAlign(expr_layout_val).size;
                                        if (expr_layout_size > 0) break :blk expr_layout_size;
                                    }
                                }
                            ...
                            try self.updateClosureStackInfoForCopy(fixed_slot, value_loc);
        ...
        fn stabilizeBindingLocation(
            ...
            return switch (loc) {
                ...
                .stack => |s| self.stackLocationForLayout(layout_idx, s.offset),
                else => loc,
            };
        }
        ...
        fn emitLoadStackF32(self: *Self, dst: FloatReg, offset: i32) Allocator.Error!void {
            if (comptime target.toCpuArch() == .aarch64) {
                if (offset >= 0) {
                    const uoffset: u12 = @intCast(@as(u32, @intCast(offset)) >> 2);
                    try self.codegen.emit.fldrRegMemUoff(.single, dst, .FP, uoffset);
                } else {
                    try self.codegen.emit.movRegImm64(.IP0, @bitCast(@as(i64, offset)));
                    try self.codegen.emit.addRegRegReg(.w64, .IP0, .FP, .IP0);
                    try self.codegen.emit.fldrRegMemUoff(.single, dst, .IP0, 0);
                }
            } else {
                try self.codegen.emit.movssRegMem(dst, .RBP, offset);
            }
        }

        fn emitStoreStackF32(self: *Self, offset: i32, src: FloatReg) Allocator.Error!void {
            if (comptime target.toCpuArch() == .aarch64) {
                if (offset >= 0) {
                    const uoffset: u12 = @intCast(@as(u32, @intCast(offset)) >> 2);
                    try self.codegen.emit.fstrRegMemUoff(.single, src, .FP, uoffset);
                } else {
                    try self.codegen.emit.movRegImm64(.IP0, @bitCast(@as(i64, offset)));
                    try self.codegen.emit.addRegRegReg(.w64, .IP0, .FP, .IP0);
                    try self.codegen.emit.fstrRegMemUoff(.single, src, .IP0, 0);
                }
            } else {
                try self.codegen.emit.movssMemReg(.RBP, offset, src);
            }
        }
        ...
        fn lambdaCodeFromExpr(self: *Self, expr_id: LirExprId) Allocator.Error!?ValueLocation {
            const expr = self.store.getExpr(expr_id);
            return switch (expr) {
                .lambda => |lam| blk: {
                    const code_offset = try self.compileLambdaAsProc(expr_id, lam, .{});
                    break :blk .{ .lambda_code = .{
                        .code_offset = code_offset,
                ...
                .closure => |closure_id| blk: {
                    const closure = self.store.getClosureData(closure_id);
                    const inner = self.store.getExpr(closure.lambda);
                    if (inner != .lambda) break :blk null;
                    const code_offset = try self.compileLambdaAsProc(closure.lambda, inner.lambda, .{});
                    break :blk .{ .lambda_code = .{
                        .code_offset = code_offset,
                        .ret_layout = inner.lambda.ret_layout,
                ...
                .nominal => |nom| try self.lambdaCodeFromExpr(nom.backing_expr),
                else => null,
            };
        }
        ...
        fn generateClosure(
            ...
            switch (closure.representation) {
                ...
                .one_capture => {
                    ...
                    // outer closure would create self-dispatch loops.
                    var mapped_underlying_closure = false;
                    const captures = self.store.getCaptures(closure.captures);
                    if (captures.len == 1 and self.isFunctionLayout(captures[0].layout_idx)) {
                        if (self.getSymbolLocation(captures[0].symbol, captures[0].layout_idx)) |cap_loc| {
                            if (self.closureInfoFromValueLocation(cap_loc)) |info| {
                                try self.closure_stack_info.put(slot, info);
                                mapped_underlying_closure = true;
                            }
                        }
                    }
                    if (!mapped_underlying_closure) {
                        try self.updateClosureStackInfoForCopy(slot, cv);
                    }
                    return cv;
                },
                .multiple_captures => |sc| {
                    ...
                    const declared_struct_size = ls.layoutSizeAlign(struct_layout).size;
        ...
        fn callSingleClosureWithCaptures(
            ...
            return self.compileLambdaAndCallWithCaptureArgs(
                cv.lambda,
                cv.captures,
                cv.stack_offset,
                args_span,
        ...
        fn compileLambdaAndCall(
            ...
            switch (lambda_expr) {
                .lambda => |lambda| {
                    ...
                    for (self.store.getExprSpan(args_span)) |arg_id| {
                        try param_layout_overrides.append(
                            self.allocator,
                            self.getExprLayout(arg_id) orelse layout.Idx.none,
                        );
                    }
        ...
        fn prebindCallableCaptures(
            self: *Self,
            captures_span: lir.LIR.LirCaptureSpan,
            capture_infos: []const ArgInfo,
        ) Allocator.Error!void {
            const captures = self.store.getCaptures(captures_span);
            const n = @min(captures.len, capture_infos.len);
            var i: usize = 0;
            while (i < n) : (i += 1) {
                const capture = captures[i];
                const loc = capture_infos[i].loc;
                const info_opt: ?ClosureStackInfo = switch (loc) {
                    .closure_value => self.closureInfoFromValueLocation(loc),
                    else => if (self.isFunctionLayout(capture.layout_idx))
                        self.closureInfoFromValueLocation(loc)
                    else
                        null,
                };
                if (info_opt) |info| {
                    try self.prebound_callables_by_layout.put(
                        self.symbolLayoutKey(capture.symbol, capture.layout_idx),
                        .{ .info = info },
                    );
                }
            }
        ...
        fn compileLambdaAndCallWithCaptureArgs(
            self: *Self,
            ...
            switch (self.store.getExpr(lambda_expr_id)) {
                .lambda => |lambda| {
                    ...
                    if (lambda.ret_layout != call_ret_layout) {
                        const lambda_body_id = @intFromEnum(lambda.body);
                        const fn_layout_id = @intFromEnum(lambda.fn_layout);
                        const lambda_tag = if (self.layout_store) |ls|
                            if (@intFromEnum(lambda.ret_layout) < ls.layouts.len()) @tagName(ls.getLayout(lambda.ret_layout).tag) else "out_of_range"
                        else
                            "no_layout_store";
                        const call_tag = if (self.layout_store) |ls|
                            if (@intFromEnum(call_ret_layout) < ls.layouts.len()) @tagName(ls.getLayout(call_ret_layout).tag) else "out_of_range"
                        else
                            "no_layout_store";
                        std.debug.panic(
                            "compileLambdaAndCallWithCaptureArgs: lambda return layout mismatch expr={} body={} fn_layout={} lambda={}({s}) call={}({s})",
                            .{
                    ...
                    self.prebound_callables_by_layout.clearRetainingCapacity();
                    try self.prebindCallableCaptures(captures_span, capture_infos.items);
                    try self.prebindCallableArgsForLambdaCall(lambda.params, args_span);
                    var param_layout_overrides = std.ArrayList(layout.Idx).empty;
                    defer param_layout_overrides.deinit(self.allocator);
                    for (self.store.getCaptures(captures_span)) |capture| {
                        try param_layout_overrides.append(self.allocator, capture.layout_idx);
                    }
                    for (self.store.getExprSpan(args_span)) |arg_id| {
                        try param_layout_overrides.append(
                            self.allocator,
                ...
                .closure => |closure_id| {
                    const closure = self.store.getClosureData(closure_id);
                    return self.compileLambdaAndCallWithCaptureArgs(closure.lambda, captures_span, captures_offset, args_span, ret_layout);
                },
                .nominal => |nom| {
                    return self.compileLambdaAndCallWithCaptureArgs(nom.backing_expr, captures_span, captures_offset, args_span, ret_layout);
                },
                else => unreachable,
            }
        }
        ...
        /// Binds captures to symbol_locations, then calls the compiled lambda procedure.
        ...
        fn generateLookupCall(self: *Self, lookup: anytype, args_span: anytype, ret_layout: layout.Idx) Allocator.Error!ValueLocation {
            ...
            const def_expr_id_opt = self.getSymbolDefExact(lookup.symbol);

            // Prefer definition-based dispatch when available. proc_registry is keyed
            // by symbol only and can represent a different specialization than this
            // call site; symbol_defs preserve the currently-lowered specialization.
            ...
            if (def_expr_id_opt) |def_expr_id| {
                ...
                return switch (def_expr) {
                    .lambda => |lam| blk: {
                        if (lam.ret_layout != ret_layout) {
                            std.debug.panic(
                                "generateLookupCall: lambda def layout mismatch symbol={} name={s} def_expr={} lambda_ret={} call_ret={}",
                                .{
                                    symbol_key,
                                    debug_name,
                                    @intFromEnum(def_expr_id),
                                    @intFromEnum(lam.ret_layout),
                                    @intFromEnum(ret_layout),
                                },
                            );
                        }
                        break :blk try self.compileLambdaAndCall(def_expr_id, args_span, ret_layout);
                    },
                    .closure => |_| {
        ...
        /// Call a builtin function using packed-args ABI:
        /// all collected call arguments are packed into a contiguous stack struct and
        /// passed as a single pointer argument.
        ///
        ...
        fn callBuiltin(self: *Self, builder: *Builder, fn_addr: usize, builtin_fn: BuiltinFn) Allocator.Error!void {
            switch (self.generation_mode) {
                .native_execution => {
                    try builder.callPacked(fn_addr);
                },
                .object_file => {
                    try builder.callRelocatablePacked(builtin_fn.symbolName(), self.allocator, &self.codegen.relocations);
                },
            }
        }
        ...
        fn normalizeResultLocForLayout(self: *Self, loc: ValueLocation, ret_layout: layout.Idx) ValueLocation {
            return switch (loc) {
                .closure_value => loc,
                .stack => |s| self.stackLocationForLayout(ret_layout, s.offset),
                else => loc,
            };
        }
        ...
        fn ensureInFloatReg(self: *Self, loc: ValueLocation) Allocator.Error!FloatReg {
            switch (loc) {
                ...
                .stack => |s| {
                    ...
                    const reg = self.codegen.allocFloat() orelse unreachable;
                    if (s.size == .dword) {
                        try self.emitLoadStackF32(reg, offset);
                        if (comptime target.toCpuArch() == .aarch64) {
        ...
        fn storeResultToSavedPtr(self: *Self, loc: ValueLocation, result_layout: layout.Idx, saved_ptr_reg: GeneralReg, tuple_len: usize) Allocator.Error!void {
            ...
            switch (result_layout) {
                ...
                .f32 => {
                    // F32: .stack may hold either raw f32 (size=dword) or f64 (size=qword).
                    switch (loc) {
                        .float_reg => |reg| {
        ...
        fn compileProc(self: *Self, proc: LirProc) Allocator.Error!void {
            ...
            var saved_symbol_layout_overrides = self.symbol_layout_overrides.clone() catch return error.OutOfMemory;
            defer saved_symbol_layout_overrides.deinit();
            ...
            self.symbol_layout_overrides.deinit();
            self.symbol_layout_overrides = saved_symbol_layout_overrides.clone() catch return error.OutOfMemory;
            self.mutable_var_slots.deinit();
            self.mutable_var_slots = saved_mutable_var_slots.clone() catch return error.OutOfMemory;
            self.closure_stack_info.deinit();
            self.closure_stack_info = saved_closure_stack_info.clone() catch return error.OutOfMemory;
        }
        ...
        fn compileLambdaAsProc(self: *Self, lambda_expr_id: LirExprId, lambda: anytype, options: LambdaCompileOptions) Allocator.Error!usize {
            ...
            if (options.param_layout_overrides) |overrides| {
                ...
                for (overrides) |li| {
                    const li_int: u32 = @intFromEnum(li);
            ...
            const key: LambdaCompileKey = .{
                ...
                .layout_override_len = override_len,
            };
            ...
            errdefer {
                ...
                self.symbol_layout_overrides.deinit();
                self.symbol_layout_overrides = saved_symbol_layout_overrides.clone() catch unreachable;
                self.mutable_var_slots.deinit();
            ...
            const needs_ret_ptr = self.needsInternalReturnByPointer(proc_ret_layout);
            if (needs_ret_ptr) {
            ...
            try self.bindLambdaParamsWithCaptures(
                lambda_expr_id,
                options.capture_params,
                lambda.params,
                if (needs_ret_ptr) 1 else 0,
                options.param_layout_overrides,
            );

            // Set early return state so generateEarlyReturn can emit jumps
            self.early_return_ret_layout = proc_ret_layout;

            // Generate the body
            const result_loc = try self.generateExpr(lambda.body);
            const normalized_result_loc = self.normalizeResultLocForLayout(result_loc, proc_ret_layout);

            // Move result to return register or copy to return pointer
            if (self.ret_ptr_slot) |ret_slot| {
                try self.copyResultToReturnPointer(normalized_result_loc, proc_ret_layout, ret_slot);
            } else {
                try self.moveToReturnRegisterWithLayout(normalized_result_loc, proc_ret_layout);
            }
        ...
        fn saveCallReturnValue(
            ...
            if (returned_closure_info != null and !self.isFunctionLayout(ret_layout)) {
                std.debug.panic("saveCallReturnValue: closure metadata on non-callable return layout={}", .{
                    @intFromEnum(ret_layout),
                });
            }
            ...
            if (returned_closure_info) |info| {
                const closure_size = self.closureStorageSize(info.representation);
                if (closure_size > 8) {
                    const stack_offset = self.codegen.allocStackSlot(closure_size);
                    const num_regs = (closure_size + 7) / 8;
                    if (comptime target.toCpuArch() == .aarch64) {
                        const regs = [_]@TypeOf(GeneralReg.X0){ .X0, .X1, .X2, .X3, .X4, .X5, .X6, .X7, .XR, .X9, .X10, .X11, .X12, .X13, .X14, .X15 };
                        if (num_regs > regs.len) {
                            std.debug.panic("saveCallReturnValue: closure return exceeds register return capacity size={}", .{
                                closure_size,
                            });
                        }
                        for (0..num_regs) |i| {
                ...
                if (reg_idx + info.num_regs <= max_arg_regs) {
                    ...
                    if (info.num_regs == 3) {
                        ...
                        switch (arg_loc) {
                            ...
                            .immediate_f64 => |val| {
                                if (arg_layout != null and arg_layout.? == .f32) {
                                    const bits32: u32 = @bitCast(@as(f32, @floatCast(val)));
                                    try self.codegen.emitLoadImm(arg_reg, @as(i64, @intCast(bits32)));
                                } else {
                                    const bits64: u64 = @bitCast(val);
                                    try self.codegen.emitLoadImm(arg_reg, @bitCast(bits64));
                                }
                            },
                            .lambda_code => |lc| {
                                const current = self.codegen.currentOffset();
        ...
        fn aliasLambdaParamSymbolsBySourceExpr(
            self: *Self,
            params: lir.LirPatternSpan,
            saved_symbol_source_exprs: *const std.AutoHashMap(u64, LirExprId),
        ) Allocator.Error!void {
            const pattern_ids = self.store.getPatternSpan(params);

            for (pattern_ids) |pattern_id| {
                const pattern = self.store.getPattern(pattern_id);
                if (pattern != .bind) continue;

                const bind = pattern.bind;
                const bind_key = symbolKey(bind.symbol);
                const source_expr_id = saved_symbol_source_exprs.get(bind_key) orelse continue;
                const bound_loc = self.getSymbolLocation(bind.symbol, bind.layout_idx) orelse continue;

                var it = saved_symbol_source_exprs.iterator();
                while (it.next()) |entry| {
                    if (entry.value_ptr.* != source_expr_id) continue;
                    const alias_key = entry.key_ptr.*;
                    if (alias_key == bind_key) continue;

                    const alias_symbol: Symbol = @bitCast(alias_key);
                    try self.putSymbolLocation(alias_symbol, bind.layout_idx, bound_loc);
                }
            }
        }

        fn bindLambdaParamsWithCaptures(
            self: *Self,
            lambda_expr_id: LirExprId,
            captures_span: lir.LIR.LirCaptureSpan,
            params: lir.LirPatternSpan,
            initial_reg_idx: u8,
            param_layout_overrides: ?[]const layout.Idx,
        ) Allocator.Error!void {
            const captures = self.store.getCaptures(captures_span);
            if (captures.len == 0) {
                return self.bindLambdaParams(params, initial_reg_idx, param_layout_overrides);
            }

            const mutable_store: *LirExprStore = @constCast(self.store);
            const region = self.store.getExprRegion(lambda_expr_id);
            const params_ids = self.store.getPatternSpan(params);

            var pattern_ids = std.ArrayList(lir.LirPatternId).empty;
            defer pattern_ids.deinit(self.allocator);
            try pattern_ids.ensureTotalCapacity(self.allocator, captures.len + params_ids.len);
            ...
            for (captures) |capture| {
                ...
                try pattern_ids.append(self.allocator, pat);
            }
            for (params_ids) |param_id| {
                try pattern_ids.append(self.allocator, param_id);
            }

            const combined_params = try mutable_store.addPatternSpan(pattern_ids.items);
            return self.bindLambdaParams(combined_params, initial_reg_idx, param_layout_overrides);
        }
        ...
        fn bindLambdaParams(
...

            {
                ...
                for (pattern_ids, 0..) |pid, pi| {
                    ...
                    const nr: u8 = switch (pat) {
                        .bind => |b| blk: {
                            const bind_layout = if (param_layout_overrides) |overrides|
                                if (pi < overrides.len and overrides[pi] != layout.Idx.none) overrides[pi] else b.layout_idx
                            else
                                b.layout_idx;
                            if (self.preboundCallableStorageSize(b.symbol, b.layout_idx)) |closure_size| {
                                break :blk @max(1, @as(u8, @intCast((closure_size + 7) / 8)));
                            }
                            break :blk self.calcParamRegCount(bind_layout);
                        },
                        .wildcard => |w| blk: {
                            const wildcard_layout = if (param_layout_overrides) |overrides|
                                if (pi < overrides.len and overrides[pi] != layout.Idx.none) overrides[pi] else w.layout_idx
                            else
                                w.layout_idx;
                            break :blk self.calcParamRegCount(wildcard_layout);
                        },
                        .struct_ => |s| blk: {
            ...
            for (pattern_ids, 0..) |pattern_id, param_idx| {
                ...
                switch (pattern) {
                    .bind => |bind| {
                        const bind_layout = if (param_layout_overrides) |overrides|
                            if (param_idx < overrides.len and overrides[param_idx] != layout.Idx.none) overrides[param_idx] else bind.layout_idx
                        else
                            bind.layout_idx;
                        ...
                        const num_regs: u8 = if (prebound_size) |closure_size|
                            ...
                            self.calcParamRegCount(bind_layout);
                        ...
                        if (param_pass_by_ptr[param_idx]) {
                            ...
                            if (prebound_size != null) {
                                if (prebound_i128) {
                                    try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, .{ .stack_i128 = local_stack_offset });
                                } else {
                                    try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, .{ .stack = .{ .offset = local_stack_offset } });
                                }
                            } else if (bind_layout == .str) {
                                ...
                                if (@intFromEnum(bind_layout) < ls.layouts.len()) {
                                    const layout_val = ls.getLayout(bind_layout);
                                    if (layout_val.tag == .list or layout_val.tag == .list_of_zst) {
                                        try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, .{ .list_stack = .{
                                            .struct_offset = local_stack_offset,
                                            .data_offset = 0,
                                            .num_elements = 0,
                                        } });
                                    } else {
                                        try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, self.stackLocationForLayout(bind_layout, local_stack_offset));
                                    }
                                } else {
                                    try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, self.stackLocationForLayout(bind_layout, local_stack_offset));
                                }
                            ...
                            if (bind_layout == .str) {
                                ...
                                try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, .{ .stack_str = stack_offset });
                                try self.applyPreboundCallableForBinding(bind.symbol, bind.layout_idx, stack_offset);
                                reg_idx += 3;
                            } else if (bind_layout == .i128 or bind_layout == .u128 or bind_layout == .dec) {
                                ...
                                try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, .{ .stack_i128 = stack_offset });
                                try self.applyPreboundCallableForBinding(bind.symbol, bind.layout_idx, stack_offset);
                                reg_idx += 2;
                            } else if (self.layout_store) |ls| {
                                if (@intFromEnum(bind_layout) < ls.layouts.len()) {
                                    ...
                                    if (prebound_size != null and num_regs > 1) {
                                        ...
                                        if (prebound_i128) {
                                            ...
                                            try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, .{ .stack = .{ .offset = stack_offset } });
                                        }
                                        try self.applyPreboundCallableForBinding(bind.symbol, bind.layout_idx, stack_offset);
                                        reg_idx += num_regs;
                                        continue;
                                ...
                                const arg_reg = self.getArgumentRegister(reg_idx);
                                const scalar_size = self.getLayoutSize(bind_layout);
                                ...
                                try self.putSymbolLocationWithOverride(bind.symbol, bind.layout_idx, bind_layout, self.stackLocationForLayout(bind_layout, stack_offset));
                                try self.applyPreboundCallableForBinding(bind.symbol, bind.layout_idx, stack_offset);
                                reg_idx += 1;
                            } else {
                                // Default: scalar single-register value
                    ...
                    .wildcard => |wc| {
                        const wildcard_layout = if (param_layout_overrides) |overrides|
                            if (param_idx < overrides.len and overrides[param_idx] != layout.Idx.none) overrides[param_idx] else wc.layout_idx
                        else
                            wc.layout_idx;
                        // Skip this argument - use the layout to determine how many
                        // registers it occupies (important for correct roc_ops placement)
                        const num_regs = self.calcParamRegCount(wildcard_layout);
                        // aarch64: i128/Dec must be even-aligned in register pairs
                        if (comptime target.toCpuArch() == .aarch64) {
                            if (num_regs == 2 and (wildcard_layout == .i128 or wildcard_layout == .u128 or wildcard_layout == .dec)) {
        ...
        fn moveToReturnRegisterWithLayout(self: *Self, loc: ValueLocation, ret_layout: layout.Idx) Allocator.Error!void {
            if (loc == .closure_value) {
                ...
                if (closure_size <= 8) {
                    try self.codegen.emitLoadStack(.w64, ret_reg_0, cv.stack_offset);
                    return;
                }
                {
                    const num_regs = (closure_size + 7) / 8;
                    if (comptime target.toCpuArch() == .aarch64) {
                        const regs = [_]@TypeOf(GeneralReg.X0){ .X0, .X1, .X2, .X3, .X4, .X5, .X6, .X7, .XR, .X9, .X10, .X11, .X12, .X13, .X14, .X15 };
                        if (num_regs > regs.len) {
                            std.debug.panic("moveToReturnRegisterWithLayout: closure return exceeds register return capacity size={}", .{
                                closure_size,
                            });
                        }
                        for (0..num_regs) |i| {
                            try self.codegen.emitLoadStack(.w64, regs[i], cv.stack_offset + @as(i32, @intCast(i * 8)));
                        }
                    } else {
                        const regs = [_]@TypeOf(GeneralReg.RAX){ .RAX, .RDX, .RCX, .R8, .R9, .R10, .R11, .RDI, .RSI };
                        if (num_regs > regs.len) {
                ...
                switch (loc) {
                    .float_reg => |freg| {
                        ...
                        if (ret_layout == .f32) {
                            if (comptime target.toCpuArch() == .aarch64) {
                                try self.codegen.emit.fcvtFloatFloat(.single, freg, .double, freg);
                                const uoffset: u12 = @intCast(@as(u32, @intCast(slot)) >> 2);
                                try self.codegen.emit.fstrRegMemUoff(.single, freg, .FP, uoffset);
                                try self.codegen.emitLoadStack(.w32, ret_reg_0, slot);
                            } else {
                                try self.codegen.emit.cvtsd2ssRegReg(freg, freg);
                            ...
                            try self.codegen.emitLoadStack(.w64, ret_reg_0, slot);
                        }
                        return;
                    },
                    .immediate_f64 => |val| {
                        if (ret_layout == .f32) {
                            const bits32: u32 = @bitCast(@as(f32, @floatCast(val)));
                            try self.codegen.emitLoadImm(ret_reg_0, @as(i64, @intCast(bits32)));
                        } else {
                            const bits64: u64 = @bitCast(val);
                            try self.codegen.emitLoadImm(ret_reg_0, @bitCast(bits64));
                        }
                        return;
                    },
                    .stack => |s| {
                        if (ret_layout == .f32) {
                            if (s.size == .dword) {
                                try self.codegen.emitLoadStack(.w32, ret_reg_0, s.offset);
                            } else {
                                const slot = self.codegen.allocStackSlot(8);
                                const freg = self.codegen.allocFloat() orelse unreachable;
                                ...
                                try self.emitStoreStackF32(slot, freg);
                                self.codegen.freeFloat(freg);
                                try self.codegen.emitLoadStack(.w32, ret_reg_0, slot);
                            }
                        } else {
                            if (s.size == .dword) {
                                const slot = self.codegen.allocStackSlot(8);
                                const freg = self.codegen.allocFloat() orelse unreachable;
                                try self.emitLoadStackF32(freg, s.offset);
                                if (comptime target.toCpuArch() == .aarch64) {
                                    try self.codegen.emit.fcvtFloatFloat(.double, freg, .single, freg);
                                } else {
                                    try self.codegen.emit.cvtss2sdRegReg(freg, freg);
                                }
                                try self.codegen.emitStoreStackF64(slot, freg);
                                self.codegen.freeFloat(freg);
                                try self.codegen.emitLoadStack(.w64, ret_reg_0, slot);
                ...
                if (source_expr) |expr_id| {
                    if (self.unwrapCallableLambdaExpr(expr_id)) |lambda_expr_id| {
                        ...
                        if (lambda_expr == .lambda and lambda_expr.lambda.ret_layout != call_ret_layout) {
                            std.debug.panic(
                                "generateCallToLambdaWithSource: lambda return layout mismatch expr={} lambda={} call={}",
                                .{
                                    @intFromEnum(lambda_expr_id),
                                    @intFromEnum(lambda_expr.lambda.ret_layout),
                                    @intFromEnum(call_ret_layout),
                                },
                            );
                        }
                    ...
                    .bind => |bind| {
                        ...
                        if (reg_idx + num_regs <= max_arg_regs) {
                            ...
                            if (is_128bit) {
                                ...
                                try self.emitStore(.w64, frame_ptr, stack_offset + 8, reg1);
                                try self.emitStore(.w64, frame_ptr, stack_offset + 16, reg2);
                                try self.putSymbolLocation(bind.symbol, param_layout_idx, .{ .list_stack = .{
                                    .struct_offset = stack_offset,
                                    .data_offset = 0,
                                    .num_elements = 0,
                                } });
                            ...
                            // Set up symbol location based on type
                            if (is_128bit) {
                                try self.putSymbolLocation(bind.symbol, param_layout_idx, .{ .stack_i128 = local_stack_offset });
                            } else if (is_str) {
                                try self.putSymbolLocation(bind.symbol, param_layout_idx, .{ .stack_str = local_stack_offset });
                            } else if (is_list) {
                                try self.putSymbolLocation(bind.symbol, param_layout_idx, .{ .list_stack = .{
                                    .struct_offset = local_stack_offset,
                                    .data_offset = 0,
        ...
        fn rebindJoinPointParams(self: *Self, join_point: JoinPointId, arg_locs: []const ValueLocation) Allocator.Error!void {
            ...
            _ = self.join_point_param_layouts.get(jp_key) orelse unreachable;
            ...
            if (arg_locs.len <= 1) {
                try self.rebindSingleParam(arg_locs, pattern_ids);
                return;
            }
        ...
        fn rebindSingleParam(self: *Self, arg_locs: []const ValueLocation, pattern_ids: anytype) Allocator.Error!void {
            for (arg_locs, 0..) |loc, param_idx| {
                if (param_idx >= pattern_ids.len) continue;
        ...
        fn generateFree(self: *Self, rc_op: anytype) Allocator.Error!ValueLocation {
            ...
            switch (layout_val.tag) {
                .list, .list_of_zst => {
                    const list_info = ls.getListInfo(layout_val);
                    try self.emitListFree(value_loc, list_info.elem_alignment, list_info.contains_refcounted);
                },
                .scalar => {
                    if (layout_val.data.scalar.tag == .str) {
                ...
                .box, .box_of_zst => {
                    const box_info = ls.getBoxInfo(layout_val);
                    try self.emitBoxFree(value_loc, box_info.elem_alignment, box_info.contains_refcounted);
                },
        ...
        fn emitListIncref(self: *Self, value_loc: ValueLocation, count: u16) Allocator.Error!void {
            ...
            const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_incref_data_ptr_packed);
        ...
        fn emitListFree(self: *Self, value_loc: ValueLocation, elem_alignment: u32, elements_refcounted: bool) Allocator.Error!void {
            const roc_ops_reg = self.roc_ops_reg orelse return;
            const fn_addr: usize = @intFromPtr(&dev_wrappers.roc_builtins_free_data_ptr_packed);
        ...
        /// Load the list allocation data pointer for RC operations.
        /// For seamless slices (`cap < len`), the allocation pointer is stored
        /// in `capacity_or_alloc_ptr` as `alloc_ptr >> 1`.
        fn loadListDataPtrForRc(self: *Self, value_loc: ValueLocation, out_reg: GeneralReg) Allocator.Error!bool {
            const base_offset: i32 = switch (value_loc) {
                .stack => |s| s.offset,
                .list_stack => |info| info.struct_offset,
                else => return false,
            ...
            // if cap < len then out = cap << 1 else out = bytes
            try self.emitCmpReg(cap_reg, len_reg);
            const not_slice_patch = try self.emitJumpIfGreaterOrEqual();
            try self.emitMovRegReg(out_reg, cap_reg);
            try self.emitShlImm(.w64, out_reg, out_reg, 1);
            self.codegen.patchJump(not_slice_patch, self.codegen.currentOffset());
            return true;
        }
