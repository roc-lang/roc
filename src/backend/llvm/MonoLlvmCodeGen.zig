...
pub const MonoLlvmCodeGen = struct {
    ...
    fn emitValueEqualNode(
        ...
        switch (layout_val.tag) {
            ...
            .box, .erased_callable => {
                ...
                out.* = wip.icmp(.eq, lhs, rhs, "") catch return error.CompilationFailed;
            },
    ...
    fn emitListEqual(
        ...
        try work.append(wa, .{ .eval = .{
            .lhs_ptr = wip.gep(.inbounds, .i8, lhs_bytes, &.{offset}, "") catch return error.CompilationFailed,
            .rhs_ptr = wip.gep(.inbounds, .i8, rhs_bytes, &.{offset}, "") catch return error.CompilationFailed,
    ...
    fn emitTagEqualCase(self: *MonoLlvmCodeGen, state: *TagEqState, wa: Allocator, work: *std.ArrayList(EqWork)) Error!void {
        ...
        if (i + 1 < state.case_blocks.len) {
            const is_case = wip.icmp(.eq, state.lhs_disc, builder.intValue(state.lhs_disc.typeOfWip(wip), i) catch return error.OutOfMemory, "") catch return error.CompilationFailed;
    ...
    fn emitTagEqualCaseAfter(self: *MonoLlvmCodeGen, state: *TagEqState, wa: Allocator, work: *std.ArrayList(EqWork)) Error!void {
        ...
        _ = wip.br(state.after) catch return error.CompilationFailed;
