...
module: WasmModule,
code_builder: CodeBuilder, // accumulates instruction bytes + relocations for current function
storage: Storage,
...
/// In the RocCall entrypoint, this is parameter 0 (provided by the host).
/// In compiled proc functions, this is also parameter 0.
roc_ops_local: u32 = 0,
...
///
/// Produces two exported functions:
/// - `entrypoint_name`: RocCall ABI `(i32 roc_ops_ptr, i32 ret_ptr, i32 args_ptr) → void`
///   The app entrypoint used by surgical linking. The name comes from the platform's
///   `provides` section (e.g. `roc__main_for_host_1_exposed`). Receives the RocOps struct
///   from the host, writes its return value to `ret_ptr`.
...
pub fn generateModule(self: *Self, expr_id: LirExprId, result_layout: layout.Idx, entrypoint_name: []const u8) Allocator.Error!GenerateResult {
    ...
    // Determine return type and representation from the expression.
    const result_vt = self.exprValType(expr_id);
    const result_repr = WasmLayout.wasmReprWithStore(result_layout, self.getLayoutStore());
    const result_byte_size: u32 = switch (result_repr) {
    ...
    // Store result to ret_ptr instead of returning on the wasm stack.
    try self.emitStoreResultToRetPtr(ret_ptr_local, result_vt, result_repr);
    ...
    // Build RocCall function body: locals + prologue + instructions + epilogue + end
    var roc_call_body: std.ArrayList(u8) = .empty;
    defer roc_call_body.deinit(self.allocator);

    // Encode locals declaration (skip 3 for the RocCall parameters)
    try self.encodeLocalsDecl(&roc_call_body, 3);
    ...
    try self.emitStackEpilogue(&roc_call_body);

    // End opcode
    roc_call_body.append(self.allocator, Op.end) catch return error.OutOfMemory;
    ...
    var eval_body: std.ArrayList(u8) = .empty;
    defer eval_body.deinit(self.allocator);

    try self.emitEvalWrapper(&eval_body, roc_call_func_idx, result_repr, result_byte_size);
...
/// Store the result from the wasm value stack into ret_ptr.
/// For primitives, stores the scalar value directly.
/// For composites (stack_memory), copies the data from the pointer on the stack.
fn emitStoreResultToRetPtr(self: *Self, ret_ptr_local: u32, result_vt: ValType, result_repr: WasmLayout.WasmRepr) Allocator.Error!void {
    switch (result_repr) {
        .primitive => {
            // Stack has: [result_value]
            // Save result to temp, then store to ret_ptr
            const tmp = self.storage.allocAnonymousLocal(result_vt) catch return error.OutOfMemory;
            try self.emitLocalSet(tmp);
            try self.emitLocalGet(ret_ptr_local);
            try self.emitLocalGet(tmp);
            try self.emitStoreOp(result_vt, 0);
        },
        .stack_memory => |byte_size| {
            if (byte_size == 0) {
                // Zero-sized type — drop the value, nothing to store
                self.code_builder.code.append(self.allocator, Op.drop) catch return error.OutOfMemory;
                return;
            }
            // Stack has: [src_ptr] (i32 pointer to composite data)
            const src_ptr = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            try self.emitLocalSet(src_ptr);

            // Copy byte_size bytes from src_ptr to ret_ptr using unrolled 4-byte chunks
            var offset: u32 = 0;
            ...
            while (offset < byte_size) : (offset += 1) {
                ...
                try self.emitLocalGet(src_ptr);
                // i32.load8_u offset
                self.code_builder.code.append(self.allocator, Op.i32_load8_u) catch return error.OutOfMemory;
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // align
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
                // i32.store8 offset
                self.code_builder.code.append(self.allocator, Op.i32_store8) catch return error.OutOfMemory;
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // align
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
            }
        },
...
/// Emit the stack frame deallocation epilogue into a function body buffer.
fn emitStackEpilogue(self: *Self, func_body: *std.ArrayList(u8)) Allocator.Error!void {
    // local.get $fp
...
/// Emit the eval wrapper function body.
///
/// The wrapper has signature `(i32 env_ptr) → result_vt` and:
/// 1. Builds a RocOps struct at memory offset 0
/// 2. Allocates a return buffer on the wasm stack
/// 3. Calls the RocCall function with (roc_ops_ptr=0, ret_buf, args_ptr=0)
/// 4. Loads the result from the return buffer and returns it
fn emitEvalWrapper(
    self: *Self,
    eval_body: *std.ArrayList(u8),
    roc_call_func_idx: u32,
    result_repr: WasmLayout.WasmRepr,
    result_byte_size: u32,
) Allocator.Error!void {
    // Eval wrapper locals (manually managed, not using self.storage):
    ...
    eval_body.append(self.allocator, Op.global_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, eval_body, 0) catch return error.OutOfMemory;
    // i32.const eval_frame_size
    eval_body.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, eval_body, @intCast(eval_frame_size)) catch return error.OutOfMemory;
    // i32.sub
    eval_body.append(self.allocator, Op.i32_sub) catch return error.OutOfMemory;
    // local.tee $fp
    eval_body.append(self.allocator, Op.local_tee) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, eval_body, eval_fp_local) catch return error.OutOfMemory;
    // global.set $__stack_pointer
    eval_body.append(self.allocator, Op.global_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, eval_body, 0) catch return error.OutOfMemory;

    // ret_buf_ptr = fp + 0 (return buffer at start of frame)
    ...
    eval_body.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, eval_body, eval_ret_buf_local) catch return error.OutOfMemory;
    // arg 2: args_ptr = 0 (no arguments for eval)
    eval_body.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, eval_body, 0) catch return error.OutOfMemory;
    // call <entrypoint>
    eval_body.append(self.allocator, Op.call) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, eval_body, roc_call_func_idx) catch return error.OutOfMemory;

    // Load result from return buffer
    switch (result_repr) {
        .primitive => |vt| {
            // Load the scalar value from ret_buf and return it
            eval_body.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, eval_body, eval_ret_buf_local) catch return error.OutOfMemory;
            const load_op: u8 = switch (vt) {
                .i32 => Op.i32_load,
                .i64 => Op.i64_load,
                .f32 => Op.f32_load,
                .f64 => Op.f64_load,
            };
            eval_body.append(self.allocator, load_op) catch return error.OutOfMemory;
            const align_log2: u32 = switch (vt) {
                .i32, .f32 => 2,
                .i64, .f64 => 3,
            };
            WasmModule.leb128WriteU32(self.allocator, eval_body, align_log2) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, eval_body, 0) catch return error.OutOfMemory; // offset
        },
        .stack_memory => {
            // Composite data was written to ret_buf by the RocCall function.
            // Return the pointer to the buffer (i32).
            eval_body.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, eval_body, eval_ret_buf_local) catch return error.OutOfMemory;
        },
    }
    ...
    // End opcode
    eval_body.append(self.allocator, Op.end) catch return error.OutOfMemory;
}
...
fn generateExpr(self: *Self, expr_id: LirExprId) Allocator.Error!void {
    ...
    switch (expr) {
        .i64_literal => |val| {
            switch (self.resolveValType(val.layout_idx)) {
                .i32 => {
                    ...
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @truncate(val.value)) catch return error.OutOfMemory;
                },
                .i64 => {
                    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, val.value) catch return error.OutOfMemory;
                },
                .f32 => {
                    self.code_builder.code.append(self.allocator, Op.f32_const) catch return error.OutOfMemory;
                    const bytes: [4]u8 = @bitCast(@as(f32, @floatFromInt(val.value)));
                    self.code_builder.code.appendSlice(self.allocator, &bytes) catch return error.OutOfMemory;
                },
                .f64 => {
                    self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
        ...
        .bool_literal => |val| {
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, if (val) 1 else 0) catch return error.OutOfMemory;
        },
        .dec_literal => |val| {
            ...
            // Store low 8 bytes
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, low) catch return error.OutOfMemory;
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, high) catch return error.OutOfMemory;
            ...
            for (stmts) |stmt_union| {
                ...
                switch (pattern) {
                    .bind => |bind| {
                        {
                            ...
                            if (expr_is_composite and !target_is_composite) {
                                ...
                                if (target_size == expr_size or target_size < expr_size) {
                                    ...
                                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, local_idx) catch return error.OutOfMemory;
                                    continue;
                                }
                                unreachable;
                                ...
                                if (target_size > expr_size) {
                                        ...
                                        (bind.layout_idx == .i128 or bind.layout_idx == .dec))
                                    {
                                        ...
                                        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_local) catch return error.OutOfMemory;
                                        self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
                                        WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, -1) catch return error.OutOfMemory;
                                ...
                                if (stack_offset > 0) {
                                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(stack_offset)) catch return error.OutOfMemory;
                    ...
                    .struct_ => |s| {
                        ...
                        self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, ptr) catch return error.OutOfMemory;
                    ...
                    .as_pattern => |as_pat| {
                        ...
                        self.code_builder.code.append(self.allocator, Op.local_tee) catch return error.OutOfMemory;
                        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, local_idx) catch return error.OutOfMemory;
                        ...
                        switch (inner) {
                            .bind => |inner_bind| {
                                ...
                                self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, inner_local) catch return error.OutOfMemory;
                            },
                            .wildcard => {
                                self.code_builder.code.append(self.allocator, Op.drop) catch return error.OutOfMemory;
                            },
        ...
        .lookup => |l| {
            ...
            if (self.storage.locals.get(key)) |local_info| {
                self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, local_info.idx) catch return error.OutOfMemory;
        ...
        .empty_list => {
            ...
            if (base_offset > 0) {
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(base_offset)) catch return error.OutOfMemory;
                self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
            }
        },
        .runtime_error => {
            self.code_builder.code.append(self.allocator, Op.@"unreachable") catch return error.OutOfMemory;
        },
        .crash => |crash| {
            ...
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(data_offset)) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_store) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;

            // Write len
            try self.emitFpOffset(crashed_slot);
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(msg_bytes.len)) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_store) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 4) catch return error.OutOfMemory;
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, self.roc_ops_type_idx) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        ...
        .early_return => |er| {
            ...
            self.code_builder.code.append(self.allocator, Op.@"return") catch return error.OutOfMemory;
        },
        .proc_call => |c| {
        ...
        .break_expr => {
            ...
            self.code_builder.code.append(self.allocator, Op.br) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(
                self.allocator,
                &self.code_builder.code,
    ...
    switch (l.tag) {
        ...
        .tag_union => {
            ...
            for (0..variants.len) |variant_i| {
                ...
                self.code_builder.code.append(self.allocator, Op.block) catch return error.OutOfMemory;
                ...
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(variant_i)) catch return error.OutOfMemory;
                self.code_builder.code.append(self.allocator, Op.i32_ne) catch return error.OutOfMemory;
                self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
...
fn emitDecodeListAllocPtr(self: *Self, list_ptr_local: u32, out_alloc_ptr: u32, out_is_slice: u32) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_lt_s) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, @intFromEnum(ValType.i32)) catch return error.OutOfMemory;
    try self.emitLocalGet(cap_local);
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 1) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_shl) catch return error.OutOfMemory;
...
fn emitPtrWithOffset(self: *Self, ptr_local: u32, offset: i32) Allocator.Error!void {
    ...
    if (offset != 0) {
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
    }
}
...
fn emitPrepareListSliceMetadata(self: *Self, list_ptr_local: u32, elements_refcounted: bool, out_encoded_cap: u32) Allocator.Error!void {
    ...
    if (elements_refcounted) {
        ...
        self.code_builder.code.append(self.allocator, Op.i32_eqz) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 1) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_shr_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @as(i32, @bitCast(@as(u32, 0x80000000)))) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_or) catch return error.OutOfMemory;
...
fn emitCallRocDealloc(self: *Self, ptr_local: u32, alignment: u32) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(alignment)) catch return error.OutOfMemory;
    try self.emitStoreOp(.i32, 0);
...
fn emitFreeRcPtr(self: *Self, rc_ptr_local: u32, element_alignment: u32, elements_refcounted: bool) Allocator.Error!void {
    ...
    if (alloc_adjust > 0) {
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, alloc_adjust) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_sub) catch return error.OutOfMemory;
    }
...
fn emitDataPtrIncref(self: *Self, data_ptr_local: u32, amount: u16) Allocator.Error!void {
    ...
    try self.emitLocalGet(data_ptr_local);
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, -4) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
    ...
    try self.emitLocalGet(rc_val);
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(amount)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
    try self.emitStoreOp(.i32, 0);
...
fn emitDataPtrDecref(self: *Self, data_ptr_local: u32, alignment: u32, elements_refcounted: bool) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 1) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_eq) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
...
fn emitListElementDecrefsIfUnique(
    ...
    self.code_builder.code.append(self.allocator, Op.block) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.loop_) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;

    try self.emitLocalGet(idx_local);
    try self.emitLocalGet(count_local);
    self.code_builder.code.append(self.allocator, Op.i32_ge_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 1) catch return error.OutOfMemory;

    try self.emitLocalGet(alloc_ptr_local);
    try self.emitLocalGet(idx_local);
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(elem_size)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_mul) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
...
fn generateIfChain(self: *Self, branches: []const LIR.LirIfBranch, final_else: LirExprId, bt: BlockType) Allocator.Error!void {
    ...
    // if (block_type)
    self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, @intFromEnum(bt)) catch return error.OutOfMemory;
...
fn generateMatch(self: *Self, w: anytype) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, temp_local) catch return error.OutOfMemory;
...
fn generateMatchBranches(self: *Self, branches: []const LIR.LirMatchBranch, value_local: u32, value_vt: ValType, bt: BlockType) Allocator.Error!void {
    ...
    switch (pattern) {
        ...
        .bind => |bind| {
            ...
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, value_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, local_idx) catch return error.OutOfMemory;
            try self.generateExpr(branch.body);
        },
        .int_literal => |int_pat| {
            ...
            switch (value_vt) {
                .i32 => {
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @truncate(@as(i64, @truncate(int_pat.value)))) catch return error.OutOfMemory;
                },
                .i64 => {
                    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, @truncate(int_pat.value)) catch return error.OutOfMemory;
                },
                .f32, .f64 => unreachable,
            }
            ...
            self.code_builder.code.append(self.allocator, eq_op) catch return error.OutOfMemory;
        ...
        .tag => |tag_pat| {
            ...
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(tag_pat.discriminant)) catch return error.OutOfMemory;
                ...
                for (arg_patterns) |arg_pat_id| {
                    ...
                    switch (arg_pat) {
                        .bind => |bind| {
                            ...
                            switch (wasm_repr) {
                                .stack_memory => {
                                    ...
                                    if (payload_offset > 0) {
                                        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                                        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(payload_offset)) catch return error.OutOfMemory;
                                        self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
                                    }
                                },
                                .primitive => {
                        ...
                        .struct_ => |inner_struct| {
                            ...
                            const field_ptr = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
                            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, field_ptr) catch return error.OutOfMemory;
        ...
        .float_literal => |float_pat| {
            ...
            switch (value_vt) {
                .f64 => {
                    ...
                    self.code_builder.code.appendSlice(self.allocator, &bytes) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.f64_eq) catch return error.OutOfMemory;
                },
                .f32 => {
                    self.code_builder.code.append(self.allocator, Op.f32_const) catch return error.OutOfMemory;
                    const bytes: [4]u8 = @bitCast(@as(f32, @floatCast(float_pat.value)));
                    self.code_builder.code.appendSlice(self.allocator, &bytes) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.f32_eq) catch return error.OutOfMemory;
                },
                .i32, .i64 => unreachable,
            }
        ...
        .str_literal => |str_idx| {
            ...
            try self.emitPtrLenCapArgs(value_local);
            try self.emitPtrLenCapArgs(pat_str);
        ...
        .list => |list_pat| {
            ...
            // Compare length
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(prefix_count)) catch return error.OutOfMemory;
                    ...
                    switch (pat) {
                        .bind => |bind| {
                            if (is_composite and elem_size > 0) {
                                ...
                                if (elem_offset > 0) {
                                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(elem_offset)) catch return error.OutOfMemory;
                                    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
                                }
                            } else {
...
fn compareTagUnionByLayout(self: *Self, lhs_local: u32, rhs_local: u32, layout_idx: layout.Idx) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.block) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, 0x40) catch return error.OutOfMemory; // void block type
    try self.emitLocalGet(disc_eq_local);
    self.code_builder.code.append(self.allocator, Op.i32_eqz) catch return error.OutOfMemory; // disc_ne
    self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // break out of block
    ...
    if (variants.len > 0) {
        ...
        if (payload_size > 0) {
            ...
            for (0..variants.len) |variant_i| {
                ...
                self.code_builder.code.append(self.allocator, Op.block) catch return error.OutOfMemory;
                self.code_builder.code.append(self.allocator, 0x40) catch return error.OutOfMemory; // void block
                ...
                self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory; // end block
            }
    ...
    self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory; // end outer block
...
fn compareFieldByLayout(
    ...
    if (field_layout_idx == .str) {
        // String: compare the embedded RocStr fields at the struct offset.
        const import_idx = self.builtin_syms.str_equal;
        ...
        try self.emitLocalGetPlusOffset(lhs_local, field_offset);
        try self.emitLocalSet(lhs_field_local);
        try self.emitLocalGetPlusOffset(rhs_local, field_offset);
        try self.emitLocalSet(rhs_field_local);
        try self.emitPtrLenCapArgs(lhs_field_local);
        try self.emitPtrLenCapArgs(rhs_field_local);
        try self.emitCallBuiltin(import_idx);
        return;
    }
    ...
    switch (field_layout.tag) {
        .list => {
            // List: compare the embedded RocList fields at the struct offset.
            ...
            if (elem_layout == .str) {
                ...
                try self.emitPtrLenCapArgs(lhs_list_local);
                try self.emitPtrLenCapArgs(rhs_list_local);
                try self.emitCallBuiltin(import_idx);
                ...
                try self.emitPtrLenCapArgs(rhs_list_local);
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(inner_elem_size)) catch return error.OutOfMemory;
                try self.emitCallBuiltin(import_idx);
...
fn emitBytewiseEq(self: *Self, lhs_local: u32, rhs_local: u32, byte_size: u32) Allocator.Error!void {
    ...
    while (offset + 4 <= byte_size) : (offset += 4) {
        try self.emitLocalGet(lhs_local);
        self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
    ...
    if (offset + 2 <= byte_size) {
        try self.emitLocalGet(lhs_local);
        self.code_builder.code.append(self.allocator, Op.i32_load16_u) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 1) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
...
fn emitBytewiseEqAtOffset(self: *Self, lhs_local: u32, rhs_local: u32, base_offset: u32, byte_size: u32) Allocator.Error!void {
    ...
    while (offset + 4 <= byte_size) : (offset += 4) {
        ...
        self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_offset + offset) catch return error.OutOfMemory;
...
fn emitLoadBySize(self: *Self, disc_size: u8, offset: u16) Allocator.Error!void {
    switch (disc_size) {
        ...
        8 => {
            self.code_builder.code.append(self.allocator, Op.i64_load) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 3) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
            // Wrap to i32 since callers use i32 locals for discriminant values
            self.code_builder.code.append(self.allocator, Op.i32_wrap_i64) catch return error.OutOfMemory;
        },
        else => unreachable,
    }
}
        ...
        switch (op) {
            ...
            .num_times => {
                if (operand_layout == .dec) {
                    ...
                    try self.emitI128HostBinOp(lhs_local, rhs_local, import_idx, false);
                    return;
                }
            ...
            .num_div_by => {
                if (operand_layout == .dec) {
                    ...
                    try self.emitI128HostBinOp(lhs_local, rhs_local, import_idx, true);
                } else {
            ...
            .num_abs_diff => {
                ...
                self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
                self.code_builder.code.append(self.allocator, @intFromEnum(WasmModule.BlockType.i32)) catch return error.OutOfMemory;
...
fn allocStackResultPtr(self: *Self, size: u32, alignment: u32) Allocator.Error!u32 {
    const result_offset = try self.allocStackMemory(size, alignment);
    const result_local = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
...
fn emitCallBuiltin(self: *Self, func_idx: u32) Allocator.Error!void {
...
fn emitLocalGetPlusOffset(self: *Self, base_local: u32, offset: u32) Allocator.Error!void {
    try self.emitLocalGet(base_local);
    if (offset == 0) return;

    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(offset)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
}

fn emitLoadI32FromPtr(self: *Self, ptr_local: u32, offset: u32) Allocator.Error!void {
    try self.emitLocalGet(ptr_local);
    try self.emitLoadOp(.i32, offset);
}

fn emitLoadI64FromPtr(self: *Self, ptr_local: u32, offset: u32) Allocator.Error!void {
    try self.emitLocalGet(ptr_local);
    try self.emitLoadOp(.i64, offset);
}

/// Emit the raw `{ ptr/bytes, len, cap }` fields used by RocStr and RocList builtins.
fn emitPtrLenCapArgs(self: *Self, value_local: u32) Allocator.Error!void {
    try self.emitLoadI32FromPtr(value_local, 0);
    try self.emitLoadI32FromPtr(value_local, 4);
    try self.emitLoadI32FromPtr(value_local, 8);
}

/// Emit a wasm32 ABI i128/u128/Dec value as two i64 halves.
fn emitI128AbiArgs(self: *Self, value_local: u32) Allocator.Error!void {
    try self.emitLoadI64FromPtr(value_local, 0);
    try self.emitLoadI64FromPtr(value_local, 8);
}

/// Emit an i128/Dec builtin call using the merged wasm32 ABI.
/// Builtins take two output pointers (`out_low`, `out_high`), followed by the
/// decomposed lhs/rhs words, and optionally the trailing `roc_ops` pointer.
fn emitI128HostBinOp(self: *Self, lhs_local: u32, rhs_local: u32, import_idx: u32, needs_roc_ops: bool) Allocator.Error!void {
    const result_local = try self.allocStackResultPtr(16, 8);

    try self.emitLocalGet(result_local);
    try self.emitLocalGetPlusOffset(result_local, 8);
    try self.emitI128AbiArgs(lhs_local);
    try self.emitI128AbiArgs(rhs_local);
    if (needs_roc_ops) {
        try self.emitLocalGet(self.roc_ops_local);
    }
...
fn emitI128Add(self: *Self, lhs_local: u32, rhs_local: u32) Allocator.Error!void {
    ...
    try self.emitFpOffset(result_offset);
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_local) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, lhs_local) catch return error.OutOfMemory;
    try self.emitLoadOp(.i64, 0);
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a_low) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a_high) catch return error.OutOfMemory;

    const b_low = self.storage.allocAnonymousLocal(.i64) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, rhs_local) catch return error.OutOfMemory;
    try self.emitLoadOp(.i64, 0);
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_low) catch return error.OutOfMemory;
    ...
    try self.emitLoadOp(.i64, 8);
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_high) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_low) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_add) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_low) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a_low) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_lt_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_extend_i32_u) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_high) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_high) catch return error.OutOfMemory;
...
fn emitI128Sub(self: *Self, lhs_local: u32, rhs_local: u32) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_low) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_sub) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_low) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, borrow_local) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_high_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_high_local) catch return error.OutOfMemory;
...
fn generateI128Shift(self: *Self, op: anytype, args: []const LirExprId) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 64) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_ge_u) catch return error.OutOfMemory;

    self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, @intFromEnum(WasmModule.BlockType.void)) catch return error.OutOfMemory;
    ...
    switch (op) {
        .num_shift_left_by => {
            ...
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 64) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_sub) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_shl) catch return error.OutOfMemory;
        ...
        .num_shift_right_by => {
            ...
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 63) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_shr_s) catch return error.OutOfMemory;
            try self.emitLocalSet(r_high);
            try self.emitLocalGet(a_high);
            try self.emitLocalGet(shift_local);
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
        ...
        .num_shift_right_zf_by => {
            ...
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 64) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_sub) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_shr_u) catch return error.OutOfMemory;
            try self.emitLocalSet(r_low);
        },
        else => unreachable,
    }
    ...
    switch (op) {
        .num_shift_left_by => {
            ...
            self.code_builder.code.append(self.allocator, Op.i64_shr_u) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_or) catch return error.OutOfMemory;
            try self.emitLocalSet(r_high);
        },
        .num_shift_right_by => {
...
fn emitI128Mul(self: *Self, lhs_local: u32, rhs_local: u32) Allocator.Error!void {
    ...
    try self.emitLoadOp(.i64, 0);
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a_lo) catch return error.OutOfMemory;
    ...
    try self.emitLoadOp(.i64, 8);
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a_hi) catch return error.OutOfMemory;
    ...
    try self.emitLoadOp(.i64, 0);
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_lo) catch return error.OutOfMemory;
    ...
    try self.emitLoadOp(.i64, 8);
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_hi) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_lo) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_mul) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_lo_val) catch return error.OutOfMemory;
    ...
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a_lo) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 0xFFFFFFFF) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_and) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, al0) catch return error.OutOfMemory;

    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a_lo) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 32) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_shr_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, al1) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i64_and) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, bl0) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i64_shr_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, bl1) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i64_mul) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, t) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i64_add) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, cross) catch return error.OutOfMemory;
    ...
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, cross) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, old_cross) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i64_extend_i32_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, carry) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i64_add) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, hi_of_lo_mul) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_hi_val) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_hi_val) catch return error.OutOfMemory;
...
fn emitI128CompareWithSignedness(self: *Self, lhs_local: u32, rhs_local: u32, cmp_op: I128CmpOp, is_signed: bool) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_high) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_eq) catch return error.OutOfMemory;
    // if (result is i32)
    self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, @intFromEnum(WasmModule.BlockType.i32)) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, low_cmp) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, high_cmp) catch return error.OutOfMemory;
...
fn generateCompositeI128Negate(self: *Self, expr: LirExprId, _: layout.Idx) Allocator.Error!void {
    ...
    const src_local = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, src_local) catch return error.OutOfMemory;
...
fn emitCompositeI128NegateFromLocal(self: *Self, src_local: u32) Allocator.Error!void {
    ...
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, src_local) catch return error.OutOfMemory;
    try self.emitLoadOp(.i64, 0);
    self.code_builder.code.append(self.allocator, Op.i64_xor) catch return error.OutOfMemory;
    // Stack: ~a_low
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 1) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_add) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_low_local) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_low_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_eqz) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_extend_i32_u) catch return error.OutOfMemory;
...
fn generateCompositeI128Abs(self: *Self, expr: LirExprId) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_lt_s) catch return error.OutOfMemory;
...
fn emitI64MulToI128(self: *Self, a_local: u32, b_local: u32) Allocator.Error!void {
    ...
    // a0 = a & 0xFFFFFFFF
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 0xFFFFFFFF) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_and) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a0) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i64_shr_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a1) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 0xFFFFFFFF) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_and) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b0) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 32) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_shr_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b1) catch return error.OutOfMemory;
...
fn emitIntToI128(self: *Self, signed: bool) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val_local) catch return error.OutOfMemory;
...
fn emitF64ToI128(self: *Self, val_local: u32, result_local: u32, signed: bool) Allocator.Error!void {
    ...
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
    self.code_builder.code.appendSlice(self.allocator, &@as([8]u8, @bitCast(@as(f64, 18446744073709551616.0)))) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_div) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_trunc) catch return error.OutOfMemory;
    const high_f = self.storage.allocAnonymousLocal(.f64) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_tee) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, high_f) catch return error.OutOfMemory;
    if (signed) {
        self.code_builder.code.append(self.allocator, Op.i64_trunc_f64_s) catch return error.OutOfMemory;
    } else {
        self.code_builder.code.append(self.allocator, Op.i64_trunc_f64_u) catch return error.OutOfMemory;
    }
    const high_i = self.storage.allocAnonymousLocal(.i64) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, high_i) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
    self.code_builder.code.appendSlice(self.allocator, &@as([8]u8, @bitCast(@as(f64, 18446744073709551616.0)))) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_mul) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_sub) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_trunc_f64_u) catch return error.OutOfMemory;
    const low_i = self.storage.allocAnonymousLocal(.i64) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, low_i) catch return error.OutOfMemory;
...
fn emitFloatToIntTryUnsafe(self: *Self, val_size: u32, is_i64: bool, min_f: f64, max_f: f64) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_abs) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
    self.code_builder.code.appendSlice(self.allocator, &@as([8]u8, @bitCast(std.math.inf(f64)))) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_ne) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
    // trunc(val) == val
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, is_int) catch return error.OutOfMemory;

    // Compute in_range: val >= min_f && val <= max_f
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
    self.code_builder.code.appendSlice(self.allocator, &@as([8]u8, @bitCast(min_f))) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_ge) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
    self.code_builder.code.appendSlice(self.allocator, &@as([8]u8, @bitCast(max_f))) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.f64_le) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
    const in_range = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, in_range) catch return error.OutOfMemory;
    ...
    if (is_i64) {
        ...
        self.code_builder.code.append(self.allocator, Op.i32_trunc_f64_s) catch return error.OutOfMemory;
        try self.emitStoreOp(.i32, 0);
    }
...
fn emitFloatToI128TryUnsafe(self: *Self, signed: bool) Allocator.Error!void {
    ...
    if (signed) {
        ...
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
        self.code_builder.code.appendSlice(self.allocator, &@as([8]u8, @bitCast(@as(f64, 0.0)))) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.f64_ge) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, val) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
...
fn emitI128TryNarrow(
    ...
    const src_ptr = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, src_ptr) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, low) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, high) catch return error.OutOfMemory;
    ...
    if (!signed_target) {
        ...
        if (target_bytes < 8) {
            ...
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, max_val) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_le_u) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
        }
        ...
        WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, max_signed) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i64_le_u) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
    } else {
        ...
        if (target_bytes < 8) {
            ...
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, @intCast(64 - bit_count)) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_shl) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, @intCast(64 - bit_count)) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_shr_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, sign_ext_low) catch return error.OutOfMemory;
...
fn emitI128TryToU128(self: *Self, _: bool) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i64_ge_s) catch return error.OutOfMemory;
...
fn emitHeapAlloc(self: *Self, size_local: u32, alignment: u32) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.call_indirect) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, self.roc_ops_type_idx) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // table index 0

    // Read answer from struct (offset +8) → result pointer on stack
    try self.emitFpOffset(alloc_slot);
    self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 8) catch return error.OutOfMemory;
}
...
fn emitHeapAllocConst(self: *Self, size: u32, alignment: u32) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(size)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, size_local) catch return error.OutOfMemory;
...
fn emitFpOffset(self: *Self, offset: u32) Allocator.Error!void {
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, self.fp_local) catch return error.OutOfMemory;
    if (offset > 0) {
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(offset)) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
...
fn emitI64Store(self: *Self, offset: u32) Allocator.Error!void {
    self.code_builder.code.append(self.allocator, Op.i64_store) catch return error.OutOfMemory;
...
fn emitConversion(self: *Self, source: ValType, target: ValType) Allocator.Error!void {
    ...
    if (op) |opcode| {
        self.code_builder.code.append(self.allocator, opcode) catch return error.OutOfMemory;
    }
}
...
fn compileProcSpecBody(self: *Self, proc: LirProcSpec) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.block) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, @intFromEnum(ret_vt)) catch return error.OutOfMemory;
...
const SavedState = struct {
    ...
    code_capacity: usize,
    preamble_items: []u8,
    preamble_capacity: usize,
    import_relocs_items: []CodeBuilder.Relocation,
    import_relocs_capacity: usize,
...
fn saveState(self: *Self) Allocator.Error!SavedState {
    return .{
        .code_items = self.code_builder.code.items,
        .code_capacity = self.code_builder.code.capacity,
        .preamble_items = self.code_builder.preamble.items,
        .preamble_capacity = self.code_builder.preamble.capacity,
        .import_relocs_items = self.code_builder.import_relocations.items,
        .import_relocs_capacity = self.code_builder.import_relocations.capacity,
...
fn restoreState(self: *Self, saved: SavedState) void {
    self.code_builder.code.deinit(self.allocator);
    self.code_builder.code.items = saved.code_items;
    self.code_builder.code.capacity = saved.code_capacity;
    self.code_builder.preamble.deinit(self.allocator);
    self.code_builder.preamble.items = saved.preamble_items;
    self.code_builder.preamble.capacity = saved.preamble_capacity;
    self.code_builder.import_relocations.deinit(self.allocator);
    self.code_builder.import_relocations.items = saved.import_relocs_items;
    self.code_builder.import_relocations.capacity = saved.import_relocs_capacity;
...
fn generateCFStmt(self: *Self, stmt_id: CFStmtId) Allocator.Error!void {
    ...
    switch (stmt) {
        ...
        .ret => |r| {
            ...
            self.code_builder.code.append(self.allocator, Op.br) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, self.cf_depth - 1) catch return error.OutOfMemory;
        },
        .expr_stmt => |es| {
        ...
        .switch_stmt => |sw| {
            ...
            for (branches) |branch| {
                ...
                if (cond_vt == .i64) {
                    self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, @bitCast(branch.value)) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i64_eq) catch return error.OutOfMemory;
                } else {
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(@as(i64, @bitCast(branch.value)))) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_eq) catch return error.OutOfMemory;
                }
        ...
        .jump => |jmp| {
            ...
            if (self.join_point_depths.get(jp_key)) |loop_depth| {
                ...
                self.code_builder.code.append(self.allocator, Op.br) catch return error.OutOfMemory;
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, br_target) catch return error.OutOfMemory;
            }
...
fn generateCFMatchBranches(self: *Self, branches: []const LIR.CFMatchBranch, value_local: u32, value_vt: ValType) Allocator.Error!void {
    ...
    switch (pattern) {
        ...
        .int_literal => |int_pat| {
            ...
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, 0x40) catch return error.OutOfMemory; // void
            self.cf_depth += 1;
            try self.generateCFStmtWithGuard(branch, remaining, value_local, value_vt);
            self.code_builder.code.append(self.allocator, Op.@"else") catch return error.OutOfMemory;
...
fn emitCall(self: *Self, func_idx: u32) Allocator.Error!void {
    try self.code_builder.code.append(self.allocator, Op.call);
    try WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, func_idx);
}
...
fn emitStoreOp(self: *Self, vt: ValType, mem_offset: u32) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, op) catch return error.OutOfMemory;
    // Alignment (log2): i32=2, i64=3, f32=2, f64=3
    const align_log2: u32 = switch (vt) {
        .i32, .f32 => 2,
        .i64, .f64 => 3,
    };
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, align_log2) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, mem_offset) catch return error.OutOfMemory;
}
...
fn emitLoadOpSized(self: *Self, vt: ValType, byte_size: u32, mem_offset: u32) Allocator.Error!void {
    if (byte_size == 0) {
        ...
        switch (vt) {
            ...
            .f32 => {
                self.code_builder.code.append(self.allocator, Op.f32_const) catch return error.OutOfMemory;
                try self.code_builder.code.appendSlice(self.allocator, std.mem.asBytes(&@as(f32, 0)));
            },
            .f64 => {
                self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
                try self.code_builder.code.appendSlice(self.allocator, std.mem.asBytes(&@as(f64, 0)));
            },
        }
        return;
    }
...
fn emitStoreToMem(self: *Self, base_local: u32, field_offset: u32, vt: ValType) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, temp) catch return error.OutOfMemory;
    // local.get base
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_local) catch return error.OutOfMemory;
...
fn emitMemCopy(self: *Self, dst_local: u32, dst_offset: u32, src_local: u32, byte_count: u32) Allocator.Error!void {
    ...
    while (offset + 4 <= byte_count) : (offset += 4) {
        // dst_local
        self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, dst_local) catch return error.OutOfMemory;
        // load from src
        self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, src_local) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
        ...
        self.code_builder.code.append(self.allocator, Op.i32_store) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // align
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, dst_offset + offset) catch return error.OutOfMemory;
    }
    ...
    if (offset + 2 <= byte_count) {
        ...
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_store16) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, dst_offset + offset) catch return error.OutOfMemory;
        offset += 2;
    }
...
fn generateStruct(self: *Self, r: anytype) Allocator.Error!void {
    ...
    for (fields, 0..) |_, i| {
        ...
        if (is_composite and field_byte_size > 0) {
            ...
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, field_val_locals[i]) catch return error.OutOfMemory;
        ...
        switch (pat) {
            .bind => |bind| {
                ...
                if (is_composite and field_byte_size > 0) {
                    ...
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, ptr_local) catch return error.OutOfMemory;
                    if (field_offset > 0) {
                        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(field_offset)) catch return error.OutOfMemory;
                        self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
...
fn generateZeroArgTag(self: *Self, z: anytype) Allocator.Error!void {
    ...
    if (l.tag == .tag_union) {
        ...
        if (tu_size <= 4) {
            ...
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(z.discriminant)) catch return error.OutOfMemory;
            return;
        }
...
fn generateTag(self: *Self, t: anytype) Allocator.Error!void {
    ...
    if (tu_size <= 4 and disc_offset == 0) {
        ...
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(t.discriminant)) catch return error.OutOfMemory;
        return;
    }
...
fn generateDiscriminantSwitch(self: *Self, ds: anytype) Allocator.Error!void {
    ...
    if (union_layout.tag == .tag_union) {
        ...
        if (tu_size <= 4) {
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, disc_local) catch return error.OutOfMemory;
        } else {
...
fn generateDiscSwitchBranches(self: *Self, branches: []const LirExprId, disc_local: u32, bt: BlockType, disc_value: u32) Allocator.Error!void {
    ...
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, disc_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(disc_value)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_eq) catch return error.OutOfMemory;
...
fn generateWhileLoop(self: *Self, wl: anytype) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_eqz) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 1) catch return error.OutOfMemory; // break out of block (depth 1)
    ...
    self.code_builder.code.append(self.allocator, Op.br) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // continue loop (depth 0)
...
fn generateForLoopExpr(self: *Self, fl: anytype) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, list_ptr) catch return error.OutOfMemory;
    ...
    const elems_ptr = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, elems_ptr) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, list_len) catch return error.OutOfMemory;
    ...
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, idx_local) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i32_ge_u) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 1) catch return error.OutOfMemory; // break out of block
    ...
    switch (elem_pattern) {
        ...
        .struct_ => |s| {
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, elem_ptr) catch return error.OutOfMemory;
...
fn generateList(self: *Self, l: anytype) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, list_base) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(elems.len)) catch return error.OutOfMemory;
...
fn generateLowLevel(self: *Self, ll: anytype) Allocator.Error!void {
    ...
    switch (ll.op) {
        ...
        .i8_to_i16, .i8_to_i32 => {
            ...
            self.code_builder.code.append(self.allocator, Op.i32_extend8_s) catch return error.OutOfMemory;
        },
        ...
        .i16_to_i32 => {
            ...
            self.code_builder.code.append(self.allocator, Op.i32_extend16_s) catch return error.OutOfMemory;
        },
        ...
        .i8_to_i64, .i16_to_i64, .i32_to_i64 => {
            ...
            self.code_builder.code.append(self.allocator, Op.i64_extend_i32_s) catch return error.OutOfMemory;
        },
        ...
        => {
            ...
            // Mask to 8 bits
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0xFF) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
        },
        .i32_to_i16_wrap,
        .u32_to_u16_wrap,
        ...
        => {
            ...
            // Mask to 16 bits
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0xFFFF) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
        },
        .i32_to_u32_wrap,
        .u32_to_i32_wrap,
        ...
        .i32_to_u64_try => {
            ...
            // Check: val >= 0
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_ge_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
            // Ok path: sign-extend i32 to i64, store payload
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.result_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_extend_i32_s) catch return error.OutOfMemory;
            try self.emitStoreOp(.i64, 0);
        ...
        .f32_to_f64 => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
        },
        .f64_to_f32_wrap => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.f32_demote_f64) catch return error.OutOfMemory;
        },
        ...
        .i32_to_f32, .i8_to_f32, .i16_to_f32 => {
            ...
            self.code_builder.code.append(self.allocator, Op.f32_convert_i32_s) catch return error.OutOfMemory;
        },
        .u32_to_f32, .u8_to_f32, .u16_to_f32 => {
            ...
            self.code_builder.code.append(self.allocator, Op.f32_convert_i32_u) catch return error.OutOfMemory;
        },
        .i32_to_f64, .i8_to_f64, .i16_to_f64 => {
            ...
            self.code_builder.code.append(self.allocator, Op.f64_convert_i32_s) catch return error.OutOfMemory;
        },
        .u32_to_f64, .u8_to_f64, .u16_to_f64 => {
            ...
            self.code_builder.code.append(self.allocator, Op.f64_convert_i32_u) catch return error.OutOfMemory;
        },
        .i64_to_f32 => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.f32_convert_i64_s) catch return error.OutOfMemory;
        },
        .u64_to_f32 => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.f32_convert_i64_u) catch return error.OutOfMemory;
        },
        .i64_to_f64 => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.f64_convert_i64_s) catch return error.OutOfMemory;
        },
        .u64_to_f64 => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.f64_convert_i64_u) catch return error.OutOfMemory;
        },

        // Float to int (truncating)
        .f32_to_i32_trunc, .f32_to_i8_trunc, .f32_to_i16_trunc => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.i32_trunc_f32_s) catch return error.OutOfMemory;
        },
        .f32_to_u32_trunc, .f32_to_u8_trunc, .f32_to_u16_trunc => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.i32_trunc_f32_u) catch return error.OutOfMemory;
        },
        ...
        .f64_to_u32_trunc, .f64_to_u8_trunc, .f64_to_u16_trunc => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.i32_trunc_f64_u) catch return error.OutOfMemory;
        },
        .f32_to_i64_trunc => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.i64_trunc_f32_s) catch return error.OutOfMemory;
        },
        .f32_to_u64_trunc => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.i64_trunc_f32_u) catch return error.OutOfMemory;
        },
        .f64_to_i64_trunc => {
            try self.generateExpr(args[0]);
            self.code_builder.code.append(self.allocator, Op.i64_trunc_f64_s) catch return error.OutOfMemory;
        ...
        .num_sqrt => {
            ...
            self.code_builder.code.append(self.allocator, wasm_op) catch return error.OutOfMemory;
        },
        .num_floor => {
            try self.generateExpr(args[0]);
            const vt = self.resolveValType(ll.ret_layout);
        ...
        .list_get_unsafe => {
            ...
            switch (index_expr) {
                .dec_literal => |v| {
                    ...
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, actual) catch return error.OutOfMemory;
                },
                .i64_literal => |v| {
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(v.value)) catch return error.OutOfMemory;
                },
                else => {
                    try self.generateExpr(args[1]);
        ...
        .list_last => {
            ...
            const list_local = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, list_local) catch return error.OutOfMemory;
            ...
            const ret_byte_size = self.layoutStorageByteSize(ll.ret_layout);
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(ret_byte_size)) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_mul) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
        ...
        .list_drop_first => {
            ...
            const count_local = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, count_local) catch return error.OutOfMemory;
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, encoded_cap) catch return error.OutOfMemory;
        ...
        .list_take_first => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, list_local) catch return error.OutOfMemory;
            try self.emitLoadOp(.i32, 4);
            self.code_builder.code.append(self.allocator, Op.i32_le_u) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.select) catch return error.OutOfMemory;
            try self.emitStoreOp(.i32, 4);
        ...
        .list_take_last => {
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, len_local) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, actual_count) catch return error.OutOfMemory;
        ...
        .list_contains => blk: {
            ...
            if (elem_is_composite) {
                ...
                self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, needle_ptr_local) catch return error.OutOfMemory;
            } else {
                try self.generateExpr(args[1]);
                ...
                self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, needle_tmp) catch return error.OutOfMemory;
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, idx_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(elem_byte_size)) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_mul) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
            const elem_ptr_local = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, elem_ptr_local) catch return error.OutOfMemory;
        ...
        .list_sublist => {
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, rec_local) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, sub_len) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, start_local) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, old_len) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, actual_start) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, remaining) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, actual_len) catch return error.OutOfMemory;
        ...
        .str_count_utf8_bytes => {
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, str_local) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_tee) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, tag_byte) catch return error.OutOfMemory;

            // Check if SSO: high bit set
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0x80) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, tag_byte) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0x7F) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"else") catch return error.OutOfMemory;
            // Heap path
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
        ...
        .str_is_eq => {
            // String equality via merged builtin (handles both SSO and heap strings)
            const import_idx = self.builtin_syms.str_equal;
            try self.generateExpr(args[0]);
            ...
            try self.emitPtrLenCapArgs(a);
            try self.emitPtrLenCapArgs(b);
            try self.emitCallBuiltin(import_idx);
        },
        .str_concat => {
        ...
        => {
            ...
            const result_local = try self.allocStackResultPtr(12, 4);
            try self.emitLocalGet(result_local);
        ...
        .str_split_on => {
            const import_idx = self.builtin_syms.str_split;
            try self.generateExpr(args[0]);
            const a = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            try self.emitLocalSet(a);
            try self.generateExpr(args[1]);
        ...
        .str_join_with => {
            const import_idx = self.builtin_syms.str_join_with;
            try self.generateExpr(args[0]);
            const list_local = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            try self.emitLocalSet(list_local);
            try self.generateExpr(args[1]);
            const sep_local = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            try self.emitLocalSet(sep_local);
            const result_local = try self.allocStackResultPtr(12, 4);
            try self.emitLocalGet(result_local);
            try self.emitPtrLenCapArgs(list_local);
            try self.emitPtrLenCapArgs(sep_local);
            try self.emitLocalGet(self.roc_ops_local);
            try self.emitCallBuiltin(import_idx);
            try self.emitLocalGet(result_local);
        },
        .str_repeat, .str_reserve => {
            ...
            const int_local = self.storage.allocAnonymousLocal(.i64) catch return error.OutOfMemory;
            try self.emitLocalSet(int_local);
            const result_local = try self.allocStackResultPtr(12, 4);
            try self.emitLocalGet(result_local);
            try self.emitPtrLenCapArgs(str_local);
        ...
        .num_from_str => {
            ...
            if (ok_payload == .dec) {
                ...
                try self.emitPtrLenCapArgs(input);
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(disc_offset)) catch return error.OutOfMemory;
                try self.emitCallBuiltin(import_idx);
                ...
                try self.emitPtrLenCapArgs(input);
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, float_width) catch return error.OutOfMemory;
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(disc_offset)) catch return error.OutOfMemory;
                try self.emitCallBuiltin(import_idx);
            } else {
                ...
                try self.emitPtrLenCapArgs(input);
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, int_width) catch return error.OutOfMemory;
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, is_signed) catch return error.OutOfMemory;
                self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(disc_offset)) catch return error.OutOfMemory;
                try self.emitCallBuiltin(import_idx);
            }
            ...
            if (value_vt == .i32 and value_size > 4) {
                ...
                if (value_size <= 16) {
                    ...
                    while (offset + 4 <= value_size) : (offset += 4) {
                        ...
                        try self.emitLocalGet(src_ptr);
                        self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
                        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory; // align
                        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
                        self.code_builder.code.append(self.allocator, Op.i32_store) catch return error.OutOfMemory;
                        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory;
                        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, offset) catch return error.OutOfMemory;
                    ...
                    try self.emitLocalGet(i);
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(value_size)) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_lt_u) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        ...
        .compare => {
            ...
            switch (arg_vt) {
                .i32 => {
                    ...
                    const b = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a) catch return error.OutOfMemory;
                    // gt_flag
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, if (is_unsigned) Op.i32_gt_u else Op.i32_gt_s) catch return error.OutOfMemory;
                    // lt_flag * 2
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, if (is_unsigned) Op.i32_lt_u else Op.i32_lt_s) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_mul) catch return error.OutOfMemory;
                ...
                .i64 => {
                    ...
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, if (is_unsigned) Op.i64_gt_u else Op.i64_gt_s) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, if (is_unsigned) Op.i64_lt_u else Op.i64_lt_s) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_mul) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
                ...
                .f32 => {
                    ...
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.f32_gt) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.f32_lt) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_mul) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
                ...
                .f64 => {
                    ...
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.f64_gt) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, a) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, b) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.f64_lt) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_mul) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
        ...
        .i32_to_i8_try, .i16_to_i8_try, .u16_to_i8_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, -128) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_ge_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 127) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i32_to_u8_try, .i16_to_u8_try, .u16_to_u8_try, .i8_to_u8_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 255) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i32_to_i16_try, .u32_to_i16_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, -32768) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_ge_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 32767) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i32_to_u16_try, .u32_to_u16_try, .i16_to_u16_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 65535) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i64_to_i8_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, -128) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_ge_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 127) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i64_to_i16_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, -32768) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_ge_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 32767) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i64_to_i32_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, -2147483648) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_ge_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 2147483647) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i64_to_u8_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 255) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i64_to_u16_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 65535) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i64_to_u32_try => {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, r.val_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 4294967295) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        ...
        .i128_to_f64 => {
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, src) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.f64_convert_i64_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.f64_const) catch return error.OutOfMemory;
            self.code_builder.code.appendSlice(self.allocator, &@as([8]u8, @bitCast(@as(f64, 18446744073709551616.0)))) catch return error.OutOfMemory; // 2^64
            self.code_builder.code.append(self.allocator, Op.f64_mul) catch return error.OutOfMemory;
            // low_f64 = (f64)(u64)low
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, src) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.f64_add) catch return error.OutOfMemory;
        },
        .u128_to_f64 => {
        ...
        .u128_to_dec_try_unsafe, .i128_to_dec_try_unsafe => {
            ...
            const result_local = try self.allocStackResultPtr(
                self.layoutByteSize(ll.ret_layout),
                self.layoutByteAlign(ll.ret_layout),
            );
            try self.emitLocalGet(result_local);
            try self.emitI128AbiArgs(val_ptr);
            try self.emitCallBuiltin(import_idx);
            try self.emitLocalGet(result_local);
        },
        ...
        .u8_to_dec, .u16_to_dec, .u32_to_dec => {
            ...
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 1_000_000_000_000_000_000) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, dec_factor) catch return error.OutOfMemory;
        ...
        .dec_to_i64_trunc => {
            ...
            try self.emitI128HostBinOp(dec_local, divisor_local, self.builtin_syms.i128_div_s, true);
            ...
            switch (ll.op) {
                ...
                .dec_to_i16_trunc, .dec_to_i8_trunc, .dec_to_u16_trunc, .dec_to_u8_trunc => {
                    ...
                    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
                    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, mask) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
                },
                else => unreachable,
            }
        ...
        .dec_to_f64 => {
            ...
            self.code_builder.code.appendSlice(self.allocator, &dec_f64_bytes) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.f64_div) catch return error.OutOfMemory;
        },
        .dec_to_f32_wrap => {
        ...
        => {
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, dec_low) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 1_000_000_000_000_000_000) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_rem_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_eqz) catch return error.OutOfMemory;
            const is_int = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, is_int) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 1_000_000_000_000_000_000) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_div_s) catch return error.OutOfMemory;
            const int_val = self.storage.allocAnonymousLocal(.i64) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, int_val) catch return error.OutOfMemory;
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, int_val) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, info.min_i) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_ge_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, int_val) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, info.max_i) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i64_le_s) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_and) catch return error.OutOfMemory;
            const in_range = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
        ...
        .dec_to_i128_try_unsafe, .dec_to_u128_try_unsafe => {
            ...
            const is_signed: i32 = @intFromBool(ll.op == .dec_to_i128_try_unsafe);
            ...
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, is_signed) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 16) catch return error.OutOfMemory;
            try self.emitCallBuiltin(import_idx);
            try self.emitLocalGet(result_local);
        },
        ...
        .f32_to_i8_try_unsafe, .f64_to_i8_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_i8_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
            try self.emitFloatToIntTryUnsafe(1, false, -128.0, 127.0);
        },
        .f32_to_u8_try_unsafe, .f64_to_u8_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_u8_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
            try self.emitFloatToIntTryUnsafe(1, false, 0.0, 255.0);
        },
        .f32_to_i16_try_unsafe, .f64_to_i16_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_i16_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
            try self.emitFloatToIntTryUnsafe(2, false, -32768.0, 32767.0);
        },
        .f32_to_u16_try_unsafe, .f64_to_u16_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_u16_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
            try self.emitFloatToIntTryUnsafe(2, false, 0.0, 65535.0);
        },
        .f32_to_i32_try_unsafe, .f64_to_i32_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_i32_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
            try self.emitFloatToIntTryUnsafe(4, false, -2147483648.0, 2147483647.0);
        },
        .f32_to_u32_try_unsafe, .f64_to_u32_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_u32_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
            try self.emitFloatToIntTryUnsafe(4, false, 0.0, 4294967295.0);
        },
        .f32_to_i64_try_unsafe, .f64_to_i64_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_i64_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
        ...
        .f32_to_u64_try_unsafe, .f64_to_u64_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_u64_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
        ...
        .f32_to_i128_try_unsafe, .f64_to_i128_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_i128_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
            try self.emitFloatToI128TryUnsafe(true);
        },
        .f32_to_u128_try_unsafe, .f64_to_u128_try_unsafe => {
            try self.generateExpr(args[0]);
            if (ll.op == .f32_to_u128_try_unsafe) self.code_builder.code.append(self.allocator, Op.f64_promote_f32) catch return error.OutOfMemory;
        ...
        .f64_to_f32_try_unsafe => {
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, f32_val) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, f32_val) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.f32_abs) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.f32_const) catch return error.OutOfMemory;
            self.code_builder.code.appendSlice(self.allocator, &@as([4]u8, @bitCast(std.math.inf(f32)))) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.f32_ne) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, success) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, result_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, success) catch return error.OutOfMemory;
...
fn generateNumericLowLevel(self: *Self, op: anytype, args: []const LirExprId, ret_layout: layout.Idx) Allocator.Error!void {
    ...
    switch (op) {
        ...
        .num_rem_by => {
            ...
            const is_unsigned = isUnsignedLayout(ret_layout);
            switch (vt) {
                .i32 => self.code_builder.code.append(self.allocator, if (is_unsigned) Op.i32_rem_u else Op.i32_rem_s) catch return error.OutOfMemory,
                .i64 => self.code_builder.code.append(self.allocator, if (is_unsigned) Op.i64_rem_u else Op.i64_rem_s) catch return error.OutOfMemory,
        ...
        .num_negate => {
            switch (vt) {
                ...
                .f32 => {
                    try self.generateExpr(args[0]);
                    self.code_builder.code.append(self.allocator, Op.f32_neg) catch return error.OutOfMemory;
                },
                .f64 => {
                    try self.generateExpr(args[0]);
                    self.code_builder.code.append(self.allocator, Op.f64_neg) catch return error.OutOfMemory;
                },
            }
        },
        .num_is_eq => {
        ...
        .num_abs_diff => {
            ...
            switch (vt) {
                .i32 => {
                    ...
                    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, lhs) catch return error.OutOfMemory;
                    try self.generateExpr(args[1]);
                    const rhs = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, rhs) catch return error.OutOfMemory;

                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, lhs) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, rhs) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, if (is_unsigned) Op.i32_ge_u else Op.i32_ge_s) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, @intFromEnum(WasmModule.BlockType.i32)) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, lhs) catch return error.OutOfMemory;
                ...
                .i64 => {
                    ...
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, rhs) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, if (is_unsigned) Op.i64_ge_u else Op.i64_ge_s) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.@"if") catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, @intFromEnum(WasmModule.BlockType.i64)) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, lhs) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
                    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, rhs) catch return error.OutOfMemory;
                ...
                .f32 => {
                    try self.generateExpr(args[0]);
                    try self.generateExpr(args[1]);
                    self.code_builder.code.append(self.allocator, Op.f32_sub) catch return error.OutOfMemory;
                    self.code_builder.code.append(self.allocator, Op.f32_abs) catch return error.OutOfMemory;
                },
                .f64 => {
                    try self.generateExpr(args[0]);
        ...
        .num_shift_left_by => {
            ...
            if (vt == .i64) self.code_builder.code.append(self.allocator, Op.i64_extend_i32_u) catch return error.OutOfMemory;
            const wasm_op: u8 = switch (vt) {
                .i32 => Op.i32_shl,
                .i64 => Op.i64_shl,
                .f32, .f64 => unreachable,
...
fn generateStrEq(self: *Self, lhs: LirExprId, rhs: LirExprId, negate: bool) Allocator.Error!void {
    ...
    try self.emitPtrLenCapArgs(lhs_local);
    try self.emitPtrLenCapArgs(rhs_local);
    try self.emitCallBuiltin(import_idx);
...
fn generateStrLiteral(self: *Self, str_idx: anytype) Allocator.Error!void {
    ...
    if (len <= 11) {
        ...
        for (str_bytes, 0..) |byte, i| {
            ...
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_local) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
            WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(byte)) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.i32_store8) catch return error.OutOfMemory;
            // alignment = 0 (byte-aligned)
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
            // offset
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_offset + @as(u32, @intCast(i))) catch return error.OutOfMemory;
        }
        ...
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_local) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @as(i32, @intCast(len)) | @as(i32, 0x80)) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_store8) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // align
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_offset + 11) catch return error.OutOfMemory; // offset
    } else {
        ...
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, base_local) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(len)) catch return error.OutOfMemory;
...
fn generateStrConcat(self: *Self, span: anytype) Allocator.Error!void {
    ...
    for (expr_ids[1..]) |expr_id| {
        ...
        const result_local = try self.allocStackResultPtr(12, 4);
        try self.emitLocalGet(result_local);
        try self.emitPtrLenCapArgs(current);
        try self.emitPtrLenCapArgs(rhs);
        try self.emitLocalGet(self.roc_ops_local);
        try self.emitCallBuiltin(import_idx);
        try self.emitLocalGet(result_local);
        try self.emitLocalSet(current);
...
fn emitExtractStrPtrLen(self: *Self, str_local: u32, ptr_local: u32, len_local: u32) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_load8_u) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // align
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 11) catch return error.OutOfMemory; // offset = 11

    const sso_marker = self.storage.allocAnonymousLocal(.i32) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_tee) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, sso_marker) catch return error.OutOfMemory;
    ...
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, str_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory; // align = 2
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 4) catch return error.OutOfMemory; // offset = 4
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, len_local) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory; // align = 2
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // offset = 0
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, ptr_local) catch return error.OutOfMemory;
...
fn emitMemCopyLoop(self: *Self, dst_base_local: u32, dst_offset_local: u32, src_ptr_local: u32, len_local: u32) Allocator.Error!void {
    ...
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_set) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, loop_i) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, dst_base_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, dst_offset_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, loop_i) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, src_ptr_local) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, loop_i) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_load8_u) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // align = 0
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // offset = 0
    ...
    // br back to loop
    self.code_builder.code.append(self.allocator, Op.br) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // continue loop
...
fn generateStrToUtf8(self: *Self, str_arg: LirExprId) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_load8_u) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // align
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 11) catch return error.OutOfMemory; // offset
...
fn generateStrFromUtf8Lossy(self: *Self, list_arg: LirExprId) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory; // align = 4-byte
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 4) catch return error.OutOfMemory; // offset
    ...
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 12) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_lt_u) catch return error.OutOfMemory;
    ...
    self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
    {
        ...
        self.code_builder.code.append(self.allocator, Op.i32_load) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 2) catch return error.OutOfMemory; // align
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory; // offset
...
fn generateIntToStr(self: *Self, its: anytype) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 20) catch return error.OutOfMemory;
    ...
    if (is_64bit) {
        self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 10) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i64_rem_u) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_wrap_i64) catch return error.OutOfMemory;
    } else {
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 10) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_rem_u) catch return error.OutOfMemory;
    }
    ...
    if (is_64bit) {
        self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 10) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i64_div_u) catch return error.OutOfMemory;
    } else {
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 10) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_div_u) catch return error.OutOfMemory;
    }
    try self.emitLocalSet(value_local);
    ...
    if (is_64bit) {
        self.code_builder.code.append(self.allocator, Op.i64_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI64(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i64_gt_u) catch return error.OutOfMemory;
    } else {
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_gt_u) catch return error.OutOfMemory;
    }
    self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
    ...
    if (is_signed) {
        ...
        self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
        WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, '-') catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i32_store8) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
        WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0) catch return error.OutOfMemory;
...
///
/// wasm32 ABI: roc_builtins_float_to_str returns a RocStr via sret convention:
///   (result_ptr: i32, val_bits: i64, is_f32: i32, roc_ops: i32) -> void
/// The result_ptr receives the 12-byte RocStr.
fn generateFloatToStr(self: *Self, fts: anytype) Allocator.Error!void {
    ...
    if (is_f32) {
        self.code_builder.code.append(self.allocator, Op.i32_reinterpret_f32) catch return error.OutOfMemory;
        self.code_builder.code.append(self.allocator, Op.i64_extend_i32_u) catch return error.OutOfMemory;
    } else {
        self.code_builder.code.append(self.allocator, Op.i64_reinterpret_f64) catch return error.OutOfMemory;
    }

    const val_bits = self.storage.allocAnonymousLocal(.i64) catch return error.OutOfMemory;
    try self.emitLocalSet(val_bits);

    const result_ptr = try self.allocStackResultPtr(12, 4);
    try self.emitLocalGet(result_ptr);
    try self.emitLocalGet(val_bits);
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intFromBool(is_f32)) catch return error.OutOfMemory;
    try self.emitLocalGet(self.roc_ops_local);
    try self.emitCallBuiltin(import_idx);

    // Leave result_ptr (pointer to 12-byte RocStr) on the wasm stack.
    try self.emitLocalGet(result_ptr);
}
...
fn generateI128ToStr(self: *Self, value_expr: anytype, is_signed: bool) Allocator.Error!void {
    const import_idx = self.builtin_syms.int_to_str;
    ...
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, 16) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intFromBool(is_signed)) catch return error.OutOfMemory;
    try self.emitLocalGet(self.roc_ops_local);
    try self.emitCallBuiltin(import_idx);
    try self.emitLocalGet(result_ptr);
}
...
/// Generate dec_to_str: convert a Dec (i128) value to its decimal string representation.
///
/// wasm32 ABI: roc_builtins_dec_to_str uses sret + i128 split:
///   (result_ptr: i32, dec_lo: i64, dec_hi: i64, roc_ops: i32) -> void
/// The result_ptr receives the 12-byte RocStr.
fn generateDecToStr(self: *Self, dec_expr: anytype) Allocator.Error!void {
    ...
    // Load the two i64 halves of the Dec value from memory
    const dec_lo = self.storage.allocAnonymousLocal(.i64) catch return error.OutOfMemory;
    const dec_hi = self.storage.allocAnonymousLocal(.i64) catch return error.OutOfMemory;
    try self.emitLocalGet(dec_ptr);
    self.code_builder.code.append(self.allocator, Op.i64_load) catch return error.OutOfMemory;
    // alignment=3 (8 bytes), offset=0
    self.code_builder.code.append(self.allocator, 0x03) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, 0x00) catch return error.OutOfMemory;
    try self.emitLocalSet(dec_lo);
    try self.emitLocalGet(dec_ptr);
    self.code_builder.code.append(self.allocator, Op.i64_load) catch return error.OutOfMemory;
    // alignment=3, offset=8
    self.code_builder.code.append(self.allocator, 0x03) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, 0x08) catch return error.OutOfMemory;
    try self.emitLocalSet(dec_hi);

    const result_ptr = try self.allocStackResultPtr(12, 4);
    try self.emitLocalGet(result_ptr);
    try self.emitLocalGet(dec_lo);
    try self.emitLocalGet(dec_hi);
    try self.emitLocalGet(self.roc_ops_local);
    try self.emitCallBuiltin(import_idx);
...
fn generateStrEscapeAndQuote(self: *Self, quote_expr: anytype) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory; // end if
    ...
    self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory; // end loop
    self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory; // end block
...
fn emitLocalGet(self: *Self, local: u32) Allocator.Error!void {
    self.code_builder.code.append(self.allocator, Op.local_get) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, local) catch return error.OutOfMemory;
}
...
fn emitFloatMod(self: *Self, vt: ValType) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, div_op) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, trunc_op) catch return error.OutOfMemory;
    // * b
    try self.emitLocalGet(b);
    self.code_builder.code.append(self.allocator, mul_op) catch return error.OutOfMemory;
    // a - ...
    self.code_builder.code.append(self.allocator, sub_op) catch return error.OutOfMemory;
}
...
fn generateLLStrSearch(self: *Self, args: anytype, mode: StrSearchMode) Allocator.Error!void {
    ...
    switch (mode) {
        ...
        .contains => {
            ...
            try self.emitLocalSet(result_local);
            self.code_builder.code.append(self.allocator, Op.br) catch return error.OutOfMemory;
            WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 3) catch return error.OutOfMemory; // break out of block (past loop + block)
            self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory;
            ...
            self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory; // end if a_len >= b_len
            self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory; // end if b_len == 0
        },
    }
...
fn emitBytewiseCompare(self: *Self, ptr_a: u32, ptr_b: u32, len: u32, result_local: u32) Allocator.Error!void {
    ...
    try self.emitLocalSet(result_local);
    self.code_builder.code.append(self.allocator, Op.br) catch return error.OutOfMemory;
    WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 3) catch return error.OutOfMemory; // break out of block (skip if + loop + block)
    self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory;
...
fn generateLLListSortWith(self: *Self, ll: anytype, args: anytype, ret_layout: layout.Idx) Allocator.Error!void {
    ...
    self.code_builder.code.append(self.allocator, Op.@"else") catch return error.OutOfMemory;
    {
        ...
        self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
        {
            ...
            self.code_builder.code.append(self.allocator, Op.i32_ge_u) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.br_if) catch return error.OutOfMemory;
            try WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 1);
            ...
            self.code_builder.code.append(self.allocator, @intFromEnum(BlockType.void)) catch return error.OutOfMemory;
            {
                ...
                self.code_builder.code.append(self.allocator, Op.br) catch return error.OutOfMemory;
                try WasmModule.leb128WriteU32(self.allocator, &self.code_builder.code, 0);
            }
            self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory;
            self.code_builder.code.append(self.allocator, Op.end) catch return error.OutOfMemory;
...
fn generateLLListSplitFirst(self: *Self, args: anytype, ret_layout: layout.Idx) Allocator.Error!void {
    ...
    try self.emitLocalGet(result_ptr);
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(first_offset)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
    ...
    try self.emitLocalGet(result_ptr);
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(rest_offset)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
...
fn generateLLListSplitLast(self: *Self, args: anytype, ret_layout: layout.Idx) Allocator.Error!void {
    ...
    try self.emitLocalGet(result_ptr);
    self.code_builder.code.append(self.allocator, Op.i32_const) catch return error.OutOfMemory;
    WasmModule.leb128WriteI32(self.allocator, &self.code_builder.code, @intCast(aligned_last_offset)) catch return error.OutOfMemory;
    self.code_builder.code.append(self.allocator, Op.i32_add) catch return error.OutOfMemory;
