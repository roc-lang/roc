...
pub fn flush(memory: []const u8) Error!void {
    ...
    switch (builtin.cpu.arch) {
        ...
        .arm, .armeb => switch (builtin.os.tag) {
            .linux => {
            ...
            else => @compileError("instruction-cache synchronization is not implemented for this OS"),
        },
...
test "new instructions execute after repeated writes to reused addresses" {
    ...
    for (0..128) |generation| {
        ...
        const instructions: [2]u32 = switch (builtin.cpu.arch) {
            .arm => .{ 0xe3a00000 | value, 0xe12fff1e }, // mov r0, value; bx lr
            .aarch64 => .{ 0x52800000 | (value << 5), 0xd65f03c0 }, // mov w0, value; ret
            else => unreachable,
        };
