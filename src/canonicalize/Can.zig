...
const ExprResultMap = std.AutoHashMapUnmanaged(AST.Expr.Idx, ?CanonicalizedExpr);

const ExprChildFailure = union(enum) {
    generic,
    tuple_elem,
    lambda_body,
    if_condition,
...
fn canonicalizedExprFromResultOrMalformed(
    self: *Self,
    ast_expr_idx: AST.Expr.Idx,
    result: ?CanonicalizedExpr,
    failure: ExprChildFailure,
) std.mem.Allocator.Error!CanonicalizedExpr {
    if (result) |canonicalized| return canonicalized;

    const ast_expr = self.parse_ir.store.getExpr(ast_expr_idx);
    const region = self.parse_ir.tokenizedRegionToRegion(ast_expr.to_tokenized_region());
    const diagnostic: Diagnostic = switch (failure) {
        .generic => .{ .expr_not_canonicalized = .{ .region = region } },
        .tuple_elem => .{ .tuple_elem_not_canonicalized = .{ .region = region } },
        .lambda_body => .{ .lambda_body_not_canonicalized = .{ .region = region } },
        .if_condition => .{ .if_condition_not_canonicalized = .{ .region = region } },
        .if_then => .{ .if_then_not_canonicalized = .{ .region = region } },
        .if_else => .{ .if_else_not_canonicalized = .{ .region = region } },
        .invalid_string_interpolation => .{ .invalid_string_interpolation = .{ .region = region } },
    };
...
fn canonicalizeMatchBranchPatterns(
    self: *Self,
    ast_branch: AST.MatchBranch,
    branch_pat_scratch_top: u32,
...
const AliasCycleContext = struct {
    ...
    fn visit(self: *AliasCycleContext, stmt_idx: AST.Statement.Idx) std.mem.Allocator.Error!void {
        ...
        try frames.append(frame_allocator, .{
            .stmt_idx = stmt_idx,
            .dep_offset = 0,
            .entered = false,
        });
        ...
        while (frames.items.len > 0) {
            ...
            var frame = &frames.items[frame_idx];

            if (!frame.entered) {
                const index = self.next_index;
                self.next_index += 1;

                try self.index_by_stmt.put(gpa, frame.stmt_idx, index);
                try self.lowlink_by_stmt.put(gpa, frame.stmt_idx, index);
                try self.stack.append(gpa, frame.stmt_idx);
                try self.on_stack.put(gpa, frame.stmt_idx, {});
                frame.entered = true;
            }

            const decl = self.aliasDecl(frame.stmt_idx) orelse unreachable;
            const dependencies = self.can.parse_ir.decl_index.typeDependencies(decl.type_dependencies);
            if (frame.dep_offset < dependencies.len) {
                const dependency = dependencies[frame.dep_offset];
                frame.dep_offset += 1;
                ...
                if (self.on_stack.contains(dep_stmt_idx)) {
                    const dep_index = self.index_by_stmt.get(dep_stmt_idx).?;
                    const lowlink_ptr = self.lowlink_by_stmt.getPtr(frame.stmt_idx).?;
                    lowlink_ptr.* = @min(lowlink_ptr.*, dep_index);
                }
                continue;
            }

            const finished_stmt_idx = frame.stmt_idx;
            if (self.lowlink_by_stmt.get(finished_stmt_idx).? == self.index_by_stmt.get(finished_stmt_idx).?) {
                ...
                while (true) {
                    ...
                    if (member == finished_stmt_idx) break;
                }
            ...
            if (frames.items.len > 0) {
                const parent_stmt_idx = frames.items[frames.items.len - 1].stmt_idx;
                const finished_lowlink = self.lowlink_by_stmt.get(finished_stmt_idx).?;
                const parent_lowlink = self.lowlink_by_stmt.getPtr(parent_stmt_idx).?;
                parent_lowlink.* = @min(parent_lowlink.*, finished_lowlink);
            }
        }
    }
...
/// the parent path without the module prefix (null for top-level, `Num` for
/// `U8` inside `Num`). Associated blocks are canonicalized by the caller's
/// explicit associated-block walk.
fn registerTypeDeclNoAssociated(
    self: *Self,
    ast_stmt_idx: ?AST.Statement.Idx,
    type_decl: std.meta.fieldInfo(AST.Statement, .type_decl).type,
...
fn processAssociatedBlock(
    ...
    const State = struct {
        ...
        alias_sinks_owned: bool,
    };

    const Frame = union(enum) {
        enter: struct {
            request: AssociatedBlockRequest,
            alias_sinks_owned: bool,
        },
        next: struct {
            state: *State,
    ...
    var stack_fallback = std.heap.stackFallback(4096, self.env.gpa);
    const work_allocator = stack_fallback.get();

    var frames: std.ArrayList(Frame) = .empty;
    defer frames.deinit(work_allocator);

    try frames.append(work_allocator, .{ .enter = .{
        .request = .{
        ...
        .alias_sinks_owned = false,
    } });

    while (frames.pop()) |frame| {
        switch (frame) {
            .enter => |enter_data| {
                const request = enter_data.request;
                try self.scopeEnter(self.env.gpa, false);
                errdefer self.scopeExit(self.env.gpa) catch {};
                try self.declScopeEnter(request.assoc.scope);
                errdefer self.declScopeExit();

                const current_scope = &self.scopes.items[self.scopes.items.len - 1];
                current_scope.associated_type_name = request.type_name;
                try current_scope.introduceTypeAlias(self.env.gpa, request.type_name, request.owner_stmt_idx);

                const associated_scope = self.parse_ir.decl_index.scopes.items[@intFromEnum(request.assoc.scope)];
                const state = try work_allocator.create(State);
                state.* = .{
                    .owner_stmt_idx = request.owner_stmt_idx,
                    .parent_name = request.qualified_name_idx,
                    .relative_name_idx = request.relative_name_idx,
                    .type_name = request.type_name,
                    .stmt_idxs = self.parse_ir.store.statementSlice(request.assoc.statements),
                    .alias_sinks = request.alias_sinks,
                    .block_context = request.block_context,
                    .owner_is_redeclaration = request.owner_is_redeclaration,
                    ...
                    .associated_value_regions = .{},
                    .alias_sinks_owned = enter_data.alias_sinks_owned,
                };

                try frames.append(work_allocator, .{ .finish = state });
                try frames.append(work_allocator, .{ .next = .{
                    .state = state,
                    .stmt_offset = 0,
                } });
            },
            .next => |frame_data| {
                const state = frame_data.state;
                if (frame_data.stmt_offset >= state.stmt_idxs.len) continue;

                const stmt_idx = state.stmt_idxs[frame_data.stmt_offset];
                const stmt = self.parse_ir.store.getStatement(stmt_idx);
                if (state.owner_is_redeclaration) {
                    switch (stmt) {
                        .type_decl => {},
                        else => {
                            try frames.append(work_allocator, .{ .next = .{
                                .state = state,
                                .stmt_offset = frame_data.stmt_offset + 1,
                            } });
                            continue;
                        },
                    }
                ...
                switch (stmt) {
                    .type_decl => |nested_type_decl| {
                        const nested_header = self.parse_ir.store.getTypeHeader(nested_type_decl.header) catch {
                            try frames.append(work_allocator, .{ .next = .{
                                .state = state,
                                .stmt_offset = frame_data.stmt_offset + 1,
                            } });
                        ...
                        const nested_type_ident = self.parse_ir.tokens.resolveIdentifier(nested_header.name) orelse {
                            try frames.append(work_allocator, .{ .next = .{
                                .state = state,
                                .stmt_offset = frame_data.stmt_offset + 1,
                            } });
                        ...
                        const nested_qualified_idx = try self.insertQualifiedIdent(
                            self.env.getIdent(state.parent_name),
                            self.env.getIdent(nested_type_ident),
                        );

                        const nested_relative_for_block: ?Ident.Idx = if (state.relative_name_idx) |rel_idx|
                        ...
                        if (nested_type_decl.associated) |nested_assoc| {
                            const child_alias_sinks = try work_allocator.alloc(AssociatedAliasSink, state.alias_sinks.len + 1);
                            child_alias_sinks[0] = .{
                                .scope_index = self.currentScopeIdx(),
                                .prefix = nested_type_ident,
                            };
                            for (state.alias_sinks, 0..) |sink, sink_offset| {
                                ...
                                child_alias_sinks[sink_offset + 1] = .{
                            ...
                            try frames.append(work_allocator, .{ .enter = .{
                                .request = .{
                                    .owner_stmt_idx = nested_registration.stmt_idx,
                                    .qualified_name_idx = nested_qualified_idx,
                                    .relative_name_idx = nested_relative_for_block,
                                    .type_name = nested_type_ident,
                                    .assoc = nested_assoc,
                                    .alias_sinks = child_alias_sinks,
                                    .block_context = state.block_context,
                                    .owner_is_redeclaration = state.owner_is_redeclaration or nested_registration.is_redeclaration,
                                },
                                .alias_sinks_owned = true,
                            } });
                        }
                    },
                    .type_anno => |ta| {
                        ...
                        const next_i = frame_data.stmt_offset + 1;
                        var matched_decl = false;
                        if (next_i < state.stmt_idxs.len) {
                            const next_stmt_id = state.stmt_idxs[next_i];
                                ...
                                if (pattern == .ident) {
                                    ...
                                    if (self.parse_ir.tokens.resolveIdentifier(pattern_ident_tok)) |decl_ident| {
                                        if (name_ident.eql(decl_ident)) {
                                            ...
                                            if (try self.associatedValueWasRedeclared(&state.associated_value_regions, decl_ident, decl_region)) {
                                                try frames.append(work_allocator, .{ .next = .{
                                                    .state = state,
                                                    .stmt_offset = frame_data.stmt_offset + 2,
                                                } });
                                                continue;
                                            }

                                            const qualified_idx = try self.insertQualifiedIdent(
                                                self.env.getIdent(state.parent_name),
                                                self.env.getIdent(decl_ident),
                                            );
                                            const type_qualified_idx: ?Ident.Idx = if (state.parent_name.eql(state.type_name))
                                                qualified_idx
                                            else
                                                try self.insertQualifiedIdent(self.env.getIdent(state.type_name), self.env.getIdent(decl_ident));
                                            const assoc_key: ?AST.DeclIndex.AssocValue = if (state.owner_type_path) |owner|
                                            ...
                                            const method_binding = try self.recordAssociatedValue(associated_def, state.owner_is_module_visible, state.block_context);
                                            ...
                                            try self.registerAssociatedMethodIdent(state.owner_stmt_idx, decl_ident, qualified_idx, method_binding);
                                            ...
                                            const type_qualified_ident_idx = if (state.parent_name.eql(state.type_name))
                                                qualified_idx
                                            else
                                                try self.insertQualifiedIdent(self.env.getIdent(state.type_name), self.env.getIdent(decl_ident));
                                            ...
                                            try self.publishAssociatedAliasSinks(state.alias_sinks, decl_ident, pattern_idx);
                        ...
                        if (matched_decl) continue;

                        const region = self.parse_ir.tokenizedRegionToRegion(ta.region);
                        if (!try self.associatedValueWasRedeclared(&state.associated_value_regions, name_ident, region)) {
                            const qualified_idx = try self.insertQualifiedIdent(
                                self.env.getIdent(state.parent_name),
                                self.env.getIdent(name_ident),
                            );
                            const assoc_key: ?AST.DeclIndex.AssocValue = if (state.owner_type_path) |owner|
                            ...
                            const adopted_pattern_idx: ?CIR.Pattern.Idx = blk_adopt: {
                                ...
                                const tq_for_adopt = if (state.parent_name.eql(state.type_name))
                                    qualified_idx
                                else
                                    try self.insertQualifiedIdent(self.env.getIdent(state.type_name), self.env.getIdent(name_ident));
                            ...
                            try self.registerAssociatedMethodIdent(state.owner_stmt_idx, name_ident, qualified_idx, method_binding);
                            ...
                            const anno_type_qualified_idx: Ident.Idx = if (state.parent_name.eql(state.type_name))
                                qualified_idx
                            else
                                try self.insertQualifiedIdent(self.env.getIdent(state.type_name), self.env.getIdent(name_ident));
                            ...
                            try self.publishAssociatedAliasSinks(state.alias_sinks, name_ident, anno_pattern_idx);
                        }
                    ...
                    .decl => |decl| {
                        ...
                        if (pattern == .ident) {
                            const pattern_ident_tok = pattern.ident.ident_tok;
                            if (self.parse_ir.tokens.resolveIdentifier(pattern_ident_tok)) |decl_ident| {
                                if (try self.associatedValueWasRedeclared(&state.associated_value_regions, decl_ident, pattern_region)) {
                                    try frames.append(work_allocator, .{ .next = .{
                                        .state = state,
                                        .stmt_offset = frame_data.stmt_offset + 1,
                                    } });
                                ...
                                const parent_text = self.env.getIdent(state.parent_name);
                                const decl_text = self.env.getIdent(decl_ident);
                                const qualified_idx = try self.insertQualifiedIdent(parent_text, decl_text);

                                const type_qualified_decl_idx: ?Ident.Idx = if (state.parent_name.eql(state.type_name))
                                    qualified_idx
                                else
                                    try self.insertQualifiedIdent(self.env.getIdent(state.type_name), decl_text);
                                const assoc_key: ?AST.DeclIndex.AssocValue = if (state.owner_type_path) |owner|
                                    .{ .owner = owner, .item = decl_ident }
                                else
                                    null;
            ...
            .finish => |state| {
                ...
                if (state.alias_sinks_owned) {
                    work_allocator.free(state.alias_sinks);
                }
                self.declScopeExit();
                self.scopeExit(self.env.gpa) catch unreachable;
                work_allocator.destroy(state);
            },
        }
    }
}
...
fn registerNestedTypeDecl(
    ...
    const registration = switch (try self.registerTypeDeclNoAssociated(ast_stmt_idx, nested_type_decl, parent_name, relative_name_idx, true)) {
...
fn canonicalizeTopLevelTypeDecl(
    ...
    const type_decl_stmt_idx = switch (try self.registerTypeDeclNoAssociated(ast_stmt_idx, type_decl, null, null, true)) {
        .registered => |stmt_idx| stmt_idx,
...
const OpenLocalTypeAnno = struct {
    info: TypeAnnoIdent,
    type_var_scope: TypeVarScopeIdx,
};

const LocalTypeAnnoResult = union(enum) {
    anno: OpenLocalTypeAnno,
    malformed_stmt: CanonicalizedStatement,
};

const AssociatedBlockRequest = struct {
    owner_stmt_idx: Statement.Idx,
    qualified_name_idx: Ident.Idx,
    relative_name_idx: ?Ident.Idx,
    type_name: Ident.Idx,
    assoc: AST.Associated,
    alias_sinks: []const AssociatedAliasSink,
    block_context: ?BlockStatementContext,
    owner_is_redeclaration: bool,
};

const LocalTypeDeclResult = union(enum) {
...
fn canonicalizeLocalTypeAnnoStatement(
    self: *Self,
    name_tok: Token.Idx,
    anno: AST.TypeAnno.Idx,
    where: ?AST.Collection.Idx,
    tokenized_region: AST.TokenizedRegion,
) std.mem.Allocator.Error!LocalTypeAnnoResult {
    const region = self.parse_ir.tokenizedRegionToRegion(tokenized_region);

    const name_ident = self.parse_ir.tokens.resolveIdentifier(name_tok) orelse {
        ...
        return .{ .malformed_stmt = CanonicalizedStatement{ .idx = malformed_idx, .free_vars = DataSpan.empty() } };
    };

    const type_vars_top: u32 = @intCast(self.scratch_idents.top());
    const type_var_scope = self.scopeEnterTypeVar();
    errdefer self.scopeExitTypeVar(type_var_scope);

    const type_anno_idx = try self.canonicalizeTypeAnno(anno, .local_anno);
    try self.extractTypeVarIdentsFromASTAnno(anno, type_vars_top);

    const where_clauses = if (where) |where_coll| inner_blk: {
        const where_slice = self.parse_ir.store.whereClauseSlice(.{ .span = self.parse_ir.store.getCollection(where_coll).span });
        const where_start = self.env.store.scratchWhereClauseTop();
...
fn canonicalizeLocalAnnoOnlyStatement(
    self: *Self,
    anno_info: TypeAnnoIdent,
) std.mem.Allocator.Error!CanonicalizedStatement {
    const region = anno_info.anno_region;
    const name_ident = anno_info.name;
    ...
    const pattern_idx = if (self.isPlaceholder(name_ident)) placeholder_check: {
        ...
        const existing_pattern = current_scope.idents.get(name_ident) orelse {
            const pattern = Pattern{ .assign = .{ .ident = name_ident } };
            break :placeholder_check try self.env.addPattern(pattern, region);
        };
    ...
    const annotation_idx = try self.env.addAnnotation(CIR.Annotation{
        .anno = anno_info.anno_idx,
        .where = anno_info.where,
    }, region);
...
fn canonicalizeLocalTypeDeclStatement(
    self: *Self,
    type_decl: std.meta.fieldInfo(AST.Statement, .type_decl).type,
    ast_stmt_idx: AST.Statement.Idx,
    block_context: ?BlockStatementContext,
) std.mem.Allocator.Error!LocalTypeDeclResult {
    const region = self.parse_ir.tokenizedRegionToRegion(type_decl.region);
    ...
    const is_type_var_alias = type_var_alias_check: {
        ...
        const type_var_ident = self.parse_ir.tokens.resolveIdentifier(ast_anno.ty_var.tok) orelse break :type_var_alias_check false;
        if (self.scopeLookupTypeVar(type_var_ident) != .found) break :type_var_alias_check false;
    ...
    if (is_type_var_alias) {
        ...
        const ast_anno = self.parse_ir.store.getTypeAnno(type_decl.anno);
        const type_var_ident = self.parse_ir.tokens.resolveIdentifier(ast_anno.ty_var.tok) orelse unreachable;
...
fn collectBoundVarsToScratch(self: *Self, pattern_idx: Pattern.Idx) !void {
    ...
    var stack: std.ArrayList(Pattern.Idx) = .empty;
    defer stack.deinit(stack_allocator);

    try stack.append(stack_allocator, pattern_idx);
    while (stack.pop()) |current_idx| {
        const pattern = self.env.store.getPattern(current_idx);
        switch (pattern) {
            .assign => try self.scratch_bound_vars.append(current_idx),
            .record_destructure => |destructure| {
                ...
                while (i > 0) {
                    ...
                    try stack.append(stack_allocator, destruct.kind.toPatternIdx());
                }
            },
            .tuple => |tuple| {
                const patterns = self.env.store.slicePatterns(tuple.patterns);
                var i = patterns.len;
                while (i > 0) {
                    i -= 1;
                    try stack.append(stack_allocator, patterns[i]);
                }
            },
            .applied_tag => |tag| {
            ...
            .as => |as_pat| {
                ...
                try stack.append(stack_allocator, as_pat.pattern);
            },
            .list => |list| {
                if (list.rest_info) |rest| {
                    if (rest.pattern) |rest_pat_idx| {
                        try stack.append(stack_allocator, rest_pat_idx);
                    }
                }
                const patterns = self.env.store.slicePatterns(list.patterns);
                var i = patterns.len;
                while (i > 0) {
                    i -= 1;
                    try stack.append(stack_allocator, patterns[i]);
            ...
            .nominal => |nom| try stack.append(stack_allocator, nom.backing_pattern),
            .nominal_external => |nom| try stack.append(stack_allocator, nom.backing_pattern),
            .num_literal,
            .small_dec_literal,
            .dec_literal,
            .frac_f32_literal,
...
pub fn canonicalizeExpr(
    ...
    const BlockFinalKind = enum { expr, dbg, @"return" };

    const BlockStmtExprKind = enum { expr, dbg, expect, @"return" };

    const BlockDeclKind = union(enum) {
        decl: struct {
            pattern_idx: Pattern.Idx,
            mb_validated_anno: ?Annotation.Idx,
    ...
    const AssociatedValueFinishData = struct {
        body: AST.Expr.Idx,
        pattern_idx: Pattern.Idx,
        pattern_region: Region,
        qualified_idx: Ident.Idx,
        decl_ident: Ident.Idx,
        type_qualified_ident_idx: Ident.Idx,
        assoc_key: ?AST.DeclIndex.AssocValue,
        annotation_idx: ?Annotation.Idx,
        owner_stmt_idx: Statement.Idx,
        relative_name_idx: ?Ident.Idx,
        alias_sinks: []const AssociatedAliasSink,
        block_context: ?BlockStatementContext,
    ...
    const Frame = union(enum) {
        ...
        type_method_apply_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            args: AST.Expr.Span,
            scratch_top: u32,
            binding: Scope.TypeVarAliasBinding,
            method_name: Ident.Idx,
            region: Region,
        },
        type_var_field_dispatch_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            args: AST.Expr.Span,
            scratch_top: u32,
            binding: Scope.TypeVarAliasBinding,
        ...
        record_after_field: struct {
            ...
            seen_fields_top: u32,
            free_vars_start: u32,
            next_field_offset: usize,
            field_name: Ident.Idx,
            field_region: Region,
            value_idx: AST.Expr.Idx,
            region: Region,
        },
        lambda_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            body: AST.Expr.Idx,
        ...
        record_builder_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            rb: @TypeOf(@as(AST.Expr, undefined).record_builder),
            type_name: Ident.Idx,
            captures_top: u32,
            field_names_top: u32,
            field_values_top: u32,
            region: Region,
        },
        match_after_cond: struct {
            ast_expr_idx: AST.Expr.Idx,
            match_expr: @TypeOf(@as(AST.Expr, undefined).match),
            free_vars_start: u32,
            region: Region,
        },
        match_branch_start: struct {
            ast_expr_idx: AST.Expr.Idx,
            cond: Expr.Idx,
        ...
        field_access_after_receiver: struct {
            ast_expr_idx: AST.Expr.Idx,
            field_access: AST.BinOp,
            free_vars_start: u32,
            region: Region,
        },
        field_access_method_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            receiver: Expr.Idx,
            args: AST.Expr.Span,
            scratch_top: u32,
        ...
        block_stmt_next: struct {
            data: BlockData,
            stmt_offset: u32,
        },
        block_after_final_expr: struct {
            data: BlockData,
            child: AST.Expr.Idx,
            kind: BlockFinalKind,
            region: Region,
        ...
        block_finish: struct {
            data: BlockData,
            final_expr: CanonicalizedExpr,
        },
        block_after_expr_stmt: struct {
            data: BlockData,
            stmt_offset: u32,
            child: AST.Expr.Idx,
            kind: BlockStmtExprKind,
            region: Region,
            saved_in_expect: bool,
        },
        block_after_var_stmt: struct {
            data: BlockData,
            stmt_offset: u32,
            stmt_advance: u32,
            body: AST.Expr.Idx,
        ...
        associated_block_enter: AssociatedBlockRequest,
        associated_block_next: struct {
            state: *AssociatedBlockState,
            stmt_offset: usize,
        },
        associated_block_finish: *AssociatedBlockState,
        associated_decl_finish: AssociatedValueFinishData,
        tuple_one_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            item: AST.Expr.Idx,
        },
        tuple_finish: struct {
        ...
        tuple_access_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            tuple_expr: AST.Expr.Idx,
            elem_token: Token.Idx,
            free_vars_start: u32,
            region: Region,
        },
        method_call_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
        ...
        arrow_after_left: struct {
            ast_expr_idx: AST.Expr.Idx,
            arrow_call: AST.BinOp,
            free_vars_start: u32,
            region: Region,
        },
        arrow_after_apply_fn: struct {
            ast_expr_idx: AST.Expr.Idx,
            left: AST.Expr.Idx,
            fn_idx: AST.Expr.Idx,
            args: AST.Expr.Span,
        ...
        arrow_call_single_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            left: AST.Expr.Idx,
            right: AST.Expr.Idx,
            free_vars_start: u32,
        ...
        single_question_after_left: struct {
            ast_expr_idx: AST.Expr.Idx,
            binop: AST.BinOp,
            free_vars_start: u32,
            region: Region,
        },
        single_question_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            binop: AST.BinOp,
            free_vars_start: u32,
            region: Region,
        },
        suffix_question_finish: struct {
            ast_expr_idx: AST.Expr.Idx,
            child: AST.Expr.Idx,
            free_vars_start: u32,
            region: Region,
        },
        if_chain_next: struct {
            ast_expr_idx: AST.Expr.Idx,
            current_if_idx: AST.Expr.Idx,
            captures_top: u32,
            free_vars_start: u32,
            scratch_top: u32,
            region: Region,
        },
        if_after_cond: struct {
            ast_expr_idx: AST.Expr.Idx,
            current_if_idx: AST.Expr.Idx,
            captures_top: u32,
            free_vars_start: u32,
        ...
        if_after_else: struct {
            ast_expr_idx: AST.Expr.Idx,
            else_idx: AST.Expr.Idx,
            captures_top: u32,
            free_vars_start: u32,
            scratch_top: u32,
            region: Region,
        },
        if_without_else_after_cond: struct {
            ast_expr_idx: AST.Expr.Idx,
        ...
        if_without_else_after_then: struct {
            ast_expr_idx: AST.Expr.Idx,
            condition: AST.Expr.Idx,
            then: AST.Expr.Idx,
            captures_top: u32,
    ...
    var stack_fallback = std.heap.stackFallback(64 * 1024, self.env.gpa);
    const work_allocator = stack_fallback.get();

    var results: ExprResultMap = .{};
    defer results.deinit(work_allocator);

    var frames: std.ArrayList(Frame) = .empty;
    defer frames.deinit(work_allocator);

    try frames.append(work_allocator, .{ .enter = ast_expr_idx });
    while (frames.pop()) |frame| {
        switch (frame) {
            .enter => |current_idx| {
                if (results.contains(current_idx)) continue;

                const expr = self.parse_ir.store.getExpr(current_idx);
                switch (expr) {
                    .apply => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        ...
                        if (ast_fn == .tag) {
                            ...
                            try frames.append(work_allocator, .{ .tag_finish = .{
                                .ast_expr_idx = current_idx,
                                .tag = ast_fn.tag,
                                .args = e.args,
                            ...
                            while (i > 0) {
                                i -= 1;
                                try frames.append(work_allocator, .{ .enter = args_slice[i] });
                            }
                            continue;
                        }
                        ...
                        if (ast_fn == .ident) {
                            ...
                            if (qualifier_tokens.len == 1) {
                                ...
                                if (self.parse_ir.tokens.resolveIdentifier(qualifier_tok)) |alias_name| {
                                    var found_binding: ?Scope.TypeVarAliasBinding = null;
                                    for (self.scopes.items) |*scope| {
                                    ...
                                    if (found_binding) |binding| {
                                        if (self.parse_ir.tokens.resolveIdentifier(ident_expr.token)) |method_name| {
                                            const scratch_top = self.env.store.scratchExprTop();
                                            try frames.append(work_allocator, .{ .type_method_apply_finish = .{
                                                .ast_expr_idx = current_idx,
                                                .args = e.args,
                                                .scratch_top = scratch_top,
                                                .binding = binding,
                        ...
                        const free_vars_start = self.scratch_free_vars.top();
                        const scratch_top = self.env.store.scratchExprTop();
                        try frames.append(work_allocator, .{ .apply_finish = .{
                            .ast_expr_idx = current_idx,
                            .fn_idx = e.@"fn",
                            .args = e.args,
                            .scratch_top = scratch_top,
                        ...
                        try frames.append(work_allocator, .{ .enter = e.@"fn" });
                    },
                    .list => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        const items_slice = self.parse_ir.store.exprSlice(e.items);
                        if (items_slice.len == 0) {
                            const expr_idx = try self.env.addExpr(CIR.Expr{ .e_empty_list = .{} }, region);
                            try results.put(work_allocator, current_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = DataSpan.empty() });
                            continue;
                        }

                        const free_vars_start = self.scratch_free_vars.top();
                        const scratch_top = self.env.store.scratchExprTop();
                        try frames.append(work_allocator, .{ .list_finish = .{
                            .ast_expr_idx = current_idx,
                            .items = e.items,
                            .scratch_top = scratch_top,
                            .free_vars_start = free_vars_start,
                        ...
                        while (i > 0) {
                            i -= 1;
                            try frames.append(work_allocator, .{ .enter = items_slice[i] });
                        }
                    },
                    .tuple => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        const items_slice = self.parse_ir.store.exprSlice(e.items);
                        if (items_slice.len == 0) {
                            const body_region = self.parse_ir.tokenizedRegionToRegion(expr.to_tokenized_region());
                            const expr_idx = try self.env.pushMalformed(Expr.Idx, Diagnostic{ .empty_tuple = .{ .region = body_region } });
                            try results.put(work_allocator, current_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = DataSpan.empty() });
                        } else if (items_slice.len == 1) {
                            try frames.append(work_allocator, .{ .tuple_one_finish = .{
                                .ast_expr_idx = current_idx,
                                .item = items_slice[0],
                            } });
                            try frames.append(work_allocator, .{ .enter = items_slice[0] });
                        } else {
                            const free_vars_start = self.scratch_free_vars.top();
                            const scratch_top = self.env.store.scratchExprTop();
                            try frames.append(work_allocator, .{ .tuple_finish = .{
                                .ast_expr_idx = current_idx,
                                .items = e.items,
                                .scratch_top = scratch_top,
                                .free_vars_start = free_vars_start,
                    ...
                    .record => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .record_after_ext = .{
                            .ast_expr_idx = current_idx,
                            .ext = e.ext,
                            .free_vars_start = free_vars_start,
                            .region = region,
                        ...
                        if (e.ext) |ext_idx| {
                            try frames.append(work_allocator, .{ .enter = ext_idx });
                        }
                    },
                    .lambda => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        ...
                        const lambda_idx = try self.env.addExpr(Expr{
                            .e_lambda = .{
                                .args = args_span,
                                .body = undefined,
                            },
                        ...
                        try frames.append(work_allocator, .{ .lambda_finish = .{
                            .ast_expr_idx = current_idx,
                            .body = e.body,
                            .args_span = args_span,
                        ...
                        try frames.append(work_allocator, .{ .enter = e.body });
                    },
                    .string => |e| {
                        const parts_slice = self.parse_ir.store.exprSlice(e.parts);
                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .string_finish = .{
                            .ast_expr_idx = current_idx,
                            .parts = e.parts,
                            .scratch_top = self.env.store.scratchExprTop(),
                            .free_vars_start = free_vars_start,
                            .is_multiline = false,
                            .region = self.parse_ir.tokenizedRegionToRegion(e.region),
                        } });

                        var i = parts_slice.len;
                        while (i > 0) {
                            i -= 1;
                            const part_idx = parts_slice[i];
                            const part_node = self.parse_ir.store.getExpr(part_idx);
                            if (part_node != .string_part) {
                                try frames.append(work_allocator, .{ .enter = part_idx });
                            }
                        }
                    },
                    .multiline_string => |e| {
                    ...
                    .for_expr => |for_expr| {
                        ...
                        try frames.append(work_allocator, .{ .for_after_list = .{
                            .ast_expr_idx = current_idx,
                            .patt = for_expr.patt,
                            .list_expr = for_expr.expr,
                            .body = for_expr.body,
                            ...
                            .saved_defining_patterns_start = saved_defining_patterns_start,
                            .saved_defining_pattern = saved_defining_pattern,
                            .region = self.parse_ir.tokenizedRegionToRegion(for_expr.region),
                        } });
                        try frames.append(work_allocator, .{ .enter = for_expr.expr });
                    },
                    .record_builder => |rb| {
                        ...
                        const type_name: Ident.Idx = switch (mapper_expr) {
                            .tag => |tag| self.parse_ir.tokens.resolveIdentifier(tag.token) orelse {
                                ...
                                try results.put(work_allocator, current_idx, CanonicalizedExpr{ .idx = malformed_idx, .free_vars = DataSpan.empty() });
                                continue;
                            },
                            else => {
                        ...
                        var malformed_field_name: ?Expr.Idx = null;
                        for (fields_slice) |field_idx| {
                            const field = self.parse_ir.store.getRecordField(field_idx);
                            const field_name = self.parse_ir.tokens.resolveIdentifier(field.name) orelse {
                                malformed_field_name = try self.env.pushMalformed(Expr.Idx, Diagnostic{ .expr_not_canonicalized = .{
                                    .region = region,
                                } });
                                break;
                            };
                        ...
                        if (malformed_field_name) |malformed_idx| {
                            self.scratch_captures.clearFrom(captures_top);
                            self.scratch_idents.clearFrom(field_names_top);
                            self.scratch_expr_ids.clearFrom(field_values_top);
                            try results.put(work_allocator, current_idx, CanonicalizedExpr{ .idx = malformed_idx, .free_vars = DataSpan.empty() });
                            continue;
                        }

                        try frames.append(work_allocator, .{ .record_builder_finish = .{
                            .ast_expr_idx = current_idx,
                            .rb = rb,
                            .type_name = type_name,
                            .captures_top = captures_top,
                            .field_names_top = field_names_top,
                            .field_values_top = field_values_top,
                            .region = region,
                        } });
                        ...
                        while (i > 0) {
                            i -= 1;
                            const field = self.parse_ir.store.getRecordField(fields_slice[i]);
                            if (field.value) |value_idx| {
                                try frames.append(work_allocator, .{ .enter = value_idx });
                            }
                        }
                    },
                    .match => |m| {
                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .match_after_cond = .{
                            .ast_expr_idx = current_idx,
                            .match_expr = m,
                            .free_vars_start = free_vars_start,
                            .region = self.parse_ir.tokenizedRegionToRegion(m.region),
                        } });
                        try frames.append(work_allocator, .{ .enter = m.expr });
                    },
                    .field_access => |field_access| {
                        ...
                        if (left_expr == .ident) {
                            ...
                            if (self.parse_ir.tokens.resolveIdentifier(left_ident.token)) |left_name| {
                                        ...
                                        switch (right_expr) {
                                            .apply => |apply| {
                                                ...
                                                if (method_expr != .ident) {
                                                    const expr_idx = try self.env.pushMalformed(Expr.Idx, Diagnostic{ .expr_not_canonicalized = .{
                                                        .region = region,
                                                    } });
                                                    const free_vars_span = self.scratch_free_vars.spanFrom(free_vars_start);
                                                    try results.put(work_allocator, current_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = free_vars_span });
                                                    continue;
                                                }
                                                ...
                                                const scratch_top = self.env.store.scratchExprTop();
                                                try frames.append(work_allocator, .{ .type_var_field_dispatch_finish = .{
                                                    .ast_expr_idx = current_idx,
                                                    .args = apply.args,
                                                    .scratch_top = scratch_top,
                                                    .binding = binding,
                                ...
                                const maybe_module_name: ?Ident.Idx = if (self.scopeLookupModule(left_name)) |info|
                                    info.module_name
                                else if (self.hasAvailableModuleEnv(left_name))
                                    left_name
                                else
                                    null;

                                if (maybe_module_name) |module_name| {
                                    const maybe_import_idx: ?Import.Idx = if (self.scopeLookupImportedModule(module_name)) |idx|
                                        idx
                                    else blk: {
                                    ...
                                    if (maybe_import_idx) |import_idx| {
                                        const right_expr = self.parse_ir.store.getExpr(field_access.right);
                                        if (right_expr == .apply) {
                                                ...
                                                if (auto_imported_type.statement_idx != null) {
                                                    ...
                                                    if (module_env.common.findIdent(qualified_text)) |method_ident_idx| {
                                                        if (module_env.getExposedNodeIndexById(method_ident_idx)) |method_node_idx| {
                                                            ...
                                                            const scratch_top = self.env.store.scratchExprTop();
                                                            try frames.append(work_allocator, .{ .arrow_call_args_finish = .{
                                                                .ast_expr_idx = current_idx,
                                                                .func = func_expr_idx,
                                                                .args = apply.args,
                                                                .scratch_top = scratch_top,
                                                    ...
                                                    const expr_idx = try self.env.pushMalformed(Expr.Idx, Diagnostic{ .nested_value_not_found = .{
                                            ...
                                            if (target_node_idx_opt) |target_node_idx| {
                                                ...
                                                const expr_idx = try self.env.pushMalformed(Expr.Idx, Diagnostic{ .qualified_ident_does_not_exist = .{
                                                    .ident = method_name,
                                                    .region = region,
                                                } });
                                                const free_vars_span = self.scratch_free_vars.spanFrom(free_vars_start);
                        ...
                        try frames.append(work_allocator, .{ .field_access_after_receiver = .{
                            .ast_expr_idx = current_idx,
                            .field_access = field_access,
                            .free_vars_start = free_vars_start,
                            .region = region,
                        } });
                        try frames.append(work_allocator, .{ .enter = field_access.left });
                    },
                    .block => |block| {
                        const block_region = self.parse_ir.tokenizedRegionToRegion(block.region);

                        try self.scopeEnter(self.env.gpa, false);
                        try self.declScopeEnter(block.scope);
                        ...
                        const data = BlockData{
                            .ast_expr_idx = current_idx,
                            .block = block,
                            .stmt_start = self.env.store.scratch.?.statements.top(),
                            .bound_vars_top = self.scratch_bound_vars.top(),
                            .captures_top = self.scratch_captures.top(),
                            .local_functions_top = self.scratch_local_function_patterns.top(),
                            .block_defs_top = self.scratch_block_local_defs.top(),
                            .free_vars_top = self.scratch_free_vars.top(),
                            .saved_defining_patterns_start = saved_defining_patterns_start,
                            .saved_defining_pattern = saved_defining_pattern,
                            .saved_stmt_pos = saved_stmt_pos,
                            .region = block_region,
                        ...
                        try self.recordParserBlockLocalDefs(block.scope);
                        try frames.append(work_allocator, .{ .block_stmt_next = .{
                            .data = data,
                            .stmt_offset = 0,
                        } });
                    },
                    .dbg => |d| {
                        try frames.append(work_allocator, .{ .dbg_finish = .{
                            .ast_expr_idx = current_idx,
                            .child = d.expr,
                            .region = self.parse_ir.tokenizedRegionToRegion(d.region),
                        } });
                        try frames.append(work_allocator, .{ .enter = d.expr });
                    },
                    .unary_op => |unary| {
                        try frames.append(work_allocator, .{ .unary_finish = .{
                            .ast_expr_idx = current_idx,
                            .child = unary.expr,
                            .operator = unary.operator,
                            .region = self.parse_ir.tokenizedRegionToRegion(unary.region),
                        } });
                        try frames.append(work_allocator, .{ .enter = unary.expr });
                    },
                    .tuple_access => |tuple_access| {
                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .tuple_access_finish = .{
                            .ast_expr_idx = current_idx,
                            .tuple_expr = tuple_access.expr,
                            .elem_token = tuple_access.elem_token,
                            .free_vars_start = free_vars_start,
                            .region = self.parse_ir.tokenizedRegionToRegion(tuple_access.region),
                        } });
                        try frames.append(work_allocator, .{ .enter = tuple_access.expr });
                    },
                    .method_call => |mc| {
                        const free_vars_start = self.scratch_free_vars.top();
                        const scratch_top = self.env.store.scratchExprTop();
                        try frames.append(work_allocator, .{ .method_call_finish = .{
                            .ast_expr_idx = current_idx,
                            .receiver = mc.receiver,
                            .method_token = mc.method_token,
                            .args = mc.args,
                            .scratch_top = scratch_top,
                            .free_vars_start = free_vars_start,
                            .region = self.parse_ir.tokenizedRegionToRegion(mc.region),
                        } });

                        const args_slice = self.parse_ir.store.exprSlice(mc.args);
                        var i = args_slice.len;
                        while (i > 0) {
                            i -= 1;
                            try frames.append(work_allocator, .{ .enter = args_slice[i] });
                        }
                        try frames.append(work_allocator, .{ .enter = mc.receiver });
                    },
                    .arrow_call => |arrow_call| {
                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .arrow_after_left = .{
                            .ast_expr_idx = current_idx,
                            .arrow_call = arrow_call,
                            .free_vars_start = free_vars_start,
                            .region = self.parse_ir.tokenizedRegionToRegion(arrow_call.region),
                        } });
                        try frames.append(work_allocator, .{ .enter = arrow_call.left });
                    },
                    .bin_op => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        ...
                        if (op_token_early.tag == .OpQuestion) {
                            const free_vars_start = self.scratch_free_vars.top();
                            try frames.append(work_allocator, .{ .single_question_after_left = .{
                                .ast_expr_idx = current_idx,
                                .binop = e,
                                .free_vars_start = free_vars_start,
                                .region = region,
                            } });
                            try frames.append(work_allocator, .{ .enter = e.left });
                            continue;
                        }

                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .binop_finish = .{
                            .ast_expr_idx = current_idx,
                            .binop = e,
                            .free_vars_start = free_vars_start,
                            .region = region,
                        } });
                        try frames.append(work_allocator, .{ .enter = e.right });
                        try frames.append(work_allocator, .{ .enter = e.left });
                    },
                    .suffix_single_question => |unary| {
                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .suffix_question_finish = .{
                            .ast_expr_idx = current_idx,
                            .child = unary.expr,
                            .free_vars_start = free_vars_start,
                            .region = self.parse_ir.tokenizedRegionToRegion(unary.region),
                    ...
                    .if_then_else => {
                        ...
                        try frames.append(work_allocator, .{ .if_chain_next = .{
                            .ast_expr_idx = current_idx,
                            .current_if_idx = current_idx,
                            .captures_top = captures_top,
                            .free_vars_start = free_vars_start,
                            .scratch_top = scratch_top,
                    ...
                    .if_without_else => |e| {
                        ...
                        const captures_top = self.scratch_captures.top();
                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .if_without_else_after_cond = .{
                            .ast_expr_idx = current_idx,
                        ...
                        try frames.append(work_allocator, .{ .enter = e.condition });
                    },
                    .int,
                    .frac,
                    .typed_int,
                    ...
                    .ellipsis,
                    .malformed,
                    => try results.put(work_allocator, current_idx, try self.canonicalizeExprShallow(current_idx)),
                    .tag => |e| {
                        const free_vars_start = self.scratch_free_vars.top();
                        try frames.append(work_allocator, .{ .tag_finish = .{
                            .ast_expr_idx = current_idx,
            ...
            .apply_finish => |data| {
                const can_fn_expr = (results.get(data.fn_idx) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };

                for (self.parse_ir.store.exprSlice(data.args)) |arg| {
                    if (results.get(arg) orelse null) |can_arg| {
                        try self.env.store.addScratchExpr(can_arg.idx);
                    }
                }
                const args_span = try self.env.store.exprSpanFrom(data.scratch_top);
                ...
                const free_vars_span = self.scratch_free_vars.spanFrom(data.free_vars_start);
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = free_vars_span });
            },
            .type_method_apply_finish => |data| {
                for (self.parse_ir.store.exprSlice(data.args)) |arg| {
                    if (results.get(arg) orelse null) |can_arg| {
                        try self.env.store.addScratchExpr(can_arg.idx);
                    }
                ...
                const dispatch_expr_idx = try self.env.addExpr(CIR.Expr{ .e_type_method_call = .{
                    .type_var_alias_stmt = data.binding.statement_idx,
                    .method_name = data.method_name,
                    .method_name_region = data.region,
                    .args = args_span,
                } }, data.region);

                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = dispatch_expr_idx, .free_vars = DataSpan.empty() });
            },
            .type_var_field_dispatch_finish => |data| {
                for (self.parse_ir.store.exprSlice(data.args)) |arg| {
                    if (results.get(arg) orelse null) |can_arg| {
                        try self.env.store.addScratchExpr(can_arg.idx);
                    }
                ...
                const free_vars_span = self.scratch_free_vars.spanFrom(data.free_vars_start);
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = dispatch_expr_idx, .free_vars = free_vars_span });
            },
            .tag_finish => |data| {
                var args_span = Expr.Span{ .span = DataSpan.empty() };
                if (data.args) |args| {
                    for (self.parse_ir.store.exprSlice(args)) |arg| {
                        if (results.get(arg) orelse null) |can_arg| {
                            try self.env.store.addScratchExpr(can_arg.idx);
                        }
                    }
                    args_span = try self.env.store.exprSpanFrom(data.scratch_top);
                }

                try results.put(
                    work_allocator,
                    data.ast_expr_idx,
                    try self.canonicalizeTagExprWithArgsSpan(data.tag, args_span, data.region, data.free_vars_start),
                );
            },
            .list_finish => |data| {
                for (self.parse_ir.store.exprSlice(data.items)) |item| {
                    if (results.get(item) orelse null) |can_item| {
                        try self.env.store.addScratchExpr(can_item.idx);
                    }
                }
                const elems_span = try self.env.store.exprSpanFrom(data.scratch_top);

                if (elems_span.span.len == 0) {
                    const expr_idx = try self.env.addExpr(CIR.Expr{ .e_empty_list = .{} }, data.region);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = DataSpan.empty() });
                    continue;
                }

                const expr_idx = try self.env.addExpr(CIR.Expr{ .e_list = .{ .elems = elems_span } }, data.region);
                const free_vars_span = self.scratch_free_vars.spanFrom(data.free_vars_start);
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = free_vars_span });
            },
            .record_after_ext => |data| {
                const ext_expr: ?Expr.Idx = if (data.ext) |ext_idx| blk: {
                    if (results.get(ext_idx) orelse null) |can_ext| {
                        break :blk can_ext.idx;
                    }
                    break :blk null;
                } else null;

                const record = self.parse_ir.store.getExpr(data.ast_expr_idx).record;
                const fields_slice = self.parse_ir.store.recordFieldSlice(record.fields);
                if (fields_slice.len == 0) {
                    const expr_idx = try self.env.addExpr(CIR.Expr{ .e_empty_record = .{} }, data.region);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = DataSpan.empty() });
                    continue;
                }

                try frames.append(work_allocator, .{ .record_next = .{
                    .ast_expr_idx = data.ast_expr_idx,
                    .ext_expr = ext_expr,
                    .scratch_top = self.env.store.scratch.?.record_fields.top(),
                    .seen_fields_top = self.scratch_seen_record_fields.top(),
                    .free_vars_start = data.free_vars_start,
                    .field_offset = 0,
                    .region = data.region,
                } });
            },
            .record_next => |data| {
                const record = self.parse_ir.store.getExpr(data.ast_expr_idx).record;
                const fields_slice = self.parse_ir.store.recordFieldSlice(record.fields);
                if (data.field_offset >= fields_slice.len) {
                    self.scratch_seen_record_fields.clearFrom(data.seen_fields_top);

                    const fields_span = try self.env.store.recordFieldSpanFrom(data.scratch_top);
                    const expr_idx = try self.env.addExpr(CIR.Expr{
                        .e_record = .{
                            .fields = fields_span,
                            .ext = data.ext_expr,
                ...
                const field_idx = fields_slice[data.field_offset];
                const ast_field = self.parse_ir.store.getRecordField(field_idx);
                const field_name_ident = self.parse_ir.tokens.resolveIdentifier(ast_field.name) orelse {
                    try frames.append(work_allocator, .{ .record_next = .{
                        .ast_expr_idx = data.ast_expr_idx,
                        .ext_expr = data.ext_expr,
                        .scratch_top = data.scratch_top,
                        .seen_fields_top = data.seen_fields_top,
                        .free_vars_start = data.free_vars_start,
                        .field_offset = data.field_offset + 1,
                        .region = data.region,
                    } });
                    continue;
                };
                ...
                for (self.scratch_seen_record_fields.sliceFromStart(data.seen_fields_top)) |seen_field| {
                ...
                const value_idx = if (ast_field.value) |value_idx|
                    value_idx
                else blk: {
                    ...
                    break :blk try self.parse_ir.store.addExpr(ident_expr);
                };

                try frames.append(work_allocator, .{ .record_after_field = .{
                    .ast_expr_idx = data.ast_expr_idx,
                    .ext_expr = data.ext_expr,
                    .scratch_top = data.scratch_top,
                    .seen_fields_top = data.seen_fields_top,
                    .free_vars_start = data.free_vars_start,
                    .next_field_offset = data.field_offset + 1,
                    .field_name = field_name_ident,
            ...
            .record_after_field => |data| {
                if (results.get(data.value_idx) orelse null) |can_value| {
                    const field_idx = try self.env.addRecordField(RecordField{
                ...
                try frames.append(work_allocator, .{ .record_next = .{
                    ...
                    .seen_fields_top = data.seen_fields_top,
                    .free_vars_start = data.free_vars_start,
                    .field_offset = data.next_field_offset,
                    .region = data.region,
                } });
            },
            .lambda_finish => |data| {
                ...
                defer self.scopeExit(self.env.gpa) catch {};
                defer self.enclosing_lambda = data.saved_enclosing_lambda;
                defer self.scratch_captures.clearFrom(data.captures_top);
                defer self.defining_patterns_start = data.saved_defining_patterns_start;
                defer self.defining_pattern = data.saved_defining_pattern;

                const can_body = (results.get(data.body) orelse null) orelse {
                    const ast_body = self.parse_ir.store.getExpr(data.body);
                    ...
                    self.scratch_free_vars.clearFrom(data.body_free_vars_start);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = malformed_idx, .free_vars = DataSpan.empty() });
                    continue;
                };
                ...
                for (self.env.store.slicePatterns(data.args_span)) |arg_pat_idx| {
                ...
                for (body_free_vars_slice) |fv| {
                    if (!self.scratch_captures.containsFrom(data.captures_top, fv) and
                ...
                self.scratch_free_vars.clearFrom(data.body_free_vars_start);
                self.env.store.updateLambdaBody(data.lambda_idx, can_body.idx);

                const captures_slice = self.scratch_captures.sliceFromStart(data.captures_top);
                if (captures_slice.len == 0) {
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = data.lambda_idx, .free_vars = DataSpan.empty() });
                    continue;
                }
                ...
                const capture_info: Expr.Capture.Span = blk: {
                    ...
                    for (captures_slice) |pattern_idx| {
                        ...
                        const capture_idx = try self.env.addCapture(capture, data.region);
            ...
            .string_finish => |data| {
                const parts_slice = self.parse_ir.store.exprSlice(data.parts);
                if (!data.is_multiline) {
                    for (parts_slice) |part| {
                        ...
                        switch (part_node) {
                            .string_part => |sp| {
                                ...
                                try self.addInterpolationResultToScratch(part_node, results.get(part) orelse null);
                            },
                        }
                    }
                } else {
                ...
                const can_str_span = try self.env.store.exprSpanFrom(data.scratch_top);
            ...
            .for_after_list => |data| {
                const list_expr = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.list_expr,
                    results.get(data.list_expr) orelse null,
                    .generic,
                );

                const list_free_vars_slice = self.scratch_free_vars.sliceFromSpan(list_expr.free_vars);
                for (list_free_vars_slice) |fv| {
                    try self.appendPropagatedFreeVarExcludingBound(data.captures_top, data.bound_vars_top, fv);
                }
                self.scratch_free_vars.clearFrom(data.list_free_vars_start);

                try self.scopeEnter(self.env.gpa, false);
                const patt = try self.canonicalizePatternOrMalformed(data.patt);
                try self.collectBoundVarsToScratch(patt);

                self.loop_depth += 1;
                try frames.append(work_allocator, .{ .for_after_body = .{
                    .ast_expr_idx = data.ast_expr_idx,
                    .patt = patt,
                    .list_expr = list_expr.idx,
                    .body = data.body,
                    .body_free_vars_start = self.scratch_free_vars.top(),
                    .captures_top = data.captures_top,
                    .bound_vars_top = data.bound_vars_top,
                    .saved_stmt_pos = data.saved_stmt_pos,
                    .saved_defining_patterns_start = data.saved_defining_patterns_start,
                    .saved_defining_pattern = data.saved_defining_pattern,
                    .region = data.region,
                } });
                try frames.append(work_allocator, .{ .enter = data.body });
            },
            .for_after_body => |data| {
                defer self.scopeExit(self.env.gpa) catch {};
                defer self.scratch_bound_vars.clearFrom(data.bound_vars_top);
                defer self.scratch_captures.clearFrom(data.captures_top);
                defer self.in_statement_position = data.saved_stmt_pos;
                defer self.defining_patterns_start = data.saved_defining_patterns_start;
                defer self.defining_pattern = data.saved_defining_pattern;
                defer self.loop_depth -= 1;

                const body_expr = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.body,
                    results.get(data.body) orelse null,
                    .generic,
                );
                ...
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = for_expr_idx, .free_vars = free_vars });
            },
            .record_builder_finish => |data| {
                defer self.scratch_captures.clearFrom(data.captures_top);
                defer self.scratch_idents.clearFrom(data.field_names_top);
                defer self.scratch_expr_ids.clearFrom(data.field_values_top);

                const fields_slice = self.parse_ir.store.recordFieldSlice(data.rb.fields);
                const field_names = self.scratch_idents.slice(data.field_names_top, self.scratch_idents.top());
                std.debug.assert(field_names.len == fields_slice.len);

                for (fields_slice, field_names) |field_idx, field_name| {
                    const field = self.parse_ir.store.getRecordField(field_idx);
                    if (field.value) |value_idx| {
                        if (results.get(value_idx) orelse null) |can_value| {
                            ...
                            for (value_free_vars) |fv| {
                                try self.appendPropagatedFreeVar(data.captures_top, fv);
                            }
                        } else {
                            const malformed_idx = try self.env.pushMalformed(Expr.Idx, Diagnostic{ .expr_not_canonicalized = .{
                                .region = data.region,
                        ...
                        if (self.scopeContains(.ident, field_name)) |pattern_idx| {
                            ...
                            try self.appendPropagatedFreeVar(data.captures_top, pattern_idx);
                        } else {
                ...
                const type_name_text = self.env.getIdent(data.type_name);
                ...
                if (map2_pattern_idx == null) {
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{
                ...
                const field_values = self.scratch_expr_ids.slice(data.field_values_top, self.scratch_expr_ids.top());
                ...
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = result_expr, .free_vars = free_vars_span });
            },
            .match_after_cond => |data| {
                const can_cond = (results.get(data.match_expr.expr) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };

                try frames.append(work_allocator, .{ .match_branch_start = .{
                    .ast_expr_idx = data.ast_expr_idx,
                    .cond = can_cond.idx,
                    .branches = data.match_expr.branches,
                    .branch_offset = 0,
            ...
            .match_branch_start => |data| {
                const branches_slice = self.parse_ir.store.matchBranchSlice(data.branches);
                if (data.branch_offset >= branches_slice.len) {
                    const branches_span = try self.env.store.matchBranchSpanFrom(data.scratch_top);
                    const expr_idx = try self.env.addExpr(CIR.Expr{ .e_match = Expr.Match{
                        .cond = data.cond,
                ...
                const ast_branch = self.parse_ir.store.getBranch(branches_slice[data.branch_offset]);

                try self.scopeEnter(self.env.gpa, false);

                const branch_pat_scratch_top = self.env.store.scratchMatchBranchPatternTop();
                const branch_pat_span = try self.canonicalizeMatchBranchPatterns(ast_branch, branch_pat_scratch_top);
                ...
                if (ast_branch.guard) |guard| {
                    try frames.append(work_allocator, .{ .match_after_guard = .{
                        .ast_expr_idx = data.ast_expr_idx,
                        .cond = data.cond,
                        .branches = data.branches,
                        .branch_offset = data.branch_offset,
                        .scratch_top = data.scratch_top,
                        .free_vars_start = data.free_vars_start,
                        ...
                        .saved_defining_pattern = saved_defining_pattern,
                        .guard = guard,
                        .body = ast_branch.body,
                        .region = data.region,
                    } });
                    try frames.append(work_allocator, .{ .enter = guard });
                } else {
                    try frames.append(work_allocator, .{ .match_after_body = .{
                        .ast_expr_idx = data.ast_expr_idx,
                        .cond = data.cond,
                        .branches = data.branches,
                        .branch_offset = data.branch_offset,
                        ...
                        .branch_pat_span = branch_pat_span,
                        .body_free_vars_start = body_free_vars_start,
                        .body_free_vars_start_after_guard = self.scratch_free_vars.top(),
                        .branch_bound_vars_top = branch_bound_vars_top,
                        .saved_defining_patterns_start = saved_defining_patterns_start,
                        .saved_defining_pattern = saved_defining_pattern,
                        .guard = null,
                    ...
                    try frames.append(work_allocator, .{ .enter = ast_branch.body });
                }
            },
            .match_after_guard => |data| {
                const maybe_guard_result = results.get(data.guard) orelse null;
                const can_guard: ?Expr.Idx = if (maybe_guard_result) |can_guard_result| blk: {
                    if (can_guard_result.free_vars.len > 0) {
                        ...
                        self.scratch_free_vars.clearFrom(data.body_free_vars_start);
                        var bound_vars_view = try self.scratch_bound_vars.setViewFrom(data.branch_bound_vars_top, self.env.gpa);
                        defer bound_vars_view.deinit();
                ...
                try frames.append(work_allocator, .{ .match_after_body = .{
                    ...
                    .scratch_top = data.scratch_top,
                    .free_vars_start = data.free_vars_start,
                    .branch_pat_span = data.branch_pat_span,
                    .body_free_vars_start = data.body_free_vars_start,
                    .body_free_vars_start_after_guard = self.scratch_free_vars.top(),
                    .branch_bound_vars_top = data.branch_bound_vars_top,
                    .saved_defining_patterns_start = data.saved_defining_patterns_start,
                    .saved_defining_pattern = data.saved_defining_pattern,
                    .guard = can_guard,
                    .body = data.body,
            ...
            .match_after_body => |data| {
                defer self.scopeExit(self.env.gpa) catch {};
                defer self.scratch_bound_vars.clearFrom(data.branch_bound_vars_top);
                defer self.defining_patterns_start = data.saved_defining_patterns_start;
                defer self.defining_pattern = data.saved_defining_pattern;

                const can_body = (results.get(data.body) orelse null) orelse {
                    const body = self.parse_ir.store.getExpr(data.body);
                ...
                if (can_body.free_vars.len > 0) {
                    ...
                    self.scratch_free_vars.clearFrom(data.body_free_vars_start_after_guard);
                    var bound_vars_view = try self.scratch_bound_vars.setViewFrom(data.branch_bound_vars_top, self.env.gpa);
                    defer bound_vars_view.deinit();
                ...
                try frames.append(work_allocator, .{ .match_branch_start = .{
                    ...
                    .cond = data.cond,
                    .branches = data.branches,
                    .branch_offset = data.branch_offset + 1,
                    .scratch_top = data.scratch_top,
                    .free_vars_start = data.free_vars_start,
                    .region = data.region,
                } });
            },
            .field_access_after_receiver => |data| {
                const receiver_idx = if (results.get(data.field_access.left) orelse null) |can_receiver|
                    can_receiver.idx
                else
                    try self.env.pushMalformed(Expr.Idx, Diagnostic{ .expr_not_canonicalized = .{
                        .region = data.region,
                    } });

                const right_expr = self.parse_ir.store.getExpr(data.field_access.right);
                switch (right_expr) {
                    .apply => |apply| {
                        const method_expr = self.parse_ir.store.getExpr(apply.@"fn");
                        ...
                        const scratch_top = self.env.store.scratchExprTop();
                        try frames.append(work_allocator, .{ .field_access_method_finish = .{
                            .ast_expr_idx = data.ast_expr_idx,
                    ...
                    .ident => |ident| {
                        const field_name = try self.resolveIdentOrUnknown(ident.token);
                        const field_name_region = self.parse_ir.tokenizedRegionToRegion(ident.region);
                        ...
                        const expr_idx = try self.env.addExpr(CIR.Expr{ .e_field_access = .{
                            .receiver = receiver_idx,
                            .field_name = try self.createUnknownIdent(),
                            .field_name_region = data.region,
                        } }, data.region);
                        const free_vars_span = self.scratch_free_vars.spanFrom(data.free_vars_start);
                        try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = free_vars_span });
                    },
            ...
            .field_access_method_finish => |data| {
                var args: ?Expr.Span = null;
                var args_ok = true;
                for (self.parse_ir.store.exprSlice(data.args)) |arg_idx| {
                    if (results.get(arg_idx) orelse null) |canonicalized| {
                ...
                if (args_ok) {
                    args = try self.env.store.exprSpanFrom(data.scratch_top);
                } else {
                    self.env.store.clearScratchExprsFrom(data.scratch_top);
                }

                const dot_access_expr = if (args) |a| CIR.Expr{
                    .e_method_call = .{
                        .receiver = data.receiver,
                        .method_name = data.field_name,
                        .method_name_region = data.field_name_region,
                        .args = a,
                    },
                } else CIR.Expr{
                    .e_field_access = .{
                        ...
                        .field_name_region = data.field_name_region,
                    },
                };

                const expr_idx = try self.env.addExpr(dot_access_expr, data.region);
                const free_vars_span = self.scratch_free_vars.spanFrom(data.free_vars_start);
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = free_vars_span });
            },
            .block_stmt_next => |frame_data| {
                const data = frame_data.data;
                const ast_stmt_idxs = self.parse_ir.store.statementSlice(data.block.statements);
                if (frame_data.stmt_offset >= ast_stmt_idxs.len) {
                    const expr_idx = try self.env.addExpr(CIR.Expr{ .e_empty_record = .{} }, data.region);
                    try frames.append(work_allocator, .{ .block_finish = .{
                        .data = data,
                        .final_expr = CanonicalizedExpr{ .idx = expr_idx, .free_vars = DataSpan.empty() },
                    } });
                    continue;
                }

                const ast_stmt_idx = ast_stmt_idxs[frame_data.stmt_offset];
                const ast_stmt = self.parse_ir.store.getStatement(ast_stmt_idx);
                const is_last = frame_data.stmt_offset == ast_stmt_idxs.len - 1;
                if (is_last and (ast_stmt == .expr or ast_stmt == .dbg or ast_stmt == .@"return" or ast_stmt == .crash)) {
                    switch (ast_stmt) {
                        .expr => |expr_stmt| {
                            try frames.append(work_allocator, .{ .block_after_final_expr = .{
                                .data = data,
                                .child = expr_stmt.expr,
                                .kind = .expr,
                                .region = self.parse_ir.tokenizedRegionToRegion(expr_stmt.region),
                            } });
                            try frames.append(work_allocator, .{ .enter = expr_stmt.expr });
                        },
                        .dbg => |dbg_stmt| {
                            try frames.append(work_allocator, .{ .block_after_final_expr = .{
                                .data = data,
                            ...
                            try frames.append(work_allocator, .{ .enter = dbg_stmt.expr });
                        },
                        .@"return" => |return_stmt| {
                            try frames.append(work_allocator, .{ .block_after_final_expr = .{
                                .data = data,
                                .child = return_stmt.expr,
                                .kind = .@"return",
                                .region = self.parse_ir.tokenizedRegionToRegion(return_stmt.region),
                            } });
                            try frames.append(work_allocator, .{ .enter = return_stmt.expr });
                        },
                        .crash => |crash_stmt| {
                            ...
                            try frames.append(work_allocator, .{ .block_finish = .{
                                .data = data,
                                .final_expr = CanonicalizedExpr{ .idx = crash_expr, .free_vars = DataSpan.empty() },
                            } });
                        },
                        else => unreachable,
                    }
                ...
                switch (ast_stmt) {
                    .expr => |expr_stmt| {
                        try frames.append(work_allocator, .{ .block_after_expr_stmt = .{
                            .data = data,
                            .stmt_offset = frame_data.stmt_offset,
                            .child = expr_stmt.expr,
                            .kind = .expr,
                            .region = self.parse_ir.tokenizedRegionToRegion(expr_stmt.region),
                            .saved_in_expect = self.in_expect,
                        } });
                        try frames.append(work_allocator, .{ .enter = expr_stmt.expr });
                    },
                    .dbg => |dbg_stmt| {
                    ...
                    .expect => |expect_stmt| {
                        ...
                        try frames.append(work_allocator, .{ .block_after_expr_stmt = .{
                            .data = data,
                            .stmt_offset = frame_data.stmt_offset,
                            .child = expect_stmt.body,
                            .kind = .expect,
                            .region = self.parse_ir.tokenizedRegionToRegion(expect_stmt.region),
                            .saved_in_expect = saved_in_expect,
                        } });
                        try frames.append(work_allocator, .{ .enter = expect_stmt.body });
                    },
                    .@"return" => |return_stmt| {
                        try frames.append(work_allocator, .{ .block_after_expr_stmt = .{
                            .data = data,
                    ...
                    .@"var" => |var_stmt| blk: {
                        ...
                        const var_name = self.parse_ir.tokens.resolveIdentifier(var_stmt.name) orelse {
                            ...
                            const malformed_stmt = CanonicalizedStatement{
                            ...
                            try self.addBlockStatement(BlockStatementContext{
                                .captures_top = data.captures_top,
                                .bound_vars_top = data.bound_vars_top,
                            }, malformed_stmt);
                            try frames.append(work_allocator, .{ .block_stmt_next = .{
                        ...
                        try frames.append(work_allocator, .{ .block_after_var_stmt = .{
                            .data = data,
                            .stmt_offset = frame_data.stmt_offset,
                            .stmt_advance = 1,
                            .body = var_stmt.body,
                        ...
                        try frames.append(work_allocator, .{ .enter = var_stmt.body });
                    },
                    .decl => |decl| blk: {
                        const decl_region = self.parse_ir.tokenizedRegionToRegion(decl.region);
                        const ast_decl_pattern = self.parse_ir.store.getPattern(decl.pattern);

                        if (ast_decl_pattern == .ident) {
                            const ident_region = self.parse_ir.tokenizedRegionToRegion(ast_decl_pattern.ident.region);
                            if (self.parse_ir.tokens.resolveIdentifier(ast_decl_pattern.ident.ident_tok)) |ident_idx| {
                                    ...
                                    .found => |existing_pattern_idx| {
                                        if (self.isVarReassignmentAcrossFunctionBoundary(existing_pattern_idx)) {
                                            ...
                                            try self.addBlockStatement(BlockStatementContext{
                                                .captures_top = data.captures_top,
                                                .bound_vars_top = data.bound_vars_top,
                                            }, CanonicalizedStatement{ .idx = reassign_idx, .free_vars = DataSpan.empty() });
                                            try frames.append(work_allocator, .{ .block_stmt_next = .{
                                                .data = data,
                                                .stmt_offset = frame_data.stmt_offset + 1,
                                            } });
                                        ...
                                        if (self.isVarPattern(existing_pattern_idx)) {
                                            try frames.append(work_allocator, .{ .block_after_decl_body = .{
                                                .data = data,
                                                .stmt_offset = frame_data.stmt_offset,
                                                .stmt_advance = 1,
                                                .body = decl.body,
                                            ...
                                            try frames.append(work_allocator, .{ .enter = decl.body });
                                            break :blk;
                                        }
                                    },
                                    .not_found => {},
                    ...
                    .type_anno => |ta| blk: {
                        const anno_result = try self.canonicalizeLocalTypeAnnoStatement(ta.name, ta.anno, ta.where, ta.region);
                        const open_anno = switch (anno_result) {
                            .anno => |anno| anno,
                            .malformed_stmt => |stmt| {
                                try self.addBlockStatement(BlockStatementContext{
                                    .captures_top = data.captures_top,
                        ...
                        const anno_info = open_anno.info;
                        var close_type_var_scope = true;
                        defer if (close_type_var_scope) {
                            self.scopeExitTypeVar(open_anno.type_var_scope);
                        };
                            ...
                            switch (next_stmt) {
                                .decl => |decl| decl_blk: {
                                    ...
                                    const names_match = name_check: {
                                        if (ast_decl_pattern == .ident) {
                                            if (self.parse_ir.tokens.resolveIdentifier(ast_decl_pattern.ident.ident_tok)) |decl_ident| {
                                                break :name_check anno_info.name.eql(decl_ident);
                                            }
                                        }
                                        break :name_check false;
                                    };
                                    if (!names_match) break :decl_blk;

                                    const decl_region = self.parse_ir.tokenizedRegionToRegion(decl.region);
                                    const combined_region = Region{
                                                ...
                                                .found => |existing_pattern_idx| {
                                                    ...
                                                    if (self.isVarPattern(existing_pattern_idx)) {
                                                        try frames.append(work_allocator, .{ .block_after_decl_body = .{
                                                            ...
                                                            .kind = .{ .reassign = .{
                                                                .pattern_idx = existing_pattern_idx,
                                                                .type_var_scope = open_anno.type_var_scope,
                                                            } },
                                                            .region = ident_region,
                                                        } });
                                                        try frames.append(work_allocator, .{ .enter = decl.body });
                                                        close_type_var_scope = false;
                                                        break :blk;
                                                    }
                                                },
                                                .not_found => {},
                                    ...
                                    const pattern_region = self.parse_ir.tokenizedRegionToRegion(ast_decl_pattern.to_tokenized_region());
                                    const annotation_idx = try self.createAnnotationFromTypeAnno(anno_info.anno_idx, anno_info.where, pattern_region);
                                    ...
                                    const is_lambda = self.parserValueDeclIsLambda(next_stmt_id);
                                    if (is_lambda and !self.scratch_local_function_patterns.contains(pattern_idx)) {
                                        try self.scratch_local_function_patterns.append(pattern_idx);
                                    }
                                    ...
                                    try frames.append(work_allocator, .{ .block_after_decl_body = .{
                                        ...
                                        .kind = .{ .decl = .{
                                            .pattern_idx = pattern_idx,
                                            .mb_validated_anno = annotation_idx,
                                            .pattern_reused_existing_var = pattern_reused_existing_var,
                                            .type_var_scope = open_anno.type_var_scope,
                                            .saved_current_local_def_ident = saved_current_local_def_ident,
                                            .saved_current_local_def_index = saved_current_local_def_index,
                                ...
                                .@"var" => |var_stmt| var_blk: {
                                    const var_ident = self.parse_ir.tokens.resolveIdentifier(var_stmt.name) orelse break :var_blk;
                                    if (!anno_info.name.eql(var_ident)) break :var_blk;

                                    const var_region = self.parse_ir.tokenizedRegionToRegion(var_stmt.region);
                                    const annotation_idx = try self.env.addAnnotation(CIR.Annotation{
                                        .anno = anno_info.anno_idx,
                                        .where = anno_info.where,
                                    }, anno_info.anno_region);
                        ...
                        const anno_stmt = try self.canonicalizeLocalAnnoOnlyStatement(anno_info);
                        try self.addBlockStatement(BlockStatementContext{
                            .captures_top = data.captures_top,
                            .bound_vars_top = data.bound_vars_top,
                        }, anno_stmt);
                    ...
                    .@"for" => |for_stmt| {
                        ...
                        try frames.append(work_allocator, .{ .block_for_after_list = .{
                            .data = data,
                            .stmt_offset = frame_data.stmt_offset,
                            .patt = for_stmt.patt,
                            .list_expr = for_stmt.expr,
                            .body = for_stmt.body,
                        ...
                        try frames.append(work_allocator, .{ .enter = for_stmt.expr });
                    },
                    .@"while" => |while_stmt| {
                        try frames.append(work_allocator, .{ .block_while_after_cond = .{
                            .data = data,
                            .stmt_offset = frame_data.stmt_offset,
                            .cond = while_stmt.cond,
                            .body = while_stmt.body,
                            .cond_free_vars_start = self.scratch_free_vars.top(),
                            .captures_top = self.scratch_captures.top(),
                            .region = self.parse_ir.tokenizedRegionToRegion(while_stmt.region),
                        } });
                        try frames.append(work_allocator, .{ .enter = while_stmt.cond });
                    },
                    .type_decl => |type_decl| {
                        switch (try self.canonicalizeLocalTypeDeclStatement(
                            type_decl,
                            ast_stmt_idx,
                            BlockStatementContext{
                                .captures_top = data.captures_top,
                            ...
                            .associated => |associated| {
                                ...
                                try frames.append(work_allocator, .{ .associated_block_enter = associated.request });
                            },
                        }
                    },
                    .crash => |crash_stmt| {
                        const region = self.parse_ir.tokenizedRegionToRegion(crash_stmt.region);
                        const mb_msg_literal = blk_msg: {
                            const msg_expr = self.parse_ir.store.getExpr(crash_stmt.expr);
                            switch (msg_expr) {
                                .string => |s| {
                                    const parts = self.parse_ir.store.exprSlice(s.parts);
                                    if (parts.len > 0) {
                                        ...
                                        if (first_part == .string_part) {
                                            const part_text = self.parse_ir.resolve(first_part.string_part.token);
                                            break :blk_msg try self.env.insertString(part_text);
                                        }
                                    }
                                    break :blk_msg try self.env.insertString("crash");
                                },
                                else => break :blk_msg null,
                            }
                        };
            ...
            .block_after_final_expr => |data| {
                const inner_expr = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.child,
                    results.get(data.child) orelse null,
                    .generic,
                );
                const final_expr = switch (data.kind) {
                    .expr => inner_expr,
                    .dbg => blk: {
                        const dbg_expr = try self.env.addExpr(Expr{ .e_dbg = .{
                            .expr = inner_expr.idx,
                        } }, data.region);
                        break :blk CanonicalizedExpr{ .idx = dbg_expr, .free_vars = inner_expr.free_vars };
                    },
                    .@"return" => blk: {
                        ...
                        break :blk CanonicalizedExpr{ .idx = return_expr_idx, .free_vars = inner_expr.free_vars };
                    },
                };
                try frames.append(work_allocator, .{ .block_finish = .{
                    .data = data.data,
            ...
            .block_after_expr_stmt => |data| {
                defer if (data.kind == .expect) {
                    self.in_expect = data.saved_in_expect;
                };

                const expr = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.child,
                    results.get(data.child) orelse null,
                    .generic,
                ...
                const stmt_idx = switch (data.kind) {
                    .expr => try self.env.addStatement(Statement{ .s_expr = .{
                        .expr = expr.idx,
                    } }, data.region),
                    .dbg => try self.env.addStatement(Statement{ .s_dbg = .{
                        .expr = expr.idx,
                    } }, data.region),
                    .expect => try self.env.addStatement(Statement{ .s_expect = .{
                        .body = expr.idx,
                    } }, data.region),
                ...
                try self.addBlockStatement(BlockStatementContext{
                    .captures_top = data.data.captures_top,
                    .bound_vars_top = data.data.bound_vars_top,
                }, CanonicalizedStatement{ .idx = stmt_idx, .free_vars = expr.free_vars });
                try frames.append(work_allocator, .{ .block_stmt_next = .{
                    .data = data.data,
                    .stmt_offset = data.stmt_offset + 1,
                } });
            },
            .block_after_var_stmt => |data| {
                defer if (data.type_var_scope) |type_var_scope| {
                    self.scopeExitTypeVar(type_var_scope);
                };
                ...
                const pattern_idx = try self.env.addPattern(
                    Pattern{ .assign = .{ .ident = data.var_name } },
                    data.region,
                );
                _ = try self.scopeIntroduceVar(data.var_name, pattern_idx, data.region, true, Pattern.Idx);
                const stmt_idx = try self.env.addStatement(Statement{ .s_var = .{
                    .pattern_idx = pattern_idx,
                    .expr = expr.idx,
                    .anno = data.annotation_idx,
                } }, data.region);
                ...
                try frames.append(work_allocator, .{ .block_stmt_next = .{
                    .data = data.data,
                    .stmt_offset = data.stmt_offset + data.stmt_advance,
                } });
            },
            .block_after_decl_body => |data| {
                const expr = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.body,
                    results.get(data.body) orelse null,
                ...
                const type_var_scope: ?TypeVarScopeIdx = switch (data.kind) {
                    .reassign => |reassign_data| reassign_data.type_var_scope,
                    .decl => |decl_data| decl_data.type_var_scope,
                };
                defer if (type_var_scope) |scope_idx| {
                    self.scopeExitTypeVar(scope_idx);
                };

                const stmt_idx = switch (data.kind) {
                    .reassign => |reassign_data| try self.env.addStatement(Statement{ .s_reassign = .{
                        .pattern_idx = reassign_data.pattern_idx,
                        .expr = expr.idx,
                    } }, data.region),
                    .decl => |decl_data| blk: {
                        self.defining_patterns_start = decl_data.saved_defining_patterns_start;
                        self.defining_pattern = decl_data.saved_defining_pattern;
                        self.current_local_def_ident = decl_data.saved_current_local_def_ident;
                        self.current_local_def_index = decl_data.saved_current_local_def_index;

                        break :blk if (decl_data.pattern_reused_existing_var)
                            try self.env.addStatement(Statement{ .s_reassign = .{
                                .pattern_idx = decl_data.pattern_idx,
                                .expr = expr.idx,
                            } }, data.region)
                        else
                            try self.env.addStatement(Statement{ .s_decl = .{
                                .pattern = decl_data.pattern_idx,
                                .expr = expr.idx,
                                .anno = decl_data.mb_validated_anno,
                            } }, data.region);
                    },
                };
            ...
            .block_for_after_list => |data| {
                ...
                self.loop_depth += 1;
                try frames.append(work_allocator, .{ .block_for_after_body = .{
                    .data = data.data,
                    .stmt_offset = data.stmt_offset,
                    .patt = patt,
                    .list_expr = list_expr.idx,
                    .body = data.body,
                    .body_free_vars_start = self.scratch_free_vars.top(),
            ...
            .block_for_after_body => |data| {
                ...
                try self.addBlockStatement(BlockStatementContext{
                    .captures_top = data.data.captures_top,
                    .bound_vars_top = data.data.bound_vars_top,
                }, CanonicalizedStatement{ .idx = stmt_idx, .free_vars = free_vars });
                try frames.append(work_allocator, .{ .block_stmt_next = .{
                    .data = data.data,
                    .stmt_offset = data.stmt_offset + 1,
                } });
            },
            .block_while_after_cond => |data| {
                const cond = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.cond,
                    results.get(data.cond) orelse null,
                    .generic,
                );
                ...
                self.scratch_free_vars.clearFrom(data.cond_free_vars_start);

                self.loop_depth += 1;
                try frames.append(work_allocator, .{ .block_while_after_body = .{
                    .data = data.data,
                    .stmt_offset = data.stmt_offset,
                    .cond = cond.idx,
                    .body = data.body,
            ...
            .associated_block_enter => |request| {
                try self.scopeEnter(self.env.gpa, false);
                errdefer self.scopeExit(self.env.gpa) catch {};
                try self.declScopeEnter(request.assoc.scope);
                errdefer self.declScopeExit();
                ...
                const associated_scope = self.parse_ir.decl_index.scopes.items[@intFromEnum(request.assoc.scope)];
                const state = try work_allocator.create(AssociatedBlockState);
                state.* = .{
                    .owner_stmt_idx = request.owner_stmt_idx,
                    .parent_name = request.qualified_name_idx,
                    .relative_name_idx = request.relative_name_idx,
                    .type_name = request.type_name,
                    .associated_scope_idx = request.assoc.scope,
                    .stmt_idxs = self.parse_ir.store.statementSlice(request.assoc.statements),
                    .alias_sinks = request.alias_sinks,
                    .block_context = request.block_context,
                    .owner_is_redeclaration = request.owner_is_redeclaration,
                ...
                try frames.append(work_allocator, .{ .associated_block_finish = state });
                try frames.append(work_allocator, .{ .associated_block_next = .{
                    .state = state,
                    .stmt_offset = 0,
                } });
            },
            .associated_block_next => |frame_data| {
                const state = frame_data.state;
                if (frame_data.stmt_offset >= state.stmt_idxs.len) continue;
                ...
                switch (stmt) {
                    .type_decl => |nested_type_decl| {
                        ...
                        if (nested_type_decl.associated) |nested_assoc| {
                            ...
                            try frames.append(work_allocator, .{ .associated_block_enter = .{
                                .owner_stmt_idx = nested_registration.stmt_idx,
                                .qualified_name_idx = nested_qualified_idx,
                                .relative_name_idx = nested_relative_for_block,
                                .type_name = nested_type_ident,
                    ...
                    .type_anno => |ta| {
                        ...
                        if (next_i < state.stmt_idxs.len) {
                            ...
                            if (next_stmt == .decl) {
                                ...
                                const pattern = self.parse_ir.store.getPattern(decl.pattern);
                                if (pattern == .ident) {
                                    if (self.parse_ir.tokens.resolveIdentifier(pattern.ident.ident_tok)) |decl_ident| {
                                        if (name_ident.eql(decl_ident)) {
                                            matched_decl = true;
                                            const decl_region = self.parse_ir.tokenizedRegionToRegion(decl.region);
                                            if (try self.associatedValueWasRedeclared(&state.associated_value_regions, decl_ident, decl_region)) {
                                            ...
                                            try frames.append(work_allocator, .{ .associated_decl_finish = .{
                                                .body = decl.body,
                                                .pattern_idx = pattern_idx,
                                                .pattern_region = pattern_region,
                                                .qualified_idx = qualified_idx,
                                                .decl_ident = decl_ident,
                                                .type_qualified_ident_idx = type_qualified_idx,
                                                .assoc_key = assoc_key,
                                                .annotation_idx = annotation_idx,
                                                .owner_stmt_idx = state.owner_stmt_idx,
                                                .relative_name_idx = state.relative_name_idx,
                                                .alias_sinks = state.alias_sinks,
                                                .block_context = state.block_context,
                                                .owner_is_module_visible = state.owner_is_module_visible,
                                                .saved_stmt_pos = saved_stmt_pos,
                                            } });
                                            try frames.append(work_allocator, .{ .enter = decl.body });
                                        }
                        ...
                        if (pattern == .ident) {
                            if (self.parse_ir.tokens.resolveIdentifier(pattern.ident.ident_tok)) |decl_ident| {
                                ...
                                try frames.append(work_allocator, .{ .associated_decl_finish = .{
                                    ...
                                    .qualified_idx = qualified_idx,
                                    .decl_ident = decl_ident,
                                    .type_qualified_ident_idx = type_qualified_decl_idx,
                                    .assoc_key = assoc_key,
                                    .annotation_idx = null,
                                    .owner_stmt_idx = state.owner_stmt_idx,
                                    .relative_name_idx = state.relative_name_idx,
            ...
            .associated_decl_finish => |data| {
                defer self.in_statement_position = data.saved_stmt_pos;

                const can_expr = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.body,
                    results.get(data.body) orelse null,
                    .generic,
                ...
                const def_idx = try self.env.addDef(CIR.Def{
                    .pattern = data.pattern_idx,
                    .expr = can_expr.idx,
                    .annotation = data.annotation_idx,
                    .kind = .{ .let = {} },
                }, data.pattern_region);
                const associated_def = AssociatedValueDef{ .def_idx = def_idx, .free_vars = can_expr.free_vars };
                const method_binding = try self.recordAssociatedValue(associated_def, data.owner_is_module_visible, data.block_context);

                if (data.owner_is_module_visible) {
                    try self.env.setExposedNodeIndexById(data.qualified_idx, @intFromEnum(def_idx));
                }
                try self.registerAssociatedMethodIdent(data.owner_stmt_idx, data.decl_ident, data.qualified_idx, method_binding);

                const current_scope = &self.scopes.items[self.scopes.items.len - 1];
                try current_scope.idents.put(self.env.gpa, data.decl_ident, data.pattern_idx);

                try self.putAssociatedPatternAliases(
                    data.qualified_idx,
                    data.relative_name_idx,
                    data.type_qualified_ident_idx,
                    data.decl_ident,
                    data.pattern_idx,
                    data.owner_is_module_visible,
                );
                try self.publishAssociatedAliasSinks(data.alias_sinks, data.decl_ident, data.pattern_idx);
            },
            ...
            .block_finish => |data| {
                defer self.scopeExit(self.env.gpa) catch {};
                defer self.declScopeExit();
                defer self.defining_patterns_start = data.data.saved_defining_patterns_start;
                defer self.defining_pattern = data.data.saved_defining_pattern;
                defer self.in_statement_position = data.data.saved_stmt_pos;
                defer self.scratch_bound_vars.clearFrom(data.data.bound_vars_top);
                defer self.scratch_captures.clearFrom(data.data.captures_top);
                defer self.scratch_local_function_patterns.clearFrom(data.data.local_functions_top);
                defer self.scratch_block_local_defs.clearFrom(data.data.block_defs_top);

                try self.classifyBlockLocalForwardRefs(data.data.block_defs_top);

                const final_expr_free_vars_slice = self.scratch_free_vars.sliceFromSpan(data.final_expr.free_vars);
                for (final_expr_free_vars_slice) |fv| {
                    try self.appendPropagatedFreeVarExcludingBound(data.data.captures_top, data.data.bound_vars_top, fv);
                }

                const captures_slice = self.scratch_captures.sliceFromStart(data.data.captures_top);
                self.scratch_free_vars.clearFrom(data.data.free_vars_top);
                ...
                const stmt_span = try self.env.store.statementSpanFrom(data.data.stmt_start);
                const block_idx = try self.env.addExpr(CIR.Expr{ .e_block = .{
                    .stmts = stmt_span,
                    .final_expr = data.final_expr.idx,
                } }, data.data.region);

                try results.put(work_allocator, data.data.ast_expr_idx, CanonicalizedExpr{ .idx = block_idx, .free_vars = block_free_vars });
            },
            .tuple_one_finish => |data| {
                try results.put(work_allocator, data.ast_expr_idx, results.get(data.item) orelse null);
            },
            .tuple_finish => |data| {
                for (self.parse_ir.store.exprSlice(data.items)) |item| {
                    const item_expr_idx = try canonicalizedExprFromResultOrMalformed(
                        self,
                        item,
                        results.get(item) orelse null,
                        .tuple_elem,
                    );
                    try self.env.store.addScratchExpr(item_expr_idx.idx);
                }
                const elems_span = try self.env.store.exprSpanFrom(data.scratch_top);
                const expr_idx = try self.env.addExpr(CIR.Expr{ .e_tuple = .{ .elems = elems_span } }, data.region);
                const free_vars_span = self.scratch_free_vars.spanFrom(data.free_vars_start);
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = free_vars_span });
            },
            .tuple_access_finish => |data| {
                const can_tuple = (results.get(data.tuple_expr) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };

                const elem_index_str = self.parse_ir.resolve(data.elem_token);
            ...
            .method_call_finish => |data| {
                const can_receiver = (results.get(data.receiver) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };

                const method_name = self.parse_ir.tokens.resolveIdentifier(data.method_token) orelse {
                    const malformed_idx = try self.env.pushMalformed(Expr.Idx, Diagnostic{ .expr_not_canonicalized = .{
                        .region = data.region,
                    } });
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = malformed_idx, .free_vars = DataSpan.empty() });
                ...
                const raw_method_region = self.parse_ir.tokens.resolve(data.method_token);
            ...
            .arrow_after_left => |data| {
                const can_first_arg = (results.get(data.arrow_call.left) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };

                const right_expr = self.parse_ir.store.getExpr(data.arrow_call.right);
                switch (right_expr) {
                    .apply => |apply| {
                        ...
                        if (ast_fn == .tag) {
                            ...
                            try frames.append(work_allocator, .{ .arrow_tag_args_finish = .{
                                .ast_expr_idx = data.ast_expr_idx,
                                .tag_name = tag_name,
                                .args = apply.args,
                                .scratch_top = scratch_top,
                            ...
                            while (i > 0) {
                                i -= 1;
                                try frames.append(work_allocator, .{ .enter = additional_args[i] });
                            }
                            continue;
                        }

                        try frames.append(work_allocator, .{ .arrow_after_apply_fn = .{
                            .ast_expr_idx = data.ast_expr_idx,
                            .left = data.arrow_call.left,
                            .fn_idx = apply.@"fn",
                            .args = apply.args,
                            .free_vars_start = data.free_vars_start,
                            .region = data.region,
                        } });
                        try frames.append(work_allocator, .{ .enter = apply.@"fn" });
                    },
                    .ident, .tag => {
                        if (right_expr == .tag) {
                            ...
                            const expr_idx = try self.env.addExpr(CIR.Expr{ .e_tag = .{
                                .name = tag_name,
                                .args = args_span,
                            } }, data.region);
                            const free_vars_span = self.scratch_free_vars.spanFrom(data.free_vars_start);
                        ...
                        try frames.append(work_allocator, .{ .arrow_call_single_finish = .{
                            .ast_expr_idx = data.ast_expr_idx,
                            .left = data.arrow_call.left,
                            .right = data.arrow_call.right,
                            .free_vars_start = data.free_vars_start,
                            .region = data.region,
                        } });
                        try frames.append(work_allocator, .{ .enter = data.arrow_call.right });
                    },
                    else => {
                        try frames.append(work_allocator, .{ .arrow_call_single_finish = .{
                            .ast_expr_idx = data.ast_expr_idx,
            ...
            .arrow_after_apply_fn => |data| {
                const can_first_arg = (results.get(data.left) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };
                const can_fn_expr = (results.get(data.fn_idx) orelse null) orelse {
                ...
                const additional_args = self.parse_ir.store.exprSlice(data.args);
                var i = additional_args.len;
                while (i > 0) {
                    i -= 1;
                    try frames.append(work_allocator, .{ .enter = additional_args[i] });
            ...
            .arrow_call_args_finish => |data| {
                ...
                const args_span = try self.env.store.exprSpanFrom(data.scratch_top);
                const expr_idx = try self.env.addExpr(CIR.Expr{ .e_call = .{
                    .func = data.func,
                    .args = args_span,
                    .called_via = CalledVia.apply,
                } }, data.region);
            ...
            .arrow_call_single_finish => |data| {
                const can_first_arg = (results.get(data.left) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };
                const can_fn_expr = (results.get(data.right) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };
            ...
            .dbg_finish => |data| {
                const can_inner = (results.get(data.child) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };
                const dbg_expr = try self.env.addExpr(Expr{ .e_dbg = .{ .expr = can_inner.idx } }, data.region);
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = dbg_expr, .free_vars = can_inner.free_vars });
            },
            .unary_finish => |data| {
                const can_operand = (results.get(data.child) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };
                const operator_token = self.parse_ir.tokens.tokens.get(data.operator);
                switch (operator_token.tag) {
                    .OpUnaryMinus => {
                        const expr_idx = try self.env.addExpr(Expr{ .e_unary_minus = Expr.UnaryMinus.init(can_operand.idx) }, data.region);
                        try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = can_operand.free_vars });
                    },
                    .OpBang => {
                        const expr_idx = try self.env.addExpr(Expr{ .e_unary_not = Expr.UnaryNot.init(can_operand.idx) }, data.region);
                        try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = can_operand.free_vars });
                    },
                    else => {
            ...
            .binop_finish => |data| {
                const can_lhs = (results.get(data.binop.left) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };
                const can_rhs = (results.get(data.binop.right) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };

                const op_token = self.parse_ir.tokens.tokens.get(data.binop.operator);
                const op: Expr.Binop.Op = switch (op_token.tag) {
                    ...
                    .OpDoubleQuestion => {
                        try results.put(work_allocator, data.ast_expr_idx, try self.canonicalizeDoubleQuestionOp(
                            data.binop,
                            data.region,
                            can_lhs,
                            can_rhs,
                            data.free_vars_start,
                        ));
                        continue;
                    },
                    else => {
                ...
                if (op == .@"and" or op == .@"or") {
                    ...
                    const if_free_vars = self.scratch_free_vars.spanFrom(data.free_vars_start);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = if_expr_idx, .free_vars = if_free_vars });
                    continue;
                }
            ...
            .single_question_after_left => |data| {
                const can_lhs = (results.get(data.binop.left) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };

                const rhs_ast = self.parse_ir.store.getExpr(data.binop.right);
                if (rhs_ast == .tag) {
                    try results.put(
                        work_allocator,
                        data.ast_expr_idx,
                        try self.canonicalizeSingleQuestionBinop(data.binop, data.region, can_lhs, null, data.free_vars_start),
                    );
                    continue;
                }

                try frames.append(work_allocator, .{ .single_question_finish = .{
                    .ast_expr_idx = data.ast_expr_idx,
                    .binop = data.binop,
                    .free_vars_start = data.free_vars_start,
                    .region = data.region,
                } });
                try frames.append(work_allocator, .{ .enter = data.binop.right });
            },
            .single_question_finish => |data| {
                const can_lhs = (results.get(data.binop.left) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };
                ...
                try results.put(
                    work_allocator,
                    data.ast_expr_idx,
                    try self.canonicalizeSingleQuestionBinop(data.binop, data.region, can_lhs, can_rhs.idx, data.free_vars_start),
                );
            },
            .suffix_question_finish => |data| {
                const can_cond = (results.get(data.child) orelse null) orelse {
                    try results.put(work_allocator, data.ast_expr_idx, null);
                    continue;
                };
                ...
                const expr_idx = try self.env.addExpr(CIR.Expr{ .e_match = match_expr }, data.region);

                const free_vars_span = self.scratch_free_vars.spanFrom(data.free_vars_start);
                try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = expr_idx, .free_vars = free_vars_span });
            },
            .if_chain_next => |data| {
                const current_if = self.parse_ir.store.getExpr(data.current_if_idx).if_then_else;
                try frames.append(work_allocator, .{ .if_after_cond = .{
                    .ast_expr_idx = data.ast_expr_idx,
                    .current_if_idx = data.current_if_idx,
                    .captures_top = data.captures_top,
                    .free_vars_start = data.free_vars_start,
                    .scratch_top = data.scratch_top,
                    .region = data.region,
                } });
                try frames.append(work_allocator, .{ .enter = current_if.condition });
            },
            .if_after_cond => |data| {
                const current_if = self.parse_ir.store.getExpr(data.current_if_idx).if_then_else;
                const can_cond = try canonicalizedExprFromResultOrMalformed(
                    self,
                    current_if.condition,
                    results.get(current_if.condition) orelse null,
                    .if_condition,
                );
                if ((results.get(current_if.condition) orelse null) == null) {
                    self.scratch_captures.clearFrom(data.captures_top);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = can_cond.idx, .free_vars = DataSpan.empty() });
                    continue;
                }
                ...
                try frames.append(work_allocator, .{ .if_after_then = .{
                    .ast_expr_idx = data.ast_expr_idx,
                    .current_if_idx = data.current_if_idx,
                    .captures_top = data.captures_top,
                    .free_vars_start = data.free_vars_start,
                ...
                try frames.append(work_allocator, .{ .enter = current_if.then });
            },
            .if_after_then => |data| {
                const current_if = self.parse_ir.store.getExpr(data.current_if_idx).if_then_else;
                const can_cond = (results.get(current_if.condition) orelse null).?;
                const can_then = try canonicalizedExprFromResultOrMalformed(
                    self,
                    current_if.then,
                    results.get(current_if.then) orelse null,
                    .if_then,
                );
                if ((results.get(current_if.then) orelse null) == null) {
                    self.scratch_captures.clearFrom(data.captures_top);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = can_then.idx, .free_vars = DataSpan.empty() });
                    continue;
                }
                ...
                if (else_expr == .if_then_else) {
                    try frames.append(work_allocator, .{ .if_chain_next = .{
                        .ast_expr_idx = data.ast_expr_idx,
                        .current_if_idx = current_if.@"else",
                        .captures_top = data.captures_top,
                        .free_vars_start = data.free_vars_start,
                        .scratch_top = data.scratch_top,
                        .region = data.region,
                    ...
                    try frames.append(work_allocator, .{ .if_after_else = .{
                        .ast_expr_idx = data.ast_expr_idx,
                        .else_idx = current_if.@"else",
                        .captures_top = data.captures_top,
                        .free_vars_start = data.free_vars_start,
                        .scratch_top = data.scratch_top,
                        .region = data.region,
                    } });
                    try frames.append(work_allocator, .{ .enter = current_if.@"else" });
                }
            },
            .if_after_else => |data| {
                const can_else = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.else_idx,
                    results.get(data.else_idx) orelse null,
                    .if_else,
                );
                if ((results.get(data.else_idx) orelse null) == null) {
                    self.scratch_captures.clearFrom(data.captures_top);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = can_else.idx, .free_vars = DataSpan.empty() });
                    continue;
                }
                ...
                const branches_span = try self.env.store.ifBranchSpanFrom(data.scratch_top);
                const expr_idx = try self.env.addExpr(CIR.Expr{
                    .e_if = .{
                        .branches = branches_span,
                ...
                self.scratch_free_vars.clearFrom(data.free_vars_start);
            ...
            .if_without_else_after_cond => |data| {
                const can_cond = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.condition,
                    results.get(data.condition) orelse null,
                    .if_condition,
                );
                if ((results.get(data.condition) orelse null) == null) {
                    self.scratch_captures.clearFrom(data.captures_top);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = can_cond.idx, .free_vars = DataSpan.empty() });
                    continue;
                }
                ...
                try frames.append(work_allocator, .{ .if_without_else_after_then = .{
                    .ast_expr_idx = data.ast_expr_idx,
                ...
                try frames.append(work_allocator, .{ .enter = data.then });
            },
            .if_without_else_after_then => |data| {
                const can_cond = (results.get(data.condition) orelse null).?;
                const can_then = try canonicalizedExprFromResultOrMalformed(
                    self,
                    data.then,
                    results.get(data.then) orelse null,
                    .if_then,
                );
                if ((results.get(data.then) orelse null) == null) {
                    self.scratch_captures.clearFrom(data.captures_top);
                    try results.put(work_allocator, data.ast_expr_idx, CanonicalizedExpr{ .idx = can_then.idx, .free_vars = DataSpan.empty() });
                    continue;
                }
                ...
                const empty_record_idx = try self.env.addExpr(CIR.Expr{ .e_empty_record = .{} }, data.region);
                const scratch_top = self.env.store.scratchIfBranchTop();
                const if_branch = Expr.IfBranch{
                    .cond = can_cond.idx,
                    .body = can_then.idx,
                };
                const if_branch_idx = try self.env.addIfBranch(if_branch, data.region);
                try self.env.store.addScratchIfBranch(if_branch_idx);
    ...
    return results.get(ast_expr_idx) orelse null;
}

fn canonicalizeIdentExprShallow(
    self: *Self,
...
/// Canonicalize an expression variant that does not need child-expression traversal.
fn canonicalizeExprShallow(
    self: *Self,
    ast_expr_idx: AST.Expr.Idx,
) std.mem.Allocator.Error!?CanonicalizedExpr {
    const trace = tracy.trace(@src());
    ...
    switch (expr) {
        .ident => |e| return try self.canonicalizeIdentExprShallow(e),
        .int => |e| {
            const region = self.parse_ir.tokenizedRegionToRegion(e.region);
...
fn canonicalizeSingleQuestionBinop(
    ...
    std.debug.assert(rhs_is_bare_tag or can_rhs_idx != null);
...
fn canonicalizeTagExprWithArgsSpan(
    self: *Self,
    e: AST.TagExpr,
    args_span: Expr.Span,
    region: base.Region,
...
fn addInterpolationResultToScratch(self: *Self, part_node: AST.Expr, result: ?CanonicalizedExpr) std.mem.Allocator.Error!void {
    if (result) |can_expr| {
...
fn finishTagPattern(
    self: *Self,
    e: @TypeOf(@as(AST.Pattern, undefined).tag),
...
pub fn canonicalizePattern(
    ...
    const Frame = union(enum) {
        enter: AST.Pattern.Idx,
        tag_finish: struct { ast_pattern_idx: AST.Pattern.Idx, scratch_top: u32 },
        tuple_finish: struct { ast_pattern_idx: AST.Pattern.Idx, scratch_top: u32 },
        record_next: struct { ast_pattern_idx: AST.Pattern.Idx, scratch_top: u32, field_offset: usize },
        record_after_child: struct {
            ast_pattern_idx: AST.Pattern.Idx,
            scratch_top: u32,
            next_field_offset: usize,
            field_name_ident: Ident.Idx,
            field_region: Region,
            child_idx: AST.Pattern.Idx,
        },
        list_next: struct {
            ast_pattern_idx: AST.Pattern.Idx,
            scratch_top: u32,
        ...
        list_after_child: struct {
            ast_pattern_idx: AST.Pattern.Idx,
            scratch_top: u32,
            next_pattern_offset: usize,
            rest_index: ?u32,
            rest_pattern: ?Pattern.Idx,
            child_idx: AST.Pattern.Idx,
        },
        as_finish: struct { ast_pattern_idx: AST.Pattern.Idx, child_idx: AST.Pattern.Idx },
    };

    var results: std.AutoHashMapUnmanaged(AST.Pattern.Idx, ?Pattern.Idx) = .{};
    defer results.deinit(self.env.gpa);

    var frames: std.ArrayList(Frame) = .empty;
    defer frames.deinit(self.env.gpa);
    try frames.append(self.env.gpa, .{ .enter = ast_pattern_idx });
    ...
    while (frames.pop()) |frame| {
        switch (frame) {
            .enter => |current_idx| {
                ...
                switch (self.parse_ir.store.getPattern(current_idx)) {
                    .ident => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        const result = if (self.parse_ir.tokens.resolveIdentifier(e.ident_tok)) |ident_idx| blk: {
                            const current_scope = &self.scopes.items[self.scopes.items.len - 1];
                            ...
                            while (s_idx > 0) {
                                ...
                                if (scope_ptr.forward_references.fetchRemove(ident_idx)) |kv| {
                                    ...
                                    break :blk kv.value.pattern_idx;
                                }
                            }
                            ...
                            if (placeholder_exists) {
                                ...
                                switch (try self.scopeIntroduceInternal(self.env.gpa, .ident, ident_idx, pattern_idx, false, false)) {
                                    ...
                                    .top_level_var_error => {
                                        break :blk try self.env.pushMalformed(Pattern.Idx, Diagnostic{
                                    ...
                                    .var_across_function_boundary => {
                                        break :blk try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .var_across_function_boundary = .{
                                            .region = region,
                                        } });
                                    },
                                    .var_reassignment_ok => |existing_pattern_idx| {
                                        self.pattern_reused_existing_var = true;
                                        break :blk existing_pattern_idx;
                                    },
                                }
                            }
                            ...
                            const feature = try self.env.insertString("report an error when unable to resolve identifier");
                            break :blk try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .not_implemented = .{
                                .feature = feature,
                                .region = region,
                            } });
                        };
                        try results.put(self.env.gpa, current_idx, result);
                    },
                    .typed_int => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        const feature = try self.env.insertString("typed_int pattern");
                        try results.put(self.env.gpa, current_idx, try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .not_implemented = .{ .feature = feature, .region = region } }));
                    },
                    .typed_frac => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                    ...
                    .var_ident => |e| {
                        ...
                        const result = if (self.parse_ir.tokens.resolveIdentifier(e.ident_tok)) |ident_idx| blk: {
                            ...
                            const introduced = try self.scopeIntroduceVar(
                                ident_idx,
                                pattern_idx,
                                region,
                            ...
                            if (self.allow_pattern_var_reuse and introduced == pattern_idx and self.isVarPattern(pattern_idx)) {
                                // Fresh mutable binder in a mixed declaration pattern; no-op.
                            } else if (introduced != pattern_idx) {
                                self.pattern_reused_existing_var = true;
                            }
                            break :blk introduced;
                        } else blk: {
                            const feature = try self.env.insertString("report an error when unable to resolve identifier");
                            break :blk try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .not_implemented = .{
                                .feature = feature,
                    ...
                    .underscore => |p| {
                        ...
                        try results.put(self.env.gpa, current_idx, try self.env.addPattern(Pattern{ .underscore = {} }, region));
                    },
                    .int => |e| {
                        const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                        const literal = self.parse_ir.store.getNumericLiteral(e.literal);
                        const result = switch (literal.compact) {
                    ...
                    .string => |e| {
                        ...
                        const result = switch (str_expr) {
                            .string => |se| blk: {
                                ...
                                if (parts.len == 1) {
                                    ...
                                    if (part == .string_part) {
                                        ...
                                        break :blk try self.env.addPattern(Pattern{ .str_literal = .{
                                            .literal = literal,
                                        } }, region);
                                    }
                                }
                            ...
                            else => try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .pattern_arg_invalid = .{
                                .region = region,
                            } }),
                        };
                        try results.put(self.env.gpa, current_idx, result);
                    ...
                    .single_quote => |e| {
                        try results.put(self.env.gpa, current_idx, try self.canonicalizeSingleQuote(e.region, e.token, Pattern.Idx));
                    },
                    .tag => |e| {
                        if (self.parse_ir.tokens.resolveIdentifier(e.tag_tok) == null) {
                            try results.put(self.env.gpa, current_idx, null);
                            continue;
                        }

                        const args = self.parse_ir.store.patternSlice(e.args);
                        const scratch_top = self.env.store.scratchPatternTop();
                        try frames.append(self.env.gpa, .{ .tag_finish = .{ .ast_pattern_idx = current_idx, .scratch_top = scratch_top } });
                        var i = args.len;
                        while (i > 0) {
                            i -= 1;
                            try frames.append(self.env.gpa, .{ .enter = args[i] });
                        }
                    },
                    .record => {
                        try frames.append(self.env.gpa, .{ .record_next = .{
                            .ast_pattern_idx = current_idx,
                            .scratch_top = self.env.store.scratchRecordDestructTop(),
                            .field_offset = 0,
                        } });
                    },
                    .tuple => |e| {
                        const patterns = self.parse_ir.store.patternSlice(e.patterns);
                        const scratch_top = self.env.store.scratchPatternTop();
                        try frames.append(self.env.gpa, .{ .tuple_finish = .{ .ast_pattern_idx = current_idx, .scratch_top = scratch_top } });
                        var i = patterns.len;
                        while (i > 0) {
                            i -= 1;
                            try frames.append(self.env.gpa, .{ .enter = patterns[i] });
                        }
                    },
                    .list => {
                        try frames.append(self.env.gpa, .{ .list_next = .{
                            .ast_pattern_idx = current_idx,
                    ...
                    .list_rest => |e| {
                        ...
                        try results.put(self.env.gpa, current_idx, try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .not_implemented = .{
                            .feature = feature,
                            .region = region,
                        } }));
                    },
                    ...
                    .as => |e| {
                        try frames.append(self.env.gpa, .{ .as_finish = .{ .ast_pattern_idx = current_idx, .child_idx = e.pattern } });
                        try frames.append(self.env.gpa, .{ .enter = e.pattern });
                    },
                    .malformed => {
                        try results.put(self.env.gpa, current_idx, null);
                    },
            ...
            .tag_finish => |data| {
                const e = self.parse_ir.store.getPattern(data.ast_pattern_idx).tag;
                const tag_name = self.parse_ir.tokens.resolveIdentifier(e.tag_tok) orelse unreachable;
                const region = self.parse_ir.tokenizedRegionToRegion(e.region);

                for (self.parse_ir.store.patternSlice(e.args)) |sub_ast_pattern_idx| {
                    if (results.get(sub_ast_pattern_idx) orelse null) |idx| {
                ...
                const args = try self.env.store.patternSpanFrom(data.scratch_top);
                const tag_pattern_idx = try self.env.addPattern(Pattern{ .applied_tag = .{
                    .name = tag_name,
                    .args = args,
                } }, region);
                try results.put(self.env.gpa, data.ast_pattern_idx, try self.finishTagPattern(e, tag_pattern_idx, region));
            },
            .tuple_finish => |data| {
                const e = self.parse_ir.store.getPattern(data.ast_pattern_idx).tuple;
                const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                for (self.parse_ir.store.patternSlice(e.patterns)) |child_idx| {
                    if (results.get(child_idx) orelse null) |canonicalized| {
                        try self.env.store.addScratchPattern(canonicalized);
                    }
                }
                const patterns_span = try self.env.store.patternSpanFrom(data.scratch_top);
                try results.put(self.env.gpa, data.ast_pattern_idx, try self.env.addPattern(Pattern{ .tuple = .{
                    .patterns = patterns_span,
                } }, region));
            },
            .record_next => |data| {
                const e = self.parse_ir.store.getPattern(data.ast_pattern_idx).record;
                const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                const fields = self.parse_ir.store.patternRecordFieldSlice(e.fields);
                if (data.field_offset >= fields.len) {
                    const destructs_span = try self.env.store.recordDestructSpanFrom(data.scratch_top);
                    try results.put(self.env.gpa, data.ast_pattern_idx, try self.env.addPattern(Pattern{ .record_destructure = .{
                        .destructs = destructs_span,
                    } }, region));
                    continue;
                }

                const field = self.parse_ir.store.getPatternRecordField(fields[data.field_offset]);
                ...
                if (field.rest and field.name == 0) {
                    ...
                    const destruct_idx = try self.env.addRecordDestruct(.{
                    ...
                    try frames.append(self.env.gpa, .{ .record_next = .{
                        .ast_pattern_idx = data.ast_pattern_idx,
                        .scratch_top = data.scratch_top,
                        .field_offset = data.field_offset + 1,
                    } });
                    continue;
                ...
                const field_name_ident = self.parse_ir.tokens.resolveIdentifier(field.name) orelse {
                    ...
                    try results.put(self.env.gpa, data.ast_pattern_idx, try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .not_implemented = .{
                        .feature = feature,
                        .region = field_region,
                    } }));
                    continue;
                ...
                if (field.value) |sub_pattern_idx| {
                    try frames.append(self.env.gpa, .{ .record_after_child = .{
                        .ast_pattern_idx = data.ast_pattern_idx,
                        .scratch_top = data.scratch_top,
                        .next_field_offset = data.field_offset + 1,
                        .field_name_ident = field_name_ident,
                        .field_region = field_region,
                        .child_idx = sub_pattern_idx,
                    } });
                    try frames.append(self.env.gpa, .{ .enter = sub_pattern_idx });
                } else {
                    const assign_pattern_idx = try self.env.addPattern(Pattern{ .assign = .{ .ident = field_name_ident } }, field_region);
                    const destruct_idx = try self.env.addRecordDestruct(.{
                        .label = field_name_ident,
                        .ident = field_name_ident,
                        .kind = if (field.rest) .{ .Rest = assign_pattern_idx } else .{ .Required = assign_pattern_idx },
                    }, field_region);
                    try self.env.store.addScratchRecordDestruct(destruct_idx);
                    ...
                    switch (try self.scopeIntroduceInternal(self.env.gpa, .ident, field_name_ident, assign_pattern_idx, false, true)) {
                        ...
                        .top_level_var_error => {
                            try results.put(self.env.gpa, data.ast_pattern_idx, try self.env.pushMalformed(Pattern.Idx, Diagnostic{
                                .invalid_top_level_statement = .{
                                    .stmt = try self.env.insertString("var"),
                                    .region = field_region,
                                },
                        ...
                        .var_across_function_boundary => {
                            try results.put(self.env.gpa, data.ast_pattern_idx, try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .ident_already_in_scope = .{
                                .ident = field_name_ident,
                                .region = field_region,
                            } }));
                            continue;
            ...
            .record_after_child => |data| {
                const canonicalized_sub_pattern = (results.get(data.child_idx) orelse null) orelse
                    try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .pattern_not_canonicalized = .{
                        .region = data.field_region,
                    } });

                const destruct_idx = try self.env.addRecordDestruct(.{
                    .label = data.field_name_ident,
                    .ident = data.field_name_ident,
            ...
            .list_next => |data| {
                const e = self.parse_ir.store.getPattern(data.ast_pattern_idx).list;
                const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                const patterns = self.parse_ir.store.patternSlice(e.patterns);
                if (data.pattern_offset >= patterns.len) {
                    const patterns_span = try self.env.store.patternSpanFrom(data.scratch_top);
                    try results.put(self.env.gpa, data.ast_pattern_idx, try self.env.addPattern(Pattern{ .list = .{
                        .patterns = patterns_span,
                        .rest_info = if (data.rest_index) |idx| .{ .index = idx, .pattern = data.rest_pattern } else null,
                    } }, region));
                    continue;
                }

                const child_idx = patterns[data.pattern_offset];
                const ast_pattern = self.parse_ir.store.getPattern(child_idx);
                if (ast_pattern == .list_rest) {
                    if (data.rest_index != null) {
                        ...
                        try frames.append(self.env.gpa, .{ .list_next = .{
                            .ast_pattern_idx = data.ast_pattern_idx,
                            .scratch_top = data.scratch_top,
                            .pattern_offset = data.pattern_offset + 1,
                            .rest_index = data.rest_index,
                            .rest_pattern = data.rest_pattern,
                        } });
                        continue;
                    }
                    ...
                    const patterns_so_far = self.env.store.scratch.?.patterns.top() - data.scratch_top;
                    try frames.append(self.env.gpa, .{ .list_next = .{
                        .ast_pattern_idx = data.ast_pattern_idx,
                        .scratch_top = data.scratch_top,
                        .pattern_offset = data.pattern_offset + 1,
                    ...
                    try frames.append(self.env.gpa, .{ .list_after_child = .{
                        .ast_pattern_idx = data.ast_pattern_idx,
                        .scratch_top = data.scratch_top,
                        .next_pattern_offset = data.pattern_offset + 1,
                        .rest_index = data.rest_index,
                        .rest_pattern = data.rest_pattern,
                        .child_idx = child_idx,
                    } });
                    try frames.append(self.env.gpa, .{ .enter = child_idx });
                }
            },
            .list_after_child => |data| {
                if (results.get(data.child_idx) orelse null) |canonicalized| {
                    try self.env.store.addScratchPattern(canonicalized);
                } else {
                    const pattern_region = self.parse_ir.tokenizedRegionToRegion(self.parse_ir.store.getPattern(data.child_idx).to_tokenized_region());
                    const malformed_idx = try self.env.pushMalformed(Pattern.Idx, Diagnostic{ .pattern_not_canonicalized = .{
                        .region = pattern_region,
                    } });
                    try self.env.store.addScratchPattern(malformed_idx);
                ...
                try frames.append(self.env.gpa, .{ .list_next = .{
                    .ast_pattern_idx = data.ast_pattern_idx,
                    .scratch_top = data.scratch_top,
                    .pattern_offset = data.next_pattern_offset,
                    .rest_index = data.rest_index,
                    .rest_pattern = data.rest_pattern,
                } });
            },
            .as_finish => |data| {
                const e = self.parse_ir.store.getPattern(data.ast_pattern_idx).as;
                const region = self.parse_ir.tokenizedRegionToRegion(e.region);
                const inner_pattern = (results.get(data.child_idx) orelse null) orelse blk: {
                ...
                const ident_idx = self.parse_ir.tokens.resolveIdentifier(e.name) orelse {
                ...
                const pattern_idx = try self.env.addPattern(Pattern{ .as = .{
                ...
                try results.put(self.env.gpa, data.ast_pattern_idx, pattern_idx);
            },
        }
    }

    return results.get(ast_pattern_idx) orelse null;
}
...
fn canonicalizeTypeAnnoHelp(self: *Self, anno_idx: AST.TypeAnno.Idx, type_anno_ctx: *TypeAnnoCtx) std.mem.Allocator.Error!TypeAnno.Idx {
    ...
    const Mode = enum { normal, tag };
    const Phase = enum { enter, exit };
    const Frame = struct {
    ...
    var normal_results: std.AutoHashMapUnmanaged(AST.TypeAnno.Idx, TypeAnno.Idx) = .{};
    defer normal_results.deinit(self.env.gpa);

    var tag_results: std.AutoHashMapUnmanaged(AST.TypeAnno.Idx, TypeAnno.Idx) = .{};
    defer tag_results.deinit(self.env.gpa);

    var frames: std.ArrayList(Frame) = .empty;
    defer frames.deinit(self.env.gpa);

    try frames.append(self.env.gpa, .{ .anno_idx = anno_idx, .mode = .normal, .phase = .enter });
    while (frames.pop()) |frame| {
        const result_map = switch (frame.mode) {
            .normal => &normal_results,
            .tag => &tag_results,
        };
        if (result_map.contains(frame.anno_idx)) continue;

        const ast_anno = self.parse_ir.store.getTypeAnno(frame.anno_idx);
        switch (frame.phase) {
            .enter => switch (frame.mode) {
                .normal => switch (ast_anno) {
                    .apply => |apply| {
                        ...
                        if (args_slice.len == 0) {
                            const malformed = try self.env.pushMalformed(TypeAnno.Idx, Diagnostic{ .malformed_type_annotation = .{ .region = region } });
                            try normal_results.put(self.env.gpa, frame.anno_idx, malformed);
                            continue;
                        }
                        switch (self.parse_ir.store.getTypeAnno(args_slice[0])) {
                            .ty => |ty| {
                                const base_anno_idx = try self.canonicalizeTypeAnnoBasicType(ty);
                                try normal_results.put(self.env.gpa, args_slice[0], base_anno_idx);
                            },
                            else => {
                                const malformed = try self.env.pushMalformed(TypeAnno.Idx, Diagnostic{ .malformed_type_annotation = .{ .region = region } });
                                try normal_results.put(self.env.gpa, frame.anno_idx, malformed);
                        ...
                        try frames.append(self.env.gpa, .{ .anno_idx = frame.anno_idx, .mode = .normal, .phase = .exit });
                        var i = args_slice.len;
                        while (i > 1) {
                            i -= 1;
                            try frames.append(self.env.gpa, .{ .anno_idx = args_slice[i], .mode = .normal, .phase = .enter });
                        }
                    },
                    .ty_var => |ty_var| {
                        ...
                        const result = switch (self.scopeLookupTypeVar(name_ident)) {
                            .found => |found_anno_idx| blk: {
                                try self.scratch_type_var_validation.append(name_ident);
                                break :blk try self.env.addTypeAnno(.{ .rigid_var_lookup = .{ .ref = found_anno_idx } }, region);
                            },
                            .not_found => blk: {
                                const can_introduce = type_anno_ctx.canIntroduceTypeVar(name_ident.attributes.ignored);
                                if (can_introduce) {
                                    try self.scratch_type_var_validation.append(name_ident);
                                    const new_anno_idx = try self.env.addTypeAnno(.{ .rigid_var = .{ .name = name_ident } }, region);
                        ...
                        try normal_results.put(self.env.gpa, frame.anno_idx, result);
                    },
                    ...
                    .ty => |ty| {
                        try normal_results.put(self.env.gpa, frame.anno_idx, try self.canonicalizeTypeAnnoBasicType(ty));
                    },
                    .underscore => |underscore| {
                        ...
                        try normal_results.put(self.env.gpa, frame.anno_idx, try self.env.addTypeAnno(.{ .underscore = {} }, region));
                    },
                    .tuple => |tuple| {
                        ...
                        while (i > 0) {
                            i -= 1;
                            try frames.append(self.env.gpa, .{ .anno_idx = elems[i], .mode = .normal, .phase = .enter });
                        }
                    },
                    .record => |record| {
                        try frames.append(self.env.gpa, .{ .anno_idx = frame.anno_idx, .mode = .normal, .phase = .exit });
                        if (record.ext == .named) {
                            try frames.append(self.env.gpa, .{ .anno_idx = record.ext.named.anno, .mode = .normal, .phase = .enter });
                        }
                        ...
                        while (i > 0) {
                            ...
                            try frames.append(self.env.gpa, .{ .anno_idx = field.ty, .mode = .normal, .phase = .enter });
                        }
                    },
                    .tag_union => |tag_union| {
                        try frames.append(self.env.gpa, .{ .anno_idx = frame.anno_idx, .mode = .normal, .phase = .exit });
                        if (tag_union.ext == .named) {
                            try frames.append(self.env.gpa, .{ .anno_idx = tag_union.ext.named.anno, .mode = .normal, .phase = .enter });
                        }
                        const tags = self.parse_ir.store.typeAnnoSlice(tag_union.tags);
                        var i = tags.len;
                        while (i > 0) {
                            i -= 1;
                            try frames.append(self.env.gpa, .{ .anno_idx = tags[i], .mode = .tag, .phase = .enter });
                        }
                    },
                    .@"fn" => |func| {
                        try frames.append(self.env.gpa, .{ .anno_idx = frame.anno_idx, .mode = .normal, .phase = .exit });
                        try frames.append(self.env.gpa, .{ .anno_idx = func.ret, .mode = .normal, .phase = .enter });
                        const args = self.parse_ir.store.typeAnnoSlice(func.args);
                        var i = args.len;
                        while (i > 0) {
                            i -= 1;
                            try frames.append(self.env.gpa, .{ .anno_idx = args[i], .mode = .normal, .phase = .enter });
                        }
                    },
                    .parens => |parens| {
                        try frames.append(self.env.gpa, .{ .anno_idx = frame.anno_idx, .mode = .normal, .phase = .exit });
                        try frames.append(self.env.gpa, .{ .anno_idx = parens.anno, .mode = .normal, .phase = .enter });
                    },
                    .malformed => |malformed| {
                        const region = self.parse_ir.tokenizedRegionToRegion(malformed.region);
                        const result = try self.env.pushMalformed(TypeAnno.Idx, Diagnostic{ .malformed_type_annotation = .{ .region = region } });
                        try normal_results.put(self.env.gpa, frame.anno_idx, result);
                    },
                },
                .tag => switch (ast_anno) {
                    .ty => |ty| {
                        ...
                        const result = try self.env.addTypeAnno(.{ .tag = .{
                            .name = ident_idx,
                            .args = .{ .span = DataSpan.empty() },
                        } }, region);
                        try tag_results.put(self.env.gpa, frame.anno_idx, result);
                    },
                    .apply => |apply| {
                        const region = self.parse_ir.tokenizedRegionToRegion(apply.region);
                        const args_slice = self.parse_ir.store.typeAnnoSlice(apply.args);
                        if (args_slice.len == 0) {
                            const malformed = try self.env.pushMalformed(TypeAnno.Idx, Diagnostic{ .malformed_type_annotation = .{ .region = region } });
                            try tag_results.put(self.env.gpa, frame.anno_idx, malformed);
                            continue;
                        }
                        switch (self.parse_ir.store.getTypeAnno(args_slice[0])) {
                            .ty => {},
                        ...
                        try frames.append(self.env.gpa, .{ .anno_idx = frame.anno_idx, .mode = .tag, .phase = .exit });
                        var i = args_slice.len;
                        while (i > 1) {
                            i -= 1;
                            try frames.append(self.env.gpa, .{ .anno_idx = args_slice[i], .mode = .normal, .phase = .enter });
                        ...
                        const malformed = try self.env.pushMalformed(TypeAnno.Idx, Diagnostic{
            ...
            .exit => switch (frame.mode) {
                .normal => switch (ast_anno) {
                    .apply => |apply| {
                        const region = self.parse_ir.tokenizedRegionToRegion(apply.region);
                        const args_slice = self.parse_ir.store.typeAnnoSlice(apply.args);
                        const base_anno_idx = normal_results.get(args_slice[0]).?;
                        ...
                        for (args_slice[1..]) |arg_idx| {
                            try self.env.store.addScratchTypeAnno(normal_results.get(arg_idx).?);
                        }
                        const args_span = try self.env.store.typeAnnoSpanFrom(scratch_top);

                        const result = switch (base_anno) {
                            .lookup => |ty| blk: {
                                ...
                                break :blk try self.env.addTypeAnno(.{ .apply = .{
                                    .name = ty.name,
                                    .base = ty.base,
                                    .args = args_span,
                                } }, region);
                    ...
                    .tuple => |tuple| {
                        ...
                        const elems = self.parse_ir.store.typeAnnoSlice(tuple.annos);
                        if (elems.len == 1) {
                            try normal_results.put(self.env.gpa, frame.anno_idx, normal_results.get(elems[0]).?);
                            continue;
                        }
                        ...
                        for (elems) |elem_idx| {
                            const canonicalized_elem_idx = normal_results.get(elem_idx).?;
                            try self.env.store.addScratchTypeAnno(canonicalized_elem_idx);
                            try self.scratch_vars.append(ModuleEnv.varFrom(canonicalized_elem_idx));
                        }
                        const annos = try self.env.store.typeAnnoSpanFrom(scratch_top);
                        try normal_results.put(self.env.gpa, frame.anno_idx, try self.env.addTypeAnno(.{ .tuple = .{ .elems = annos } }, region));
                    },
                    .record => |record| {
                        ...
                        for (self.parse_ir.store.annoRecordFieldSlice(record.fields)) |field_idx| {
                            ...
                            const canonicalized_ty = normal_results.get(ast_field.ty).?;
                            const field_cir_idx = try self.env.addAnnoRecordField(
                                .{ .name = field_name, .ty = canonicalized_ty },
                        ...
                        const mb_ext_anno: ?TypeAnno.Idx = switch (record.ext) {
                            .closed => null,
                            .open => |open_tok| blk: switch (type_anno_ctx.type) {
                                .local_anno => break :blk try self.env.addTypeAnno(.{ .rigid_var = .{
                            ...
                            .named => |named| normal_results.get(named.anno).?,
                        };

                        try normal_results.put(self.env.gpa, frame.anno_idx, try self.env.addTypeAnno(.{ .record = .{
                    ...
                    .tag_union => |tag_union| {
                        ...
                        const mb_ext_anno: ?TypeAnno.Idx = switch (tag_union.ext) {
                            .closed => null,
                            .open => blk: switch (type_anno_ctx.type) {
                                .local_anno, .for_clause_anno => break :blk try self.env.addTypeAnno(.{ .rigid_var = .{
                                    .name = self.env.idents.open_ext,
                                } }, region),
                                .type_decl_anno => {
                                    const malformed = try self.env.pushMalformed(TypeAnno.Idx, Diagnostic{
                        ...
                        for (self.parse_ir.store.typeAnnoSlice(tag_union.tags)) |tag_idx| {
                            try self.env.store.addScratchTypeAnno(tag_results.get(tag_idx).?);
                        }
                        const tag_anno_idxs = try self.env.store.typeAnnoSpanFrom(scratch_annos_top);
                        try normal_results.put(self.env.gpa, frame.anno_idx, try self.env.addTypeAnno(.{ .tag_union = .{
                            .tags = tag_anno_idxs,
                            .ext = mb_ext_anno,
                        } }, region));
                    },
                    .@"fn" => |func| {
                        ...
                        const args_span = try self.env.store.typeAnnoSpanFrom(scratch_top);
                        try normal_results.put(self.env.gpa, frame.anno_idx, try self.env.addTypeAnno(.{ .@"fn" = .{
                            .args = args_span,
                            .ret = normal_results.get(func.ret).?,
                            .effectful = func.effectful,
                        } }, region));
                    },
                    .parens => |parens| {
                        const region = self.parse_ir.tokenizedRegionToRegion(parens.region);
                        try normal_results.put(self.env.gpa, frame.anno_idx, try self.env.addTypeAnno(.{ .parens = .{
                            .anno = normal_results.get(parens.anno).?,
                        } }, region));
                    },
                    .ty_var, .underscore_type_var, .ty, .underscore, .malformed => unreachable,
                },
                .tag => switch (ast_anno) {
                    .apply => |apply| {
                        const region = self.parse_ir.tokenizedRegionToRegion(apply.region);
                        ...
                        try tag_results.put(self.env.gpa, frame.anno_idx, try self.env.addTypeAnno(.{ .tag = .{
                            .name = type_name,
                            .args = args,
                        } }, region));
                    },
    ...
    return normal_results.get(anno_idx).?;
}
...
const BlockStatementExprKind = enum { expr, dbg, expect, @"return" };

fn canonicalizeBlockVarStatement(
    self: *Self,
    name_tok: Token.Idx,
    body: AST.Expr.Idx,
    annotation_idx: ?Annotation.Idx,
    ...
    const var_name = self.parse_ir.tokens.resolveIdentifier(name_tok) orelse {
        const feature = try self.env.insertString("resolve var name");
...
fn canonicalizeBlockExprStatement(
    self: *Self,
    child: AST.Expr.Idx,
    kind: BlockStatementExprKind,
    tokenized_region: AST.TokenizedRegion,
) std.mem.Allocator.Error!CanonicalizedStatement {
    const region = self.parse_ir.tokenizedRegionToRegion(tokenized_region);

    const saved_in_expect = self.in_expect;
    if (kind == .expect) self.in_expect = true;
    defer if (kind == .expect) {
        self.in_expect = saved_in_expect;
    };

    const expr = try self.canonicalizeExprOrMalformed(child);
    const stmt_idx = switch (kind) {
        .expr => try self.env.addStatement(Statement{ .s_expr = .{
            .expr = expr.idx,
        } }, region),
        .dbg => try self.env.addStatement(Statement{ .s_dbg = .{
...
fn canonicalizeBlockCrashStatement(
    self: *Self,
    msg_expr_idx: AST.Expr.Idx,
    tokenized_region: AST.TokenizedRegion,
) std.mem.Allocator.Error!CanonicalizedStatement {
    const region = self.parse_ir.tokenizedRegionToRegion(tokenized_region);
    const mb_msg_literal = blk: {
        const msg_expr = self.parse_ir.store.getExpr(msg_expr_idx);
        switch (msg_expr) {
            .string => |s| {
                const parts = self.parse_ir.store.exprSlice(s.parts);
                if (parts.len > 0) {
...
fn canonicalizeBlockForStatement(
    self: *Self,
    ...
    const list_expr = blk: {
        const list_free_vars_start = self.scratch_free_vars.top();
        defer self.scratch_free_vars.clearFrom(list_free_vars_start);

        const can_list = try self.canonicalizeExprOrMalformed(ast_list_expr);
        const free_vars_slice = self.scratch_free_vars.sliceFromSpan(can_list.free_vars);
        for (free_vars_slice) |fv| {
    ...
    const patt = try self.canonicalizePatternOrMalformed(ast_patt);
    try self.collectBoundVarsToScratch(patt);
    ...
    const body = blk: {
        ...
        const can_body = try self.canonicalizeExprOrMalformed(ast_body);
        const free_vars_slice = self.scratch_free_vars.sliceFromSpan(can_body.free_vars);
        for (free_vars_slice) |fv| {
            try self.appendPropagatedFreeVarExcludingBound(captures_top, for_bound_vars_top, fv);
        }
        break :blk can_body;
...
fn canonicalizeBlockWhileStatement(
    self: *Self,
    ast_cond: AST.Expr.Idx,
    ast_body: AST.Expr.Idx,
    tokenized_region: AST.TokenizedRegion,
) std.mem.Allocator.Error!CanonicalizedStatement {
    const captures_top = self.scratch_captures.top();
    ...
    const cond = blk: {
        ...
        const can_cond = try self.canonicalizeExprOrMalformed(ast_cond);
        const free_vars_slice = self.scratch_free_vars.sliceFromSpan(can_cond.free_vars);
        for (free_vars_slice) |fv| {
...
pub fn canonicalizeBlockStatement(
    ...
    var mb_canonicalized_stmt: ?CanonicalizedStatement = null;
    var stmts_processed: StatementsProcessed = .one;

    switch (ast_stmt) {
        .decl => |decl| {
            mb_canonicalized_stmt = try self.canonicalizeBlockDecl(decl, ast_stmt_idxs[current_index], null);
        },
        .@"var" => |var_stmt| {
            mb_canonicalized_stmt = try self.canonicalizeBlockVarStatement(
                var_stmt.name,
                var_stmt.body,
                null,
                var_stmt.region,
        ...
        .expr => |expr_stmt| {
            mb_canonicalized_stmt = try self.canonicalizeBlockExprStatement(expr_stmt.expr, .expr, expr_stmt.region);
        },
        .dbg => |dbg_stmt| {
            mb_canonicalized_stmt = try self.canonicalizeBlockExprStatement(dbg_stmt.expr, .dbg, dbg_stmt.region);
        },
        .expect => |expect_stmt| {
            mb_canonicalized_stmt = try self.canonicalizeBlockExprStatement(expect_stmt.body, .expect, expect_stmt.region);
        },
        .@"return" => |return_stmt| {
            mb_canonicalized_stmt = try self.canonicalizeBlockExprStatement(return_stmt.expr, .@"return", return_stmt.region);
        },
        .crash => |crash_stmt| {
            mb_canonicalized_stmt = try self.canonicalizeBlockCrashStatement(crash_stmt.expr, crash_stmt.region);
        },
        .type_decl => |type_decl| {
            switch (try self.canonicalizeLocalTypeDeclStatement(
                type_decl,
                ast_stmt_idxs[current_index],
                block_context,
            )) {
                .statement => |canonicalized_stmt| {
                    mb_canonicalized_stmt = canonicalized_stmt;
                },
                .associated => |associated| {
                    try self.addBlockStatement(block_context, associated.statement);
                    try self.processAssociatedBlock(
                        associated.request.owner_stmt_idx,
                        associated.request.qualified_name_idx,
                        associated.request.relative_name_idx,
                        associated.request.type_name,
                        associated.request.assoc,
                        associated.request.alias_sinks,
                        associated.request.block_context,
                        associated.request.owner_is_redeclaration,
                    );
                    mb_canonicalized_stmt = null;
                },
            }
        },
        .type_anno => |type_anno| blk: {
            const anno_result = try self.canonicalizeLocalTypeAnnoStatement(
                type_anno.name,
                type_anno.anno,
                type_anno.where,
                type_anno.region,
            ...
            const open_anno = switch (anno_result) {
                .anno => |anno| anno,
                .malformed_stmt => |stmt| {
                    mb_canonicalized_stmt = stmt;
                    break :blk;
                },
            };
            const anno_info = open_anno.info;
            defer self.scopeExitTypeVar(open_anno.type_var_scope);
            ...
            if (next_i < ast_stmt_idxs.len) {
                ...
                switch (next_stmt) {
                    .decl => |decl| {
                        ...
                        if (names_match) {
                            mb_canonicalized_stmt = try self.canonicalizeBlockDecl(decl, next_stmt_id, anno_info);
                            stmts_processed = .two;
                            break :blk;
                        }
                    },
            ...
            mb_canonicalized_stmt = try self.canonicalizeLocalAnnoOnlyStatement(anno_info);
        },
        .@"for" => |for_stmt| {
            mb_canonicalized_stmt = try self.canonicalizeBlockForStatement(
                for_stmt.patt,
                for_stmt.expr,
                for_stmt.body,
                for_stmt.region,
        ...
        .@"while" => |while_stmt| {
            mb_canonicalized_stmt = try self.canonicalizeBlockWhileStatement(
                while_stmt.cond,
                while_stmt.body,
                while_stmt.region,
            );
        ...
        .@"break" => |break_stmt| {
            ...
            mb_canonicalized_stmt = CanonicalizedStatement{ .idx = stmt_idx, .free_vars = DataSpan.empty() };
        },
        .import => |import_stmt| {
            _ = try self.canonicalizeImportStatement(import_stmt);
        },
    ...
    return StatementResult{ .canonicalized_stmt = mb_canonicalized_stmt, .stmts_processed = stmts_processed };
}
...
pub fn canonicalizeBlockDecl(
    ...
    if (ast_pattern == .ident) {
        ...
        if (self.parse_ir.tokens.resolveIdentifier(pattern_ident.ident_tok)) |ident_idx| {
            switch (self.scopeLookup(.ident, ident_idx)) {
                .found => |existing_pattern_idx| {
                    if (self.isVarReassignmentAcrossFunctionBoundary(existing_pattern_idx)) {
                        const malformed_idx = try self.env.pushMalformed(Expr.Idx, Diagnostic{ .var_across_function_boundary = .{
...
fn extractTypeVarIdentsFromASTAnno(self: *Self, anno_idx: AST.TypeAnno.Idx, idents_start_idx: u32) std.mem.Allocator.Error!void {
    var stack: std.ArrayList(AST.TypeAnno.Idx) = .empty;
    defer stack.deinit(self.env.gpa);

    try stack.append(self.env.gpa, anno_idx);
    while (stack.pop()) |current_idx| {
        switch (self.parse_ir.store.getTypeAnno(current_idx)) {
            .ty_var => |ty_var| {
                if (self.parse_ir.tokens.resolveIdentifier(ty_var.tok)) |ident| {
                    ...
                    if (!already_added) try self.scratch_idents.append(ident);
                }
            },
            .underscore_type_var => |underscore_ty_var| {
            ...
            .apply => |apply| {
                ...
                while (i > 0) {
                    i -= 1;
                    try stack.append(self.env.gpa, args[i]);
                }
            },
            .@"fn" => |fn_anno| {
                try stack.append(self.env.gpa, fn_anno.ret);
            ...
            .tuple => |tuple| {
                ...
                while (i > 0) {
                    i -= 1;
                    try stack.append(self.env.gpa, elems[i]);
                }
            },
            .parens => |parens| try stack.append(self.env.gpa, parens.anno),
            .record => |record| {
                if (record.ext == .named) {
                    try stack.append(self.env.gpa, record.ext.named.anno);
                }
                const fields = self.parse_ir.store.annoRecordFieldSlice(record.fields);
                var i = fields.len;
                while (i > 0) {
                    ...
                    try stack.append(self.env.gpa, field.ty);
                }
            },
            .tag_union => |tag_union| {
                if (tag_union.ext == .named) {
                    try stack.append(self.env.gpa, tag_union.ext.named.anno);
                }
                const tags = self.parse_ir.store.typeAnnoSlice(tag_union.tags);
                var i = tags.len;
                while (i > 0) {
                    i -= 1;
                    try stack.append(self.env.gpa, tags[i]);
                }
            },
...
fn tryModuleQualifiedLookup(self: *Self, field_access: AST.BinOp) std.mem.Allocator.Error!?Expr.Idx {
    ...
    if (right_expr == .apply) return null;
