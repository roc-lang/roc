        ...
        while (true) {
            while (self.invoked_walk.pop()) |invoked_idx| {
                ...
                switch (self.env.store.getExpr(invoked_idx)) {
                    ...
                    // Every other invoked form is not name-resolvable to a
                    // function body (a call result, a field access, a
                    // non-local lookup, a literal): invoked-ness adds no
                    // edge, and the drain's value re-push above covers the
                    // ordinary walk. Listed exhaustively so a new expression
                    // form forces an invoked-semantics decision here.
                    .e_num,
                    .e_frac_f32,
                    .e_frac_f64,
                    .e_dec,
