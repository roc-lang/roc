...
fn emitExprValue(self: *Self, expr_idx: Expr.Idx, expr: Expr) EmitError!void {
    switch (expr) {
        ...
        .e_dec => |dec| {
            ...
            if (frac_part == 0) {
                try self.writer().writeAll(try self.formatI128(whole));
            } else {
                try self.writer().writeAll(try self.formatI128(whole));
...
fn emitPatternValue(self: *Self, pattern: Pattern) EmitError!void {
    switch (pattern) {
        ...
        .frac_f32_literal => |frac| {
            try self.writer().writeAll(try self.formatF32(frac.value));
            try self.writer().writeAll("f32");
        },
        .frac_f64_literal => |frac| {
            try self.writer().writeAll(try self.formatF64(frac.value));
