...
use combine::parser::char::{alpha_num, char, digit, hex_digit, spaces, string, HexDigit};
use combine::parser::combinator::{look_ahead, not_followed_by};
use combine::parser::item::{any, position, satisfy, satisfy_map, value};
use combine::parser::repeat::{
    count_min_max, many, sep_by, sep_by1, skip_many, skip_many1, skip_until,
};
use combine::stream::state::State;
use combine::stream::{Positioned, Stream};
use combine::{
    attempt, between, choice, eof, many1, optional, parser, unexpected, unexpected_any, Parser,
};
...
pub fn parse_string(
    string: &str,
) -> Result<Located<Expr>, combine::easy::Errors<char, &str, IndentablePosition>> {
    let parse_state = State::with_positioner(string, IndentablePosition::default());

    located(expr())
        .skip(eof())
        .easy_parse(parse_state)
        .map(|(expr, _)| expr)
}
...
where
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    choice((whitespace1(), eof().with(value(()))))
}
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    attempt(string("case").skip(indented_whitespaces1(min_indent)))
        ...
        .and(many::<Vec<_>, _>(
            ...
            .with(located(pattern(min_indent)))
            .skip(indented_whitespaces1(min_indent))
            .skip(string("then"))
            .skip(indented_whitespaces1(min_indent))
            .and(located(expr_body(min_indent))),
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    ...
    .and(
        ...
        optional(attempt(apply_args(min_indent))),
    )
    .map(
        |(located_expr, opt_args): (Located<Expr>, Option<Vec<Located<Expr>>>)| match opt_args {
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    located(choice((
        ...
        attempt(variant_name())
            .map(|name| Expr::ApplyVariant(VariantName::Unqualified(name), None)),
        ...
        indented_whitespaces1(min_indent)
            ...
            .with(
                ...
                sep_by1(
                    ...
                    attempt(indented_whitespaces1(min_indent).skip(
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    choice((
        ...
        // either < or <=
        char('<').with(optional(char('=')).map(|opt_eq| {
            if opt_eq.is_none() {
                Operator::LessThan
            } else {
        ...
        char('|').with(
            char('>')
                .map(|_| Operator::Pizza)
                .or(char('|').map(|_| Operator::Or)),
        ),
        // either / or //
        char('/').with(optional(char('/')).map(|opt_slash| {
...
where
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    located(ident()).and(optional(apply_args(min_indent))).map(
        |(loc_name, opt_args): (Located<String>, Option<Vec<Located<Expr>>>)| {
            ...
            match opt_args {
                ...
                Some(args) => Expr::Apply(
                    Box::new(Located {
                        region: loc_name.region,
                        value: Expr::Var(Ident::Unqualified(loc_name.value)),
                    }),
                    args,
                ),
            }
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    char('\\')
        ...
        .with(sep_by1(
            ...
            attempt(many1::<Vec<_>, _>(
                skipped_indented_whitespace_char(min_indent).skip(not_followed_by(string("->"))),
            )),
        ))
        .skip(indented_whitespaces(min_indent))
        .skip(string("->"))
        .skip(indented_whitespaces1(min_indent))
        .and(located(expr_body(min_indent)))
        .map(|(patterns, closure_body)| Expr::Closure(patterns, Box::new(closure_body)))
}
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    attempt(located(variant_name()))
        .and(optional(attempt(indented_whitespaces(min_indent).with(
            sep_by1(
                located(pattern(min_indent)),
                attempt(indented_whitespaces1(min_indent).skip(not_followed_by(string("then")))),
            ),
        ))))
        .map(
            |(loc_name, opt_args): (Located<String>, Option<Vec<Located<Pattern>>>)|
            // Use optional(sep_by1()) over sep_by() to avoid
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    ...
    // combination of letters or numbers afterwards.
    // No underscores, dashes, or apostrophes.
    many1::<Vec<_>, _>(alpha_num()).then(|chars: Vec<char>| {
        ...
        if valid_start_char {
            ...
            unexpected_any("First character in an identifier that was not a lowercase letter")
                .left()
        }
    })
}
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    between(
        char('"'),
        char('"'),
        many::<Vec<(String, Located<String>)>, _>(choice((
        ...
        .map(|mut pairs| {
            match pairs.pop() {
                ...
                Some((trailing_str, located_name)) => {
                    let mut ident_pairs = pairs
                        .into_iter()
                        .map(|(string, located_name)| {
                            (
                                string,
...
where
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    between(char('\''), char('\''), char_body().expected(ERR_EMPTY_CHAR)).map(|ch| Expr::Char(ch))
}
    ...
    I::Error: ParseError<I::Item, I::Range, I::Position>,
{
    ...
    optional(attempt(char('-')))
        ...
        .and(optional(char('.').with(digits_after_decimal)))
        .then(
            |(((opt_minus, _), int_digits), decimals): (
                ((Option<char>, _), Vec<char>),
                Option<Vec<char>>,
            )| {
                ...
                match (int_str.parse::<i64>(), decimals) {
                    ...
                    (Ok(int_val), Some(nums)) => {
                        ...
                        match format!("{}.{}", int_str, decimal_str).parse::<f64>() {
                            Ok(float) => value(Pattern::FloatLiteral(float)).right(),
                            Err(_) => unexpected_any(
                                "non-digit characters after decimal point in a number literal",
                            )
                            .left(),
                    ...
                    (Err(_), _) => {
                        unexpected_any("looked like a number but was actually malformed identifier")
                            .left()
                    }
                }
            },
