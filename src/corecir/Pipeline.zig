...
const ProgramAssemblyDriver = struct {
    pass: *Pass,
    ...
    fn ensureCallableInstRealized(
        ...
        return self.pass.ensureCallableInstRealized(result, callable_inst_id);
    }
    ...
    fn ensureProgramExpr(
        ...
        return self.pass.ensureProgramExpr(result, source_context, module_idx, expr_idx);
    }
    ...
    fn exprHasExactProgramSemantics(
        ...
        const monotype = try self.pass.lookupRecordedExprMonotypeIfReadyForSourceContext(
            result,
            thread,
            source_context,
            module_idx,
    ...
    fn ensureProgramExprSemanticShape(
        ...
        switch (expr) {
            .e_lookup_local, .e_lookup_external, .e_lookup_required => {
                try self.pass.realizeLookupExprSemantics(
                    result,
            ...
            .e_type_var_dispatch => {
                try self.pass.realizeDispatchExprSemantics(
                    result,
                    SemanticThread.trackedThread(source_context),
                    module_idx,
                    expr_idx,
...
const RootAnalysisDriver = struct {
    pass: *Pass,
    result: *Result,
...
const MaterializeCallableValueFailure = enum {
    ...
    fn runRootSourceExprsInto(
        ...
        try Lambdasolved.analyzeRoots(
            ...
            exprs,
            RootAnalysisDriver{ .pass = self, .result = result },
        );
        ...
        try Lambdamono.assembleRoots(
            ...
            self.current_module_idx,
            result,
            ProgramAssemblyDriver{ .pass = self },
            exprs,
        );
    ...
    fn ensureProgramExpr(
        ...
        const monotype = try self.requireProgramExprMonotype(
            result,
            source_context,
            module_idx,
            expr_idx,
    ...
    fn realizeCallableArgSemantics(
        ...
        try Lambdasolved.withRootAnalysis(self.allocator, struct {
            pass: *Pass,
            result: *Result,
            thread: SemanticThread,
            module_idx: u32,
            ...
            fn run(self: @This(), root_analysis: *Lambdasolved.RootAnalysisState) Allocator.Error!void {
                const module_env = self.pass.all_module_envs[self.module_idx];
                for (module_env.store.sliceExpr(self.args)) |arg_expr_idx| {
                    try self.pass.scanCirValueExpr(root_analysis, self.result, self.thread, self.module_idx, arg_expr_idx);
                    var visiting: std.AutoHashMapUnmanaged(ContextExprVisitKey, void) = .empty;
                    defer visiting.deinit(self.pass.allocator);
                    try self.pass.realizeStructuredExprCallableSemantics(
                        self.result,
                    ...
                    _ = self.pass.getValueExprCallableValueForThread(self.result, self.thread, self.module_idx, arg_expr_idx);
                }
            }
        }{
            .pass = self,
            ...
            .func => {
                const Worker = struct {
                    ...
                    fn run(self: *@This(), root_analysis: *Lambdasolved.RootAnalysisState) Allocator.Error!void {
                        ...
                        defer demand_visiting.deinit(self.pass.allocator);
                        try self.pass.scanCirValueExpr(
                            root_analysis,
                            self.result,
                            SemanticThread.trackedThread(self.source_context),
                            self.module_idx,
                            self.expr_idx,
                        );

                        if (try self.pass.resolveProjectedExprCallableValue(
                            self.result,
                            self.source_context,
                            self.module_idx,
                            self.expr_idx,
                            ...
                            try self.pass.appendCallableParamSpecEntry(self.result, self.out, .{
                                .param_index = self.param_index,
                                .projections = try self.pass.addCallableParamProjectionEntries(self.result, self.projections.items),
                                .callable_value = callable_value,
                            });
                            self.completed = true;
                        }
                ...
                try Lambdasolved.withRootAnalysis(self.allocator, &worker);
    ...
    fn recordCallResultCallableValueFromCallee(
        ...
        try Lambdasolved.withRootAnalysis(self.allocator, struct {
            ...
            fn run(self: @This(), root_analysis: *Lambdasolved.RootAnalysisState) Allocator.Error!void {
                try self.pass.ensureCallableInstRealized(self.result, self.callee_callable_inst_id);
                const callee_callable_inst = self.result.getCallableInst(self.callee_callable_inst_id);
                const callee_fn_mono = switch (self.result.context_mono.monotype_store.getMonotype(callee_callable_inst.fn_monotype)) {
                    .func => |func| func,
                    else => unreachable,
                };
                if (self.result.context_mono.monotype_store.getMonotype(callee_fn_mono.ret) != .func) return;

                const in_progress_key = CallResultCallableInstKey{
                    .context_expr = Result.contextExprKey(self.target_source_context, self.target_module_idx, self.call_expr_idx),
                    .callee_callable_inst_raw = @intFromEnum(self.callee_callable_inst_id),
                };
            ...
            .pass = self,
            .result = result,
            .target_source_context = target_source_context,
            .target_module_idx = target_module_idx,
            .call_expr_idx = call_expr_idx,
    ...
    fn realizeDispatchExprSemantics(
        ...
        if (dispatch_method_name) |method_name| {
            try self.ensureRecordedExactDispatchSiteForExpr(
                result,
                thread,
                module_idx,
                expr_idx,
    ...
    fn recordDispatchExprTarget(
        ...
        const semantics = try self.ensureProgramExpr(result, source_context, module_idx, expr_idx);
        const common = semantics.common().*;
        const next_expr: Lambdamono.Expr = .{ .dispatch_target = .{
            .common = common,
    ...
    fn recordDispatchExprIntrinsic(
        ...
        const semantics = try self.ensureProgramExpr(result, source_context, module_idx, expr_idx);
        const common = semantics.common().*;
        const next_expr: Lambdamono.Expr = .{ .dispatch_intrinsic = .{
            .common = common,
    ...
    fn extractExactDispatchSiteForExpr(
        self: *Pass,
        result: *Result,
        thread: SemanticThread,
        module_idx: u32,
        ...
        for (self.dispatchConstraintsForTypeVar(module_idx, receiver_type_var)) |constraint| {
            ...
            if (matched) |existing| {
                ...
                if (std.debug.runtime_safety) {
                    std.debug.panic(
                        "Pipeline invariant violated: dispatch expr={d} method='{s}' had multiple non-equivalent exact dispatch sites (existing_var={d} existing_mono={d}@{d} next_var={d} next_mono={d}@{d})",
                        .{
                            @intFromEnum(expr_idx),
                            module_env.getIdent(method_name),
    ...
    fn ensureRecordedExactDispatchSiteForExpr(
        self: *Pass,
        result: *Result,
        thread: SemanticThread,
        module_idx: u32,
        ...
        const source_context = thread.requireSourceContext();
        if (self.getRecordedExactDispatchSiteForExpr(result, source_context, module_idx, expr_idx, method_name) != null) {
            return;
        }
        const exact = try self.extractExactDispatchSiteForExpr(
            result,
            thread,
            module_idx,
            expr_idx,
        ...
        try self.recordExactDispatchSite(
            result,
            source_context,
            module_idx,
            expr_idx,
    ...
    fn exactDispatchSiteForExpr(
        ...
        return self.getRecordedExactDispatchSiteForExpr(
            result,
            thread.requireSourceContext(),
            module_idx,
            expr_idx,
    ...
    fn dotDispatchHandledWithoutCallableInst(
        ...
        const receiver_monotype = try self.resolveExprMonotype(result, thread, module_idx, dot_expr.receiver);
        if (receiver_monotype == null) {
            const eq_site = self.exactDispatchSiteForExpr(result, thread, module_idx, expr_idx, module_env.idents.is_eq);
            return self.lookupResolvedDispatchTarget(result, thread, module_idx, expr_idx) == null and eq_site != null;
        }
    ...
    fn dispatchHandledAsPrimitiveToStrIntrinsic(
        ...
        const module_env = self.all_module_envs[module_idx];
        const constraint = self.exactDispatchSiteForExpr(result, thread, module_idx, expr_idx, method_name) orelse return false;
        if (self.lookupResolvedDispatchTarget(result, thread, module_idx, expr_idx) != null) return false;
    ...
    fn resolveAssociatedMethodDispatchTargetForTypeVar(
        ...
        const site = try self.exactAssociatedMethodDispatchSite(
            result,
            thread,
            module_idx,
            expr_idx,
    ...
    fn exactAssociatedMethodDispatchSite(
        self: *Pass,
        result: *Result,
        thread: SemanticThread,
        module_idx: u32,
        ...
        const module_env = self.all_module_envs[module_idx];
        _ = method_info;
        const site = self.exactDispatchSiteForExpr(
            result,
            thread,
            module_idx,
            expr_idx,
        ...
        if (fn_monotype.isNone()) {
            if (std.debug.runtime_safety) {
                std.debug.panic(
                    "Pipeline invariant violated: associated dispatch expr={d} method='{s}' cached an exact dispatch site without an exact function monotype",
                    .{ @intFromEnum(expr_idx), module_env.getIdent(method_name) },
                );
            }
            unreachable;
    ...
    fn resolveDispatchTargetForExpr(
        ...
        const module_env = self.all_module_envs[module_idx];
        const site = self.exactDispatchSiteForExpr(result, thread, module_idx, expr_idx, method_name) orelse {
            if (std.debug.runtime_safety) {
                const expr = module_env.store.getExpr(expr_idx);
        ...
        const desired_func_monotype = try self.resolveTypeVarMonotypeResolved(result, thread, module_idx, site.fn_var);
        const resolved_target = try self.resolveDispatchTargetFromExactSite(
            result,
            thread,
            module_idx,
            expr_idx,
            ...
            if (std.debug.runtime_safety) {
                std.debug.panic(
                    "Pipeline invariant violated: dispatch expr={d} method='{s}' reached specialization without a resolvable exact target (fn_var={d} desired={d}@{d} desired_mono={any} receiver_mono={any})",
                    .{
                        @intFromEnum(expr_idx),
                        module_env.getIdent(method_name),
    ...
    fn resolveDispatchTargetFromExactSite(
        self: *Pass,
        result: *Result,
        thread: SemanticThread,
        module_idx: u32,
        ...
        const resolved = switch (expr) {
            .e_binop => |binop_expr| blk: {
                if (try self.resolveAssociatedMethodDispatchTargetForTypeVar(
                    result,
                    thread,
                    module_idx,
                    expr_idx,
                ...
                if (try self.resolveExprMonotypeResolved(result, thread, module_idx, binop_expr.lhs)) |lhs_monotype| {
                    break :blk try self.resolveAssociatedMethodDispatchTargetForMonotype(
                        result,
                        thread,
                        module_idx,
                        expr_idx,
            ...
            .e_unary_minus => |unary_expr| blk: {
                ...
                if (try self.resolveExprMonotypeResolved(result, thread, module_idx, unary_expr.expr)) |operand_monotype| {
                    break :blk try self.resolveAssociatedMethodDispatchTargetForMonotype(
                        result,
                        thread,
                        module_idx,
            ...
            .e_type_var_dispatch => |dispatch_expr| blk: {
                const alias_stmt = self.all_module_envs[module_idx].store.getStatement(dispatch_expr.type_var_alias_stmt).s_type_var_alias;
                ...
                const alias_monotype = try self.resolveTypeVarMonotypeResolved(result, thread, module_idx, receiver_type_var);
        ...
        if (resolved) |target| {
            if (std.debug.runtime_safety and target.fn_var != fn_var) {
                std.debug.panic(
                    "Pipeline invariant violated: dispatch target fn var {d} did not match exact dispatch site fn var {d} for expr={d} method='{s}'",
                    .{
                        @intFromEnum(target.fn_var),
                        @intFromEnum(fn_var),
                        @intFromEnum(expr_idx),
                        self.all_module_envs[module_idx].getIdent(method_name),
                    },
                );
            }
        }
    ...
    fn ensureCallableInstRealized(
        ...
        try Lambdasolved.withRootAnalysis(self.allocator, struct {
            ...
            fn run(self: @This(), root_analysis: *Lambdasolved.RootAnalysisState) Allocator.Error!void {
                try self.pass.realizeCallableInst(root_analysis, self.result, self.callable_inst_id);
            }
        }{
            .pass = self,
            .result = result,
