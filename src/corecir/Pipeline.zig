...
const LambdaSetId = Lambdasolved.LambdaSetId;
const SolvedCallableValue = Lambdasolved.SolvedCallableValue;
const SolvedCall = Lambdasolved.SolvedCall;
...
const CallableParamProjection = union(enum) {
    field: Monotype.Name,
    tuple_elem: u32,
};

/// Span of callable-parameter projections stored in specialization-internal state.
const CallableParamProjectionSpan = extern struct {
    start: u32,
    len: u16,
...
const CallableParamSpecEntry = struct {
    param_index: u16,
    projections: CallableParamProjectionSpan = .empty(),
    callable_value: SolvedCallableValue,
};

/// Span of callable-parameter specification entries stored in specialization-internal state.
const CallableParamSpecSpan = extern struct {
    start: u32,
    len: u16,
...
fn materializeSolvedCallableValue(
    self: *Pass,
    result: *Result,
    solved_value: SolvedCallableValue,
) Allocator.Error!CallableValue {
    return switch (solved_value) {
        .direct => |callable_inst_id| .{ .direct = callable_inst_id },
        .lambda_set => |lambda_set_id| .{ .packed_fn = try self.ensurePackedFnForLambdaSet(result, lambda_set_id) },
    };
}

fn materializeSolvedCall(
    self: *Pass,
    result: *Result,
    solved_call: SolvedCall,
) Allocator.Error!CallSite {
    return switch (solved_call) {
        .direct => |callable_inst_id| .{ .direct = callable_inst_id },
        .lambda_set => |lambda_set_id| .{ .indirect_call = try self.ensureIndirectCallForLambdaSet(result, lambda_set_id) },
    };
}

fn solvedCallableValueMonotype(
    self: *Pass,
    result: *const Result,
    solved_value: SolvedCallableValue,
    ...
    return switch (solved_value) {
        ...
        .lambda_set => |lambda_set_id| self.requireLambdaSetFnMonotype(result, result.lambdasolved.getLambdaSetMembers(lambda_set_id)),
    };
}
...
const SemanticThread = union(enum) {
    ...
    fn trackedThread(source_context: SourceContext) SemanticThread {
        return .{ .tracked = .{ .source_context = source_context } };
    }

    fn completionThread(source_context: SourceContext) SemanticThread {
        return .{ .completion = .{ .source_context = source_context } };
    }
    ...
    fn withIterationMonotypes(
        self: SemanticThread,
        iteration_expr_monotypes: ?*std.AutoHashMapUnmanaged(ContextExprKey, ResolvedMonotype),
    ...
    fn requireSourceContext(self: SemanticThread) SourceContext {
        return self.state().source_context;
    }

    fn hasCallableInst(self: SemanticThread) bool {
        return sourceContextHasCallableInst(self.state().source_context);
    }

    fn callableInst(self: SemanticThread) ?CallableInstId {
        return sourceContextCallableInst(self.state().source_context);
    }

    fn requireCallableInst(self: SemanticThread) CallableInstId {
        const source_context = self.requireSourceContext();
        return requireSourceContextCallableInst(source_context);
    }

    fn callableInstRawForDebug(self: SemanticThread) u32 {
        const source_context = self.state().source_context;
...
pub const Result = struct {
    ...
    pub fn getContextPatternMonotype(
        ...
        switch (source_context) {
            .callable_inst => |context_id| {
                const callable_inst_id: CallableInstId = @enumFromInt(@intFromEnum(context_id));
                if (self.getCallableParamBindingValue(callable_inst_id, pattern_idx)) |callable_value| {
    ...
    pub fn getPackedFnMembers(self: *const Result, packed_fn_id: PackedFnId) []const CallableInstId {
        return Lambdamono.getCallableMembers(&self.lambdamono, self.getPackedFn(packed_fn_id).members);
    }

    pub fn getIndirectCallMembers(self: *const Result, indirect_call_id: IndirectCallId) []const CallableInstId {
        return Lambdamono.getCallableMembers(&self.lambdamono, self.getIndirectCall(indirect_call_id).members);
    }
...
pub const Pass = struct {
    ...
    app_module_idx: ?u32,
    default_type_scope: ?TypeScopeContext,
    ...
    callable_param_specs_by_inst: std.AutoHashMapUnmanaged(CallableInstId, CallableParamSpecSpan),
    ...
    pub fn init(
        ...
        return .{
            ...
            .default_type_scope = default_type_scope,
            ...
            .callable_param_specs_by_inst = .empty,
            .mutation_revision = 0,
    ...
    fn exactCallableInstFromSolvedCallableValue(callable_value: SolvedCallableValue) ?CallableInstId {
        return switch (callable_value) {
    ...
    fn callableMembersFromSolvedCallableValue(result: *const Result, callable_value: SolvedCallableValue) []const CallableInstId {
        return switch (callable_value) {
            .direct => |callable_inst_id| result.lambdamono.getDirectCallableMembers(callable_inst_id),
            .lambda_set => |lambda_set_id| result.lambdasolved.getLambdaSetMembers(lambda_set_id),
        };
    }

    fn exactCallableInstFromSolvedCallSite(call_site: SolvedCall) ?CallableInstId {
    ...
    fn callableMembersFromSolvedCallSite(result: *const Result, call_site: SolvedCall) []const CallableInstId {
        return switch (call_site) {
            .direct => |callable_inst_id| result.lambdamono.getDirectCallableMembers(callable_inst_id),
            .lambda_set => |lambda_set_id| result.lambdasolved.getLambdaSetMembers(lambda_set_id),
        };
    ...
    fn getCallableParamSpecEntries(self: *const Pass, span: CallableParamSpecSpan) []const CallableParamSpecEntry {
        if (span.len == 0) return &.{};
        return self.callable_param_spec_entries.items[span.start..][0..span.len];
    }

    fn getCallableParamProjectionEntries(self: *const Pass, span: CallableParamProjectionSpan) []const CallableParamProjection {
        if (span.len == 0) return &.{};
        return self.callable_param_projection_entries.items[span.start..][0..span.len];
    }

    fn getCallableParamSpecSpanForInst(self: *const Pass, callable_inst_id: CallableInstId) CallableParamSpecSpan {
        return self.callable_param_specs_by_inst.get(callable_inst_id) orelse .empty();
    }
    ...
    fn resolveBoundPatternCallableInstInContext(
        ...
        return exactCallableInstFromSolvedCallableValue(callable_value);
    }
    ...
    fn resolveBoundPatternCallableMembersInContext(
        ...
        return callableMembersFromSolvedCallableValue(result, callable_value);
    }

    fn resolveBoundPatternCallableMembersForThread(
        self: *const Pass,
        result: *const Result,
        thread: SemanticThread,
        module_idx: u32,
    ...
    fn getExprCallableMembersInContext(
        self: *const Pass,
        result: *const Result,
        context_callable_inst: CallableInstId,
        module_idx: u32,
    ...
    fn getExprCallableMembersForThread(
        self: *const Pass,
        result: *const Result,
        thread: SemanticThread,
        module_idx: u32,
    ...
    fn getCallSiteCallableInstInContext(
        ...
        const call_site = self.readExprCallSite(
            result,
            callableInstSourceContext(context_callable_inst),
            module_idx,
            expr_idx,
        ) orelse return null;
        return exactCallableInstFromSolvedCallSite(call_site);
    }
    ...
    fn getCallSiteCallableMembersInContext(
        self: *const Pass,
        result: *const Result,
        context_callable_inst: CallableInstId,
        module_idx: u32,
        ...
        return callableMembersFromSolvedCallSite(result, call_site);
    }

    fn getCallSiteCallableMembersForThread(
        self: *const Pass,
        result: *const Result,
        thread: SemanticThread,
        module_idx: u32,
    ...
    fn getCallSiteCallableInstForSourceContext(
        ...
        const call_site = self.readExprCallSite(result, source_context, module_idx, expr_idx) orelse return null;
        return exactCallableInstFromSolvedCallSite(call_site);
    }

    fn getCallSiteCallableMembersForSourceContext(
        self: *const Pass,
        result: *const Result,
        source_context: SourceContext,
        module_idx: u32,
    ...
    fn getCallableParamSpecSolvedCallableValueForSourceContext(
        self: *const Pass,
        result: *const Result,
        source_context: SourceContext,
        module_idx: u32,
        ...
        for (self.getCallableParamSpecEntries(self.getCallableParamSpecSpanForInst(resolved_context_callable_inst))) |spec| {
            if (spec.param_index != param_index) continue;
            if (!self.callableParamProjectionSeqEqual(
                self.getCallableParamProjectionEntries(spec.projections),
                projections.items,
            )) continue;
    ...
    fn getCallableParamSpecCallableMembersForSourceContext(
        self: *const Pass,
        result: *const Result,
        source_context: SourceContext,
        module_idx: u32,
        ...
        const callable_value = self.getCallableParamSpecSolvedCallableValueForSourceContext(
            result,
            source_context,
            module_idx,
            expr_idx,
    ...
    fn getCallableParamSpecSolvedCallableValueInContext(
        self: *const Pass,
        result: *const Result,
        context_callable_inst: CallableInstId,
        module_idx: u32,
        ...
        return self.getCallableParamSpecSolvedCallableValueForSourceContext(
            result,
            callableInstSourceContext(context_callable_inst),
            module_idx,
            expr_idx,
    ...
    fn getCallableParamSpecSolvedCallableValueForThread(
        self: *const Pass,
        result: *const Result,
        thread: SemanticThread,
        module_idx: u32,
    ...
    fn getCallableParamSpecCallableMembersInContext(
        self: *const Pass,
        result: *const Result,
        context_callable_inst: CallableInstId,
        module_idx: u32,
        ...
        return self.getCallableParamSpecCallableMembersForSourceContext(
            result,
            callableInstSourceContext(context_callable_inst),
            module_idx,
            expr_idx,
    ...
    fn getCallableParamSpecCallableMembersForThread(
        self: *const Pass,
        result: *const Result,
        thread: SemanticThread,
        module_idx: u32,
    ...
    fn getValueExprCallableInstInContext(
        ...
        if (self.getCallableParamSpecSolvedCallableValueInContext(result, context_callable_inst, module_idx, expr_idx)) |callable_value| {
            if (exactCallableInstFromSolvedCallableValue(callable_value)) |callable_inst_id| {
                return callable_inst_id;
            }
        }
    ...
    fn getValueExprCallableInstForThread(
        ...
        if (self.getCallableParamSpecSolvedCallableValueForThread(result, thread, module_idx, expr_idx)) |callable_value| {
            if (exactCallableInstFromSolvedCallableValue(callable_value)) |callable_inst_id| {
                return callable_inst_id;
            }
        }

        if (self.getExprCallableInstForThread(result, thread, module_idx, expr_idx)) |callable_inst_id| {
            return callable_inst_id;
        }
        return null;
    }

    fn getValueExprCallableMembersInContext(
        self: *const Pass,
        result: *const Result,
        context_callable_inst: CallableInstId,
        module_idx: u32,
        ...
        if (self.getCallableParamSpecCallableMembersInContext(result, context_callable_inst, module_idx, expr_idx)) |callable_inst_ids| {
            return callable_inst_ids;
        }

        if (self.getExprCallableMembersInContext(result, context_callable_inst, module_idx, expr_idx)) |callable_inst_ids| {
            return callable_inst_ids;
        }
        return null;
    }

    fn getValueExprCallableMembersForThread(
        self: *const Pass,
        result: *const Result,
        thread: SemanticThread,
        module_idx: u32,
        ...
        switch (module_env.store.getExpr(expr_idx)) {
            .e_lookup_local => |lookup| {
                if (self.resolveBoundPatternCallableMembersForThread(result, thread, module_idx, lookup.pattern_idx)) |callable_inst_ids| {
                    return callable_inst_ids;
                }
            },
            else => {},
        ...
        if (self.getCallableParamSpecCallableMembersForThread(result, thread, module_idx, expr_idx)) |callable_inst_ids| {
            return callable_inst_ids;
        }

        if (self.getExprCallableMembersForThread(result, thread, module_idx, expr_idx)) |callable_inst_ids| {
            return callable_inst_ids;
        }
        return null;
    }

    fn getValueExprSolvedCallableValueForSourceContext(
        self: *Pass,
        result: *Result,
        source_context: SourceContext,
        module_idx: u32,
        ...
        switch (module_env.store.getExpr(expr_idx)) {
            .e_lookup_local => |lookup| {
                ...
                if (sourceContextHasCallableInst(source_context)) {
                    ...
                    return self.getValueExprSolvedCallableValueForSourceContext(
                        result,
                        replayExprSourceContext(source_context, source.module_idx, source.expr_idx),
                        source.module_idx,
                        source.expr_idx,
                    );
                }
        ...
        if (self.getCallableParamSpecSolvedCallableValueForSourceContext(result, source_context, module_idx, expr_idx)) |callable_value| {
            return callable_value;
        }
    ...
    fn getValueExprCallableInstForSourceContext(
        ...
        if (self.getValueExprSolvedCallableValueForSourceContext(result, source_context, module_idx, expr_idx)) |callable_value| {
            if (exactCallableInstFromSolvedCallableValue(callable_value)) |callable_inst_id| {
                return callable_inst_id;
            }
        }
    ...
    fn getValueExprCallableMembersForSourceContext(
        ...
        switch (source_context) {
            .callable_inst => |context_id| {
                return self.getValueExprCallableMembersInContext(
                    result,
                    @enumFromInt(@intFromEnum(context_id)),
                    module_idx,
                    expr_idx,
    ...
    pub fn deinit(self: *Pass) void {
        ...
        self.callable_param_spec_entries.deinit(self.allocator);
        self.callable_param_projection_entries.deinit(self.allocator);
        self.callable_param_specs_by_inst.deinit(self.allocator);
    }
    ...
    pub fn runRootSourceExpr(self: *Pass, expr_idx: CIR.Expr.Idx) Allocator.Error!Result {
        ...
        try self.buildLambdamonoProgram(&result, &.{expr_idx});
    ...
    pub fn runRootSourceExprs(self: *Pass, exprs: []const CIR.Expr.Idx) Allocator.Error!Result {
        ...
        try self.buildLambdamonoProgram(&result, exprs);
    ...
    pub fn runModule(self: *Pass) Allocator.Error!Result {
        ...
        try self.buildLambdamonoProgram(&result, root_source_exprs.items);

        return result;
    }

    fn buildLambdamonoProgram(
        self: *Pass,
        result: *Result,
        explicit_root_exprs: []const CIR.Expr.Idx,
    ) Allocator.Error!void {
        try self.buildCallableBodies(result);
        try self.buildExplicitRootExprs(result, explicit_root_exprs);
        try self.buildVisitedRootExprs(result);
    }

    fn buildExplicitRootExprs(
        self: *Pass,
        result: *Result,
        explicit_root_exprs: []const CIR.Expr.Idx,
    ) Allocator.Error!void {
        for (explicit_root_exprs) |expr_idx| {
            try self.buildProgramRootExpr(
                result,
    ...
    fn collectCallableParamBindings(
        ...
        for (arg_patterns) |pattern_idx| {
            ...
            try result.lambdamono.callable_param_bindings.append(self.allocator, .{
                ...
                .callable_value = try materializeSolvedCallableValue(self, result, callable_value),
            });
        }
        return .{
            .start = start,
    ...
    fn buildProgramRootExpr(
        ...
        const body_expr = try self.buildProgramExpr(result, source_context, module_idx, expr_idx);
    ...
    fn buildProgramExpr(
        ...
        switch (expr) {
            .e_str => |str_expr| for (module_env.store.sliceExpr(str_expr.span)) |child_expr_idx| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, child_expr_idx));
            },
            ...
            .e_match => |match_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, match_expr.cond));
                for (module_env.store.sliceMatchBranches(match_expr.branches)) |branch_idx| {
                    ...
                    if (branch.guard) |guard_expr_idx| {
                        try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, guard_expr_idx));
                    }
                    try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, branch.value));
                }
            },
            .e_if => |if_expr| {
                for (module_env.store.sliceIfBranches(if_expr.branches)) |branch_idx| {
                    const branch = module_env.store.getIfBranch(branch_idx);
                    try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, branch.cond));
                    try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, branch.body));
                }
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, if_expr.final_else));
            },
            .e_call => |call_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, call_expr.func));
                for (module_env.store.sliceExpr(call_expr.args)) |arg_expr_idx| {
                    try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, arg_expr_idx));
                }
            },
            .e_record => |record_expr| {
                for (module_env.store.sliceRecordFields(record_expr.fields)) |field_idx| {
                    const field = module_env.store.getRecordField(field_idx);
                    try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, field.value));
                }
                if (record_expr.ext) |ext_expr_idx| {
                    try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, ext_expr_idx));
                }
            },
            .e_block => |block_expr| {
                for (module_env.store.sliceStatements(block_expr.stmts)) |stmt_idx| {
                    try child_stmts.append(self.allocator, try self.buildProgramStmt(result, source_context, module_idx, stmt_idx));
                }
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, block_expr.final_expr));
            },
            .e_tag => |tag_expr| for (module_env.store.sliceExpr(tag_expr.args)) |child_expr_idx| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, child_expr_idx));
            },
            .e_nominal => |nominal_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, nominal_expr.backing_expr));
            },
            .e_nominal_external => |nominal_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, nominal_expr.backing_expr));
            },
            .e_binop => |binop_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, binop_expr.lhs));
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, binop_expr.rhs));
            },
            .e_unary_minus => |unary_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, unary_expr.expr));
            },
            .e_unary_not => |unary_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, unary_expr.expr));
            },
            .e_dot_access => |dot_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, dot_expr.receiver));
                if (dot_expr.args) |args| for (module_env.store.sliceExpr(args)) |arg_expr_idx| {
                    try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, arg_expr_idx));
                };
            },
            .e_tuple_access => |tuple_access| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, tuple_access.tuple));
            },
            .e_dbg => |dbg_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, dbg_expr.expr));
            },
            .e_expect => |expect_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, expect_expr.body));
            },
            .e_return => |return_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, return_expr.expr));
            },
            .e_type_var_dispatch => |dispatch_expr| for (module_env.store.sliceExpr(dispatch_expr.args)) |arg_expr_idx| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, arg_expr_idx));
            },
            .e_for => |for_expr| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, for_expr.expr));
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, for_expr.body));
            },
        ...
        const callable_value = if (self.readExprCallableValue(result, source_context, module_idx, expr_idx)) |solved_value|
            try materializeSolvedCallableValue(self, result, solved_value)
        else
            null;
        const call_site = if (self.readExprCallSite(result, source_context, module_idx, expr_idx)) |solved_call|
            try materializeSolvedCall(self, result, solved_call)
        else
            null;
        const dispatch_target = result.context_mono.getDispatchExprTarget(source_context, module_idx, expr_idx);
        const lookup_resolution = switch (expr) {
            .e_lookup_local => |lookup| blk: {
                if (findDefByPattern(module_env, lookup.pattern_idx)) |def_idx| {
                    break :blk LookupResolution{ .def = .{
                        .module_idx = module_idx,
                        .def_idx = def_idx,
                    } };
                }
                if (result.getPatternSourceExpr(module_idx, lookup.pattern_idx)) |source| {
                    break :blk LookupResolution{ .expr = .{
                        .source_context = replayExprSourceContext(source_context, source.module_idx, source.expr_idx),
    ...
    fn buildProgramStmt(
        ...
        switch (stmt) {
            .s_decl => |decl| try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, decl.expr)),
            .s_var => |var_decl| try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, var_decl.expr)),
            .s_reassign => |reassign| try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, reassign.expr)),
            .s_dbg => |dbg_stmt| try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, dbg_stmt.expr)),
            .s_expr => |expr_stmt| try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, expr_stmt.expr)),
            .s_expect => |expect_stmt| try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, expect_stmt.body)),
            .s_for => |for_stmt| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, for_stmt.expr));
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, for_stmt.body));
            },
            .s_while => |while_stmt| {
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, while_stmt.cond));
                try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, while_stmt.body));
            },
            .s_return => |return_stmt| try child_exprs.append(self.allocator, try self.buildProgramExpr(result, source_context, module_idx, return_stmt.expr)),
            else => {},
        }
    ...
    fn requireProgramExprMonotype(
        ...
        const resolved = blk: {
            ...
            if (self.intrinsicExprResultMonotype(result, module_idx, expr_idx)) |intrinsic| {
                break :blk intrinsic;
            }
            if (self.readExprCallableValue(result, source_context, module_idx, expr_idx)) |callable_value| {
                break :blk try solvedCallableValueMonotype(self, result, callable_value);
            }
    ...
    fn writeExprCallableValue(
        ...
        try self.putTracked(
            .solved_values,
            &result.lambdasolved.context_expr_callable_values,
    ...
    fn writeCallableParamValue(
        ...
        try self.putTracked(
            .solved_patterns,
            &result.lambdasolved.context_pattern_callable_values,
    ...
    fn readExprCallableValue(
        ...
        return result.lambdasolved.getExprCallableValue(source_context, module_idx, expr_idx);
    }
    ...
    fn readCallableParamValue(
        ...
        return result.lambdasolved.getPatternCallableValue(source_context, module_idx, pattern_idx);
    }
    ...
    fn writeExprCallSite(
        ...
        module_idx: u32,
        expr_idx: CIR.Expr.Idx,
        call_site: SolvedCall,
    ) Allocator.Error!void {
        try self.putTracked(
            .solved_calls,
            &result.lambdasolved.context_expr_calls,
            Result.contextExprKey(source_context, module_idx, expr_idx),
            call_site,
        );
    }
    ...
    fn readExprCallSite(
        ...
        return result.lambdasolved.getExprCall(source_context, module_idx, expr_idx);
    }
    ...
    fn bindPatternCallableValueCallableInsts(
        ...
        switch (callable_value) {
            ...
            .lambda_set => |lambda_set_id| {
                const packed_members = result.lambdasolved.getLambdaSetMembers(lambda_set_id);
                try self.setExprCallableMembers(
                    result,
                    thread.requireSourceContext(),
                    module_idx,
                    expr_idx,
                ...
                if (thread.hasCallableInst()) {
                    try self.setCallableParamMembers(
                        result,
                        thread.requireSourceContext(),
                        module_idx,
                        pattern_idx,
    ...
    fn scanCirExprInternal(
        ...
        if (materialize_if_callable and callableKind(expr) == null) {
            if (self.getValueExprCallableInstForThread(result, thread, module_idx, expr_idx) == null and
                self.getValueExprCallableMembersForThread(result, thread, module_idx, expr_idx) == null)
            {
                var visiting: std.AutoHashMapUnmanaged(ContextExprVisitKey, void) = .empty;
                defer visiting.deinit(self.allocator);
    ...
    fn setExprCallableMembersInCallableContext(
        self: *Pass,
        result: *Result,
        context_callable_inst: CallableInstId,
        module_idx: u32,
        ...
        return self.setExprCallableMembers(
            result,
            callableInstSourceContext(context_callable_inst),
            module_idx,
            expr_idx,
    ...
    fn setExprCallableMembers(
        ...
        const callable_value = try self.solveCallableValueFromMembers(result, callable_inst_ids);
        try self.writeExprCallableValue(result, source_context, module_idx, expr_idx, callable_value);
        const callable_value_mono = try solvedCallableValueMonotype(self, result, callable_value);
        try self.recordExprMonotypeForSourceContext(
            result,
            source_context,
            module_idx,
    ...
    fn captureFieldExactCallableInst(field: CaptureField) ?CallableInstId {
        return switch (field.callable_binding) {
            .non_callable => null,
            .direct => |callable_inst_id| callable_inst_id,
        };
    }
    ...
    fn resolveCaptureValueSourceMonotype(
        ...
        return switch (capture_value_source) {
            .expr => |capture_expr_ref| self.resolveExprMonotypeResolvedForSourceContext(
                result,
                SemanticThread.trackedThread(capture_expr_ref.source_context),
    ...
    fn finalizeCallableDefForCallableInst(
        ...
        for (module_env.store.sliceCaptures(closure_expr.captures)) |capture_idx| {
            ...
            try capture_fields.append(self.allocator, .{
                ...
                .callable_binding = if (exact_capture_callable_inst) |callable_inst_id|
                    .{ .direct = callable_inst_id }
                else
                    .non_callable,
        ...
        switch (expr) {
            ...
            .e_lookup_local => |lookup| {
                ...
                if (try self.resolveExprCallableTemplate(result, thread.requireSourceContext(), module_idx, expr_idx)) |template_id| {
                    ...
                    if (result.getPatternSourceExpr(module_idx, lookup.pattern_idx)) |source| {
                        const source_replay_context = replayExprSourceContext(
                            thread.requireSourceContext(),
                            source.module_idx,
                            source.expr_idx,
                        );
    ...
    fn resolvePatternMonotypeInSourceContext(
        ...
        var thread = SemanticThread.trackedThread(source_context);
        switch (source_context) {
            .callable_inst => |context_id| {
                try self.seedBindingsForCallableInst(result, @enumFromInt(@intFromEnum(context_id)), &bindings);
                thread = thread.withBindings(&bindings);
            },
            .root_expr, .provenance_expr, .template_expr => {},
        }
    ...
    fn resolveExprMonotypeInSourceContext(
        ...
        return self.resolveExprMonotypeResolved(result, thread, module_idx, expr_idx);
    }
    ...
    fn callableInstSatisfiesCallSiteRequirements(
        ...
        return callableParamSpecsEqual(
            self,
            self.getCallableParamSpecEntries(self.getCallableParamSpecSpanForInst(callable_inst_id)),
    ...
    fn resolveDirectCallSite(
        ...
        if (self.getValueExprCallableMembersForThread(result, thread, module_idx, callee_expr_idx)) |callable_inst_ids| {
            ...
            if (all_satisfy) {
                var visiting: std.AutoHashMapUnmanaged(ContextExprVisitKey, void) = .empty;
                defer visiting.deinit(self.allocator);
                try self.setExprCallSiteMembers(
                    result,
                    thread.requireSourceContext(),
                    module_idx,
                    call_expr_idx,
                ...
                if (self.readExprCallSite(result, thread.requireSourceContext(), module_idx, call_expr_idx)) |call_site| {
                    if (exactCallableInstFromSolvedCallSite(call_site)) |direct_callable_inst_id| {
                        try self.finalizeResolvedDirectCallCallableInst(
                            result,
                            thread,
                            module_idx,
        ...
        if (resolved_template_id) |template_id| {
            ...
            {
                switch (try self.specializeDirectCallExactCallable(result, thread, module_idx, call_expr_idx, call_expr, template_id)) {
        ...
        const callable_inst_id = resolved: switch (try self.specializeDirectCallExactCallable(result, thread, module_idx, call_expr_idx, call_expr, template_id)) {
    ...
    fn seedCallableParamValuesFromActualArgs(
        ...
        for (arg_patterns, arg_exprs) |pattern_idx, arg_expr_idx| {
            ...
            switch (callable_value) {
                ...
                .lambda_set => |lambda_set_id| try self.setCallableParamMembersInCallableContext(
                    result,
                    callee_callable_inst_id,
                    template.module_idx,
                    pattern_idx,
                    result.lambdasolved.getLambdaSetMembers(lambda_set_id),
                ),
            }
        }
    }
    ...
    fn recordCallableParamPatternFactsForCallableInst(
        ...
        const callable_inst = result.getCallableInst(callable_inst_id);
        if (self.getCallableParamSpecSpanForInst(callable_inst_id).isEmpty()) return;

        const template = result.getCallableTemplate(callable_inst.template);
        const arg_patterns = self.callableBoundaryArgPatterns(template.module_idx, template.cir_expr) orelse return;

        for (self.getCallableParamSpecEntries(self.getCallableParamSpecSpanForInst(callable_inst_id))) |spec| {
            ...
            switch (spec.callable_value) {
                .direct => |member_callable_inst| try self.setCallableParamDirectValueInCallableContext(
                    result,
                    callable_inst_id,
                    template.module_idx,
                    arg_patterns[spec.param_index],
                    member_callable_inst,
                ),
                .lambda_set => |lambda_set_id| {
                    const packed_members = result.lambdasolved.getLambdaSetMembers(lambda_set_id);
                    if (packed_members.len == 0) {
                        std.debug.panic(
                            "Pipeline: callable param spec for callable inst {d} param {d} had an empty solved lambda set",
                            .{ @intFromEnum(callable_inst_id), spec.param_index },
                        );
                    }

                    try self.setCallableParamMembersInCallableContext(
                        result,
                        callable_inst_id,
                        template.module_idx,
                        arg_patterns[spec.param_index],
    ...
    fn ensureCallableArgCallableInstsScanned(
        ...
        for (module_env.store.sliceExpr(args)) |arg_expr_idx| {
            ...
            if (self.getValueExprCallableMembersForThread(result, thread, module_idx, arg_expr_idx)) |callable_inst_ids| {
                for (callable_inst_ids) |callable_inst_id| {
                    try self.scanCallableInst(result, callable_inst_id);
                }
                continue;
            ...
            const callable_inst_id = self.getValueExprCallableInstForThread(result, thread, module_idx, arg_expr_idx) orelse continue;
            try self.scanCallableInst(result, callable_inst_id);
        }
    }
    ...
    fn tryCompleteTemplateBindingsFromBody(
        ...
        var thread = SemanticThread.completionThread(source_context)
        ...
        while (true) {
            ...
            switch (expr) {
                ...
                .e_closure => |closure_expr| {
                    const lambda_expr = module_env.store.getExpr(closure_expr.lambda_idx);
                    if (lambda_expr == .e_lambda) {
                        try self.scanCirValueExprWithoutDirectCallResolution(result, thread, template.module_idx, lambda_expr.e_lambda.body);
                    }
                ...
                .e_hosted_lambda => |hosted_expr| try self.scanCirValueExprWithoutDirectCallResolution(result, thread, template.module_idx, hosted_expr.body),
                else => return true,
            }

            if (!try self.tryBindTemplateBodyResultFromExactExpr(result, thread, template, bindings)) return false;
    ...
    fn tryBindTemplateBodyResultFromExactExpr(
        ...
        const body_mono = try self.resolveExprMonotypeResolved(result, thread, template.module_idx, boundary.body_expr);
    ...
    fn specializeDirectCallExactCallable(
        ...
        return .{ .resolved = try self.registerCallableInstWithCallableParamSpecs(
            result,
            thread.requireSourceContext(),
            template_id,
            fn_monotype,
    ...
    fn collectCallableParamSpecsFromArgument(
        ...
        switch (result.context_mono.monotype_store.getMonotype(monotype)) {
            .func => {
                ...
                if (self.getValueExprSolvedCallableValueForSourceContext(result, source_context, module_idx, expr_idx)) |callable_value| {
                    try self.appendCallableParamSpecEntry(out, .{
                        .param_index = param_index,
                        .projections = try self.addCallableParamProjectionEntries(projections.items),
                        .callable_value = callable_value,
                    });
                    return true;
                }
    ...
    fn requireFullyBoundExprMonotype(
        ...
        const resolved = try self.resolveExprMonotypeIfFullyBound(result, thread, module_idx, expr_idx);
        if (!resolved.isNone()) return resolved;
    ...
    fn tryBindTemplateParamsFromActualArgs(
        ...
        for (param_vars, actual_args, 0..) |param_var, arg_expr_idx, arg_i| {
            const callable_template = try self.resolveExprCallableTemplate(result, thread.requireSourceContext(), actual_module_idx, arg_expr_idx);
        ...
        for (deferred_actuals.items) |arg_i| {
            ...
            const arg_mono = if (bound_param_mono) |bound|
                ...
                try self.requireFullyBoundExprMonotype(result, thread, actual_module_idx, arg_expr_idx);
    ...
    fn inferCallableActualFromCallerParam(
        ...
        const actual_template_id = (try self.resolveExprCallableTemplate(result, thread.requireSourceContext(), actual_module_idx, arg_expr_idx)) orelse return null;
        ...
        const defining_source_context = self.resolveTemplateDefiningSourceContext(result, thread.requireSourceContext(), actual_template);
        if (!try self.tryCompleteTemplateBindingsFromBody(
            result,
            actual_template,
            &actual_bindings,
    ...
    fn appendExprCallableMembers(
        ...
        if (self.getValueExprCallableMembersForSourceContext(result, source_context, module_idx, expr_idx)) |callable_inst_ids| {
            try self.appendUniqueCallableMembers(out, callable_inst_ids);
        }
    }

    fn copyExprCallableMembers(
        self: *Pass,
        result: *Result,
    ...
    fn recordCallResultCallableMembersFromCallee(
        self: *Pass,
        result: *Result,
        target_source_context: SourceContext,
        target_module_idx: u32,
    ...
    fn realizeStructuredExprCallableSemantics(
        ...
        switch (module_env.store.getExpr(expr_idx)) {
            ...
            .e_if => |if_expr| {
                for (module_env.store.sliceIfBranches(if_expr.branches)) |branch_idx| {
                    const branch = module_env.store.getIfBranch(branch_idx);
                    try self.appendExprCallableMembers(result, source_context, module_idx, branch.body, visiting, &callable_inst_ids);
                }
                try self.appendExprCallableMembers(result, source_context, module_idx, if_expr.final_else, visiting, &callable_inst_ids);
            },
            .e_match => |match_expr| {
                for (module_env.store.sliceMatchBranches(match_expr.branches)) |branch_idx| {
                    const branch = module_env.store.getMatchBranch(branch_idx);
    ...
    fn specializeDispatchExactCallable(
        ...
        if (resolveFuncTypeInStore(template_types, template.type_root)) |resolved_func| {
            ...
            const ret_mono = try self.resolveExprMonotypeResolved(result, thread, module_idx, expr_idx);
            if (!ret_mono.isNone()) {
                if (self.callableBoundaryInfo(template.module_idx, template.cir_expr)) |boundary| {
                    if (!try self.tryBindTypeVarMonotypes(
                        result,
    ...
    fn resolveBinopDispatchTarget(
        ...
        return self.resolveDispatchTargetForExpr(
            result,
            thread.withBindings(&merged_bindings),
            module_idx,
            expr_idx,
            method_name,
        );
    ...
    fn materializeLookupExprCallableValue(
        ...
        const callable_inst_id = if (existing_callable_inst_id) |callable_inst_id| blk: {
            ...
            break :blk if (thread.isTracked())
                try self.requireCallableInst(result, thread.requireSourceContext(), template_id, desired_fn_monotype.idx, desired_fn_monotype.module_idx)
            else
                try self.registerCallableInst(result, thread.requireSourceContext(), template_id, desired_fn_monotype.idx, desired_fn_monotype.module_idx);
        };
        try self.setExprDirectCallable(
            result,
            thread.requireSourceContext(),
    ...
    fn internLambdaSet(
    ...
    ) Allocator.Error!LambdaSetId {
        for (result.lambdasolved.lambda_sets.items, 0..) |_, idx| {
            const existing_members = result.lambdasolved.getLambdaSetMembers(@enumFromInt(idx));
        ...
        const span: Lambdasolved.LambdaSetMemberSpan = if (members.len == 0)
            Lambdasolved.LambdaSetMemberSpan.empty()
        else blk: {
            const start: u32 = @intCast(result.lambdasolved.lambda_set_member_entries.items.len);
            try result.lambdasolved.lambda_set_member_entries.appendSlice(self.allocator, members);
            break :blk .{
                .start = start,
        ...
        const set_id: LambdaSetId = @enumFromInt(result.lambdasolved.lambda_sets.items.len);
        const new_set: @TypeOf(result.lambdasolved.lambda_sets.items[0]) = .{ .members = span };
        try self.appendTracked(.solved_lambda_sets, &result.lambdasolved.lambda_sets, new_set);
        return set_id;
    }

    fn internCallableMemberGroup(
        self: *Pass,
        result: *Result,
        members: []const CallableInstId,
    ) Allocator.Error!Lambdamono.CallableMemberGroupId {
        for (result.lambdamono.callable_member_groups.items, 0..) |_, idx| {
            const existing_members = result.lambdamono.getCallableMemberGroupMembers(@enumFromInt(idx));
            if (existing_members.len != members.len) continue;
        ...
        const set_id: Lambdamono.CallableMemberGroupId = @enumFromInt(result.lambdamono.callable_member_groups.items.len);
        const new_set: @TypeOf(result.lambdamono.callable_member_groups.items[0]) = .{ .members = span };
        try self.appendTracked(.packed_fns, &result.lambdamono.callable_member_groups, new_set);
        return set_id;
    }

    fn appendUniqueCallableMembers(
        self: *Pass,
        members: *std.ArrayList(CallableInstId),
        callable_inst_ids: []const CallableInstId,
    ) Allocator.Error!void {
        for (callable_inst_ids) |callable_inst_id| {
            for (members.items) |existing_callable_inst_id| {
                if (existing_callable_inst_id == callable_inst_id) break;
            } else {
                try members.append(self.allocator, callable_inst_id);
            }
        }
    }

    fn normalizeCallableMembers(
        self: *Pass,
        members: *std.ArrayList(CallableInstId),
        callable_inst_ids: []const CallableInstId,
    ) Allocator.Error!void {
        try self.appendUniqueCallableMembers(members, callable_inst_ids);
        std.mem.sortUnstable(CallableInstId, members.items, {}, struct {
    ...
    fn solveCallableValueFromMembers(
        self: *Pass,
        result: *Result,
        callable_inst_ids: []const CallableInstId,
    ) Allocator.Error!SolvedCallableValue {
        if (callable_inst_ids.len == 0) unreachable;

        var normalized = std.ArrayList(CallableInstId).empty;
        defer normalized.deinit(self.allocator);
        try self.normalizeCallableMembers(&normalized, callable_inst_ids);
        try self.ensureRecordedCallableInstsScanned(result, normalized.items);

        if (normalized.items.len == 1) {
            return .{ .direct = normalized.items[0] };
        }

        return .{ .lambda_set = try self.internLambdaSet(result, normalized.items) };
    }

    fn solveCallSiteFromMembers(
        self: *Pass,
        result: *Result,
        callable_inst_ids: []const CallableInstId,
    ) Allocator.Error!SolvedCall {
        const callable_value = try self.solveCallableValueFromMembers(result, callable_inst_ids);
        return switch (callable_value) {
            .direct => |callable_inst_id| .{ .direct = callable_inst_id },
            .lambda_set => |lambda_set_id| .{ .lambda_set = lambda_set_id },
        };
    }

    fn setExprCallSiteMembers(
        self: *Pass,
        result: *Result,
        source_context: SourceContext,
        module_idx: u32,
        ...
        try self.writeExprCallSite(
            ...
            module_idx,
            expr_idx,
            try self.solveCallSiteFromMembers(result, callable_inst_ids),
        );
    }
    ...
    fn setExprDirectCallSite(
        ...
        return self.setExprCallSiteMembers(result, source_context, module_idx, expr_idx, &.{callable_inst_id});
    }
    ...
    fn setCallableParamMembersInCallableContext(
        self: *Pass,
        result: *Result,
        context_callable_inst: CallableInstId,
        module_idx: u32,
        ...
        return self.setCallableParamMembers(
            result,
            callableInstSourceContext(context_callable_inst),
            module_idx,
            pattern_idx,
    ...
    fn setCallableParamMembers(
        ...
        try self.writeCallableParamValue(
            ...
            module_idx,
            pattern_idx,
            try self.solveCallableValueFromMembers(result, callable_inst_ids),
        );
    }

    fn ensurePackedFnForLambdaSet(
        self: *Pass,
        result: *Result,
        lambda_set_id: LambdaSetId,
    ) Allocator.Error!PackedFnId {
        const member_group_id = try self.internCallableMemberGroup(
            result,
            result.lambdasolved.getLambdaSetMembers(lambda_set_id),
        );
        if (result.lambdamono.packed_fn_ids_by_member_group.get(member_group_id)) |existing| return existing;

        const fn_monotype = try self.requireLambdaSetFnMonotype(
            result,
            result.lambdasolved.getLambdaSetMembers(lambda_set_id),
        );
        ...
        const packed_fn: @TypeOf(result.lambdamono.packed_fns.items[0]) = .{
            .members = result.lambdamono.callable_member_groups.items[@intFromEnum(member_group_id)].members,
        ...
        try self.putTracked(
            .packed_fns,
            &result.lambdamono.packed_fn_ids_by_member_group,
            member_group_id,
            packed_fn_id,
        );
        return packed_fn_id;
    ...
    fn requireLambdaSetFnMonotype(
        ...
        for (callable_inst_ids[1..]) |callable_inst_id| {
            ...
            if (!try self.monotypesStructurallyEqualAcrossModules(
                ...
                std.debug.panic(
                    "Pipeline invariant violated: lambda set contained mismatched fn monotypes between callable insts {d} and {d}",
    ...
    fn ensureIndirectCallForLambdaSet(
        self: *Pass,
        result: *Result,
        lambda_set_id: LambdaSetId,
        ...
        if (result.lambdamono.indirect_call_ids_by_member_group.get(member_group_id)) |existing| return existing;
        ...
        try self.putTracked(
            .indirect_calls,
            &result.lambdamono.indirect_call_ids_by_member_group,
            member_group_id,
            indirect_call_id,
        );
    ...
    fn resolveTupleElemSourceExpr(
        ...
        var visiting: std.AutoHashMapUnmanaged(ContextExprVisitKey, void) = .empty;
        defer visiting.deinit(self.allocator);
        return self.resolveTupleElemExpr(result, source_context, module_idx, expr_idx, elem_index, &visiting);
    }
    ...
    fn resolveListElemSourceExpr(
        ...
        var visiting: std.AutoHashMapUnmanaged(ContextExprVisitKey, void) = .empty;
        defer visiting.deinit(self.allocator);
        return self.resolveListElemExpr(result, source_context, module_idx, expr_idx, elem_index, &visiting);
    }
            ...
            .e_record => |record| blk: {
                for (module_env.store.sliceRecordFields(record.fields)) |field_idx| {
                    ...
                    if (self.identsStructurallyEqualAcrossModules(module_idx, field.name, field_name_module_idx, field_name)) {
                        break :blk .{ .source = .{
                            .source_context = replayExprSourceContext(source_context, module_idx, field.value),
                            .module_idx = module_idx,
    ...
    fn resolveTupleElemExprInContext(
        ...
        return switch (module_env.store.getExpr(expr_idx)) {
            .e_tuple => |tuple_expr| blk: {
                ...
                break :blk .{ .source = .{
                    .source_context = replayExprSourceContext(source_context, module_idx, elems[elem_index]),
                    .module_idx = module_idx,
    ...
    fn resolveExprCallableTemplateWithVisited(
        ...
        const resolved = switch (expr) {
            ...
            .e_lookup_local => |lookup| blk: {
                ...
                break :blk try self.resolveExprCallableTemplateWithVisited(result, replayExprSourceContext(source_context, source.module_idx, source.expr_idx), source.module_idx, source.expr_idx, visiting);
            },
            .e_lookup_external => |lookup| blk: {
                const target_module_idx = self.resolveImportedModuleIdx(module_env, lookup.module_idx) orelse break :blk null;
    ...
    fn resolveRecordFieldExpr(
        ...
        return switch (module_env.store.getExpr(expr_idx)) {
            ...
            .e_lookup_local => |lookup| blk: {
                const source = result.getPatternSourceExpr(module_idx, lookup.pattern_idx) orelse break :blk null;
                break :blk try self.resolveRecordFieldExpr(result, replayExprSourceContext(source_context, source.module_idx, source.expr_idx), source.module_idx, source.expr_idx, field_name_module_idx, field_name, visiting);
            },
            .e_lookup_external => |lookup| blk: {
                const target_module_idx = self.resolveImportedModuleIdx(module_env, lookup.module_idx) orelse break :blk null;
                const source = try self.resolveExternalDefSourceExpr(result, target_module_idx, lookup.target_node_idx) orelse break :blk null;
    ...
    fn resolveTupleElemExpr(
        ...
        return switch (module_env.store.getExpr(expr_idx)) {
            ...
            .e_lookup_local => |lookup| blk: {
                const source = result.getPatternSourceExpr(module_idx, lookup.pattern_idx) orelse break :blk null;
                break :blk try self.resolveTupleElemExpr(result, replayExprSourceContext(source_context, source.module_idx, source.expr_idx), source.module_idx, source.expr_idx, elem_index, visiting);
            },
            .e_lookup_external => |lookup| blk: {
                const target_module_idx = self.resolveImportedModuleIdx(module_env, lookup.module_idx) orelse break :blk null;
                const source = try self.resolveExternalDefSourceExpr(result, target_module_idx, lookup.target_node_idx) orelse break :blk null;
    ...
    fn resolveListElemExpr(
        ...
        return switch (module_env.store.getExpr(expr_idx)) {
            ...
            .e_lookup_local => |lookup| blk: {
                const source = result.getPatternSourceExpr(module_idx, lookup.pattern_idx) orelse break :blk null;
                break :blk try self.resolveListElemExpr(result, replayExprSourceContext(source_context, source.module_idx, source.expr_idx), source.module_idx, source.expr_idx, elem_index, visiting);
            },
            .e_lookup_external => |lookup| blk: {
                const target_module_idx = self.resolveImportedModuleIdx(module_env, lookup.module_idx) orelse break :blk null;
                const source = try self.resolveExternalDefSourceExpr(result, target_module_idx, lookup.target_node_idx) orelse break :blk null;
    ...
    fn ensureCallableInstRecord(
        ...
        for (result.lambdamono.callable_insts.items, 0..) |existing_callable_inst, idx| {
            ...
            if (!callableParamSpecsEqual(
                self,
                self.getCallableParamSpecEntries(self.getCallableParamSpecSpanForInst(@enumFromInt(idx))),
                callable_param_specs,
            )) continue;
            ...
            if (mono_equal) {
                ...
                try self.ensureDirectCallableMemberGroup(result, existing_id);
                return existing_id;
            }
        }
        ...
        try self.callable_param_specs_by_inst.put(
            self.allocator,
            callable_inst_id,
            try self.addCallableParamSpecEntries(callable_param_specs),
        );
        try self.ensureDirectCallableMemberGroup(result, callable_inst_id);
        return callable_inst_id;
    }
    ...
    fn addCallableParamSpecEntries(
        ...
        try self.callable_param_spec_entries.appendSlice(self.allocator, entries);
        return .{
            .start = start,
    ...
    fn addCallableParamProjectionEntries(
        ...
        const start: u32 = @intCast(self.callable_param_projection_entries.items.len);
        try self.callable_param_projection_entries.appendSlice(self.allocator, entries);
        return .{
            .start = start,
            .len = @intCast(entries.len),
        };
    ...
    fn ensureDirectCallableMemberGroup(
        self: *Pass,
        result: *Result,
        callable_inst_id: CallableInstId,
    ) Allocator.Error!void {
        if (result.lambdamono.direct_callable_member_group_ids_by_callable_inst.contains(callable_inst_id)) return;
        const member_group_id = try self.internCallableMemberGroup(result, &.{callable_inst_id});
        try result.lambdamono.direct_callable_member_group_ids_by_callable_inst.put(self.allocator, callable_inst_id, member_group_id);
    }
    ...
    fn appendCallableParamSpecEntry(
        ...
        for (out.items) |existing| {
            ...
            if (!callableParamProjectionsEqual(
                self.getCallableParamProjectionEntries(existing.projections),
                self.getCallableParamProjectionEntries(entry.projections),
            )) continue;
            return;
        }
    ...
    fn callableParamSpecsEqual(
        ...
        for (lhs, rhs) |lhs_entry, rhs_entry| {
            ...
            if (!callableParamProjectionsEqual(
                self.getCallableParamProjectionEntries(lhs_entry.projections),
                self.getCallableParamProjectionEntries(rhs_entry.projections),
            )) return false;
    ...
    fn scanCallableInst(self: *Pass, result: *Result, callable_inst_id: CallableInstId) Allocator.Error!void {
        ...
        const thread = SemanticThread.trackedThread(callableInstSourceContext(callable_inst_id))
            .withBindings(&bindings)
            .withIterationMonotypes(&iteration_expr_monotypes, &iteration_pattern_monotypes);
        ...
        while (true) {
            ...
            switch (module_env.store.getExpr(template.cir_expr)) {
                .e_lambda => |lambda_expr| {
                    try self.scanCirValueExpr(result, thread, template.module_idx, lambda_expr.body);
                },
                .e_closure => |closure_expr| {
                    const lambda_expr = module_env.store.getExpr(closure_expr.lambda_idx);
                    if (lambda_expr == .e_lambda) {
                        try self.scanCirValueExpr(result, thread, template.module_idx, lambda_expr.e_lambda.body);
                    }
                },
                .e_hosted_lambda => |hosted_expr| try self.scanCirValueExpr(result, thread, template.module_idx, hosted_expr.body),
                else => unreachable,
            }
    ...
    fn resolveExprMonotypeResolved(
        ...
        const source_context = thread.requireSourceContext();
        return self.resolveExprMonotypeResolvedForSourceContext(
            result,
            thread,
            source_context,
            module_idx,
    ...
    fn resolveExprMonotypeResolvedForSourceContext(
        self: *Pass,
        result: *Result,
        thread: SemanticThread,
        source_context: SourceContext,
        ...
        if (self.intrinsicExprResultMonotype(result, module_idx, expr_idx)) |resolved| {
            return resolved;
        }
        ...
        const resolved = try self.resolveTypeVarMonotypeResolved(result, thread, module_idx, ModuleEnv.varFrom(expr_idx));
        if (!resolved.isNone()) return resolved;
    ...
    fn resolveExprMonotypeIfFullyBound(
        ...
        const source_context = thread.requireSourceContext();
        return self.resolveExprMonotypeIfFullyBoundForSourceContext(
            result,
            thread,
            source_context,
            module_idx,
    ...
    fn resolveExprMonotypeIfFullyBoundForSourceContext(
        self: *Pass,
        result: *Result,
        thread: SemanticThread,
        source_context: SourceContext,
    ...
    fn intrinsicExprResultMonotype(
        self: *Pass,
        result: *Result,
        module_idx: u32,
        expr_idx: CIR.Expr.Idx,
        ...
        switch (expr) {
            .e_run_low_level => |run_low_level| {
                if (run_low_level.op == .str_inspect) {
                    return resolvedMonotype(result.context_mono.monotype_store.primIdx(.str), module_idx);
                }
            },
            .e_call => |call_expr| {
                const low_level_op = self.getCallLowLevelOp(module_env, call_expr.func) orelse return null;
