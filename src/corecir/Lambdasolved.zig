...
pub fn exprVisitStillPendingForThread(
    ...
    return root_analysis.exprVisitStillPending(driver.resultExprKeyForThread(thread, module_idx, expr_idx));
}
...
pub fn scanDemandedValueDefExpr(
    ...
    const key = driver.resultExprKeyForThread(thread, module_idx, expr_idx);
    const Worker = struct {
...
pub fn resolveBoundCapturePatternMonotype(
    ...
    const binding_mono = try driver.resolvePatternMonotypeResolved(
        result,
        binding_thread,
    ...
    var source_mono = (try driver.resolveExprMonotypeResolved(
        result,
        source_thread,
        bound_expr.expr_ref.module_idx,
        bound_expr.expr_ref.expr_idx,
    ...
    for (result.lambdamono.getValueProjectionEntries(bound_expr.expr_ref.projections)) |projection| {
        source_mono = try driver.projectResolvedMonotypeByValueProjection(result, source_mono, projection);
        if (source_mono.isNone()) return null;
    }
    try driver.mergeContextPatternMonotype(
        result,
        driver.resultPatternKeyForSourceContext(
...
pub fn resolveCaptureValueSourceMonotype(
    ...
    return switch (contextualizeCaptureValueSource(
        ...
        .lexical_pattern => |lexical| blk: {
            ...
            if (driver.lookupContextTypeVarMonotype(
                result,
                lexical.source_context,
                lexical.module_idx,
...
pub fn resolveCaptureCallableValue(
    ...
    return switch (contextualizeCaptureValueSource(
        ...
        .bound_expr => |bound_expr| blk: {
            if (driver.readCallableParamValue(
                result,
                bound_expr.binding_source_context,
                bound_expr.binding_module_idx,
                bound_expr.binding_pattern_idx,
            ...
            if (try driver.resolveExprRefCallableValue(result, bound_expr.expr_ref, visiting)) |callable_value| {
            ...
            if (bound_expr.expr_ref.projections.isEmpty()) {
                ...
                if (try driver.resolveExprRefCallableValue(result, materialization_expr_ref, visiting)) |callable_value| {
                    break :blk callable_value;
                }
                ...
                if (maybe_template_id) |template_id| {
                    const fn_monotype = driver.resolvedIfFunctionMonotype(
                        result,
                        try driver.resolveExprMonotypeResolved(
                            result,
                    ...
                    const fn_monotype = (try driver.resolveExprMonotypeResolved(
                        result,
                        SemanticThread.trackedThread(materialization_expr_ref.source_context),
                        materialization_expr_ref.module_idx,
                        materialization_expr_ref.expr_idx,
        ...
        .lexical_pattern => |lexical| driver.readCallableParamValue(
            result,
            lexical.source_context,
            lexical.module_idx,
            lexical.pattern_idx,
...
pub fn updateCallableDefRuntimeValue(
    ...
    const capture_span: CaptureFieldSpan = if (capture_fields.len == 0)
        ...
        try result.lambdamono.capture_fields.appendSlice(driver.allocator, capture_fields);
        break :blk .{
            .start = start,
    ...
    try withRootAnalysis(driver.allocator, struct {
        ...
        fn run(self: @This(), root_analysis: *RootAnalysisState) std.mem.Allocator.Error!void {
            ...
            for (module_env.store.sliceExpr(self.args_inner)) |arg_expr_idx| {
                ...
                _ = self.driver_inner.getValueExprCallableValueForThread(
                    self.result_inner,
                    self.thread_inner,
                    self.module_idx_inner,
                    arg_expr_idx,
...
pub fn realizeCallableArgSemanticsSlice(
    ...
    for (arg_exprs) |arg_expr_idx| {
        ...
        _ = driver.getValueExprCallableValueForThread(result, thread, module_idx, arg_expr_idx);
    }
}
...
pub fn collectCallableParamSpecsFromFunctionArgument(
    ...
    const Worker = struct {
        ...
        fn run(self: *@This(), root_analysis: *RootAnalysisState) std.mem.Allocator.Error!void {
            ...
            if (try self.driver_inner.resolveProjectedExprCallableValue(
                self.result_inner,
                ...
                try self.driver_inner.appendCallableParamSpecEntry(self.result_inner, self.out_inner, .{
                    .param_index = self.param_index_inner,
                    .projections = try self.driver_inner.addCallableParamProjectionEntries(
                        self.result_inner,
                        self.projections_inner.items,
                    ),
...
pub fn collectCallableParamSpecsFromArgument(
    ...
    try driver.propagateDemandedValueMonotypeToValueExpr(
        result,
        source_context,
        module_idx,
        expr_idx,
...
pub fn bindCurrentCallFromCallableInst(
    ...
    for (arg_exprs, 0..) |arg_expr_idx, i| {
        ...
        if (result.context_mono.monotype_store.getMonotype(param_mono) != .func) {
            try driver.bindCurrentExprTypeRoot(result, thread, module_idx, arg_expr_idx, param_mono, callable_inst.fn_monotype_module_idx);
            try driver.propagateDemandedValueMonotypeToValueExpr(
                result,
                thread.requireSourceContext(),
                module_idx,
    ...
    if (result.context_mono.monotype_store.getMonotype(fn_mono.ret) != .func) {
        try driver.bindCurrentExprTypeRoot(result, thread, module_idx, call_expr_idx, fn_mono.ret, callable_inst.fn_monotype_module_idx);
        try driver.recordCurrentExprMonotype(result, thread, module_idx, call_expr_idx, fn_mono.ret, callable_inst.fn_monotype_module_idx);
    }
}
...
pub fn bindCurrentCallFromFnMonotype(
    ...
    for (arg_exprs, 0..) |arg_expr_idx, i| {
        const param_mono = result.context_mono.monotype_store.getIdxSpanItem(fn_mono.args, i);
        if (result.context_mono.monotype_store.getMonotype(param_mono) != .func) {
            try driver.bindCurrentExprTypeRoot(result, thread, module_idx, arg_expr_idx, param_mono, fn_monotype_module_idx);
            try driver.propagateDemandedValueMonotypeToValueExpr(
                result,
                thread.requireSourceContext(),
                module_idx,
    ...
    if (result.context_mono.monotype_store.getMonotype(fn_mono.ret) != .func) {
        try driver.bindCurrentExprTypeRoot(result, thread, module_idx, call_expr_idx, fn_mono.ret, fn_monotype_module_idx);
        try driver.recordCurrentExprMonotype(result, thread, module_idx, call_expr_idx, fn_mono.ret, fn_monotype_module_idx);
    }
}
...
pub fn specializeDirectCallExactCallable(
    ...
    if (try driver.directCallContainsErrorType(module_idx, call_expr_idx, call_expr)) {
        return null;
    }

    const template = result.getCallableTemplate(template_id).*;
    const defining_source_context = driver.resolveTemplateDefiningSourceContext(result, template_source_context, template);
    const arg_exprs = driver.all_module_envs[module_idx].store.sliceExpr(call_expr.args);
    ...
    const CallableParamSpecEntry = @TypeOf(result.lambdamono.callable_param_spec_entries.items[0]);
    ...
    if (std.debug.runtime_safety) {
        ...
        for (result.context_mono.monotype_store.getIdxSpan(func_mono.args), 0..) |param_mono, param_index| {
            if (!try driver.monotypeRequiresCallableParamSpec(result, param_mono)) continue;
    ...
    return try driver.requireCallableInstWithCallableParamSpecs(
        result,
        defining_source_context,
        template_id,
        fn_monotype,
...
pub fn assignCallableArgCallableInstsFromParams(
    ...
    for (arg_exprs, 0..) |arg_expr_idx, i| {
        ...
        const callable_inst_id = (try driver.ensureCallableInstRecord(
            result,
            thread.requireSourceContext(),
            template_id,
            param_mono,
...
pub fn seedCallableParamValuesFromActualArgs(
    ...
    for (arg_patterns, arg_exprs) |pattern_idx, arg_expr_idx| {
        ...
        const callable_value = driver.getValueExprCallableValueForSourceContext(
            result,
            thread.requireSourceContext(),
            module_idx,
            arg_expr_idx,
...
pub fn assignCallableArgCallableInstsFromCallMonotype(
    ...
    if (driver.getCallSiteCallableInstForThread(result, thread, module_idx, call_expr_idx) != null) return;
...
pub fn finalizeResolvedDirectCallCallableInst(
    ...
    var visiting: std.AutoHashMapUnmanaged(ContextExprKey, void) = .empty;
    defer visiting.deinit(driver.allocator);
    var variant_builder = driver.initCallableVariantBuilder();
    defer driver.deinitCallableVariantBuilder(&variant_builder);
    ...
    if (try driver.finishCallableVariantBuilderValue(&variant_builder, result)) |call_result_value| {
    ...
    if (result.context_mono.monotype_store.getMonotype(callable_inst_fn_mono.ret) != .func) {
        try driver.recordExprMonotypeForThread(
            result,
            thread,
            module_idx,
            call_expr_idx,
...
pub fn inferCallableParamPatternValueForCallableInst(
...
) std.mem.Allocator.Error!?@TypeOf(result.lambdamono.callable_param_spec_entries.items[0].callable_value) {
    const callable_inst = result.getCallableInst(callable_inst_id);
    var variant_builder = driver.initCallableVariantBuilder();
    defer driver.deinitCallableVariantBuilder(&variant_builder);
    ...
    return try driver.finishCallableVariantBuilderValue(&variant_builder, result);
}
...
pub fn realizeCallableInst(
    ...
    switch (template.binding) {
        .pattern => |binding_pattern| {
            try driver.mergeContextPatternMonotype(
                result,
                driver.resultPatternKey(callable_inst_id, template.module_idx, binding_pattern),
...
pub fn scanStmt(
    ...
    switch (stmt) {
        .s_decl => |decl| {
            ...
            if (driver.readCallableParamValue(result, thread.requireSourceContext(), module_idx, decl.pattern) == null) {
                if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, decl.expr)) |expr_mono| {
                    try driver.bindCurrentPatternFromResolvedMonotype(
                        result,
                        thread,
                        module_idx,
                        decl.pattern,
        ...
        .s_var => |var_decl| {
            ...
            if (driver.readCallableParamValue(result, thread.requireSourceContext(), module_idx, var_decl.pattern_idx) == null) {
                if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, var_decl.expr)) |expr_mono| {
                    try driver.bindCurrentPatternFromResolvedMonotype(
                        result,
                        thread,
                        module_idx,
        ...
        .s_reassign => |reassign| {
            ...
            if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, reassign.expr)) |expr_mono| {
                try driver.bindCurrentPatternFromResolvedMonotype(
                    result,
                    thread,
                    module_idx,
        ...
        .s_for => |for_stmt| {
            ...
            if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, for_stmt.expr)) |iter_mono| {
                ...
                try driver.bindCurrentPatternFromResolvedMonotype(result, thread, module_idx, for_stmt.patt, item_mono);
            }
...
pub fn scanCirExprInternal(
    ...
    if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, expr_idx)) |monotype| {
        try driver.recordExprMonotypeForThread(
            result,
            thread,
            module_idx,
    ...
    const Worker = struct {
        ...
        fn run(self: @This()) std.mem.Allocator.Error!void {
            ...
            {
                if (self.driver.getValueExprCallableValueForThread(self.result, self.thread, self.module_idx, self.expr_idx) == null) {
                    var visiting: std.AutoHashMapUnmanaged(ContextExprKey, void) = .empty;
...
pub fn completeCurrentExprMonotype(
    ...
    const resolved = (try driver.resolveExprMonotypeResolved(result, thread, module_idx, expr_idx)) orelse return;
    try driver.recordExprMonotypeForThread(
        result,
        thread,
        module_idx,
...
pub fn materializeDemandedExprCallableInst(
    ...
    const fn_monotype = try driver.lookupRecordedExprMonotypeIfReady(
        result,
        thread,
        module_idx,
        expr_idx,
...
pub fn extendExprRef(
    ...
    return .{
        ...
        .projections = try driver.appendValueProjectionEntries(
            result,
            source.projections,
            &.{projection},
        ),
...
pub fn propagatePatternDemandToExpr(
    ...
    const pattern_mono = try driver.resolvePatternMonotypeResolved(
        result,
        thread,
        module_idx,
        pattern_idx,
    ...
    if (result.getContextPatternSourceExpr(thread.requireSourceContext(), module_idx, pattern_idx)) |source| {
        try driver.propagateDemandedValueMonotypeToExprRef(
            result,
            source,
            pattern_mono.idx,
    ...
    switch (callable_value) {
        .direct => |callable_inst_id| {
            ...
            if (result.context_mono.monotype_store.getMonotype(callable_inst.fn_monotype) != .func) {
                try driver.recordCurrentExprMonotype(
                    result,
                    thread,
                    module_idx,
                    expr_idx,
...
pub fn seedCallableBoundaryContextMonotypes(
    ...
    try driver.recordExprMonotypeForSourceContext(
        result,
        source_context,
        module_idx,
        callable_expr_idx,
    ...
    for (boundary_arg_patterns, 0..) |pattern_idx, i| {
        ...
        try driver.mergeContextPatternMonotype(
            result,
            driver.resultPatternKeyForSourceContext(source_context, module_idx, pattern_idx),
...
pub fn includeExprCallableValue(
    ...
    if (driver.getValueExprCallableValueForSourceContext(result, source_context, module_idx, expr_idx)) |callable_value| {
...
pub fn materializeExprCallableValueWithKnownFnMonotype(
    ...
    if (try driver.monotypeContainsNestedCallableValueShape(result, fn_monotype)) {
...
pub fn materializeLookupExprCallableValue(
    ...
    const module_env = driver.all_module_envs[module_idx];
    const template = result.getCallableTemplate(template_id).*;
    const desired_fn_monotype = driver.resolvedIfFunctionMonotype(
        result,
        try driver.resolveExprMonotypeResolved(result, thread, module_idx, expr_idx),
    );
    const existing_callable_inst_id = if (module_env.store.getExpr(expr_idx) == .e_lookup_local) blk: {
        ...
        if (driver.readCallableParamValue(result, thread.requireSourceContext(), module_idx, lookup.pattern_idx)) |callable_value| {
            switch (callable_value) {
    ...
    const callable_inst_id = if (existing_callable_inst_id) |callable_inst_id| blk: {
        ...
        break :blk (try driver.ensureCallableInstRecord(
            result,
            thread.requireSourceContext(),
            template_id,
            fn_monotype.idx,
...
pub fn realizeLookupExprSemantics(
    ...
    switch (expr) {
        .e_lookup_local => |lookup| {
            ...
            } else if (driver.findDefByPatternInModule(module_env, lookup.pattern_idx)) |def_idx| {
                try writeExprLookupResolution(
                    driver,
                    result,
                    source_context,
        ...
        .e_lookup_external => |lookup| {
            const target_module_idx = driver.resolveImportedModuleIdx(module_env, lookup.module_idx) orelse return;
        ...
        .e_lookup_required => |lookup| {
            const target = driver.resolveRequiredLookupTarget(module_env, lookup) orelse return;
...
pub fn scanCirExprChildren(
    ...
    switch (expr) {
        ...
        .e_tuple => |tuple_expr| {
            try driver.recordDemandedTupleElemMonotypes(result, thread, module_idx, expr_idx, tuple_expr);
        ...
        .e_match => |match_expr| {
            ...
            try driver.recordMatchBranchPatternSources(result, thread, module_idx, match_expr);
            if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, match_expr.cond)) |cond_mono| {
                try driver.bindMatchBranchPatternsFromCondMonotype(result, thread, module_idx, match_expr, cond_mono);
            }

            const branches = module_env.store.sliceMatchBranches(match_expr.branches);
            for (branches) |branch_idx| {
                const branch = module_env.store.getMatchBranch(branch_idx);
                try driver.propagateDemandedValueResultMonotypeToChild(result, thread, module_idx, expr_idx, branch.value);
        ...
        .e_if => |if_expr| {
            ...
            for (branches) |branch_idx| {
                ...
                try driver.propagateDemandedValueResultMonotypeToChild(result, thread, module_idx, expr_idx, branch.body);
            ...
            try driver.propagateDemandedValueResultMonotypeToChild(result, thread, module_idx, expr_idx, if_expr.final_else);
        ...
        .e_call => |call_expr| {
            ...
            if (!driver.calleeUsesFirstClassCallableValuePath(callee_expr)) {
                try scanCirValueExprWithDirectCallResolution(
                    driver,
                    root_analysis,
                    result,
            ...
            if (result.getExprLowLevelOp(thread.requireSourceContext(), module_idx, call_expr.func)) |low_level_op| {
                if (low_level_op == .str_inspect and arg_exprs.len != 0) {
                    try driver.resolveStrInspectHelperCallableInstsForTypeVar(
                        result,
                        thread,
                        module_idx,
            ...
            if (resolve_direct_calls) {
                ...
                if (driver.getCallSiteForThread(result, thread, module_idx, expr_idx) == null and
                    driver.calleeUsesFirstClassCallableValuePath(callee_expr))
                {
            ...
            if (resolve_direct_calls and
                driver.getCallSiteCallableInstForThread(result, thread, module_idx, expr_idx) == null)
            {
                try resolveDirectCallSite(driver, root_analysis, result, thread, module_idx, expr_idx, call_expr);
            }
        },
        .e_record => |record_expr| {
            ...
            try driver.recordDemandedRecordFieldMonotypes(result, thread, module_idx, expr_idx, record_expr);
        ...
        .e_block => |block_expr| {
            ...
            try driver.propagateDemandedValueResultMonotypeToChild(result, thread, module_idx, expr_idx, block_expr.final_expr);
        ...
        .e_binop => |binop_expr| {
            ...
            try driver.realizeDispatchExprSemantics(result, thread, module_idx, expr_idx, expr);
        },
        ...
        .e_dbg => |dbg_expr| {
            try driver.propagateDemandedValueResultMonotypeToChild(result, thread, module_idx, expr_idx, dbg_expr.expr);
        ...
        .e_expect => |expect_expr| {
            try driver.propagateDemandedValueResultMonotypeToChild(result, thread, module_idx, expr_idx, expect_expr.body);
...
pub fn requireTemplateDemandSourceContextForExpr(
    ...
    if (result.getExprOriginExpr(source_context, module_idx, expr_idx)) |source| {
        ...
        if (driver.getValueExprCallableValueForSourceContext(
            result,
...
pub fn resolveDirectCallFnMonotype(
    ...
    _ = call_expr_idx;
    const callee_monotype = (try driver.resolveExprMonotypeResolved(result, thread, module_idx, call_expr.func)) orelse return null;
...
pub fn resolveDirectCallSite(
    ...
    if (resolved_template_id == null and desired_fn_monotype != null) {
        var visiting: std.AutoHashMapUnmanaged(ContextExprKey, void) = .empty;
        defer visiting.deinit(driver.allocator);
        try driver.propagateDemandedCallableFnMonotypeToValueExpr(
            result,
            thread.requireSourceContext(),
            module_idx,
            callee_expr_idx,
    ...
    if (resolved_template_id == null) {
        if (driver.getValueExprCallableInstForThread(result, thread, module_idx, callee_expr_idx)) |callable_inst_id| {
            resolved_template_id = result.getCallableInst(callable_inst_id).template;
        }
    }
    if (resolved_template_id == null and !driver.calleeUsesFirstClassCallableValuePath(callee_expr)) {
        if (std.debug.runtime_safety) {
            std.debug.panic(
            ...
            .e_lookup_external, .e_lookup_required => {
                ...
                for (arg_exprs) |arg_expr_idx| {
                    try arg_monos.append(
                        driver.allocator,
                        try driver.requireFullyBoundExprMonotype(
                            result,
                            thread,
                            module_idx,
                            arg_expr_idx,
    ...
    if (std.debug.runtime_safety) switch (callee_expr) {
        .e_lookup_external, .e_lookup_required => {
            const current_call_site = driver.getCallSiteForThread(result, thread, module_idx, call_expr_idx);
    ...
    if (driver.getValueExprCallableValueForThread(result, thread, module_idx, callee_expr_idx)) |callable_value| {
        ...
        if (all_satisfy) {
            ...
            var variant_builder = driver.initCallableVariantBuilder();
            defer driver.deinitCallableVariantBuilder(&variant_builder);
            var call_variant_builder = driver.initCallableVariantBuilder();
            defer driver.deinitCallableVariantBuilder(&call_variant_builder);
            ...
            if (driver.readExprCallSite(result, thread.requireSourceContext(), module_idx, call_expr_idx)) |call_site| {
                switch (call_site) {
...
pub fn realizeStructuredExprCallableSemantics(
    ...
    const visit_key = driver.contextExprVisitKeyForSourceContext(source_context, module_idx, expr_idx);
    if (visiting.contains(visit_key)) return;
    try visiting.put(driver.allocator, visit_key, {});
    defer _ = visiting.remove(visit_key);

    if (driver.readExprCallableValue(result, source_context, module_idx, expr_idx) != null) {
        return;
    }
    ...
    const expr = module_env.store.getExpr(expr_idx);
    if (result.getExprOriginExpr(source_context, module_idx, expr_idx)) |source| {
        if (!(driver.sameSourceContext(source.source_context, source_context) and
            ...
            source.projections.isEmpty()))
        {
            ...
            try driver.copyExprCallableValueFromRef(
                result,
                source_context,
                module_idx,
                expr_idx,
    ...
    switch (expr) {
        .e_lookup_local, .e_lookup_external, .e_lookup_required => {
            ...
            if (result.getExprTemplateId(source_context, module_idx, expr_idx)) |template_id| {
                ...
                if (std.debug.runtime_safety and
                    driver.readExprCallableValue(result, source_context, module_idx, expr_idx) == null)
                {
                    ...
                    const context_template = if (driver.sourceContextCallableInstId(source_context)) |context_callable_inst|
        ...
        .e_call => |call_expr| {
            if (driver.getCallSiteForSourceContext(
                result,
                source_context,
                module_idx,
                expr_idx,
            ...
            var call_site = driver.getCallSiteForSourceContext(
                result,
                source_context,
                module_idx,
                expr_idx,
            ...
            if (call_site == null) {
                ...
                call_site = driver.getCallSiteForSourceContext(
                    result,
                    source_context,
                    module_idx,
                    expr_idx,
            ...
            const resolved_call_site = call_site orelse {
                ...
                const callee_value = driver.getValueExprCallableValueForSourceContext(
                    result,
                    source_context,
                    module_idx,
                    callee_expr_idx,
                ...
                const callee_pattern_value = if (callee_pattern_idx) |pattern_idx|
                    driver.readCallableParamValue(result, source_context, module_idx, pattern_idx)
                else
                    null;
                const source_context_callable_inst = driver.sourceContextCallableInstId(source_context);
                ...
                const callee_expr_monotype = try driver.requireFullyBoundExprMonotype(
                    result,
                    SemanticThread.trackedThread(source_context),
                    module_idx,
                    callee_expr_idx,
                ...
                const callee_source_value = if (callee_source) |source_expr_ref|
                    driver.getValueExprCallableValueForSourceContext(
                        result,
    ...
    if (try driver.finishCallableVariantBuilderValue(&variant_builder, result)) |callable_value| {
        try setExprCallableValue(driver,
            result,
            source_context,
            module_idx,
...
pub fn realizeDispatchExprSemantics(
    ...
    const dispatch_method_name = switch (expr) {
        .e_binop => |binop_expr| driver.dispatchMethodIdentForExprBinop(module_env, binop_expr.op),
    ...
    if (expr == .e_binop) {
        ...
        if (try binopDispatchHandledWithoutCallableInst(driver, result, thread, module_idx, expr_idx, binop_expr)) {
            try driver.bindCurrentIntrinsicDispatchSemantics(result, thread, module_idx, expr_idx, expr);
            return;
        }
        const method_name = driver.dispatchMethodIdentForExprBinop(module_env, binop_expr.op) orelse return;
        ...
        if (associated_target == null) {
            if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, binop_expr.lhs)) |lhs_monotype| {
    ...
    if (expr == .e_dot_access) {
        ...
        if (dot_expr.args != null) {
            ...
            if (associated_target == null) {
                if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, dot_expr.receiver)) |receiver_monotype| {
                    if (try resolveAssociatedMethodCallableInstForMonotype(
                        driver,
                        result,
                        thread,
    ...
    if (associated_target == null and expr == .e_type_var_dispatch) {
        ...
        const alias_monotype = try driver.resolveTypeVarMonotypeResolved(result, thread, module_idx, ModuleEnv.varFrom(alias_stmt.type_var_anno));
    ...
    const resolved_target = if (associated_target) |target| target else switch (expr) {
        ...
        .e_dot_access => |dot_expr| blk: {
            ...
            if (driver.dotCallUsesRuntimeReceiverExpr(module_env, dot_expr.receiver)) {
                _ = try driver.resolveExprMonotypeResolved(result, thread, module_idx, dot_expr.receiver);
    ...
    const callable_inst_id = blk: {
        if (try driver.specializeDispatchExactCallable(result, thread, module_idx, expr_idx, expr, template_id)) |exact_callable_inst| {
            break :blk exact_callable_inst;
        }

        const fn_monotype = try driver.resolveTypeVarMonotypeResolved(result, thread, module_idx, resolved_target.fn_var);
        if (!fn_monotype.isNone()) {
            break :blk try driver.requireCallableInst(result, thread.requireSourceContext(), template_id, fn_monotype.idx, fn_monotype.module_idx);
        }
...
pub fn dotDispatchHandledWithoutCallableInst(
    ...
    const module_env = driver.all_module_envs[module_idx];
    if (!driver.dotCallUsesRuntimeReceiverExpr(module_env, dot_expr.receiver)) return false;
    if (std.mem.eql(u8, module_env.getIdent(dot_expr.field_name), "to_str")) {
        const receiver_monotype = try driver.resolveExprMonotype(result, thread, module_idx, dot_expr.receiver);
...
pub fn binopDispatchHandledWithoutCallableInst(
    ...
    const module_env = driver.all_module_envs[module_idx];
    const lhs_monotype = try driver.resolveExprMonotype(result, thread, module_idx, binop_expr.lhs);
...
pub fn typeVarDispatchHandledWithoutCallableInst(
    ...
    try driver.appendDispatchActualArgsFromProgram(
        result,
        thread.requireSourceContext(),
        module_idx,
        expr_idx,
    ...
    const arg_monotype = (try driver.resolveExprMonotype(result, thread, module_idx, arg_exprs.items[0])) orelse return false;
...
pub fn resolveBinopDispatchTarget(
    ...
    const lhs_monotype = (try driver.resolveExprMonotypeResolved(result, thread, module_idx, binop_expr.lhs)) orelse {
        return resolveDispatchTargetForExpr(driver, result, thread, module_idx, expr_idx, method_name);
    };

    try driver.recordTypeVarMonotypeForThread(
        result,
        thread,
        module_idx,
...
pub fn resolveAssociatedMethodCallableInstForTypeVar(
    ...
    const module_env = driver.all_module_envs[module_idx];
    const receiver_nominal = driver.resolveNominalTypeInStore(&module_env.types, receiver_type_var) orelse return null;
    ...
    return try driver.specializeDispatchExactCallable(result, thread, module_idx, expr_idx, expr, method_info.template_id);
}
...
pub fn resolveAssociatedMethodDispatchTargetForTypeVar(
    ...
    const receiver_nominal = driver.resolveNominalTypeInStore(&module_env.types, receiver_type_var) orelse return null;
    const method_info = try DispatchSolved.lookupAssociatedMethodTemplate(driver, result, module_idx, receiver_nominal, method_ident) orelse return null;
    const receiver_monotype = try driver.resolveTypeVarMonotypeResolved(result, thread, module_idx, receiver_type_var);
...
pub fn exactAssociatedMethodDispatchSite(
    ...
    const fn_monotype = try driver.resolveTypeVarMonotypeResolved(result, thread, module_idx, site.fn_var);
    if (fn_monotype.isNone()) {
        if (std.debug.runtime_safety) {
            std.debug.panic(
...
pub fn resolveDispatchTargetForExpr(
        ...
        if (std.debug.runtime_safety) {
            ...
            const receiver_monotype: ?ResolvedMonotype = switch (expr) {
                .e_dot_access => |dot_expr| if (dot_expr.args != null)
                    try driver.resolveExprMonotypeResolved(result, thread, module_idx, dot_expr.receiver)
                else
                    null,
                .e_type_var_dispatch => blk: {
                    ...
                    break :blk try driver.resolveExprMonotypeResolved(result, thread, module_idx, args.items[0]);
                },
                else => null,
            };
    ...
    const desired_func_monotype = try driver.resolveTypeVarMonotypeResolved(result, thread, module_idx, site.fn_var);
...
pub fn resolveDispatchTargetFromExactSite(
    ...
    const resolved = switch (expr) {
        ...
        .e_unary_minus => |unary_expr| blk: {
            ...
            if (try driver.resolveExprMonotypeResolved(result, thread, module_idx, unary_expr.expr)) |operand_monotype| {
                break :blk try resolveAssociatedMethodDispatchTargetForMonotype(
                    driver,
                    result,
                    thread,
        ...
        .e_type_var_dispatch => |dispatch_expr| blk: {
            ...
            const alias_monotype = try driver.resolveTypeVarMonotypeResolved(result, thread, module_idx, receiver_type_var);
...
pub fn monotypeDispatchCompatible(
    ...
    return driver.monotypesStructurallyEqualAcrossModules(
        result,
        expected,
        expected_module_idx,
        actual,
...
pub fn bindCurrentDispatchFromCallableInst(
    ...
    if (result.context_mono.monotype_store.getMonotype(fn_mono.ret) != .func) {
        try driver.bindCurrentExprTypeRoot(result, thread, module_idx, expr_idx, fn_mono.ret, callable_inst.fn_monotype_module_idx);
        try driver.recordCurrentExprMonotype(result, thread, module_idx, expr_idx, fn_mono.ret, callable_inst.fn_monotype_module_idx);
    }
}
