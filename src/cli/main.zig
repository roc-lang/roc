...
test "post-check diagnostics preserve labeled Monotype counts" {
    ...
    try std.testing.expectEqualStrings("Generated-private containment queries", graph[16].name);
    try std.testing.expectEqual(@as(u64, 205), graph[16].count);
    try std.testing.expectEqualStrings("Generated-private nodes visited", graph[19].name);
    try std.testing.expectEqual(@as(u64, 202), graph[19].count);
    try std.testing.expectEqualStrings("Nominal backing tombstone deletions", graph[24].name);
    try std.testing.expectEqual(@as(u64, 203), graph[24].count);
