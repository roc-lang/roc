...
fn monotypeGraphCounters(diagnostics: postcheck.Monotype.Lower.Diagnostics) [27]progress.Counter {
    ...
    return .{
        ...
        .{ .name = "Generated-private containment scans", .count = graph.generated_private_scans },
...
test "post-check diagnostics preserve labeled Monotype counts" {
    ...
    try std.testing.expectEqualStrings("Generated-private guard returns", graph[15].name);
    try std.testing.expectEqual(@as(u64, 204), graph[15].count);
    try std.testing.expectEqualStrings("Generated-private containment scans", graph[16].name);
    try std.testing.expectEqual(@as(u64, 205), graph[16].count);
    try std.testing.expectEqualStrings("Generated-private nodes visited", graph[18].name);
    try std.testing.expectEqual(@as(u64, 202), graph[18].count);
    try std.testing.expectEqualStrings("Nominal backing tombstone deletions", graph[23].name);
    try std.testing.expectEqual(@as(u64, 203), graph[23].count);
...
test "check report recovery propagates terminal interpreter failures" {
    try std.testing.expectError(error.RuntimeError, requireReportableCheckFailure(error.RuntimeError));
    try std.testing.expectError(error.OutOfMemory, requireReportableCheckFailure(error.OutOfMemory));
