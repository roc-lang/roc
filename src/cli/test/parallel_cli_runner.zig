...
const subcommand_cases = [_]CliCase{
    ...
    // Repros for https://github.com/roc-lang/roc/issues/10411,
    // https://github.com/roc-lang/roc/issues/10412, and
    // https://github.com/roc-lang/roc/issues/10413: rejected constructor and
    // early-return relations must not contaminate a callee specialization.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10411: mismatched Try payload reports without Monotype panic", .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/issue_10411_mismatched_try_payload.roc", .exit = .failure, .stderr_min_len = 1, .contains = &.{ .{ .stream = .stderr, .text = "TYPE MISMATCH" }, .{ .stream = .stderr, .text = "ErrTag(Str)" }, .{ .stream = .stderr, .text = "ErrTag(U64)" } }, .not_contains = &.{ .{ .stream = .stderr, .text = "instantiation unified two different primitive types" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    .{ .id = 0, .suite = .subcommands, .name = "issue 10412: mismatched list payload reports without Monotype panic", .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/issue_10412_mismatched_list_payload.roc", .exit = .failure, .stderr_min_len = 1, .contains = &.{ .{ .stream = .stderr, .text = "TYPE MISMATCH" }, .{ .stream = .stderr, .text = "ErrTag(List([CustomType, ..]))" } }, .not_contains = &.{ .{ .stream = .stderr, .text = "instantiation list read had a non-list node" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    .{ .id = 0, .suite = .subcommands, .name = "issue 10413: incompatible Try rows report without Monotype panic", .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/issue_10413_mismatched_try_row.roc", .exit = .failure, .stderr_min_len = 1, .contains = &.{ .{ .stream = .stderr, .text = "TYPE MISMATCH" }, .{ .stream = .stderr, .text = "BadUtf8" }, .{ .stream = .stderr, .text = "ParsingIncomplete" } }, .not_contains = &.{ .{ .stream = .stderr, .text = "instantiation widened a closed tag union" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    // This constructor has nominally identical source and target fields. Its
    // specialization must retain the constructor child through explicit
    // generated-private/interface evidence instead of comparing foreign cells.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10424: record constructor retains partition-owned evidence", .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/issue_10424_record_constructor_evidence.roc", .exit = .success, .contains_any = &.{.{ .needles = &no_errors_needles }}, .not_contains = &.{ .{ .stream = .stderr, .text = "record graph constructor child differed without generated-private evidence" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    // T.map creates a result specialization with a different first nominal
    // argument. The method body and caller may share only the projected
    // function interface, not their tag-union graph cells.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10344: nominal map owns its specialization graph", .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/issue_10344_nominal_map_specialization/app.roc", .exit = .success, .contains_any = &.{.{ .needles = &no_errors_needles }}, .not_contains = &.{ .{ .stream = .stderr, .text = "instantiation unified a tag union with a non-tag-union type" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    // The boxed closure closes over a polymorphic wrapper result and is then
    // called from an annotated nested function. The nested specialization must
    // retain its checked lambda identity and own both sides of the Box boundary.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10365: boxed polymorphic closure keeps nested specialization identity", .body = .{ .command = .{ .args = &.{ "build", "--no-cache" }, .roc_file = "test/cli/issue_10365_boxed_polymorphic_closure.roc", .exit = .success, .contains = &.{.{ .stream = .stdout, .text = "successfully building" }}, .not_contains = &.{ .{ .stream = .stderr, .text = "checked closure did not point at a lambda expression" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    // A named nested closure widens the Try row produced by map_err. Its body
    // must be solved in the nested specialization partition before Lambda
    // Solved consumes the closed Monotype result.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10270: nested Try closure seals in its own specialization partition", .skip = .{ .always = "follow-up: nested functions still share their caller partition in this change" }, .body = .{ .command = .{ .args = &.{ "build", "--no-cache" }, .roc_file = "test/cli/issue_10270_nested_try_closure.roc", .exit = .success, .contains = &.{.{ .stream = .stdout, .text = "successfully building" }}, .not_contains = &.{ .{ .stream = .stderr, .text = "instantiation widened a closed tag union" }, .{ .stream = .stderr, .text = "tag count failed Lambda Solved unification" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    // The unannotated function stored in a record is constrained only when the
    // platform-required procedure is specialized. That contextual constraint
    // must enter through the procedure interface rather than the caller graph.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10403: unannotated record closure specializes through platform interface", .skip = .{ .always = "follow-up: platform-required procedures need an explicit checked interface" }, .body = .{ .command = .{ .args = &.{ "build", "--no-cache" }, .roc_file = "test/cli/issue_10403_unannotated_record_closure/app.roc", .exit = .success, .contains = &.{.{ .stream = .stdout, .text = "successfully building" }}, .not_contains = &.{ .{ .stream = .stderr, .text = "instantiation unified a primitive type with a non-primitive type" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    // Recursive iterator concat restores nested local-procedure closures into
    // separate static initializers. Every initializer must stay closed after
    // its specialization demand moves into an independent graph partition.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10427: recursive iterator static-data candidates remain closed", .body = .{ .command = .{ .args = &.{ "build", "--target=wasm32", "--opt=dev", "--no-cache" }, .roc_file = "test/wasm/issue_10427_recursive_concat_static_data.roc", .exit = .success, .contains = &.{.{ .stream = .stdout, .text = "successfully building" }}, .not_contains = &.{ .{ .stream = .stderr, .text = "static initializer read an uninitialized local" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    // An unresolved where-clause dispatch currently reaches runtime as an
    // uninhabited demand. The demand must either receive explicit evidence or
    // be rejected before execution; it must never become a synthesized crash.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10239: uninhabited dispatch demand never reaches runtime", .skip = .{ .always = "follow-up: uninhabited dispatch requires checker evidence or rejection" }, .body = .{ .command = .{ .args = &.{"--no-cache"}, .roc_file = "test/cli/issue_10239_uninhabited_dispatch.roc", .exit = .not_panic, .not_contains = &.{ .{ .stream = .stderr, .text = "dispatch on a value that can never exist" }, .{ .stream = .stderr, .text = "Roc application crashed" }, .{ .stream = .stderr, .text = "overflowed its stack memory" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
    ...
    // Regression guard for #10121: stale conditional-payload state in the
    // debug ARC certifier enumerated the 2^N field-presence subsets and took
    // about a minute at twelve fields.
    .{ .id = 0, .suite = .subcommands, .name = "roc test round-trips JSON records with many optional fields", .timeout_ms = 15_000, .body = .{ .command = .{ .args = &.{ "test", "--no-cache" }, .roc_file = "test/cli/JsonOptionalRecordRoundTrip.roc", .contains = &.{.{ .stream = .stdout, .text = "passed" }}, .not_contains = &.{.{ .stream = .stderr, .text = "panic" }} } } },
