...
const subcommand_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .subcommands, .name = "roc check rejects recursive record defaults without overflowing", .timeout_ms = 10_000, .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/recursive_record_default.roc", .exit = .failure, .contains = &.{.{ .stream = .stderr, .text = "RECURSIVE DEFAULT VALUE" }}, .not_contains = &.{ .{ .stream = .stderr, .text = "overflowed its stack" }, .{ .stream = .stderr, .text = "panic" } } } } },
