...
const subcommand_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .subcommands, .name = "boxy stored record function field retains its erased callable representation", .backend = .interpreter, .body = .{ .command = .{ .args = &.{ "test", "--opt=interpreter", "--specialize=no", "--no-cache" }, .roc_file = "test/cli/BoxyStoredFunctionField.roc", .exit = .success, .contains = &.{.{ .stream = .stdout, .text = "passed" }}, .not_contains = &.{.{ .stream = .stderr, .text = "panic" }} } } },
    .{ .id = 0, .suite = .subcommands, .name = "boxy generic stored record function field resolves transitive nominal backing substitutions", .backend = .interpreter, .body = .{ .command = .{ .args = &.{ "test", "--opt=interpreter", "--specialize=no", "--no-cache" }, .roc_file = "test/cli/BoxyGenericStoredFunctionField.roc", .exit = .success, .contains = &.{.{ .stream = .stdout, .text = "passed" }}, .not_contains = &.{.{ .stream = .stderr, .text = "panic" }} } } },
    ...
    .{ .id = 0, .suite = .subcommands, .name = "boxy roc build archive output lowers generic platform required init consts", .body = .{ .command = .{ .args = &.{ "build", "--specialize=no", "--opt=dev", "--no-cache" }, .roc_file = "test/postcheck/platform_required_init/app.roc", .contains = &.{.{ .stream = .stdout, .text = "successfully building" }}, .not_contains = &.{.{ .stream = .stderr, .text = "panic" }} } } },
