...
const subcommand_cases = [_]CliCase{
    ...
    // RED TEST: overwriting a key that is already present allocates once per
    // call, while inserting an absent key into the same uniquely owned,
    // pre-sized dictionary allocates nothing. Drop this skip once the Found
    // branch of Dict.insert stops copying the entry list.
    .{ .id = 0, .suite = .subcommands, .name = "test/alloc-count/app_dict_insert_overwrite.roc: overwriting an existing key allocates nothing (speed)", .backend = .speed, .skip = .{ .always = "known bug: Dict.insert reallocates the entry list on every overwrite of an existing key" }, .body = .{ .command = .{ .args = &.{ "--opt=speed", "--no-cache" }, .roc_file = "test/alloc-count/app_dict_insert_overwrite.roc", .contains = &.{.{ .stream = .stderr, .text = "entries: 1016, overwrite allocations: 0" }}, .not_contains = &.{.{ .stream = .stderr, .text = "Expect failed" }} } } },
