...
const subcommand_cases = [_]CliCase{
    ...
    // When the lifted nominal opts into a derived parser, the final relation
    // must produce the matching generated-codec contract instead of the issue's
    // second postcheck invariant failure.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10977: nominal lift with derived parser builds without invariant failure", .body = .{ .command = .{ .args = &.{ "build", "--no-cache" }, .roc_file = "test/postcheck/issue_10977_decoded_structural_tag_lifted_to_nominal/app_derived_parser.roc", .exit = .not_panic, .contains = &.{.{ .stream = .stdout, .text = "successfully building" }}, .not_contains = &.{ .{ .stream = .stderr, .text = "missing method" }, .{ .stream = .stderr, .text = "checked generated codec contract was missing its subject method call" }, .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "Segmentation fault" }, .{ .stream = .stderr, .text = "panic" } } } } },
