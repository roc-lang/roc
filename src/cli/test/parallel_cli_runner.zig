...
const echo_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .echo, .name = "echo platform: statically dispatched, propagated, open error union does not crash (known bug #9588)", .backend = .interpreter, .skip = .{ .always = "known bug #9588: monotype lowering panic (dispatch result type conflicted with Monotype constraint)" }, .body = .{ .command = .{ .args = &.{"--no-cache"}, .roc_file = "test/echo/issue_9588.roc", .exit = .not_panic, .not_contains = &.{ .{ .stream = .stderr, .text = "panic" }, .{ .stream = .stderr, .text = "invariant violated" } } } } },
};
