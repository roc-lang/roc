...
const subcommand_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .subcommands, .name = "issue 10093: non-regular recursive nominal reports a diagnostic", .timeout_ms = 10_000, .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/issue_10093_non_regular_recursive_nominal.roc", .exit = .failure, .stderr_min_len = 1, .contains = &.{.{ .stream = .stderr, .text = "UNDECLARED TYPE VARIABLE" }}, .not_contains = &.{ .{ .stream = .stderr, .text = "checked artifact invariant violated" }, .{ .stream = .stderr, .text = "invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
