...
const echo_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .echo, .name = "echo platform: boxy open tag-union parameter keeps its row open (interpreter)", .backend = .interpreter, .body = .{ .command = .{ .args = &.{ "--opt=interpreter", "--specialize=no" }, .roc_file = "test/echo/boxy_open_row_rigid_tail.roc", .stdout_exact = "other red\n" } } },
    .{ .id = 0, .suite = .echo, .name = "echo platform: boxy open tag-union parameter keeps its row open (dev)", .backend = .dev, .body = .{ .command = .{ .args = &.{ "--opt=dev", "--specialize=no" }, .roc_file = "test/echo/boxy_open_row_rigid_tail.roc", .stdout_exact = "other red\n" } } },
