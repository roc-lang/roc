...
fn buildTestSpecs(allocator: Allocator, filters: []const []const u8) Allocator.Error![]const CliTestSpec {
...
fn fmtTestName(allocator: Allocator, roc_file: []const u8, backend: ?[]const u8) Allocator.Error![]const u8 {
    if (backend) |b| {
...
fn deleteIfExists(path: []const u8) Allocator.Error!void {
...
fn deleteOutputArtifacts(allocator: Allocator, output_name: []const u8) Allocator.Error!void {
...
fn buildCliWorkerArgvTemplate(arena: Allocator, process_args: std.process.Args) Allocator.Error![]const []const u8 {
