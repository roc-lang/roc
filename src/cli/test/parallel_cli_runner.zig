...
const subcommand_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .subcommands, .name = "roc run fx-open platform alias accepts named error row", .body = .{ .command = .{ .args = &.{ "--opt=interpreter", "--no-cache" }, .roc_file = "test/fx-open/open_error_alias.roc", .exit = .{ .code = 1 }, .contains = &.{.{ .stream = .stderr, .text = "exited with other error: ReadFailed" }} } } },
