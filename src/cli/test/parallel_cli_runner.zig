...
const subcommand_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .subcommands, .name = "issue 10892: a blocked destructure names the effectful statement that stopped compile-time validation", .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/Issue10892DestructureBlockedByEffect.roc", .exit = .failure, .contains = &.{ .{ .stream = .stderr, .text = "non exhaustive destructure" }, .{ .stream = .stderr, .text = "This earlier statement stopped compile-time validation:" }, .{ .stream = .stderr, .text = "echo!(\"hi" } }, .not_contains = &.{.{ .stream = .stderr, .text = "panic" }} } } },
