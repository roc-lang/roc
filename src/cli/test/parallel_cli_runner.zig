...
const subcommand_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .subcommands, .name = "roc check rejects old parser parse_from structural method", .body = .{ .command = .{ .args = &.{ "check", "--no-cache" }, .roc_file = "test/cli/ParserOldParseFromMethodRejected.roc", .exit = .failure, .stderr_min_len = 1, .contains_any = &.{.{ .needles = &type_error_needles }}, .not_contains = &.{.{ .stream = .stderr, .text = "panic" }} } } },
