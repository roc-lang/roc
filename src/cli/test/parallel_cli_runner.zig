...
const subcommand_cases = [_]CliCase{
    ...
    // the receiver is still a flex var. However the checker settles it,
    // compiling the module has to terminate.
    .{ .id = 0, .suite = .subcommands, .name = "issue 10788: nominal record update rewrap under an inline lambda terminates", .timeout_ms = 60_000, .body = .{ .command = .{ .args = &.{ "test", "--no-cache" }, .roc_file = "test/cli/issue_10788_nominal_record_update_rewrap/Sc.roc", .exit = .not_panic, .contains_any = &.{.{ .needles = &.{ .{ .stream = .stdout, .text = "passed" }, .{ .stream = .stderr, .text = "passed" } } }}, .not_contains = &.{ .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "Segmentation fault" }, .{ .stream = .stderr, .text = "panic" } } } } },
