...
const subcommand_cases = [_]CliCase{
    ...
    .{ .id = 0, .suite = .subcommands, .name = "roc build accepts an unannotated record closure with a method call and literal (issue 10403)", .body = .{ .command = .{ .args = &.{ "build", "--no-cache", "--target=x64glibc" }, .roc_file = "test/postcheck/issue_10403_unannotated_record_closure/app.roc", .exit = .success, .contains = &.{.{ .stream = .stdout, .text = "successfully building" }}, .not_contains = &.{ .{ .stream = .stderr, .text = "postcheck invariant violated" }, .{ .stream = .stderr, .text = "panic" } } } } },
