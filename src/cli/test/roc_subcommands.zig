...
test "roc build --opt=dev wasm32 static_lib list builtins" {
    const testing = std.testing;
    const gpa = testing.allocator;
    ...
    const output_path = try std.fs.path.join(gpa, &.{ tmp_path, "list_builtin_static_lib_app.wasm" });
    defer gpa.free(output_path);

    const output_arg = try std.fmt.allocPrint(gpa, "--output={s}", .{output_path});
    defer gpa.free(output_arg);

    const result = try util.runRoc(gpa, &.{ "build", "--opt=dev", "--target=wasm32", output_arg }, "test/wasm/list_builtin_static_lib_app.roc");
    defer gpa.free(result.stdout);
    defer gpa.free(result.stderr);
    ...
    const stat = tmp_dir.dir.statFile(std.testing.io, "list_builtin_static_lib_app.wasm", .{}) catch |err| {
        std.debug.print("Failed to stat wasm output: {}\nstdout: {s}\nstderr: {s}\n", .{ err, result.stdout, result.stderr });
        return err;
    };
    try testing.expect(stat.size > 0);
...
test "roc build --opt=dev defaults to wasm32 static_lib target when no native target exists" {
    const testing = std.testing;
    const gpa = testing.allocator;
    ...
    const output_path = try std.fs.path.join(gpa, &.{ tmp_path, "dev_default_static_lib_app.wasm" });
    defer gpa.free(output_path);

    const output_arg = try std.fmt.allocPrint(gpa, "--output={s}", .{output_path});
    defer gpa.free(output_arg);

    const result = try util.runRoc(gpa, &.{ "build", "--opt=dev", output_arg }, "test/wasm/static_lib_app.roc");
    defer gpa.free(result.stdout);
    defer gpa.free(result.stderr);
    ...
    try testing.expect(result.term == .exited and result.term.exited == 0);
    try testing.expect(std.mem.find(u8, result.stdout, "checked-artifact wasm backend") != null);

    const stat = tmp_dir.dir.statFile(std.testing.io, "dev_default_static_lib_app.wasm", .{}) catch |err| {
        std.debug.print("Failed to stat wasm output: {}\nstdout: {s}\nstderr: {s}\n", .{ err, result.stdout, result.stderr });
        return err;
    };
    try testing.expect(stat.size > 0);
...
test "roc build --opt=interpreter errors after selecting default wasm32 static_lib target" {
    const testing = std.testing;
    const gpa = testing.allocator;

    const result = try util.runRoc(gpa, &.{ "build", "--opt=interpreter", "--no-cache" }, "test/wasm/static_lib_app.roc");
    defer gpa.free(result.stdout);
    defer gpa.free(result.stderr);

    try testing.expect(result.term == .exited and result.term.exited != 0);
    try testing.expect(std.mem.find(u8, result.stderr, "interpreter backend only supports building for the native target") != null);
}
