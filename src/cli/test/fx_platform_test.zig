...
/// State for one-time roc binary build
const RocBinaryState = enum(u8) {
    not_built,
    building,
    built,
    failed,
...
var roc_binary_state: std.atomic.Value(RocBinaryState) = std.atomic.Value(RocBinaryState).init(.not_built);

/// Ensures the roc binary is up-to-date by rebuilding it once.
/// Uses atomic state to ensure only one build happens even with parallel tests.
fn ensureRocBinary(allocator: std.mem.Allocator) !void {
    // Fast path: already built
    if (roc_binary_state.load(.acquire) == .built) return;
    if (roc_binary_state.load(.acquire) == .failed) return error.RocBuildFailed;

    // Try to become the builder
    if (roc_binary_state.cmpxchgStrong(.not_built, .building, .acq_rel, .acquire)) |_| {
        // Someone else is building or already built, wait for them
        while (true) {
            const state = roc_binary_state.load(.acquire);
            switch (state) {
                .built => return,
                .failed => return error.RocBuildFailed,
                .building => {
                    std.Thread.sleep(10 * std.time.ns_per_ms);
                    continue;
                },
                .not_built => unreachable,
            }
        }
    }
    ...
    const build_result = std.process.Child.run(.{
        ...
        roc_binary_state.store(.failed, .release);
    ...
    // Clear the roc cache once so the first test builds the interpreted host fresh,
    // then subsequent tests can reuse it from cache
    const clear_result = std.process.Child.run(.{
        .allocator = allocator,
        .argv = &[_][]const u8{ roc_binary_path, "--clear-cache" },
    }) catch |err| {
        roc_binary_state.store(.failed, .release);
        std.debug.print("Failed to clear roc cache: {}\n", .{err});
        return error.RocBuildFailed;
    };
    allocator.free(clear_result.stdout);
    allocator.free(clear_result.stderr);

    roc_binary_state.store(.built, .release);
}
...
/// The roc cache is cleared once at startup, then tests reuse the cached interpreted host.
