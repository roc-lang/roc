...
const ReplOps = struct {
    ...
    fn replRocRealloc(realloc_args: *RocRealloc, env: *anyopaque) callconv(.c) void {
        ...
        const new_base_ptr = repl_env.allocator.rawAlloc(new_total_size, align_enum, @returnAddress()) orelse {
        ...
        repl_env.allocator.rawFree(old_slice, align_enum, @returnAddress());
...
pub fn run(ctx: *CliContext, backend: Backend) !void {
    ...
    while (true) : ({
        ...
        // Read line
        const line = try repl_line.readLine(ctx.arena, "» ", std.fs.File.stdin());
