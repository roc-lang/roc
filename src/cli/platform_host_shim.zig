...
pub fn createInterpreterShim(builder: *Builder, entrypoints: []const EntryPoint, serialized_module: ?[]const u8) !void {
    ...
    try addRocSerializedModule(builder, serialized_module);
}
...
fn addRocSerializedModule(builder: *Builder, serialized_module: ?[]const u8) !void {
    const ptr_type = try builder.ptrType(.default);

    // Determine usize type based on target pointer width
    const usize_type: Builder.Type = switch (builtin.target.ptrBitWidth()) {
        32 => .i32,
        64 => .i64,
        else => unreachable,
    };
    ...
    const base_ptr_name_str = if (builtin.target.os.tag == .macos)
    ...
    const size_name_str = if (builtin.target.os.tag == .macos)
