...
pub fn findLibc(ctx: *CliContext) !LibcInfo {
    const sys_io = ctx.io.sys_io;
    // Try compiler-based detection first (most reliable)
    if (try findViaCompiler(ctx.arena, sys_io)) |info|
        return info
    else
        // Fall back to filesystem search
        return try findViaFilesystem(ctx.arena, sys_io);
}

/// Find libc using compiler queries (gcc/clang)
fn findViaCompiler(arena: std.mem.Allocator, sys_io: std.Io) !?LibcInfo {
    const compilers = [_][]const u8{ "gcc", "clang", "cc" };

    // Get architecture first
    const arch = try getArchitecture(arena, sys_io);
    ...
    for (compilers) |compiler| {
        ...
        _ = process.run(arena, sys_io, .{
            .argv = &[_][]const u8{ compiler, ld_cmd },
        }) catch continue;

        // Try to get libc path from compiler
        const libc_result = process.run(arena, sys_io, .{
            .argv = &[_][]const u8{ compiler, "-print-file-name=libc.so" },
        }) catch continue;

        const libc_path = std.mem.trimEnd(u8, libc_result.stdout, "\n\r \t");
        ...
        const libc_file = std.Io.Dir.openFileAbsolute(sys_io, libc_path, .{}) catch continue;
        libc_file.close(sys_io);

        const lib_dir = fs.path.dirname(libc_path) orelse continue;

        // Find dynamic linker
        const dynamic_linker = try findDynamicLinker(arena, sys_io, arch, lib_dir) orelse continue;
...
fn findViaFilesystem(arena: std.mem.Allocator, sys_io: std.Io) !LibcInfo {
    const arch = try getArchitecture(arena, sys_io);
    const search_paths = try getSearchPaths(arena, sys_io, arch);

    // Search for libc in standard paths
    for (search_paths) |lib_dir| {
        var dir = std.Io.Dir.openDirAbsolute(sys_io, lib_dir, .{}) catch continue;
...
fn findDynamicLinker(arena: std.mem.Allocator, sys_io: std.Io, arch: []const u8, lib_dir: []const u8) !?[]const u8 {
    ...
    for (ld_names) |ld_name| {
        ...
        if (std.Io.Dir.openFileAbsolute(sys_io, path, .{})) |file| {
            file.close(sys_io);
            return path;
        } else |_| {}
    }
...
fn getArchitecture(arena: std.mem.Allocator, sys_io: std.Io) ![]const u8 {
    const result = try process.run(arena, sys_io, .{
...
fn getSearchPaths(arena: std.mem.Allocator, sys_io: std.Io, arch: []const u8) ![]const []const u8 {
    const triplet = getMultiarchTriplet(arena, sys_io, arch) catch |err| blk: {
        switch (err) {
...
fn getMultiarchTriplet(arena: std.mem.Allocator, sys_io: std.Io, arch: []const u8) ![]const u8 {
    // Try to get from gcc first
    const result = process.run(arena, sys_io, .{
...
test "libc detection integration test" {
    ...
    const test_sys_io = ctx.io.sys_io;

    // Verify the dynamic linker file exists and is accessible
    const ld_file = std.Io.Dir.openFileAbsolute(test_sys_io, libc_info.dynamic_linker, .{}) catch |err| {
        std.log.err("Dynamic linker not accessible at {s}: {}", .{ libc_info.dynamic_linker, err });
        return err;
    };
    ld_file.close(test_sys_io);

    // Verify the libc file exists and is accessible
    const libc_file = std.Io.Dir.openFileAbsolute(test_sys_io, libc_info.libc_path, .{}) catch |err| {
        std.log.err("Libc not accessible at {s}: {}", .{ libc_info.libc_path, err });
        return err;
    };
    libc_file.close(test_sys_io);
}
