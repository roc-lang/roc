...
pub fn validateUtf8Chunk(input: @Vector(16, u8), state: *Utf8State) @Vector(16, u8) {
    ...
    // Check for bytes that are always invalid (0xF5-0xFF)
    // These encode code points beyond U+10FFFF
    const too_large = input >= @as(@Vector(16, u8), @splat(0xF5));
    const too_large_errors = @select(u8, too_large, @as(@Vector(16, u8), @splat(TOO_LARGE)), @as(@Vector(16, u8), @splat(0)));

    // Check for incomplete sequences from previous chunk
    const combined_errors = errors | state.prev_incomplete | too_large_errors;
