...
/// Represents the upper bound for compilation time in milliseconds.
/// Note: if you change this value, don't forget the title in the table column also
const COMPILATION_UPPER_BOUND_MS = 10;

const DiagnosticCounts = struct {
    tokenize_problems: usize = 0,
    parser_problems: usize = 0,
    canonicalize_problems: usize = 0,
    check_types_problems: usize = 0,
};
...
/// NOTE -- this file is automatically generated by the Roc snapshot tool in CI across multiple platforms and architectures. It must only contain information that is deterministic and consistent across runs.
fn generateSummaryFile(gpa: Allocator, snapshots_dir: []const u8, snapshot_stats: []const SnapshotStats) !void {
    ...
    try writer.print("# Snapshot Summary\n\n", .{});
    try writer.print("This file is automatically generated by the Roc snapshot tool.\n\n", .{});
    try writer.print("## Statistics\n\n", .{});

    // Calculate aggregate statistics
    var total_source_bytes: usize = 0;
    var total_tokenize_problems: usize = 0;
    var total_parser_problems: usize = 0;
    var total_canonicalize_problems: usize = 0;
    var total_check_types_problems: usize = 0;

    for (snapshot_stats) |stats| {
        total_source_bytes += stats.source_bytes;
        total_tokenize_problems += stats.diagnostics.tokenize_problems;
        total_parser_problems += stats.diagnostics.parser_problems;
        total_canonicalize_problems += stats.diagnostics.canonicalize_problems;
        total_check_types_problems += stats.diagnostics.check_types_problems;
    }

    const total_files_str = try formatNumber(usize, snapshot_stats.len, gpa);
    defer gpa.free(total_files_str);
    const total_source_bytes_str = try formatNumber(usize, total_source_bytes, gpa);
    defer gpa.free(total_source_bytes_str);
    const total_tokenize_problems_str = try formatNumber(usize, total_tokenize_problems, gpa);
    defer gpa.free(total_tokenize_problems_str);
    const total_parser_problems_str = try formatNumber(usize, total_parser_problems, gpa);
    defer gpa.free(total_parser_problems_str);
    const total_canonicalize_problems_str = try formatNumber(usize, total_canonicalize_problems, gpa);
    defer gpa.free(total_canonicalize_problems_str);
    const total_check_types_problems_str = try formatNumber(usize, total_check_types_problems, gpa);
    defer gpa.free(total_check_types_problems_str);

    try writer.print("- **Total files:** {s}\n", .{total_files_str});
    try writer.print("- **Total source bytes:** {s}\n", .{total_source_bytes_str});
    try writer.print("- **Total tokenize problems:** {s}\n", .{total_tokenize_problems_str});
    try writer.print("- **Total parser problems:** {s}\n", .{total_parser_problems_str});
    try writer.print("- **Total canonicalize problems:** {s}\n", .{total_canonicalize_problems_str});
    try writer.print("- **Total check types problems:** {s}\n", .{total_check_types_problems_str});

    // Sort all stats by filename for deterministic output
    const sorted_stats = try gpa.alloc(SnapshotStats, snapshot_stats.len);
    defer gpa.free(sorted_stats);
    @memcpy(sorted_stats, snapshot_stats);

    // Sort by filename alphabetically
    std.sort.pdq(SnapshotStats, sorted_stats, {}, struct {
        fn lessThan(_: void, a: SnapshotStats, b: SnapshotStats) bool {
            return std.mem.lessThan(u8, std.fs.path.basename(a.file_path), std.fs.path.basename(b.file_path));
        }
    }.lessThan);

    try writer.print("\n## Diagnostic Problems by File\n\n", .{});

    // Count files with problems and fast compilation for summary
    var problem_count: usize = 0;
    var fast_compilation_count: usize = 0;
    for (sorted_stats) |stats| {
        if (stats.diagnostics.tokenize_problems > 0 or
            stats.diagnostics.parser_problems > 0 or
            stats.diagnostics.canonicalize_problems > 0 or
            stats.diagnostics.check_types_problems > 0)
        {
            problem_count += 1;
        }
        if (stats.compilation_time_ms < COMPILATION_UPPER_BOUND_MS) {
            fast_compilation_count += 1;
        }
    }

    const problem_count_str = try formatNumber(usize, problem_count, gpa);
    defer gpa.free(problem_count_str);
    const fast_compilation_count_str = try formatNumber(usize, fast_compilation_count, gpa);
    defer gpa.free(fast_compilation_count_str);

    try writer.print("**{s} files have diagnostic problems.**\n\n", .{problem_count_str});
    try writer.print("**{s} files compile in <10ms.**\n\n", .{fast_compilation_count_str});

    try writer.print("| File | <10ms | Tokenize Problems | Parser Problems | Canonicalize Problems | Type Check Problems |\n", .{});
    try writer.print("| --- | --- | --- | --- | --- | --- |\n", .{});

    for (sorted_stats) |stats| {
        const fast_icon = if (stats.compilation_time_ms < 10) "✅" else "❌";
        const tokenize_problems_str = try formatNumber(usize, stats.diagnostics.tokenize_problems, gpa);
        defer gpa.free(tokenize_problems_str);
        const parser_problems_str = try formatNumber(usize, stats.diagnostics.parser_problems, gpa);
        defer gpa.free(parser_problems_str);
        const canonicalize_problems_str = try formatNumber(usize, stats.diagnostics.canonicalize_problems, gpa);
        defer gpa.free(canonicalize_problems_str);
        const check_types_problems_str = try formatNumber(usize, stats.diagnostics.check_types_problems, gpa);
        defer gpa.free(check_types_problems_str);

        try writer.print("| `{s}` | {s} | {s} | {s} | {s} | {s} |\n", .{
            std.fs.path.basename(stats.file_path),
            fast_icon,
            tokenize_problems_str,
            parser_problems_str,
            canonicalize_problems_str,
            check_types_problems_str,
        });
    }

    log("generated summary file: {s}", .{summary_path});
}
...
pub fn main() !void {
    ...
    var all_stats = std.ArrayList(SnapshotStats).init(gpa);
    defer all_stats.deinit();
    var timer = std.time.Timer.start() catch unreachable;

    if (snapshot_paths.items.len > 0) {
        for (snapshot_paths.items) |path| {
            try processPath(gpa, path, maybe_fuzz_corpus_path, &all_stats);
        }
    } else {
        // process all files in snapshots_dir
        try processPath(gpa, snapshots_dir, maybe_fuzz_corpus_path, &all_stats);
    }

    const duration_ms = timer.read() / std.time.ns_per_ms;

    // Generate summary file
    generateSummaryFile(gpa, snapshots_dir, all_stats.items) catch |err| {
        std.log.warn("failed to generate summary file: {s}", .{@errorName(err)});
    };

    // Free duplicated file paths
    for (all_stats.items) |stats| {
        gpa.free(stats.file_path);
    }

    std.log.info("processed {d} snapshots in {d} ms.", .{ all_stats.items.len, duration_ms });
}

/// Check if a file has a valid snapshot extension
fn isSnapshotFile(filename: []const u8) bool {
    const basename = std.fs.path.basename(filename);
    if (std.mem.eql(u8, basename, "README.md")) {
        return false;
    }
    return std.mem.endsWith(u8, basename, ".md");
}

fn processPath(gpa: Allocator, path: []const u8, maybe_fuzz_corpus_path: ?[]const u8, all_stats: *std.ArrayList(SnapshotStats)) !void {
    ...
    // Try to open as directory first (cross-platform compatible)
    if (std.fs.cwd().openDir(canonical_path, .{ .iterate = true })) |dir_val| {
        var dir = dir_val;
        defer dir.close();
        ...
        while (try dir_iterator.next()) |entry| {
            ...
            if (entry.kind == .directory) {
                try processPath(gpa, full_path, maybe_fuzz_corpus_path, all_stats);
            } else if (entry.kind == .file and isSnapshotFile(entry.name)) {
                const stats = processSnapshotFileUnified(gpa, full_path, maybe_fuzz_corpus_path) catch blk: {
                    const file_path_copy = gpa.dupe(u8, full_path) catch full_path;
                    break :blk SnapshotStats{
                ...
                log("processed file: {s}", .{full_path});
            }
        }
    } else |_| {
        // If opening as directory failed, try to process as a file
        if (isSnapshotFile(canonical_path)) {
            const stats = processSnapshotFileUnified(gpa, canonical_path, maybe_fuzz_corpus_path) catch blk: {
                const file_path_copy = gpa.dupe(u8, canonical_path) catch canonical_path;
                break :blk SnapshotStats{
                    .file_path = file_path_copy,
                    .source_bytes = 0,
                    .diagnostics = DiagnosticCounts{},
            ...
            try all_stats.append(stats);
            log("processed file: {s}", .{canonical_path});
        } else {
...
fn generateProblemsSection(output: *DualOutput, parse_ast: *AST, can_ir: *CIR, solver: *Solver, content: *const Content, snapshot_path: []const u8, module_env: *base.ModuleEnv) !DiagnosticCounts {
    ...
    return DiagnosticCounts{
        .tokenize_problems = tokenize_problems,
        .parser_problems = parser_problems,
        .canonicalize_problems = canonicalize_problems,
        .check_types_problems = check_types_problem,
    };
}
...
fn generateParseSection(output: *DualOutput, content: *const Content, parse_ast: *AST, module_env: *base.ModuleEnv) !void {
    ...
    defer if (node_opt) |*node| node.deinit(module_env.gpa);
...
fn processSnapshotFileUnified(gpa: Allocator, snapshot_path: []const u8, maybe_fuzz_corpus_path: ?[]const u8) !SnapshotStats {
    // Log the file path that was written to
    log("processing snapshot file: {s}", .{snapshot_path});

    // Use arena allocator for core compilation phases, gpa for output generation
    ...
    // Start timing compilation phases
    const compilation_start = std.time.milliTimestamp();

    var module_env = base.ModuleEnv.init(arena_allocator);
    ...
    defer parse_ast.deinit(arena_allocator);
    ...
    // Types (ONCE)
    var solver = try Solver.init(arena_allocator, &can_ir.env.types, &can_ir);
    defer solver.deinit();
    ...
    // End timing compilation phases
    const compilation_end = std.time.milliTimestamp();
    const compilation_time_ms: u64 = @intCast(@max(0, compilation_end - compilation_start));
    ...
    const diagnostic_counts = try generateProblemsSection(&output, &parse_ast, &can_ir, &solver, &content, snapshot_path, &module_env);
    ...
    var md_file = std.fs.cwd().createFile(snapshot_path, .{}) catch |err| {
        ...
        return SnapshotStats{
            .file_path = snapshot_path,
            .source_bytes = content.source.len,
    ...
    if (maybe_fuzz_corpus_path != null) {
        ...
        var corpus_file = std.fs.cwd().createFile(corpus_file_path, .{}) catch |err| {
            ...
            const file_path_copy = gpa.dupe(u8, snapshot_path) catch snapshot_path;
            return SnapshotStats{
                .file_path = file_path_copy,
                .source_bytes = content.source.len,
                .diagnostics = DiagnosticCounts{},
...
fn formatNumber(comptime T: type, value: T, allocator: Allocator) ![]u8 {
    ...
    const comma_count = (str.len - 1) / 3;
    const result_len = str.len + comma_count;
    const result = try allocator.alloc(u8, result_len);

    var result_idx: usize = result_len;
    var str_idx: usize = str.len;
    ...
    while (str_idx > 0) {
        ...
        if (digit_count == 3 and str_idx > 0) {
            result_idx -= 1;
            result[result_idx] = ',';
            digit_count = 0;
        }
...
test "formatNumber formats with thousands separators" {
    const test_cases = [_]struct { input: usize, expected: []const u8 }{
        .{ .input = 0, .expected = "0" },
        .{ .input = 5, .expected = "5" },
    ...
    for (test_cases) |case| {
        const result = try formatNumber(usize, case.input, testing.allocator);
