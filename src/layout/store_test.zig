...
test "fromTypeVar - host opaque types compile to opaque_ptr" {
    ...
    const box_flex_idx = try lt.layout_store.fromTypeVar(0, box_flex_var, &lt.type_scope, false);
    ...
    const box_rigid_idx = try lt.layout_store.fromTypeVar(0, box_rigid_var, &lt.type_scope, false);
...
test "fromTypeVar - zero-sized types (ZST)" {
    ...
    const empty_record_idx = try lt.layout_store.fromTypeVar(0, empty_record_var, &lt.type_scope, false);
    try testing.expect(lt.layout_store.getLayout(empty_record_idx).tag == .zst);
    const empty_tag_union_idx = try lt.layout_store.fromTypeVar(0, empty_tag_union_var, &lt.type_scope, false);
    ...
    const box_zst_idx = try lt.layout_store.fromTypeVar(0, box_zst_var, &lt.type_scope, false);
    ...
    const list_zst_idx = try lt.layout_store.fromTypeVar(0, list_zst_var, &lt.type_scope, false);
...
test "fromTypeVar - record with only zero-sized fields" {
    ...
    const record_idx = try lt.layout_store.fromTypeVar(0, record_var, &lt.type_scope, false);
    ...
    const box_idx = try lt.layout_store.fromTypeVar(0, box_record_var, &lt.type_scope, false);
...
test "deeply nested containers with inner ZST" {
    ...
    const result_idx = try lt.layout_store.fromTypeVar(0, outer_list_var, &lt.type_scope, false);
...
test "nested ZST detection - List of record with ZST field" {
    ...
    const list_idx = try lt.layout_store.fromTypeVar(0, list_var, &lt.type_scope, false);
...
test "nested ZST detection - Box of tuple with ZST elements" {
    ...
    const tuple_idx = try lt.layout_store.fromTypeVar(0, tuple_var, &lt.type_scope, false);
    ...
    const box_idx = try lt.layout_store.fromTypeVar(0, box_var, &lt.type_scope, false);
    try testing.expect(lt.layout_store.getLayout(box_idx).tag == .box_of_zst);
}
...
test "fromTypeVar - flex var with method constraint returning open tag union" {
    ...
    const try_result_idx = try lt.layout_store.fromTypeVar(0, try_var, &lt.type_scope, false);
...
test "fromTypeVar - type alias inside Try nominal (issue #8708)" {
    ...
    const result_idx = try lt.layout_store.fromTypeVar(0, try_var, &lt.type_scope, false);
    const result_layout = lt.layout_store.getLayout(result_idx);
...
test "fromTypeVar - recursive nominal type with nested Box at depth 2+ (issue #8816)" {
    ...
    const result_idx = try lt.layout_store.fromTypeVar(0, rich_doc_var, &lt.type_scope, false);
    const result_layout = lt.layout_store.getLayout(result_idx);
...
test "layoutSizeAlign - recursive nominal type with record containing List (issue #8923)" {
    ...
    const result_idx = try lt.layout_store.fromTypeVar(0, statement_var, &lt.type_scope, false);
    const result_layout = lt.layout_store.getLayout(result_idx);
...
test "fromTypeVar - recursive nominal with Box has no double-boxing (issue #8916)" {
    ...
    // Compute the layout
    const nat_layout_idx = try lt.layout_store.fromTypeVar(0, nat_var, &lt.type_scope, false);
