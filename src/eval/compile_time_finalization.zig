...
fn finishConstRoot(
    ...
    // scheme: under the ML value restriction (design.md "Polarity"),
    // annotated syntactic values generalize, quantifying their annotation's
    // type variables — including the implicit row extensions polarity opening
    // mints — so one stored ground representation cannot serve its uses: a
    // use may instantiate the scheme at a wider row than the value was
    // produced at. Leave such templates on the eval path, where every
    // distinct requested monotype specializes the value at its own
    // representation.
