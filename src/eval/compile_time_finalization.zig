...
fn transcodeCompletedSlots(
    ...
    for (target.lir_result.static_data_values.items, 0..) |target_entry, index| {
        ...
        if (target_root.role == .value and !failed) try lir.ComptimeValueGuards.completeSuccessfulSlot(&target.lir_result, target_slot);
    }
...
test "shared frozen erased callables execute on interpreter dev and LLVM" {
    ...
    const proc_name = try static_data_exports.procSymbolName(allocator, program.store.getProcSpec(worker).identity);
