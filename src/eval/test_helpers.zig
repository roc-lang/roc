...
const EvalDynLib = switch (builtin.target.os.tag) {
    .windows => struct {
        ...
        fn open(allocator: Allocator, path: []const u8) (Allocator.Error || error{ InvalidUtf8, LlvmBackendUnavailable })!@This() {
...
const SharedMemoryAllocator = if (builtin.target.os.tag == .freestanding) struct {
    ...
    fn getSystemPageSize() Allocator.Error!usize {
        return 64 * 1024;
    }

    fn create(_: anytype, size: usize, page_size: usize) Allocator.Error!@This() {
    ...
    fn createWithMinSize(_: std.Io, preferred_size: usize, _: usize, page_size: usize) Allocator.Error!@This() {
...
) (Allocator.Error || error{ TooNested, ParseError })!CheckedModule {
...
fn publishImportKeysWithBuiltin(
    ...
    pre_published_builtin: ?PrePublishedBuiltin,
) Allocator.Error![]check.CheckedArtifact.PublishImportArtifact {
...
pub fn mainProcArgLayouts(allocator: Allocator, lowered: *const LoweredProgram) Allocator.Error![]LayoutIdx {
