...
test "interpreter: tuples and records" {
    ...
    var it = try Interpreter.init(std.testing.allocator, res_t.module_env, res_t.bool_stmt, res_t.bool_module.env);
    defer it.deinit();
    ...
    var ir = try Interpreter.init(std.testing.allocator, res_r.module_env, res_r.bool_stmt, res_r.bool_module.env);
    defer ir.deinit();
