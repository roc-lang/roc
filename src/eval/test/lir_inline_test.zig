...
test "optimized for over if-selected mapped list iter append uses private iterator cursors" {
    const source =
        \\module [main]
        \\
        \\main : {}
        \\main = {
        \\    var $sum = 0.I64
        \\    for item in if 1.I64 == 1.I64 { [1.I64, 2.I64].iter().append(3.I64).map(|n| n * 2) } else { [4.I64, 5.I64].iter().map(|n| n * 3) } {
        \\        $sum = $sum + item
        \\    }
        \\    dbg $sum
        \\    {}
    ...
    try expectOptimizedDbgEvents(source, &.{"12"});
...
test "optimized for over match-selected filtered list iter append preserves producer order" {
    const source =
        ...
        \\    var $sum = 0.I64
        \\    for item in match choose() {
        \\        0 => [tap(4.I64), tap(5.I64)].iter().drop_if(|n| n == 4)
        \\        _ => [tap(1.I64), tap(2.I64)].iter().append(tap(3.I64)).keep_if(|n| tap(n + 10) > 11)
        \\    } {
        \\        dbg item
        \\        $sum = $sum + item
        \\    }
    ...
    try expectOptimizedDbgEvents(source, &.{ "\"scrutinee\"", "1", "2", "3", "11", "12", "2", "13", "3", "5" });
