...
test "debug: nested List.any captured Str proc body rc shape" {
    ...
    const predicate_ls = lambda_set_store.getExprLambdaSet(root_args[1]) orelse return error.TestUnexpectedResult;
    const predicate_members = lambda_set_store.getMembers(lambda_set_store.getLambdaSet(predicate_ls).members);
    try std.testing.expectEqual(@as(usize, 1), predicate_members.len);
    const predicate_proc = predicate_members[0].proc;
    ...
    const lowered_root = try translator.lower(mir_expr);

    var specialized_proc_id: ?lir.LIR.LirProcSpecId = null;
    var specialization_it = translator.direct_proc_specs.iterator();
    while (specialization_it.next()) |entry| {
        const callee_key = std.mem.bytesToValue(u64, entry.key_ptr.*[0..@sizeOf(u64)]);
        if (callee_key == ((@as(u64, 1) << 63) | @as(u64, @intFromEnum(predicate_proc)))) {
            specialized_proc_id = entry.value_ptr.proc;
            break;
        }
    }
    ...
    const Search = struct {
        fn printStmtTree(store: *const LirExprStore, stmt_id: lir.LIR.CFStmtId, depth: usize) void {
            ...
            switch (stmt) {
                .ret => |ret| {
                    for (0..depth) |_| std.debug.print("  ", .{});
                    std.debug.print("ret value={s}\n", .{@tagName(store.getExpr(ret.value))});
                    printExprTree(store, ret.value, depth + 1);
                },
                .expr_stmt => |expr_stmt| {
                    for (0..depth) |_| std.debug.print("  ", .{});
                    std.debug.print("expr_stmt value={s}\n", .{@tagName(store.getExpr(expr_stmt.value))});
                    printExprTree(store, expr_stmt.value, depth + 1);
                    printStmtTree(store, expr_stmt.next, depth);
                },
                .let_stmt => |let_stmt| {
                    ...
                    std.debug.print(
                        "{s} value={s}",
                        .{ @tagName(stmt), @tagName(store.getExpr(let_stmt.value)) },
                    );
                    if (pat == .bind) {
                        std.debug.print(
                            " symbol={d} layout={d}",
                ...
                .switch_stmt => |switch_stmt| {
                    for (0..depth) |_| std.debug.print("  ", .{});
                    std.debug.print("switch cond={s}\n", .{@tagName(store.getExpr(switch_stmt.cond))});
                    printExprTree(store, switch_stmt.cond, depth + 1);
                    for (store.getCFSwitchBranches(switch_stmt.branches), 0..) |branch, i| {
                        for (0..depth) |_| std.debug.print("  ", .{});
                        std.debug.print("branch[{d}]\n", .{i});
                        printStmtTree(store, branch.body, depth + 1);
                    }
                    for (0..depth) |_| std.debug.print("  ", .{});
                    std.debug.print("default\n", .{});
                    printStmtTree(store, switch_stmt.default_branch, depth + 1);
                },
                .jump => {},
                .join => {},
                .match_stmt => {},
        ...
        fn printExprTree(store: *const LirExprStore, expr_id: lir.LIR.LirExprId, depth: usize) void {
            ...
            switch (expr) {
                    ...
                    for (store.getStmts(block.stmts), 0..) |stmt, i| {
                        switch (stmt) {
                            .decl, .mutate => |binding| {
                                ...
                                for (0..depth) |_| std.debug.print("  ", .{});
                                std.debug.print(
                                    "stmt[{d}] {s} semantics={s} expr={s}",
                                    .{ i, @tagName(stmt), @tagName(binding.semantics), @tagName(store.getExpr(binding.expr)) },
                                );
                                if (pat == .bind) {
                                    std.debug.print(
                                        " symbol={d} layout={d}",
                                ...
                                std.debug.print("\n", .{});
                                printExprTree(store, binding.expr, depth + 1);
                            },
                            else => {},
                        }
                    }
                    for (0..depth) |_| std.debug.print("  ", .{});
                    std.debug.print("final={s}\n", .{@tagName(store.getExpr(block.final_expr))});
                    printExprTree(store, block.final_expr, depth + 1);
                },
                .proc_call => |call| {
                    for (0..depth) |_| std.debug.print("  ", .{});
                    std.debug.print("proc_call proc={d} argc={d}\n", .{
                        @intFromEnum(call.proc),
                ...
                .low_level => |ll| {
                    for (0..depth) |_| std.debug.print("  ", .{});
                    std.debug.print("low_level {s} argc={d}\n", .{
                        @tagName(ll.op),
                        store.getExprSpan(ll.args).len,
                    });
                },
                else => {},
            }
        ...
        fn countExprDecrefsForSymbol(store: *const LirExprStore, expr_id: lir.LIR.LirExprId, symbol: lir.LIR.Symbol) u32 {
            ...
            return switch (expr) {
                ...
                .match_expr => |m| blk: {
                    var total: u32 = countExprDecrefsForSymbol(store, m.value, symbol);
                    for (store.getMatchBranches(m.branches)) |branch| {
                        total += countExprDecrefsForSymbol(store, branch.guard, symbol);
                        total += countExprDecrefsForSymbol(store, branch.body, symbol);
                    }
                ...
                .for_loop => |fl| countExprDecrefsForSymbol(store, fl.list_expr, symbol) + countExprDecrefsForSymbol(store, fl.body, symbol),
                .while_loop => |wl| countExprDecrefsForSymbol(store, wl.cond, symbol) + countExprDecrefsForSymbol(store, wl.body, symbol),
                .discriminant_switch => |ds| blk: {
                    var total: u32 = countExprDecrefsForSymbol(store, ds.value, symbol);
                    for (store.getExprSpan(ds.branches)) |branch_id| total += countExprDecrefsForSymbol(store, branch_id, symbol);
                    break :blk total;
                },
                .proc_call => |call| blk: {
                    var total: u32 = 0;
                    for (store.getExprSpan(call.args)) |arg| total += countExprDecrefsForSymbol(store, arg, symbol);
                    break :blk total;
                },
                .low_level => |ll| blk: {
                    var total: u32 = 0;
                    for (store.getExprSpan(ll.args)) |arg| total += countExprDecrefsForSymbol(store, arg, symbol);
                    break :blk total;
                },
                .list => |list_expr| blk: {
                    var total: u32 = 0;
                    for (store.getExprSpan(list_expr.elems)) |elem| total += countExprDecrefsForSymbol(store, elem, symbol);
                    break :blk total;
                },
                .struct_ => |s| blk: {
                    var total: u32 = 0;
                    for (store.getExprSpan(s.fields)) |field| total += countExprDecrefsForSymbol(store, field, symbol);
                    break :blk total;
                },
                .tag => |t| blk: {
                    var total: u32 = 0;
                    for (store.getExprSpan(t.args)) |arg| total += countExprDecrefsForSymbol(store, arg, symbol);
                    break :blk total;
                },
                .struct_access => |sa| countExprDecrefsForSymbol(store, sa.struct_expr, symbol),
                .tag_payload_access => |tpa| countExprDecrefsForSymbol(store, tpa.value, symbol),
                .nominal => |n| countExprDecrefsForSymbol(store, n.backing_expr, symbol),
                .early_return => |ret| countExprDecrefsForSymbol(store, ret.expr, symbol),
                .dbg => |d| countExprDecrefsForSymbol(store, d.expr, symbol),
                .expect => |e| countExprDecrefsForSymbol(store, e.cond, symbol) + countExprDecrefsForSymbol(store, e.body, symbol),
                .str_concat => |parts| blk: {
                    var total: u32 = 0;
                    for (store.getExprSpan(parts)) |part| total += countExprDecrefsForSymbol(store, part, symbol);
                    break :blk total;
                },
                .int_to_str => |its| countExprDecrefsForSymbol(store, its.value, symbol),
                .float_to_str => |fts| countExprDecrefsForSymbol(store, fts.value, symbol),
                .dec_to_str => |d| countExprDecrefsForSymbol(store, d, symbol),
                .str_escape_and_quote => |s| countExprDecrefsForSymbol(store, s, symbol),
                .incref => |inc| countExprDecrefsForSymbol(store, inc.value, symbol),
                .decref => |dec| blk: {
                    const dec_expr = store.getExpr(dec.value);
                    const hit: u32 = if (dec_expr == .lookup and @as(u64, @bitCast(dec_expr.lookup.symbol)) == key) 1 else 0;
                    break :blk hit + countExprDecrefsForSymbol(store, dec.value, symbol);
                },
                .free => |free_expr| countExprDecrefsForSymbol(store, free_expr.value, symbol),
                else => 0,
        ...
        fn countStmtDecrefsForSymbol(store: *const LirExprStore, stmt_id: lir.LIR.CFStmtId, symbol: lir.LIR.Symbol) u32 {
            ...
            return switch (stmt) {
                ...
                .jump => |jump| blk: {
                    var total: u32 = 0;
                    for (store.getExprSpan(jump.args)) |arg| total += countExprDecrefsForSymbol(store, arg, symbol);
                    break :blk total;
                },
                .ret => |ret| countExprDecrefsForSymbol(store, ret.value, symbol),
                .expr_stmt => |expr_stmt| countExprDecrefsForSymbol(store, expr_stmt.value, symbol) +
        ...
        fn collectExprBindSymbols(
            store: *const LirExprStore,
            expr_id: lir.LIR.LirExprId,
            out: *std.ArrayList(lir.LIR.Symbol),
        ) !void {
            if (expr_id.isNone()) return;
            const expr = store.getExpr(expr_id);
            switch (expr) {
                    ...
                    for (store.getStmts(block.stmts)) |stmt| {
                        switch (stmt) {
                            .decl, .mutate => |binding| {
                                const pat = store.getPattern(binding.pattern);
                                if (pat == .bind) {
                                    try out.append(test_allocator, pat.bind.symbol);
                                }
                                try collectExprBindSymbols(store, binding.expr, out);
                            },
                            .cell_init, .cell_store => |binding| try collectExprBindSymbols(store, binding.expr, out),
                            .cell_drop => {},
                        }
                    }
                    try collectExprBindSymbols(store, block.final_expr, out);
                },
                .if_then_else => |ite| {
                    for (store.getIfBranches(ite.branches)) |branch| {
                        try collectExprBindSymbols(store, branch.cond, out);
                        try collectExprBindSymbols(store, branch.body, out);
                    }
                    try collectExprBindSymbols(store, ite.final_else, out);
                },
                .match_expr => |match_expr| {
                    try collectExprBindSymbols(store, match_expr.value, out);
                    for (store.getMatchBranches(match_expr.branches)) |branch| {
                        try collectExprBindSymbols(store, branch.guard, out);
                        try collectExprBindSymbols(store, branch.body, out);
                    }
                },
                .proc_call => |call| for (store.getExprSpan(call.args)) |arg| try collectExprBindSymbols(store, arg, out),
                .low_level => |ll| for (store.getExprSpan(ll.args)) |arg| try collectExprBindSymbols(store, arg, out),
                .list => |list_expr| for (store.getExprSpan(list_expr.elems)) |elem| try collectExprBindSymbols(store, elem, out),
                .struct_ => |s| for (store.getExprSpan(s.fields)) |field| try collectExprBindSymbols(store, field, out),
                .tag => |t| for (store.getExprSpan(t.args)) |arg| try collectExprBindSymbols(store, arg, out),
                .struct_access => |sa| try collectExprBindSymbols(store, sa.struct_expr, out),
                .tag_payload_access => |tpa| try collectExprBindSymbols(store, tpa.value, out),
                .nominal => |n| try collectExprBindSymbols(store, n.backing_expr, out),
                .early_return => |ret| try collectExprBindSymbols(store, ret.expr, out),
                .dbg => |d| try collectExprBindSymbols(store, d.expr, out),
                .expect => |e| {
                    try collectExprBindSymbols(store, e.cond, out);
                    try collectExprBindSymbols(store, e.body, out);
                },
                .str_concat => |parts| for (store.getExprSpan(parts)) |part| try collectExprBindSymbols(store, part, out),
                .int_to_str => |its| try collectExprBindSymbols(store, its.value, out),
                .float_to_str => |fts| try collectExprBindSymbols(store, fts.value, out),
                .dec_to_str => |d| try collectExprBindSymbols(store, d, out),
                .str_escape_and_quote => |s| try collectExprBindSymbols(store, s, out),
                .incref => |inc| try collectExprBindSymbols(store, inc.value, out),
                .decref => |dec| try collectExprBindSymbols(store, dec.value, out),
                .free => |free_expr| try collectExprBindSymbols(store, free_expr.value, out),
                else => {},
            }
        }

        fn collectStmtBindSymbols(
            store: *const LirExprStore,
            stmt_id: lir.LIR.CFStmtId,
            out: *std.ArrayList(lir.LIR.Symbol),
        ) !void {
            ...
            switch (stmt) {
                .let_stmt => |let_stmt| {
                    ...
                    try collectExprBindSymbols(store, let_stmt.value, out);
                    try collectStmtBindSymbols(store, let_stmt.next, out);
                },
                .join => |join| {
                    try collectStmtBindSymbols(store, join.body, out);
                    try collectStmtBindSymbols(store, join.remainder, out);
                },
                .jump => |jump| for (store.getExprSpan(jump.args)) |arg| try collectExprBindSymbols(store, arg, out),
                .ret => |ret| try collectExprBindSymbols(store, ret.value, out),
                .expr_stmt => |expr_stmt| {
                    try collectExprBindSymbols(store, expr_stmt.value, out);
                    try collectStmtBindSymbols(store, expr_stmt.next, out);
                },
                .switch_stmt => |switch_stmt| {
                    try collectExprBindSymbols(store, switch_stmt.cond, out);
                    for (store.getCFSwitchBranches(switch_stmt.branches)) |branch| {
                        try collectStmtBindSymbols(store, branch.body, out);
                    }
                    try collectStmtBindSymbols(store, switch_stmt.default_branch, out);
                },
                .match_stmt => |match_stmt| {
                    try collectExprBindSymbols(store, match_stmt.value, out);
                    for (store.getCFMatchBranches(match_stmt.branches)) |branch| {
                        try collectExprBindSymbols(store, branch.guard, out);
                        try collectStmtBindSymbols(store, branch.body, out);
                    }
        ...
        fn collectExprBindSymbolsWithLayouts(
            store: *const LirExprStore,
            expr_id: lir.LIR.LirExprId,
            out: *std.ArrayList(BindWithLayout),
        ) !void {
            if (expr_id.isNone()) return;
            const expr = store.getExpr(expr_id);
            switch (expr) {
                    ...
                    for (store.getStmts(block.stmts)) |stmt| {
                        switch (stmt) {
                            .decl, .mutate => |binding| {
                                const pat = store.getPattern(binding.pattern);
                                if (pat == .bind) {
                                    try out.append(test_allocator, .{ .symbol = pat.bind.symbol, .layout_idx = pat.bind.layout_idx });
                                }
                                try collectExprBindSymbolsWithLayouts(store, binding.expr, out);
                            },
                            .cell_init, .cell_store => |binding| try collectExprBindSymbolsWithLayouts(store, binding.expr, out),
                            .cell_drop => {},
                        }
                    }
                    try collectExprBindSymbolsWithLayouts(store, block.final_expr, out);
                },
                .if_then_else => |ite| {
                    for (store.getIfBranches(ite.branches)) |branch| {
                        try collectExprBindSymbolsWithLayouts(store, branch.cond, out);
                        try collectExprBindSymbolsWithLayouts(store, branch.body, out);
                    }
                    try collectExprBindSymbolsWithLayouts(store, ite.final_else, out);
                },
                .match_expr => |match_expr| {
                    try collectExprBindSymbolsWithLayouts(store, match_expr.value, out);
                    for (store.getMatchBranches(match_expr.branches)) |branch| {
                        try collectExprBindSymbolsWithLayouts(store, branch.guard, out);
                        try collectExprBindSymbolsWithLayouts(store, branch.body, out);
                    }
                },
                .proc_call => |call| for (store.getExprSpan(call.args)) |arg| try collectExprBindSymbolsWithLayouts(store, arg, out),
                .low_level => |ll| for (store.getExprSpan(ll.args)) |arg| try collectExprBindSymbolsWithLayouts(store, arg, out),
                .list => |list_expr| for (store.getExprSpan(list_expr.elems)) |elem| try collectExprBindSymbolsWithLayouts(store, elem, out),
                .struct_ => |s| for (store.getExprSpan(s.fields)) |field| try collectExprBindSymbolsWithLayouts(store, field, out),
                .tag => |t| for (store.getExprSpan(t.args)) |arg| try collectExprBindSymbolsWithLayouts(store, arg, out),
                .struct_access => |sa| try collectExprBindSymbolsWithLayouts(store, sa.struct_expr, out),
                .tag_payload_access => |tpa| try collectExprBindSymbolsWithLayouts(store, tpa.value, out),
                .nominal => |n| try collectExprBindSymbolsWithLayouts(store, n.backing_expr, out),
                .early_return => |ret| try collectExprBindSymbolsWithLayouts(store, ret.expr, out),
                .dbg => |d| try collectExprBindSymbolsWithLayouts(store, d.expr, out),
                .expect => |e| {
                    try collectExprBindSymbolsWithLayouts(store, e.cond, out);
                    try collectExprBindSymbolsWithLayouts(store, e.body, out);
                },
                .str_concat => |parts| for (store.getExprSpan(parts)) |part| try collectExprBindSymbolsWithLayouts(store, part, out),
                .int_to_str => |its| try collectExprBindSymbolsWithLayouts(store, its.value, out),
                .float_to_str => |fts| try collectExprBindSymbolsWithLayouts(store, fts.value, out),
                .dec_to_str => |d| try collectExprBindSymbolsWithLayouts(store, d, out),
                .str_escape_and_quote => |s| try collectExprBindSymbolsWithLayouts(store, s, out),
                .incref => |inc| try collectExprBindSymbolsWithLayouts(store, inc.value, out),
                .decref => |dec| try collectExprBindSymbolsWithLayouts(store, dec.value, out),
                .free => |free_expr| try collectExprBindSymbolsWithLayouts(store, free_expr.value, out),
                else => {},
            }
        }
    };
    ...
    if (false) {
        const params = lir_store.getPatternSpan(lifted_proc.args);
        std.debug.print("predicate proc={d} specialized_proc={d} params={d}\n", .{
            @intFromEnum(predicate_proc),
            @intFromEnum(specialized_proc_id.?),
            params.len,
        });
        Search.printStmtTree(&lir_store, lifted_proc.body, 1);
        if (@intFromEnum(@as(lir.LIR.LirProcSpecId, @enumFromInt(3))) < lir_store.proc_specs.items.len) {
            const proc3 = lir_store.getProcSpec(@enumFromInt(3));
            std.debug.print("callee proc=3 args={d}\n", .{lir_store.getPatternSpan(proc3.args).len});
            Search.printStmtTree(&lir_store, proc3.body, 1);
        }
        for (params, 0..) |param_id, i| {
            const param = lir_store.getPattern(param_id);
            std.debug.print("  param[{d}] tag={s}\n", .{ i, @tagName(param) });
            if (param == .bind) {
                std.debug.print(
                    "    symbol={d} layout={d} decrefs={d}\n",
                    .{
                        param.bind.symbol.raw(),
                        @intFromEnum(param.bind.layout_idx),
                        Search.countStmtDecrefsForSymbol(&lir_store, lifted_proc.body, param.bind.symbol),
                    },
                );
            }
        }

        var body_binds = std.ArrayList(lir.LIR.Symbol).empty;
        defer body_binds.deinit(test_allocator);
        try Search.collectStmtBindSymbols(&lir_store, lifted_proc.body, &body_binds);
        for (body_binds.items) |symbol| {
            std.debug.print(
                "  body bind symbol={d} decrefs={d}\n",
                .{ symbol.raw(), Search.countStmtDecrefsForSymbol(&lir_store, lifted_proc.body, symbol) },
            );
        }

        var rc_pass = try lir.RcInsert.RcInsertPass.init(test_allocator, &lir_store, &layout_store);
        defer rc_pass.deinit();
        const with_rc = try rc_pass.insertRcOps(lowered_root);

        Search.printExprTree(&lir_store, with_rc, 1);

        var root_binds = std.ArrayList(BindWithLayout).empty;
        defer root_binds.deinit(test_allocator);
        try Search.collectExprBindSymbolsWithLayouts(&lir_store, with_rc, &root_binds);
        for (root_binds.items) |bind| {
            const layout_val = if (@intFromEnum(bind.layout_idx) < layout_store.layouts.len())
                layout_store.getLayout(bind.layout_idx)
            else
                null;
            std.debug.print(
                "  root bind symbol={d} layout={d} tag={s} decrefs={d}\n",
                .{
                    bind.symbol.raw(),
                    @intFromEnum(bind.layout_idx),
                    if (layout_val) |lv| @tagName(lv.tag) else "sentinel",
                    Search.countExprDecrefsForSymbol(&lir_store, with_rc, bind.symbol),
                },
            );
        }
    }
