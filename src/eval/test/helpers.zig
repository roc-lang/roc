...
pub fn runExpectTypeMismatchAndCrash(src: []const u8) !void {
    ...
    // Step 2: Run the LIR interpreter anyway and verify it crashes
    _ = lirInterpreterStr(test_allocator, resources.module_env, resources.expr_idx, resources.builtin_module.env) catch {
        // Any error during lowering or evaluation counts as a crash for type-mismatched code
        return;
    };

    // If we reach here, the interpreter succeeded when it should have crashed
    std.debug.print("Expected runtime crash, but LIR interpreter succeeded\n", .{});
...
pub fn runExpectStr(src: []const u8, expected_str: []const u8, should_trace: enum { trace, no_trace }) !void {
    ...
    // Str.inspect wraps strings in quotes, so compare with quoted expected value.
    // Strip surrounding quotes from Str.inspect output if present, then compare.
    const actual_str = if (interpreter_str.len >= 2 and interpreter_str[0] == '"' and interpreter_str[interpreter_str.len - 1] == '"')
        interpreter_str[1 .. interpreter_str.len - 1]
    else
        interpreter_str;
