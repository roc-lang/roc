...
pub const LlvmEvaluator = struct {
    ...
    pub fn generateCode(
        ...
        // 7. Extract code and find the entry point within the text section.
        const object_reader = backend.dev.object_reader;
        const extracted = object_reader.extractCodeSectionWithEntry(object_bytes) catch return error.CompilationFailed;
        const code_copy = self.allocator.dupe(u8, extracted.code) catch return error.OutOfMemory;
