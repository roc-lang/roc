...
fn getStructTagAndPayloadFields(self: *Interpreter, dest: *StackValue, result_layout: Layout) !struct { StackValue, StackValue } {
    if (isRecordStyleStruct(result_layout, &self.runtime_layout_store)) {
        ...
        const tag_field_idx = acc.findFieldIndex(layout_env.idents.tag) orelse
            debugUnreachable(null, "tag field not found in struct tag union", @src());
        const payload_field_idx = acc.findFieldIndex(layout_env.idents.payload) orelse
        ...
        const tag_rt_var = blk: {
            if (resolved.desc.content == .structure) {
                ...
                while (i < fields.len) : (i += 1) {
                    ...
                    if (f.name.eql(layout_env.idents.tag)) {
                        break :blk f.var_;
                    }
                }
            }
...
pub const Interpreter = struct {
    ...
    env: *can.ModuleEnv,
    /// Root module for well-known identifiers and type info; never changes during execution.
    ...
    pub fn initWithModuleEnvs(
        ...
        // Create layout store with all_module_envs, then set override to use runtime_types
        // NOTE: Callers must ensure all_module_envs is non-empty and contains env
        //
        ...
        result.runtime_layout_store.setMutableEnv(env);
    ...
    pub fn setupForClauseTypeMappings(self: *Interpreter, platform_env: *const can.ModuleEnv) Error!void {
        ...
        for (requires_types_slice) |required_type| {
            ...
            for (type_aliases_slice) |alias| {
                ...
                const rt_rigid_name = self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(rigid_name_str)) catch continue;
    ...
    pub fn callLowLevelBuiltin(self: *Interpreter, op: can.CIR.Expr.LowLevel, args: []StackValue, roc_ops: *RocOps, return_rt_var: ?types.Var) !StackValue {
        ...
        switch (op) {
            ...
            .list_len => {
                ...
                // This ensures nominal handling works when the CT return type was a flex var.
                const result_rt_var = blk: {
                    ...
                    // Create canonical U64 type for nominal handling
            ...
            .list_sublist => {
                ...
                const start_ident = layout_env.insertIdent(base_pkg.Ident.for_text("start")) catch return error.Crash;
                const len_ident = layout_env.insertIdent(base_pkg.Ident.for_text("len")) catch return error.Crash;
                const field_rt = try self.runtime_types.fresh();
                const sublist_start_stack = sublist_config.getFieldByIdent(start_ident, field_rt) catch debugUnreachable(roc_ops, "sublist config should have a start field", @src());
                const field_rt2 = try self.runtime_types.fresh();
                const sublist_len_stack = sublist_config.getFieldByIdent(len_ident, field_rt2) catch debugUnreachable(roc_ops, "sublist config should have a len field", @src());
            ...
            .dec_from_str,
            => {
                // typed from_str : Str -> Try(num, [BadNumStr])
                std.debug.assert(args.len == 1);
                const str_arg = args[0];
                const roc_str = str_arg.asRocStr().?;

                const result_rt_var = return_rt_var orelse debugUnreachable(roc_ops, "return type required for from_str", @src());

                return switch (op) {
                    .u8_from_str => self.numFromStrInt(u8, roc_str, result_rt_var),
                    .i8_from_str => self.numFromStrInt(i8, roc_str, result_rt_var),
                    .u16_from_str => self.numFromStrInt(u16, roc_str, result_rt_var),
                    .i16_from_str => self.numFromStrInt(i16, roc_str, result_rt_var),
                    .u32_from_str => self.numFromStrInt(u32, roc_str, result_rt_var),
                    .i32_from_str => self.numFromStrInt(i32, roc_str, result_rt_var),
                    .u64_from_str => self.numFromStrInt(u64, roc_str, result_rt_var),
                    .i64_from_str => self.numFromStrInt(i64, roc_str, result_rt_var),
                    .u128_from_str => self.numFromStrInt(u128, roc_str, result_rt_var),
                    .i128_from_str => self.numFromStrInt(i128, roc_str, result_rt_var),
                    .f32_from_str => self.numFromStrFloat(f32, roc_str, result_rt_var),
                    .f64_from_str => self.numFromStrFloat(f64, roc_str, result_rt_var),
                    .dec_from_str => self.numFromStrDec(roc_str, result_rt_var),
                    else => unreachable,
                };
            },
            .dec_to_str => {
    ...
    const UnaryOpKind = enum { negate, not };
    ...
    fn evalNumericNegate(self: *Interpreter, operand: StackValue, roc_ops: *RocOps) !StackValue {
        const num_val = try self.extractNumericValue(operand);
        var out = try self.pushRaw(operand.layout, 0, operand.rt_var);
        out.is_initialized = false;
    ...
    fn evalUnaryOp(self: *Interpreter, op: UnaryOpKind, operand: StackValue, roc_ops: *RocOps) !StackValue {
        return switch (op) {
            .negate => try self.evalNumericNegate(operand, roc_ops),
            .not => try self.makeBoolValue(!self.boolValueEquals(true, operand, roc_ops)),
        };
    }

    fn evalNumericComparisonOp(
        self: *Interpreter,
        lhs: StackValue,
        rhs: StackValue,
        op: can.CIR.Expr.Binop.Op,
        ...
        const result = try self.compareNumericValues(lhs, rhs, compare_op);
        return try self.makeBoolValue(if (negate_result) !result else result);
    }

    fn evalNumericOperator(
        self: *Interpreter,
        op: can.CIR.Expr.Binop.Op,
        lhs: StackValue,
        rhs: StackValue,
        ...
        return switch (op) {
            .add, .sub, .mul, .div, .div_trunc, .rem => try self.evalNumericBinop(op, lhs, rhs, roc_ops),
            else => null,
        };
    }
    ...
    fn valuesStructurallyEqual(
        ...
        // Check for nominal types first so their equality semantics are preserved
        // before following aliases and backing vars.
        ...
        if (direct_resolved.desc.content == .structure) {
            if (direct_resolved.desc.content.structure == .nominal_type) {
                ...
                return try self.structuralEqualNominal(lhs, rhs, nom, roc_ops);
            }
        }
        ...
        return switch (lhs_content.structure) {
            .nominal_type => |nom| try self.structuralEqualNominal(lhs, rhs, nom, roc_ops),
            .tuple => |tuple| {
    ...
    /// Structural equality for nominal values.
    fn structuralEqualNominal(
        self: *Interpreter,
        lhs: StackValue,
        rhs: StackValue,
        ...
        // Compare directly by element here to avoid nested polymorphic-context mismatches.
    ...
    pub fn getCanonicalBoolRuntimeVar(self: *Interpreter) !types.Var {
        ...
        // We keep the nominal type itself rather than its backing type.
    ...
    pub fn appendUnionTags(self: *Interpreter, runtime_var: types.Var, list: *std.array_list.AlignedManaged(types.Tag, null)) !void {
        ...
        const sort_ident_store = self.runtime_layout_store.getEnv().common.getIdentStore();
    ...
    pub fn findTagIndexByIdent(
        ...
        const rt_tag_ident = try self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(source_name_str));
    ...
    fn extractTagValue(self: *Interpreter, value: StackValue, union_rt_var: types.Var) !TagValue {
        switch (value.layout.tag) {
            ...
            .struct_ => {
                ...
                const layout_env = self.runtime_layout_store.getEnv();
                if (rec_acc.findFieldIndex(layout_env.idents.tag)) |tag_field_idx_found| {
    ...
    fn extractTagValueFromRecord(self: *Interpreter, _: StackValue, union_rt_var: types.Var, acc: StackValue.RecordAccessor, tag_field_idx: usize) !TagValue {
        ...
        var payload_value: ?StackValue = null;
        const layout_env = self.runtime_layout_store.getEnv();
        if (acc.findFieldIndex(layout_env.idents.payload)) |payload_idx| {
    ...
    fn writeErrBadUtf8ToStruct(self: *Interpreter, dest: *StackValue, result: anytype, err_index: ?usize) !void {
        if (isRecordStyleStruct(dest.layout, &self.runtime_layout_store)) {
            ...
            var acc = try dest.asRecord(&self.runtime_layout_store);
            const layout_env = self.runtime_layout_store.getEnv();
            const tag_field_idx = acc.findFieldIndex(layout_env.idents.tag) orelse return;
            const payload_field_idx = acc.findFieldIndex(layout_env.idents.payload) orelse return;
    ...
    fn writeBadUtf8InnerPayload(self: *Interpreter, payload: StackValue, result: anytype) !void {
        ...
        if (isRecordStyleStruct(payload.layout, &self.runtime_layout_store)) {
            ...
            const layout_env = self.runtime_layout_store.getEnv();
            if (err_rec.findFieldIndex(layout_env.idents.tag)) |tag_idx| {
                const field_rt = try self.runtime_types.fresh();
            ...
            if (err_rec.findFieldIndex(layout_env.idents.payload)) |inner_payload_idx| {
                const field_rt = try self.runtime_types.fresh();
    ...
    fn writeProblemIndexFields(self: *Interpreter, value: StackValue, result: anytype) !void {
        ...
        const layout_env = self.runtime_layout_store.getEnv();
        if (inner_acc.findFieldIndex(layout_env.idents.problem)) |problem_idx| {
        ...
        if (inner_acc.findFieldIndex(layout_env.idents.index)) |index_idx| {
    ...
    fn patternMatchesBind(
        ...
        switch (pat) {
            ...
            .list => |list_pat| {
                ...
                // Using the value's type preserves concrete element information through polymorphic calls.
            ...
            .applied_tag => |tag_pat| {
                ...
                const expected_ident = try self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(expected_name_str));
    ...
    pub fn deinit(self: *Interpreter) void {
        ...
        // Free all_module_envs if we allocated it
        if (self.owns_all_module_envs) {
    ...
    fn mkListStrTypeRuntime(self: *Interpreter) !types.Var {
        ...
        const str_type_name_ident = try self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(str_type_name));
        ...
        const list_type_name_ident = try self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(list_type_name));
    ...
    /// Used by str_to_utf8 to ensure correct nominal handling.
    ...
    fn mkNumberTypeContentRuntime(self: *Interpreter, type_name: []const u8) !types.Content {
        ...
        // This keeps runtime nominal ids aligned with the canonical Builtin.Num.* names.
        // Insert into runtime_layout_store.getEnv() to be consistent with translateTypeVar's nominal handling.
        ...
        const type_name_ident = try self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(qualified_type_name));
                ...
                .tag_union => |ct_tu| {
                    ...
                    if (rt_tu_opt) |rt_tu| {
                        ...
                        for (ct_tags.items(.name), ct_tags.items(.args)) |ct_tag_name, ct_tag_args| {
                            ...
                            const rt_ct_tag_ident = try self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(ct_tag_name_str));
                    ...
                    switch (flat) {
                        .tag_union => |tu| {
                            ...
                            for (rt_tags.items) |*tag| {
                                ...
                                const rt_tag_name = try self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(source_name_str));
                                tag.* = .{
                                    .name = rt_tag_name,
                                    .args = rt_args_range,
                                };
                            ...
                            const ident_store = self.runtime_layout_store.getEnv().common.getIdentStore();
                ...
                .flex => |flex| {
                    ...
                    const rt_flex = if (flex.constraints.len() > 0) blk_flex: {
                        ...
                        for (ct_constraints) |ct_constraint| {
                            ...
                            const rt_fn_name = try self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(ct_fn_name_str));
    ...
    fn instantiateType(self: *Interpreter, type_var: types.Var, subst_map: *std.AutoHashMap(types.Var, types.Var)) Error!types.Var {
        ...
        var instantiator = types.instantiate.Instantiator{
            ...
            .idents = self.runtime_layout_store.getEnv().common.getIdentStore(),
    ...
    fn gatherTags(
        ...
        for (tag_slice.items(.name), tag_slice.items(.args)) |name, args| {
            try scratch_tags.append(ctx.allocator, .{ .name = name, .args = args });
        }
    ...
    pub const Continuation = union(enum) {
        ...
        /// Unary operation - apply an operator after the operand is evaluated.
        unary_op_apply: UnaryOpApply,

        /// Binary operation - evaluate RHS after LHS is evaluated.
        binop_eval_rhs: BinopEvalRhs,

        /// Binary operation - apply an operator after both operands are evaluated.
        binop_apply: BinopApply,

        /// Field access - await receiver evaluation and capture immediately.
        field_access_await_receiver: FieldAccessAwaitReceiver,

        /// Field access - resolve a field after receiver is evaluated.
        field_access_resolve: FieldAccessResolve,
        ...
        /// This ensures nominal handling finds the nominal type info.
        ...
        pub const TagCollect = struct {
            ...
            /// Used for type identity and nominal handling.
            rt_var: types.Var,
        ...
        pub const CallCleanup = struct {
            ...
            call_ret_rt_var: ?types.Var,
            /// Saved rigid_subst to restore after a polymorphic call
            saved_rigid_subst: ?std.AutoHashMap(types.Var, types.Var),
        ...
        /// Field access - await receiver evaluation, then capture receiver for resolve.
        pub const FieldAccessAwaitReceiver = struct {
            field_name: base_pkg.Ident.Idx,
            expr_idx: can.CIR.Expr.Idx,
        };

        /// Field access - resolve a field with receiver carried in continuation.
        pub const FieldAccessResolve = struct {
            field_name: base_pkg.Ident.Idx,
            expr_idx: can.CIR.Expr.Idx,
    ...
    fn scheduleExprEval(
        ...
        switch (expr) {
            ...
            .e_binop => |binop| {
                switch (binop.op) {
                    ...
                    .@"or" => {
                        ...
                        const normalized_op: can.CIR.Expr.Binop.Op = if (binop.op == .ne) .eq else binop.op;
                        const negate_result = binop.op == .ne;

                        // Schedule: first evaluate LHS, then evaluate RHS, then apply the operator.
            ...
            .e_nominal => |nom| {
                ...
                // with the nominal type's rt_var.
            ...
            .e_unary_minus => |unary_minus| {
                ...
                // Schedule: first evaluate operand, then apply the operator.
            ...
            .e_field_access => |field_access| {
                try work_stack.push(.{ .apply_continuation = .{ .field_access_await_receiver = .{
    ...
    fn evalNum(
        ...
        // If so, we need to give it a concrete Dec type
        ...
        // Also update the rt_var to be a concrete Dec type
        ...
        // If the rt_var is still flex, update it to a concrete type
    ...
    fn evalFracF32(
        ...
        // Check if the resolved type is flex/rigid (unconstrained).
        // If so, we need to give it a concrete F32 type
        const resolved_rt = self.runtime_types.resolveVar(layout_rt_var);
        const is_flex_or_rigid = resolved_rt.desc.content == .flex or resolved_rt.desc.content == .rigid;
    ...
    fn evalFracF64(
        ...
        // Check if the resolved type is flex/rigid (unconstrained).
        // If so, we need to give it a concrete F64 type
        const resolved_rt = self.runtime_types.resolveVar(layout_rt_var);
        const is_flex_or_rigid = resolved_rt.desc.content == .flex or resolved_rt.desc.content == .rigid;
        const final_rt_var = if (is_flex_or_rigid) blk: {
    ...
    fn evalDec(
        ...
        // Check if the resolved type is flex/rigid (unconstrained).
        // If so, give it a concrete Dec type
        // REPL rendering will still strip .0 from whole-number Dec values regardless of type.
        const resolved_rt = self.runtime_types.resolveVar(layout_rt_var);
        const is_flex_or_rigid = resolved_rt.desc.content == .flex or resolved_rt.desc.content == .rigid;
        const final_rt_var = if (is_flex_or_rigid) blk: {
        ...
        switch (layout_val.tag) {
            .struct_ => {
                if (isRecordStyleStruct(layout_val, &self.runtime_layout_store)) {
                    ...
                    if (acc.findFieldIndex(layout_env.idents.payload)) |payload_field_idx| {
                        const field_rt2 = try self.runtime_types.fresh();
    ...
    fn resolveCapture(self: *Interpreter, cap: can.CIR.Expr.Capture, roc_ops: *RocOps) ?StackValue {
        ...
        if (self.active_closures.items.len > 0) {
            ...
            const rt_cap_name = self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(cap_name_text)) catch return null;
            ...
            while (closure_idx > 0) {
                ...
                if (cls_val.layout.tag == .closure and cls_val.ptr != null) {
                    ...
                    if (rec_acc.findFieldIndex(rt_cap_name)) |fidx| {
                        ...
                        if (has_real_captures) {
                            ...
                            if (captured_pattern_idx) |cap_pattern| {
                                ...
                                if (!is_top_level_def) {
                                    ...
                                    const var_ident_text = self.env.getIdent(var_ident);
                                    const runtime_ident = self.runtime_layout_store.getMutableEnv().?.insertIdent(base_pkg.Ident.for_text(var_ident_text)) catch return error.Crash;
                                    if (accessor.findFieldIndex(runtime_ident)) |fidx| {
                                        const field_rt = try self.runtime_types.fresh();
    ...
    fn scheduleNextStatement(
        ...
        switch (stmt) {
            ...
            .s_dbg => |dbg_stmt| {
                ...
                // can corrupt type resolution for subsequent polymorphic uses of the
                ...
                // causing downstream uses (like `List.fold`) to infer wrong types.
        ...
        switch (cont) {
            ...
            .list_collect => |lc| {
                ...
                if (lc.remaining_elems.len > 0) {
                    ...
                    if (total_count == 0) {
                        ...
                        // Set the runtime type variable so nominal handling works correctly.
            ...
            .record_collect => |rc| {
                ...
                if (rc.remaining_fields.len > 0) {
                    ...
                    // Create record layout using runtime_layout_store.getEnv() for field name lookups
                    const record_layout_idx = try self.runtime_layout_store.putRecord(self.runtime_layout_store.getMutableEnv().?, union_layouts.items, union_names.items);
                        ...
                        if (payload_field.ptr) |payload_ptr| {
                            if (total_count == 1) {
                                ...
                                if (layouts_differ) {
                                    ...
                                    const field_names = [2]base_pkg.Ident.Idx{ layout_env.idents.tag, layout_env.idents.payload };
                                    const proper_record_idx = try self.runtime_layout_store.putRecord(layout_env, &field_layouts, &field_names);
                                    ...
                                    // Write tag discriminant
                                    const proper_tag_field_idx = proper_acc.findFieldIndex(layout_env.idents.tag) orelse unreachable;
                                    ...
                                    // Write payload
                                    const proper_payload_field_idx = proper_acc.findFieldIndex(layout_env.idents.payload) orelse unreachable;
                                ...
                                if (tuple_layouts_differ) {
                                    ...
                                    const field_names_arr = [2]base_pkg.Ident.Idx{ layout_env.idents.tag, layout_env.idents.payload };
                                    const proper_record_idx = try self.runtime_layout_store.putRecord(layout_env, &field_layouts_arr, &field_names_arr);
                                    const proper_record_layout = self.runtime_layout_store.getLayout(proper_record_idx);
                                    var proper_dest = try self.pushRaw(proper_record_layout, 0, tc.rt_var);
                                    var proper_acc = try proper_dest.asRecord(&self.runtime_layout_store);
            ...
            .call_invoke_closure => |ci| {
                ...
                if (func_val.layout.tag == .closure) {
                    ...
                    if (extractLowLevelOp(lambda_expr, self.env.store)) |ll_op| {
                        ...
                        // polymorphic type and needed for correct nominal handling on the result.
                        try value_stack.push(result);
                        return true;
                    }
                    ...
                    try work_stack.push(.{
                        .apply_continuation = .{
                            .call_cleanup = .{
                                ...
                                // Don't pass call_ret_rt_var for regular calls.
            ...
            .call_cleanup => |cleanup| {
                ...
                // This corrects the return type for polymorphic calls where the
                // function body's type (for example from Builtin) may have
            ...
            .unary_op_apply => |ua| {
                ...
                defer cont_trace.end();
                // Unary operation: operand is on stack, apply the operator.
                const operand = value_stack.pop() orelse return error.Crash;
                defer operand.decref(&self.runtime_layout_store, roc_ops);

                const result = try self.evalUnaryOp(ua.op, operand, roc_ops);
                try value_stack.push(result);
                return true;
            },
            .binop_eval_rhs => |be| {
                ...
                // We keep LHS on stack, then apply the operator after RHS is evaluated.
            ...
            .binop_apply => |ba| {
                ...
                defer cont_trace.end();
                // Binary operation: both operands are on the stack, apply the operator.
                ...
                if (resolved_check.desc.content == .flex or resolved_check.desc.content == .rigid) {
                    ...
                    // so we use direct numeric handling instead of nominal handling
                    defaulted_to_dec = true;
                }
                    ...
                    current_resolved.desc.content.structure == .nominal_type)
                {
                    ...
                    var result = self.structuralEqualNominal(lhs, rhs, nom, roc_ops) catch |err| switch (err) {
                ...
                // This handles cases where nominal handling would fail (e.g., polymorphic values)
                ...
                if (lhs_is_numeric_layout and rhs_is_numeric_layout and defaulted_to_dec) {
                    if (try self.evalNumericComparisonOp(lhs, rhs, ba.op, ba.negate_result)) |result| {
                        try value_stack.push(result);
                        return true;
                    }
                    if (try self.evalNumericOperator(ba.op, lhs, rhs, roc_ops)) |result| {
                        try value_stack.push(result);
                        return true;
                    }
                }
            ...
            .field_access_await_receiver => |da| {
                const cont_trace = tracy.traceNamed(@src(), "cont.field_access_await_receiver");
                defer cont_trace.end();
                ...
                try work_stack.push(.{ .apply_continuation = .{ .field_access_resolve = .{
            ...
            .field_access_resolve => |da| {
                const cont_trace = tracy.traceNamed(@src(), "cont.field_access_resolve");
                defer cont_trace.end();
            ...
            .for_iterate => |fl_in| {
                ...
                // that we need for downstream type handling.
                const elem_rt_var = blk: {
                ...
                // Override elem_layout if physical is struct but type-based is tag_union.
            ...
            .dbg_print_stmt => |dp| {
                ...
                // containing dbg lose their expected type, causing downstream expressions
            ...
            .nominal_wrap => |nw| {
                ...
                // This ensures nominal handling can find methods defined on the nominal type.
                var result = value_stack.pop() orelse {
