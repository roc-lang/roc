    ...
    pub fn callLowLevelBuiltin(self: *Interpreter, op: can.CIR.Expr.LowLevel, args: []StackValue, roc_ops: *RocOps, return_rt_var: ?types.Var) !StackValue {
        switch (op) {
            ...
            .list_prepend => {
                ...
                std.debug.assert(args.len == 2); // low-level .list_prepend expects 2 arguments

                const roc_list_arg = args[0];
                const elt_arg = args[1];

                std.debug.assert(roc_list_arg.ptr != null); // low-level .list_prepend expects non-null list pointer
                std.debug.assert(elt_arg.ptr != null); // low-level .list_prepend expects non-null 2nd argument

                // Extract element layout from List(a)
                std.debug.assert(roc_list_arg.layout.tag == .list or roc_list_arg.layout.tag == .list_of_zst); // low-level .list_prepend expects list layout

                // Format arguments into proper types
                const roc_list: *const builtins.list.RocList = @ptrCast(@alignCast(roc_list_arg.ptr.?));
                const non_null_bytes: [*]u8 = @ptrCast(elt_arg.ptr.?);
                const prepend_elt: builtins.list.Opaque = non_null_bytes;
                ...
                const result_list = builtins.list.listPrepend(roc_list.*, elem_alignment_u32, prepend_elt, elem_size, elements_refcounted, if (elements_refcounted) @ptrCast(&refcount_context) else null, if (elements_refcounted) &listElementInc else &builtins.list.rcNone, update_mode, copy_fn, roc_ops);
