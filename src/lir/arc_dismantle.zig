...
const Analysis = struct {
    ...
    fn passesGate(self: *Analysis, local: LIR.LocalId) bool {
        ...
        if (self.solution.isBorrowed(local) and !self.is_param[local_index] and self.view_root[local_index] == no_index) return false;
    ...
    /// chance to dismantle.
    fn touchUnion(self: *Analysis, local: LIR.LocalId) void {
        const root_index = self.unionRootOf(local) orelse return;
        if (self.union_roots.getPtr(root_index)) |entry| {
            entry.disqualified = true;
        } else {
            self.union_roots.put(self.gpa, root_index, .{ .disqualified = true }) catch {};
        }
    }
    ...
    fn noteAliasDef(self: *Analysis, stmt: LIR.CFStmtId, target: LIR.LocalId, source: LIR.LocalId) Error!void {
        ...
        const transparent = self.solution.isBorrowed(target) and
            (self.solution.leaderOf(target) == root or self.solution.unitLocalOf(target) == root) and
        ...
        if (transparent) {
            ...
            if (!union_alias) self.touchUnion(source);
    ...
    candidates: while (it.next()) |entry| {
        ...
        const spine_start: LIR.CFStmtId = if (candidate.def_count == 1)
            switch (store.getCFStmt(candidate.def_stmt)) {
                ...
                .assign_ref => |stmt| if (union_root != null) stmt.next else continue :candidates,
                .init_uninitialized,
                ...
                switch (store.getCFStmt(cursor)) {
                    ...
                    .jump => |stmt| {
                        ...
                        const body = join_bodies.get(@intFromEnum(stmt.target)) orelse {
                            ...
                            // its taken fields absent only when the container
                            // is a parameter: a container with a defining
                            // statement is bound exactly once, so every path
                            // back to one of its reads runs that definition
                            // first and starts from an intact container.
                            if (candidate.def_count == 0) poison |= state.may;
                            break :chain;
                        };
        ...
        // every emission of its proc. A payload view is borrowed too, but the
        // unit its takes spend is its owned root's, so they are unconditional.
        const owned_only = solution.isBorrowed(local) and union_root == null;
