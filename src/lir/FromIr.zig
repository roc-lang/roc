...
const ProcLowerer = struct {
    ...
    fn bridgeTagUnionIntoLocal(
        ...
        if (builtin.mode == .Debug and actual_info.variants.len != target_info.variants.len) {
            std.debug.print(
                "lir.from_ir bridgeTagUnionIntoLocal proc={d} current_expr={any}\n",
                .{ @intFromEnum(self.proc_id), self.current_expr_id },
            );
            if (self.current_expr_id) |expr_id| {
                const expr = self.parent.input.store.getExpr(expr_id);
                std.debug.print(
                    "lir.from_ir current_expr_tag={s}\n",
                    .{@tagName(expr)},
                );
            }
            switch (actual_ref) {
                .local => |node_id| {
                    const node = self.parent.input.layouts.getNode(node_id);
                    std.debug.print(
                        "lir.from_ir bridgeTagUnionIntoLocal actual_input_node={s}\n",
                        .{@tagName(node)},
                    );
                    if (node == .tag_union) {
                        const refs = self.parent.input.layouts.getRefs(node.tag_union);
                        for (refs, 0..) |ref, i| {
                            std.debug.print("  actual input variant {d} ref={any}\n", .{ i, ref });
                        }
                    }
                },
                .canonical => |idx| std.debug.print(
                    "lir.from_ir bridgeTagUnionIntoLocal actual_input_ref_canonical={d}\n",
                    .{@intFromEnum(idx)},
                ),
            }
            switch (target_ref) {
                .local => |node_id| {
                    const node = self.parent.input.layouts.getNode(node_id);
                    std.debug.print(
                        "lir.from_ir bridgeTagUnionIntoLocal target_input_node={s}\n",
                        .{@tagName(node)},
                    );
                    if (node == .tag_union) {
                        const refs = self.parent.input.layouts.getRefs(node.tag_union);
                        for (refs, 0..) |ref, i| {
                            std.debug.print("  target input variant {d} ref={any}\n", .{ i, ref });
                        }
                    }
                },
                .canonical => |idx| std.debug.print(
                    "lir.from_ir bridgeTagUnionIntoLocal target_input_ref_canonical={d}\n",
                    .{@intFromEnum(idx)},
                ),
            }
            std.debug.print(
                "lir.from_ir bridgeTagUnionIntoLocal source_local={d} target_local={d} actual_ref={any} target_ref={any}\n",
                .{
                    @intFromEnum(source_local),
                    @intFromEnum(target_local),
                    actual_ref,
                    target_ref,
                },
            );
            std.debug.print(
                "lir.from_ir bridgeTagUnionIntoLocal actual_layout={d} variants={d}\n",
                .{ @intFromEnum(actual_layout), actual_info.variants.len },
            );
            for (0..actual_info.variants.len) |i| {
                std.debug.print(
                    "  actual variant {d} payload_layout={d}\n",
                    .{ i, @intFromEnum(actual_info.variants.get(@intCast(i)).payload_layout) },
                );
            }
            std.debug.print(
                "lir.from_ir bridgeTagUnionIntoLocal target_layout={d} variants={d}\n",
                .{ @intFromEnum(target_layout), target_info.variants.len },
            );
            for (0..target_info.variants.len) |i| {
                std.debug.print(
                    "  target variant {d} payload_layout={d}\n",
                    .{ i, @intFromEnum(target_info.variants.get(@intCast(i)).payload_layout) },
                );
            }
            std.debug.panic(
