...
pub const TargetConfig = struct {
    ...
    /// Direct-call inlining scope for SpecConstr's whole-body rewrite.
    /// Optimized builds use `.all_calls`; dev builds use `.iterator_fusion`
    /// so post-check time stays bounded. Consulted only when `inline_mode`
    /// is not `.none`, since that is what gates SpecConstr itself.
    spec_constr_body_rewrite: SpecConstrBodyRewrite = .all_calls,
...
pub const SpecConstrBodyRewrite = postcheck.MonotypeLifted.SpecConstr.BodyRewriteInlining;
...
pub fn lowerCheckedModulesToLir(
    ...
    var procedure_usage = if (target.inline_mode != .none) blk: {
        ...
        const usage = try postcheck.MonotypeLifted.SpecConstr.runAndCollectProcedureUsage(allocator, &lifted, target.spec_constr_body_rewrite);
