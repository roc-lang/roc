...
const ProcPass = struct {
    ...
    fn rewriteLinear(self: *ProcPass, original_stmt_id: CFStmtId, stmt_template: CFStmt, next_stmt: CFStmtId) Allocator.Error!CFStmtId {
        ...
        const rewritten_stmt = try switch (stmt_template) {
            ...
            .assign_call_indirect => |assign| self.store.addCFStmt(.{ .assign_call_indirect = .{ .target = assign.target, .result = assign.result, .closure = assign.closure, .args = assign.args, .capture_layout = assign.capture_layout, .next = with_drops } }),
            .assign_low_level => |assign| self.store.addCFStmt(.{ .assign_low_level = .{ .target = assign.target, .result = assign.result, .op = assign.op, .args = assign.args, .next = with_drops } }),
            .assign_list => |assign| self.store.addCFStmt(.{ .assign_list = .{ .target = assign.target, .result = assign.result, .elems = assign.elems, .next = with_drops } }),
            .assign_struct => |assign| self.store.addCFStmt(.{ .assign_struct = .{ .target = assign.target, .result = assign.result, .fields = assign.fields, .next = with_drops } }),
            .assign_tag => |assign| self.store.addCFStmt(.{ .assign_tag = .{ .target = assign.target, .result = assign.result, .discriminant = assign.discriminant, .payload = assign.payload, .next = with_drops } }),
