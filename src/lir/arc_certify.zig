    ...
    fn summarize(self: *Certifier, state: *const State) Allocator.Error![]const LocalSummary {
        ...
        for (0..self.proc_locals.items.len) |dense| {
            ...
            if (value != no_value) {
                ...
                } else if (self.isInlineStructRepresentation(self.proc_locals.items[dense]) or self.isListRepresentation(self.proc_locals.items[dense])) {
                    summary = .{
            ...
            if (relevant) {
                ...
                if (canonicalize_conditional) {
                    ...
                    if (value != no_value) {
                        ...
                        } else if (self.isInlineStructRepresentation(local) or self.isListRepresentation(local)) {
                            summary = .{
                                .class = .representation,
                                .repr = repr,
                                .balance = 0,
