    ...
    fn uniqueArgsInProc(self: *const ArcTest, proc_id: LIR.LirProcSpecId, target: LIR.LocalId) Allocator.Error!u64 {
        ...
        while (stack.pop()) |current| {
            ...
            switch (self.store.getCFStmt(current)) {
                ...
                inline .assign_ref, .assign_literal, .assign_call, .assign_call_erased, .assign_packed_erased_fn, .assign_list, .assign_struct, .assign_tag, .set_local, .debug, .expect, .comptime_branch_taken, .incref, .decref, .decref_if_initialized, .free => |s| {
    ...
    fn expectReachableConditionalDecrefBeforeSet(
        ...
        while (remaining > 0) : (remaining -= 1) {
            ...
            switch (stmt) {
                ...
                .switch_stmt, .switch_initialized_payload, .join => return error.NonLinearPath,
            }
        }
        return error.CyclicPath;
    }
