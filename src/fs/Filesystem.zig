...
const PosixReadError = if (is_freestanding) error{} else std.posix.ReadError;
const PosixRealPathError = if (is_freestanding) error{} else std.posix.RealPathError;
const PosixRenameError = if (is_freestanding) error{} else std.posix.RenameError;
...
pub const ReadError = std.fs.File.OpenError || PosixReadError || Allocator.Error || error{StreamTooLong};
...
pub const CanonicalizeError = error{ FileNotFound, Unknown, OutOfMemory } || PosixRealPathError;

/// All errors that can occur when renaming a file.
pub const RenameError = std.fs.Dir.RenameError || PosixRenameError;
