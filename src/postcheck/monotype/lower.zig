    ...
    fn restoreConstFnExpr(
        ...
        const expr = try fn_ctx.addExpr(.{
            ...
            .data = switch (lambda_expr.data) {
                .lambda => .{ .fn_def = .{
                    .fn_id = draftFinalFn(try self.lowerNestedFnFromContext(&fn_ctx, lambda_expr_id, template)),
        ...
        switch (checked_body.data) {
            .block => |block| {
                ...
                if (!statements.diverges) {
                    const final_stmt = try self.addStmt(.{ .expr = if (self.checkedExprDivergesInLoweredRuntime(block.final_expr))
                ...
                const stmt = try self.addStmt(.{ .expr = value });
                return try self.addExpr(.{ .ty = state_ty, .data = .{ .block = .{
        ...
        switch (checked_body.data) {
            .block => |block| {
                ...
                if (!statement_diverges) {
                    lowered_statements[block.statements.len] = try self.addStmt(.{ .expr = if (self.checkedExprDivergesInLoweredRuntime(block.final_expr))
                        try self.lowerDivergentExprAtType(block.final_expr, result_ty)
                    else
                        try self.lowerExpr(block.final_expr) });
                }
                ...
                const body_stmt = try self.addStmt(.{ .expr = body_expr });
                return try self.addExpr(.{ .ty = result_ty, .data = .{ .block = .{
