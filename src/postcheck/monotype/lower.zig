...
const BodyContext = struct {
    ...
    fn lowerLambdaArgsAndBody(
        ...
        const checked_body_expr = self.view.bodies.exprs[@intFromEnum(checked_body)];
        const saved_loc = self.builder.program.current_loc;
        defer self.builder.program.current_loc = saved_loc;
        self.builder.program.current_loc = try self.sourceLocFor(checked_body_expr.source_region);
