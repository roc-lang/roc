...
const BodyContext = struct {
    ...
    fn appendPrivateIteratorPlanStatement(
        ...
        if (!try self.binderOnlyUsedAsPrivateIterator(binder, remaining_statements, final_expr)) return false;

        const iterator_ty = try self.lowerType(self.view.bodies.expr(decl.expr).ty);
        const plan_id = (try self.privateIteratorPlanForLocalDeclaration(decl.expr, iterator_ty, lowered)) orelse return false;
        try self.local_iter_plans.put(binder, plan_id);
        return true;
    }

    fn privateIteratorPlanForLocalDeclaration(
        self: *BodyContext,
        expr_id: checked.CheckedExprId,
    ...
    fn appendIteratorPlanForPrivateLocal(
        ...
        const expr = self.view.bodies.expr(expr_id);
        const plan = self.dispatchPlanForIteratorProducerExpr(expr) orelse return null;
        if (!self.methodNameIs(plan.method, "append")) return null;
        switch (plan.result_mode) {
            .value => {},
            else => return null,
        }
        ...
        const receiver_index = switch (plan.dispatcher) {
            .arg => |index| index,
            .type_only => Common.invariant("checked Iter.append dispatch did not have an argument receiver"),
        };
        if (plan_args.len != 2 or receiver_index >= plan_args.len) {
            Common.invariant("checked Iter.append dispatch plan had an unexpected argument shape");
        }
        const after_index: usize = if (receiver_index == 0) 1 else 0;

        const dispatcher_ty = try self.lowerType(plan.dispatcher_ty);
        const before_plan = (try self.iteratorPlanForPrivateProducerOperand(plan_args[receiver_index], dispatcher_ty, lowered)) orelse return null;
        const before = self.builder.program.iterPlan(before_plan);
        const item_ty = self.iterItemType(iterator_ty);

        const after_value = try self.lowerDispatchOperandAtType(plan_args[after_index], item_ty);
        ...
        try lowered.append(self.allocator, try self.builder.program.addStmt(.{ .let_ = .{
            .pat = try self.builder.bindPat(after_local, item_ty),
            .value = after_value,
        } }));
        ...
        var steps = before.steps;
        steps.one = true;
        steps.done = before.done == .reachable;

        return try self.builder.program.addIterPlan(.{
            .item_ty = item_ty,
            .length = switch (before.length) {
                .known => |len| .{ .known = try self.builder.lowLevelExpr(.num_plus, &.{ len, one_expr }, u64_ty) },
                .unknown => .unknown,
            },
            .steps = steps,
            .done = before.done,
    ...
    fn iteratorPlanForPrivateProducerOperand(
        ...
        if (self.lookupExprBinder(expr_id)) |binder| {
            if (self.local_iter_plans.get(binder)) |plan_id| return plan_id;
        }
        const expr_ty = try self.lowerType(self.view.bodies.expr(expr_id).ty);
        if (!self.sameType(expr_ty, iterator_ty)) return null;
        return try self.privateIteratorPlanForLocalDeclaration(expr_id, iterator_ty, lowered);
    }
    ...
    fn binderOnlyUsedAsPrivateIterator(
        self: *BodyContext,
        binder: checked.PatternBinderId,
        remaining_statements: []const checked.CheckedStatementId,
        final_expr: checked.CheckedExprId,
        ...
        for (remaining_statements, 0..) |statement, index| {
            try self.scanStatementForPrivateIteratorUse(statement, remaining_statements[index + 1 ..], final_expr, &scan, true);
    ...
    fn scanStatementForPrivateIteratorUse(
        ...
        switch (statement.data) {
            .decl => |decl| {
                if (try self.scanPrivateProducerDeclarationUse(decl, remaining_statements, final_expr, scan, allow_for_iterable)) return;
                try self.scanExprForPrivateIteratorUse(decl.expr, scan, allow_for_iterable);
            },
        ...
        switch (expr.data) {
            ...
            .block => |block| {
                for (block.statements, 0..) |statement, index| {
                    try self.scanStatementForPrivateIteratorUse(statement, block.statements[index + 1 ..], block.final_expr, scan, allow_for_iterable);
                }
    ...
    fn scanPrivateProducerDeclarationUse(
        self: *BodyContext,
        decl: anytype,
        remaining_statements: []const checked.CheckedStatementId,
        final_expr: checked.CheckedExprId,
        ...
        if (!allow_for_iterable) return false;
        const produced_binder = switch (self.view.bodies.pattern(decl.pattern).data) {
            .assign => |binder| binder,
            else => return false,
        };
        if (produced_binder == scan.target) return false;

        const receiver_index = (try self.privateProducerReceiverUse(decl.expr, scan.target)) orelse return false;
        if (!try self.binderOnlyUsedAsPrivateIterator(produced_binder, remaining_statements, final_expr)) return false;

        try self.scanPrivateProducerNonReceiverOperands(decl.expr, receiver_index, scan, allow_for_iterable);
        if (scan.invalid) return true;
        scan.allowed_for_uses += 1;
        return true;
    }

    fn privateProducerReceiverUse(
        self: *BodyContext,
        expr_id: checked.CheckedExprId,
        target: checked.PatternBinderId,
        ...
        const plan_args = plan.argsSlice(self.view.static_dispatch_plans);
        if (receiver_index >= plan_args.len) {
            Common.invariant("checked private iterator producer plan had an unexpected receiver index");
        }
        const receiver_expr = switch (plan_args[receiver_index]) {
            .checked_expr => |receiver| receiver,
            else => return null,
        };
        return if (self.lookupExprBinder(receiver_expr) == target) receiver_index else null;
    }

    fn scanPrivateProducerNonReceiverOperands(
        self: *BodyContext,
        expr_id: checked.CheckedExprId,
        ...
        const expr = self.view.bodies.expr(expr_id);
        const plan = self.dispatchPlanForIteratorProducerExpr(expr) orelse return;
        for (plan.argsSlice(self.view.static_dispatch_plans), 0..) |operand, index| {
            if (index == receiver_index) continue;
            switch (operand) {
    ...
    const ListAppendIteratorPlan = struct {
        list: Ast.IterPlan.ListIter,
        item_ty: Type.TypeId,
        append_items: []const Ast.ExprId = &.{},

        fn deinit(self: ListAppendIteratorPlan, allocator: Allocator) void {
            if (self.append_items.len != 0) allocator.free(self.append_items);
        }
    };

    const ListIteratorMapFn = union(enum) {
        direct: Ast.FnId,
        value: Ast.ExprId,
    };

    const IteratorFoldFn = union(enum) {
        direct: Ast.FnId,
        value: Ast.ExprId,
    };

    const ListIteratorItemAdapter = union(enum) {
        none,
        map: struct {
    ...
    fn lowerIteratorConsumerDispatchExpr(
        ...
        if (try self.lowerIteratorFoldConsumerDispatchExpr(plan, resolved, lowered_call, ret_ty)) |fold_expr| {
            return fold_expr;
        }
    ...
    fn lowerIteratorFoldConsumerDispatchExpr(
        self: *BodyContext,
        plan: static_dispatch.StaticDispatchCallPlan,
        resolved: MethodLookup,
        lowered_call: LoweredResolvedDispatchCall,
        ...
        if (!self.methodNameIs(plan.method, "fold")) return null;

        const plan_args = plan.argsSlice(self.view.static_dispatch_plans);
        const args = self.builder.program.exprSpan(lowered_call.args);
        if (plan_args.len != 3 or args.len != 3) {
            Common.invariant("checked Iter.fold dispatch plan had an unexpected arity");
        }

        const iter_expr = args[0];
        const iter_ty = self.builder.program.exprs.items[@intFromEnum(iter_expr)].ty;
        if (!self.resolvedDispatchMatchesIteratorMethod(resolved, iter_ty, "fold")) return null;

        return try self.lowerIteratorFoldConsumerArgs(iter_expr, args[1], args[2], acc_ty);
    }

    fn lowerIteratorFoldConsumerArgs(
        self: *BodyContext,
        iter_expr: Ast.ExprId,
        acc_expr: Ast.ExprId,
        step_expr: Ast.ExprId,
        acc_ty: Type.TypeId,
    ) Allocator.Error!?Ast.ExprId {
        const iter_ty = self.builder.program.exprs.items[@intFromEnum(iter_expr)].ty;
        const item_ty = self.iterItemType(iter_ty);

        const acc_expr_ty = self.builder.program.exprs.items[@intFromEnum(acc_expr)].ty;
        if (!self.sameType(acc_expr_ty, acc_ty)) {
            Common.invariant("checked Iter.fold accumulator argument type differed from return type");
        }

        const step_expr_data = self.builder.program.exprs.items[@intFromEnum(step_expr)];
        if (!self.foldStepFunctionMatches(step_expr_data.ty, acc_ty, item_ty)) {
            Common.invariant("checked Iter.fold step function type differed from iterator item and accumulator types");
        }
        const step_fn: IteratorFoldFn = switch (step_expr_data.data) {
            .fn_def => |fn_id| .{ .direct = fn_id },
            else => .{ .value = step_expr },
        };
    ...
    fn foldStepFunctionMatches(
        self: *BodyContext,
        step_fn_ty: Type.TypeId,
        acc_ty: Type.TypeId,
        item_ty: Type.TypeId,
    ) bool {
        const step_shape = switch (self.builder.shapeContent(step_fn_ty)) {
            .func => |func| FunctionShape{ .args = func.args, .ret = func.ret },
            else => return false,
        };
        const step_args = self.builder.program.types.span(step_shape.args);
        if (step_args.len != 2) return false;
        return self.sameType(step_args[0], acc_ty) and
            self.sameType(step_args[1], item_ty) and
            self.sameType(step_shape.ret, acc_ty);
    }
    ...
    fn lowerAppendIteratorProducerPlanExpr(
        self: *BodyContext,
        plan: static_dispatch.StaticDispatchCallPlan,
        dispatcher_ty: Type.TypeId,
        lowered_call: LoweredResolvedDispatchCall,
        ...
        const lowered_args = self.builder.program.exprSpan(lowered_call.args);
        if (lowered_args.len != plan_args.len) {
            Common.invariant("lowered Iter.append call argument count differed from its dispatch plan");
        }

        const before_plan = try self.iteratorPlanForLoweredPublicExpr(lowered_args[receiver_index], dispatcher_ty);
        const before = self.builder.program.iterPlan(before_plan);
        const after_expr = lowered_args[after_index];
        const after_ty = self.builder.program.exprs.items[@intFromEnum(after_expr)].ty;
        if (!self.sameType(after_ty, self.iterItemType(dispatcher_ty))) {
            Common.invariant("checked Iter.append item type differed from its iterator item type");
        }
        const u64_ty = try self.builder.primitiveType(.u64);
        const bool_ty = try self.builder.primitiveType(.bool);
        const phase_expr = try self.boolLiteral(false, bool_ty);
    ...
    fn lowerIteratorDirectConsumerCallExpr(
        self: *BodyContext,
        call: anytype,
        lowered: LoweredCall,
    ) Allocator.Error!?Ast.ExprId {
        ...
        if (lowered_args.len == 0) return null;
        ...
        return try self.lowerIteratorFoldConsumerArgs(iter_expr, lowered_args[1], lowered_args[2], lowered.ret_ty);
    }
    ...
    fn iteratorPlanForLoweredPublicExpr(
        self: *BodyContext,
        iter_expr: Ast.ExprId,
        iter_ty: Type.TypeId,
    ) Allocator.Error!Ast.IterPlanId {
        const expr = self.builder.program.exprs.items[@intFromEnum(iter_expr)];
        if (!self.sameType(expr.ty, iter_ty)) {
            Common.invariant("lowered iterator producer operand had a type different from its dispatch receiver type");
        }
        switch (expr.data) {
            .iter_plan => |plan_id| return plan_id,
            else => {},
        }

        return try self.builder.program.addIterPlan(.{
            .item_ty = self.iterItemType(iter_ty),
            .length = .unknown,
            .steps = .{
                .append = true,
                .one = true,
            ...
            .done = .reachable,
            .materialized = iter_expr,
            .data = .{ .public = .{
                .iter_value = iter_expr,
            } },
        });
    }
    ...
    fn lowerIteratorFor(
        ...
        if (try self.appendListIteratorPlanForPlanValue(for_, result_ty, carries, plan)) |append_for| {
            return append_for;
        }
        ...
        if (try self.filterListIteratorPlanForPlanValue(for_, result_ty, carries, plan)) |filter_for| {
            return filter_for;
        }
    ...
    fn listIteratorPlanForPlanValue(
        ...
        return try self.lowerListIteratorPlanFor(for_, result_ty, carries, list, iter_plan_value.item_ty, iter_plan_value.item_ty, &.{}, .none);
    }
    ...
    fn lowerListIteratorPlanFor(
        ...
        append_items: []const Ast.ExprId,
        adapter: ListIteratorItemAdapter,
    ) Allocator.Error!Ast.ExprData {
        const u64_ty = try self.builder.primitiveType(.u64);
        const bool_ty = try self.builder.primitiveType(.bool);
        ...
        const append_locals = try self.allocator.alloc(Ast.LocalId, append_items.len);
        defer self.allocator.free(append_locals);
        const append_exprs = try self.allocator.alloc(Ast.ExprId, append_items.len);
        defer self.allocator.free(append_exprs);
        for (append_items, 0..) |_, index| {
            append_locals[index] = try self.builder.program.addLocal(self.builder.symbols.fresh(), source_item_ty);
            append_exprs[index] = try self.builder.localExpr(append_locals[index], source_item_ty);
        }
        ...
        const limit_value = if (append_items.len == 0)
            len_expr
        else
            try self.builder.lowLevelExpr(
                .num_plus,
                &.{ len_expr, try self.builder.intLiteralExpr(append_items.len, u64_ty) },
                u64_ty,
            );
        ...
        const continue_body = try self.lowerListIteratorContinue(
            for_,
            result_ty,
            list_expr,
            len_expr,
        ...
        while (append_index > 0) {
            append_index -= 1;
            rest = try self.wrapLet(append_locals[append_index], source_item_ty, append_items[append_index], rest, result_ty);
        }
    ...
    fn appendListIteratorPlanForPlanValue(
        self: *BodyContext,
        for_: anytype,
        result_ty: Type.TypeId,
        carries: []const LoopCarry,
        ...
        const iterator_ty = try self.lowerType(plan.iterator_ty);
        if (!try self.checkedExprCanProduceAppendPlan(for_.expr, iterator_ty)) return null;

        const iterable = try self.lowerExprAtType(for_.expr, iterator_ty);
        const plan_id = switch (self.builder.program.exprs.items[@intFromEnum(iterable)].data) {
            .iter_plan => |lowered_plan_id| lowered_plan_id,
            else => Common.invariant("checked Iter.append expression did not lower to an iterator plan"),
        };
        const append_plan = (try self.listAppendIteratorPlanFromPlan(plan_id)) orelse return null;
        defer append_plan.deinit(self.allocator);

        return try self.lowerListIteratorPlanFor(for_, result_ty, carries, append_plan.list, append_plan.item_ty, append_plan.item_ty, append_plan.append_items, .none);
    }

    fn checkedExprCanProduceAppendPlan(
        self: *BodyContext,
        expr_id: checked.CheckedExprId,
        iterator_ty: Type.TypeId,
    ) Allocator.Error!bool {
        ...
        const producer = self.dispatchPlanForIteratorProducerExpr(expr) orelse return false;
        if (!self.methodNameIs(producer.method, "append")) return false;
    ...
    fn listAppendIteratorPlanFromPlan(
        self: *BodyContext,
        plan_id: Ast.IterPlanId,
    ) Allocator.Error!?ListAppendIteratorPlan {
        const plan = self.builder.program.iterPlan(plan_id);
        switch (plan.data) {
            .list => |list| return .{
                .list = list,
            ...
            .append => |append| {
                var before = (try self.listAppendIteratorPlanFromPlan(append.before)) orelse return null;
                errdefer before.deinit(self.allocator);
                const append_items = try self.allocator.alloc(Ast.ExprId, before.append_items.len + 1);
                @memcpy(append_items[0..before.append_items.len], before.append_items);
                append_items[before.append_items.len] = append.after;
                before.deinit(self.allocator);
                before.append_items = append_items;
                return before;
            },
            else => return null,
        }
    ...
    fn lowerListAppendIteratorPlanFold(
        ...
        const step_fn_local: ?Ast.LocalId = switch (step_fn) {
            .direct => null,
        ...
        const loop_step_fn: IteratorFoldFn = switch (step_fn) {
            ...
            .value => .{ .value = try self.builder.localExpr(step_fn_local orelse Common.invariant("Iter.fold step local was missing"), step_fn_ty) },
        };
        ...
        const continue_body = try self.lowerListAppendIteratorPlanFoldContinue(
            source_list_expr,
            len_expr,
            index_expr,
            next_index,
    ...
    fn lowerMapSourceIteratorPlanFold(
        ...
        switch (source.data) {
            ...
            .concat => |concat| return try self.lowerConcatListIteratorPlanFold(concat, source.item_ty, initial_acc, step_fn, step_fn_ty, acc_ty, result_item_ty, adapter),
            else => return null,
        }
    }
    ...
    fn lowerConcatListIteratorPlanFold(
        ...
        if (!self.sameType(first.item_ty, source_item_ty) or !self.sameType(second.item_ty, source_item_ty)) {
            Common.invariant("Iter.concat fold child item type differed from concat item type");
        }
        ...
        defer self.allocator.free(first_append_exprs);
        for (first.append_items, 0..) |_, index| {
            first_append_locals[index] = try self.builder.program.addLocal(self.builder.symbols.fresh(), source_item_ty);
            first_append_exprs[index] = try self.builder.localExpr(first_append_locals[index], source_item_ty);
        }
        ...
        defer self.allocator.free(second_append_exprs);
        for (second.append_items, 0..) |_, index| {
            second_append_locals[index] = try self.builder.program.addLocal(self.builder.symbols.fresh(), source_item_ty);
            second_append_exprs[index] = try self.builder.localExpr(second_append_locals[index], source_item_ty);
        }
        ...
        const first_done = try self.builder.lowLevelExpr(.num_is_eq, &.{ index_expr, first_limit_expr }, bool_ty);
        const second_done = try self.builder.lowLevelExpr(.num_is_eq, &.{ index_expr, second_limit_expr }, bool_ty);
        const switch_to_second = try self.builder.program.addExpr(.{
            .ty = acc_ty,
            .data = .{ .continue_ = .{ .values = try self.builder.program.addExprSpan(&[_]Ast.ExprId{ true_expr, second.list.index, acc_expr }) } },
        });
        const done_body = try self.builder.program.addExpr(.{
            .ty = acc_ty,
            .data = .{ .break_ = acc_expr },
        });
        const first_continue = try self.lowerListAppendIteratorPlanFoldContinueWithPrefix(
            first_list_expr,
            first_len_expr,
            index_expr,
            acc_expr,
        ...
        const second_continue = try self.lowerListAppendIteratorPlanFoldContinueWithPrefix(
            second_list_expr,
            second_len_expr,
            index_expr,
            acc_expr,
        ...
        const first_body = try self.builder.ifExpr(first_done, switch_to_second, first_continue, acc_ty);
        const second_body = try self.builder.ifExpr(second_done, done_body, second_continue, acc_ty);
        const body = try self.builder.ifExpr(phase_expr, second_body, first_body, acc_ty);

        const params = [_]Ast.TypedLocal{
            .{ .local = phase_local, .ty = bool_ty },
            .{ .local = index_local, .ty = u64_ty },
            .{ .local = acc_local, .ty = acc_ty },
        };
        const initial_values = [_]Ast.ExprId{
        ...
        while (second_append_index > 0) {
            second_append_index -= 1;
            rest = try self.wrapLet(second_append_locals[second_append_index], source_item_ty, second.append_items[second_append_index], rest, acc_ty);
        }
        rest = try self.wrapLet(second_limit_local, u64_ty, second_limit_value, rest, acc_ty);
        rest = try self.wrapLet(second_len_local, u64_ty, second_len_value, rest, acc_ty);
        rest = try self.wrapLet(second_list_local, second_list_ty, second.list.list, rest, acc_ty);

        var first_append_index = first_append_locals.len;
        while (first_append_index > 0) {
            first_append_index -= 1;
            rest = try self.wrapLet(first_append_locals[first_append_index], source_item_ty, first.append_items[first_append_index], rest, acc_ty);
        }
        rest = try self.wrapLet(first_limit_local, u64_ty, first_limit_value, rest, acc_ty);
        rest = try self.wrapLet(first_len_local, u64_ty, first_len_value, rest, acc_ty);
        return try self.wrapLet(first_list_local, first_list_ty, first.list.list, rest, acc_ty);
    }
    ...
    fn lowerRangeIteratorPlanFold(
        ...
        const range_step_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), item_ty);
        const range_step_expr = try self.builder.localExpr(range_step_local, item_ty);
        ...
        const next_state = try self.rangeNextState(current_expr, end_expr, range_step_expr, range.inclusivity, item_ty);
        ...
        const params = [_]Ast.TypedLocal{
            .{ .local = current_local, .ty = item_ty },
            .{ .local = done_local, .ty = bool_ty },
            .{ .local = acc_local, .ty = acc_ty },
        };
        const initial_values = [_]Ast.ExprId{ start_expr, initial_done, initial_acc_expr };
        const loop_expr = try self.builder.program.addExpr(.{ .ty = acc_ty, .data = .{ .loop_ = .{
            .params = try self.builder.program.addTypedLocalSpan(&params),
            .initial_values = try self.builder.program.addExprSpan(&initial_values),
            .body = body,
        ...
        rest = try self.wrapLet(initial_acc_local, acc_ty, initial_acc, rest, acc_ty);
        rest = try self.wrapLet(range_step_local, item_ty, range.step, rest, acc_ty);
        rest = try self.wrapLet(end_local, item_ty, range.end, rest, acc_ty);
    ...
    fn lowerListAppendIteratorPlanFoldContinueWithPrefix(
        self: *BodyContext,
        source_list_expr: Ast.ExprId,
        len_expr: Ast.ExprId,
        index_expr: Ast.ExprId,
        ...
        const list_item = try self.builder.lowLevelExpr(.list_get_unsafe, &.{ source_list_expr, index_expr }, source_item_ty);
        const list_continue = try self.foldContinue(acc_expr, list_item, source_item_ty, body_item_ty, step_fn, step_fn_ty, acc_ty, adapter, prefix_values);
        if (append_exprs.len == 0) return list_continue;
    ...
    fn foldStepExpr(
        ...
        const next_acc_data: Ast.ExprData = switch (step_fn) {
            .direct => |fn_id| .{ .call_proc = .{
                .callee = .{ .func = fn_id },
                .args = try self.builder.program.addExprSpan(&[_]Ast.ExprId{ acc_expr, item_expr }),
            } },
            .value => |step_expr| .{ .call_value = .{
                .callee = step_expr,
                .args = try self.builder.program.addExprSpan(&[_]Ast.ExprId{ acc_expr, item_expr }),
            } },
        };
    ...
    fn mapListIteratorPlanForPlanValue(
        ...
        const iterator_ty = try self.lowerType(plan.iterator_ty);
        if (!try self.checkedExprCanProduceIteratorAdapterPlan(for_.expr, iterator_ty, "map")) {
            return null;
        }

        const plan_id = (try self.mapIteratorPlanForPrivateConsumer(for_.expr, iterator_ty)) orelse
            Common.invariant("checked Iter.map expression did not lower to an iterator plan");
        const iter_plan_value = self.builder.program.iterPlan(plan_id);
        const map = switch (iter_plan_value.data) {
            .map => |map| map,
        ...
        const mapping_fn_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), mapping_fn_ty);
        const mapping_fn_expr = try self.builder.localExpr(mapping_fn_local, mapping_fn_ty);
        const loop = try self.builder.program.addExpr(.{
            .ty = result_ty,
            .data = try self.lowerListIteratorPlanFor(
                for_,
                result_ty,
                carries,
                source.list,
        ...
        return .{ .let_ = .{
            .bind = try self.builder.bindPat(mapping_fn_local, mapping_fn_ty),
            .value = map.mapping_fn,
            .rest = loop,
        } };
    }

    fn mapIteratorPlanForPrivateConsumer(
        self: *BodyContext,
        expr_id: checked.CheckedExprId,
        iterator_ty: Type.TypeId,
    ) Allocator.Error!?Ast.IterPlanId {
        ...
        const lowered_args = self.builder.program.exprSpan(try self.lowerDispatchOperandsAtTypes(plan_args, plan_arg_tys, null));
        const source_plan = try self.iteratorPlanForLoweredPublicExpr(lowered_args[receiver_index], dispatcher_ty);
        const source = self.builder.program.iterPlan(source_plan);
        const mapping_fn = lowered_args[mapping_index];
    ...
    fn filterListIteratorPlanForPlanValue(
        self: *BodyContext,
        for_: anytype,
        result_ty: Type.TypeId,
        carries: []const LoopCarry,
        ...
        const iterator_ty = try self.lowerType(plan.iterator_ty);
        if (!try self.checkedExprCanProduceIteratorAdapterPlan(for_.expr, iterator_ty, "keep_if") and
            !try self.checkedExprCanProduceIteratorAdapterPlan(for_.expr, iterator_ty, "drop_if"))
        {
            return null;
        }
        ...
        const plan_id = switch (self.builder.program.exprs.items[@intFromEnum(iterable)].data) {
            .iter_plan => |lowered_plan_id| lowered_plan_id,
            else => Common.invariant("checked Iter filter expression did not lower to an iterator plan"),
        };
        const iter_plan_value = self.builder.program.iterPlan(plan_id);
        const filter = switch (iter_plan_value.data) {
        ...
        const predicate_fn_ty = self.builder.program.exprs.items[@intFromEnum(filter.predicate_fn)].ty;
        ...
        return .{ .let_ = .{
            .bind = try self.builder.bindPat(predicate_fn_local, predicate_fn_ty),
            .value = filter.predicate_fn,
            .rest = loop,
        } };
    }

    fn checkedExprCanProduceIteratorAdapterPlan(
        self: *BodyContext,
        expr_id: checked.CheckedExprId,
        iterator_ty: Type.TypeId,
        comptime method_name: []const u8,
        ...
        const producer = self.dispatchPlanForIteratorProducerExpr(expr) orelse return false;
        if (!self.methodNameIs(producer.method, method_name)) return false;
        return try self.dispatchPlanOwnerMatchesResult(producer, iterator_ty);
    }
    ...
    fn lowerListIteratorContinue(
        ...
        const list_item = try self.builder.lowLevelExpr(.list_get_unsafe, &.{ list_expr, index_expr }, source_item_ty);
        const list_body = try self.lowerListIteratorAdaptedItemThenContinue(for_, result_ty, list_item, source_item_ty, body_item_ty, next_index, carries, adapter);
    ...
    fn lowerListIteratorAdaptedItemThenContinue(
        self: *BodyContext,
        for_: anytype,
        result_ty: Type.TypeId,
        source_item: Ast.ExprId,
        ...
        switch (adapter) {
            .none => {
                if (!self.sameType(source_item_ty, body_item_ty)) {
                    Common.invariant("unadapted list iterator item type differed from loop body item type");
                }
                return try self.lowerListIteratorBodyThenContinue(for_, result_ty, source_item, source_item_ty, next_index, carries);
            },
            .map => |map| {
                const mapping_shape = self.builder.functionShape(map.mapping_fn_ty, "Iter.map mapping function had a non-function type");
                const mapping_args = self.builder.program.types.span(mapping_shape.args);
                ...
                if (!self.sameType(map.result_ty, body_item_ty)) {
                    Common.invariant("Iter.map result type differed from loop body item type");
                }
                const mapped_data: Ast.ExprData = switch (map.mapping_fn) {
                    .direct => |fn_id| .{ .call_proc = .{
                        .callee = .{ .func = fn_id },
                ...
                const mapped = try self.builder.program.addExpr(.{
                    .ty = map.result_ty,
                    .data = mapped_data,
                });
                return try self.lowerListIteratorBodyThenContinue(for_, result_ty, mapped, map.result_ty, next_index, carries);
            },
            .filter => |filter| {
                if (!self.sameType(source_item_ty, body_item_ty)) {
                    Common.invariant("Iter filter item type differed from loop body item type");
                }
                const item_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), source_item_ty);
                const predicate_fn_ty = self.builder.program.exprs.items[@intFromEnum(filter.predicate_fn)].ty;
                ...
                if (!self.sameType(predicate_args[0], source_item_ty) or !self.typeHasBuiltinOwner(predicate_shape.ret, .bool)) {
                    Common.invariant("Iter filter predicate type differed from iterator plan types");
                }
                const passes = try self.builder.program.addExpr(.{
                    .ty = predicate_shape.ret,
                    .data = .{ .call_value = .{
                        .callee = filter.predicate_fn,
                        .args = try self.builder.program.addExprSpan(&[_]Ast.ExprId{try self.builder.localExpr(item_local, source_item_ty)}),
                    } },
                });
                const body = try self.lowerListIteratorBodyThenContinue(
                    for_,
                    result_ty,
                    try self.builder.localExpr(item_local, source_item_ty),
                    source_item_ty,
                    next_index,
                    carries,
                );
                const skip = try self.continueWithPrefixAndCarries(result_ty, &[_]Ast.ExprId{next_index}, carries);
                const branch = switch (filter.kind) {
                    .keep_if => try self.builder.ifExpr(passes, body, skip, result_ty),
                    .drop_if => try self.builder.ifExpr(passes, skip, body, result_ty),
                };
                return try self.wrapLet(item_local, source_item_ty, source_item, branch, result_ty);
            },
        }
    }
