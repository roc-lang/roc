...
const Builder = struct {
            ...
            .nominal => |nominal| blk: {
                switch (nominal.representation) {
                    .builtin => |builtin| switch (checked.builtinRuntimeEncoding(builtin)) {
                        ...
                        => {},
                        .parse_record_state => {
                            if (nominal.args.len != 2) Common.invariant("checked ParseRecordState nominal must have exactly two type arguments");
                            const shape_ty = try self.lowerType(view, nominal.args[0]);
                            ...
                            const slots = try self.allocator.alloc(Type.TypeId, fields.len);
                            defer self.allocator.free(slots);
                            if (fields.len > 0) {
                                const fmt_ty = try self.lowerType(view, nominal.args[1]);
                                const state_slot_ty = try self.parseRecordStateSlotType(fmt_ty);
                                @memset(slots, state_slot_ty);
                            }
                            break :blk .{ .tuple = try self.program.types.addSpan(slots) };
                        },
                    },
                    else => {},
                }
    ...
    fn parseRecordStateSlotType(self: *Builder, fmt_ty: Type.TypeId) Allocator.Error!Type.TypeId {
        const missing_name = try self.program.names.internTagLabel("Missing");
...
const BodyContext = struct {
    ...
    fn instParseRecordStateSlotNode(self: *BodyContext, fmt_node: NodeId) Allocator.Error!NodeId {
        const missing_name = try self.builder.program.names.internTagLabel("Missing");
        const present_name = try self.builder.program.names.internTagLabel("Present");
        const present_payloads = try self.graph.arena().alloc(NodeId, 1);
        present_payloads[0] = fmt_node;
    ...
    fn instParseRecordStateFields(self: *BodyContext, shape_node: NodeId) []const InstField {
        var current = shape_node;
        while (true) {
            switch (self.graph.content(current)) {
                .record => |record| return record.fields,
                .empty_record => return &.{},
                .named => |named| {
                    const backing = named.backing orelse Common.invariant("checked ParseRecordState shape argument was an opaque named type");
    ...
    fn instNominalNode(
        ...
        switch (nominal.representation) {
            .builtin => |builtin| switch (checked.builtinRuntimeEncoding(builtin)) {
                ...
                .parse_record_state => {
                    if (nominal.args.len != 2) Common.invariant("checked ParseRecordState nominal must have exactly two type arguments");
                    const shape_node = try self.instNode(nominal.args[0]);
                    const fields = self.instParseRecordStateFields(shape_node);
                    const slots = try self.graph.arena().alloc(NodeId, fields.len);
                    if (fields.len > 0) {
                        const fmt_node = try self.instNode(nominal.args[1]);
                        const state_slot_node = try self.instParseRecordStateSlotNode(fmt_node);
                        @memset(slots, state_slot_node);
                    }
                    return try self.graph.newNode(.{ .tuple = slots });
                },
            },
            else => {},
        }
    ...
    fn lowerCallExpr(self: *BodyContext, checked_ret_ty: checked.CheckedTypeId, call: anytype) Allocator.Error!Ast.ExprId {
        if (try self.lowerDecoderIntrinsicCallExpr(checked_ret_ty, call, null)) |expr| return expr;
    ...
    fn lowerDecoderIntrinsicCallExpr(
        ...
        const intrinsic = self.decoderIntrinsicForResolvedTarget(target) orelse return null;
        ...
        const ret_ty = try self.decoderIntrinsicReturnType(intrinsic, checked_ret_ty, call.args, arg_tys, expected_ret_ty);
        if (expected_ret_ty) |expected| {
            if (!self.sameType(expected, ret_ty)) Common.invariant("checked decoder intrinsic lowered at a type different from its context type");
        }
        ...
        return switch (intrinsic) {
            ...
            .record_finish => try self.lowerDecoderRecordFinish(call.args, arg_tys, ret_ty),
            .tag_union_parse => try self.lowerDecoderTagUnionDecode(call.args, arg_tys, ret_ty),
        };
    }
    ...
    fn decoderRecordInitSlotType(
        ...
        if (function.args.len != 2 or call.args.len != 2) Common.invariant("ParseRecordSpec.init source arity differed from call arity");
    ...
    fn decoderIntrinsicReturnType(
        ...
        const derived = switch (intrinsic) {
            .record_init => blk: {
                if (args.len != 2 or arg_tys.len != 2) Common.invariant("ParseRecordSpec.init reached Monotype with an unexpected arity");
                if (expected_ret_ty) |expected| break :blk expected;
                break :blk try self.lowerType(checked_ret_ty);
            },
            .record_put => blk: {
                if (args.len != 5 or arg_tys.len != 5) Common.invariant("ParseRecordSpec.put reached Monotype with an unexpected arity");
                break :blk arg_tys[1];
            },
            .record_finish => blk: {
                if (args.len != 3 or arg_tys.len != 3) Common.invariant("ParseRecordSpec.finish reached Monotype with an unexpected arity");
                if (expected_ret_ty) |expected| break :blk expected;
                break :blk try self.lowerType(checked_ret_ty);
            },
            .tag_union_parse => blk: {
                if (args.len != 5 or arg_tys.len != 5) Common.invariant("ParseTagUnionSpec.parse reached Monotype with an unexpected arity");
                if (expected_ret_ty) |expected| break :blk expected;
                break :blk try self.lowerType(checked_ret_ty);
            },
        };
        ...
        if (expected_ret_ty) |expected| {
            if (!self.sameType(expected, derived)) Common.invariant("checked decoder intrinsic result differed from expected result type");
            return expected;
        }
        return derived;
    }

    fn decoderIntrinsicArgumentMonoType(
        self: *BodyContext,
        ...
        return (try self.callArgumentMonoType(checked_arg, null)) orelse Common.invariant("checked decoder intrinsic argument did not lower to a monotype");
    }

    fn lowerDecoderIntrinsicArgAtType(
        self: *BodyContext,
        checked_arg: checked.CheckedExprId,
        ty: Type.TypeId,
    ) Allocator.Error!Ast.ExprId {
        ...
        if (maybe_ref) |ref_id| {
            if (self.currentLocalForResolvedValue(ref_id)) |local_id| {
                ...
                if (!self.sameType(ty, local_ty)) {
                    Common.invariant("checked decoder intrinsic local argument type differed from its concrete local type");
                }
        ...
        return switch (proc.binding) {
            ...
            .hosted => |hosted| blk: {
                const source_fn_ty = proc.source_fn_ty_payload orelse
                    Common.invariant("hosted procedure value reached decoder intrinsic callback lowering without source type");
            ...
            .platform_required => Common.invariant("platform required procedure reached decoder intrinsic callback lowering"),
        };
    }

    fn decoderIntrinsicForResolvedTarget(self: *BodyContext, target: checked.ResolvedValueId) ?DecoderIntrinsic {
    ...
    fn decoderIntrinsicForBuiltinDef(_: *BodyContext, view: ModuleView, def_idx: can.CIR.Def.Idx) ?DecoderIntrinsic {
        ...
        if (Ident.textEql(text, "Builtin.ParseRecordSpec.init")) return .record_init;
        if (Ident.textEql(text, "Builtin.ParseRecordSpec.put")) return .record_put;
        if (Ident.textEql(text, "Builtin.ParseRecordSpec.finish")) return .record_finish;
        if (Ident.textEql(text, "Builtin.ParseTagUnionSpec.parse")) return .tag_union_parse;
        return null;
    }

    fn lowerParseShapeFromSlot(
        self: *BodyContext,
        shape_ty: Type.TypeId,
        ...
        const method_name: []const u8 = if (Ident.textEql(selected.tag_text, "Str"))
            ...
            Common.invariant("parse_from selected an unsupported shape");

        const parse_lookup = self.methodLookupForTypeName(slot_ty, method_name);
        const slot_arg_index: usize = if (Ident.textEql(selected.tag_text, "Str")) 0 else 1;
        const parse_mono_ty = try self.methodTargetMonoTypeFromArgAtIndexAndRet(parse_lookup, slot_arg_index, slot_ty, ret_ty);
        const parse_fn = self.builder.functionShape(parse_mono_ty, "parse_from target method was not a function");
        const parse_arg_tys = self.builder.program.types.span(parse_fn.args);
        if (Ident.textEql(selected.tag_text, "Str")) {
            if (parse_arg_tys.len != 1) Common.invariant("parse_from target Str method had an unexpected arity");
            if (!self.sameType(parse_arg_tys[0], slot_ty)) Common.invariant("parse_from target Str slot type differed from input slot type");

            const parse_args = [_]Ast.ExprId{slot_expr};
            return try self.builder.program.addExpr(.{
                .ty = ret_ty,
                .data = .{ .call_proc = .{
        ...
        if (parse_arg_tys.len != 2) Common.invariant("parse_from target structural method had an unexpected arity");
        if (!self.sameType(parse_arg_tys[1], slot_ty)) Common.invariant("parse_from target structural slot type differed from input slot type");

        const spec_expr = try self.builder.program.addExpr(.{
            .ty = parse_arg_tys[0],
            .data = .unit,
        });
        const parse_args = [_]Ast.ExprId{ spec_expr, slot_expr };
        return try self.builder.program.addExpr(.{
            .ty = ret_ty,
            .data = .{ .call_proc = .{
                .callee = .{ .func = try self.methodTargetCalleeWithMono(parse_lookup, parse_mono_ty) },
    ...
    fn lowerParseValueFromSlot(
        ...
        if (!self.sameType(ret_info.ok_ty, shape_ty)) Common.invariant("parse value wrapper Ok type differed from parsed shape");
    ...
    fn lowerDecoderRecordInit(
        ...
        const witness_value = try self.lowerDecoderIntrinsicArgAtType(args[1], arg_tys[1]);

        const items = try self.allocator.alloc(Ast.ExprId, item_tys.len);
        defer self.allocator.free(items);
        for (item_tys, 0..) |item_ty, index| {
            const slot_info = self.parseRecordStateSlotInfo(item_ty);
            if (!self.sameType(slot_info.present_payload_ty, arg_tys[1])) Common.invariant("ParseRecordState slot type did not match init format witness type");
        ...
        const with_witness = try self.wrapWildcardLet(arg_tys[1], witness_value, body, ret_ty);
        return try self.wrapWildcardLet(arg_tys[0], spec_value, with_witness, ret_ty);
    }

    fn lowerDecoderRecordPut(
        self: *BodyContext,
        args: []const checked.CheckedExprId,
        arg_tys: []const Type.TypeId,
        ret_ty: Type.TypeId,
        ...
        const item_tys = try self.allocator.dupe(Type.TypeId, self.builder.tupleItemTypes(ret_ty));
        defer self.allocator.free(item_tys);
        if (fields.len != item_tys.len) Common.invariant("ParseRecordState arity did not match derived record field count");

        const spec_value = try self.lowerDecoderIntrinsicArgAtType(args[0], arg_tys[0]);
        const state_value = try self.lowerDecoderIntrinsicArgAtType(args[1], arg_tys[1]);
        const key_value = try self.lowerDecoderIntrinsicArgAtType(args[2], arg_tys[2]);
        const slot_value = try self.lowerDecoderIntrinsicArgAtType(args[3], arg_tys[3]);
        const matcher_value = try self.lowerDecoderIntrinsicArgAtType(args[4], arg_tys[4]);

        const state_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), arg_tys[1]);
        const key_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), arg_tys[2]);
        const slot_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), arg_tys[3]);
        const matcher_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), arg_tys[4]);

        var body = try self.builder.localExpr(state_local, ret_ty);
        var index = fields.len;
        while (index > 0) {
            index -= 1;
            const updated = try self.decoderStateWithSlot(state_local, ret_ty, item_tys, slot_local, arg_tys[3], index);
            const cond = try self.decoderFieldMatchCall(key_local, arg_tys[2], matcher_local, arg_tys[4], fields[index]);
            body = try self.builder.ifExpr(cond, updated, body, ret_ty);
        }

        body = try self.wrapLet(matcher_local, arg_tys[4], matcher_value, body, ret_ty);
        body = try self.wrapLet(slot_local, arg_tys[3], slot_value, body, ret_ty);
        body = try self.wrapLet(key_local, arg_tys[2], key_value, body, ret_ty);
        body = try self.wrapLet(state_local, arg_tys[1], state_value, body, ret_ty);
        return try self.wrapWildcardLet(arg_tys[0], spec_value, body, ret_ty);
    }

    fn lowerDecoderRecordFinish(
        self: *BodyContext,
        args: []const checked.CheckedExprId,
        arg_tys: []const Type.TypeId,
        ret_ty: Type.TypeId,
        ...
        if (record_fields.len != state_items.len) Common.invariant("ParseRecordState arity did not match finish record field count");

        const spec_value = try self.lowerDecoderIntrinsicArgAtType(args[0], arg_tys[0]);
        const state_value = try self.lowerDecoderIntrinsicArgAtType(args[1], arg_tys[1]);
        const missing_value = try self.lowerDecoderIntrinsicArgAtType(args[2], arg_tys[2]);
        const state_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), arg_tys[1]);
        const missing_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), arg_tys[2]);
        ...
        for (record_fields, 0..) |field, index| {
            const slot_expr = try self.builder.program.addExpr(.{
                ...
                .data = .{ .tuple_access = .{
                    .tuple = try self.builder.localExpr(state_local, arg_tys[1]),
            ...
            field_tries[index] = try self.parseRecordFieldFromStateSlot(slot_expr, state_items[index], field.ty, field_try_ty, missing_local, arg_tys[2]);
            out_fields[index] = .{
                .name = field.name,
        ...
        body = try self.wrapLet(state_local, arg_tys[1], state_value, body, ret_ty);
        body = try self.wrapLet(missing_local, arg_tys[2], missing_value, body, ret_ty);
        return try self.wrapWildcardLet(arg_tys[0], spec_value, body, ret_ty);
    }

    fn decoderRecordFieldsFromSpecExpr(self: *BodyContext, spec_expr: checked.CheckedExprId) Allocator.Error![]const Type.Field {
    ...
    fn checkedParseRecordSpecShape(self: *BodyContext, spec_expr: checked.CheckedExprId) checked.CheckedTypeId {
        ...
        return switch (resolvedPayload(self.view, expr.ty).payload) {
            .nominal => |nominal| blk: {
                if ((nominal.builtin orelse Common.invariant("ParseRecordSpec intrinsic spec argument was not a builtin nominal")) != .parse_record_spec) {
                    Common.invariant("ParseRecordSpec intrinsic spec argument was not ParseRecordSpec");
                }
                if (nominal.args.len != 1) Common.invariant("ParseRecordSpec had an unexpected type argument count");
    ...
    fn lowerDecoderTagUnionDecode(
        ...
        const spec_value = try self.lowerDecoderIntrinsicArgAtType(args[0], arg_tys[0]);
        const key_value = try self.lowerDecoderIntrinsicArgAtType(args[1], arg_tys[1]);
        const slot_value = try self.lowerDecoderIntrinsicArgAtType(args[2], arg_tys[2]);
        const matcher_value = try self.lowerDecoderIntrinsicArgAtType(args[3], arg_tys[3]);
        const missing_value = try self.lowerDecoderIntrinsicArgAtType(args[4], arg_tys[4]);
        ...
        const missing_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), arg_tys[4]);
        const tags = try self.allocator.dupe(Type.Tag, self.builder.program.types.tagSpan(tag_span));
        defer self.allocator.free(tags);

        var body = try self.unknownDecodedTagErr(ret_ty, missing_local, arg_tys[4]);
        var index = tags.len;
        while (index > 0) {
            index -= 1;
            const tag_expr = try self.decodedTagUnionValue(tags[index], ret_info.ok_ty, ret_ty, slot_local, arg_tys[2]);
            const cond = try self.decoderTagMatchCall(key_local, arg_tys[1], matcher_local, arg_tys[3], tags[index]);
            body = try self.builder.ifExpr(cond, tag_expr, body, ret_ty);
        }

        body = try self.wrapLet(missing_local, arg_tys[4], missing_value, body, ret_ty);
        body = try self.wrapLet(matcher_local, arg_tys[3], matcher_value, body, ret_ty);
        body = try self.wrapLet(slot_local, arg_tys[2], slot_value, body, ret_ty);
        body = try self.wrapLet(key_local, arg_tys[1], key_value, body, ret_ty);
        return try self.wrapWildcardLet(arg_tys[0], spec_value, body, ret_ty);
    }

    fn decoderStateWithSlot(
        self: *BodyContext,
        state_local: Ast.LocalId,
        state_ty: Type.TypeId,
        item_tys: []const Type.TypeId,
        fmt_local: Ast.LocalId,
        fmt_ty: Type.TypeId,
        replace_index: usize,
    ) Allocator.Error!Ast.ExprId {
        const items = try self.allocator.alloc(Ast.ExprId, item_tys.len);
        ...
        for (item_tys, 0..) |item_ty, index| {
            items[index] = if (index == replace_index)
                try self.parseRecordStatePresentExpr(fmt_local, fmt_ty, item_ty)
            else
    ...
    fn decoderTagMatchCall(
        self: *BodyContext,
        key_local: Ast.LocalId,
        key_ty: Type.TypeId,
        matcher_local: Ast.LocalId,
        matcher_ty: Type.TypeId,
        tag: Type.Tag,
    ) Allocator.Error!Ast.ExprId {
        const matcher_args = self.builder.program.types.span(self.builder.functionShape(matcher_ty, "ParseTagUnionSpec.parse matcher was not a function").args);
        if (matcher_args.len != 2) Common.invariant("ParseTagUnionSpec.parse matcher had an unexpected arity");
        if (!self.sameType(matcher_args[0], key_ty)) Common.invariant("ParseTagUnionSpec.parse matcher key type differed from provided key type");
        const tag_text = self.builder.program.names.tagLabelText(tag.name);
        const tag_expr = try self.builder.stringExpr(tag_text, matcher_args[1]);
        const key_expr = try self.builder.localExpr(key_local, key_ty);
        const matcher_expr = try self.builder.localExpr(matcher_local, matcher_ty);
        const call_args = [_]Ast.ExprId{ key_expr, tag_expr };
        return try self.builder.program.addExpr(.{
            .ty = self.builder.functionShape(matcher_ty, "ParseTagUnionSpec.parse matcher was not a function").ret,
            .data = .{ .call_value = .{
                .callee = matcher_expr,
                .args = try self.builder.program.addExprSpan(&call_args),
            } },
        });
    }

    fn decoderFieldMatchCall(
        self: *BodyContext,
        key_local: Ast.LocalId,
        key_ty: Type.TypeId,
        matcher_local: Ast.LocalId,
        ...
        const matcher_fn = self.builder.functionShape(matcher_ty, "ParseRecordSpec.put matcher was not a function");
        const matcher_args = self.builder.program.types.span(matcher_fn.args);
        if (matcher_args.len != 2) Common.invariant("ParseRecordSpec.put matcher did not have two arguments");
        if (!self.sameType(matcher_args[0], key_ty)) Common.invariant("ParseRecordSpec.put matcher key type differed from provided key type");

        const key_expr = try self.builder.localExpr(key_local, matcher_args[0]);
        const field_expr = try self.builder.stringExpr(self.builder.program.names.recordFieldLabelText(field.name), matcher_args[1]);
        const matcher_expr = try self.builder.localExpr(matcher_local, matcher_ty);
        const call_args = [_]Ast.ExprId{ key_expr, field_expr };
        return try self.builder.program.addExpr(.{
            .ty = matcher_fn.ret,
            .data = .{ .call_value = .{
                .callee = matcher_expr,
    ...
    fn lowerExprAtType(
        ...
        switch (expr.data) {
            .call => |call| {
                if (try self.lowerDecoderIntrinsicCallExpr(expr.ty, call, ty)) |lowered| return lowered;
    ...
    fn lowerDispatchExprAtType(
        ...
        if (lookup == null) {
            return switch (plan.result_mode) {
                ...
                .parse_from => try self.lowerStructuralParseFrom(plan, callable_mono_ty, plan_ret_ty, self, pre_lowered),
    ...
    fn dispatchTarget(
        ...
        const owner = methodOwnerFromType(&self.builder.program.types, dispatcher_ty) orelse {
            ...
            if (plan.result_mode == .parse_from and plan.result_mode.parse_from.structural_allowed) return null;
    ...
    fn methodLookupForTypeName(
        ...
        const owner = methodOwnerFromType(&self.builder.program.types, owner_ty) orelse
            Common.invariant("parse_from format type did not have a method owner");
        return self.builder.lookupMethodTargetByName(owner, method_name) orelse
            Common.invariant("checked method registry is missing parse_from format method");
    }
    ...
    fn lowerStructuralEquality(
        ...
        return switch (plan.result_mode) {
            ...
            .parse_from => Common.invariant("parse_from dispatch plan reached structural equality lowering"),
        };
    }

    fn lowerStructuralParseFrom(
        self: *BodyContext,
        plan: static_dispatch.StaticDispatchCallPlan,
        callable_mono_ty: Type.TypeId,
        ret_ty: Type.TypeId,
        ...
        const parse_from = switch (plan.result_mode) {
            .parse_from => |parse_from| parse_from,
            else => Common.invariant("non-parse_from dispatch plan reached structural parse lowering"),
        };
        if (!parse_from.structural_allowed) Common.invariant("structural parse_from dispatch plan did not permit structural parse lowering");

        const fn_data = self.builder.functionShape(callable_mono_ty, "checked structural parse_from target had a non-function type");
        const arg_tys = try self.allocator.dupe(Type.TypeId, self.builder.program.types.span(fn_data.args));
        defer self.allocator.free(arg_tys);
        if (arg_tys.len != 1 or plan.args.len != 1) Common.invariant("structural parse_from callable type must have one format argument");
            ...
            switch (self.builder.shapeContent(shape_ty)) {
                .record => |fields_span| blk: {
                    for (self.builder.program.types.fieldSpan(fields_span)) |field| {
                        if (!self.decoderFieldTypeIsSupported(field.ty)) Common.invariant("structural parse_from record field type was not supported");
                    }
                    break :blk;
                },
        ...
        const input_value = if (pre_lowered != null and pre_lowered.?.index == 0)
            pre_lowered.?.expr
        else
            try arg_ctx.lowerDispatchOperandAtType(plan.args[0], arg_tys[0]);
        const input_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), arg_tys[0]);
        const parsed = try self.lowerParseShapeFromSlot(
            shape_ty,
            try self.builder.localExpr(input_local, arg_tys[0]),
            arg_tys[0],
            ret_ty,
        );
        return try self.wrapLet(input_local, arg_tys[0], input_value, parsed, ret_ty);
    }
    ...
    const ParseRecordStateSlotInfo = struct {
    ...
    fn parseRecordStateSlotInfo(self: *BodyContext, slot_ty: Type.TypeId) ParseRecordStateSlotInfo {
        const tags_span = switch (self.builder.shapeContent(slot_ty)) {
            .tag_union => |tags| tags,
            else => Common.invariant("ParseRecordState field slot was not an internal tag union"),
        };
        const tags = self.builder.program.types.tagSpan(tags_span);
        if (tags.len != 2) Common.invariant("ParseRecordState field slot must have exactly one present and one missing tag");
        ...
        for (tags) |tag| {
            ...
            if (payloads.len == 1) {
                ...
                Common.invariant("ParseRecordState field slot had an unsupported tag shape");
            }
        }

        return .{
            .present_tag = present_tag orelse Common.invariant("ParseRecordState field slot was missing a present tag"),
            .missing_tag = missing_tag orelse Common.invariant("ParseRecordState field slot was missing a missing tag"),
            .present_payload_ty = present_payload_ty orelse Common.invariant("ParseRecordState field slot present tag was missing its payload"),
        };
    }
    ...
    fn parseRecordStatePresentExpr(
        self: *BodyContext,
        fmt_local: Ast.LocalId,
        fmt_ty: Type.TypeId,
        state_slot_ty: Type.TypeId,
    ) Allocator.Error!Ast.ExprId {
        const slot_info = self.parseRecordStateSlotInfo(state_slot_ty);
        if (!self.sameType(slot_info.present_payload_ty, fmt_ty)) Common.invariant("ParseRecordState present payload type differed from put format type");
        const fmt_expr = try self.builder.localExpr(fmt_local, fmt_ty);
        return try self.builder.program.addExpr(.{
            .ty = state_slot_ty,
            .data = .{ .tag = .{
                .name = slot_info.present_tag.name,
                .payloads = try self.builder.program.addExprSpan(&[_]Ast.ExprId{fmt_expr}),
            } },
        });
    }
    ...
    fn parseRecordFieldFromStateSlot(
        ...
        if (!self.sameType(field_try_info.ok_ty, field_ty)) Common.invariant("structural decoder field Try Ok type differed from field type");
        if (!self.sameType(field_try_info.err_ty, missing_ty)) Common.invariant("structural decoder field Try Err type differed from missing-required error type");
        const slot_info = self.parseRecordStateSlotInfo(state_slot_ty);
        ...
        const fmt_expr = try self.builder.localExpr(value_local, slot_info.present_payload_ty);
        const present_body = try self.parsePresentRecordField(field_ty, fmt_expr, slot_info.present_payload_ty, field_try_ty);

        const absent_body = if (self.typeHasBuiltinOwner(field_ty, .str)) blk: {
            break :blk try self.tryErr(field_try_ty, try self.builder.localExpr(missing_local, missing_ty));
        } else if (self.tryStrOptionalInfo(field_ty)) |info| blk: {
            const missing_field = try self.tryErrMissing(info, field_ty);
            break :blk try self.tryOk(field_try_ty, missing_field);
        } else try self.tryErr(field_try_ty, try self.builder.localExpr(missing_local, missing_ty));
    ...
    fn parsePresentRecordField(
        ...
        if (self.tryStrOptionalInfo(field_ty)) |info| {
            const outer_info = self.tryInfo(field_try_ty);
            const parse_ok_ty = try self.parseResultOkType(info.ok_payload_ty, fmt_ty);
            const parse_ret_ty = try self.tryTypeLike(field_try_ty, parse_ok_ty, outer_info.err_ty);
            const parse_expr = try self.lowerParseResultFromSlot(info.ok_payload_ty, fmt_expr, fmt_ty, parse_ret_ty);
            const parsed_local = try self.builder.program.addLocal(self.builder.symbols.fresh(), parse_ok_ty);
            const value_name = try self.builder.program.names.internRecordFieldLabel("value");
            const value_expr = try self.builder.program.addExpr(.{
                .ty = info.ok_payload_ty,
                .data = .{ .field_access = .{
                    .receiver = try self.builder.localExpr(parsed_local, parse_ok_ty),
                    .field = value_name,
                } },
            });
            const field_value = try self.tryOk(field_ty, value_expr);
            const ok_body = try self.tryOk(field_try_ty, field_value);
            return try self.sequenceTry(parse_expr, parse_ret_ty, parsed_local, ok_body, field_try_ty);
        }

        return try self.lowerParseValueFromSlot(field_ty, fmt_expr, fmt_ty, field_try_ty);
    }
