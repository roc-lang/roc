...
const Builder = struct {
        ...
        switch (template.target) {
            .hosted => {
                ...
                if (self.hostedUseNeedsTryAdapter(declared_mono_fn_ty, lower_fn_ty)) {
                    ...
                    self.program.fns.items[@intFromEnum(reservation.fn_id)].source = adapter_template;
                ...
                self.program.fns.items[@intFromEnum(reservation.fn_id)].source = hosted_fn_template;
                self.markTemplateReady(family, reservation.def, lower_fn_ty);
                return reservation.def;
            },
            .roc,
    ...
    fn hostedTryAdapterBody(
        ...
        if (source_args.len != target_args.len or source_args.len != adapter_args.len) {
            Common.invariant("hosted Try adapter arity differed from its function types");
        }

        const call_args = try self.allocator.alloc(Ast.ExprId, adapter_args.len);
        defer self.allocator.free(call_args);
        for (adapter_args, source_args, target_args, 0..) |arg, source_arg_ty, target_arg_ty, index| {
    ...
    fn errorRowInjectionExpr(
        ...
        defer self.allocator.free(branches);
        for (source_tags, 0..) |source_tag, index| {
            ...
            defer self.allocator.free(payload_exprs);
            for (source_payload_tys, target_payload_tys, 0..) |source_payload_ty, target_payload_ty, payload_index| {
    ...
    fn errorRowIsIncludedIn(self: *Builder, source_err_ty: Type.TypeId, target_err_ty: Type.TypeId) bool {
        ...
        for (self.tagUnionTags(source_err_ty)) |source_tag| {
    ...
    fn tagUnionTags(self: *Builder, ty: Type.TypeId) []const Type.Tag {
        return switch (self.shapeContent(ty)) {
