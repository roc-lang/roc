...
const Lowerer = struct {
    ...
    const WorkerPreparation = struct {
        ...
        fn prepareExpr(self: *WorkerPreparation, id: Lifted.ExprId) Common.LowerError!void {
            ...
            switch (data) {
                .static_data_candidate, .comptime_branch_taken, .comptime_exhaustiveness_failed => {
                ...
                .call_proc => |call| {
                    const callee = switch (Lifted.directCallee(call)) {
                        .local => |local| local,
                        .imported => Common.invariant("unlinked imported call reached Solved-LIR worker preparation"),
                    };
