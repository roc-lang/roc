...
const Lowerer = struct {
    ...
    fn captureLocalMatchesLocal(self: *Lowerer, local_id: Lifted.LocalId, capture_id: Lifted.LocalId) bool {
        ...
        if (local.binder != null and capture.binder != null and local.binder.? == capture.binder.?) return true;
    ...
    fn captureLocalMatchesMember(
        ...
        if (local.binder != null and member_capture.binder != null and local.binder.? == member_capture.binder.?) return true;
    ...
    fn bindPattern(self: *Lowerer, pat_id: Lifted.PatId, source: LIR.LocalId, next: LIR.CFStmtId) Common.LowerError!LIR.CFStmtId {
        ...
        return switch (pat_data.data) {
            .bind => |local| blk: {
                const target = try self.localForTyped(local, pat_ty);
                break :blk try self.assignBoxBoundary(target, source, self.result.store.getLocal(source).layout_idx, next);
            },
            .wildcard => next,
            .as => |as| blk: {
                const bound = try self.bindPattern(as.pattern, source, next);
                const target = try self.localForTyped(as.local, pat_ty);
                break :blk try self.assignBoxBoundary(target, source, self.result.store.getLocal(source).layout_idx, bound);
            },
    ...
    fn nominalPatternBackingLocal(
        ...
        return try self.addLocalForLayout(try self.layoutOfType(try self.nominalPatternBackingType(nominal_ty, inner)));
    }
    ...
    fn bindNominalPattern(
        ...
        return try self.assignNominalBoundary(backing_local, source, source_layout, bound);
    }
    ...
    fn lowerNominalPatternThen(
        ...
        return try self.assignNominalBoundary(backing_local, source, source_layout, matched);
    }
    ...
    fn mappedLocalForMatchingLocal(self: *Lowerer, local: Lifted.LocalId) Common.LowerError!?LIR.LocalId {
        ...
        for (self.local_map, 0..) |maybe_existing, raw_other| {
            ...
            if (found) |previous| {
                if (previous != existing) Common.invariant("lifted local matched multiple lowered local bindings");
            } else {
                found = existing;
            }
        }
