...
const Lowerer = struct {
    ...
    /// Frozen view of the solved type store. Lowering never appends to it,
    /// and the mutable store deliberately has no whole-slice accessors.
...
fn cloneSolvedTypeStore(allocator: std.mem.Allocator, source: *const SolvedType.Store) std.mem.Allocator.Error!SolvedType.Store {
    ...
    return .{
        .allocator = allocator,
        .vars = @TypeOf(source.vars).fromArrayList(try cloneSlice(SolvedType.Content, allocator, view.vars)),
        .spans = @TypeOf(source.spans).fromArrayList(try cloneSlice(SolvedType.TypeVarId, allocator, view.spans)),
        .fields = @TypeOf(source.fields).fromArrayList(try cloneSlice(SolvedType.Field, allocator, view.fields)),
        .tags = @TypeOf(source.tags).fromArrayList(try cloneSlice(SolvedType.Tag, allocator, view.tags)),
        .captures = @TypeOf(source.captures).fromArrayList(try cloneSlice(SolvedType.Capture, allocator, view.captures)),
        .fn_members = @TypeOf(source.fn_members).fromArrayList(try cloneSlice(SolvedType.FnMember, allocator, view.fn_members)),
        .declared_fields = @TypeOf(source.declared_fields).fromArrayList(try cloneSlice(SolvedType.DeclaredField, allocator, view.declared_fields)),
    };
}
