...
fn getCacheFile(allocator: Allocator, file_cache_path: []const u8) !?[]const u8 {
    ...
    const file = std.fs.cwd().openFile(file_cache_path, .{ .mode = .read_only }) catch |e| {
        return switch (e) {
            // SKILL_ISSUE: can't these error's be inferred?
...
fn fileExists(path: []const u8) bool {
    ...
    std.fs.cwd().access(path, .{}) catch |err| {
        file_exists = if (err == error.FileNotFound) false else true;
    };
    return file_exists;
}
...
test "Should save to XDG_CACHE_HOME if it is set for compiler cache" {
    ...
    try testing.expect(fileExists(expected_cache_dir));
}
...
test "Should save to XDG_CACHE_HOME if it is set for app cache" {
    ...
    const actual_ir = try tryToGetCanonicalizeIr(allocator, project_name, version_name, roc_code) orelse {
