...
pub fn decrefDataPtr(
    ...
    if (comptime builtin.mode == .Debug) {
        if (data_ptr % @alignOf(usize) != 0) {
            ...
            const msg = std.fmt.bufPrint(&buf, "decrefDataPtr: data_ptr=0x{x} not {d}-byte aligned", .{ data_ptr, @alignOf(usize) }) catch "decrefDataPtr: alignment error";
            roc_ops.crash(msg);
            return;
        }
    }
    ...
    if (comptime builtin.mode == .Debug) {
        if (unmasked_ptr % @alignOf(isize) != 0) {
            var buf: [128]u8 = undefined;
            const msg = std.fmt.bufPrint(&buf, "decrefDataPtr: unmasked=0x{x} (data=0x{x}) not {d}-byte aligned", .{ unmasked_ptr, data_ptr, @alignOf(isize) }) catch "decrefDataPtr: unmasked alignment error";
            roc_ops.crash(msg);
            return;
        }
    }
...
pub fn increfDataPtr(
    ...
    if (comptime builtin.mode == .Debug) {
        if (rc_addr % @alignOf(isize) != 0) {
            var buf: [128]u8 = undefined;
            const msg = std.fmt.bufPrint(&buf, "increfDataPtr: rc_addr=0x{x} (ptr=0x{x}, masked=0x{x}) is not {d}-byte aligned", .{ rc_addr, ptr, masked_ptr, @alignOf(isize) }) catch "increfDataPtr: rc_addr alignment error";
            roc_ops.crash(msg);
            return;
        }
    }
