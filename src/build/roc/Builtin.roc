		...
		Json :: {}.{
			...
			dec_from_json_number = |value|
				match Json.split_json_number_exponent(value) {
					...
					Err(NotFound) => Ok(dec_from_str(value)?)
				}
...
range_len_dec = |lower, upper, step, upper_bound| {
	...
	if !has_members {
		...
		match upper.minus_try(lower) {
			Err(Overflow) => Unknown
			Ok(distance) => match upper_bound {
				Exclusive => match (distance / step).ceiling_to_u64_try() {
					Ok(len) => Known(len)
					Err(_) => Unknown
				}
				Inclusive => match (distance / step).floor_to_u64_try() {
					Ok(base) => match base.plus_try(1) {
						Ok(len) => Known(len)
						Err(_) => Unknown
					}
					Err(_) => Unknown
