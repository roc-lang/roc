...
pub fn waitForShutdown(std_io: std.Io) void {
    ...
    // stderr not available on freestanding. The notice is best-effort: a failed
    // write must not prevent tracy from flushing its collected profiling data.
    if (comptime builtin.os.tag != .freestanding) {
        std.Io.File.stderr().writeStreamingAll(std_io, "Program ended, waiting for tracy to finish collecting data.\n") catch {};
    }
