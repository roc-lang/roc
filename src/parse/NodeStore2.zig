...
pub fn addMalformed(store: *Self, comptime T: type, reason: Diagnostic.Tag, region: Region) std.mem.Allocator.Error!T {
...
pub fn addFile(store: *Self, file: AST.File) std.mem.Allocator.Error!void {
...
pub fn addCollection(store: *Self, tag: Node.Tag, collection: AST.Collection) std.mem.Allocator.Error!AST.Collection.Idx {
...
pub fn addHeader(store: *Self, header: AST.Header) std.mem.Allocator.Error!AST.Header.Idx {
    var node = Node{
        .tag = .statement,
        .main_token = 0,
        .data = .{
...
pub fn addExposedItem(store: *Self, item: AST.ExposedItem) std.mem.Allocator.Error!AST.ExposedItem.Idx {
    var node = Node{
        .tag = .malformed,
        .main_token = 0,
        .data = .{
...
pub fn addPattern(store: *Self, pattern: AST.Pattern) std.mem.Allocator.Error!AST.Pattern.Idx {
    var node = Node{
        .tag = .statement,
        .main_token = 0,
        .data = .{
...
pub fn addExpr(store: *Self, expr: AST.Expr) std.mem.Allocator.Error!AST.Expr.Idx {
    var node = Node{
        .tag = .statement,
        .main_token = 0,
        .data = .{ .lhs = 0, .rhs = 0 },
...
/// TODO
pub fn addPatternRecordField(store: *Self, field: AST.PatternRecordField) std.mem.Allocator.Error!AST.PatternRecordField.Idx {
    var node = Node{
...
/// TODO
pub fn getPatternRecordField(store: *const Self, field: AST.PatternRecordField.Idx) AST.PatternRecordField {
...
/// TODO
pub fn addRecordField(store: *Self, field: AST.RecordField) std.mem.Allocator.Error!AST.RecordField.Idx {
    var node = Node{
        .tag = .statement,
        .main_token = 0,
        .data = .{
...
/// TODO
pub fn addTypeHeader(store: *Self, header: AST.TypeHeader) std.mem.Allocator.Error!AST.TypeHeader.Idx {
    var node = Node{
        .tag = .ty_header,
...
/// TODO
pub fn addAnnoRecordField(store: *Self, field: AST.AnnoRecordField) std.mem.Allocator.Error!AST.AnnoRecordField.Idx {
    const node = Node{
        .tag = .ty_record_field,
        .main_token = 0,
        .data = .{
...
pub fn addWhereClause(store: *Self, clause: AST.WhereClause) std.mem.Allocator.Error!AST.WhereClause.Idx {
    var node = Node{
...
/// TODO
pub fn addTypeAnno(store: *Self, anno: AST.TypeAnno) std.mem.Allocator.Error!AST.TypeAnno.Idx {
    var node = Node{
        .tag = .branch,
        .main_token = 0,
        .data = .{
...
/// TODO
pub fn getFile(store: *const Self) AST.File {
...
pub fn getCollection(store: *const Self, collection_idx: AST.Collection.Idx) AST.Collection {
...
pub fn getHeader(store: *const Self, header_idx: AST.Header.Idx) AST.Header {
...
pub fn getExposedItem(store: *const Self, exposed_item_idx: AST.ExposedItem.Idx) AST.ExposedItem {
...
pub fn getStatement(store: *const Self, statement_idx: AST.Statement.Idx) AST.Statement {
...
pub fn getPattern(store: *const Self, pattern_idx: AST.Pattern.Idx) AST.Pattern {
...
pub fn getExpr(store: *const Self, expr_idx: AST.Expr.Idx) AST.Expr {
...
pub fn getRecordField(store: *const Self, field_idx: AST.RecordField.Idx) AST.RecordField {
...
pub fn getBranch(store: *const Self, branch_idx: AST.MatchBranch.Idx) AST.MatchBranch {
...
pub fn getTypeHeader(store: *const Self, header_idx: AST.TypeHeader.Idx) !AST.TypeHeader {
    const node = store.nodes.get(@enumFromInt(@intFromEnum(header_idx)));
...
pub fn getAnnoRecordField(store: *const Self, anno_record_field_idx: AST.AnnoRecordField.Idx) !AST.AnnoRecordField {
...
pub fn getWhereClause(store: *const Self, where_clause_idx: AST.WhereClause.Idx) AST.WhereClause {
...
pub fn getTypeAnno(store: *const Self, ty_anno_idx: AST.TypeAnno.Idx) AST.TypeAnno {
...
pub fn addScratchExpr(store: *Self, idx: AST.Expr.Idx) std.mem.Allocator.Error!void {
...
pub fn exprSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.Expr.Span {
...
pub fn exprSlice(store: *const Self, span: AST.Expr.Span) []AST.Expr.Idx {
...
pub fn scratchStatementTop(store: *Self) u32 {
    return store.scratch_statements.top();
}

/// Places a new AST.Statement.Idx in the scratch.
pub fn addScratchStatement(store: *Self, idx: AST.Statement.Idx) std.mem.Allocator.Error!void {
    try store.scratch_statements.append(store.gpa, idx);
}

/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn statementSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.Statement.Span {
...
/// Should be used wherever the scratch items will not be used,
/// as in when parsing fails.
pub fn clearScratchStatementsFrom(store: *Self, start: u32) void {
...
pub fn addScratchPattern(store: *Self, idx: AST.Pattern.Idx) std.mem.Allocator.Error!void {
    try store.scratch_patterns.append(store.gpa, idx);
}

/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn patternSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.Pattern.Span {
...
/// all items in the span.
pub fn patternSlice(store: *const Self, span: AST.Pattern.Span) []AST.Pattern.Idx {
...
/// all items in the span.
pub fn patternRecordFieldSlice(store: *const Self, span: AST.PatternRecordField.Span) []AST.PatternRecordField.Idx {
...
pub fn scratchPatternRecordFieldTop(store: *Self) u32 {
    return store.scratch_pattern_record_fields.top();
}

/// Places a new AST.PatternRecordField.Idx in the scratch.
pub fn addScratchPatternRecordField(store: *Self, idx: AST.PatternRecordField.Idx) std.mem.Allocator.Error!void {
    try store.scratch_pattern_record_fields.append(store.gpa, idx);
}

/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn patternRecordFieldSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.PatternRecordField.Span {
...
/// Should be used wherever the scratch items will not be used,
/// as in when parsing fails.
pub fn clearScratchPatternRecordFieldsFrom(store: *Self, start: u32) void {
...
/// all items in the span.
pub fn recordFieldSlice(store: *const Self, span: AST.RecordField.Span) []AST.RecordField.Idx {
...
pub fn scratchRecordFieldTop(store: *Self) u32 {
    return store.scratch_record_fields.top();
}

/// Places a new AST.RecordField.Idx in the scratch.
pub fn addScratchRecordField(store: *Self, idx: AST.RecordField.Idx) std.mem.Allocator.Error!void {
    try store.scratch_record_fields.append(store.gpa, idx);
}

/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn recordFieldSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.RecordField.Span {
...
/// Should be used wherever the scratch items will not be used,
/// as in when parsing fails.
pub fn clearScratchRecordFieldsFrom(store: *Self, start: u32) void {
...
pub fn scratchTypeAnnoTop(store: *Self) u32 {
    return store.scratch_type_annos.top();
}

/// Places a new AST.TypeAnno.Idx in the scratch.
pub fn addScratchTypeAnno(store: *Self, idx: AST.TypeAnno.Idx) std.mem.Allocator.Error!void {
    try store.scratch_type_annos.append(store.gpa, idx);
}

/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn typeAnnoSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.TypeAnno.Span {
...
/// Should be used wherever the scratch items will not be used,
/// as in when parsing fails.
pub fn clearScratchTypeAnnosFrom(store: *Self, start: u32) void {
    store.scratch_type_annos.clearFrom(start);
}

/// Returns a new TypeAnno slice so that the caller can iterate through
/// all items in the span.
pub fn typeAnnoSlice(store: *const Self, span: AST.TypeAnno.Span) []AST.TypeAnno.Idx {
...
pub fn scratchAnnoRecordFieldTop(store: *Self) u32 {
    return store.scratch_anno_record_fields.top();
}

/// Places a new AST.AnnoRecordField.Idx in the scratch.
pub fn addScratchAnnoRecordField(store: *Self, idx: AST.AnnoRecordField.Idx) std.mem.Allocator.Error!void {
    try store.scratch_anno_record_fields.append(store.gpa, idx);
}

/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn annoRecordFieldSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.AnnoRecordField.Span {
...
/// Should be used wherever the scratch items will not be used,
/// as in when parsing fails.
pub fn clearScratchAnnoRecordFieldsFrom(store: *Self, start: u32) void {
...
/// all items in the span.
pub fn annoRecordFieldSlice(store: *const Self, span: AST.AnnoRecordField.Span) []AST.AnnoRecordField.Idx {
...
/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn tokenSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!Token.Span {
...
/// all items in the span.
pub fn tokenSlice(store: *const Self, span: Token.Span) []Token.Idx {
    return store.sliceFromSpan(Token.Idx, span.span);
}

/// Returns the start position for a new Span of exposedItemIdxs in scratch
pub fn scratchExposedItemTop(store: *Self) u32 {
    return store.scratch_exposed_items.top();
}

/// Places a new AST.ExposedItem.Idx in the scratch.
pub fn addScratchExposedItem(store: *Self, idx: AST.ExposedItem.Idx) std.mem.Allocator.Error!void {
    try store.scratch_exposed_items.append(store.gpa, idx);
}

/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn exposedItemSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.ExposedItem.Span {
...
/// Should be used wherever the scratch items will not be used,
/// as in when parsing fails.
pub fn clearScratchExposedItemsFrom(store: *Self, start: u32) void {
    store.scratch_exposed_items.clearFrom(start);
}

/// Returns a new ExposedItem slice so that the caller can iterate through
/// all items in the span.
pub fn exposedItemSlice(store: *const Self, span: AST.ExposedItem.Span) []AST.ExposedItem.Idx {
...
pub fn scratchWhereClauseTop(store: *Self) u32 {
    return store.scratch_where_clauses.top();
}

/// Places a new AST.WhereClause.Idx in the scratch.
pub fn addScratchWhereClause(store: *Self, idx: AST.WhereClause.Idx) std.mem.Allocator.Error!void {
    try store.scratch_where_clauses.append(store.gpa, idx);
}

/// Creates a new span starting at start.  Moves the items from scratch
/// to extra_data as appropriate.
pub fn whereClauseSpanFrom(store: *Self, start: u32) std.mem.Allocator.Error!AST.WhereClause.Span {
...
/// Should be used wherever the scratch items will not be used,
/// as in when parsing fails.
pub fn clearScratchWhereClausesFrom(store: *Self, start: u32) void {
    store.scratch_where_clauses.clearFrom(start);
}

/// Returns a new WhereClause slice so that the caller can iterate through
/// all items in the span.
pub fn whereClauseSlice(store: *const Self, span: AST.WhereClause.Span) []AST.WhereClause.Idx {
