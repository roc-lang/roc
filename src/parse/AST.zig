...
pub fn parseDiagnosticToReport(self: *AST, env: *const CommonEnv, diagnostic: Diagnostic, allocator: std.mem.Allocator, filename: []const u8) Allocator.Error!reporting.Report {
    ...
    return switch (diagnostic.tag) {
        ...
        .where_alias_cannot_have_associated => reportParseProblem(ctx, "Where Alias With Associated Items", "I was parsing a where alias, but only nominal types can have associated items.", "A where alias names a set of method constraints, so it has no associated items. Remove the associated item block.", .{ .example = "a.Sortable : where [a.compare : a -> [LessThan, Equal, GreaterThan]]" }),
        .where_alias_expected_where => reportParseProblem(ctx, "Expected Where Clause", "I was parsing a where alias declaration, and I expected `where` after the `:`.", "A where alias is declared by naming a type variable and giving it a set of method constraints.", .{ .example = "a.Sortable : where [a.compare : a -> [LessThan, Equal, GreaterThan]]" }),
...
pub const WhereClause = union(enum) {
    ...
    /// Example:
    /// ```roc
    /// elem.Sort : where [elem.order : elem -> [LessThan, Equal, GreaterThan]]
    ///
