...
root_node_idx: u32 = 0, // Root node index
tokenize_diagnostics: std.ArrayListUnmanaged(Diagnostic) = .{}, // Tokenization diagnostics
parse_diagnostics: std.ArrayListUnmanaged(Diagnostic) = .{}, // Parse diagnostics
...
pub fn deinit(self: *Ast, allocator: Allocator) void {
    ...
    self.tokenize_diagnostics.deinit(allocator);
    self.parse_diagnostics.deinit(allocator);
}

/// Convert a tokenized region to a region
pub fn tokenizedRegionToRegion(self: *const Ast, tokenized_region: Region) Region {
    // Already a Region, so this is a direct conversion
    _ = self;
    return tokenized_region;
}
...
// File structure for the root of the AST
pub const File = struct {
    header: ?Node.Idx = null,
    expressions: []Node.Idx = &.{},
    statements: Collection = .{ .span = .{ .start = 0, .len = 0 } },
    region: Region,
};
...
// Type definitions needed by parser and other modules
pub const TokenizedRegion = base.Region;
...
pub const TypeAnno = struct {
    ...
    apply: struct {
        @"fn": TypeAnno.Idx,
        args: TypeAnno.Idx,
    } = .{ .@"fn" = @enumFromInt(0), .args = @enumFromInt(0) },
};
