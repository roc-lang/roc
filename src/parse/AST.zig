...
pub fn parseDiagnosticToReport(self: *AST, env: *const CommonEnv, diagnostic: Diagnostic, allocator: std.mem.Allocator, filename: []const u8) Allocator.Error!reporting.Report {
    ...
    return switch (diagnostic.tag) {
        ...
        .optional_field_mark_before_colon => reportParseProblem(ctx, "Old Optional Field Syntax", "I was parsing a record type, and this optional field puts the `?` before the `:`.", "Optional fields are now written with the `?` after the `:`, matching `.?` access: a trailing `?` makes it optional.", .{ .example = "{ name :? Str }", .show_found = false }),
...
pub const Diagnostic = struct {
    ...
    pub const Tag = enum {
        ...
        optional_field_mark_before_colon,
