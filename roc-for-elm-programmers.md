...
- No `Tuple` module, there are no module functions like Elm's `mapFirst` `mapSecond`, `mapBoth`, so for usecases demanding those, consider using a `List`. But there is syntax support for tuples, allowing destructuring and direct access to all elements via numerical indexing (e.g Elm- `Tuple.first (5,10)`, `Tuple.second (5,10)` which are limited to two-sized tuples. Roc- `(5,10).0`, `(5,10).1`, `(5, 10, 15).2`).
