...
# CLI Parser demonstrating the record builder (applicative builder pattern)
#
...
main! = || {
	...
	config2 = parser2.run()
	msg2 = 
		\\Test 2: Three-field record builder
		\\  Result: name=${config2.name}, count=${config2.count}, verbose=${Str.inspect(config2.verbose)}
		\\
	...
	Stdout.line!(
		\\Test 3: Four-field record builder
		\\  Result: w=${config3.w}, x=${config3.x}, y=${config3.y}, z=${config3.z}
		\\
		,
	)
