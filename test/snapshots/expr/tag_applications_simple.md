...
# TYPES
~~~clojure
(expr (type "List([Err(a), Just(b), Left(c), None, Nothing, Ok(d), Right(e), Some([Ok([Just(f)]_others)]_others2), Try([Ok([Some([True]_others3)]_others4)]_others5)]_others6) where [a.try_from_str : Str -> Try(a, [InvalidStr(Str)]), b.from_numeral : Numeral -> Try(b, [InvalidNumeral(Str)]), c.from_numeral : Numeral -> Try(c, [InvalidNumeral(Str)]), d.try_from_str : Str -> Try(d, [InvalidStr(Str)]), e.from_numeral : Numeral -> Try(e, [InvalidNumeral(Str)]), f.from_numeral : Numeral -> Try(f, [InvalidNumeral(Str)])]"))
~~~
