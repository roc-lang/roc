...
**syntax_grab_bag.md:117:3:117:7:**
```roc
		3.14 => 314
```
		^^^^
...
**syntax_grab_bag.md:121:3:121:29:**
```roc
		{ foo: 1, bar: 2, ..rest } => 12->add(34)
```
		^^^^^^^^^^^^^^^^^^^^^^^^^^
...
**syntax_grab_bag.md:122:3:129:4:**
```roc
		{ # After pattern record open
			foo # After pattern record field name
				: # Before pattern record field value
...
**syntax_grab_bag.md:136:3:136:17:**
```roc
		Ok(Some(dude)) => dude
```
		^^^^^^^^^^^^^^
...
**syntax_grab_bag.md:143:36:143:37:**
```roc
main! : List(String) -> Result({}, _)
```
                                   ^
