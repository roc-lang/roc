# META
~~~ini
description=Derived == on a record must check a recursive nominal field's own is_eq method where clause like a direct comparison does (issue 11575)
type=snippet
~~~
# SOURCE
~~~roc
Expr(a) := [Leaf(a), Next(Expr(a))].{
    is_eq : Expr(a), Expr(a) -> Bool where [a.frob : a -> Bool]
    is_eq = |x, _y| match x {
        Leaf(v) => v.frob()
        Next(_) => Bool.True
    }
...
~~~
# EXPECTED
MISSING METHOD - derived_eq_recursive_nominal_component_issue_11575.md:12:5:12:11
# PROBLEMS
~~~clojure
(reports
	(report
		...
		(document
			(source-region (file "derived_eq_recursive_nominal_component_issue_11575.md") (start 12 5) (end 12 11) (annotation error) (line-text "y = x == x"))
			(line-break)
...
# TOKENS
~~~zig
UpperIdent,NoSpaceOpenRound,LowerIdent,CloseRound,OpColonEqual,OpenSquare,UpperIdent,NoSpaceOpenRound,LowerIdent,CloseRound,Comma,UpperIdent,NoSpaceOpenRound,UpperIdent,NoSpaceOpenRound,LowerIdent,CloseRound,CloseRound,CloseSquare,Dot,OpenCurly,
...
(can-ir
	(d-let
		(p-assign (ident "derived_eq_recursive_nominal_component_issue_11575.Expr.is_eq"))
		(e-lambda
			(args
				(p-assign (ident "x"))
				(p-assign (ident "_y")))
						...
						(branch
							(patterns
								(pattern (degenerate false)
									(p-applied-tag)))
							(value
								(e-dispatch-call (method "frob") (constraint-fn-var 304)
									(receiver
										(e-lookup-local
											(p-assign (ident "v"))))
									(args))))
...
(inferred-types
	(defs
		(patt (type "Expr(a), Expr(a) -> Bool where [a.frob : a -> Bool]"))
		(patt (type "{ e: Expr(Str) }"))
		(patt (type "Bool")))
	(type_decls
		(nominal (type "Expr(a)")
			(ty-header (name "Expr")
				(ty-args
					(ty-rigid-var (name "a"))))))
	(expressions
		(expr (type "Expr(a), Expr(a) -> Bool where [a.frob : a -> Bool]"))
		(expr (type "{ e: Expr(Str) }"))
		(expr (type "Bool"))))
~~~
