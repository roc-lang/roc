# META
~~~ini
description=Derived == on a tag union must check a nominal payload's own is_eq method where clause like a direct comparison does (issue 11575)
type=snippet
~~~
# SOURCE
~~~roc
...
x : [Some(Nullable(Str)), None]
x = Some(NotNull("a"))

y = x == x
~~~
# EXPECTED
MISSING METHOD - derived_eq_tag_union_nominal_component_issue_11575.md:12:5:12:11
# PROBLEMS
~~~clojure
(reports
	(report
		...
		(document
			(source-region (file "derived_eq_tag_union_nominal_component_issue_11575.md") (start 12 5) (end 12 11) (annotation error) (line-text "y = x == x"))
			(line-break)
...
CloseCurly,
CloseCurly,
LowerIdent,OpColon,OpenSquare,UpperIdent,NoSpaceOpenRound,UpperIdent,NoSpaceOpenRound,UpperIdent,CloseRound,CloseRound,Comma,UpperIdent,CloseSquare,
LowerIdent,OpAssign,UpperIdent,NoSpaceOpenRound,UpperIdent,NoSpaceOpenRound,StringStart,StringPart,StringEnd,CloseRound,CloseRound,
...
(can-ir
	(d-let
		(p-assign (ident "derived_eq_tag_union_nominal_component_issue_11575.Nullable.is_eq"))
		(e-lambda
			(args
				(p-assign (ident "x"))
				(p-assign (ident "_y")))
...
(inferred-types
	(defs
		...
		(patt (type "[None, Some(Nullable(Str))]"))
		(patt (type "Bool")))
	(type_decls
	...
	(expressions
		...
		(expr (type "[None, Some(Nullable(Str))]"))
		(expr (type "Bool"))))
~~~
