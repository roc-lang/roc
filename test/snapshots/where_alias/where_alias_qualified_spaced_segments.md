# META
~~~ini
description=A qualified where alias reference resolves when its segments are separated by spaces
type=snippet
~~~
# SOURCE
~~~roc
...
CloseCurly,
LowerIdent,OpColon,LowerIdent,OpArrow,UpperIdent,KwWhere,OpenSquare,LowerIdent,DotUpperIdent,DotUpperIdent,CloseSquare,
...
(can-ir
	(d-let
		...
		(e-lambda
			...
			(e-dispatch-call (method "to_str") (constraint-fn-var 249)
				(receiver
					(e-lookup-local
						(p-assign (ident "value"))))
				(args)))
	...
	(s-nominal-decl
		...
		(ty-record))
	(s-where-alias-decl
		(ty-header (name "where_alias_qualified_spaced_segments.Holder.Show"))
		(ty-rigid-var (name "a"))
		(where
