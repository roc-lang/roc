...
(inferred-types
	(defs
		...
		(patt (type "Num(num where [num.from_dec_digits : (List(U8), List(U8)) -> Try(num, [OutOfRange])])"))
		(patt (type "_arg -> Num(num where [num.from_dec_digits : (List(U8), List(U8)) -> Try(num, [OutOfRange])])")))
	(expressions
		...
		(expr (type "Num(num where [num.from_dec_digits : (List(U8), List(U8)) -> Try(num, [OutOfRange])])"))
		(expr (type "_arg -> Num(num where [num.from_dec_digits : (List(U8), List(U8)) -> Try(num, [OutOfRange])])"))))
~~~
