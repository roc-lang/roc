...
# PROBLEMS
                                        ┌──────────────────────────────────────┐
┌─ The local definitions is_even and ───┤ MUTUALLY RECURSIVE LOCAL DEFINITIONS │
│  is_odd are mutually recursive,       └─────────────────────────────────────┬┘
│  which isn't supported for local                                            │
│  definitions.                                                               │
│                                                                             │
│      is_even = |n| if (n == 0) Bool.True else is_odd(n - 1)                 │
│                                               ‾‾‾‾‾‾                        │
				...
				(e-if
					...
					(if-else
						(e-call (constraint-fn-var 211)
							(e-lookup-local
								(p-assign (ident "is_even")))
							(e-dispatch-call (method "minus") (constraint-fn-var 209)
								(receiver
									(e-lookup-local
										(p-assign (ident "n"))))
								(args
