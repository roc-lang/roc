...
# TYPES
~~~clojure
(expr (type "{ callback: _size -> _size2, metadata: [Ok({ permissions: List([Read, Write, Admin]_others), tags: List(Str) })]_others2, name: Str, nested: { items: List([Some(Str)][None]_others3), result: [Success({ data: List(_size3), timestamp: Str })]_others4 }, preferences: { notifications: [Email(Str)]_others5, theme: [Dark]_others6 }, scores: List(_size4), status: [Active({ since: Str })]_others7 } where [_a.from_int_digits : _arg -> _ret]"))
~~~
