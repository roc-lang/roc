...
# TYPES
~~~clojure
(expr (type "{ callback: a -> a, metadata: [Ok({ permissions: List([Read, Write, Admin]_others), tags: List(b) })]_others2, name: c, nested: { items: List([InvalidStr(Str), Some(d)][None]_others3), result: [Success({ data: List(e), timestamp: f })]_others4 }, preferences: { notifications: [Email(g)]_others5, theme: [Dark]_others6 }, scores: List(h), status: [Active({ since: i })]_others7 } where [a.from_numeral : Numeral -> Try(a, [InvalidNumeral(Str)]), b.try_from_str : Str -> Try(b, [InvalidStr(Str)]), c.try_from_str : Str -> Try(c, [InvalidStr(Str)]), d.try_from_str : Str -> Try(d, [InvalidStr(Str)]), e.from_numeral : Numeral -> Try(e, [InvalidNumeral(Str)]), f.try_from_str : Str -> Try(f, [InvalidStr(Str)]), g.try_from_str : Str -> Try(g, [InvalidStr(Str)]), h.from_numeral : Numeral -> Try(h, [InvalidNumeral(Str)]), i.try_from_str : Str -> Try(i, [InvalidStr(Str)])]"))
~~~
