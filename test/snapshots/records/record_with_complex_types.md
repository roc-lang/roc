...
# TYPES
~~~clojure
(expr @1.1-15.2 (type "{ name: Str, scores: List(Num(_size)), status: [Active({ since: Str })]_others, preferences: { theme: [Dark]_others2, notifications: [Email(Str)]_others3 }, metadata: Error, callback: _arg -> Error, nested: { items: List([Some(Str)][None]_others4), result: [Success({ data: List(Num(_size2)), timestamp: Str })]_others5 } }"))
~~~
