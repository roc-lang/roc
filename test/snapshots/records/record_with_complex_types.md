...
# TYPES
~~~clojure
(expr (type "{ callback: a -> a, metadata: [Ok({ permissions: List([Read, Write, Admin, ..]), tags: List(Str) }), ..], name: Str, nested: { items: List([Some(Str), ..[None, ..]]), result: [Success({ data: List(b), timestamp: Str }), ..] }, preferences: { notifications: [Email(Str), ..], theme: [Dark, ..] }, scores: List(c), status: [Active({ since: Str }), ..] } where [a.plus : a, d -> a, b.from_numeral : Numeral -> Try(b, [InvalidNumeral(Str)]), c.from_numeral : Numeral -> Try(c, [InvalidNumeral(Str)]), d.from_numeral : Numeral -> Try(d, [InvalidNumeral(Str)])]"))
~~~
