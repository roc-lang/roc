...
expect {
	result : Try(U8, Json.ParseErr)
	result = Json.parse("256")
	result == Err(Json.invalid_json)
}

expect {
	too_high : Try(I8, Json.ParseErr)
	too_high = Json.parse("128")

	too_low : Try(I8, Json.ParseErr)
...
expect {
	result : Try(U16, Json.ParseErr)
	result = Json.parse("65536")
	result == Err(Json.invalid_json)
}

expect {
	too_high : Try(I16, Json.ParseErr)
	too_high = Json.parse("32768")

	too_low : Try(I16, Json.ParseErr)
...
expect {
	result : Try(U32, Json.ParseErr)
	result = Json.parse("4294967296")
	result == Err(Json.invalid_json)
}

expect {
	too_high : Try(I32, Json.ParseErr)
	too_high = Json.parse("2147483648")

	too_low : Try(I32, Json.ParseErr)
...
expect {
	too_high : Try(I64, Json.ParseErr)
	too_high = Json.parse("9223372036854775808")

	too_low : Try(I64, Json.ParseErr)
