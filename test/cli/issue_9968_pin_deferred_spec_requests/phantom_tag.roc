...
# argument is a non-empty tag row (`[GetParams({})]`), threaded through a
# combinator chain with a shared type variable and consumed only at the
# type level.
app [main!] {
...
describe = |result|
	match result {
		SuccessfullyParsed({ file, files }) => {
			...
			"tag file=${file_str} files=${joined}"
		}
