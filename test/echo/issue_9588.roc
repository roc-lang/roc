# Repro for https://github.com/roc-lang/roc/issues/9588
#
# `roc test/echo/issue_9588.roc --no-cache` panics in monotype lowering:
...
# signature (here via `?`, but a re-raising `match` crashes identically).
Foo :: { x : U64 }.{
    bar : Foo -> Try(I32, [Baz, ..])
    bar = |{ x: _ }| Ok(0)
}
