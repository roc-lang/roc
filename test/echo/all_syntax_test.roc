...
# The err payload can be wrapped before early returning by matching on the
# Try and returning from the Err branch.
...
question_with_err_lambda = |strings| {
	# `.map_err` wraps the err payload, then `?` returns the Err early
	first_str = strings.first().map_err(|e| NoFirstError(e))?
	Ok(first_str)
}
