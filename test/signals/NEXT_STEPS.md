...
3. retained `HostValue` edges carry coherent app-compiled capabilities instead
   of scattered split/eq/drop/read descriptors, and
...
where it belongs. The remaining ownership-design gap is broader: retained
`HostValue` cells are still manipulated through capabilities that are produced at
the right time (app compile time, after monomorphization) but stored and invoked
as separate descriptor fields. The target design is one coherent capability per
retained edge, generated by monomorphized platform Roc closures and passed to the
type-blind host through stable ABI function pointers.

This phase is not a host-side refcount workaround and not ahead-of-time platform
glue generation. The prebuilt Zig host and `roc_platform_abi.zig` cannot know app
types. The app artifact can: platform Roc is compiled with the app, so each
...
1. **Define the capability contract.** Introduce a platform-level capability shape
   for retained erased values. It should at minimum cover split/clone, drop, and
   equality; read/accessor/hash operations can either be separate typed thunks
   attached to the same capability or explicitly documented as edge-specific
   extensions. The contract must state that `get` is split-and-replace, never a
   borrow, and that typed aliasing operations retain shared backing in typed Roc or
   backend code, not in the host.
2. **Make debug identity part of the capability.** Preserve the current type-tag
   assertion value, but attach it to the capability object rather than passing an
   independent tag beside unrelated thunks. Debug checks should prove that a value
   is only handed to the capability that produced or owns that edge.
3. **Move registry operations to capabilities.** Change host-value registry calls
   so clone/get/drop/equality are driven by the capability stored with the value or
   by the expected capability supplied by the owning edge. Avoid untagged cleanup
   paths except for strictly internal, already-proven ownership transfers.
4. **Migrate producers by edge class.** Convert `Signal.const`, `map`, `map2`,
   `combine`, `task_source`, `interval`, `Ui.state`, event payload descriptors,
   `Ui.each` key/item cells, `Ui.when` conditions, and signal-backed HTML sinks so
   each retained value is created with the same capability used for later reads,
   equality checks, clones, and drops.
5. **Audit host sequencing against the capability object.** Re-check cache
   replacement, dirty-map result cloning, post-transform input cleanup, source
   dispatch, event payload cleanup, row-scope disposal, structural splice cleanup,
   and unmount teardown. The host should sequence opaque capabilities; it should
   not reconstruct which split/drop/eq/read thunk belongs to a value.
6. **Prove both success and failure task lifetimes.** Keep the minimal wasm task
   regression that covers `Done(Str)` and `Failed(Str)` heap payloads through
   `fold_task`, `Str.to_utf8`, render, and unmount. Add capability-mismatch tests
   that fail in debug/safe builds if a value crosses to the wrong edge.
7. **Retire scratch repros once covered.** Keep useful reduced apps only if they
   are promoted into maintained regressions or documented examples. Delete
   untracked `tmp_*.roc` scratch files after their behaviours are covered by the
   capability/lifetime tests.

How we'll know this phase is complete:

- each retained host value has one owning capability object rather than scattered
  split/eq/drop/read thunks;
