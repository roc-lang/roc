...
Graph := [].{
	...
	EventNode := {
		...
		make_map_unit_i64_const = |source, value| { sources: source.sources, source_ids: [], expr: MapUnitI64Const({ source: Box.box(source), value }) }

		make_merge : EventNode, EventNode -> EventNode
		make_merge = |left, right| { sources: merge_deps(left.sources, right.sources), source_ids: [], expr: Merge({ left: Box.box(left), right: Box.box(right) }) }
	}
