...
Elem := [
	...
	Text(Str),
	TextSignal({ signal : Box(Node.SignalExpr), read_cap : HostValue.CapabilityHandle, read : Box((HostValue -> Str)) }),
	State({ binder : Node.BinderRef, initial : Box(({} -> HostValue)), cap : HostValue.CapabilityHandle, child : Box(Elem) }),
	When({ condition : Box(Node.SignalExpr), read_cap : HostValue.CapabilityHandle, read : Box((HostValue -> Bool)), when_true : Box(Elem), when_false : Box(Elem) }),
	Each(
		{
			items : Box(Node.SignalExpr),
			items_to_values : Box((HostValue -> List(HostValue))),
			item_cap : HostValue.CapabilityHandle,
			key_cap : HostValue.CapabilityHandle,
