...
Counter := { count : I64 }.{
	...
	render! = |state| {
		...
		count_label = Reactive.Signal.map(count, |n| n.to_str())

		Elem.component(
			Elem.div(
				[
					Elem.button({ on_click: dec_send, label: Reactive.Signal.const("-") }),
					Elem.label(count_label),
					Elem.button({ on_click: inc_send, label: Reactive.Signal.const("+") }),
				],
			),
			changes,
		)
...
App := { left : Counter, right : Counter }.{
	...
	local_counter = |name, initial| {
		...
		decs = Reactive.Event.map(dec_clicks, |_| -1)
		incs : Reactive.Event(I64)
		incs = Reactive.Event.map(inc_clicks, |_| 1)
		changes = Reactive.Event.merge(decs, incs)
		count = Reactive.Signal.fold(initial, changes, |current, delta| current + delta)
		count_label = Reactive.Signal.map(count, |n| n.to_str())
	...
	render_item = |mount_send, unmount_send, item| {
		...
		incs = Reactive.Event.map(inc_clicks, |_| 1)
		count : Reactive.Signal(I64)
		count = Reactive.Signal.fold(0, incs, |current, delta| current + delta)
		count_label = Reactive.Signal.map(count, |n| n.to_str())
	...
	render! = |state| {
		...
		controlled_total_label = Reactive.Signal.map(controlled_total, |n| n.to_str())
		...
		local_total_label = Reactive.Signal.map(local_total, |n| n.to_str())
		diamond_left : Reactive.Signal(I64)
		diamond_left = Reactive.Signal.map(local_total, |n| n + 1)
		diamond_right : Reactive.Signal(I64)
		diamond_right = Reactive.Signal.map(local_total, |n| n * 10)
		diamond_label =
			Reactive.Signal.map2(
				diamond_left,
				diamond_right,
		...
		merge_left = Reactive.Event.map(merge_clicks, |_| 1)
		merge_right : Reactive.Event(I64)
		merge_right = Reactive.Event.map(merge_clicks, |_| 10)
		merge_count : Reactive.Signal(I64)
		merge_count = Reactive.Signal.fold(0, Reactive.Event.merge(merge_left, merge_right), |current, delta| current + delta)
		merge_label = Reactive.Signal.map(merge_count, |n| n.to_str())

		{ sender: toggle_send, receiver: toggle_clicks } = Reactive.Event.unit_channel!()
		visible : Reactive.Signal(Bool)
		visible = Reactive.Signal.fold(True, toggle_clicks, |current, _| !current)
		...
		dyn_mount_deltas = Reactive.Event.map(dyn_mounts, |_| 1)
		dyn_unmount_deltas : Reactive.Event(I64)
		dyn_unmount_deltas = Reactive.Event.map(dyn_unmounts, |_| 1)
		dyn_mount_count : Reactive.Signal(I64)
		dyn_mount_count = Reactive.Signal.fold(0, dyn_mount_deltas, |current, delta| current + delta)
		dyn_unmount_count : Reactive.Signal(I64)
		dyn_unmount_count = Reactive.Signal.fold(0, dyn_unmount_deltas, |current, delta| current + delta)
		...
		item_mount_deltas = Reactive.Event.map(item_mounts, |_| 1)
		item_unmount_deltas : Reactive.Event(I64)
		item_unmount_deltas = Reactive.Event.map(item_unmounts, |_| 1)
		item_mount_count : Reactive.Signal(I64)
		item_mount_count = Reactive.Signal.fold(0, item_mount_deltas, |current, delta| current + delta)
		item_unmount_count : Reactive.Signal(I64)
		item_unmount_count = Reactive.Signal.fold(0, item_unmount_deltas, |current, delta| current + delta)
		...
		Elem.component(
			Elem.div(
				[
					...
					Elem.text("Merge"),
					Elem.button({ on_click: merge_send, label: Reactive.Signal.const("merge") }),
					Elem.label(merge_label),

					Elem.text("Dynamic"),
					Elem.label(Reactive.Signal.map(dyn_mount_count, |n| n.to_str())),
					Elem.label(Reactive.Signal.map(dyn_unmount_count, |n| n.to_str())),
					Elem.button({ on_click: toggle_send, label: Reactive.Signal.const("toggle") }),
					Elem.when(
						visible,
						Elem.div(
							[
					...
					Elem.text("Keyed"),
					Elem.label(Reactive.Signal.map(item_mount_count, |n| n.to_str())),
					Elem.label(Reactive.Signal.map(item_unmount_count, |n| n.to_str())),
					Elem.button({ on_click: reorder_send, label: Reactive.Signal.const("reorder") }),
					Elem.button({ on_click: remove_send, label: Reactive.Signal.const("remove") }),
