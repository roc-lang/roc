...
my @RULES = (
    ...
    # The mutable-view/refill API is deleted: a graph node's solved content is
    # read into an immutable snapshot id, never refilled into a visible id in
    # place. These deleted APIs must stay gone; graph reads go through
    # `activeTypeViewForNode`/`importMono`, whose snapshots are invalidated
    # rather than rewritten, so no Monotype payload is mutated after its id
    # becomes visible. `structural_test.zig` names them in the assertions that
    # prove they are absent from the sources it embeds.
    { category => 'deleted-mutable-monotype-refill', regex => qr/\baddMonoView\b/, allowed => { 'src/postcheck/structural_test.zig' => 1 } },
    { category => 'deleted-mutable-monotype-refill', regex => qr/\bregisterMonoView\b/, allowed => { 'src/postcheck/structural_test.zig' => 1 } },
    { category => 'deleted-mutable-monotype-refill', regex => qr/\bfillMono\b/, allowed => { 'src/postcheck/structural_test.zig' => 1 } },
    { category => 'deleted-mutable-monotype-refill', regex => qr/\bdrainDirty\b/, allowed => { 'src/postcheck/structural_test.zig' => 1 } },
    { category => 'deleted-mutable-monotype-refill', regex => qr/\breplaceGraphView\b/, allowed => { 'src/postcheck/structural_test.zig' => 1 } },
);
...
sub check_production_store_construction {
    my %allow = (
        ...
        'src/postcheck/monotype/lower.zig' => { map { $_ => 1 } qw(
            ...
            tryTypeLike appendTags closedFunctionType hostedTryTypeLike
            hostedTryAdapterSourceType defaultedEvidenceParamType
            cloneNamedTypeWithBackingAuthority lowerConstCaptureTypeContent
            lowerConstCaptureTypeInner
        ) },
