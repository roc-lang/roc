...
Installing the official LLVM's prebuilt binaries is not enough for the `llvm-sys` crate that Roc depends on. Instead:
...
1. In powershell, set the `LLVM_SYS_130_PREFIX` environment variable 
    ```
    $Env:LLVM_SYS_130_PREFIX="C:\Users\anton\Downloads\LLVM-13.0.0-win64"
    ```
    (check [here](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_environment_variables?view=powershell-7.2#saving-changes-to-environment-variables) to make this a permanent environment variable):
    To have easy access to llvm binaries (like llc), set
    ```
...
Once all that was done, `cargo check` and `cargo build` should run successfully!
