...
mod test_peg_grammar {
  use roc_highlight::tokenizer::{Token, tokenize};
  ...
  fn test_platform_header_1() {
      ...
      let tokens = tokenize( r#"platform "rtfeldman/blah" requires {} { main : {} } exposes [] packages {} imports [] provides [] effects fx.Blah {}"#);
  ...
  fn test_platform_header_2() {
      ...
      let tokens = tokenize( r#"platform "examples/cli"
  ...
  fn test_annotated_def() {
    let tokens = tokenize( r#"test1 : Bool
test1 =
    example1 == [ 2, 4 ]"#);
  ...
  #[test]
  fn test_when_1() {
    let tokens = tokenize( r#"when list is
    Cons _ rest ->
        1 + len rest
  ...
  fn test_when_in_defs() {
    let tokens = tokenize(r#"fromBytes = \bytes ->
    when bytes is
        Ok v -> v
  "#);
  ...
  #[test]
  fn test_pizza_1() {
    let tokens = tokenize(r#"closure = \_ ->
    Task.succeed {}
        |> Task.map (\_ -> x)"#);
  ...
  fn test_quicksort_help() {
    let tokens = tokenize(r#"quicksortHelp = \list, low, high ->
    if low < high then
  ...
  #[test]
  fn test_pizza_line() {
    let tokens = tokenize(r#"unoptimized
        |> Num.toStr
        |> Task.putLine"#);
  ...
  fn test_defs_w_apply() {
    let tokens = tokenize(r#"unoptimized = eval e
...
balance = \color ->
  ...
  fn test_nested_apply() {
    let tokens = tokenize(r#"after = \effect ->
    Effect.after
        transform a
  ...
  fn test_record_type_def() {
    let tokens = tokenize(r#"Model position :
    {
        evaluated : Set,
    }"#);
  ...
  fn test_backpass_in_def_2() {
    let tokens = tokenize(r#"with = \path ->
    handle <- withOpen
