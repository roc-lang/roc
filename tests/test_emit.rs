...
mod test_crane {
    ...
    macro_rules! assert_evals_to {
        ($src:expr, $expected:expr, $ty:ty) => {
            ...
            // Populate Procs and Subs, and get the monomorphized Expr from the canonical Expr
            let mut procs = MutMap::default();
            let mono_expr = Expr::new(&arena, &subs, expr, &mut procs);

            // Run Cranelift tests in their own scope
            {
            ...
            // Now run LLVM tests in their own scope
            {
