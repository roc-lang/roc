...
mod test_format {
    ...
    #[test]
    fn ground_true() {
        let (_sub, term) = boolean_unification::unify(&Ground(true), vec![].iter());
        assert_eq!(boolean_unification::solve(&term), Some(true));
    }

    #[test]
    fn ground_false() {
        let (_sub, term) = boolean_unification::unify(&Ground(false), vec![].iter());
    ...
    fn disjunction() {
        ...
        let (_sub, term) = boolean_unification::unify(&input, vars.iter());
        assert_eq!(boolean_unification::solve(&term), Some(true));
    }
}
