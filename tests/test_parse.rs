...
mod test_parser {
    ...
    use roc::parse::ast::{Attempting, Lossless};
    ...
    /// Wrap a value and str in a Lossless
    fn ll<'a, T>(value: T, raw: &'a str) -> Lossless<'a, T> {
        Lossless { value, raw }
    }
    ...
    fn expect_parsed_str(input: &str, expected: &str) {
        assert_parses_to(expected, Str(ll(input.into(), input.clone())));
    }
    ...
    //             "#
    //         );
    //     assert_parses_to(input,
    //         Str(ll(r#"abcd\(efg)hij"#.into(), input)),
    //     );
    // }
