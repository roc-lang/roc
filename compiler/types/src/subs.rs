...
pub trait ContentVisitor<'a> {
    fn subs(&self) -> &'a Subs;

    fn insert_recursion_var(&mut self, var: Variable);
    fn seen_recursion_var(&self, var: Variable) -> bool;

    /// Visits the `content` for `var` and returns whether or not descend into `content` further.
    /// Don't call this directly; `visit` will call this.
    fn handle_content(&mut self, var: Variable, content: &Content) -> bool;

    /// Entry point to the visitor; descends the content of a variable until its end, or a cycle.
    /// Attach handlers for different content using `handle_content`. You likely don't want to call
    /// `visit` directly, as it uses an iterative rather than recursive visitor.
    fn visit(&mut self, var: Variable) {
        ...
        while let Some(var) = stack.pop() {
            if self.seen_recursion_var(var) {
                continue;
            }

            let content = subs.get_content_without_compacting(var);

            if !self.handle_content(var, content) {
                continue;
            }
            ...
            match content {
                ...
                Structure(flat_type) => match flat_type {
                    ...
                    RecursiveTagUnion(rec_var, tags, ext_var) => {
                        self.insert_recursion_var(*rec_var);
                        for i in tags.variables() {
                            push_var_slice!(subs[i]);
                        }
                        stack.push(*ext_var);
