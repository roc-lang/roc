...
fn main() {
    ...
    // copy over files as a workaround for https://github.com/ziglang/zig/issues/3254
    let zig_build_file = "build.zig";
    let zig_src_dir = "src";
    fs::copy(bitcode_path.join(zig_build_file), out_dir_path.join(zig_build_file)).expect("Failed to copy zig build file.");
    ...
    fs_extra::dir::copy(
        bitcode_path.join(zig_src_dir),
        out_dir_path.join(zig_src_dir),
        &copy_options
    ).expect("Failed to copy zig src dir."); 
    ...
    // LLVM .bc FILES
    generate_bc_file(&out_dir_path, "ir", "builtins-host");

    if !DEBUG {
        generate_bc_file(&out_dir_path, "ir-wasm32", "builtins-wasm32");
    }

    generate_bc_file(&out_dir_path, "ir-i386", "builtins-i386");

    generate_bc_file(&out_dir_path, "ir-x86_64", "builtins-x86_64");
...
fn generate_object_file(
    ...
    let dest_obj_path = output_path.join(object_file_name);
    ...
    let src_obj_path = output_path.join(object_file_name);
...
fn generate_bc_file(output_path: &Path, zig_object: &str, file_name: &str) {
    let mut ll_path = output_path.join(file_name);
    ...
    let mut bc_path = output_path.join(file_name);
