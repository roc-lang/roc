                    ...
                    match closure_layout.layout_for_member(proc_name) {
                        ...
                        ClosureRepresentation::Other(layout) => match layout {
                            Layout::Struct(field_layouts) => {
                                ...
                                } else if field_layouts.is_empty() {
                                    // do nothing
                                } else {
        ...
        RunLowLevel { op, args, ret_var } => {
            ...
            match op {
                ListMap => {
                    ...
                    let list_symbol = arg_symbols[0];
                    let closure_data_symbol = arg_symbols[1];

                    let closure_data_layout = return_on_layout_error!(
                        env,
                        layout_cache.from_var(env.arena, args[1].0, env.subs)
                    );

                    let arena = env.arena;

                    match closure_data_layout {
                        Layout::Closure(_, lambda_set, _) => lowlevel_match_on_lambda_set(
                            env,
                            lambda_set,
                            closure_data_symbol,
                            |top_level_function, closure_data| self::Call {
                                call_type: CallType::LowLevel { op },
                                arguments: arena.alloc([list_symbol, top_level_function]),
                            },
                            layout,
                            assigned,
                            hole,
...
fn reuse_function_symbol<'a>(
    ...
    match procs.partial_procs.get(&original) {
        ...
        Some(partial_proc) => {
            ...
            match res_layout {
                Ok(Layout::Closure(argument_layouts, lambda_set, ret_layout)) if captures => {
                    ...
                    let function_ptr_layout = TopLevelFunctionLayout::from_layout(
                        env.arena,
                        lambda_set.extend_function_layout(env.arena, argument_layouts, ret_layout),
                    );
    ...
    F: Fn(Symbol, Symbol) -> Call<'a> + Copy,
{
    match lambda_set.runtime_representation() {
        ...
        Layout::Struct(_) => {
            ...
            Stmt::Let(
                assigned,
                Expr::Call(to_lowlevel_call(function_symbol, closure_data_symbol)),
                return_layout,
                env.arena.alloc(hole),
            )
        }
