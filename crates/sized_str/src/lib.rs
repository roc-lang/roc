            ...
            ($ptr:expr) => {{
                #[cfg(target_arch = "aarch64")]
                {
                    ...
                    if aarch64::vmaxvq_u32(equality_reg) != 0 {
                        ...
                        // we have subtracted this from NUM_LANES, we'll end up with the first lane that matched,
                        // which is what we want this function to do.)
                        let lane_index =
                            NUM_LANES - (aarch64::vmaxvq_u32(aarch64::vandq_u32(equality_reg, answer_mask)) as usize) - 1;
