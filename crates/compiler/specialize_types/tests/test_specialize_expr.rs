...
mod specialize_types {
    ...
    use roc_specialize_types::{
        DebugInfo, MonoCache, MonoExpr, MonoTypes, RecordFieldIds, TupleElemIds,
    };
    ...
    fn infer_eq_help(src: &str) -> Result<(String, String, MonoExpr, String), std::io::Error> {
        let (
            LoadedModule {
                module_id: home,
                mut can_problems,
        ...
        let (symbol, variable) = exposed_to_host.into_iter().next().unwrap();
        ...
        let type_str = name_and_print_var(variable, subs, home, &interns, DebugPrint::NOTHING);

        Ok((type_problems, can_problems, mono_expr, type_str))
    }

    fn specializes_to(src: &str, expected_expr: MonoExpr, expected_type_str: &str) {
        let (_, can_problems, actual_expr, actual_type_str) = infer_eq_help(src).unwrap();
        ...
        assert_eq!(
            (expected_expr, expected_type_str),
            (actual_expr, &actual_type_str)
        );
    }
    ...
    #[test]
    fn str_literal_solo() {
        specializes_to("\"test\"", MonoExpr::Str);
    }
