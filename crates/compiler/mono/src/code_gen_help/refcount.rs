...
use crate::intern::LocalInterner;
use crate::ir::{
...
pub fn refcount_generic<'a>(
    ...
    debug_assert!(is_rc_implemented_yet(root.interner, &layout));
    ...
    match layout {
        ...
        Layout::LambdaSet(lambda_set) => {
            let runtime_layout = lambda_set.runtime_representation(root.interner);
    ...
    let then_stmt = {
        ...
        let alloc_addr_stmt = {
            ...
            let alignment_expr = Expr::Literal(Literal::Int(
                (layout.alignment_bytes(root.interner, root.target_info) as i128).to_ne_bytes(),
            ));
...
pub fn is_rc_implemented_yet<'a>(
...
fn refcount_list<'a>(
    ...
    let rc_ptr = root.create_symbol(ident_ids, "rc_ptr");
    let alignment = layout.alignment_bytes(root.interner, root.target_info);
...
fn refcount_list_elems<'a>(
    ...
    let elem_size_expr = Expr::Literal(Literal::Int(
        (elem_layout.stack_size(root.interner, root.target_info) as i128).to_ne_bytes(),
    ));
...
fn refcount_struct<'a>(
    ...
    for (i, field_layout) in field_layouts.iter().enumerate().rev() {
        if field_layout.contains_refcounted(root.interner) {
...
fn refcount_union_rec<'a>(
    ...
    let rc_structure_stmt = {
        ...
        let alignment =
            Layout::Union(union_layout).alignment_bytes(root.interner, root.target_info);
        let ret_stmt = rc_return_stmt(root, ident_ids, ctx);
