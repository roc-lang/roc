...
pub fn run_toplevel_expects<'a, W: std::io::Write>(
    ...
    // TODO fix the "variables" type here.. it's not RocList<u8>
    type RocExpectFailed = unsafe extern "C" fn(&mut RocStr, &mut RocStr, &mut RocList<u8>);

    let roc_expect_failed: libloading::Symbol<RocExpectFailed> = unsafe {
        lib.get("roc_expect_failed".as_bytes())
            .unwrap_or_else(|_| panic!("Unable to find roc_expect_failed symbol"))
    };
