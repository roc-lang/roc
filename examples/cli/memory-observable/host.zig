...
pub export fn malloc(size: usize) callconv(.C) ?*c_void {
    ...
    stdout.print("\x1B[36m{} | \x1B[39m The program just allocated {} bytes. Hello, ETE!\n", .{startNs, size}) catch unreachable;
...
pub export fn free(c_ptr: *u128) callconv(.C) void {
    ...
    stdout.print("\x1B[36m{} | \x1B[39m The program just deallocated some bytes. Hello again, ETE!\n", .{startNs}) catch unreachable;
}
