...
impl Focus {
    ...
    pub fn advance(&mut self, root: &RocElem) {
        if self.focused.is_null() {
            // Nothing was focused in the first place, so use the first focusable Elem from the root.
            if let Some((new_ptr, new_ancestors)) = Self::next_focusable(root, None, None) {
        ...
        while let Some((ancestor_ptr, index)) = self.focused_ancestors.pop() {
            ...
            if let Some((new_ptr, new_ancestors)) =
                Self::next_focusable(focused, Some(ancestor), Some(index))
            {
                debug_assert!(
                    !new_ptr.is_null(),
    ...
    fn next_focusable(
        ...
        match elem.tag() {
            ...
            Row | Col => {
                ...
                for child in iter {
                    if let Some(focused) = Self::next_focusable(child, ancestor, None) {
                        return Some(focused);
                    }
                }
...
fn next_focus_text_root() {
    let mut focus = Focus::default();
    let root = RocElem::text(""); // TODO make a constructor like this for tests!
