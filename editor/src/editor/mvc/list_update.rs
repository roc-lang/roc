...
pub fn add_blank_child(new_child_index:usize, ed_model: &mut EdModel) -> EdResult<InputOutcome> {
    let NodeContext {
        old_caret_pos,
        curr_mark_node_id,
        curr_mark_node: _,
    ...
    match list_ast_node {
        Expr2::List { elem_var, elems } => {
            ...
            let new_ast_child_index = // TODO filter everything from nested marknode that has list_ast_node as ast_node and count that;
