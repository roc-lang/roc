...
use bumpalo::Bump;
use crate::{
    lang::{ast::Expr2, expr::Env, pool::{PoolStr, NodeId}},
};
...
pub fn expr2_to_markup<'a, 'b>(
    ...
    match expr2 {
        ...
        Expr2::List { elems, .. } => {
            ...
            children.push(new_markup("[ ".to_string(), node_id, HighlightStyle::Bracket));
            ...
            for (idx, node_id) in elems.iter_node_ids().enumerate() {
                ...
                if idx + 1 < elems.len() {
                    children.push(new_markup(", ".to_string(), node_id, HighlightStyle::Operator));
                }
            }
            children.push(new_markup("] ".to_string(), node_id, HighlightStyle::Bracket));
        ...
        Expr2::Record { fields, .. } => {
            ...
            for (idx, field_node_id) in fields.iter_node_ids().enumerate() {
                ...
                children.push(new_markup(field_name.to_string(), node_id, HighlightStyle::RecordField));
                children.push(new_markup(": ".to_string(), node_id, HighlightStyle::Operator));
