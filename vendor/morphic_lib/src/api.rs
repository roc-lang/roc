...
forward_trait! {
    ...
    pub trait ExprContext {
        ...
        ///
        /// Returning from the resulting join point after jumping to it will yield control to the
        /// parent of the block in which the join point was defined.
        ///
        /// This function allows you to forward-declare a join point without specifying its body,
        /// which makes it possible to define recursive and mutually recursive joint points.
        /// However, you must eventually populate the body of each joint point via a call to
        /// `define_join_point`.
        ///
        /// The returned ValueId is the ValueId of the *argument* to the join point, which is in
        /// scope within the body of the join point.
        fn declare_join_point(
            &mut self,
            block: BlockId,
            arg_type: TypeId,
            ret_type: TypeId,
        ...
        /// Define a join point.
        ///
        /// Before a join point can be defined, it must be declared via a call to
        /// `declare_join_point`.
        fn define_join_point(&mut self, join_point: JoinPointId, body: BlockExpr) -> Result<()>;

        /// Add a jump to a join point.
        ///
        /// The rest of the API assumes that every code path produces a `ValueId`. Join points do
        /// not fit this model neatly because jumping to a join point yields control to outside of
        /// the current expression. Hence, this function returns a dummy unconstrained `ValueId`,
        /// which can be used as if the jump were a regular expression producing a value of type
        ...
        ///
        /// This is useful in combination with join points, because returning from a join point
        /// always yields control to the parent of the enclosing block in which the join point was
        /// defined. Without sub-blocks, there would be no way to reduce the scope of a join point
        /// within a block.
...
impl ExprContext for ExprBuilder {
    ...
    fn add_call(
        ...
        self.check_bid(block)?;
        self.check_vid(arg)?;
        match self.callee_spec_vars.entry(callee_spec_var.into()) {
    ...
    fn add_update_write_only(
        ...
        self.check_bid(block)?;
        self.check_vid(heap_cell)?;
        if !self.update_mode_vars.insert(update_mode_var.into()) {
            return Err(ErrorKind::DuplicateUpdateModeVar(update_mode_var.into()).into());
        }
        Ok(ValueId(self.vid_gen.next()))
    }
...
impl FuncDefBuilder {
    ...
    pub fn build(mut self, arg_type: TypeId, ret_type: TypeId, root: BlockExpr) -> Result<FuncDef> {
        ...
        Ok(FuncDef {
            callee_spec_vars: self.expr_builder.callee_spec_vars,
            update_mode_vars: self.expr_builder.update_mode_vars,
        })
    }
}
...
impl FuncSpecSolutions {
    pub fn callee_spec(&self, var: CalleeSpecVar) -> Result<FuncSpec> {
        ...
        match self.func_def.callee_spec_vars.get(&var.into()) {
            Some(spec) => Ok(*spec),
    ...
    pub fn update_mode(&self, var: UpdateModeVar) -> Result<UpdateMode> {
        // TODO: The clone here is unnecessary -- avoid it!
        // (might require something like a transmute)
        if !self.func_def.update_mode_vars.contains(&var.into()) {
            return Err(ErrorKind::UpdateModeVarNotFound(var.into()).into());
        }
...
impl ConstSolutions {
    pub fn callee_spec(&self, var: CalleeSpecVar) -> Result<FuncSpec> {
        // TODO: The clone here is unnecessary -- avoid it!
        // (might require something like a transmute)
        match self.const_def.callee_spec_vars.get(&var.into()) {
            Some(spec) => Ok(*spec),
            None => Err(ErrorKind::CalleeSpecVarNotFound(var.into()).into()),
        }
    }
    ...
    pub fn update_mode(&self, var: UpdateModeVar) -> Result<UpdateMode> {
        // TODO: The clone here is unnecessary -- avoid it!
        // (might require something like a transmute)
        if !self.const_def.update_mode_vars.contains(&var.into()) {
            return Err(ErrorKind::UpdateModeVarNotFound(var.into()).into());
        }
        Ok(UpdateMode::Immutable)
    }
            ...
            .map(|(mod_name, mod_def)| {
                let mod_sols = ModSolutions {
                    ...
                    consts: mod_def
                        .const_defs
                        .into_iter()
                        .map(|(const_name, const_def)| (const_name, ConstSolutions { const_def }))
                        .collect(),
                };
                (mod_name, mod_sols)
            })
