**NOTE:** This is heavily WIP! This line will be removed when the langref is considered complete, and at that point, missing sections and outdated information will be considered bugs. If you'd like to contribute to this (other than minor corrections of obvious mistakes), **definitely** mention it in the [langref Zulip channel](https://roc.zulipchat.com/#narrow/channel/316715-contributing/topic/language.20reference/with/564909757) before getting started on anything!

# Language Reference

This is Roc's language reference (or _langref_ for short).

If you're looking for a beginner tutorial, check out [roc-lang.org/tutorial](https://roc-lang.org/tutorial) instead; the langref is a detailed reference intended for experienced Roc programmers.

## Outline

- [Expressions](expressions)
  - [Values](expressions#values)
    - [Reference Counting](expressions#reference-counting)
    - [Reference Cycles](expressions#reference-cycles)
    - [Opportunistic Mutation](expressions#opportunistic-mutation)
  - [Block Expressions](expressions#block-expressions)
- [Statements](statements)
  - [`=` (assignment)](statements#assignment)
  - [`import`](statements#import)
  - [`expect`](statements#expect)
  - [`return`](statements#return)
  - [`break`](statements#break)
  - [`continue`](statements#continue)
  - [Block Statements](statements#block-statements)
- [Conditionals](if-else)
  - [`if`](if-else#if-and-else)
  - [`else`](if-else#if-and-else)
    - [`else if`](if-else#else-if)
  - [`and`](if-else#and--or)
  - [`or`](if-else#and--or)
  - [`match`](pattern-matching#match)
- [Pattern Matching](pattern-matching)
  - [`match`](pattern-matching#match)
    - [Branch Alternatives](pattern-matching#branch-alternatives)
    - [`if` Guards on Branches](pattern-matching#if-guards-on-branches)
  - [Exhaustiveness](pattern-matching#exhaustiveness)
    - [Catch-all Patterns (`_`)](pattern-matching#underscore)
  - [Destructuring](pattern-matching#destructuring)
    - [Destructuring Assignments (with `=`)](pattern-matching#destructuring-assignments-with-)
- [Functions](functions)
  - [Pure Functions](functions#pure-functions)
  - [Effectful Functions](functions#effectful-functions)
    - [`!` suffix](functions#-suffix-in-function-names)
  - [Recursive Functions](functions#recursive-functions)
    - [Self-recursive Functions](functions#self-recursive-functions)
    - [Mutually Recursive Functions](functions#mutually-recursive-functions)
  - [Tail Calls](functions#tail-calls)
    - [Self-Tail Calls](functions#self-tail-calls)
    - [Tail-Call Optimization](functions#tail-call-optimization)
      - [Modulo Cons](functions#modulo-cons)
- [Naming](naming)
  - [Rules](naming#rules)
    - [Lowercase Names](naming#lowercase-names)
    - [Uppercase Names](naming#uppercase-names)
  - [Unused Names](naming#unused-names)
  - [Shadowing](naming#shadowing)
  - [Constants](naming#constants)
  - [Variables (with `var`)](naming#variables-with-var)
    - [`var` keyword](naming#var-keyword)
    - [`$` prefix](naming#-prefix)
  - [Type Variables](naming#type-variables)
  - [Type Aliases](naming#type-aliases)
    - [Parameterized Type Aliases](naming#parameterized-type-aliases)
  - [Module Names](naming#module-names)
  - [`as`](naming#as)
- [Modules](modules)
  - [Type Modules](modules#type-modules)
  - [Imports](modules#import-statements)
  - [Package Modules](modules#package-modules)
  - [Platform Modules](modules#platform-modules)
  - [Application Modules](modules#application-modules)
- [Platforms](platforms)
  - [Applications](platforms#applications)
  - [Domain-specific functionality](platforms#domain-specific-functionality)
  - [Platform scope](platforms#platform-scope)
    - [Ecosystem benefits](platforms#ecosystem-benefits)
    - [Security benefits](platforms#security-benefits)
    - [Performance benefits](platforms#performance-benefits)
  - [How platforms are implemented](platforms#how-platforms-are-implemented)
    - [Host](platforms#host)
    - [Memory management](platforms#memory-management)
    - [Program start](platforms#program-start)
- [Types](types)
  - [Type Annotations](types#type-annotations)
    - [Where Clauses](types#where-clauses)
  - [Nominal Types](types#nominal-types)
    - [Opaque Nominal Types](types#opaque-nominal-types)
  - [Structural Types](types#structural-types)
  - [Type Aliases](types#type-aliases)
- [Static Dispatch](static-dispatch)
  - [Methods](static-dispatch#methods)
  - [Compiler-Derived Methods](static-dispatch#compiler-derived-methods)
  - [Where Clauses](static-dispatch#where-clauses)
  - [Aliases](static-dispatch#aliases)
- [Operators](operators)
  - [Desugaring](operators#desugaring)
  - [Binary Infix Operations](operators#binary-infix-operations)
    - [And](operators#and)
    - [Or](operators#or)
    - [Arithmetic Operators](operators#arithmetic-operators)
    - [Comparison Operators](operators#comparison-operators)
    - [Range Operators](operators#range-operators)
    - [`??` (default value on `Err`)](operators#-default-value-on-err)
  - [Unary Prefix Operators](operators#unary-prefix-operators)
    - [`-` (`.negate()`)](operators#--negate)
    - [`!` (`.not()`)](operators#-not)
  - [Unary Postfix Operators](operators#unary-postfix-operators)
    - [`?` (unwrap if `Ok`; early `return` if `Err`)](operators#-unwrap-if-ok-early-return-if-err)
    - [`[…]` (subscript operator)](operators#…-subscript-operator)
- [Records](records)
  - [Fields](records#fields)
  - [Structural Records](records#structural-records)
  - [Optional Fields](records#optional-fields)
  - [Nominal Records](records#nominal-records)
  - [Defaulted Fields](records#defaulted-fields)
  - [The Empty Record (`{}`)](records#empty-record)
- [Tuples](tuples)
  - [Tuple Literals](tuples#tuple-literals)
  - [Accessing Tuple Elements](tuples#accessing-tuple-elements)
  - [Tuple Types](tuples#tuple-types)
  - [Destructuring Tuples](tuples#destructuring-tuples)
  - [Tuples vs Records](tuples#tuples-vs-records)
- [Tag Unions](tag-unions)
  - [Structural Tag Unions](tag-unions#structural-tag-unions)
  - [Nominal Tag Unions](tag-unions#nominal-tag-unions)
- [Loops](loops)
  - [`for` Loops](loops#for-loops)
  - [`while` Loop](loops#while-loops)
  - [Infinite Loops](loops#infinite-loops)
- [Iterators](iterators)
  - [Transforming Iterators](iterators#transforming-iterators)
  - [Laziness](iterators#laziness)
  - [Custom Iterators](iterators#custom-iterators)
- [Numbers](numbers)
  - [Number Literals](numbers#number-literals)
  - [Builtin Number Types](numbers#builtin-number-types)
  - [Ranges](numbers#ranges)
  - [Custom Number Types](numbers#custom-number-types)
- [Strings](strings)
  - [Unicode](strings#unicode)
  - [String Normalization](strings#string-normalization)
  - [UTF-8](strings#utf-8)
- [Dictionaries and Sets](dictionaries-and-sets)
  - [Dictionaries](dictionaries-and-sets#dictionaries)
  - [Sets](dictionaries-and-sets#sets)
  - [Keys and Hashing](dictionaries-and-sets#keys-and-hashing)
- [Parsers](parsers)
  - [Parsing JSON](parsers#parsing-json)
  - [Which Types Can Be Parsed](parsers#which-types-can-be-parsed)
  - [Parsers at Compile Time](parsers#parsers-at-compile-time)
- [Compile-Time Evaluation](compile-time)
  - [Which Expressions Get Evaluated?](compile-time#which-expressions-get-evaluated)
  - [Performance](compile-time#performance)
- [Comments](comments-and-docs)
  - [Single-line Comments](comments-and-docs#single-line-comments)
  - [Doc Comments](comments-and-docs#doc-comments)
  - [Generating Docs with `roc docs`](comments-and-docs#generating-docs-with-roc-docs)
- [Packages](packages)
  - [Depending on Packages](packages#depending-on-packages)
  - [Shorthands](packages#shorthands)
